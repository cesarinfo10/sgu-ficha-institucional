...
<table cellpadding="2" cellspacing="1" border="0" bgcolor="#ffffff" class="tabla" style='margin-top: 5px'>
  ...
  <tr>
    ...
    <td class='celdaValorAttr'><select name='cat_ocupacional' class='filtro' required><option value=''>-- Seleccione --</option><?php echo(select($ACTIVIDADES,$_REQUEST['cat_ocupacional'])); ?></select></td>
  ...
  <tr>
    ...
    <td class='celdaValorAttr' colspan='2'>
      $<input type='text' size="9" name='ing_liq_mensual_prom' value="<?php echo($_REQUEST['ing_liq_mensual_prom']); ?>" class='montos' onKeyUp="puntitos(this,this.value.charAt(this.value.length-1),this.name);" required>
<?php
if ($forma <> 'editar') { 
