...
$ANO_ANT = $ANO - 1;
//$SQL_mat_ant = "SELECT id_alumno FROM matriculas WHERE ano=$ANO_ANT";
//$condicion = "WHERE true AND (m.id_alumno IS NULL) AND a.id IN ($SQL_mat_ant)  ";

$SQL_mat_sig = "SELECT id_alumno FROM finanzas.contratos WHERE ano=$ANO_MATRICULA AND estado IS NOT NULL AND id_alumno IS NOT NULL";
$condicion = "WHERE true AND (m.id_alumno IS NOT NULL) AND a.id NOT IN ($SQL_mat_sig)  ";
...
$SQL_alumnos = "SELECT a.id,a.rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,c.alias||'-'||a.jornada AS carrera,
                ...
                LEFT JOIN matriculas AS m ON (m.id_alumno=a.id AND semestre=2 AND ano=2014)
                $condicion
                ORDER BY nombre 
                $limite_reg
                OFFSET $reg_inicio;";
