...
function cuentaRegistroCapacitaciones($ano, $id_asiscapac_origen, 
                                  ...
                                  $fecha_termino) {
  $ss = "
      select count(*) as cuenta from asiscapac_usuarios_capacitaciones
    where
    ano = $ano 
...
$id_asiscapac_estado = $_REQUEST['id_asiscapac_estado'];
    ...
    */
    ...
    if ($puedeSeguir) {
      ...
      if (consulta_dml($SQL) > 0) {
        ...
        //echo(js("location='$enlbase=capac_usuario_buscar&ano=$ano&id_origen=$id_origen&id_estado_check=$id_estado_check&id_campo_actividades=$id_campo_actividades';"));
...
*/
$regimenes = consulta_sql("SELECT id,nombre FROM regimenes where id in ('PRE','POST-G');");
...
-->        
    ...
    <tr>
      ...
      <td class="celdaFiltro">
          ...
          <?php
            $puedeMostrar = false;
            if ($id_asiscapac_estado == "1") { //PROGRAMADA
              $puedeMostrar = true;
            } 
            if ($id_asiscapac_estado == "2") { //EJECUTADA
              $puedeMostrar = true;
            } 
            if ($puedeMostrar) {
          ?>
          ...
          ?>
<!--          <input type="button" name="volver" value="Volver"  style='font-size: 9pt' onClick="window.location.href='<?php echo($enlbase); ?>=capac_usuario_buscar&ano=<?php echo($ano); ?>&id_origen=<?php echo($id_origen); ?>&id_estado_check=<?php echo($id_estado_check); ?>&id_campo_actividades=<?php echo($id_campo_actividades); ?>'"> -->
