...
$depositado     = $_REQUEST['depositado'];
$fecha_deposito = $_REQUEST['fecha_deposito'];

$protestado     = $_REQUEST['protestado'];
$fecha_protesto = $_REQUEST['fecha_protesto'];
$fecha_pago     = $_REQUEST['fecha_pago'];

if ($_REQUEST['guardar'] == "Guardar") {
	if ($depositado == "Si") {
		$SQLupd_cheque = "UPDATE finanzas.cheques SET depositado=true,fecha_deposito='$fecha_deposito'::date WHERE id=$id_cheque";
	}
	if ($protestado == "Si") {
		$SQLupd_cheque = "UPDATE finanzas.cheques SET protestado=true,fecha_protesto='$fecha_protesto'::date WHERE id=$id_cheque";
	}
	if ($protestado == "No") {
		$SQLupd_cheque = "UPDATE finanzas.cheques SET protestado=false,fecha_pago='$fecha_pago'::date WHERE id=$id_cheque";
	}
	if (consulta_dml($SQLupd_cheque) > 0) {
		echo(msje_js("Se ha guardado el cambio de estado"));
	} else {
		echo(msje_js("ERROR: No se ha podido guardar el cambio de estado"));
	}
	echo(js("parent.jQuery.fancybox.close();"));
	exit;
}
...
<input type="hidden" name="id_cheque" value="<?php echo($id_cheque); ?>">
<input type="hidden" name="depositado" value="<?php echo($depositado); ?>">
<input type="hidden" name="protestado" value="<?php echo($protestado); ?>">
...
<table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla" style='margin-top: 5px'>
  ...
  <tr>
<?php if ($depositado == "Si") { ?>	  
    <td class='celdaNombreAttr'>Fec. Depósito:</td>
    <td class='celdaValorAttr' style="background: #FFFF00" colspan="3">
      <input type="date" name="fecha_deposito" value="<?php echo(date("Y-m-d")); ?>" class="boton">
      <input type="submit" name="guardar" value="Guardar">
    </td>
<?php } elseif ($protestado == "Si") { ?>
    <td class='celdaNombreAttr'>Fec. Protesto:</td>
    <td class='celdaValorAttr' style="background: #FFFF00" colspan="3">
      <input type="date" name="fecha_protesto" value="<?php echo(date("Y-m-d")); ?>" class="boton">
      <input type="submit" name="guardar" value="Guardar">
    </td>
<?php } elseif ($protestado == "No") { ?>
    <td class='celdaNombreAttr'>Fec. Pago:</td>
    <td class='celdaValorAttr' style="background: #FFFF00" colspan="3">
      <input type="date" name="fecha_pago" value="<?php echo(date("Y-m-d")); ?>" class="boton">
      <input type="submit" name="guardar" value="Guardar">
    </td>
<?php } ?>
  </tr>
