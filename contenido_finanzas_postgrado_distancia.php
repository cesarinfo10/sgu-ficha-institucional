...
if (!empty($ids_carreras)) {
	$cond_ids_carreras = " AND carrera_actual IN ($ids_carreras)";
}
...
$SQL_av_est_nuevos = "SELECT va.estado,count(va.id) as cantidad
                      ...
                      WHERE va.id IN (SELECT id_alumno FROM matriculas WHERE semestre=$SEMESTRE AND ano=$ANO)
                        AND va.cohorte=$ANO AND c.regimen='$regimen' AND a.estado NOT IN (0,2,53)  $cond_ids_carreras
...
$SQL_av_est_morosos = "SELECT va.estado,count(va.id) as cantidad
                       ...
                       LEFT JOIN carreras AS c ON c.id=a.carrera_actual
                       WHERE va.id IN (SELECT id_alumno FROM matriculas WHERE semestre=$SEMESTRE AND ano=$ANO)
                         AND c.regimen='$regimen' AND moroso_financiero AND a.estado NOT IN (0,2,53)  $cond_ids_carreras
                       GROUP BY va.estado,a.estado ORDER BY a.estado;";
...
$SQL_av_est_insc_ramos = "SELECT va.estado,count(va.id) as cantidad
                          ...
                          WHERE va.id IN (SELECT id_alumno FROM inscripciones_cursos)
                            ...
                            AND c.regimen='$regimen'  AND a.estado NOT IN (0,2,53)  $cond_ids_carreras
                          GROUP BY va.estado,a.estado ORDER BY a.estado;";
...
$y = $z = $i = $h = $cantidad_morosos = $cantidad_insc_ramos = 0;            
for ($x=0;$x<count($estados);$x++) {
	$estado          = $estados[$x]['nombre'];
	$estado_cantidad = 0;
	
	$HTML .= "  <tr class='filaTabla' onClick=\"window.location='$enlbase=gestion_alumnos&estado={$estados[$x]['id']}&regimen=$regimen';\">"
