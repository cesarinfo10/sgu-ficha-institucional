...
if (count($cheques) == 0) {
	for ($y=0;$y<count($cobros);$y++) {			
		...
		//$HTML_INST_FINAN = "<input class='boton' type='text' style='width: 50px' name='inst_finan[$y]' value='{$_REQUEST['inst_finan'][$y]}' $onBlur required>";
		
		$HTML_INST_FINAN = "<select name='inst_finan[$y] class='filtro' style='max-width: 150px' $onBlur required>"
		...
		$HTML .=  "<tr class='filaTabla'>\n"
			  ...
			  .   "  <td class='textoTabla' align='center'><input class='boton' type='text' style='width: 90px' name='nro_cuenta[$y]' value='{$_REQUEST['nro_cuenta'][$y]}' $onBlur required></td>\n"
			  .   "  <td class='textoTabla' align='center'><input class='boton' type='text' style='width: 90px' name='numero[$y]' value='{$_REQUEST['numero'][$y]}' $onBlur required></td>\n"
			  .   "  <td class='textoTabla' align='center'><input class='boton' type='text' style='width: 90px' name='rut_emisor[$y]' value='$rut_al' $onBlur onChange='var valor=this.value;this.value=valor.toUpperCase();' required></td>\n"
			  .   "  <td class='textoTabla' align='center'><input class='boton' type='text' name='nombre_emisor[$y]' size='20' value='$nombre_al' $onBlur onChange='var valor=this.value;this.value=valor.toUpperCase();' required></td>\n"
			  .   "  <td class='textoTabla' align='center'><input class='boton' type='text' style='width: 80px' name='telefono_emisor[$y]' value='$telefono_al' $onBlur required></td>\n"
			  .   "</tr>\n";
	}
} else {
...
if ($_REQUEST['registrar'] == "Registrar") {
	...
	$SQL_insPago = "INSERT INTO finanzas.pagos (cheque_afecha,cant_cheques,nro_boleta_e,id_cajero,fecha)
	                                    VALUES ($monto_total,$cant_cheques,nextval('finanzas.pagos_nro_boleta_e_seq'::regclass),{$_SESSION['id_usuario']},'$fecha_pago'::date);";
	...
	if (consulta_dml($SQL_insCheque) > 0) {	
		...
		api_manager_agregar_alumno($rut_al);
...
<table cellpadding="2" cellspacing="1" border="0" bgcolor="#ffffff" class="tabla">
  ...
  <tr>
    ...
    <td colspan="8" class='textoTabla'>
      <input type='date' class='boton' name='fecha_pago' value="<?php echo(date("Y-m-d")); ?>" <?php echo($disabled); ?> readonly><br>
      <sup>DD-MM-AAAA</sup>
    </td>
  </tr>
