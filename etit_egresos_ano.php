...
function cuentaEgresadosAcumulados($ano_buscar, $id_carrera, $regimen) {
  ...
  LEFT JOIN al_estados AS ae
  ON        ae.id=a.estado
  LEFT JOIN al_estados AS aet
  ON        aet.id=a.estado_tramite
  WHERE     ae.nombre='Egresado'
  AND       (a.ano_egreso <= $ano_buscar)
...
--c.alias||'-'||a.jornada AS alias_carrera
--a.ano_egreso
FROM      alumnos  AS a
LEFT JOIN carreras AS c
ON        c.id=a.carrera_actual
...
--          AND       ano=2022)
WHERE     ae.nombre='Egresado'
AND       (a.ano_egreso BETWEEN $ano_egreso AND       $ano_egreso)
AND       (c.regimen = '$regimen')
--group by 
--c.alias||'-'||a.jornada,
--a.ano_egreso
...
ON        aet.id=a.estado_tramite
WHERE     ae.nombre='Egresado'
AND       (a.ano_egreso BETWEEN $ano_egreso AND       $ano_egreso_fin)
AND       (c.regimen = '$regimen')
order by 
nombre_carrera
";
...
nombre_carrera
  ...
  $SQL_totalCarreras = $SQL_totalCarreras."union
  SELECT    
  distinct
  c.id id_carrera,
  c.nombre nombre_carrera,
...
nombre_carrera
  ...
  ON        c.id=a.carrera_actual
  WHERE  
   (a.cohorte) BETWEEN $ano_egreso AND       $ano_egreso_fin";
  if ($regimen <> "") {
    $SQL_totalCarreras = $SQL_totalCarreras." AND       (c.regimen = '$regimen')";
  }
*/
