...
function obtieneUniversoCarrerasPorConcepto_uno($ano, $ano_fin, $filtroIdCarreras, $regimen, $jornada,  $concepto, $id_area_conocimiento) {
  ...
  nombre_carrera
    ...
    where  
    ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')
    --AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion)) BETWEEN $ano AND       $ano_fin )
    and a.cohorte BETWEEN $ano AND       $ano_fin
  ";
    if ($filtroIdCarreras <> "") {
...
function alumnosMatriculados_uno($regimen, $ano, $id_carrera, $jornada, $id_area_conocimiento) {
  $ss = "
...
function alumnosMatriculados_uno_TODOS($regimen, $ano, $idCarreras, $jornada, $id_area_conocimiento) {
  //champion
  $ss = "
  select count(*) as cuenta
  FROM alumnos AS a
  ...
  WHERE true  
  --AND (a.semestre_cohorte = 1 ) --primer semestre 
  AND (a.cohorte = '$ano' ) AND (a.carrera_actual in ($idCarreras)) 
";
  if ($regimen <> "") {
    $ss = $ss."          and c.regimen = '$regimen'";
    }
...
function alumnosMatriculados_uno_TODOS_primerSemestre($regimen, $ano, $idCarreras, $jornada, $id_area_conocimiento) {
  //champion
  $ss = "
  select count(*) as cuenta
  FROM alumnos AS a
...
function obtieneTasaOportuna_todos($regimen, $ano, $idCarrera, $jornada) {
  //OPORTUNA
$ss = "
select count(*) cuenta_oportuna
FROM      alumnos  AS a
          LEFT JOIN carreras AS c
          ON        c.id=a.carrera_actual
          left join al_estados ale on ale.id = a.estado        
where		  a.cohorte = $ano
--AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion))  = '$ano_buscar') 
--and ale.nombre in ('Licenciado', 'Egresado', 'Titulado')
and ale.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')
and c.id = $idCarrera
and 
(
  case when (
      ( 
		  (date_part('year',coalesce(a.fecha_titulacion, a.examen_grado_titulo_fecha))*2
		  + (
		  	case 
		  		when date_part('month',coalesce(a.fecha_titulacion, a.examen_grado_titulo_fecha)) < 7
		  		then 1 
		  		else 2
		  	end)
		  )
      ...
      - ( 
        ((a.cohorte*2)+a.semestre_cohorte)
      )
    <=
      ...
      ( 
        ...
        from vista_mallas vm
        where 
        vm.id_carrera = a.carrera_actual
...
//    echo($ss);
    ...
    $contadorOportuna = $cuenta_oportuna;
    //echo("<br>contadorOportuna = $contadorOportuna");
    //total matriculados
    $valorTotal =  alumnosMatriculados_uno($regimen, $ano, $idCarrera, $jornada, $id_area_conocimiento); //cuentaEgresados($x, "", $regimen);
    //echo("<br>contadorOportuna = $contadorOportuna /  valorTotal = $valorTotal");
    //$resultado = ($contadorOportuna / $valorTotal)*100; //round($contadorOportuna / $valorTotal,1);
    $resultado = round(($contadorOportuna / $valorTotal)*100,1); //round($contadorOportuna / $valorTotal,1);
    return "($contadorOportuna)vs($valorTotal) = $resultado";
...
function muestraConceptoInstitucional_uno($nombreConcepto, $ano_egreso, $ano_egreso_fin, $totalCarreras, $regimen, $jornada, $id_area_conocimiento) {
  ...
  for ($z=0;$z<count($totalCarreras);$z++) {
      ...
      for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
        ...
        if ($valor > 0) {
          if ($valorTotal > 0) {
            $porcentaje = round($valor/$valorTotal,2)*100;
          } else {
            $porcentaje = 0;  
          }
        ...
        */
        $tasaOportuna = obtieneTasaOportuna_todos($regimen, $x, $idCarrera, $jornada);
        $HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>$tasaOportuna</td>";
      }        
      echo($HTML_anos);
...
function cuentaEgresadosInstitucional_uno($totalCarreras, $ano_buscar, $regimen, $jornada, $id_area_conocimiento) {
  $myCarrera = "";
  for ($z=0;$z<count($totalCarreras);$z++) {
    $idCarrera = $totalCarreras[$z]['id_carrera'];
...
/*
    ...
    c.id=a.carrera_actual
    and ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')
    and ae.id=a.estado
--    AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion))  = '$ano_buscar')  
    and (a.cohorte = '$ano_buscar' ) 
    AND (a.carrera_actual in ($myCarrera)) 
  ...
  */
  ...
  $sqlCuenta     = consulta_sql($ss);
  //echo("<br>este es : <br>$ss <br>");
  extract($sqlCuenta[0]);

  $valorTotal =  alumnosMatriculados_uno_TODOS($regimen, $ano_buscar, $myCarrera, $jornada, $id_area_conocimiento); 
  //echo("$cuenta / $valorTotal");
  $resultado = round(($cuenta / $valorTotal)*100,1);
  ...
  $sql_concepto_1_2 = "                and ((
   ...
   $SQL_totalCarreras = "
    ...
    nombre_carrera
      ...
      --ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')
      --AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion)) BETWEEN $ano AND       $ano_fin )
      a.cohorte BETWEEN $ano AND       $ano_fin
    ";    
    if ($filtroIdCarreras <> "") {
      $SQL_totalCarreras = $SQL_totalCarreras." AND       (c.id in ($filtroIdCarreras))";
    }
...
function obtieneTasaOportuna_primerSemestre($regimen, $ano, $idCarrera, $jornada) {
  //OPORTUNA
  $ss = "
  select count(*) cuenta_oportuna
  FROM      alumnos  AS a
  ...
  //    echo($ss);
      ...
      //echo("<br>contadorOportuna = $contadorOportuna");
      //total matriculados
      $valorTotal =  alumnosMatriculados_dos($regimen, $ano, $idCarrera, $jornada); //cuentaEgresados($x, "", $regimen);
      //echo("<br>contadorOportuna = $contadorOportuna /  valorTotal = $valorTotal");
      //$resultado = ($contadorOportuna / $valorTotal)*100; //round($contadorOportuna / $valorTotal,1);
      $resultado = round(($contadorOportuna / $valorTotal)*100,1); //round($contadorOportuna / $valorTotal,1);
      return "($contadorOportuna)vs($valorTotal) = $resultado";
...
function muestraConceptoInstitucional_dos($nombreConcepto, $ano_egreso, $ano_egreso_fin, $totalCarreras, $regimen, $jornada, $id_area_conocimiento) {
  ...
  for ($z=0;$z<count($totalCarreras);$z++) {
      ...
      for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
        ...
        //$HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>$valor</td>";
        */
        $tasaOportunaPrimerSemestre = obtieneTasaOportuna_primerSemestre($regimen, $x, $idCarrera, $jornada);
        $HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>$tasaOportunaPrimerSemestre</td>";
...
function cuentaEgresadosInstitucional_dos($totalCarreras, $ano_buscar, $regimen, $jornada, $id_area_conocimiento) {
  $myCarrera = "";
  for ($z=0;$z<count($totalCarreras);$z++) {
    $idCarrera = $totalCarreras[$z]['id_carrera'];
    $myCarrera = $myCarrera.$idCarrera.",";
...
*/
    ...
    //echo("<br>parece que es aqui! $ss <br>");
  if ($regimen <> "") {
    $ss = $ss."          and c.regimen = '$regimen'";
  }
  if ($id_area_conocimiento <> "") {
  ...
  $sqlCuenta     = consulta_sql($ss);
  extract($sqlCuenta[0]);
  $valorTotal =  alumnosMatriculados_uno_TODOS_primerSemestre($regimen, $ano_buscar, $myCarrera, $jornada, $id_area_conocimiento); 
  //echo("$cuenta / $valorTotal");
  $resultado = round(($cuenta / $valorTotal)*100,1);
...
if ($puedeSeguir){
        ...
        ";
          ...
          nombre_carrera
            ...
            where  
            ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')
          --  AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion)) BETWEEN $ano_egreso AND       $ano_egreso_fin )
            and a.cohorte BETWEEN $ano_egreso AND       $ano_egreso_fin
          ";
...
*/
        ...
        //echo("<br>$SQL_universoCarreras<br>");
/*
        $SQL_totalCarreras = "
        select 
        distinct 
...
/*
 $SQL_anos_titulacion = "
 SELECT DISTINCT ON (id) id,nombre
 FROM (SELECT date_part('year',fecha_titulacion) AS id,date_part('year',fecha_titulacion) AS nombre 
     FROM alumnos AS a
     LEFT JOIN al_estados AS ae ON ae.id=a.estado
     WHERE
     ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')
     AND fecha_titulacion IS NOT NULL) AS foo
 --    and ae.nombre is not null
 ORDER BY id DESC
 ";
 */ 
 $SQL_anos_egresos = "
...
?>
        <table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
              <tr class='filaTituloTabla'>
                <td class='tituloTabla' style='text-align:left'>Tasa Titulación Oportuna (todos) </td>
                <?php
                    $HTML_anos = "";
                    for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        $HTML_anos = $HTML_anos."<td class='tituloTabla'>$x</td>";
              ...
              <tr class='filaTituloTabla'>
              <td class='tituloTabla' style='text-align:left'>Institucional 1</td>
              <?php
                      $HTML_anos = "";
                      if ($tipo_carrera == "1"){
                        $concepto = "1";
                      ...
                      for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                          $valor =  cuentaEgresadosInstitucional_uno($totalCarreras, $x, $regimen, $jornada, $id_area_conocimiento);
                          $HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>$valor</td>";
                      }        
                      echo($HTML_anos);
...
?>
        <table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
              <tr class='filaTituloTabla'>
                <td class='tituloTabla' style='text-align:left'>Tasa Titulación Oportuna (1er Sem)</td>
                <?php
                    $HTML_anos = "";
                    for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        $HTML_anos = $HTML_anos."<td class='tituloTabla'>$x</td>";
              ...
              <td class='tituloTabla' style='text-align:left'>Institucional 2</td>
              <?php
                      $HTML_anos = "";
                      if ($tipo_carrera == "1"){
                        $concepto = "1";
                      ...
                      for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                          $valor =  cuentaEgresadosInstitucional_dos($totalCarreras, $x, $regimen, $jornada, $id_area_conocimiento);
                          $HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>$valor</td>";
                      }        
                      echo($HTML_anos);
