...
*/
/*
function duracionCarrera($idCarrera) {
  $ss = "
  select 
  max(vm.niveles/2) max_duracion_ano
  from vista_mallas vm
  where 
  vm.id_carrera = $idCarrera
";

    $sqlCuenta     = consulta_sql($ss);
    extract($sqlCuenta[0]);
    return $max_duracion_ano;

}
*/
function obtieneSuma_semestresEnTitularse_todos($regimen, $ano, $idCarrera, $jornada, $id_area_conocimiento) {
  //OPORTUNA
$ss = "
select sum(
  (
  ...
  ) suma_semestres_en_titularse
FROM      alumnos  AS a
          LEFT JOIN carreras AS c
          ON        c.id=a.carrera_actual
          left join al_estados ale on ale.id = a.estado        
where		  a.cohorte = $ano
--AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion))  = '$ano') 
...
//    echo($ss);
    ...
    $suma = $suma_semestres_en_titularse;
    return $suma;
...
function muestraConceptoInstitucional_uno($nombreConcepto, $ano_egreso, $ano_egreso_fin, $totalCarreras, $regimen, $jornada, $id_area_conocimiento) {
  ...
  for ($z=0;$z<count($totalCarreras);$z++) {
      ...
      //$anosCarrera = duracionCarrera($idCarrera);
      $HTML_anos = "";
      ...
      for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
        ...
        */
        ...
        //$HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>años=$anosCarrera, cohorteIngreso=$x</td>";
        $suma = obtieneSuma_semestresEnTitularse_todos($regimen, $x, $idCarrera, $jornada, $id_area_conocimiento);
        $resultado =  round($suma / $valor,1);
        $HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>($valor)-->($resultado)</td>";
        //$HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>($valor)vs($valorTotal) -> $porcentaje%</td>";
      }        
      echo($HTML_anos);
...
--AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion))  = '$ano')  
and a.cohorte = '$ano'
AND (a.carrera_actual = $id_carrera) 
";
*/
if ($jornada <> "") {
  $ss = $ss." AND       (a.jornada = '$jornada')";
...
?>
        <table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
              <tr class='filaTituloTabla'>
                <td class='tituloTabla' style='text-align:left'>Tiempo de Titulación Institucional 1 (todos) </td>
                <?php
                    $HTML_anos = "";
                    for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        $HTML_anos = $HTML_anos."<td class='tituloTabla'>$x</td>";
...
?>
        <table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
              <tr class='filaTituloTabla'>
                <td class='tituloTabla' style='text-align:left'>Tiempo de Titulación Institucional 2 (1er Sem)</td>
                <?php
                    $HTML_anos = "";
                    for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        $HTML_anos = $HTML_anos."<td class='tituloTabla'>$x</td>";
