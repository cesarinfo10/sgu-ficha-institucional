...
--               AND fecha_titulacion IS NOT NULL
--               AND (date_part('year',a.fecha_titulacion) <= $ano_buscar) 
...
nombre_carrera
  ...
  where       (a.ano_egreso BETWEEN $ano_egreso AND       $ano_egreso_fin)
            AND (date_part('year',a.fecha_titulacion) BETWEEN $ano_egreso AND       $ano_egreso_fin) ";
  if ($regimen <> "") {
...
FROM alumnos AS a
WHERE 
 ano_egreso is not null 
 ORDER BY nombre DESC";
 */
/*
 $SQL_anos_egresos = "
 select distinct date_part('year',a.fecha_titulacion) as id, 
 date_part('year',a.fecha_titulacion) as nombre
 FROM alumnos AS a
 order by date_part('year',a.fecha_titulacion) desc
 ";
...
$anos_egresos = consulta_sql($SQL_anos_titulacion);
$anos_egresos_fin = consulta_sql($SQL_anos_titulacion);
...
<table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
      <tr class='filaTituloTabla'>
        <td class='tituloTabla' style='text-align:left'>Nro.Titulados por año</td>
        <?php
            $HTML_anos = "";
            for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                $HTML_anos = $HTML_anos."<td class='tituloTabla'>$x</td>";
