...
function alumnosMatriculados_uno_TODOS($ano_buscar, $regimen, $id_area_conocimiento, $totalCarreras, $jornada) {
  $myCarrera = "";
...
--    AND ( Date_part('year', Coalesce(a.fecha_titulacion, a.fecha_graduacion))  = '$ano_buscar')  
  ...
  $sqlCuenta     = consulta_sql($ss);
  //echo("<br>troya este es : <br>$ss <br>");
  //echo("chemp uno : $ss");
  extract($sqlCuenta[0]);
  return $cuenta;
...
function alumnosMatriculados_dos_TODOS($ano_buscar, $regimen, $id_area_conocimiento, $totalCarreras, $jornada) {
  $myCarrera = "";
  for ($z=0;$z<count($totalCarreras);$z++) {
    $idCarrera = $totalCarreras[$z]['id_carrera'];
    $myCarrera = $myCarrera.$idCarrera.",";
...
?>
        <table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
              <tr class='filaTituloTabla'>
                <td class='tituloTabla' style='text-align:left'>Tasa Titulación Institucional 1 (todos) </td>
                <?php
                    $HTML_anos = "";
                    for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        $HTML_anos = $HTML_anos."<td class='tituloTabla'>$x</td>";
...
//}
                      ...
                      for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        //CAMBIO
                          ...
                          $total = alumnosMatriculados_uno_TODOS($x, $regimen, $id_area_conocimiento, $totalCarreras, $jornada);
                          $resultado = round(($valor / $total)*100, 1); 

                          //FIN CAMBIO
                          $HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>($valor)vs($total) -> $resultado%</td>";
                      }        
                      echo($HTML_anos);
...
?>
        <table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
              <tr class='filaTituloTabla'>
                <td class='tituloTabla' style='text-align:left'>Tasa Titulación Institucional 2 (1er Sem)</td>
                <?php
                    $HTML_anos = "";
                    for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        $HTML_anos = $HTML_anos."<td class='tituloTabla'>$x</td>";
...
//}
                      ...
                      for ($x=$ano_egreso;$x<=$ano_egreso_fin;$x++) {
                        //CAMBIO
                          ...
                          $total = alumnosMatriculados_dos_TODOS($x, $regimen, $id_area_conocimiento, $totalCarreras, $jornada);
                          $resultado = round(($valor / $total)*100, 1); 
                          //FIN CAMBIO
                          $HTML_anos = $HTML_anos."<td class='textoTabla' style='text-align:right'>($valor)vs($total) -> $resultado%</td>";
                      }        
