<h4>Tasa de ocupación de vacantes, por sede, nivel de formación, jornada, modalidad, área del conocimiento y tipo de admisión</h4>
<!--<button type="button" class="btn btn-success" data-toggle="modal" data-target="#modalBenPregrado">Agregar +</button>-->
<div class="table-responsive">
<table class="table table-bordered">
    <thead>
        <tr>
            <th></th>
            <th class="tituloTabla">2019</th>
            <th class="tituloTabla">2020</th>
            <th class="tituloTabla">2021</th>
            <th class="tituloTabla">2022</th>
            <th class="tituloTabla">2023</th>
        </tr>

        <tr>
            <th class="tituloTabla" colspan="6" style="text-align:left !important">Por sede</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>Sede 1</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Sede 2</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Sede N</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <th class="tituloTabla" colspan="6" style="text-align:left !important">Por nivel de formación</th>
        </tr>
        <tr>
            <td>Profesional con Licenciatura</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Profesional sin Licenciatura</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Licenciatura no Conducente a Título</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Bachillerato, Ciclo Inicial o Plan Común</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <th class="tituloTabla" colspan="6" style="text-align:left !important">Por jornada</th>
        </tr>
        <tr>
            <td>Diurna</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Vespertina</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Otra</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <th class="tituloTabla" colspan="6" style="text-align:left !important">Por modalidad</th>
        </tr>
        <tr>
            <td>Presencial</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Semipresencial</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>No presencial</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        </tr>
        <tr>
            <th class="tituloTabla" colspan="6" style="text-align:left !important">Por área del conocimiento</th>
        </tr>
        <tr>
            <td>Área 1</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Área 2</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Área N</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <th class="tituloTabla" colspan="6" style="text-align:left !important">Por tipo de admisión</th>
        </tr>
        <tr>
            <td>Centralizada o regular</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Directa o especial</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Reconocimiento de aprendizajes previos</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Prueba de conocimientos relevantes</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td>Otros</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
        <tr>
            <td class="tituloTabla">Total Institucional</td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
    </tbody>
</table>
</div>