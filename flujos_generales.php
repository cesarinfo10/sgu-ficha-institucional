...
$SQL_flujo_detalle = "SELECT fd.id,fc.nombre AS categoria,fcg.nombre AS totalizador,fcg.tipo,monto_presupuesto,montos,comentarios 
                      FROM finanzas.flujos_detalle AS fd 
                      LEFT JOIN finanzas.flujos_categorias AS fc  ON fc.id=fd.id_cat_flujo
                      LEFT JOIN finanzas.flujos_cat_grupos AS fcg ON fcg.id=fc.id_cat_grupo
                      WHERE ano_flujo=$ano 
                      ORDER BY fcg.tipo DESC,fcg.nombre,fc.nombre";
$flujo_detalle = consulta_sql($SQL_flujo_detalle);
...
$SQL_ingresos_desg = "SELECT r.nombre AS regimen,g.tipo,
                      ...
                      LEFT JOIN regimenes              AS r   ON r.id=coalesce(car.regimen,ca2.regimen)
                      WHERE NOT p.nulo AND c.id IS NOT NULL AND date_part('year',p.fecha) = $ano 
