...
for ($x=0;$x<count($asig_alumno);$x++) {
	...
	$nf = floatval($nf);
	$sum_notas += $nf;
	if (stristr($asignatura,"Trabajo de Grado") == "Trabajo de Grado I")  { $trab_grado1 = $nf; $sum_notas -= $nf; }
	if (stristr($asignatura,"Trabajo de Grado") == "Trabajo de Grado II") { $trab_grado2 = $nf; $sum_notas -= $nf; }
}
...
$texto_docto = "<h2 align='center'>CERTIFICADO</h2><p align='justify'>"
             ...
             . "<br><br>"
             . "Asimismo, también se certifica que el mencionado Programa tiene una duración de $duracion_carrera_alumno semestres "
