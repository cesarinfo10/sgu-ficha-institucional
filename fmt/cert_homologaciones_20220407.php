...
/* este certificado usa el periodo de la asignatura aprobada originalmente y no el periodo en que fue homologada */
...
$SQL_periodo = "SELECT max(c.ano||'-'||c.semestre)::text FROM cargas_academicas AS ca LEFT JOIN cursos AS c on c.id=ca.id_curso WHERE id_estado=1 AND id_alumno=$id_alumno AND c.id_prog_asig=vpa.id";
...
if (empty($texto_adicional)) { $texto_adicional = "<br><br>"; }
