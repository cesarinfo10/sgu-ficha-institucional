...
if (empty($_REQUEST['orden'])) { $orden = "coalesce(a.fecha_titulacion,a.fecha_graduacion) DESC"; }
if (empty($cond_base)) { $cond_base = "ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado') "; }
...
if ($buscar == 'Buscar' && $texto_buscar <> "") {
	...
	if ($ano_titulacion > 0) {
		//$condicion .= "AND ($ano_titulacion IN (date_part('year',a.fecha_titulacion),date_part('year',a.fecha_graduacion)) ) ";
		$condicion .= "AND (date_part('year',coalesce(a.fecha_titulacion,a.fecha_graduacion)) = $ano_titulacion ) ";
		//if ($mes_titulacion <> "-1") { $condicion .= "AND ($mes_titulacion IN (date_part('month',a.fecha_titulacion),date_part('month',a.fecha_graduacion) ) "; }
		if ($mes_titulacion <> "-1") { $condicion .= "AND (date_part('month',coalesce(a.fecha_titulacion,a.fecha_graduacion)) = $mes_titulacion ) "; }
	} elseif ($ano_titulacion == "-2") {
		if ($fec_ini_tit <> "" && $fec_fin_tit <> "") {
			//$condicion .= " AND ( (a.fecha_titulacion between '$fec_ini_tit'::date AND '$fec_fin_tit'::date) OR (a.fecha_graduacion between '$fec_ini_tit'::date AND '$fec_fin_tit'::date) )";
			$condicion .= " AND ( (coalesce(a.fecha_titulacion,a.fecha_graduacion) between '$fec_ini_tit'::date AND '$fec_fin_tit'::date) )";
		}
	}
...
$SQL_alumnos = "SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,c.alias||'-'||a.jornada AS carrera,
                       ...
                       to_char(coalesce(a.fecha_titulacion,a.fecha_graduacion),'DD-tmMon-YYYY') AS fecha_titulacion,
                       a.nota_titulacion,a.nota_graduacion,
...
$SQL_al_SIES = "SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                       'R' AS TIPO_DOCUMENTO,
                       ...
                       to_char(coalesce(a.fecha_titulacion,a.fecha_graduacion),'DD-MM-YYYY') AS FECHA_OBT_TIT_GRA,
...
$SQL_al_estados = "SELECT id,nombre FROM al_estados AS ae WHERE nombre NOT IN ('Moroso') AND $cond_base ORDER BY id;";
...
$ORDENES = array(array('id' => "a.apellidos,a.nombres"                                ,'nombre' => "Nombre (asc.)"),
                 ...
                 array('id' => "coalesce(a.fecha_titulacion,a.fecha_graduacion)"      ,'nombre' => "Fec. Titulación (asc.)"),
                 array('id' => "coalesce(a.fecha_titulacion,a.fecha_graduacion) DESC" ,'nombre' => "Fec. Titulación (desc.)"));
...
$SQL_anos_titulados = "SELECT DISTINCT ON (id) id,nombre
                       FROM (SELECT date_part('year',fecha_titulacion) AS id,date_part('year',fecha_titulacion) AS nombre 
                             FROM alumnos AS a
                             LEFT JOIN al_estados AS ae ON ae.id=a.estado
                             WHERE $cond_base AND fecha_titulacion IS NOT NULL) AS foo
                       ORDER BY id DESC";
...
<table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
  ...
  <tr class='filaTituloTabla'>
    ...
    <td class='tituloTabla' colspan="4">Egreso</td>
    <td class='tituloTabla' colspan="5">Titulación y/o Graduación</td>
  </tr>
  <tr>
    <td class='tituloTabla'>Fecha</td>
    <td class='tituloTabla'>Periodo</td>
    <td class='tituloTabla'>Nota</td>
    <td class='tituloTabla'>Dur.</td> 
    <td class='tituloTabla'>Fecha</td> 
    <td class='tituloTabla'>Nota T.</td> 
    <td class='tituloTabla'>Nota G.</td> 
    <td class='tituloTabla'>Dur.</td> 
    <td class='tituloTabla'>Oport.</td> 
  </tr>
<?php
	$HTML_alumnos = "";
	if (count($alumnos) > 0) {
		for ($x=0;$x<count($alumnos);$x++) {
			...
			$HTML_alumnos .= "  <tr class='filaTabla'>\n"
			               ...
			               . "    <td class='textoTabla' align='center'>$nota_titulacion</td>\n"
			               . "    <td class='textoTabla' align='center'>$nota_graduacion</td>\n"
