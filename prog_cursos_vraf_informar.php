...
$EMAIL_INF  = "jeugenio@umcervantes.cl";

if (!is_numeric($_REQUEST['id_prog_curso']) && ($_REQUEST['tipo'] <> "ACEPTAR" || $_REQUEST['tipo'] <> "RECHAZAR")) {
...
if ($_REQUEST['informar'] == "Informar" && $confirmacion == "" && $tipo == "ACEPTAR") {
	$confirmacion = md5($id_prog_curso);
	$msje = "Está seguro de $tipo esta programación de cursos?\\n"
	      . "Se notificará inmediatamente al Vicerrector de Administración Académico y la escuela de $pc_escuela";	      
	$url_si = "$enlbase=prog_cursos_vraf_informar&id_prog_curso=$id_prog_curso&tipo=$tipo&confirmacion=$confirmacion&comentarios=$comentarios&informar=Informar";
	$url_no = "$enlbase=prog_cursos_vraf_ver&id_prog_curso=$id_prog_curso";
	echo(confirma_js($msje,$url_si,$url_no));
	exit;
} elseif ($confirmacion == md5($id_prog_curso) && $tipo == "ACEPTAR" && $_REQUEST['informar'] == "Informar") {	
	consulta_dml("UPDATE prog_cursos_detalle SET vobo_vraf='t' WHERE id_prog_curso=$id_prog_curso AND vobo_vra");
}

if	($confirmacion == md5($id_prog_curso) && ($tipo == "ACEPTAR" || $tipo == "RECHAZAR")) {
	$cabeceras = "From: SGU" . "\r\n"
	           . "Content-Type: text/plain;charset=utf-8" . "\r\n";
	
	if ($tipo == "ACEPTAR") { $tipo_vraf = "ACEPATADO"; } else { $tipo_vraf = "RECHAZADO"; }
	 		           
	$cuerpo = "La Vicerrectoria de Administración y Finanzas ha $tipo_vraf la programación de cursos de la escuela de $pc_escuela del periodo $pc_periodo.";
	$cuerpo .= !empty($comentarios) ? "\r\nSe ha añadido un comentario de VRAF a esta programación:\r\n$comentarios" : "";
	         
	$asunto = "Prog. Cursos: $pc_escuela $tipo_vraf por VRAF ";
	
	mail($EMAIL_VRAF,$asunto,$cuerpo,$cabeceras);
	mail($EMAIL_VRA,$asunto,$cuerpo,$cabeceras);
	mail($EMAIL_INF,$asunto,$cuerpo,$cabeceras);
	...
	echo(js("window.location='$enlbase=prog_cursos_vraf';"));
	exit;
}
...
<table bgcolor="#ffffff" cellspacing="1" cellpadding="2" class="tabla">
  ...
  <tr>
    ...
    <td class="celdaValorAttr" colspan="3"><b><?php echo($tipo); ?></b></td>
  </tr>
  <tr>
