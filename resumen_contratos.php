...
$condicion = $cond_pagos = $cond_nc = "WHERE true ";
...
$cond_nc = str_replace("p.fecha","nc.fecha",$cond_pagos);

$SQL_nc = "(SELECT sum(ncd.monto) 
            FROM finanzas.cobros AS c 
            LEFT JOIN finanzas.notas_credito_detalle AS ncd ON ncd.id_cobro=c.id
            LEFT JOIN finanzas.notas_credito         AS nc  ON nc.nro_docto=ncd.nro_nc_docto
            $cond_nc AND id_glosa NOT IN (1,10001) AND id_contrato=vc.id
           )";

$SQL_nc_arancel_pagares = "(SELECT sum(ncd.monto) 
                            FROM finanzas.cobros AS c 
                            LEFT JOIN finanzas.notas_credito_detalle AS ncd ON ncd.id_cobro=c.id
                            LEFT JOIN finanzas.notas_credito         AS nc  ON nc.nro_docto=ncd.nro_nc_docto
                            $cond_nc AND id_glosa IN (2,20) AND id_contrato=vc.id
                           )";
          
$SQL_nc_arancel_cheques = "(SELECT sum(ncd.monto) 
                            FROM finanzas.cobros AS c 
                            LEFT JOIN finanzas.notas_credito_detalle AS ncd ON ncd.id_cobro=c.id
                            LEFT JOIN finanzas.notas_credito         AS nc  ON nc.nro_docto=ncd.nro_nc_docto
                            $cond_nc AND id_glosa IN (21,22) AND id_contrato=vc.id
                           )";
          
$SQL_nc_arancel_efectivo = "(SELECT sum(ncd.monto) 
                             FROM finanzas.cobros AS c 
                             LEFT JOIN finanzas.notas_credito_detalle AS ncd ON ncd.id_cobro=c.id
                             LEFT JOIN finanzas.notas_credito         AS nc  ON nc.nro_docto=ncd.nro_nc_docto
                             $cond_nc AND id_glosa IN (3,31,10003) AND id_contrato=vc.id
                            )";
...
$SQL_contratos = "SELECT estado,
                         ...
                         sum($SQL_nc)::bigint AS nc_tot,
                         sum($SQL_nc_arancel_efectivo)::bigint AS nc_arancel_efectivo,
                         sum($SQL_nc_arancel_cheques)::bigint AS nc_arancel_cheques,
                         sum($SQL_nc_arancel_pagares)::bigint AS nc_arancel_pagares
                  FROM vista_contratos AS vc
                  LEFT JOIN carreras   AS car ON car.id=vc.id_carrera
...
$SQL_contratos = "SELECT estado,
                         ...
                         sum(nc_tot)::bigint AS nc_tot,
                         sum(nc_arancel_efectivo)::bigint AS nc_arancel_efectivo,
                         sum(nc_arancel_cheques)::bigint AS nc_arancel_cheques,
                         sum(nc_arancel_pagares)::bigint AS nc_arancel_pagares
	...
	foreach($aEstados_contratos AS $cod_estado => $nom_estado) {
		for ($x=0;$x<count($contratos_resumen);$x++) {
			if ($contratos_resumen[$x]['estado'] == $cod_estado) {
				...
				$aSaldos['y_(13) Notas de Crédito'][$nom_estado]                                      = intval($contratos_resumen[$x]['nc_tot']) * -1;
				$aSaldos['y_(13.2) N. Créd. Aranceles pactado en Efectivo o T. Crédito'][$nom_estado] = intval($contratos_resumen[$x]['nc_arancel_efectivo']) * -1;
				$aSaldos['y_(13.3) N. Créd. Aranceles pactado con Cheques'][$nom_estado]              = intval($contratos_resumen[$x]['nc_arancel_cheques']) * -1;
				$aSaldos['y_(13.4) N. Créd. Aranceles pactado con Pagaré'][$nom_estado]               = intval($contratos_resumen[$x]['nc_arancel_pagares']) * -1;

				$aSaldos['s_(14) Total Saldos Adeudados [ 11 - (12 + 13) ]'][$nom_estado] = $aSaldos['n_(11) Aranceles Cobrables (9+10)'][$nom_estado] 
				                                                           - ($aSaldos['p_(12) Pagos Aranceles'][$nom_estado]
																		   +  $aSaldos['y_(13) Notas de Crédito'][$nom_estado]);
     ...
     //   $aSaldos['s_(13.1.1) Saldos Adeudados pactado en Efectivo y T. Credito (11-12)'][$nom_estado] = $aSaldos['n_(11) Aranceles Cobrables (9+10)'][$nom_estado] 
		//		                                                           - $aSaldos['p_(12) Pagos Aranceles'][$nom_estado];
				...
				$aSaldos['s_(13.1) Total Saldos Adeudados Contables [ 11.1 + 10.1 - (12 + 13) ]'][$nom_estado] = $aSaldos['n_(11.1) Aranceles Cobrables Contable (9.1 + 10)'][$nom_estado] 
				                                                                                               + $aSaldos['c_(10.1) Diferencia por no uso de servicio'][$nom_estado] 
				                                                                                               - ($aSaldos['p_(12) Pagos Aranceles'][$nom_estado]
																											   +  $aSaldos['y_(13) Notas de Crédito'][$nom_estado]);

				$aSaldos['z_(13.2) Saldos Adeudados pactado en Efectivo o T. Crédito [ 9.2 + 9.5 + 10.2 - (12.2 + 13.2) ]'][$nom_estado] = $aSaldos['f_(9.2) Pactado en Efectivo'][$nom_estado] 
																															             ...
																															             +  $aSaldos['y_(13.2) N. Créd. Aranceles pactado en Efectivo o T. Crédito'][$nom_estado]);

				$aSaldos['z_(13.3) Saldos Adeudados pactados en Cheques [ 9.3 + 10.3 - (12.3 + 13.3) ]'][$nom_estado] = $aSaldos['f_(9.3) Pactado con Cheques'][$nom_estado] 
																										              ...
																										              +  $aSaldos['y_(13.3) N. Créd. Aranceles pactado con Cheques'][$nom_estado]);
							
				$aSaldos['z_(13.4) Cuentas por Cobrar (pactados con Pagarés) [ 9.4 + 10.4 - (12.4 + 13.4) ]'][$nom_estado] = $aSaldos['f_(9.4) Pactado con Pagaré'][$nom_estado] 
																											               + $aSaldos['x_(10.4) Condonaciones pactado Pagarés'][$nom_estado]
																												           - ($aSaldos['d_(12.4) Pagos Aranceles pactado con Pagaré'][$nom_estado]
																												           +  $aSaldos['y_(13.4) N. Créd. Aranceles pactado con Pagaré'][$nom_estado]);
