...
for ($ano_cohorte=$ano_remat-1;$ano_cohorte>=$ano_remat-2;$ano_cohorte--) { 
	$aCohortes[] = $ano_cohorte;
	$aSQL_resumen[$ano_cohorte] = sql_mat_sies($ano_cohorte)
	                            . " UNION ALL "
								. sql_desertores($ano_cohorte)
								. " UNION ALL "
								. sql_remat($ano_cohorte)
								. " UNION ALL "
								. sql_posible_desert($ano_cohorte)
								. " UNION ALL "
								. sql_comprometidos_remat($ano_cohorte)
								. " UNION ALL "
								. sql_no_contactados($ano_cohorte);
...
function sql_mat_sies($cohorte) {
	global $ano_remat,$regimen,$id_carrera,$ids_carreras,$jornada;
	$condiciones = "";
...
function sql_desertores($cohorte) {
	global $ano_remat,$regimen,$id_carrera,$ids_carreras,$jornada;
	$condiciones = "";
	if ($regimen <> "") { $condiciones .= " AND c.regimen='$regimen' "; }
	if ($ids_carreras <> "") { $condiciones .= " AND a.carrera_actual IN ($ids_carreras) "; }
...
function sql_remat($cohorte) {
	...
	$SQL = "SELECT 'Rematriculados' AS clasif,at.agrupador AS admision,count(a.id) AS cant_estudiantes
			...
			WHERE cohorte=$cohorte  AND ae.nombre = 'Vigente'
			  ...
			  AND a.id IN (SELECT id_alumno FROM matriculas WHERE ano=$ano_remat)
			GROUP BY at.agrupador";
	return $SQL;
}

function sql_posible_desert($cohorte) {
	global $ano_remat,$regimen,$id_carrera,$ids_carreras,$jornada;
	$condiciones = "";
	if ($regimen <> "") { $condiciones .= " AND c.regimen='$regimen' "; }
	if ($ids_carreras <> "") { $condiciones .= " AND a.carrera_actual IN ($ids_carreras) "; }
...
function sql_comprometidos_remat($cohorte) {
	global $ano_remat,$regimen,$id_carrera,$ids_carreras,$jornada;
	$condiciones = "";
	if ($regimen <> "") { $condiciones .= " AND c.regimen='$regimen' "; }
	if ($ids_carreras <> "") { $condiciones .= " AND a.carrera_actual IN ($ids_carreras) "; }
...
function sql_no_contactados($cohorte) {
	global $ano_remat,$regimen,$id_carrera,$ids_carreras,$jornada;
	$condiciones = "";
	if ($regimen <> "") { $condiciones .= " AND c.regimen='$regimen' "; }
	if ($ids_carreras <> "") { $condiciones .= " AND a.carrera_actual IN ($ids_carreras) "; }
