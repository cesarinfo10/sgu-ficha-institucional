...
$unidad    = consulta_sql("SELECT id FROM gestion.unidades WHERE alias='VRAF'");
$id_unidad = $unidad[0]['id'];

if ($id_unidad > 0) {
	$SQL_cant_conceptos_pol = "SELECT count(id) FROM gestion.segpol_conceptos WHERE id_concepto_padre=spcp.id AND activa";
	...
	$SQL_segpol = "SELECT sp.id,spc.id AS id_concepto,spcp.id AS id_politica,sp.ano,meta,sp.indicador,
				   ...
				   WHERE spc.id_unidad=$id_unidad AND spc.id_concepto_padre IS NOT NULL AND spc.activa AND spcp.activa
	...
	if (count($segpol) > 0) {
		...
		for($x=0;$x<count($segpol);$x++) {
			...
			$HTML .= "  <tr class='filaTabla' onMouseOver=\"elementos=['edpol_$x'];elementos.forEach(mostrar_elementos);\" onMouseOut=\"elementos=['edpol_$x'];elementos.forEach(ocultar_elementos);\">\n"
				  .  "    <td class='celdaNombreAttr' style='text-align: justify' width='200' rowspan='{$segpol[$x]['cant_conceptos']}'>"
			...
			while ($nom_pol == $segpol[$x]['nombre_politica']) {
				...
				$HTML .= "    <td class='textoTabla' width='200' style='text-align: justify' onMouseOver=\"elementos=['edcon_$x'];elementos.forEach(mostrar_elementos);\" onMouseOut=\"elementos=['edcon_$x'];elementos.forEach(ocultar_elementos);\">\n"
				...
				while ($nom_con == $segpol[$x]['nombre_concepto']) {
					...
					$cumplimiento_ratio = "<big style='color:red'>🅧</big> ".$segpol[$x]['cumplimiento_ratio']."%";
					if ($segpol[$x]['cumplimiento_ratio'] >= 100) { $cumplimiento_ratio = "<big>🆗</big>"; }

					$HTML .= "    <td class='textoTabla' align='center' onMouseOver=\"elementos=['edmeta_$x'];elementos.forEach(mostrar_elementos);\" onMouseOut=\"elementos=['edmeta_$x'];elementos.forEach(ocultar_elementos);\">\n"
					      .  "      <big title='Meta'>🎯</big> $meta<br>\n"
					      .  "      <big title='Indicador'>📏</big> $indicador<hr>\n"
					      .  "      $cumplimiento_ratio\n"
				...
				$HTML .= "  </tr>\n";
				if ($nom_con <> $segpol[$x]['nombre_concepto']) { $HTML .= "  <tr class='filaTabla'>\n"; }
			}
			$x--;
		}
		$HTML_segpol = $HTML;
...
<div class='texto' style="margin-top: 5px">
  <big title='Meta'>🎯</big>: Meta &nbsp;&nbsp;
  <big title='Indicador'>📏</big>: Indicador &nbsp;&nbsp;
  <big>🆗</big>: Logrado &nbsp;&nbsp;
  <big style='color:red'>🅧</big>: Nivel de cumplimiento &nbsp;&nbsp;
</div>
<table bgcolor="#ffffff" cellspacing="1" cellpadding="4" class="tabla" style="margin-top: 5px">
  ...
  <tr class='filaTituloTabla'>
    <td class='tituloTabla' rowspan='2' width='250'>Política</td>
    <td class='tituloTabla' rowspan='2' width='250'>Concepto</td>    
