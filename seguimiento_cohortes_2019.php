...
$SQL__als    = "SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos) AS apellidos,initcap(a.nombres) AS nombres,
                       ...
                       CASE WHEN coalesce(niv.nota::text,'NSP') <> 'NSP' AND (($SQL_c1_aprob) > 0 OR ($SQL_s1_aprob) > 0) THEN 'Bajo'
                            WHEN coalesce(niv.nota::text,'NSP') = 'NSP' AND (($SQL_c1_aprob) > 0 OR ($SQL_s1_aprob) > 0) THEN 'Medio'
                            ELSE 'Alto'
                       END AS riesgo_desercion
                FROM alumnos AS a
...
$SQL_alumnos = "SELECT * FROM ($SQL__als) AS al $cond_ext";
if ($id_riesgo_desercion <> "" && $id_riesgo_desercion <> "t") {
	$SQL_alumnos = "SELECT * FROM ($SQL_alumnos) AS foo WHERE riesgo_desercion = '$id_riesgo_desercion'	";
}
$SQL_tabla_completa = "COPY ($SQL_alumnos) to stdout WITH CSV HEADER";
$SQL_alumnos .= "$limite_reg OFFSET $reg_inicio";
...
if (count($alumnos) > 0) {
	$SQL_total_alumnos = "SELECT count(a.id) AS total_alumnos 
	                      ...
	                      LEFT JOIN matriculas AS mat ON (mat.id_alumno=a.id AND mat.semestre=$SEMESTRE AND mat.ano=$ANO)
	                      $condicion";
	$SQL_total_alumnos = "SELECT count(id) AS total_alumnos FROM ($SQL__als) AS foo $cond_ext";
...
$RIESGO_DESERCION = array(array("id" => 'Alto',         "nombre" => 'Alto'),
                          array("id" => 'Medio',        "nombre" => 'Medio'),
                          array("id" => 'Bajo',         "nombre" => 'Bajo'));
			...
			*/
			...
			switch ($riesgo_desercion) {
				...
				case "Medio":
					...
					break;
				case "Bajo":
					$riesgo_desercion = "<div style='background: #1E90FF; color: #FFFFFF; padding: 2px; border-radius: 3px 3px 3px 3px;'>$riesgo_desercion</div>";
					break;
			}
			...
			$HTML_alumnos .= "  <tr class='filaTabla' onClick=\"window.location='$enl';\">\n"
			               ...
			               . "    <td class='textoTabla' align='center'>$matriculado<br>$cant_cursos_insc</td>\n"
			               . "    <td class='textoTabla' align='center'>$nivelacion</td>\n"
			               . "    <td class='textoTabla' align='center'>$rinde_test</td>\n"
                           ...
                           . "    <td class='textoTabla' align='center' style='vertical-align: middle'>$c1</td>\n"
			               . "    <td class='textoTabla' align='center' style='vertical-align: middle'>$s1</td>\n"
			               . "    <td class='textoTabla' align='center' style='vertical-align: middle'>$s2</td>\n"
			               . "    <td class='textoTabla' align='center' style='vertical-align: middle'><b>$riesgo_desercion</b></td>\n"
			               . "  </tr>\n";
		}
	} else {
		$HTML_alumnos = "  <tr>"
