COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')    AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]rb[eé]l[ií]nd[aá]' OR  a.rut ~* '[uú]rb[eé]l[ií]nd[aá]' OR  text(a.id) ~* '[uú]rb[eé]l[ií]nd[aá]' ) 
