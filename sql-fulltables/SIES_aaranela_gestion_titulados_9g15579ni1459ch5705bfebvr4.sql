COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')    AND ( (coalesce(a.fecha_titulacion,a.fecha_graduacion) between '2024-01-25'::date AND '2024-01-25'::date) )AND (a.carrera_actual = '4') AND (c.regimen = 'PRE') 
