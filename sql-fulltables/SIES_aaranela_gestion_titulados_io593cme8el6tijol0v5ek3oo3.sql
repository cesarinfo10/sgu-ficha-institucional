COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')   AND (date_part('year',coalesce(a.fecha_titulacion,a.fecha_graduacion)) = 2023 ) AND (a.carrera_actual = '19') AND (c.regimen = 'PRE') 
