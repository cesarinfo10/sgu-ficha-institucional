COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')    AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]x[aá]nd[eé]r' OR  a.rut ~* '[aá]l[eé]x[aá]nd[eé]r' OR  text(a.id) ~* '[aá]l[eé]x[aá]nd[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé]p[eé]' OR  a.rut ~* 'l[eé]p[eé]' OR  text(a.id) ~* 'l[eé]p[eé]' ) 
