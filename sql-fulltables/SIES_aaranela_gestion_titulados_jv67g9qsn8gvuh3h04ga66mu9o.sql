COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')   AND (cohorte = '2022') AND (semestre_cohorte = 1) AND (mes_cohorte = 6) AND (estado = '53')  AND ( (coalesce(a.fecha_titulacion,a.fecha_graduacion) between '2023-01-01'::date AND '2023-07-10'::date) )AND (a.carrera_actual = '55') AND (c.regimen = 'POST-TD') 
