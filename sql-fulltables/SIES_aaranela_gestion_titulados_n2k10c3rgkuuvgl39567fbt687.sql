COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')   AND (estado = '5')  AND ( (coalesce(a.fecha_titulacion,a.fecha_graduacion) between '2023-01-18'::date AND '2023-01-18'::date) )AND (a.carrera_actual = '4') AND (c.regimen = 'PRE') 
