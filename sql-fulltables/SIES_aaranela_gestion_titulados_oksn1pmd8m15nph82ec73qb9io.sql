COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')   AND (cohorte = '2018') AND (estado = '52')  AND ( (coalesce(a.fecha_titulacion,a.fecha_graduacion) between '2020-01-01'::date AND '2020-12-31'::date) )AND (c.regimen = 'POST-GD') 
