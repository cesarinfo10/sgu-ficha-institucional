COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')   AND (estado = '51')  AND ( (coalesce(a.fecha_titulacion,a.fecha_graduacion) between '2022-03-01'::date AND '2023-02-28'::date) )AND (a.carrera_actual = '19') AND (c.regimen = 'PRE') 
