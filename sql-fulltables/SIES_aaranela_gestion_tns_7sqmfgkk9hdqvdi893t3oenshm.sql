COPY (SELECT 2 AS TIPO_REGISTRO,
                ...
                LEFT JOIN matriculas AS m ON (m.id_alumno=a.id AND m.semestre=1 AND m.ano=2024)
                WHERE a.salida_int_fecha IS NOT NULL AND a.salida_int_nroreg_libro IS NOT NULL AND salida_int_calif IS NOT NULL   AND (a.salida_int_fecha between '2023-03-01'::date AND '2024-02-28'::date) AND (c.regimen = 'PRE') 
