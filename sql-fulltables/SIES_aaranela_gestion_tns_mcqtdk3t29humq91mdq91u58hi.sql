COPY (SELECT 2 AS TIPO_REGISTRO,
                       'R' AS TIPO_DOCUMENTO,
                ...
                LEFT JOIN matriculas AS m ON (m.id_alumno=a.id AND m.semestre=2 AND m.ano=2022)
                WHERE a.salida_int_fecha IS NOT NULL AND a.salida_int_nroreg_libro IS NOT NULL AND salida_int_calif IS NOT NULL  AND (date_part('year',a.salida_int_fecha) = 2022) AND (date_part('month',a.salida_int_fecha) = 11) AND (c.regimen = 'PRE') 
