COPY (SELECT 'R' AS tipo_docto_ident,
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2023' ) AND (a.admision IN ('1','3')) AND (c.regimen = 'PRE') AND (a.id NOT IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2))  AND al_sies.rut IS NOT NULL  
