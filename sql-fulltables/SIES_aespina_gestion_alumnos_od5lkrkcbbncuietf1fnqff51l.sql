COPY (SELECT 'R' AS tipo_docto_ident,
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2022' ) AND (a.admision IN ('1','3')) AND (c.regimen = 'PRE')  AND benef_fiscal IS NULL AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023))  AND al_sies.rut IS NOT NULL  
