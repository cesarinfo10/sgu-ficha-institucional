COPY (SELECT 'R' AS tipo_docto_ident,
                ...
                WHERE (SELECT count(id_alumno) FROM inscripciones_cursos WHERE id_alumno=a.id)=0  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2023' ) AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=1))  
