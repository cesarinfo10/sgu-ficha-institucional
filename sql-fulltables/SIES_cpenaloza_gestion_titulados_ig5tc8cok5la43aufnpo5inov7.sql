COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')    AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]st[ií][oó]n' OR  a.rut ~* 'g[eé]st[ií][oó]n' OR  text(a.id) ~* 'g[eé]st[ií][oó]n' )  AND carrera_actual IN (101,62,26,88,18,3,36,67) 
