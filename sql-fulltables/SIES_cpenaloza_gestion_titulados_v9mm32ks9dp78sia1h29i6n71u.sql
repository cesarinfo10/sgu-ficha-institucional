COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                       'R' AS TIPO_DOCUMENTO,
                ...
                LEFT JOIN matriculas AS m ON (m.id_alumno=a.id AND semestre=2 AND ano=2023)
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')    AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]st[ií][oó]n' OR  a.rut ~* 'g[eé]st[ií][oó]n' OR  text(a.id) ~* 'g[eé]st[ií][oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'y' OR  a.rut ~* 'y' OR  text(a.id) ~* 'y' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[eé]g[oó]c[ií][oó]s' OR  a.rut ~* 'n[eé]g[oó]c[ií][oó]s' OR  text(a.id) ~* 'n[eé]g[oó]c[ií][oó]s' )  AND carrera_actual IN (101,62,26,88,18,3,36,67,150) 
