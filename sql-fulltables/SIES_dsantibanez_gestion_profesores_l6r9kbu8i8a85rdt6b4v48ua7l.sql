COPY (SELECT 'R' AS "TIPO_DOCUMENTO",
                           ...
                           (SELECT sum(horas_semanal) FROM cursos AS c1 LEFT JOIN prog_asig AS pa1 ON pa1.id=c1.id_prog_asig WHERE id_fusion IS NULL AND c1.ano=2021 AND semestre=1 AND id_profesor=u.id) AS "TOTAL_HORAS_PRINCIPAL_PROGRAMA",
                           'SANTIAGO' AS "COMUNA_PRINCIPAL_PROGRAMA",
                           CASE WHEN u.horas_planta IS NOT NULL AND (SELECT sum(horas_semanal) FROM cursos AS c1 LEFT JOIN prog_asig AS pa1 ON pa1.id=c1.id_prog_asig WHERE id_fusion IS NULL AND c1.ano=2021 AND semestre=1 AND id_profesor=u.id) > 0 THEN u.horas_planta + ceil(((SELECT sum(horas_semanal) FROM cursos AS c1 LEFT JOIN prog_asig AS pa1 ON pa1.id=c1.id_prog_asig WHERE id_fusion IS NULL AND c1.ano=2021 AND semestre=1 AND id_profesor=u.id)-coalesce(u.horas_planta_docencia,0))*1.25)
                           ...
                           CASE WHEN coalesce(u.horas_planta,0)+coalesce(u.horas_plazo_fijo,0)>0 THEN 0
                                WHEN coalesce(u.horas_planta,0)+coalesce(u.horas_plazo_fijo,0)+coalesce(u.horas_honorarios,0)=0 THEN ceil((SELECT sum(horas_semanal) FROM cursos AS c1 LEFT JOIN prog_asig AS pa1 ON pa1.id=c1.id_prog_asig WHERE id_fusion IS NULL AND c1.ano=2021 AND semestre=1 AND id_profesor=u.id)*1.25)
                     ...
                     WHERE u.id IN (SELECT vc.id_profesor 
						  ...
						  WHERE true  AND vc.ano=2021  AND vc.semestre=2  AND vc.id_carrera IN (61,68,21) )) to stdout WITH CSV HEADER