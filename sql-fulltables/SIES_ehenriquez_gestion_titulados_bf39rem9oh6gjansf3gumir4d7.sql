COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')    AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]n[ií][eé]l[aá]' OR  a.rut ~* 'd[aá]n[ií][eé]l[aá]' OR  text(a.id) ~* 'd[aá]n[ií][eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]v[ií][eé]d[oó]' OR  a.rut ~* '[oó]v[ií][eé]d[oó]' OR  text(a.id) ~* '[oó]v[ií][eé]d[oó]' )  AND carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
