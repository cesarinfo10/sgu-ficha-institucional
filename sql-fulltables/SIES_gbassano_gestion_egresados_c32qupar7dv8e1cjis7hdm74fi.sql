COPY (SELECT 4 AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre='Egresado'   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]wls[aá]' OR  a.rut ~* '[eé]wls[aá]' OR  text(a.id) ~* '[eé]wls[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]b[eé]ll[oó]' OR  a.rut ~* 'c[aá]b[eé]ll[oó]' OR  text(a.id) ~* 'c[aá]b[eé]ll[oó]' ) 
