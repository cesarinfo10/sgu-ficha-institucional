COPY (SELECT 'R' AS tipo_docto_ident,
                    ...
                    GROUP BY id_alumno) AS semestres_susp,
                       1 AS vigencia,c.nombre AS nombre_carrera,a.jornada  ,CASE WHEN (SELECT count(id_alumno) FROM vista_alumnos_asistencia WHERE id_alumno=a.id  AND fecha BETWEEN '2023-03-12'::date AND '2023-07-25'::date)>0 THEN round(100*(SELECT count(id_alumno) FROM vista_alumnos_asistencia WHERE id_alumno=a.id  AND fecha BETWEEN '2023-03-12'::date AND '2023-07-25'::date AND presente)::real/(SELECT count(id_alumno) FROM vista_alumnos_asistencia WHERE id_alumno=a.id  AND fecha BETWEEN '2023-03-12'::date AND '2023-07-25'::date)) ELSE 0 END AS tasa_presente
                FROM alumnos AS a
