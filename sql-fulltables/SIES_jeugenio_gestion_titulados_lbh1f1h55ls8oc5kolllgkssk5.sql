COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')   AND (cohorte = '2021') AND (estado = '5') AND (date_part('year',coalesce(a.fecha_titulacion,a.fecha_graduacion)) = 2022 ) AND (c.regimen = 'PRE') 
