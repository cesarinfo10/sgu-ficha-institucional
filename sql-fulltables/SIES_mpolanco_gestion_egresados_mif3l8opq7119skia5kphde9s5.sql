COPY (SELECT 4 AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre='Egresado'   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]rll[aá]' OR  a.rut ~* 'k[aá]rll[aá]' OR  text(a.id) ~* 'k[aá]rll[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]t[oó]' OR  a.rut ~* 's[oó]t[oó]' OR  text(a.id) ~* 's[oó]t[oó]' )  AND carrera_actual IN (95,100,2,69,1,19) 
