COPY (SELECT 4 AS TIPO_REGISTRO,
                ...
                WHERE ae.nombre='Egresado'   AND (lower(a.nombres||' '||a.apellidos) ~* 'fl[oó]r[eé]s' OR  a.rut ~* 'fl[oó]r[eé]s' OR  text(a.id) ~* 'fl[oó]r[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  text(a.id) ~* '' )  AND carrera_actual IN (95,100,19,2,69,1) 
