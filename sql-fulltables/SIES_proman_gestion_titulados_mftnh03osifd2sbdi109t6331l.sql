COPY (SELECT CASE WHEN c.alias IN ('DE','D°D°','D°V°','LTS','LED','LED-D') THEN 3 ELSE 1 END AS TIPO_REGISTRO,
                ...
                LEFT JOIN matriculas AS m ON (m.id_alumno=a.id AND semestre=1 AND ano=2023)
                WHERE ae.nombre IN ('Titulado','Graduado','Licenciado','Post-Titulado')   AND (date_part('year',coalesce(a.fecha_titulacion,a.fecha_graduacion)) = 2023 ) AND (c.regimen = 'PRE')  AND carrera_actual IN (52,99,92,49,111,89,93,107,86,87,72,84,85,44,40,46,64,39,45,42,47,53,43,51,71,73,74,41,65,66,90,75,63,113,124,121,122,123,129,130,134,133,132,127,131,136) 
