COPY (SELECT *,(SELECT char_comma_sum(tipo_publico||': '||coalesce(cant_personas::text,'*')) 
            ...
            WHERE id_actividad=act.id) AS indicadores,(SELECT sum(coalesce(cant_personas,0)) 
                  FROM vcm.participacion_act
			      WHERE id_actividad=act.id) AS asist_tot 
            FROM vista_vcm_actividades AS act
			WHERE true  AND (ano = 2023) AND (id_tipo IN (SELECT id FROM vcm.tipos_act WHERE dimension='Extensión')) AND (28 IN (id_unidad1,id_unidad2,id_unidad3)) AND (id_responsable = 1271	) 
