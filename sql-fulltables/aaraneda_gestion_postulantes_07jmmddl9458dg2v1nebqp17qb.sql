COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]nd[oó]z[aá]' OR  pap.rut ~* 'm[eé]nd[oó]z[aá]' OR  text(pap.id) ~* 'm[eé]nd[oó]z[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]r[aá]g[aá]' OR  pap.rut ~* 'm[oó]r[aá]g[aá]' OR  text(pap.id) ~* 'm[oó]r[aá]g[aá]' ) 
