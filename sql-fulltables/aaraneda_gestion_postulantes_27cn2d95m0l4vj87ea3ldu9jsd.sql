COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]v[aá]n' OR  pap.rut ~* '[ií]v[aá]n' OR  text(pap.id) ~* '[ií]v[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]st[oó]rg[aá]' OR  pap.rut ~* '[aá]st[oó]rg[aá]' OR  text(pap.id) ~* '[aá]st[oó]rg[aá]' ) 
