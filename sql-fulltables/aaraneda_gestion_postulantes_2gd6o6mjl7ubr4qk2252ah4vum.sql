COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ndr[eé]s' OR  pap.rut ~* '[aá]ndr[eé]s' OR  text(pap.id) ~* '[aá]ndr[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[oó]nz[aá]l[eé]z' OR  pap.rut ~* 'g[oó]nz[aá]l[eé]z' OR  text(pap.id) ~* 'g[oó]nz[aá]l[eé]z' ) 
