COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[aá]ns' OR  pap.rut ~* 'h[aá]ns' OR  text(pap.id) ~* 'h[aá]ns' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[uú]t[ií][eé]rr[eé]z' OR  pap.rut ~* 'g[uú]t[ií][eé]rr[eé]z' OR  text(pap.id) ~* 'g[uú]t[ií][eé]rr[eé]z' ) 
