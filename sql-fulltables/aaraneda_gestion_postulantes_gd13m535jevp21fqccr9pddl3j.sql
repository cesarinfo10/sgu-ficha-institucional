COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]t[aá]l[ií]n[aá]' OR  pap.rut ~* 'c[aá]t[aá]l[ií]n[aá]' OR  text(pap.id) ~* 'c[aá]t[aá]l[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]h[uú]m[aá]d[aá]' OR  pap.rut ~* '[aá]h[uú]m[aá]d[aá]' OR  text(pap.id) ~* '[aá]h[uú]m[aá]d[aá]' ) 
