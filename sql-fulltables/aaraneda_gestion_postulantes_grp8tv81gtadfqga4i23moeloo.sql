COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'k[aá]r[eé]n' OR  pap.rut ~* 'k[aá]r[eé]n' OR  text(pap.id) ~* 'k[aá]r[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sc[oó]b[aá]r' OR  pap.rut ~* '[eé]sc[oó]b[aá]r' OR  text(pap.id) ~* '[eé]sc[oó]b[aá]r' ) 
