COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]s[eé]' OR  pap.rut ~* 'j[oó]s[eé]' OR  text(pap.id) ~* 'j[oó]s[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]v[aá]n' OR  pap.rut ~* '[ií]v[aá]n' OR  text(pap.id) ~* '[ií]v[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]rn[aá]nd[eé]z' OR  pap.rut ~* 'f[eé]rn[aá]nd[eé]z' OR  text(pap.id) ~* 'f[eé]rn[aá]nd[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]mp[oó]s' OR  pap.rut ~* 'c[aá]mp[oó]s' OR  text(pap.id) ~* 'c[aá]mp[oó]s' ) 
