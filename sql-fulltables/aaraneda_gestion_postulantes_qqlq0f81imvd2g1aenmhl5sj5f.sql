COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]l[ií][aá]' OR  pap.rut ~* 'd[eé]l[ií][aá]' OR  text(pap.id) ~* 'd[eé]l[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]nd[oó]r[ií]' OR  pap.rut ~* 'c[oó]nd[oó]r[ií]' OR  text(pap.id) ~* 'c[oó]nd[oó]r[ií]' ) 
