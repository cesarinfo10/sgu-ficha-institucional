COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]g[uú]st[ií]n[aá]' OR  pap.rut ~* '[aá]g[uú]st[ií]n[aá]' OR  text(pap.id) ~* '[aá]g[uú]st[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá][eé]t[eé]' OR  pap.rut ~* 'g[aá][eé]t[eé]' OR  text(pap.id) ~* 'g[aá][eé]t[eé]' ) 
