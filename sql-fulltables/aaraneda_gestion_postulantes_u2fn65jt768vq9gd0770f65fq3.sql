COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]bl[oó]' OR  pap.rut ~* 'p[aá]bl[oó]' OR  text(pap.id) ~* 'p[aá]bl[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'fl[oó]r[eé]s' OR  pap.rut ~* 'fl[oó]r[eé]s' OR  text(pap.id) ~* 'fl[oó]r[eé]s' ) 
