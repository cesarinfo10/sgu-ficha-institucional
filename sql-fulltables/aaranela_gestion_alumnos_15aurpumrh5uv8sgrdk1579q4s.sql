COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]sc[aá]r' OR  a.rut ~* '[oó]sc[aá]r' OR  lower(a.email) ~* '[oó]sc[aá]r' OR  text(a.id) ~* '[oó]sc[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]z[aá]n[oó]' OR  a.rut ~* 'l[oó]z[aá]n[oó]' OR  lower(a.email) ~* 'l[oó]z[aá]n[oó]' OR  text(a.id) ~* 'l[oó]z[aá]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]b[aá]c[aá]ch[eé]' OR  a.rut ~* '[ií]b[aá]c[aá]ch[eé]' OR  lower(a.email) ~* '[ií]b[aá]c[aá]ch[eé]' OR  text(a.id) ~* '[ií]b[aá]c[aá]ch[eé]' ) 
