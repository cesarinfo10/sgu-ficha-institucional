COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]ll[eé]g[uú][ií]ll[oó]s' OR  a.rut ~* 'g[aá]ll[eé]g[uú][ií]ll[oó]s' OR  lower(a.email) ~* 'g[aá]ll[eé]g[uú][ií]ll[oó]s' OR  text(a.id) ~* 'g[aá]ll[eé]g[uú][ií]ll[oó]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[eé]rn[aá]nd[eé]z' OR  a.rut ~* 'f[eé]rn[aá]nd[eé]z' OR  lower(a.email) ~* 'f[eé]rn[aá]nd[eé]z' OR  text(a.id) ~* 'f[eé]rn[aá]nd[eé]z' ) 
