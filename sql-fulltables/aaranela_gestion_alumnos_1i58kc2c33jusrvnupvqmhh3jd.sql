COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]rn[aá]n' OR  a.rut ~* 'h[eé]rn[aá]n' OR  lower(a.email) ~* 'h[eé]rn[aá]n' OR  text(a.id) ~* 'h[eé]rn[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]h[uú]m[aá]d[aá]' OR  a.rut ~* '[aá]h[uú]m[aá]d[aá]' OR  lower(a.email) ~* '[aá]h[uú]m[aá]d[aá]' OR  text(a.id) ~* '[aá]h[uú]m[aá]d[aá]' ) 
