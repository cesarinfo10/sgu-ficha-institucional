COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]s[aá]' OR  a.rut ~* 'm[eé]s[aá]' OR  lower(a.email) ~* 'm[eé]s[aá]' OR  text(a.id) ~* 'm[eé]s[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[aá]rc[oó]n' OR  a.rut ~* '[aá]l[aá]rc[oó]n' OR  lower(a.email) ~* '[aá]l[aá]rc[oó]n' OR  text(a.id) ~* '[aá]l[aá]rc[oó]n' ) 
