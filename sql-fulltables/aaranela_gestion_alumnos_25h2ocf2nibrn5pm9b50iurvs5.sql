COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]ch[uú]ñ[aá]nt[eé]' OR  a.rut ~* 'p[ií]ch[uú]ñ[aá]nt[eé]' OR  lower(a.email) ~* 'p[ií]ch[uú]ñ[aá]nt[eé]' OR  text(a.id) ~* 'p[ií]ch[uú]ñ[aá]nt[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rch[aá]nt' OR  a.rut ~* 'm[aá]rch[aá]nt' OR  lower(a.email) ~* 'm[aá]rch[aá]nt' OR  text(a.id) ~* 'm[aá]rch[aá]nt' ) 
