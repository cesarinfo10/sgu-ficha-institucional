COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]j[aá]s' OR  a.rut ~* 'r[oó]j[aá]s' OR  lower(a.email) ~* 'r[oó]j[aá]s' OR  text(a.id) ~* 'r[oó]j[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]rq[uú][eé]r[oó]s' OR  a.rut ~* '[aá]rq[uú][eé]r[oó]s' OR  lower(a.email) ~* '[aá]rq[uú][eé]r[oó]s' OR  text(a.id) ~* '[aá]rq[uú][eé]r[oó]s' ) 
