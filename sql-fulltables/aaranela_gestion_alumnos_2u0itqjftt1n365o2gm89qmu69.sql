COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]sq[uú][eé]z' OR  a.rut ~* 'v[aá]sq[uú][eé]z' OR  lower(a.email) ~* 'v[aá]sq[uú][eé]z' OR  text(a.id) ~* 'v[aá]sq[uú][eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]d[ií]ll[aá]' OR  a.rut ~* 'p[aá]d[ií]ll[aá]' OR  lower(a.email) ~* 'p[aá]d[ií]ll[aá]' OR  text(a.id) ~* 'p[aá]d[ií]ll[aá]' ) 
