COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]c[eé]r[eé]s' OR  a.rut ~* 'c[aá]c[eé]r[eé]s' OR  lower(a.email) ~* 'c[aá]c[eé]r[eé]s' OR  text(a.id) ~* 'c[aá]c[eé]r[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]r[eé]z' OR  a.rut ~* 'p[eé]r[eé]z' OR  lower(a.email) ~* 'p[eé]r[eé]z' OR  text(a.id) ~* 'p[eé]r[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]n[eé]' OR  a.rut ~* 'r[eé]n[eé]' OR  lower(a.email) ~* 'r[eé]n[eé]' OR  text(a.id) ~* 'r[eé]n[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[ií]b[aá]l' OR  a.rut ~* '[aá]n[ií]b[aá]l' OR  lower(a.email) ~* '[aá]n[ií]b[aá]l' OR  text(a.id) ~* '[aá]n[ií]b[aá]l' ) 
