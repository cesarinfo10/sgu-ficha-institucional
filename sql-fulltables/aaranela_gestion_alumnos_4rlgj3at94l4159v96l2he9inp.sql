COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[ií][aá]s' OR  a.rut ~* '[aá]r[ií][aá]s' OR  lower(a.email) ~* '[aá]r[ií][aá]s' OR  text(a.id) ~* '[aá]r[ií][aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]nj[eé]' OR  a.rut ~* 'm[oó]nj[eé]' OR  lower(a.email) ~* 'm[oó]nj[eé]' OR  text(a.id) ~* 'm[oó]nj[eé]' ) 
