COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]c[aá]rd[oó]' OR  a.rut ~* 'r[ií]c[aá]rd[oó]' OR  lower(a.email) ~* 'r[ií]c[aá]rd[oó]' OR  text(a.id) ~* 'r[ií]c[aá]rd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[uú]ñ[oó]z' OR  a.rut ~* 'm[uú]ñ[oó]z' OR  lower(a.email) ~* 'm[uú]ñ[oó]z' OR  text(a.id) ~* 'm[uú]ñ[oó]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  a.rut ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  lower(a.email) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  text(a.id) ~* 'b[uú]st[aá]m[aá]nt[eé]' ) 
