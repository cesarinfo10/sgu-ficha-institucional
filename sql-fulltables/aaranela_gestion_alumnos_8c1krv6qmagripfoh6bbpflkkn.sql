COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2023 AND regimen='PRE-D' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2023  
                FROM alumnos AS a
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2023' ) AND (a.estado = '1') AND (c.regimen = 'PRE-D') 
