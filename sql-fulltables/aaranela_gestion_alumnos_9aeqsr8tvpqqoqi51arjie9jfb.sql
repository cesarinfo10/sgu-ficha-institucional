COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2022 AND regimen='DIP-D' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2022  
                FROM alumnos AS a
                ...
                WHERE true  AND (a.cohorte = '2021' ) AND (a.estado = '4') AND (a.carrera_actual = 125) AND (c.regimen = 'DIP-D') 
