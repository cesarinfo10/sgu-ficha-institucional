COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[aá]t[ií][aá]n[aá]' OR  a.rut ~* 't[aá]t[ií][aá]n[aá]' OR  lower(a.email) ~* 't[aá]t[ií][aá]n[aá]' OR  text(a.id) ~* 't[aá]t[ií][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]rr[uú]t[ií][aá]' OR  a.rut ~* '[uú]rr[uú]t[ií][aá]' OR  lower(a.email) ~* '[uú]rr[uú]t[ií][aá]' OR  text(a.id) ~* '[uú]rr[uú]t[ií][aá]' ) 
