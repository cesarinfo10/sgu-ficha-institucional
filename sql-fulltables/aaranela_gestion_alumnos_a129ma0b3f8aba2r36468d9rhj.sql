COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]r[aá]' OR  a.rut ~* 's[aá]r[aá]' OR  lower(a.email) ~* 's[aá]r[aá]' OR  text(a.id) ~* 's[aá]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]v[ií]l[aá]' OR  a.rut ~* '[aá]v[ií]l[aá]' OR  lower(a.email) ~* '[aá]v[ií]l[aá]' OR  text(a.id) ~* '[aá]v[ií]l[aá]' ) 
