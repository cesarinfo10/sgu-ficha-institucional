COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]rr[eé]r[aá]' OR  a.rut ~* 'h[eé]rr[eé]r[aá]' OR  lower(a.email) ~* 'h[eé]rr[eé]r[aá]' OR  text(a.id) ~* 'h[eé]rr[eé]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]r[eé]n' OR  a.rut ~* 'l[oó]r[eé]n' OR  lower(a.email) ~* 'l[oó]r[eé]n' OR  text(a.id) ~* 'l[oó]r[eé]n' ) 
