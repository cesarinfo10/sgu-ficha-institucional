COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true  AND ((a.cohorte = 2022 AND a.semestre_cohorte = 1 AND a.mes_cohorte = 3)      OR (a.cohorte_reinc = 2022 AND a.semestre_cohorte_reinc = 1 AND a.mes_cohorte_reinc = 3)) AND (a.cohorte = '2022' OR a.cohorte_reinc = '2022') AND (a.estado = '53') AND (a.carrera_actual = 57) AND (c.regimen = 'POST-TD') 
