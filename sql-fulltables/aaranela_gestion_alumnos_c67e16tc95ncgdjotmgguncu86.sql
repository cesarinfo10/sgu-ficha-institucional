COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.carrera_actual = 19) AND (a.jornada = 'D') AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1)) 
