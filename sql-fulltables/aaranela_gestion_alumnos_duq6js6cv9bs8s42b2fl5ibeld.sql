COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]l[aá]z[aá]r' OR  a.rut ~* 's[aá]l[aá]z[aá]r' OR  lower(a.email) ~* 's[aá]l[aá]z[aá]r' OR  text(a.id) ~* 's[aá]l[aá]z[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]nn[aá]rd' OR  a.rut ~* 'm[oó]nn[aá]rd' OR  lower(a.email) ~* 'm[oó]nn[aá]rd' OR  text(a.id) ~* 'm[oó]nn[aá]rd' ) 
