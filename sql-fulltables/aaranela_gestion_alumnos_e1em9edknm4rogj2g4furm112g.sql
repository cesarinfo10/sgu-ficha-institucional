COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú][aá]n' OR  a.rut ~* 'j[uú][aá]n' OR  lower(a.email) ~* 'j[uú][aá]n' OR  text(a.id) ~* 'j[uú][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]m[ií]l[ií][oó]' OR  a.rut ~* '[eé]m[ií]l[ií][oó]' OR  lower(a.email) ~* '[eé]m[ií]l[ií][oó]' OR  text(a.id) ~* '[eé]m[ií]l[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé]l' OR  a.rut ~* 'd[eé]l' OR  lower(a.email) ~* 'd[eé]l' OR  text(a.id) ~* 'd[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]ll[eé]' OR  a.rut ~* 'v[aá]ll[eé]' OR  lower(a.email) ~* 'v[aá]ll[eé]' OR  text(a.id) ~* 'v[aá]ll[eé]' ) 
