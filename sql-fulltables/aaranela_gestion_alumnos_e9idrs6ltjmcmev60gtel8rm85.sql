COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rl[aá]' OR  a.rut ~* 'c[aá]rl[aá]' OR  lower(a.email) ~* 'c[aá]rl[aá]' OR  text(a.id) ~* 'c[aá]rl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]c' OR  a.rut ~* 'm[aá]c' OR  lower(a.email) ~* 'm[aá]c' OR  text(a.id) ~* 'm[aá]c' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][aá]n' OR  a.rut ~* 'l[eé][aá]n' OR  lower(a.email) ~* 'l[eé][aá]n' OR  text(a.id) ~* 'l[eé][aá]n' ) 
