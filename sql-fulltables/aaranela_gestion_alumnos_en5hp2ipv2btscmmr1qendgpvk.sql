COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]g[uú]d[eé]l[oó]' OR  a.rut ~* '[aá]g[uú]d[eé]l[oó]' OR  lower(a.email) ~* '[aá]g[uú]d[eé]l[oó]' OR  text(a.id) ~* '[aá]g[uú]d[eé]l[oó]' ) 
