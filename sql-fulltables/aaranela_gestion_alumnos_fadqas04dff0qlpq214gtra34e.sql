COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rr[eé][aá]' OR  a.rut ~* 'c[oó]rr[eé][aá]' OR  lower(a.email) ~* 'c[oó]rr[eé][aá]' OR  text(a.id) ~* 'c[oó]rr[eé][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]t[ií]c[aá]' OR  a.rut ~* 'g[aá]t[ií]c[aá]' OR  lower(a.email) ~* 'g[aá]t[ií]c[aá]' OR  text(a.id) ~* 'g[aá]t[ií]c[aá]' ) 
