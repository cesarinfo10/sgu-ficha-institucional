COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]n[ií]b[eé]tt[eé]' OR  a.rut ~* '[eé]n[ií]b[eé]tt[eé]' OR  lower(a.email) ~* '[eé]n[ií]b[eé]tt[eé]' OR  text(a.id) ~* '[eé]n[ií]b[eé]tt[eé]' ) 
