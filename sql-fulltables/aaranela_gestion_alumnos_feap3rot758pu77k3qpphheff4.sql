COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rt[ií]ns' OR  a.rut ~* 'm[aá]rt[ií]ns' OR  lower(a.email) ~* 'm[aá]rt[ií]ns' OR  text(a.id) ~* 'm[aá]rt[ií]ns' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé]' OR  a.rut ~* 'd[eé]' OR  lower(a.email) ~* 'd[eé]' OR  text(a.id) ~* 'd[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[aá][eé]s' OR  a.rut ~* 'm[oó]r[aá][eé]s' OR  lower(a.email) ~* 'm[oó]r[aá][eé]s' OR  text(a.id) ~* 'm[oó]r[aá][eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]c[eé]l[ií][aá]' OR  a.rut ~* 'l[uú]c[eé]l[ií][aá]' OR  lower(a.email) ~* 'l[uú]c[eé]l[ií][aá]' OR  text(a.id) ~* 'l[uú]c[eé]l[ií][aá]' ) 
