COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]sc[oó]b[aá]r' OR  a.rut ~* '[eé]sc[oó]b[aá]r' OR  lower(a.email) ~* '[eé]sc[oó]b[aá]r' OR  text(a.id) ~* '[eé]sc[oó]b[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]lm[aá]r' OR  a.rut ~* 'b[eé]lm[aá]r' OR  lower(a.email) ~* 'b[eé]lm[aá]r' OR  text(a.id) ~* 'b[eé]lm[aá]r' ) 
