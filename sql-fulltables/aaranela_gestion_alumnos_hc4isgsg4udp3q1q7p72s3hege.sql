COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[oó]r[ií][eé]g[aá]' OR  a.rut ~* 'n[oó]r[ií][eé]g[aá]' OR  lower(a.email) ~* 'n[oó]r[ií][eé]g[aá]' OR  text(a.id) ~* 'n[oó]r[ií][eé]g[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rd[oó]' OR  a.rut ~* 'c[aá]rd[oó]' OR  lower(a.email) ~* 'c[aá]rd[oó]' OR  text(a.id) ~* 'c[aá]rd[oó]' ) 
