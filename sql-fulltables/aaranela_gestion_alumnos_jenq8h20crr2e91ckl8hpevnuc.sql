COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]ch[aá][eé]l' OR  a.rut ~* 'm[ií]ch[aá][eé]l' OR  lower(a.email) ~* 'm[ií]ch[aá][eé]l' OR  text(a.id) ~* 'm[ií]ch[aá][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[aá]l[eé]s' OR  a.rut ~* 'm[oó]r[aá]l[eé]s' OR  lower(a.email) ~* 'm[oó]r[aá]l[eé]s' OR  text(a.id) ~* 'm[oó]r[aá]l[eé]s' ) 
