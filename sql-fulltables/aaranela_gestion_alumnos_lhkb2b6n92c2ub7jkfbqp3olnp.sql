COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá]cq[uú][eé]l[ií]n' OR  a.rut ~* 'j[aá]cq[uú][eé]l[ií]n' OR  lower(a.email) ~* 'j[aá]cq[uú][eé]l[ií]n' OR  text(a.id) ~* 'j[aá]cq[uú][eé]l[ií]n' ) 
