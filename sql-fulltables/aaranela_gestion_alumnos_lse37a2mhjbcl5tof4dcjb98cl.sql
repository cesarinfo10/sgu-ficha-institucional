COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ndr[aá]d[eé]' OR  a.rut ~* '[aá]ndr[aá]d[eé]' OR  lower(a.email) ~* '[aá]ndr[aá]d[eé]' OR  text(a.id) ~* '[aá]ndr[aá]d[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'br[ií]t[oó]' OR  a.rut ~* 'br[ií]t[oó]' OR  lower(a.email) ~* 'br[ií]t[oó]' OR  text(a.id) ~* 'br[ií]t[oó]' ) 
