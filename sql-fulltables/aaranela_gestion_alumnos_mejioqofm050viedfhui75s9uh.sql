COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]nm[aá]' OR  a.rut ~* '[eé]nm[aá]' OR  lower(a.email) ~* '[eé]nm[aá]' OR  text(a.id) ~* '[eé]nm[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]s[uú]rt[oó]' OR  a.rut ~* 'b[aá]s[uú]rt[oó]' OR  lower(a.email) ~* 'b[aá]s[uú]rt[oó]' OR  text(a.id) ~* 'b[aá]s[uú]rt[oó]' ) 
