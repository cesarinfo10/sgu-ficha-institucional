COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]rl[aá]' OR  a.rut ~* 'k[aá]rl[aá]' OR  lower(a.email) ~* 'k[aá]rl[aá]' OR  text(a.id) ~* 'k[aá]rl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]ld[eé]s' OR  a.rut ~* 'v[aá]ld[eé]s' OR  lower(a.email) ~* 'v[aá]ld[eé]s' OR  text(a.id) ~* 'v[aá]ld[eé]s' ) 
