COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rv[aá]ch[oó]' OR  a.rut ~* 'c[aá]rv[aá]ch[oó]' OR  lower(a.email) ~* 'c[aá]rv[aá]ch[oó]' OR  text(a.id) ~* 'c[aá]rv[aá]ch[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]str[aá]d[aá]' OR  a.rut ~* '[eé]str[aá]d[aá]' OR  lower(a.email) ~* '[eé]str[aá]d[aá]' OR  text(a.id) ~* '[eé]str[aá]d[aá]' ) 
