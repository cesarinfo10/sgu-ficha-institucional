COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'cr[ií]st[ií]n[aá]' OR  a.rut ~* 'cr[ií]st[ií]n[aá]' OR  lower(a.email) ~* 'cr[ií]st[ií]n[aá]' OR  text(a.id) ~* 'cr[ií]st[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá][uú]r[eé]r[ií][aá]' OR  a.rut ~* 'm[aá][uú]r[eé]r[ií][aá]' OR  lower(a.email) ~* 'm[aá][uú]r[eé]r[ií][aá]' OR  text(a.id) ~* 'm[aá][uú]r[eé]r[ií][aá]' ) 
