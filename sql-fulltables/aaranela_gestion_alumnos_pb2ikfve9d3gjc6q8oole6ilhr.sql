COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]c[eé]v[eé]d[oó]' OR  a.rut ~* '[aá]c[eé]v[eé]d[oó]' OR  lower(a.email) ~* '[aá]c[eé]v[eé]d[oó]' OR  text(a.id) ~* '[aá]c[eé]v[eé]d[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]t[uú]rr[ií][eé]t[aá]' OR  a.rut ~* '[ií]t[uú]rr[ií][eé]t[aá]' OR  lower(a.email) ~* '[ií]t[uú]rr[ií][eé]t[aá]' OR  text(a.id) ~* '[ií]t[uú]rr[ií][eé]t[aá]' ) 
