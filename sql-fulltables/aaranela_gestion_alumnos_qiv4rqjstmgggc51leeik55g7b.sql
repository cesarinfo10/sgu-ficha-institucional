COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rr[ií]ll[oó]' OR  a.rut ~* 'c[aá]rr[ií]ll[oó]' OR  lower(a.email) ~* 'c[aá]rr[ií]ll[oó]' OR  text(a.id) ~* 'c[aá]rr[ií]ll[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rd[oó]b[aá]' OR  a.rut ~* 'c[oó]rd[oó]b[aá]' OR  lower(a.email) ~* 'c[oó]rd[oó]b[aá]' OR  text(a.id) ~* 'c[oó]rd[oó]b[aá]' ) 
