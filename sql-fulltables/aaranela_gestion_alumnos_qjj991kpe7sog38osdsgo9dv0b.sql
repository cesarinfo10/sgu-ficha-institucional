COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]m[uú][eé]l' OR  a.rut ~* 's[aá]m[uú][eé]l' OR  lower(a.email) ~* 's[aá]m[uú][eé]l' OR  text(a.id) ~* 's[aá]m[uú][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]ss[aá]nd[oó]n' OR  a.rut ~* '[oó]ss[aá]nd[oó]n' OR  lower(a.email) ~* '[oó]ss[aá]nd[oó]n' OR  text(a.id) ~* '[oó]ss[aá]nd[oó]n' ) 
