COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé][aá]nn[eé]t[eé]' OR  a.rut ~* 'j[eé][aá]nn[eé]t[eé]' OR  lower(a.email) ~* 'j[eé][aá]nn[eé]t[eé]' OR  text(a.id) ~* 'j[eé][aá]nn[eé]t[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[aá]g[aá]' OR  a.rut ~* 'm[oó]r[aá]g[aá]' OR  lower(a.email) ~* 'm[oó]r[aá]g[aá]' OR  text(a.id) ~* 'm[oó]r[aá]g[aá]' ) 
