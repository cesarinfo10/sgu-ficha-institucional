COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]y[aá]' OR  a.rut ~* 'm[oó]y[aá]' OR  lower(a.email) ~* 'm[oó]y[aá]' OR  text(a.id) ~* 'm[oó]y[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]r[eé]nz[oó]' OR  a.rut ~* 'l[oó]r[eé]nz[oó]' OR  lower(a.email) ~* 'l[oó]r[eé]nz[oó]' OR  text(a.id) ~* 'l[oó]r[eé]nz[oó]' ) 
