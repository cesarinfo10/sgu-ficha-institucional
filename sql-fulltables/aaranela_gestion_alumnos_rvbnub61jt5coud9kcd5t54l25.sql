COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[uú]ñ[eé]z' OR  a.rut ~* 'n[uú]ñ[eé]z' OR  lower(a.email) ~* 'n[uú]ñ[eé]z' OR  text(a.id) ~* 'n[uú]ñ[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]z[aá]' OR  a.rut ~* 'm[eé]z[aá]' OR  lower(a.email) ~* 'm[eé]z[aá]' OR  text(a.id) ~* 'm[eé]z[aá]' ) 
