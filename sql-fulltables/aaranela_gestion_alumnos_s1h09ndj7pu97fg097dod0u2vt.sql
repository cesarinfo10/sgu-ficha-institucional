COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[eé]n[aá]cc[ií]' OR  a.rut ~* 'c[aá]t[eé]n[aá]cc[ií]' OR  lower(a.email) ~* 'c[aá]t[eé]n[aá]cc[ií]' OR  text(a.id) ~* 'c[aá]t[eé]n[aá]cc[ií]' ) 
