COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rv[aá]j[aá]l' OR  a.rut ~* 'c[aá]rv[aá]j[aá]l' OR  lower(a.email) ~* 'c[aá]rv[aá]j[aá]l' OR  text(a.id) ~* 'c[aá]rv[aá]j[aá]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'll[aá]n[oó]s' OR  a.rut ~* 'll[aá]n[oó]s' OR  lower(a.email) ~* 'll[aá]n[oó]s' OR  text(a.id) ~* 'll[aá]n[oó]s' ) 
