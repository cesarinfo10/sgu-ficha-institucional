COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]n[ií].s[eé]pv@gm[aá][ií]l.c[oó]m' OR  a.rut ~* 'd[aá]n[ií].s[eé]pv@gm[aá][ií]l.c[oó]m' OR  lower(a.email) ~* 'd[aá]n[ií].s[eé]pv@gm[aá][ií]l.c[oó]m' OR  text(a.id) ~* 'd[aá]n[ií].s[eé]pv@gm[aá][ií]l.c[oó]m' ) 
