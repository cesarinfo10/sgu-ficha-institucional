COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]b[eé]l' OR  a.rut ~* '[aá]b[eé]l' OR  lower(a.email) ~* '[aá]b[eé]l' OR  text(a.id) ~* '[aá]b[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rt[ií]n' OR  a.rut ~* 'm[aá]rt[ií]n' OR  lower(a.email) ~* 'm[aá]rt[ií]n' OR  text(a.id) ~* 'm[aá]rt[ií]n' ) 
