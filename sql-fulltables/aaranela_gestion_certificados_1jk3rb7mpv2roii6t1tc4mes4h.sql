COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]nr[ií]q[uú][eé]z' OR  a.rut ~* 'h[eé]nr[ií]q[uú][eé]z' OR  text(ac.folio) ~* 'h[eé]nr[ií]q[uú][eé]z' OR  vac.cod ~* 'h[eé]nr[ií]q[uú][eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[oó]ch[eé]s' OR  a.rut ~* 'n[oó]ch[eé]s' OR  text(ac.folio) ~* 'n[oó]ch[eé]s' OR  vac.cod ~* 'n[oó]ch[eé]s' ) 
