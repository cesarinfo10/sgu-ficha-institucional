COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                         ...
                         CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2023 AND semestre=1 LIMIT 1)=1 THEN 'Si' ELSE 'No' END AS matric,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* '[oó]ñ[aá]' OR  a.rut ~* '[oó]ñ[aá]' OR  text(ac.folio) ~* '[oó]ñ[aá]' OR  vac.cod ~* '[oó]ñ[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]j[eé]d[aá]' OR  a.rut ~* '[oó]j[eé]d[aá]' OR  text(ac.folio) ~* '[oó]j[eé]d[aá]' OR  vac.cod ~* '[oó]j[eé]d[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]y[aá]n[aá]' OR  a.rut ~* 'd[aá]y[aá]n[aá]' OR  text(ac.folio) ~* 'd[aá]y[aá]n[aá]' OR  vac.cod ~* 'd[aá]y[aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]st[eé]f[aá]n[ií][aá]' OR  a.rut ~* '[eé]st[eé]f[aá]n[ií][aá]' OR  text(ac.folio) ~* '[eé]st[eé]f[aá]n[ií][aá]' OR  vac.cod ~* '[eé]st[eé]f[aá]n[ií][aá]' ) 
