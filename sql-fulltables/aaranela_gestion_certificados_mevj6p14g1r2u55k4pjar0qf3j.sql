COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]v[eé]z' OR  a.rut ~* 'p[aá]v[eé]z' OR  text(ac.folio) ~* 'p[aá]v[eé]z' OR  vac.cod ~* 'p[aá]v[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]r[eé]z' OR  a.rut ~* 'p[eé]r[eé]z' OR  text(ac.folio) ~* 'p[eé]r[eé]z' OR  vac.cod ~* 'p[eé]r[eé]z' ) 
