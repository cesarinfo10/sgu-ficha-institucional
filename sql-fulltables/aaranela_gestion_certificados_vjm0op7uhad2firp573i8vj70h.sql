COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'f[oó]r[oó]nd[aá]' OR  a.rut ~* 'f[oó]r[oó]nd[aá]' OR  text(ac.folio) ~* 'f[oó]r[oó]nd[aá]' OR  vac.cod ~* 'f[oó]r[oó]nd[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  a.rut ~* 'r[oó]dr[ií]g[uú][eé]z' OR  text(ac.folio) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  vac.cod ~* 'r[oó]dr[ií]g[uú][eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  text(ac.folio) ~* '' OR  vac.cod ~* '' ) 
