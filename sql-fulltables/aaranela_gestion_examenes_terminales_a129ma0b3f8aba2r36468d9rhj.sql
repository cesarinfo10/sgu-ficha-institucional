COPY (SELECT et.id,to_char(et.fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
						 ...
						 u.nombre AS ministro_de_fe,(SELECT count(ete.id_alumno) FROM examenes_terminales_estudiantes AS ete WHERE ete.id_exam_term=et.id) AS cant_estudiantes,
						 (SELECT char_comma_sum(vp.nombre) FROM examenes_terminales_docentes LEFT JOIN vista_profesores AS vp ON vp.id=id_profesor WHERE id_examen=et.id) AS comision,e.nombre AS escuela,
				  ...
				  WHERE true   AND (vete.nombres::text ~* 'v[ií]tt[oó]r[ií]' OR  vete.ruts::text ~* 'v[ií]tt[oó]r[ií]' ) AND (vete.nombres::text ~* 'w[ií]ll[ií][aá]ms' OR  vete.ruts::text ~* 'w[ií]ll[ií][aá]ms' ) AND (vete.nombres::text ~* 'n[ií]c[oó]l[aá]s' OR  vete.ruts::text ~* 'n[ií]c[oó]l[aá]s' ) AND (vete.nombres::text ~* 'c[oó]nst[aá]nt[ií]n' OR  vete.ruts::text ~* 'c[oó]nst[aá]nt[ií]n' )  			  
