COPY (SELECT et.id,to_char(fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
					...
					WHERE ete.id_exam_term=et.id ) AS estudiantes,(SELECT array_agg(a.carrera_actual)
                        FROM examenes_terminales_estudiantes AS ete 
                        LEFT JOIN alumnos AS a ON a.id=ete.id_alumno
                        WHERE ete.id_exam_term=et.id ) AS carrera_est
				  ...
				  WHERE true  AND (date_part('year',et.fecha_examen) = 2023)  AND et.fecha_examen::date BETWEEN '2023-04-01'::date AND '2024-03-22'::date AND (3 = ANY (SELECT array_agg(a.carrera_actual)
                        FROM examenes_terminales_estudiantes AS ete 
                        LEFT JOIN alumnos AS a ON a.id=ete.id_alumno
                        WHERE ete.id_exam_term=et.id ) )  			  
