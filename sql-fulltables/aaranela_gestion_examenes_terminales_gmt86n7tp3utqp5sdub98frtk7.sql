COPY (SELECT et.id,to_char(et.fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
				  ...
				  WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[aá]l[aá]n' OR  a.rut ~* 'c[aá]t[aá]l[aá]n' OR  text(a.id) ~* 'c[aá]t[aá]l[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ndr[aá]d[eé]' OR  a.rut ~* '[aá]ndr[aá]d[eé]' OR  text(a.id) ~* '[aá]ndr[aá]d[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]st[aá]v[oó]' OR  a.rut ~* 'g[uú]st[aá]v[oó]' OR  text(a.id) ~* 'g[uú]st[aá]v[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]j[aá]ndr[oó]' OR  a.rut ~* '[aá]l[eé]j[aá]ndr[oó]' OR  text(a.id) ~* '[aá]l[eé]j[aá]ndr[oó]' )  			  
