COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[aá]f[aá][eé]l' OR  pap.rut ~* 'r[aá]f[aá][eé]l' OR  text(pap.id) ~* 'r[aá]f[aá][eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[eé]s[uú]s' OR  pap.rut ~* 'j[eé]s[uú]s' OR  text(pap.id) ~* 'j[eé]s[uú]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá][eé]rt[eé]n' OR  pap.rut ~* 'm[aá][eé]rt[eé]n' OR  text(pap.id) ~* 'm[aá][eé]rt[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]c[aá]s' OR  pap.rut ~* 'j[aá]c[aá]s' OR  text(pap.id) ~* 'j[aá]c[aá]s' ) 
