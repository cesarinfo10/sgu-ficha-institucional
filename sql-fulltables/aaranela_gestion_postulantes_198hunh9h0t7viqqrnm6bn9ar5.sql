COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]ld[ií]v[ií][aá]' OR  pap.rut ~* 'v[aá]ld[ií]v[ií][aá]' OR  text(pap.id) ~* 'v[aá]ld[ií]v[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]st[uú]d[ií]ll[oó]' OR  pap.rut ~* '[aá]st[uú]d[ií]ll[oó]' OR  text(pap.id) ~* '[aá]st[uú]d[ií]ll[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'chr[ií]st[ií][aá]n' OR  pap.rut ~* 'chr[ií]st[ií][aá]n' OR  text(pap.id) ~* 'chr[ií]st[ií][aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]nt[oó]n[ií]' OR  pap.rut ~* '[aá]nt[oó]n[ií]' OR  text(pap.id) ~* '[aá]nt[oó]n[ií]' ) 
