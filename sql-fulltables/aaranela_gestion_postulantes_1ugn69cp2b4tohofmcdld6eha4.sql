COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE true  AND (pap.cohorte='2023')  AND (pap.semestre_cohorte='1')  AND (pap.estado_carpeta_doctos='Vacía')  AND vcp.estado = 'E' AND ('POST-GD' IN (c1.regimen,c2.regimen,c3.regimen,c4.regimen,c5.regimen,c6.regimen))  AND (a.id IS NOT NULL)AND (vca.rut IS NOT NULL) AND ('POST-GD' IN (SELECT regimen FROM carreras WHERE id=(SELECT id_carrera FROM finanzas.contratos WHERE estado IS NOT NULL AND ano=2023 AND id_pap=pap.id LIMIT 1)))
