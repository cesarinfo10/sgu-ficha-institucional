COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE true AND (151 IN (pap.carrera1_post,pap.carrera2_post,pap.carrera3_post))  AND (pap.cohorte='2023')  AND vcp.estado = 'E' AND ('DIP-D' IN (c1.regimen,c2.regimen,c3.regimen,c4.regimen,c5.regimen,c6.regimen))  AND (a.id IS NULL)
