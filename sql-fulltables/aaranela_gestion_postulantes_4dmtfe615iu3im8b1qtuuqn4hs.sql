COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'q[uú][eé]z[aá]d[aá]' OR  pap.rut ~* 'q[uú][eé]z[aá]d[aá]' OR  text(pap.id) ~* 'q[uú][eé]z[aá]d[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[oó]t[oó]' OR  pap.rut ~* 's[oó]t[oó]' OR  text(pap.id) ~* 's[oó]t[oó]' ) 
