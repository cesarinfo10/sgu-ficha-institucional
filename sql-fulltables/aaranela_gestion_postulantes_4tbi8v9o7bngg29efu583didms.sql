COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  pap.rut ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  text(pap.id) ~* 'b[uú]st[aá]m[aá]nt[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[oó]t[oó]' OR  pap.rut ~* 's[oó]t[oó]' OR  text(pap.id) ~* 's[oó]t[oó]' ) 
