COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[uú]ñ[oó]z' OR  pap.rut ~* 'm[uú]ñ[oó]z' OR  text(pap.id) ~* 'm[uú]ñ[oó]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]d[aá]l' OR  pap.rut ~* 'v[ií]d[aá]l' OR  text(pap.id) ~* 'v[ií]d[aá]l' ) 
