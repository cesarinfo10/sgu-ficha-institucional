COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]r[oó]' OR  pap.rut ~* 't[oó]r[oó]' OR  text(pap.id) ~* 't[oó]r[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lv[eé][aá]l' OR  pap.rut ~* '[aá]lv[eé][aá]l' OR  text(pap.id) ~* '[aá]lv[eé][aá]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rc[ií][aá]' OR  pap.rut ~* 'm[aá]rc[ií][aá]' OR  text(pap.id) ~* 'm[aá]rc[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ndr[eé][aá]' OR  pap.rut ~* '[aá]ndr[eé][aá]' OR  text(pap.id) ~* '[aá]ndr[eé][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
