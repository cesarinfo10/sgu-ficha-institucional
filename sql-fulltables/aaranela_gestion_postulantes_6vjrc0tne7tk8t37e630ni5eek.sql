COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[ií]gl[ií][aá]' OR  pap.rut ~* 'g[ií]gl[ií][aá]' OR  text(pap.id) ~* 'g[ií]gl[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]l' OR  pap.rut ~* 'd[eé]l' OR  text(pap.id) ~* 'd[eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[ií]l[aá]r' OR  pap.rut ~* 'p[ií]l[aá]r' OR  text(pap.id) ~* 'p[ií]l[aá]r' ) 
