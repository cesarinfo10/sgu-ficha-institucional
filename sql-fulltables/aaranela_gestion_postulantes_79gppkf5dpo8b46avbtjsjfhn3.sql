COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]v[aá]l[oó]s' OR  pap.rut ~* '[aá]v[aá]l[oó]s' OR  text(pap.id) ~* '[aá]v[aá]l[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé]t[eé]l[ií][eé]r' OR  pap.rut ~* 'l[eé]t[eé]l[ií][eé]r' OR  text(pap.id) ~* 'l[eé]t[eé]l[ií][eé]r' ) 
