COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]rg[eé]' OR  pap.rut ~* 'j[oó]rg[eé]' OR  text(pap.id) ~* 'j[oó]rg[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]r[eé]d[eé]s' OR  pap.rut ~* 'p[aá]r[eé]d[eé]s' OR  text(pap.id) ~* 'p[aá]r[eé]d[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]nd[eé]z' OR  pap.rut ~* 'm[eé]nd[eé]z' OR  text(pap.id) ~* 'm[eé]nd[eé]z' ) 
