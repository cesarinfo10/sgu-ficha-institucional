COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]rc[ií][aá]' OR  pap.rut ~* 'g[aá]rc[ií][aá]' OR  text(pap.id) ~* 'g[aá]rc[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  pap.rut ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  text(pap.id) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[eé]n[eé]s[ií]s' OR  pap.rut ~* 'g[eé]n[eé]s[ií]s' OR  text(pap.id) ~* 'g[eé]n[eé]s[ií]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]l' OR  pap.rut ~* 'd[eé]l' OR  text(pap.id) ~* 'd[eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rm[eé]n' OR  pap.rut ~* 'c[aá]rm[eé]n' OR  text(pap.id) ~* 'c[aá]rm[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]st[eé]r' OR  pap.rut ~* '[eé]st[eé]r' OR  text(pap.id) ~* '[eé]st[eé]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
