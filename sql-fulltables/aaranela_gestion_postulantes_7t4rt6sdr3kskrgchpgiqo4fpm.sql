COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[uú]l[ií][oó]' OR  pap.rut ~* 'j[uú]l[ií][oó]' OR  text(pap.id) ~* 'j[uú]l[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[eé]s[aá]r' OR  pap.rut ~* 'c[eé]s[aá]r' OR  text(pap.id) ~* 'c[eé]s[aá]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]st[ií]ll[oó]' OR  pap.rut ~* 'c[aá]st[ií]ll[oó]' OR  text(pap.id) ~* 'c[aá]st[ií]ll[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'h[eé]rr[eé]r[aá]' OR  pap.rut ~* 'h[eé]rr[eé]r[aá]' OR  text(pap.id) ~* 'h[eé]rr[eé]r[aá]' ) 
