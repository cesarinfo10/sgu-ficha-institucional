COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  pap.rut ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  text(pap.id) ~* 'n[aá]v[aá]rr[eé]t[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rv[aá]j[aá]l' OR  pap.rut ~* 'c[aá]rv[aá]j[aá]l' OR  text(pap.id) ~* 'c[aá]rv[aá]j[aá]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
