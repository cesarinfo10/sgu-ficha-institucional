COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'ch[ií][aá]r[aá]' OR  pap.rut ~* 'ch[ií][aá]r[aá]' OR  text(pap.id) ~* 'ch[ií][aá]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]ll[aá]v[aá]ll[eé]' OR  pap.rut ~* 'd[aá]ll[aá]v[aá]ll[eé]' OR  text(pap.id) ~* 'd[aá]ll[aá]v[aá]ll[eé]' ) 
