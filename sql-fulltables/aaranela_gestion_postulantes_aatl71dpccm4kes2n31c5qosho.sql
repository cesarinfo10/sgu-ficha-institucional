COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  pap.rut ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  text(pap.id) ~* 'v[aá]l[eé]nt[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[uú][eé]rr[eé]r[oó]' OR  pap.rut ~* 'g[uú][eé]rr[eé]r[oó]' OR  text(pap.id) ~* 'g[uú][eé]rr[eé]r[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rr[aá]sc[oó]' OR  pap.rut ~* 'c[aá]rr[aá]sc[oó]' OR  text(pap.id) ~* 'c[aá]rr[aá]sc[oó]' ) 
