COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 't[eé]j[oó]' OR  pap.rut ~* 't[eé]j[oó]' OR  text(pap.id) ~* 't[eé]j[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]l[oó]' OR  pap.rut ~* 'm[eé]l[oó]' OR  text(pap.id) ~* 'm[eé]l[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'st[eé]ph[aá]n[ií][eé]' OR  pap.rut ~* 'st[eé]ph[aá]n[ií][eé]' OR  text(pap.id) ~* 'st[eé]ph[aá]n[ií][eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]sm[ií]n[eé]' OR  pap.rut ~* 'j[aá]sm[ií]n[eé]' OR  text(pap.id) ~* 'j[aá]sm[ií]n[eé]' ) 
