COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[aá]h[aá]m[oó]nd[eé]' OR  pap.rut ~* 'b[aá]h[aá]m[oó]nd[eé]' OR  text(pap.id) ~* 'b[aá]h[aá]m[oó]nd[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[oó]n[oó]s[oó]' OR  pap.rut ~* 'd[oó]n[oó]s[oó]' OR  text(pap.id) ~* 'd[oó]n[oó]s[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
