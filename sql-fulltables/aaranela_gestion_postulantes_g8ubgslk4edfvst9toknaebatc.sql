COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]n[uú][eé]l' OR  pap.rut ~* 'm[aá]n[uú][eé]l' OR  text(pap.id) ~* 'm[aá]n[uú][eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'z[aá]mbr[aá]n[oó]' OR  pap.rut ~* 'z[aá]mbr[aá]n[oó]' OR  text(pap.id) ~* 'z[aá]mbr[aá]n[oó]' ) 
