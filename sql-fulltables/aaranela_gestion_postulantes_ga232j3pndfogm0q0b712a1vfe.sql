COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá][eé]z' OR  pap.rut ~* 's[aá][eé]z' OR  text(pap.id) ~* 's[aá][eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[oó]p[eé]z' OR  pap.rut ~* 'l[oó]p[eé]z' OR  text(pap.id) ~* 'l[oó]p[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]l[eé]n' OR  pap.rut ~* 'b[eé]l[eé]n' OR  text(pap.id) ~* 'b[eé]l[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rg[oó]' OR  pap.rut ~* 'm[aá]rg[oó]' OR  text(pap.id) ~* 'm[aá]rg[oó]' ) 
