COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ng[eé]l[aá]' OR  pap.rut ~* '[aá]ng[eé]l[aá]' OR  text(pap.id) ~* '[aá]ng[eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  pap.rut ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  text(pap.id) ~* 'n[aá]v[aá]rr[eé]t[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]n[eé]g[aá]s' OR  pap.rut ~* 'v[eé]n[eé]g[aá]s' OR  text(pap.id) ~* 'v[eé]n[eé]g[aá]s' ) 
