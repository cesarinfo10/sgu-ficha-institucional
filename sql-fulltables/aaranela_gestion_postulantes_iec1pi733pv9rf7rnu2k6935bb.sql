COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'q[uú][eé]l[eé]mp[aá]n' OR  pap.rut ~* 'q[uú][eé]l[eé]mp[aá]n' OR  text(pap.id) ~* 'q[uú][eé]l[eé]mp[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé]v[ií]l' OR  pap.rut ~* 'l[eé]v[ií]l' OR  text(pap.id) ~* 'l[eé]v[ií]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]s[eé]' OR  pap.rut ~* 'j[oó]s[eé]' OR  text(pap.id) ~* 'j[oó]s[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]g[uú]st[ií]n' OR  pap.rut ~* '[aá]g[uú]st[ií]n' OR  text(pap.id) ~* '[aá]g[uú]st[ií]n' ) 
