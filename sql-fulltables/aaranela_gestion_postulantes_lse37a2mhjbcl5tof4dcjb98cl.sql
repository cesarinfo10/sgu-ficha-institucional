COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]m[ií]' OR  pap.rut ~* 'd[eé]m[ií]' OR  text(pap.id) ~* 'd[eé]m[ií]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá][ií]l[ií]n' OR  pap.rut ~* '[aá][ií]l[ií]n' OR  text(pap.id) ~* '[aá][ií]l[ií]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]g[uú]rt[oó]' OR  pap.rut ~* '[aá]g[uú]rt[oó]' OR  text(pap.id) ~* '[aá]g[uú]rt[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]y[aá]rz[uú]n' OR  pap.rut ~* '[oó]y[aá]rz[uú]n' OR  text(pap.id) ~* '[oó]y[aá]rz[uú]n' ) 
