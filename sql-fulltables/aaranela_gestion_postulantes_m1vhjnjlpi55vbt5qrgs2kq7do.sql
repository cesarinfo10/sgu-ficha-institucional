COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE true  AND (pap.cohorte='2023')  AND (pap.semestre_cohorte='2')  AND (pap.estado_carpeta_doctos='Completa')  AND vcp.estado = 'E' AND ('PRE-D' IN (c1.regimen,c2.regimen,c3.regimen,c4.regimen,c5.regimen,c6.regimen))  AND (a.id IS NULL)AND (vca.rut IS NOT NULL) AND ('PRE-D' IN (SELECT regimen FROM carreras WHERE id=(SELECT id_carrera FROM finanzas.contratos WHERE estado IS NOT NULL AND ano=2023 AND id_pap=pap.id LIMIT 1)))
