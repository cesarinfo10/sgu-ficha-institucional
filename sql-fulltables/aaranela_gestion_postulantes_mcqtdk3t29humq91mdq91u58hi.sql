COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[ií]d' OR  pap.rut ~* 'c[ií]d' OR  text(pap.id) ~* 'c[ií]d' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[eé]r[eé]z' OR  pap.rut ~* 'p[eé]r[eé]z' OR  text(pap.id) ~* 'p[eé]r[eé]z' ) 
