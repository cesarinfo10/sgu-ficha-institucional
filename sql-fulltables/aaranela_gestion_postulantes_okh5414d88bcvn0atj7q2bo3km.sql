COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú][ií]s' OR  pap.rut ~* 'l[uú][ií]s' OR  text(pap.id) ~* 'l[uú][ií]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[ií]lv[aá]' OR  pap.rut ~* 's[ií]lv[aá]' OR  text(pap.id) ~* 's[ií]lv[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]n[eé]g[aá]s' OR  pap.rut ~* 'v[eé]n[eé]g[aá]s' OR  text(pap.id) ~* 'v[eé]n[eé]g[aá]s' ) 
