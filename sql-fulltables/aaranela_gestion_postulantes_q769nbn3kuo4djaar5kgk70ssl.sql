COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]rn[aá]l[eé]s' OR  pap.rut ~* 'b[eé]rn[aá]l[eé]s' OR  text(pap.id) ~* 'b[eé]rn[aá]l[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]rn[aá]l[eé]s' OR  pap.rut ~* 'b[eé]rn[aá]l[eé]s' OR  text(pap.id) ~* 'b[eé]rn[aá]l[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]l[oó]m[aá]' OR  pap.rut ~* 'p[aá]l[oó]m[aá]' OR  text(pap.id) ~* 'p[aá]l[oó]m[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]gn[aá]c[ií][aá]' OR  pap.rut ~* '[ií]gn[aá]c[ií][aá]' OR  text(pap.id) ~* '[ií]gn[aá]c[ií][aá]' ) 
