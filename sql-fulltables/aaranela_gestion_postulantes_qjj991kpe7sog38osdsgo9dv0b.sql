COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rm[eé]n' OR  pap.rut ~* 'c[aá]rm[eé]n' OR  text(pap.id) ~* 'c[aá]rm[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]' OR  pap.rut ~* 'd[eé]' OR  text(pap.id) ~* 'd[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]s' OR  pap.rut ~* 'l[aá]s' OR  text(pap.id) ~* 'l[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]rc[eé]d[eé]s' OR  pap.rut ~* 'm[eé]rc[eé]d[eé]s' OR  text(pap.id) ~* 'm[eé]rc[eé]d[eé]s' ) 
