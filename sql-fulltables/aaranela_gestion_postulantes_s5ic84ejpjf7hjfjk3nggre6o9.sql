COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[uú]rr[uú]t[ií][aá]' OR  pap.rut ~* '[uú]rr[uú]t[ií][aá]' OR  text(pap.id) ~* '[uú]rr[uú]t[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[ií]f[uú][eé]nt[eé]s' OR  pap.rut ~* 'c[ií]f[uú][eé]nt[eé]s' OR  text(pap.id) ~* 'c[ií]f[uú][eé]nt[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
