COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]rn[aá]nd[eé]z' OR  pap.rut ~* 'f[eé]rn[aá]nd[eé]z' OR  text(pap.id) ~* 'f[eé]rn[aá]nd[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'fl[oó]r[eé]s' OR  pap.rut ~* 'fl[oó]r[eé]s' OR  text(pap.id) ~* 'fl[oó]r[eé]s' ) 
