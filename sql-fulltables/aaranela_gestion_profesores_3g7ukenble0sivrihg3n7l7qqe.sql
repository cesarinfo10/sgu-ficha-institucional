COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá][uú]r[ií]c[ií][oó]'  OR u.rut ~* 'm[aá][uú]r[ií]c[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]sp[ií]n[oó]s[aá]'  OR u.rut ~* '[eé]sp[ií]n[oó]s[aá]') 
