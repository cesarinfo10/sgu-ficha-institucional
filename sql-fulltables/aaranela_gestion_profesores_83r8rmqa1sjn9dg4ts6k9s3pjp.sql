COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]lv[aá]r[oó]'  OR u.rut ~* '[aá]lv[aá]r[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[ií]gn[aá]c[ií][oó]'  OR u.rut ~* '[ií]gn[aá]c[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[eé][aá]s'  OR u.rut ~* 'v[eé][aá]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'z[uú]r[ií]t[aá]'  OR u.rut ~* 'z[uú]r[ií]t[aá]') 
