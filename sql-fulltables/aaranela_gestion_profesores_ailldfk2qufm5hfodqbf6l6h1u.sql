COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'g[aá]br[ií][eé]l'  OR u.rut ~* 'g[aá]br[ií][eé]l')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[eé]l[ií]s'  OR u.rut ~* 'c[eé]l[ií]s')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
