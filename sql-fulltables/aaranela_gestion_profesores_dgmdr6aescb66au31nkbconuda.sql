COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'd[aá]v[ií]d'  OR u.rut ~* 'd[aá]v[ií]d')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[oó]ntr[eé]r[aá]s'  OR u.rut ~* 'c[oó]ntr[eé]r[aá]s') 
