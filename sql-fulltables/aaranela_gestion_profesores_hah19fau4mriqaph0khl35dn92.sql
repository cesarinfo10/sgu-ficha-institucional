COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'r[ií]c[aá]rd[oó]'  OR u.rut ~* 'r[ií]c[aá]rd[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]rq[uú][eé]z'  OR u.rut ~* 'm[aá]rq[uú][eé]z') 
