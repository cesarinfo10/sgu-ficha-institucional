COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'n[ií]c[oó]l[aá]s'  OR u.rut ~* 'n[ií]c[oó]l[aá]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'b[aá]rr[ií][eé]nt[oó]s'  OR u.rut ~* 'b[aá]rr[ií][eé]nt[oó]s') 
