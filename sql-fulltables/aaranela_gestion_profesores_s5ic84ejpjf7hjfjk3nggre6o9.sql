COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[uú]rr[uú]t[ií][aá]'  OR u.rut ~* '[uú]rr[uú]t[ií][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[ií]f[uú][eé]nt[eé]s'  OR u.rut ~* 'c[ií]f[uú][eé]nt[eé]s') 
