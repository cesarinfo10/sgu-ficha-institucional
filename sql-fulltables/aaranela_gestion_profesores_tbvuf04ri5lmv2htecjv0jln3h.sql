COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'fr[aá]nc[ií]sc[oó]'  OR u.rut ~* 'fr[aá]nc[ií]sc[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]ld[eé]r[oó]n'  OR u.rut ~* 'c[aá]ld[eé]r[oó]n')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[uú]j[aá]d[aá]s'  OR u.rut ~* 'p[uú]j[aá]d[aá]s') 
