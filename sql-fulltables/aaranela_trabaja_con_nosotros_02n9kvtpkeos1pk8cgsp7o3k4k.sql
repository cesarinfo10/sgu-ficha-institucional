COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'c[aá]r[oó]l[ií]n[aá]') OR (lower(pp.apellidos) ~* 'c[aá]r[oó]l[ií]n[aá]') OR (lower(pp.rut) ~* 'c[aá]r[oó]l[ií]n[aá]') AND (lower(pp.nombres) ~* 'fr[aá]nc[ií]sc[aá]') OR (lower(pp.apellidos) ~* 'fr[aá]nc[ií]sc[aá]') OR (lower(pp.rut) ~* 'fr[aá]nc[ií]sc[aá]') AND (lower(pp.nombres) ~* 'c[oó]rt[eé]s') OR (lower(pp.apellidos) ~* 'c[oó]rt[eé]s') OR (lower(pp.rut) ~* 'c[oó]rt[eé]s') AND (lower(pp.nombres) ~* 'p[ií]n[oó]') OR (lower(pp.apellidos) ~* 'p[ií]n[oó]') OR (lower(pp.rut) ~* 'p[ií]n[oó]')
