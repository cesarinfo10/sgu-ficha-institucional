COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'r[eé]y[eé]s') OR (lower(pp.apellidos) ~* 'r[eé]y[eé]s') OR (lower(pp.rut) ~* 'r[eé]y[eé]s') AND (lower(pp.nombres) ~* 't[oó]rr[eé]s') OR (lower(pp.apellidos) ~* 't[oó]rr[eé]s') OR (lower(pp.rut) ~* 't[oó]rr[eé]s')
