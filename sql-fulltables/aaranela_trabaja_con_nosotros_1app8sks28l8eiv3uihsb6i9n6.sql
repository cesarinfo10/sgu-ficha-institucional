COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'r[ií]v[eé]r[aá]') OR (lower(pp.apellidos) ~* 'r[ií]v[eé]r[aá]') OR (lower(pp.rut) ~* 'r[ií]v[eé]r[aá]') AND (lower(pp.nombres) ~* 'm[eé]d[ií]n[aá]') OR (lower(pp.apellidos) ~* 'm[eé]d[ií]n[aá]') OR (lower(pp.rut) ~* 'm[eé]d[ií]n[aá]')
