COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'p[uú][eé]nt[eé]s') OR (lower(pp.apellidos) ~* 'p[uú][eé]nt[eé]s') OR (lower(pp.rut) ~* 'p[uú][eé]nt[eé]s') AND (lower(pp.nombres) ~* 'r[eé]y[eé]s') OR (lower(pp.apellidos) ~* 'r[eé]y[eé]s') OR (lower(pp.rut) ~* 'r[eé]y[eé]s')
