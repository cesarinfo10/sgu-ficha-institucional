COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'c[aá]rr[aá]sc[oó]') OR (lower(pp.apellidos) ~* 'c[aá]rr[aá]sc[oó]') OR (lower(pp.rut) ~* 'c[aá]rr[aá]sc[oó]') AND (lower(pp.nombres) ~* 'l[uú]n[aá]') OR (lower(pp.apellidos) ~* 'l[uú]n[aá]') OR (lower(pp.rut) ~* 'l[uú]n[aá]')
