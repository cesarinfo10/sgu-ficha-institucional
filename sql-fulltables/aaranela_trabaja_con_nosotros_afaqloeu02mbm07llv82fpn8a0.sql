COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'r[oó]x[aá]n[aá]') OR (lower(pp.apellidos) ~* 'r[oó]x[aá]n[aá]') OR (lower(pp.rut) ~* 'r[oó]x[aá]n[aá]') AND (lower(pp.nombres) ~* '[aá]ndr[eé][aá]') OR (lower(pp.apellidos) ~* '[aá]ndr[eé][aá]') OR (lower(pp.rut) ~* '[aá]ndr[eé][aá]') AND (lower(pp.nombres) ~* '[eé]sp[ií]n[oó]z[aá]') OR (lower(pp.apellidos) ~* '[eé]sp[ií]n[oó]z[aá]') OR (lower(pp.rut) ~* '[eé]sp[ií]n[oó]z[aá]') AND (lower(pp.nombres) ~* 's[aá]nch[eé]z') OR (lower(pp.apellidos) ~* 's[aá]nch[eé]z') OR (lower(pp.rut) ~* 's[aá]nch[eé]z')
