COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'j[aá]v[ií][eé]r[aá]') OR (lower(pp.apellidos) ~* 'j[aá]v[ií][eé]r[aá]') OR (lower(pp.rut) ~* 'j[aá]v[ií][eé]r[aá]') AND (lower(pp.nombres) ~* 'pl[aá]z[aá]') OR (lower(pp.apellidos) ~* 'pl[aá]z[aá]') OR (lower(pp.rut) ~* 'pl[aá]z[aá]') AND (lower(pp.nombres) ~* 'b[aá]rr[ií]g[aá]') OR (lower(pp.apellidos) ~* 'b[aá]rr[ií]g[aá]') OR (lower(pp.rut) ~* 'b[aá]rr[ií]g[aá]')
