COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'd[eé][ií]n[oó]') OR (lower(pp.apellidos) ~* 'd[eé][ií]n[oó]') OR (lower(pp.rut) ~* 'd[eé][ií]n[oó]') AND (lower(pp.nombres) ~* 'd[uú]r[aá]n') OR (lower(pp.apellidos) ~* 'd[uú]r[aá]n') OR (lower(pp.rut) ~* 'd[uú]r[aá]n')
