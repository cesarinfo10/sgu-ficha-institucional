COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'v[ií]ct[oó]r') OR (lower(pp.apellidos) ~* 'v[ií]ct[oó]r') OR (lower(pp.rut) ~* 'v[ií]ct[oó]r') AND (lower(pp.nombres) ~* 's[ií]lv[aá]') OR (lower(pp.apellidos) ~* 's[ií]lv[aá]') OR (lower(pp.rut) ~* 's[ií]lv[aá]')
