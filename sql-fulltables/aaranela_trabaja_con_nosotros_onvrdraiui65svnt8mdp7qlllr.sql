COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 's[ií]lv[aá]') OR (lower(pp.apellidos) ~* 's[ií]lv[aá]') OR (lower(pp.rut) ~* 's[ií]lv[aá]') AND (lower(pp.nombres) ~* '[aá]p[aá]bl[aá]z[aá]') OR (lower(pp.apellidos) ~* '[aá]p[aá]bl[aá]z[aá]') OR (lower(pp.rut) ~* '[aá]p[aá]bl[aá]z[aá]')
