COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'm[aá]rt[ií]n[eé]z') OR (lower(pp.apellidos) ~* 'm[aá]rt[ií]n[eé]z') OR (lower(pp.rut) ~* 'm[aá]rt[ií]n[eé]z') AND (lower(pp.nombres) ~* '[eé]sp[ií]n[oó]z[aá]') OR (lower(pp.apellidos) ~* '[eé]sp[ií]n[oó]z[aá]') OR (lower(pp.rut) ~* '[eé]sp[ií]n[oó]z[aá]')
