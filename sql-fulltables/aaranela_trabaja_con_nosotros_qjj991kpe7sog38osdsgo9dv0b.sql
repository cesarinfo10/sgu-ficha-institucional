COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'c[aá]r[oó]l[ií]n[aá]') OR (lower(pp.apellidos) ~* 'c[aá]r[oó]l[ií]n[aá]') OR (lower(pp.rut) ~* 'c[aá]r[oó]l[ií]n[aá]') AND (lower(pp.nombres) ~* 's[eé]g[uú][eé]l') OR (lower(pp.apellidos) ~* 's[eé]g[uú][eé]l') OR (lower(pp.rut) ~* 's[eé]g[uú][eé]l')
