COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'n[aá]v[aá]rr[oó]') OR (lower(pp.apellidos) ~* 'n[aá]v[aá]rr[oó]') OR (lower(pp.rut) ~* 'n[aá]v[aá]rr[oó]') AND (lower(pp.nombres) ~* 't[aá]p[ií][aá]') OR (lower(pp.apellidos) ~* 't[aá]p[ií][aá]') OR (lower(pp.rut) ~* 't[aá]p[ií][aá]')
