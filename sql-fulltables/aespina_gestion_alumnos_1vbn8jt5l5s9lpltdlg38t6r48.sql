COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN alumnos_sies AS al_sies ON (al_sies.rut=a.rut AND al_sies.regimen='PRE' AND al_sies.ano=2021)
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.estado = '1') AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2))  AND al_sies.rut IS NOT NULL 
