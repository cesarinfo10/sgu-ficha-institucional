COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN alumnos_sies AS al_sies ON (al_sies.rut=a.rut AND al_sies.regimen='PRE' AND al_sies.ano=2023)
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2023' ) AND (a.estado = '1') AND (a.moroso_financiero = 'f') AND (a.admision IN ('1','3')) AND (c.regimen = 'PRE')  AND benef_fiscal IS NOT NULL AND (a.id NOT IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1))  AND al_sies.rut IS NOT NULL 
