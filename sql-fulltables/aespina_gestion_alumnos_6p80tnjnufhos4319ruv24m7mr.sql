COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l[oó]' OR  a.rut ~* '[aá]ng[eé]l[oó]' OR  lower(a.email) ~* '[aá]ng[eé]l[oó]' OR  text(a.id) ~* '[aá]ng[eé]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[eé]rnn[aá]d[eé]s' OR  a.rut ~* 'f[eé]rnn[aá]d[eé]s' OR  lower(a.email) ~* 'f[eé]rnn[aá]d[eé]s' OR  text(a.id) ~* 'f[eé]rnn[aá]d[eé]s' ) 
