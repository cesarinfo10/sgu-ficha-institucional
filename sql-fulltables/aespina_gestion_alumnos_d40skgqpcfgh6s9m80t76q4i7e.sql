COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]lv[ií][aá]' OR  a.rut ~* 's[ií]lv[ií][aá]' OR  lower(a.email) ~* 's[ií]lv[ií][aá]' OR  text(a.id) ~* 's[ií]lv[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]str[aá]d[aá]' OR  a.rut ~* '[eé]str[aá]d[aá]' OR  lower(a.email) ~* '[eé]str[aá]d[aá]' OR  text(a.id) ~* '[eé]str[aá]d[aá]' ) 
