COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]s[aá]l[eé]' OR  a.rut ~* 'r[oó]s[aá]l[eé]' OR  lower(a.email) ~* 'r[oó]s[aá]l[eé]' OR  text(a.id) ~* 'r[oó]s[aá]l[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[aá]mp[aá]' OR  a.rut ~* '[aá]n[aá]mp[aá]' OR  lower(a.email) ~* '[aá]n[aá]mp[aá]' OR  text(a.id) ~* '[aá]n[aá]mp[aá]' ) 
