COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][oó]p[oó]ld[oó]' OR  a.rut ~* 'l[eé][oó]p[oó]ld[oó]' OR  lower(a.email) ~* 'l[eé][oó]p[oó]ld[oó]' OR  text(a.id) ~* 'l[eé][oó]p[oó]ld[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][eé]z[aá]d[aá]' OR  a.rut ~* 'q[uú][eé]z[aá]d[aá]' OR  lower(a.email) ~* 'q[uú][eé]z[aá]d[aá]' OR  text(a.id) ~* 'q[uú][eé]z[aá]d[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[uú]z' OR  a.rut ~* 'r[uú]z' OR  lower(a.email) ~* 'r[uú]z' OR  text(a.id) ~* 'r[uú]z' ) 
