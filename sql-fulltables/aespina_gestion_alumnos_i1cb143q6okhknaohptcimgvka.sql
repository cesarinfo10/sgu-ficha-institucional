COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]n[aá]' OR  a.rut ~* 'm[aá]r[ií][aá]n[aá]' OR  lower(a.email) ~* 'm[aá]r[ií][aá]n[aá]' OR  text(a.id) ~* 'm[aá]r[ií][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nt[aá]nd[eé]r' OR  a.rut ~* 's[aá]nt[aá]nd[eé]r' OR  lower(a.email) ~* 's[aá]nt[aá]nd[eé]r' OR  text(a.id) ~* 's[aá]nt[aá]nd[eé]r' ) 
