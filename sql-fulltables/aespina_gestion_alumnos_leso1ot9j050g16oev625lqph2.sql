COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2024 LIMIT 1)=1  THEN 'Si' ELSE 'No' END AS matriculado,(SELECT to_char(max(fecha),'DD/MM/YYYY') FROM matriculas WHERE id_alumno=a.id) AS fecha_ult_mat,
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2023' ) AND (a.admision IN ('1','3')) AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2024))  AND al_sies.rut IS NOT NULL 
