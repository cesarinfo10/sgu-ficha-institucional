COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]m[aá]' OR  a.rut ~* '[eé]m[aá]' OR  lower(a.email) ~* '[eé]m[aá]' OR  text(a.id) ~* '[eé]m[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'bl[eé]r' OR  a.rut ~* 'bl[eé]r' OR  lower(a.email) ~* 'bl[eé]r' OR  text(a.id) ~* 'bl[eé]r' ) 
