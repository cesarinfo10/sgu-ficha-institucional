COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN dae.renovantes_cae AS renov_cae ON (renov_cae.ano=2023 AND renov_cae.rut=split_part(trim(a.rut),'-',1)::int8)
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2023' ) AND (a.estado = '1') AND (a.moroso_financiero = 'f') AND (a.admision IN ('1','3')) AND (c.regimen = 'PRE')  AND renov_cae.rut IS NOT NULL AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=1))  AND al_sies.rut IS NOT NULL 
