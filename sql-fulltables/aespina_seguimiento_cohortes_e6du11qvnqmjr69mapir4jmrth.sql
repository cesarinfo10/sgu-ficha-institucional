COPY (SELECT *,CASE WHEN tasa_riesgo BETWEEN 75 and 100 THEN 'Alto'
                ...
                WHERE true  AND (a.cohorte = '2023') AND (a.semestre_cohorte = 1) AND (a.estado = '1') AND (a.admision IN ('1','3')) AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2))  AND a.rut IN (SELECT rut FROM alumnos_sies WHERE ano=2023 AND regimen='PRE' ) 
