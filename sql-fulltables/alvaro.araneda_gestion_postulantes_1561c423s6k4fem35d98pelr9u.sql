COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]rg[eé]' OR  pap.rut ~* 'j[oó]rg[eé]' OR  text(pap.id) ~* 'j[oó]rg[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'h[eé]rm[oó]s[ií]ll[aá]' OR  pap.rut ~* 'h[eé]rm[oó]s[ií]ll[aá]' OR  text(pap.id) ~* 'h[eé]rm[oó]s[ií]ll[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]r[oó]' OR  pap.rut ~* 'c[aá]r[oó]' OR  text(pap.id) ~* 'c[aá]r[oó]' ) 
