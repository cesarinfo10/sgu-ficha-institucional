COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'fl[oó]r[eé]nc[ií][aá]' OR  pap.rut ~* 'fl[oó]r[eé]nc[ií][aá]' OR  text(pap.id) ~* 'fl[oó]r[eé]nc[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]ll[eé]g[aá]s' OR  pap.rut ~* 'v[ií]ll[eé]g[aá]s' OR  text(pap.id) ~* 'v[ií]ll[eé]g[aá]s' ) 
