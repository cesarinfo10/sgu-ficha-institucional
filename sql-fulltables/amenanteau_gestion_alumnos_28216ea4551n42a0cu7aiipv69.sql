COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lv[aá]r[oó]' OR  a.rut ~* '[aá]lv[aá]r[oó]' OR  lower(a.email) ~* '[aá]lv[aá]r[oó]' OR  text(a.id) ~* '[aá]lv[aá]r[oó]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,150) 
