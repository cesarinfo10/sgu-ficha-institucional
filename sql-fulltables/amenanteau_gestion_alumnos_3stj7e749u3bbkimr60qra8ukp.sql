COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[eé]d[eé]s' OR  a.rut ~* 'fr[eé]d[eé]s' OR  lower(a.email) ~* 'fr[eé]d[eé]s' OR  text(a.id) ~* 'fr[eé]d[eé]s' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
