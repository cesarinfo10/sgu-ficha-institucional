COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[oó]l[aá]nc[oó]' OR  a.rut ~* 'p[oó]l[aá]nc[oó]' OR  lower(a.email) ~* 'p[oó]l[aá]nc[oó]' OR  text(a.id) ~* 'p[oó]l[aá]nc[oó]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,156,159) 
