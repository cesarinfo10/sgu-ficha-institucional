COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[ií][eé]rr[oó]' OR  a.rut ~* 'f[ií][eé]rr[oó]' OR  lower(a.email) ~* 'f[ií][eé]rr[oó]' OR  text(a.id) ~* 'f[ií][eé]rr[oó]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
