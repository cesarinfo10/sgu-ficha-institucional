COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]d[ií]t' OR  a.rut ~* '[eé]d[ií]t' OR  lower(a.email) ~* '[eé]d[ií]t' OR  text(a.id) ~* '[eé]d[ií]t' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
