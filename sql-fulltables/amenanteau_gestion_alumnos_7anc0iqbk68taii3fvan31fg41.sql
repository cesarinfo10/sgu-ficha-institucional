COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  a.rut ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  lower(a.email) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  text(a.id) ~* 'b[uú]st[aá]m[aá]nt[eé]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
