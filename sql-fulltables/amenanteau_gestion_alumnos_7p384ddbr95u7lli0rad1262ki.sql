COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé]m[ií]m[aá]' OR  a.rut ~* 'j[eé]m[ií]m[aá]' OR  lower(a.email) ~* 'j[eé]m[ií]m[aá]' OR  text(a.id) ~* 'j[eé]m[ií]m[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
