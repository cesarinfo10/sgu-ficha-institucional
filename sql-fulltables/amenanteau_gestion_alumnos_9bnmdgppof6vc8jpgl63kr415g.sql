COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]l[ií]z' OR  a.rut ~* 'v[eé]l[ií]z' OR  lower(a.email) ~* 'v[eé]l[ií]z' OR  text(a.id) ~* 'v[eé]l[ií]z' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,156,159) 
