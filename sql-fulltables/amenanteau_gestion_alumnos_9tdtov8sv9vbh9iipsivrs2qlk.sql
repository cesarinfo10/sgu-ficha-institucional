COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]nc[uú]m[ií]' OR  a.rut ~* 'l[uú]nc[uú]m[ií]' OR  lower(a.email) ~* 'l[uú]nc[uú]m[ií]' OR  text(a.id) ~* 'l[uú]nc[uú]m[ií]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
