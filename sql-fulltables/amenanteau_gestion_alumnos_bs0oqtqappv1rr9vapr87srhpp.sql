COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[aá]' OR  a.rut ~* '[aá]n[aá]' OR  lower(a.email) ~* '[aá]n[aá]' OR  text(a.id) ~* '[aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[ií]n[aá]' OR  a.rut ~* 'k[aá]r[ií]n[aá]' OR  lower(a.email) ~* 'k[aá]r[ií]n[aá]' OR  text(a.id) ~* 'k[aá]r[ií]n[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,156,159) 
