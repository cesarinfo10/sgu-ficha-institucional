COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]ct[oó]r' OR  a.rut ~* 'h[eé]ct[oó]r' OR  lower(a.email) ~* 'h[eé]ct[oó]r' OR  text(a.id) ~* 'h[eé]ct[oó]r' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
