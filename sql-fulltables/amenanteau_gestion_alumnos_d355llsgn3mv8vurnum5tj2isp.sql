COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[uú][aá]dr[aá]' OR  a.rut ~* 'c[uú][aá]dr[aá]' OR  lower(a.email) ~* 'c[uú][aá]dr[aá]' OR  text(a.id) ~* 'c[uú][aá]dr[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
