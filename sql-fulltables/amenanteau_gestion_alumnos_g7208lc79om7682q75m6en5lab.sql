COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][eé]z[aá]d[aá]' OR  a.rut ~* 'q[uú][eé]z[aá]d[aá]' OR  lower(a.email) ~* 'q[uú][eé]z[aá]d[aá]' OR  text(a.id) ~* 'q[uú][eé]z[aá]d[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
