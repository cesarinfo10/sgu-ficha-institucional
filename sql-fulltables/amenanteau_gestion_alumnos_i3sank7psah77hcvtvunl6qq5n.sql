COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rr[eé]r[aá]' OR  a.rut ~* 'b[aá]rr[eé]r[aá]' OR  lower(a.email) ~* 'b[aá]rr[eé]r[aá]' OR  text(a.id) ~* 'b[aá]rr[eé]r[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
