COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[aá]v[ií][aá]' OR  a.rut ~* 'f[aá]v[ií][aá]' OR  lower(a.email) ~* 'f[aá]v[ií][aá]' OR  text(a.id) ~* 'f[aá]v[ií][aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,150) 
