COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]ngr[ií]d' OR  a.rut ~* '[ií]ngr[ií]d' OR  lower(a.email) ~* '[ií]ngr[ií]d' OR  text(a.id) ~* '[ií]ngr[ií]d' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
