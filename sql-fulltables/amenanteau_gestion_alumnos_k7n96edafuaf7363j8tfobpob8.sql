COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '18966528' OR  a.rut ~* '18966528' OR  lower(a.email) ~* '18966528' OR  text(a.id) ~* '18966528' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67) 
