COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]x[aá]nd[eé]r' OR  a.rut ~* '[aá]l[eé]x[aá]nd[eé]r' OR  lower(a.email) ~* '[aá]l[eé]x[aá]nd[eé]r' OR  text(a.id) ~* '[aá]l[eé]x[aá]nd[eé]r' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
