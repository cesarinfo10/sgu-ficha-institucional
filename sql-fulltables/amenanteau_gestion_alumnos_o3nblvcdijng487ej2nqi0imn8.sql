COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rn[eé]ll[aá]' OR  a.rut ~* '[oó]rn[eé]ll[aá]' OR  lower(a.email) ~* '[oó]rn[eé]ll[aá]' OR  text(a.id) ~* '[oó]rn[eé]ll[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
