COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]rd[ií]' OR  a.rut ~* 'j[oó]rd[ií]' OR  lower(a.email) ~* 'j[oó]rd[ií]' OR  text(a.id) ~* 'j[oó]rd[ií]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,156,159) 
