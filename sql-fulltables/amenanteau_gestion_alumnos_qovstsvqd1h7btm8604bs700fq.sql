COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]rn[ií]g' OR  a.rut ~* 'p[eé]rn[ií]g' OR  lower(a.email) ~* 'p[eé]rn[ií]g' OR  text(a.id) ~* 'p[eé]rn[ií]g' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
