COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]r[ií]k' OR  a.rut ~* '[eé]r[ií]k' OR  lower(a.email) ~* '[eé]r[ií]k' OR  text(a.id) ~* '[eé]r[ií]k' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
