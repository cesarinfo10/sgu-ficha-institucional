COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]st[oó]n' OR  a.rut ~* 'g[aá]st[oó]n' OR  lower(a.email) ~* 'g[aá]st[oó]n' OR  text(a.id) ~* 'g[aá]st[oó]n' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
