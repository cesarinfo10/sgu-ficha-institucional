COPY (SELECT et.id,to_char(fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
						 ...
						 (SELECT char_comma_sum(upper(a.apellidos)||' '||initcap(a.nombres)||' '||CASE WHEN (SELECT count(id) FROM finanzas.cobros WHERE id_alumno=a.id AND id_glosa IN (47,5,9) AND pagado AND fecha_venc>=now()::date-'6 months'::interval) >= 1 THEN '✅' ELSE '⛔' END) 
				  ...
				  WHERE true  AND (date_part('year',et.fecha_examen) = 2023) AND (et.id_escuela = 12)  AND et.fecha_examen::date BETWEEN '2023-01-01'::date AND '2023-12-31'::date  			  
