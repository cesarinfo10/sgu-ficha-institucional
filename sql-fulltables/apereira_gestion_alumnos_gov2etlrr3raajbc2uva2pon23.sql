COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]m[uú][eé]l' OR  a.rut ~* 's[aá]m[uú][eé]l' OR  lower(a.email) ~* 's[aá]m[uú][eé]l' OR  text(a.id) ~* 's[aá]m[uú][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]lm[aá]' OR  a.rut ~* 'p[aá]lm[aá]' OR  lower(a.email) ~* 'p[aá]lm[aá]' OR  text(a.id) ~* 'p[aá]lm[aá]' ) 
