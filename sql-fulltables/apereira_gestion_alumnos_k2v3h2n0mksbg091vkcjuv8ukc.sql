COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[uú]s[aá]n[aá]' OR  a.rut ~* 's[uú]s[aá]n[aá]' OR  lower(a.email) ~* 's[uú]s[aá]n[aá]' OR  text(a.id) ~* 's[uú]s[aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]sq[uú][eé]z' OR  a.rut ~* 'v[aá]sq[uú][eé]z' OR  lower(a.email) ~* 'v[aá]sq[uú][eé]z' OR  text(a.id) ~* 'v[aá]sq[uú][eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé][aá]s' OR  a.rut ~* 'c[eé][aá]s' OR  lower(a.email) ~* 'c[eé][aá]s' OR  text(a.id) ~* 'c[eé][aá]s' ) 
