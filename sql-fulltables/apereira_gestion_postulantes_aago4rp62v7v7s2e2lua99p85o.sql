COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lfr[eé]d[oó]' OR  pap.rut ~* '[aá]lfr[eé]d[oó]' OR  text(pap.id) ~* '[aá]lfr[eé]d[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]nt[oó]n[ií][oó]' OR  pap.rut ~* '[aá]nt[oó]n[ií][oó]' OR  text(pap.id) ~* '[aá]nt[oó]n[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sp[ií]n[oó]z[aá]' OR  pap.rut ~* '[eé]sp[ií]n[oó]z[aá]' OR  text(pap.id) ~* '[eé]sp[ií]n[oó]z[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]r[aá]l[eé]s' OR  pap.rut ~* 'm[oó]r[aá]l[eé]s' OR  text(pap.id) ~* 'm[oó]r[aá]l[eé]s' ) 
