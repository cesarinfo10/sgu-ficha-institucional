COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]c[ií][oó]' OR  pap.rut ~* 'r[oó]c[ií][oó]' OR  text(pap.id) ~* 'r[oó]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]ntr[eé]r[aá]s' OR  pap.rut ~* 'c[oó]ntr[eé]r[aá]s' OR  text(pap.id) ~* 'c[oó]ntr[eé]r[aá]s' ) 
