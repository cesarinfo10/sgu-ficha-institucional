COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[uú]st[oó]s' OR  pap.rut ~* 'b[uú]st[oó]s' OR  text(pap.id) ~* 'b[uú]st[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]r[aá]' OR  pap.rut ~* 'j[aá]r[aá]' OR  text(pap.id) ~* 'j[aá]r[aá]' ) 
