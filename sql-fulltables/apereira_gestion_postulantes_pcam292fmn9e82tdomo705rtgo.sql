COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nt[ií]n' OR  pap.rut ~* 'v[aá]l[eé]nt[ií]n' OR  text(pap.id) ~* 'v[aá]l[eé]nt[ií]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]r[aá]' OR  pap.rut ~* 'v[eé]r[aá]' OR  text(pap.id) ~* 'v[eé]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nt[eé]s' OR  pap.rut ~* 'f[uú][eé]nt[eé]s' OR  text(pap.id) ~* 'f[uú][eé]nt[eé]s' ) 
