COPY (SELECT * FROM (SELECT *,CASE WHEN tasa_riesgo BETWEEN 75 and 100 THEN 'Alto'
                ...
                WHERE true  AND (a.cohorte = '2023') AND (a.semestre_cohorte = 1) AND (a.estado = '1') AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2)) 
                ORDER BY a.apellidos,a.nombres ) AS al WHERE true ) AS foo ) AS foo WHERE riesgo_desercion = 'Medio Alto'	) to stdout WITH CSV HEADER