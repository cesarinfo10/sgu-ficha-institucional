COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN alumnos_sies AS al_sies ON (al_sies.rut=a.rut AND al_sies.regimen='t' AND al_sies.ano=2022)
                ...
                WHERE true  AND (a.cohorte = '2022' ) AND (a.estado = '1') AND (a.admision IN ('1','3')) AND (a.id NOT IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1))  AND al_sies.rut IS NOT NULL  AND ((SELECT estado 
                          FROM gestion.solicitudes AS sol 
                          LEFT JOIN gestion.solic_tipos AS gst ON gst.id=sol.id_tipo 
                          WHERE id_alumno=a.id AND gst.alias='solic_excep_finan' AND sol.fecha::date>='2021-10-01'
                          ORDER BY sol.fecha DESC
                     ...
                     WHERE id_alumno=a.id and ano=2024
                     ORDER BY fecha_creacion DESC
                     LIMIT 1) IS NULL)  AND a.carrera_actual IN (95,100,2,69,1,19) 
