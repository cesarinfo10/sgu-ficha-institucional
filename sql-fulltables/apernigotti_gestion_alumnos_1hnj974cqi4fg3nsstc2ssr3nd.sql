COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú]l[ií][oó]' OR  a.rut ~* 'j[uú]l[ií][oó]' OR  lower(a.email) ~* 'j[uú]l[ií][oó]' OR  text(a.id) ~* 'j[uú]l[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]v[aá]rr[oó]' OR  a.rut ~* 'n[aá]v[aá]rr[oó]' OR  lower(a.email) ~* 'n[aá]v[aá]rr[oó]' OR  text(a.id) ~* 'n[aá]v[aá]rr[oó]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
