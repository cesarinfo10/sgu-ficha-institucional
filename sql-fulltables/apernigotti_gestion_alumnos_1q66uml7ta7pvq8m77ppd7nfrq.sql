COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]rr[oó][eé]t[aá]' OR  a.rut ~* 'b[eé]rr[oó][eé]t[aá]' OR  lower(a.email) ~* 'b[eé]rr[oó][eé]t[aá]' OR  text(a.id) ~* 'b[eé]rr[oó][eé]t[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
