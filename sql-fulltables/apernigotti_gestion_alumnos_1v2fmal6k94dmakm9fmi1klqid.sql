COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú][aá]n' OR  a.rut ~* 'j[uú][aá]n' OR  lower(a.email) ~* 'j[uú][aá]n' OR  text(a.id) ~* 'j[uú][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií]q[uú][eé][oó]' OR  a.rut ~* 'm[aá]r[ií]q[uú][eé][oó]' OR  lower(a.email) ~* 'm[aá]r[ií]q[uú][eé][oó]' OR  text(a.id) ~* 'm[aá]r[ií]q[uú][eé][oó]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
