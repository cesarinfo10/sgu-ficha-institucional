COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '20576952' OR  a.rut ~* '20576952' OR  lower(a.email) ~* '20576952' OR  text(a.id) ~* '20576952' )  AND a.carrera_actual IN (95,100,2,19,69,1) 
