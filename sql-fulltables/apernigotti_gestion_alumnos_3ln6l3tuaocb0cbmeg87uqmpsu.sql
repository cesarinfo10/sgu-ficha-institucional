COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]c[aá]rd[oó]' OR  a.rut ~* 'r[ií]c[aá]rd[oó]' OR  lower(a.email) ~* 'r[ií]c[aá]rd[oó]' OR  text(a.id) ~* 'r[ií]c[aá]rd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]s[aá]l[eé]s' OR  a.rut ~* 'r[oó]s[aá]l[eé]s' OR  lower(a.email) ~* 'r[oó]s[aá]l[eé]s' OR  text(a.id) ~* 'r[oó]s[aá]l[eé]s' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
