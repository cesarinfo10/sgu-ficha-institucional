COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[uú][eé]l[aá]' OR  a.rut ~* 'r[uú][eé]l[aá]' OR  lower(a.email) ~* 'r[uú][eé]l[aá]' OR  text(a.id) ~* 'r[uú][eé]l[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
