COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][oó]p[oó]ld[oó]' OR  a.rut ~* 'l[eé][oó]p[oó]ld[oó]' OR  lower(a.email) ~* 'l[eé][oó]p[oó]ld[oó]' OR  text(a.id) ~* 'l[eé][oó]p[oó]ld[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]gl[eé]s[ií][aá]s' OR  a.rut ~* '[ií]gl[eé]s[ií][aá]s' OR  lower(a.email) ~* '[ií]gl[eé]s[ií][aá]s' OR  text(a.id) ~* '[ií]gl[eé]s[ií][aá]s' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
