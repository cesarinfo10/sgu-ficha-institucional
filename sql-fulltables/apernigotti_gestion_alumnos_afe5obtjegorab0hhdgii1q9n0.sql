COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]ll[eé]g[oó]s' OR  a.rut ~* 'g[aá]ll[eé]g[oó]s' OR  lower(a.email) ~* 'g[aá]ll[eé]g[oó]s' OR  text(a.id) ~* 'g[aá]ll[eé]g[oó]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]rg[eé]' OR  a.rut ~* 'j[oó]rg[eé]' OR  lower(a.email) ~* 'j[oó]rg[eé]' OR  text(a.id) ~* 'j[oó]rg[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
