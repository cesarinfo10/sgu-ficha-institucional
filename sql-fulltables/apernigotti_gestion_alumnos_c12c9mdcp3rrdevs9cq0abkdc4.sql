COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]sk[aá]' OR  a.rut ~* 'v[aá]l[eé]sk[aá]' OR  lower(a.email) ~* 'v[aá]l[eé]sk[aá]' OR  text(a.id) ~* 'v[aá]l[eé]sk[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[uú]r[aá]n' OR  a.rut ~* 'd[uú]r[aá]n' OR  lower(a.email) ~* 'd[uú]r[aá]n' OR  text(a.id) ~* 'd[uú]r[aá]n' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
