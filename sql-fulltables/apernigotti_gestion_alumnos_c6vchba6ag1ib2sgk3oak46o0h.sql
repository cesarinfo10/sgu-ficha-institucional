COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '13093947' OR  a.rut ~* '13093947' OR  lower(a.email) ~* '13093947' OR  text(a.id) ~* '13093947' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
