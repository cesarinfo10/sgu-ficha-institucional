COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]r[aá]rd' OR  a.rut ~* 'g[eé]r[aá]rd' OR  lower(a.email) ~* 'g[eé]r[aá]rd' OR  text(a.id) ~* 'g[eé]r[aá]rd' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
