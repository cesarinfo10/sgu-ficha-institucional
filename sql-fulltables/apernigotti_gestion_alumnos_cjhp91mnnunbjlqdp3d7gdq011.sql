COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '17923795-4' OR  a.rut ~* '17923795-4' OR  lower(a.email) ~* '17923795-4' OR  text(a.id) ~* '17923795-4' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
