COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]c[oó]l[eé]' OR  a.rut ~* 'n[ií]c[oó]l[eé]' OR  lower(a.email) ~* 'n[ií]c[oó]l[eé]' OR  text(a.id) ~* 'n[ií]c[oó]l[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]r[eé]ll[aá]n[aá]' OR  a.rut ~* '[oó]r[eé]ll[aá]n[aá]' OR  lower(a.email) ~* '[oó]r[eé]ll[aá]n[aá]' OR  text(a.id) ~* '[oó]r[eé]ll[aá]n[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
