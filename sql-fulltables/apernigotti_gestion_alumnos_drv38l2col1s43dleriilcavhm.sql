COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]r[ií]s' OR  a.rut ~* '[ií]r[ií]s' OR  lower(a.email) ~* '[ií]r[ií]s' OR  text(a.id) ~* '[ií]r[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]lm[aá]n' OR  a.rut ~* 's[eé]lm[aá]n' OR  lower(a.email) ~* 's[eé]lm[aá]n' OR  text(a.id) ~* 's[eé]lm[aá]n' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
