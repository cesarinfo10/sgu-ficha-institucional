COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó][ií]s[eé]s' OR  a.rut ~* 'm[oó][ií]s[eé]s' OR  lower(a.email) ~* 'm[oó][ií]s[eé]s' OR  text(a.id) ~* 'm[oó][ií]s[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]nc[ií][aá]' OR  a.rut ~* 'v[aá]l[eé]nc[ií][aá]' OR  lower(a.email) ~* 'v[aá]l[eé]nc[ií][aá]' OR  text(a.id) ~* 'v[aá]l[eé]nc[ií][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
