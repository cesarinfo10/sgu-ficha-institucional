COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '8405594' OR  a.rut ~* '8405594' OR  lower(a.email) ~* '8405594' OR  text(a.id) ~* '8405594' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
