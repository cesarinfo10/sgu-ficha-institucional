COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[aá]y[eé]h[uú][aá]nq[uú][eé]' OR  a.rut ~* 'r[aá]y[eé]h[uú][aá]nq[uú][eé]' OR  lower(a.email) ~* 'r[aá]y[eé]h[uú][aá]nq[uú][eé]' OR  text(a.id) ~* 'r[aá]y[eé]h[uú][aá]nq[uú][eé]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
