COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]b[eé]rt' OR  a.rut ~* 'h[eé]b[eé]rt' OR  lower(a.email) ~* 'h[eé]b[eé]rt' OR  text(a.id) ~* 'h[eé]b[eé]rt' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]sm[eé]r' OR  a.rut ~* '[oó]sm[eé]r' OR  lower(a.email) ~* '[oó]sm[eé]r' OR  text(a.id) ~* '[oó]sm[eé]r' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
