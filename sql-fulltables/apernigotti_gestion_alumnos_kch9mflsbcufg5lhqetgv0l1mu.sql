COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '18134254' OR  a.rut ~* '18134254' OR  lower(a.email) ~* '18134254' OR  text(a.id) ~* '18134254' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
