COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú][ií]ll[eé]rm[oó]' OR  a.rut ~* 'g[uú][ií]ll[eé]rm[oó]' OR  lower(a.email) ~* 'g[uú][ií]ll[eé]rm[oó]' OR  text(a.id) ~* 'g[uú][ií]ll[eé]rm[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]str[oó]' OR  a.rut ~* 'c[aá]str[oó]' OR  lower(a.email) ~* 'c[aá]str[oó]' OR  text(a.id) ~* 'c[aá]str[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
