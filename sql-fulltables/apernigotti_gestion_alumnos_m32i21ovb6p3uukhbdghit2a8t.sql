COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé]rd[aá]' OR  a.rut ~* 'c[eé]rd[aá]' OR  lower(a.email) ~* 'c[eé]rd[aá]' OR  text(a.id) ~* 'c[eé]rd[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé]rd[aá]' OR  a.rut ~* 'c[eé]rd[aá]' OR  lower(a.email) ~* 'c[eé]rd[aá]' OR  text(a.id) ~* 'c[eé]rd[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
