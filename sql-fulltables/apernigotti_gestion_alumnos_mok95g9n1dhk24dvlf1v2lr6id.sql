COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '11751663' OR  a.rut ~* '11751663' OR  lower(a.email) ~* '11751663' OR  text(a.id) ~* '11751663' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
