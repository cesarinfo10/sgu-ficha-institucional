COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[eé]l[ií]p[eé]' OR  a.rut ~* 'f[eé]l[ií]p[eé]' OR  lower(a.email) ~* 'f[eé]l[ií]p[eé]' OR  text(a.id) ~* 'f[eé]l[ií]p[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]g[uú][ií]l[aá]r' OR  a.rut ~* '[aá]g[uú][ií]l[aá]r' OR  lower(a.email) ~* '[aá]g[uú][ií]l[aá]r' OR  text(a.id) ~* '[aá]g[uú][ií]l[aá]r' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
