COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[eé]ls[oó]n' OR  a.rut ~* 'n[eé]ls[oó]n' OR  lower(a.email) ~* 'n[eé]ls[oó]n' OR  text(a.id) ~* 'n[eé]ls[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]nt[oó]y[aá]' OR  a.rut ~* 'm[oó]nt[oó]y[aá]' OR  lower(a.email) ~* 'm[oó]nt[oó]y[aá]' OR  text(a.id) ~* 'm[oó]nt[oó]y[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
