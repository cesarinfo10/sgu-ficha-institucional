COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[eé]rn[aá]nd[oó]' OR  a.rut ~* 'f[eé]rn[aá]nd[oó]' OR  lower(a.email) ~* 'f[eé]rn[aá]nd[oó]' OR  text(a.id) ~* 'f[eé]rn[aá]nd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[ií]f[uú][eé]nt[eé]s' OR  a.rut ~* 'c[ií]f[uú][eé]nt[eé]s' OR  lower(a.email) ~* 'c[ií]f[uú][eé]nt[eé]s' OR  text(a.id) ~* 'c[ií]f[uú][eé]nt[eé]s' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
