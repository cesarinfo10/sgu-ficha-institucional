COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '27147934' OR  a.rut ~* '27147934' OR  lower(a.email) ~* '27147934' OR  text(a.id) ~* '27147934' )  AND a.carrera_actual IN (95,100,2,19,69,1) 
