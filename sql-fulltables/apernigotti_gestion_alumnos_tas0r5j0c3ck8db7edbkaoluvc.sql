COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[aá]ff[oó]' OR  a.rut ~* 'r[aá]ff[oó]' OR  lower(a.email) ~* 'r[aá]ff[oó]' OR  text(a.id) ~* 'r[aá]ff[oó]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
