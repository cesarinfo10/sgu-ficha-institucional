COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '12904887' OR  a.rut ~* '12904887' OR  lower(a.email) ~* '12904887' OR  text(a.id) ~* '12904887' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
