COPY (SELECT et.id,to_char(et.fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
					...
					WHERE ete.id_exam_term=et.id ) AS estudiantes,(SELECT char_comma_sum(lower(a.email)||coalesce(','||nombre_usuario||'@'||'alumni.unc.cl')) 
                          FROM examenes_terminales_estudiantes AS ete 
					      LEFT JOIN alumnos AS a ON a.id=ete.id_alumno
					      LEFT JOIN carreras AS c ON c.id=a.carrera_actual
				  ...
				  WHERE true  AND (et.estado IN ('Aprobado')) AND (date_part('year',et.fecha_examen) = 2023) AND (et.id_escuela = 12)  			  
