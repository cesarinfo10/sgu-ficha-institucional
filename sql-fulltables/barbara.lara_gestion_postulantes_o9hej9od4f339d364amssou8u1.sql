COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[oó]f[ií][aá]' OR  pap.rut ~* 's[oó]f[ií][aá]' OR  text(pap.id) ~* 's[oó]f[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]m[ií]l[ií][aá]' OR  pap.rut ~* '[eé]m[ií]l[ií][aá]' OR  text(pap.id) ~* '[eé]m[ií]l[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
