COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]h[aá]r[aá][ií]m' OR  pap.rut ~* 's[aá]h[aá]r[aá][ií]m' OR  text(pap.id) ~* 's[aá]h[aá]r[aá][ií]m' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá][eé]l' OR  pap.rut ~* 'j[aá][eé]l' OR  text(pap.id) ~* 'j[aá][eé]l' ) 
