COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]r[aá]' OR  a.rut ~* 'l[ií]r[aá]' OR  lower(a.email) ~* 'l[ií]r[aá]' OR  text(a.id) ~* 'l[ií]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[eé][ií]r[aá]' OR  a.rut ~* 'm[oó]r[eé][ií]r[aá]' OR  lower(a.email) ~* 'm[oó]r[eé][ií]r[aá]' OR  text(a.id) ~* 'm[oó]r[eé][ií]r[aá]' ) 
