COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[eé]c[uú]l' OR  a.rut ~* 'n[eé]c[uú]l' OR  lower(a.email) ~* 'n[eé]c[uú]l' OR  text(a.id) ~* 'n[eé]c[uú]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]nt[oó]j[aá]' OR  a.rut ~* 'p[aá]nt[oó]j[aá]' OR  lower(a.email) ~* 'p[aá]nt[oó]j[aá]' OR  text(a.id) ~* 'p[aá]nt[oó]j[aá]' ) 
