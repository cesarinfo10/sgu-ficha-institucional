COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]s[aá]' OR  a.rut ~* 'r[oó]s[aá]' OR  lower(a.email) ~* 'r[oó]s[aá]' OR  text(a.id) ~* 'r[oó]s[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[oó]z[oó]' OR  a.rut ~* 'b[oó]z[oó]' OR  lower(a.email) ~* 'b[oó]z[oó]' OR  text(a.id) ~* 'b[oó]z[oó]' ) 
