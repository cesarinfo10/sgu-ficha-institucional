COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]lv[ií]' OR  a.rut ~* 's[ií]lv[ií]' OR  lower(a.email) ~* 's[ií]lv[ií]' OR  text(a.id) ~* 's[ií]lv[ií]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]h[aá]n[aá]' OR  a.rut ~* 'j[oó]h[aá]n[aá]' OR  lower(a.email) ~* 'j[oó]h[aá]n[aá]' OR  text(a.id) ~* 'j[oó]h[aá]n[aá]' ) 
