COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[uú][eé]v[aá]s' OR  a.rut ~* 'c[uú][eé]v[aá]s' OR  lower(a.email) ~* 'c[uú][eé]v[aá]s' OR  text(a.id) ~* 'c[uú][eé]v[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  a.rut ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  lower(a.email) ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  text(a.id) ~* 'q[uú][ií]ñ[oó]n[eé]s' ) 
