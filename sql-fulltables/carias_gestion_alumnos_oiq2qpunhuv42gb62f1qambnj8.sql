COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rr[eé]r[aá]' OR  a.rut ~* 'b[aá]rr[eé]r[aá]' OR  lower(a.email) ~* 'b[aá]rr[eé]r[aá]' OR  text(a.id) ~* 'b[aá]rr[eé]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nst[aá]nz[oó]' OR  a.rut ~* 'c[oó]nst[aá]nz[oó]' OR  lower(a.email) ~* 'c[oó]nst[aá]nz[oó]' OR  text(a.id) ~* 'c[oó]nst[aá]nz[oó]' ) 
