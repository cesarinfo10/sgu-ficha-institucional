COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[oó]s' OR  a.rut ~* 'm[aá]rc[oó]s' OR  lower(a.email) ~* 'm[aá]rc[oó]s' OR  text(a.id) ~* 'm[aá]rc[oó]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó]m[eé]z' OR  a.rut ~* 'g[oó]m[eé]z' OR  lower(a.email) ~* 'g[oó]m[eé]z' OR  text(a.id) ~* 'g[oó]m[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]m[aá]n' OR  a.rut ~* 'r[oó]m[aá]n' OR  lower(a.email) ~* 'r[oó]m[aá]n' OR  text(a.id) ~* 'r[oó]m[aá]n' ) 
