COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]v[ií]d' OR  a.rut ~* 'd[aá]v[ií]d' OR  lower(a.email) ~* 'd[aá]v[ií]d' OR  text(a.id) ~* 'd[aá]v[ií]d' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]l[aá]t[eé]' OR  a.rut ~* '[oó]l[aá]t[eé]' OR  lower(a.email) ~* '[oó]l[aá]t[eé]' OR  text(a.id) ~* '[oó]l[aá]t[eé]' ) 
