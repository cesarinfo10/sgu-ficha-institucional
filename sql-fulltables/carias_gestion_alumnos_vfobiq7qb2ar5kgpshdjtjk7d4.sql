COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[oó]d[aá]l[eé][oó]' OR  a.rut ~* 'b[oó]d[aá]l[eé][oó]' OR  lower(a.email) ~* 'b[oó]d[aá]l[eé][oó]' OR  text(a.id) ~* 'b[oó]d[aá]l[eé][oó]' ) 
