COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[oó]d[uú]j[eé]' OR  pap.rut ~* 'p[oó]d[uú]j[eé]' OR  text(pap.id) ~* 'p[oó]d[uú]j[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]b[eé]z[aá]s' OR  pap.rut ~* 'c[aá]b[eé]z[aá]s' OR  text(pap.id) ~* 'c[aá]b[eé]z[aá]s' ) 
