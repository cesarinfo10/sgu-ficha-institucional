COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'n[eé]c[uú]l' OR  pap.rut ~* 'n[eé]c[uú]l' OR  text(pap.id) ~* 'n[eé]c[uú]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]nt[oó]j[aá]' OR  pap.rut ~* 'p[aá]nt[oó]j[aá]' OR  text(pap.id) ~* 'p[aá]nt[oó]j[aá]' ) 
