COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[aá]ns' OR  pap.rut ~* 'h[aá]ns' OR  text(pap.id) ~* 'h[aá]ns' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé][oó]n[aá]rd[oó]' OR  pap.rut ~* 'l[eé][oó]n[aá]rd[oó]' OR  text(pap.id) ~* 'l[eé][oó]n[aá]rd[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[eé]r[eé]z' OR  pap.rut ~* 'j[eé]r[eé]z' OR  text(pap.id) ~* 'j[eé]r[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[ií]lv[ií]' OR  pap.rut ~* 's[ií]lv[ií]' OR  text(pap.id) ~* 's[ií]lv[ií]' ) 
