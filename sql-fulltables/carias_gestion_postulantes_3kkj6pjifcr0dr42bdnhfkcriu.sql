COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sp[ií]ñ[eé][ií]r[aá]' OR  pap.rut ~* '[eé]sp[ií]ñ[eé][ií]r[aá]' OR  text(pap.id) ~* '[eé]sp[ií]ñ[eé][ií]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]b[uú]rt[oó]' OR  pap.rut ~* '[aá]b[uú]rt[oó]' OR  text(pap.id) ~* '[aá]b[uú]rt[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ndr[eé][aá]' OR  pap.rut ~* '[aá]ndr[eé][aá]' OR  text(pap.id) ~* '[aá]ndr[eé][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]l[eé]ktr[aá]' OR  pap.rut ~* '[eé]l[eé]ktr[aá]' OR  text(pap.id) ~* '[eé]l[eé]ktr[aá]' ) 
