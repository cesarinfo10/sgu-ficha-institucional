COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]g[aá]ly' OR  pap.rut ~* 'm[aá]g[aá]ly' OR  text(pap.id) ~* 'm[aá]g[aá]ly' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]y[aá]' OR  pap.rut ~* '[aá]r[aá]y[aá]' OR  text(pap.id) ~* '[aá]r[aá]y[aá]' ) 
