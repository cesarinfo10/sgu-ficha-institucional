COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]ndy' OR  pap.rut ~* 's[aá]ndy' OR  text(pap.id) ~* 's[aá]ndy' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]rr[eé]s' OR  pap.rut ~* 't[oó]rr[eé]s' OR  text(pap.id) ~* 't[oó]rr[eé]s' ) 
