COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[ií][aá]' OR  pap.rut ~* 'm[aá]r[ií][aá]' OR  text(pap.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ng[eé]l[ií]c[aá]' OR  pap.rut ~* '[aá]ng[eé]l[ií]c[aá]' OR  text(pap.id) ~* '[aá]ng[eé]l[ií]c[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'n[uú]ñ[eé]z' OR  pap.rut ~* 'n[uú]ñ[eé]z' OR  text(pap.id) ~* 'n[uú]ñ[eé]z' ) 
