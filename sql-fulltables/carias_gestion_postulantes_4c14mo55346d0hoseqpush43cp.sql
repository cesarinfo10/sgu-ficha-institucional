COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[uú]g[oó]' OR  pap.rut ~* 'h[uú]g[oó]' OR  text(pap.id) ~* 'h[uú]g[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[ií]v[ií]l' OR  pap.rut ~* 'm[aá]r[ií]v[ií]l' OR  text(pap.id) ~* 'm[aá]r[ií]v[ií]l' ) 
