COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[uú]l[ií]s[eé]s' OR  pap.rut ~* '[uú]l[ií]s[eé]s' OR  text(pap.id) ~* '[uú]l[ií]s[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'h[uú][aá]ncc[oó]' OR  pap.rut ~* 'h[uú][aá]ncc[oó]' OR  text(pap.id) ~* 'h[uú][aá]ncc[oó]' ) 
