COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]t[aá]l[ií]n[aá]' OR  pap.rut ~* 'c[aá]t[aá]l[ií]n[aá]' OR  text(pap.id) ~* 'c[aá]t[aá]l[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]v[eé]n[aá]' OR  pap.rut ~* '[aá]r[aá]v[eé]n[aá]' OR  text(pap.id) ~* '[aá]r[aá]v[eé]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[ií]q[uú][eé]lm[eé]' OR  pap.rut ~* 'r[ií]q[uú][eé]lm[eé]' OR  text(pap.id) ~* 'r[ií]q[uú][eé]lm[eé]' ) 
