COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]ll[aá]n[oó]' OR  pap.rut ~* 'r[oó]ll[aá]n[oó]' OR  text(pap.id) ~* 'r[oó]ll[aá]n[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]nd[eé]' OR  pap.rut ~* 'c[oó]nd[eé]' OR  text(pap.id) ~* 'c[oó]nd[eé]' ) 
