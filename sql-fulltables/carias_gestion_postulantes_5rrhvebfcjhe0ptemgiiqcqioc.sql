COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]nz[oó]' OR  pap.rut ~* '[eé]nz[oó]' OR  text(pap.id) ~* '[eé]nz[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[eé]n[eé]r[aá]l' OR  pap.rut ~* 'g[eé]n[eé]r[aá]l' OR  text(pap.id) ~* 'g[eé]n[eé]r[aá]l' ) 
