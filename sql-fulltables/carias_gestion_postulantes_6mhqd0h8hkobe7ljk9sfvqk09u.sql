COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  pap.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  text(pap.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]y[aá]' OR  pap.rut ~* '[aá]r[aá]y[aá]' OR  text(pap.id) ~* '[aá]r[aá]y[aá]' ) 
