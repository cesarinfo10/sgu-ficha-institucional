COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]nj[aá]m[ií]n' OR  pap.rut ~* 'b[eé]nj[aá]m[ií]n' OR  text(pap.id) ~* 'b[eé]nj[aá]m[ií]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[eé]ñ[aá]' OR  pap.rut ~* 'p[eé]ñ[aá]' OR  text(pap.id) ~* 'p[eé]ñ[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]v[eé]z' OR  pap.rut ~* 'p[aá]v[eé]z' OR  text(pap.id) ~* 'p[aá]v[eé]z' ) 
