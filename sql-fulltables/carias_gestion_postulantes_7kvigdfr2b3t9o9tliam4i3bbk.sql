COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[oó]l[aá]ng[eé]' OR  pap.rut ~* 's[oó]l[aá]ng[eé]' OR  text(pap.id) ~* 's[oó]l[aá]ng[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[ií]v[eé]r[aá]' OR  pap.rut ~* 'r[ií]v[eé]r[aá]' OR  text(pap.id) ~* 'r[ií]v[eé]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]' OR  pap.rut ~* 'd[eé]' OR  text(pap.id) ~* 'd[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]r[aá]' OR  pap.rut ~* 'v[eé]r[aá]' OR  text(pap.id) ~* 'v[eé]r[aá]' ) 
