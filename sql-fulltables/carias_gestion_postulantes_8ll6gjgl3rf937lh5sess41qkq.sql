COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rc[ií][aá]' OR  pap.rut ~* 'm[aá]rc[ií][aá]' OR  text(pap.id) ~* 'm[aá]rc[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]r[oó]' OR  pap.rut ~* 't[oó]r[oó]' OR  text(pap.id) ~* 't[oó]r[oó]' ) 
