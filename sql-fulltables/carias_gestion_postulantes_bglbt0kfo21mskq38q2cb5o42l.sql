COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú][ií]s' OR  pap.rut ~* 'l[uú][ií]s' OR  text(pap.id) ~* 'l[uú][ií]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]p[uú]lv[eé]d[aá]' OR  pap.rut ~* 's[eé]p[uú]lv[eé]d[aá]' OR  text(pap.id) ~* 's[eé]p[uú]lv[eé]d[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]rr[aá]' OR  pap.rut ~* 'p[aá]rr[aá]' OR  text(pap.id) ~* 'p[aá]rr[aá]' ) 
