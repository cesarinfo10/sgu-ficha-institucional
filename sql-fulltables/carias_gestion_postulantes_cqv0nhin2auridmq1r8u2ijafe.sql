COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[ií][oó]n' OR  pap.rut ~* 'm[aá]r[ií][oó]n' OR  text(pap.id) ~* 'm[aá]r[ií][oó]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]ntr[eé]r[aá]s' OR  pap.rut ~* 'c[oó]ntr[eé]r[aá]s' OR  text(pap.id) ~* 'c[oó]ntr[eé]r[aá]s' ) 
