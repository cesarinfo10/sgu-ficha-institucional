COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá][uú]g[uú]st[oó]' OR  pap.rut ~* '[aá][uú]g[uú]st[oó]' OR  text(pap.id) ~* '[aá][uú]g[uú]st[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[ií]n' OR  pap.rut ~* 'm[aá]r[ií]n' OR  text(pap.id) ~* 'm[aá]r[ií]n' ) 
