COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[oó]bl[eé]t[eé]' OR  pap.rut ~* 'p[oó]bl[eé]t[eé]' OR  text(pap.id) ~* 'p[oó]bl[eé]t[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ng[uú]l[oó]' OR  pap.rut ~* '[aá]ng[uú]l[oó]' OR  text(pap.id) ~* '[aá]ng[uú]l[oó]' ) 
