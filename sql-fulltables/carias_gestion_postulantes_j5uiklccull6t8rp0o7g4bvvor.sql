COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[uú]g[aá]ld[eé]' OR  pap.rut ~* '[uú]g[aá]ld[eé]' OR  text(pap.id) ~* '[uú]g[aá]ld[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'tr[oó]nc[oó]s[oó]' OR  pap.rut ~* 'tr[oó]nc[oó]s[oó]' OR  text(pap.id) ~* 'tr[oó]nc[oó]s[oó]' ) 
