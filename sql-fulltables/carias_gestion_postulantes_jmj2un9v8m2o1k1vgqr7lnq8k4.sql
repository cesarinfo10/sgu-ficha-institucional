COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'fr[aá]nc[ií]sc[aá]' OR  pap.rut ~* 'fr[aá]nc[ií]sc[aá]' OR  text(pap.id) ~* 'fr[aá]nc[ií]sc[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'z[aá]p[aá]g' OR  pap.rut ~* 'z[aá]p[aá]g' OR  text(pap.id) ~* 'z[aá]p[aá]g' ) 
