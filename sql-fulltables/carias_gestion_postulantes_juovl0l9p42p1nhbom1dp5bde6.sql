COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cr[ií]st[ií][aá]n' OR  pap.rut ~* 'cr[ií]st[ií][aá]n' OR  text(pap.id) ~* 'cr[ií]st[ií][aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]nh[uú][eé]z[aá]' OR  pap.rut ~* 's[aá]nh[uú][eé]z[aá]' OR  text(pap.id) ~* 's[aá]nh[uú][eé]z[aá]' ) 
