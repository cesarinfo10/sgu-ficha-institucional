COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]ld[eé]r[oó]n' OR  pap.rut ~* 'c[aá]ld[eé]r[oó]n' OR  text(pap.id) ~* 'c[aá]ld[eé]r[oó]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[uú]j[aá]d[aá]s' OR  pap.rut ~* 'p[uú]j[aá]d[aá]s' OR  text(pap.id) ~* 'p[uú]j[aá]d[aá]s' ) 
