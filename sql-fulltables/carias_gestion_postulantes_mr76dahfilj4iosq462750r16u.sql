COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'n[ií]c[oó]l[aá]s' OR  pap.rut ~* 'n[ií]c[oó]l[aá]s' OR  text(pap.id) ~* 'n[ií]c[oó]l[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]st[eé]b[aá]n' OR  pap.rut ~* '[eé]st[eé]b[aá]n' OR  text(pap.id) ~* '[eé]st[eé]b[aá]n' ) 
