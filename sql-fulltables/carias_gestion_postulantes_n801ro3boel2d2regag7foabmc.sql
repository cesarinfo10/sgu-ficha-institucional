COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[eé]r[aá]lt[aá]' OR  pap.rut ~* 'p[eé]r[aá]lt[aá]' OR  text(pap.id) ~* 'p[eé]r[aá]lt[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]rn[aá]nd[eé]z' OR  pap.rut ~* 'f[eé]rn[aá]nd[eé]z' OR  text(pap.id) ~* 'f[eé]rn[aá]nd[eé]z' ) 
