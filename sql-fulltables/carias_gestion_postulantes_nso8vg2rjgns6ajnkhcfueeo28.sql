COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]b[aá]st[ií][aá]n' OR  pap.rut ~* 's[eé]b[aá]st[ií][aá]n' OR  text(pap.id) ~* 's[eé]b[aá]st[ií][aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]ld[oó]n[aá]d[oó]' OR  pap.rut ~* 'm[aá]ld[oó]n[aá]d[oó]' OR  text(pap.id) ~* 'm[aá]ld[oó]n[aá]d[oó]' ) 
