COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  pap.rut ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  text(pap.id) ~* 'm[aá][uú]r[ií]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[oó]z[aá]n[oó]' OR  pap.rut ~* 'l[oó]z[aá]n[oó]' OR  text(pap.id) ~* 'l[oó]z[aá]n[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
