COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rc[oó]' OR  pap.rut ~* 'm[aá]rc[oó]' OR  text(pap.id) ~* 'm[aá]rc[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]rg[aá]r[aá]' OR  pap.rut ~* 'v[eé]rg[aá]r[aá]' OR  text(pap.id) ~* 'v[eé]rg[aá]r[aá]' ) 
