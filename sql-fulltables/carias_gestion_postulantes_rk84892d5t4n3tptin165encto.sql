COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[uú][eé]v[aá]s' OR  pap.rut ~* 'c[uú][eé]v[aá]s' OR  text(pap.id) ~* 'c[uú][eé]v[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  pap.rut ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  text(pap.id) ~* 'q[uú][ií]ñ[oó]n[eé]s' ) 
