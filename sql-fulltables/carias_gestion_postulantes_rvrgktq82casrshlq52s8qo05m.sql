COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[uú]rs[uú]l[aá]' OR  pap.rut ~* '[uú]rs[uú]l[aá]' OR  text(pap.id) ~* '[uú]rs[uú]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[oó]nz[aá]l[eé]z' OR  pap.rut ~* 'g[oó]nz[aá]l[eé]z' OR  text(pap.id) ~* 'g[oó]nz[aá]l[eé]z' ) 
