COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rc[oó]s' OR  pap.rut ~* 'm[aá]rc[oó]s' OR  text(pap.id) ~* 'm[aá]rc[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[oó]m[eé]z' OR  pap.rut ~* 'g[oó]m[eé]z' OR  text(pap.id) ~* 'g[oó]m[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]m[aá]n' OR  pap.rut ~* 'r[oó]m[aá]n' OR  text(pap.id) ~* 'r[oó]m[aá]n' ) 
