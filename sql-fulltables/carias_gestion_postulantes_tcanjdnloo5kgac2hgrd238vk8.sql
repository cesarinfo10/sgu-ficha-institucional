COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[uú]g[oó]' OR  pap.rut ~* 'h[uú]g[oó]' OR  text(pap.id) ~* 'h[uú]g[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]rm[uú]d[eé]z' OR  pap.rut ~* 'b[eé]rm[uú]d[eé]z' OR  text(pap.id) ~* 'b[eé]rm[uú]d[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
