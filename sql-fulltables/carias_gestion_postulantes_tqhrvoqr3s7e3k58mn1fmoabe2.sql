COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    LEFT JOIN vista_contratos_anos AS vca ON (vca.rut=pap.rut AND vca.ano=2024)
                    LEFT JOIN vista_contratos_pap AS vcp ON (pap.id= vcp.id_pap AND vcp.ano=2024)
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[oó]n' OR  pap.rut ~* 'v[oó]n' OR  text(pap.id) ~* 'v[oó]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]m' OR  pap.rut ~* 'd[eé]m' OR  text(pap.id) ~* 'd[eé]m' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'kn[eé]s[eé]b[eé]ck' OR  pap.rut ~* 'kn[eé]s[eé]b[eé]ck' OR  text(pap.id) ~* 'kn[eé]s[eé]b[eé]ck' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sp[ií]n[oó]z[aá]' OR  pap.rut ~* '[eé]sp[ií]n[oó]z[aá]' OR  text(pap.id) ~* '[eé]sp[ií]n[oó]z[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]str[ií]d' OR  pap.rut ~* '[aá]str[ií]d' OR  text(pap.id) ~* '[aá]str[ií]d' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'y[eé]nsy' OR  pap.rut ~* 'y[eé]nsy' OR  text(pap.id) ~* 'y[eé]nsy' ) 
