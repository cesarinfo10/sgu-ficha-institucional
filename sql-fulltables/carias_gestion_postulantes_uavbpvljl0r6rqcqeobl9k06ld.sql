COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'fr[aá]nc[ií]sc[oó]' OR  pap.rut ~* 'fr[aá]nc[ií]sc[oó]' OR  text(pap.id) ~* 'fr[aá]nc[ií]sc[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]rm[ií]j[oó]' OR  pap.rut ~* '[aá]rm[ií]j[oó]' OR  text(pap.id) ~* '[aá]rm[ií]j[oó]' ) 
