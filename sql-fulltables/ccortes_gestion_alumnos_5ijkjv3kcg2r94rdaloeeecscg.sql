COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'myr[ií][aá]m' OR  a.rut ~* 'myr[ií][aá]m' OR  lower(a.email) ~* 'myr[ií][aá]m' OR  text(a.id) ~* 'myr[ií][aá]m' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]r[eé]d[eé]s' OR  a.rut ~* 'p[aá]r[eé]d[eé]s' OR  lower(a.email) ~* 'p[aá]r[eé]d[eé]s' OR  text(a.id) ~* 'p[aá]r[eé]d[eé]s' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
