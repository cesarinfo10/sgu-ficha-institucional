COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[aá]r[ií][aá]s' OR  a.rut ~* 'f[aá]r[ií][aá]s' OR  lower(a.email) ~* 'f[aá]r[ií][aá]s' OR  text(a.id) ~* 'f[aá]r[ií][aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nst[aá]nz[aá]' OR  a.rut ~* 'c[oó]nst[aá]nz[aá]' OR  lower(a.email) ~* 'c[oó]nst[aá]nz[aá]' OR  text(a.id) ~* 'c[oó]nst[aá]nz[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
