COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]ld[eé]s' OR  a.rut ~* 'v[aá]ld[eé]s' OR  lower(a.email) ~* 'v[aá]ld[eé]s' OR  text(a.id) ~* 'v[aá]ld[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé]l[ií][aá]' OR  a.rut ~* 'c[eé]l[ií][aá]' OR  lower(a.email) ~* 'c[eé]l[ií][aá]' OR  text(a.id) ~* 'c[eé]l[ií][aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
