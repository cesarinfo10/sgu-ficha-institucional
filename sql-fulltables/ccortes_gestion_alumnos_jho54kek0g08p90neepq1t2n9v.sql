COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'ch[oó]m[aá]l[ií]' OR  a.rut ~* 'ch[oó]m[aá]l[ií]' OR  lower(a.email) ~* 'ch[oó]m[aá]l[ií]' OR  text(a.id) ~* 'ch[oó]m[aá]l[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
