COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[ií]ll[aá]m[aá]n' OR  a.rut ~* 'b[ií]ll[aá]m[aá]n' OR  lower(a.email) ~* 'b[ií]ll[aá]m[aá]n' OR  text(a.id) ~* 'b[ií]ll[aá]m[aá]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
