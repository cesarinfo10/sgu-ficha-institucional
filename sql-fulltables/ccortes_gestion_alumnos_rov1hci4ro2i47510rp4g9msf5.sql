COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '13677190-6' OR  a.rut ~* '13677190-6' OR  lower(a.email) ~* '13677190-6' OR  text(a.id) ~* '13677190-6' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
