COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE (SELECT count(id_alumno) FROM inscripciones_cursos WHERE id_alumno=a.id)=0   AND (lower(a.nombres||' '||a.apellidos) ~* '4989463-5' OR  a.rut ~* '4989463-5' OR  lower(a.email) ~* '4989463-5' OR  text(a.id) ~* '4989463-5' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
