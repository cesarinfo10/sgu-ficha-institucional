COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'q[uú][ií]nt[aá]n[ií]ll[aá]') OR (lower(pp.apellidos) ~* 'q[uú][ií]nt[aá]n[ií]ll[aá]') OR (lower(pp.rut) ~* 'q[uú][ií]nt[aá]n[ií]ll[aá]') AND 19::text = ANY(string_to_array(pp.carreras,',')) 
