COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]gr[eé]d[oó]' OR  a.rut ~* 's[aá]gr[eé]d[oó]' OR  lower(a.email) ~* 's[aá]gr[eé]d[oó]' OR  text(a.id) ~* 's[aá]gr[eé]d[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[uú]b[ií]l[aá]r' OR  a.rut ~* 'r[uú]b[ií]l[aá]r' OR  lower(a.email) ~* 'r[uú]b[ií]l[aá]r' OR  text(a.id) ~* 'r[uú]b[ií]l[aá]r' ) 
