COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]rn[aá]rd[ií]t[aá]' OR  a.rut ~* 'b[eé]rn[aá]rd[ií]t[aá]' OR  lower(a.email) ~* 'b[eé]rn[aá]rd[ií]t[aá]' OR  text(a.id) ~* 'b[eé]rn[aá]rd[ií]t[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]t[oó]' OR  a.rut ~* 's[oó]t[oó]' OR  lower(a.email) ~* 's[oó]t[oó]' OR  text(a.id) ~* 's[oó]t[oó]' ) 
