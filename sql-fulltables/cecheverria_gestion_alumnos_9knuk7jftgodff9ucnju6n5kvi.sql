COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN dae.fuas_mineduc AS fuasm ON (fuasm.ano=2024 AND fuasm.rut=split_part(trim(a.rut),'-',1)::int8)
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2024' ) AND (c.regimen = 'PRE')  AND fuasm.rut IS NOT NULL AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1)) 
