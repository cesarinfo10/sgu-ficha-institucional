COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú][ií]s' OR  a.rut ~* 'l[uú][ií]s' OR  lower(a.email) ~* 'l[uú][ií]s' OR  text(a.id) ~* 'l[uú][ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  a.rut ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  lower(a.email) ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  text(a.id) ~* 'f[ií]g[uú][eé]r[oó][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]g[uú]r[aá]' OR  a.rut ~* 's[eé]g[uú]r[aá]' OR  lower(a.email) ~* 's[eé]g[uú]r[aá]' OR  text(a.id) ~* 's[eé]g[uú]r[aá]' ) 
