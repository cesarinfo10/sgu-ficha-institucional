COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]rt[uú]r[oó]' OR  a.rut ~* '[aá]rt[uú]r[oó]' OR  lower(a.email) ~* '[aá]rt[uú]r[oó]' OR  text(a.id) ~* '[aá]rt[uú]r[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]d[aá]r[oó]' OR  a.rut ~* '[aá]d[aá]r[oó]' OR  lower(a.email) ~* '[aá]d[aá]r[oó]' OR  text(a.id) ~* '[aá]d[aá]r[oó]' ) 
