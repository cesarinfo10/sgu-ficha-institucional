COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lv[aá]r[oó]' OR  pap.rut ~* '[aá]lv[aá]r[oó]' OR  text(pap.id) ~* '[aá]lv[aá]r[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]n[eé]d[aá]' OR  pap.rut ~* '[aá]r[aá]n[eé]d[aá]' OR  text(pap.id) ~* '[aá]r[aá]n[eé]d[aá]' ) 
