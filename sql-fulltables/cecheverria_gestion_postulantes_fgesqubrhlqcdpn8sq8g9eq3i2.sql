COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]j[aá]s' OR  pap.rut ~* 'r[oó]j[aá]s' OR  text(pap.id) ~* 'r[oó]j[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[oó]l[aá]ng[eé]' OR  pap.rut ~* 's[oó]l[aá]ng[eé]' OR  text(pap.id) ~* 's[oó]l[aá]ng[eé]' ) 
