COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE true AND (151 IN (pap.carrera1_post,pap.carrera2_post,pap.carrera3_post))  AND (pap.cohorte='2023')  AND (pap.semestre_cohorte='2') AND ('DIP-D' IN (c1.regimen,c2.regimen,c3.regimen,c4.regimen,c5.regimen,c6.regimen)) AND (vca.rut IS NOT NULL) AND ('DIP-D' IN (SELECT regimen FROM carreras WHERE id=(SELECT id_carrera FROM finanzas.contratos WHERE estado IS NOT NULL AND ano=2024 AND id_pap=pap.id LIMIT 1)))
