COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]s[eé]' OR  pap.rut ~* 'j[oó]s[eé]' OR  text(pap.id) ~* 'j[oó]s[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]m[aá]s' OR  pap.rut ~* 't[oó]m[aá]s' OR  text(pap.id) ~* 't[oó]m[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  pap.rut ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  text(pap.id) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' ) 
