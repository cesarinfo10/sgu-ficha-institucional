COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]m[ií]n[aá]' OR  pap.rut ~* 'r[oó]m[ií]n[aá]' OR  text(pap.id) ~* 'r[oó]m[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[uú]r[aá]ll[aá]nc[aá]' OR  pap.rut ~* 'c[uú]r[aá]ll[aá]nc[aá]' OR  text(pap.id) ~* 'c[uú]r[aá]ll[aá]nc[aá]' ) 
