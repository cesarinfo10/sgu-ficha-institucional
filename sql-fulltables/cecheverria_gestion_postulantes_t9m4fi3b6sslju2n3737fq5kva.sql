COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cr[ií]st[oó]ph[eé]r' OR  pap.rut ~* 'cr[ií]st[oó]ph[eé]r' OR  text(pap.id) ~* 'cr[ií]st[oó]ph[eé]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[eé]ñ[aá]' OR  pap.rut ~* 'p[eé]ñ[aá]' OR  text(pap.id) ~* 'p[eé]ñ[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]r[aá]l[eé]s' OR  pap.rut ~* 'm[oó]r[aá]l[eé]s' OR  text(pap.id) ~* 'm[oó]r[aá]l[eé]s' ) 
