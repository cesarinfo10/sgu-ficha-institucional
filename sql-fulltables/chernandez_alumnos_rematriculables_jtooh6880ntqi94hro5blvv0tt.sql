COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]n[eé]s[ií]s' OR  a.rut ~* 'g[eé]n[eé]s[ií]s' OR  lower(a.email) ~* 'g[eé]n[eé]s[ií]s' OR  text(a.id) ~* 'g[eé]n[eé]s[ií]s' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
