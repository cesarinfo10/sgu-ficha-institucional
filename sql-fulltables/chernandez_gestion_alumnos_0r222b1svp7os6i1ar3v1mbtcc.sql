COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]bl[aá]' OR  a.rut ~* 'p[aá]bl[aá]' OR  lower(a.email) ~* 'p[aá]bl[aá]' OR  text(a.id) ~* 'p[aá]bl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]n[aá]l[eé]s' OR  a.rut ~* 'c[aá]n[aá]l[eé]s' OR  lower(a.email) ~* 'c[aá]n[aá]l[eé]s' OR  text(a.id) ~* 'c[aá]n[aá]l[eé]s' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
