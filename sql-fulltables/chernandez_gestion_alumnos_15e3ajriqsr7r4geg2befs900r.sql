COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[uú]r[aá]t[oó]v[ií]ch' OR  a.rut ~* 'b[uú]r[aá]t[oó]v[ií]ch' OR  lower(a.email) ~* 'b[uú]r[aá]t[oó]v[ií]ch' OR  text(a.id) ~* 'b[uú]r[aá]t[oó]v[ií]ch' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
