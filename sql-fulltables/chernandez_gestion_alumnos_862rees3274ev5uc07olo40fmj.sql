COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]n[eé]ss[aá]' OR  a.rut ~* 'v[aá]n[eé]ss[aá]' OR  lower(a.email) ~* 'v[aá]n[eé]ss[aá]' OR  text(a.id) ~* 'v[aá]n[eé]ss[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]g[oó]r' OR  a.rut ~* '[ií]g[oó]r' OR  lower(a.email) ~* '[ií]g[oó]r' OR  text(a.id) ~* '[ií]g[oó]r' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
