COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]ll[eé]g[oó]s' OR  a.rut ~* 'g[aá]ll[eé]g[oó]s' OR  lower(a.email) ~* 'g[aá]ll[eé]g[oó]s' OR  text(a.id) ~* 'g[aá]ll[eé]g[oó]s' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,157,4) 
