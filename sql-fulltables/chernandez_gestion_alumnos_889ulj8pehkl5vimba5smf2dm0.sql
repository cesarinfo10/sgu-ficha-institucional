COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]rm[oó]s[ií]ll[aá]' OR  a.rut ~* 'h[eé]rm[oó]s[ií]ll[aá]' OR  lower(a.email) ~* 'h[eé]rm[oó]s[ií]ll[aá]' OR  text(a.id) ~* 'h[eé]rm[oó]s[ií]ll[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lm[oó]n[aá]c[ií]d' OR  a.rut ~* '[aá]lm[oó]n[aá]c[ií]d' OR  lower(a.email) ~* '[aá]lm[oó]n[aá]c[ií]d' OR  text(a.id) ~* '[aá]lm[oó]n[aá]c[ií]d' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
