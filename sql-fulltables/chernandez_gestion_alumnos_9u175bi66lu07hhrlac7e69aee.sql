COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '13241538-2' OR  a.rut ~* '13241538-2' OR  lower(a.email) ~* '13241538-2' OR  text(a.id) ~* '13241538-2' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
