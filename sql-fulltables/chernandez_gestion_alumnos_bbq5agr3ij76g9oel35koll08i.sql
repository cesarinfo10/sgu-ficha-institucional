COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]j[aá]s' OR  a.rut ~* 'r[oó]j[aá]s' OR  lower(a.email) ~* 'r[oó]j[aá]s' OR  text(a.id) ~* 'r[oó]j[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]t[aá]v[eé]n[ií]t[eé]z' OR  a.rut ~* 'm[aá]t[aá]v[eé]n[ií]t[eé]z' OR  lower(a.email) ~* 'm[aá]t[aá]v[eé]n[ií]t[eé]z' OR  text(a.id) ~* 'm[aá]t[aá]v[eé]n[ií]t[eé]z' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
