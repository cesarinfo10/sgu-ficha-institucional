COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé]t[ií]c[ií][aá]' OR  a.rut ~* 'l[eé]t[ií]c[ií][aá]' OR  lower(a.email) ~* 'l[eé]t[ií]c[ií][aá]' OR  text(a.id) ~* 'l[eé]t[ií]c[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]lch[eé]s' OR  a.rut ~* 'v[ií]lch[eé]s' OR  lower(a.email) ~* 'v[ií]lch[eé]s' OR  text(a.id) ~* 'v[ií]lch[eé]s' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,157,4) 
