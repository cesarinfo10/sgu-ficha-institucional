COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[oó]rm[aá]l' OR  a.rut ~* '[aá]n[oó]rm[aá]l' OR  lower(a.email) ~* '[aá]n[oó]rm[aá]l' OR  text(a.id) ~* '[aá]n[oó]rm[aá]l' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
