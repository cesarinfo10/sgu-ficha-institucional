COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[aá]l[ií]n[aá]' OR  a.rut ~* 'c[aá]t[aá]l[ií]n[aá]' OR  lower(a.email) ~* 'c[aá]t[aá]l[ií]n[aá]' OR  text(a.id) ~* 'c[aá]t[aá]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[uú]ñ[eé]z' OR  a.rut ~* 'n[uú]ñ[eé]z' OR  lower(a.email) ~* 'n[uú]ñ[eé]z' OR  text(a.id) ~* 'n[uú]ñ[eé]z' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4,157) 
