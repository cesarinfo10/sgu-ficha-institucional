COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[ií][aá]r[eé]s[uú]dz[uú]k[ií]' OR  a.rut ~* 't[ií][aá]r[eé]s[uú]dz[uú]k[ií]' OR  lower(a.email) ~* 't[ií][aá]r[eé]s[uú]dz[uú]k[ií]' OR  text(a.id) ~* 't[ií][aá]r[eé]s[uú]dz[uú]k[ií]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,157,4) 
