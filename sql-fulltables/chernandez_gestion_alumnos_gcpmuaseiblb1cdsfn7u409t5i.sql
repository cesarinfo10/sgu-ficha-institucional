COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]lg[uú][eé]z[aá]b[aá]l' OR  a.rut ~* '[eé]lg[uú][eé]z[aá]b[aá]l' OR  lower(a.email) ~* '[eé]lg[uú][eé]z[aá]b[aá]l' OR  text(a.id) ~* '[eé]lg[uú][eé]z[aá]b[aá]l' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
