COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][uú]l[ií]n[aá]' OR  a.rut ~* 'p[aá][uú]l[ií]n[aá]' OR  lower(a.email) ~* 'p[aá][uú]l[ií]n[aá]' OR  text(a.id) ~* 'p[aá][uú]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]ll[oó][aá]' OR  a.rut ~* '[uú]ll[oó][aá]' OR  lower(a.email) ~* '[uú]ll[oó][aá]' OR  text(a.id) ~* '[uú]ll[oó][aá]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
