COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]h[aá]r[aá][ií]m' OR  a.rut ~* 's[aá]h[aá]r[aá][ií]m' OR  lower(a.email) ~* 's[aá]h[aá]r[aá][ií]m' OR  text(a.id) ~* 's[aá]h[aá]r[aá][ií]m' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
