COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][oó]n' OR  a.rut ~* 'm[aá]r[ií][oó]n' OR  lower(a.email) ~* 'm[aá]r[ií][oó]n' OR  text(a.id) ~* 'm[aá]r[ií][oó]n' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
