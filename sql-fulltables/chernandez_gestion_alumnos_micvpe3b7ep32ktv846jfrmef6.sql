COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]ss[eé]l' OR  a.rut ~* 'r[oó]ss[eé]l' OR  lower(a.email) ~* 'r[oó]ss[eé]l' OR  text(a.id) ~* 'r[oó]ss[eé]l' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
