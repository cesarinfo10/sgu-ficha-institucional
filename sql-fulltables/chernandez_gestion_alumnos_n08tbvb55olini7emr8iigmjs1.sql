COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[oó]r[ií]s' OR  a.rut ~* 'd[oó]r[ií]s' OR  lower(a.email) ~* 'd[oó]r[ií]s' OR  text(a.id) ~* 'd[oó]r[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rt[ií]n[eé]z' OR  a.rut ~* 'm[aá]rt[ií]n[eé]z' OR  lower(a.email) ~* 'm[aá]rt[ií]n[eé]z' OR  text(a.id) ~* 'm[aá]rt[ií]n[eé]z' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
