COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]r[ií][aá]' OR  a.rut ~* 'v[aá]l[eé]r[ií][aá]' OR  lower(a.email) ~* 'v[aá]l[eé]r[ií][aá]' OR  text(a.id) ~* 'v[aá]l[eé]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[ií][aá]z' OR  a.rut ~* 'd[ií][aá]z' OR  lower(a.email) ~* 'd[ií][aá]z' OR  text(a.id) ~* 'd[ií][aá]z' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4,157) 
