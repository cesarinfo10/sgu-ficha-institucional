COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'pl[aá]z[aá]' OR  a.rut ~* 'pl[aá]z[aá]' OR  lower(a.email) ~* 'pl[aá]z[aá]' OR  text(a.id) ~* 'pl[aá]z[aá]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
