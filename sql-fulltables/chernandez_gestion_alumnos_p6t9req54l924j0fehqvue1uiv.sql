COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  a.rut ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  lower(a.email) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  text(a.id) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]s[aá]l[eé]s' OR  a.rut ~* 'r[oó]s[aá]l[eé]s' OR  lower(a.email) ~* 'r[oó]s[aá]l[eé]s' OR  text(a.id) ~* 'r[oó]s[aá]l[eé]s' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
