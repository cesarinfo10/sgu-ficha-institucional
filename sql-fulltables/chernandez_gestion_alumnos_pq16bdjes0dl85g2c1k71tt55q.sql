COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]ld[eé]r[oó]n' OR  a.rut ~* 'c[aá]ld[eé]r[oó]n' OR  lower(a.email) ~* 'c[aá]ld[eé]r[oó]n' OR  text(a.id) ~* 'c[aá]ld[eé]r[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[uú]j[aá]d[aá]s' OR  a.rut ~* 'p[uú]j[aá]d[aá]s' OR  lower(a.email) ~* 'p[uú]j[aá]d[aá]s' OR  text(a.id) ~* 'p[uú]j[aá]d[aá]s' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
