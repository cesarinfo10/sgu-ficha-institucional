COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú][aá]n' OR  a.rut ~* 'j[uú][aá]n' OR  lower(a.email) ~* 'j[uú][aá]n' OR  text(a.id) ~* 'j[uú][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[eé]n[oó]' OR  a.rut ~* 'm[oó]r[eé]n[oó]' OR  lower(a.email) ~* 'm[oó]r[eé]n[oó]' OR  text(a.id) ~* 'm[oó]r[eé]n[oó]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,157,4) 
