COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'cr[ií]st[eé]l' OR  a.rut ~* 'cr[ií]st[eé]l' OR  lower(a.email) ~* 'cr[ií]st[eé]l' OR  text(a.id) ~* 'cr[ií]st[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[aá]' OR  a.rut ~* 'm[oó]r[aá]' OR  lower(a.email) ~* 'm[oó]r[aá]' OR  text(a.id) ~* 'm[oó]r[aá]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
