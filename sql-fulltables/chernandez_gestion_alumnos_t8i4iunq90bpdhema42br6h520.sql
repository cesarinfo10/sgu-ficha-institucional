COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[ií][aá]' OR  a.rut ~* 'm[aá]rc[ií][aá]' OR  lower(a.email) ~* 'm[aá]rc[ií][aá]' OR  text(a.id) ~* 'm[aá]rc[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]c[oó]l[eé]' OR  a.rut ~* 'n[ií]c[oó]l[eé]' OR  lower(a.email) ~* 'n[ií]c[oó]l[eé]' OR  text(a.id) ~* 'n[ií]c[oó]l[eé]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
