COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]ll[aá]rd[oó]' OR  a.rut ~* 'g[aá]ll[aá]rd[oó]' OR  lower(a.email) ~* 'g[aá]ll[aá]rd[oó]' OR  text(a.id) ~* 'g[aá]ll[aá]rd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]r[aá]v[ií]t[oó]' OR  a.rut ~* 'g[aá]r[aá]v[ií]t[oó]' OR  lower(a.email) ~* 'g[aá]r[aá]v[ií]t[oó]' OR  text(a.id) ~* 'g[aá]r[aá]v[ií]t[oó]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,157,4) 
