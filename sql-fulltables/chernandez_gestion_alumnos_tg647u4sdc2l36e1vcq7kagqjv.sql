COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[aá]m[aá]r[aá]' OR  a.rut ~* 't[aá]m[aá]r[aá]' OR  lower(a.email) ~* 't[aá]m[aá]r[aá]' OR  text(a.id) ~* 't[aá]m[aá]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]n[oó]' OR  a.rut ~* 'p[ií]n[oó]' OR  lower(a.email) ~* 'p[ií]n[oó]' OR  text(a.id) ~* 'p[ií]n[oó]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
