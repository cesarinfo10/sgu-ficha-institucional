COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]m[aá]ch[oó]' OR  a.rut ~* 'c[aá]m[aá]ch[oó]' OR  lower(a.email) ~* 'c[aá]m[aá]ch[oó]' OR  text(a.id) ~* 'c[aá]m[aá]ch[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií][eé]dr[aá]h[ií]t[aá]' OR  a.rut ~* 'p[ií][eé]dr[aá]h[ií]t[aá]' OR  lower(a.email) ~* 'p[ií][eé]dr[aá]h[ií]t[aá]' OR  text(a.id) ~* 'p[ií][eé]dr[aá]h[ií]t[aá]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,157,4) 
