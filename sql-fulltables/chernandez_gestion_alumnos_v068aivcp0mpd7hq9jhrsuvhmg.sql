COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[eé]ls[oó]n' OR  a.rut ~* 'n[eé]ls[oó]n' OR  lower(a.email) ~* 'n[eé]ls[oó]n' OR  text(a.id) ~* 'n[eé]ls[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]g[aá]t' OR  a.rut ~* 'r[oó]g[aá]t' OR  lower(a.email) ~* 'r[oó]g[aá]t' OR  text(a.id) ~* 'r[oó]g[aá]t' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
