COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]m[ií]n[aá]' OR  a.rut ~* 'r[oó]m[ií]n[aá]' OR  lower(a.email) ~* 'r[oó]m[ií]n[aá]' OR  text(a.id) ~* 'r[oó]m[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]t[oó]m[aá]y[oó]r' OR  a.rut ~* 's[oó]t[oó]m[aá]y[oó]r' OR  lower(a.email) ~* 's[oó]t[oó]m[aá]y[oó]r' OR  text(a.id) ~* 's[oó]t[oó]m[aá]y[oó]r' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
