COPY (SELECT et.id,to_char(et.fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
				  ...
				  WHERE true   AND (vete.nombres::text ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  vete.ruts::text ~* 'm[aá][uú]r[ií]c[ií][oó]' ) AND (vete.nombres::text ~* 'v[aá]sq[uú][eé]z' OR  vete.ruts::text ~* 'v[aá]sq[uú][eé]z' )  			  
