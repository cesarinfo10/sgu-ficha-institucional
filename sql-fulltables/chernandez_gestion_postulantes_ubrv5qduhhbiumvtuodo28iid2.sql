COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[uú]l[ií][oó]' OR  pap.rut ~* 'j[uú]l[ií][oó]' OR  text(pap.id) ~* 'j[uú]l[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  pap.rut ~* 'r[oó]dr[ií]g[uú][eé]z' OR  text(pap.id) ~* 'r[oó]dr[ií]g[uú][eé]z' ) 
