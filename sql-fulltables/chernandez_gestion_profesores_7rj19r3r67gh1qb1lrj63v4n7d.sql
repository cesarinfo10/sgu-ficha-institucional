COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'p[aá][uú]l[ií]n[aá]'  OR u.rut ~* 'p[aá][uú]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[oó]m[aá]n'  OR u.rut ~* 'r[oó]m[aá]n') 
