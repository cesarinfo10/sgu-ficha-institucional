COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]ng[eé]l[aá]'  OR u.rut ~* '[aá]ng[eé]l[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[eé]rg[aá]r[aá]'  OR u.rut ~* 'v[eé]rg[aá]r[aá]') 
