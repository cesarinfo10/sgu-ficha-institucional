COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR  a.rut ~* 'h[uú]m[aá]n[ií]sm[oó]' OR  lower(a.email) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR  text(a.id) ~* 'h[uú]m[aá]n[ií]sm[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]c[ií][aá]l' OR  a.rut ~* 's[oó]c[ií][aá]l' OR  lower(a.email) ~* 's[oó]c[ií][aá]l' OR  text(a.id) ~* 's[oó]c[ií][aá]l' ) 
