COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[oó]n' OR  a.rut ~* 'v[oó]n' OR  lower(a.email) ~* 'v[oó]n' OR  text(a.id) ~* 'v[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé]m' OR  a.rut ~* 'd[eé]m' OR  lower(a.email) ~* 'd[eé]m' OR  text(a.id) ~* 'd[eé]m' ) 
