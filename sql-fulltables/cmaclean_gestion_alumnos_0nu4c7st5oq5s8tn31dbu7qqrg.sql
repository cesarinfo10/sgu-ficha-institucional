COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]p[eé]t[ií]ll[oó]' OR  a.rut ~* 'c[aá]p[eé]t[ií]ll[oó]' OR  lower(a.email) ~* 'c[aá]p[eé]t[ií]ll[oó]' OR  text(a.id) ~* 'c[aá]p[eé]t[ií]ll[oó]' ) 
