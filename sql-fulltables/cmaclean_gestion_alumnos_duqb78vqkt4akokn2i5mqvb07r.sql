COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]x[ií]m[ií]l[ií][aá]n[oó]' OR  a.rut ~* 'm[aá]x[ií]m[ií]l[ií][aá]n[oó]' OR  lower(a.email) ~* 'm[aá]x[ií]m[ií]l[ií][aá]n[oó]' OR  text(a.id) ~* 'm[aá]x[ií]m[ií]l[ií][aá]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rt[eé]g' OR  a.rut ~* '[oó]rt[eé]g' OR  lower(a.email) ~* '[oó]rt[eé]g' OR  text(a.id) ~* '[oó]rt[eé]g' ) 
