COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'f[ií]ch[eé]r' OR  pap.rut ~* 'f[ií]ch[eé]r' OR  text(pap.id) ~* 'f[ií]ch[eé]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]rr[aá]d[ií]n[ií]' OR  pap.rut ~* 'c[oó]rr[aá]d[ií]n[ií]' OR  text(pap.id) ~* 'c[oó]rr[aá]d[ií]n[ií]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[uú]l[ií]ss[aá]' OR  pap.rut ~* 'j[uú]l[ií]ss[aá]' OR  text(pap.id) ~* 'j[uú]l[ií]ss[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'k[aá]t[ií]n[aá]' OR  pap.rut ~* 'k[aá]t[ií]n[aá]' OR  text(pap.id) ~* 'k[aá]t[ií]n[aá]' ) 
