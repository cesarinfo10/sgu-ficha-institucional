COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]sv[aá]ld[oó]' OR  pap.rut ~* '[oó]sv[aá]ld[oó]' OR  text(pap.id) ~* '[oó]sv[aá]ld[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]r[eé]n[oó]' OR  pap.rut ~* 'm[oó]r[eé]n[oó]' OR  text(pap.id) ~* 'm[oó]r[eé]n[oó]' ) 
