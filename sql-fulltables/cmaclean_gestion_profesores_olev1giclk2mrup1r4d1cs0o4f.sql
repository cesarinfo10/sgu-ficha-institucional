COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[oó]rg[eé]'  OR u.rut ~* 'j[oó]rg[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]ld[oó]n[aá]d[oó]'  OR u.rut ~* 'm[aá]ld[oó]n[aá]d[oó]') 
