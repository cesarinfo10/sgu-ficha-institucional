COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE (SELECT count(id_alumno) FROM inscripciones_cursos WHERE id_alumno=a.id)=0   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]b[eé]r[oó]n[aá]' OR  a.rut ~* 'l[ií]b[eé]r[oó]n[aá]' OR  lower(a.email) ~* 'l[ií]b[eé]r[oó]n[aá]' OR  text(a.id) ~* 'l[ií]b[eé]r[oó]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]r[eé]d[ií][aá]' OR  a.rut ~* 'h[eé]r[eé]d[ií][aá]' OR  lower(a.email) ~* 'h[eé]r[eé]d[ií][aá]' OR  text(a.id) ~* 'h[eé]r[eé]d[ií][aá]' ) 
