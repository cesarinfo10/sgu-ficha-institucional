COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE (SELECT count(id_alumno) FROM inscripciones_cursos WHERE id_alumno=a.id)=0   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]c[eé]rr[aá]' OR  a.rut ~* 'b[eé]c[eé]rr[aá]' OR  lower(a.email) ~* 'b[eé]c[eé]rr[aá]' OR  text(a.id) ~* 'b[eé]c[eé]rr[aá]' ) 
