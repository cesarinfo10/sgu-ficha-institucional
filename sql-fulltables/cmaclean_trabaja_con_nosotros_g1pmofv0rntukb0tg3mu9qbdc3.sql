COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'm[aá]rt[ií]n[aá]') OR (lower(pp.apellidos) ~* 'm[aá]rt[ií]n[aá]') OR (lower(pp.rut) ~* 'm[aá]rt[ií]n[aá]') AND (lower(pp.nombres) ~* 'g[oó]m[eé]z') OR (lower(pp.apellidos) ~* 'g[oó]m[eé]z') OR (lower(pp.rut) ~* 'g[oó]m[eé]z')
