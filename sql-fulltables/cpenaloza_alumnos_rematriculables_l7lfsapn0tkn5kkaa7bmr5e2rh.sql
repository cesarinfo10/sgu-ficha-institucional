COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]l[aá]t[eé]' OR  a.rut ~* '[oó]l[aá]t[eé]' OR  lower(a.email) ~* '[oó]l[aá]t[eé]' OR  text(a.id) ~* '[oó]l[aá]t[eé]' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67,164) 
