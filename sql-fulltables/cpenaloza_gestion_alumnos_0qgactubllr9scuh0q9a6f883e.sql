COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]s[aá]b[eé]l' OR  a.rut ~* '[ií]s[aá]b[eé]l' OR  lower(a.email) ~* '[ií]s[aá]b[eé]l' OR  text(a.id) ~* '[ií]s[aá]b[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[ií]z[aá]m[aá]' OR  a.rut ~* 'b[ií]z[aá]m[aá]' OR  lower(a.email) ~* 'b[ií]z[aá]m[aá]' OR  text(a.id) ~* 'b[ií]z[aá]m[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,150) 
