COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé]s[aá]r' OR  a.rut ~* 'c[eé]s[aá]r' OR  lower(a.email) ~* 'c[eé]s[aá]r' OR  text(a.id) ~* 'c[eé]s[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]nt[eé]c[ií]n[oó]' OR  a.rut ~* 'm[oó]nt[eé]c[ií]n[oó]' OR  lower(a.email) ~* 'm[oó]nt[eé]c[ií]n[oó]' OR  text(a.id) ~* 'm[oó]nt[eé]c[ií]n[oó]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
