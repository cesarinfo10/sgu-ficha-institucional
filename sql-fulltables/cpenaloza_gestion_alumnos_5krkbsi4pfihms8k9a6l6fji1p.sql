COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]m[ií]n[aá]' OR  a.rut ~* 'r[oó]m[ií]n[aá]' OR  lower(a.email) ~* 'r[oó]m[ií]n[aá]' OR  text(a.id) ~* 'r[oó]m[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[uú]r[aá]ll[aá]' OR  a.rut ~* 'c[uú]r[aá]ll[aá]' OR  lower(a.email) ~* 'c[uú]r[aá]ll[aá]' OR  text(a.id) ~* 'c[uú]r[aá]ll[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
