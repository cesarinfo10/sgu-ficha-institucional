COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  a.rut ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  lower(a.email) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  text(a.id) ~* 'v[aá]l[eé]nt[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]nr' OR  a.rut ~* 'h[eé]nr' OR  lower(a.email) ~* 'h[eé]nr' OR  text(a.id) ~* 'h[eé]nr' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
