COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]rn[ií][eé]' OR  a.rut ~* '[aá]rn[ií][eé]' OR  lower(a.email) ~* '[aá]rn[ií][eé]' OR  text(a.id) ~* '[aá]rn[ií][eé]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
