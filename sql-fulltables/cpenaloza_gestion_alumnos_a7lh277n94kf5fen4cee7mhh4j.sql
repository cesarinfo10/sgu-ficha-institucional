COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '16616030' OR  a.rut ~* '16616030' OR  lower(a.email) ~* '16616030' OR  text(a.id) ~* '16616030' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
