COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]y[aá]rc[eé]' OR  a.rut ~* '[oó]y[aá]rc[eé]' OR  lower(a.email) ~* '[oó]y[aá]rc[eé]' OR  text(a.id) ~* '[oó]y[aá]rc[eé]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,156,159) 
