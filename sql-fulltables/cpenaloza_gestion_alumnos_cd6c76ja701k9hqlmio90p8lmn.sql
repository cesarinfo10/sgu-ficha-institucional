COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]n[eé]' OR  a.rut ~* 'r[eé]n[eé]' OR  lower(a.email) ~* 'r[eé]n[eé]' OR  text(a.id) ~* 'r[eé]n[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]c[eé]r[eé]s' OR  a.rut ~* 'c[aá]c[eé]r[eé]s' OR  lower(a.email) ~* 'c[aá]c[eé]r[eé]s' OR  text(a.id) ~* 'c[aá]c[eé]r[eé]s' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
