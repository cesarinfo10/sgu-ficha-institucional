COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'yl[uú]ss[eé]t' OR  a.rut ~* 'yl[uú]ss[eé]t' OR  lower(a.email) ~* 'yl[uú]ss[eé]t' OR  text(a.id) ~* 'yl[uú]ss[eé]t' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
