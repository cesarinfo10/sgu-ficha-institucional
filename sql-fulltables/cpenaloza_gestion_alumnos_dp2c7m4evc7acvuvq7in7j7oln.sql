COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[oó]r[ií][eé]g[aá]' OR  a.rut ~* 'n[oó]r[ií][eé]g[aá]' OR  lower(a.email) ~* 'n[oó]r[ií][eé]g[aá]' OR  text(a.id) ~* 'n[oó]r[ií][eé]g[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67) 
