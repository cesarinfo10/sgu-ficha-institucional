COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]fr[eé]' OR  a.rut ~* 'c[oó]fr[eé]' OR  lower(a.email) ~* 'c[oó]fr[eé]' OR  text(a.id) ~* 'c[oó]fr[eé]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
