COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]y[aá]r' OR  a.rut ~* 'n[aá]y[aá]r' OR  lower(a.email) ~* 'n[aá]y[aá]r' OR  text(a.id) ~* 'n[aá]y[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][ií]nt[aá]n[aá]' OR  a.rut ~* 'q[uú][ií]nt[aá]n[aá]' OR  lower(a.email) ~* 'q[uú][ií]nt[aá]n[aá]' OR  text(a.id) ~* 'q[uú][ií]nt[aá]n[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
