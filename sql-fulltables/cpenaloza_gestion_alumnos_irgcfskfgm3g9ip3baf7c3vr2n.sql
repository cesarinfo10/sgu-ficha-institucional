COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]v[ií]d' OR  a.rut ~* 'd[aá]v[ií]d' OR  lower(a.email) ~* 'd[aá]v[ií]d' OR  text(a.id) ~* 'd[aá]v[ií]d' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[aá]' OR  a.rut ~* 'm[oó]r[aá]' OR  lower(a.email) ~* 'm[oó]r[aá]' OR  text(a.id) ~* 'm[oó]r[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
