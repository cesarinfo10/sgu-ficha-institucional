COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[oó]l[ií]n[aá]' OR  a.rut ~* 'c[aá]r[oó]l[ií]n[aá]' OR  lower(a.email) ~* 'c[aá]r[oó]l[ií]n[aá]' OR  text(a.id) ~* 'c[aá]r[oó]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]r[aá]nd[aá]' OR  a.rut ~* 'm[ií]r[aá]nd[aá]' OR  lower(a.email) ~* 'm[ií]r[aá]nd[aá]' OR  text(a.id) ~* 'm[ií]r[aá]nd[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,150) 
