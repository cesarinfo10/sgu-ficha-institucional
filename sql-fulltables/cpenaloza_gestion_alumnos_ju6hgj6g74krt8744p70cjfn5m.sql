COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[aá]tm[aá]h' OR  a.rut ~* 'f[aá]tm[aá]h' OR  lower(a.email) ~* 'f[aá]tm[aá]h' OR  text(a.id) ~* 'f[aá]tm[aá]h' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67,164) 
