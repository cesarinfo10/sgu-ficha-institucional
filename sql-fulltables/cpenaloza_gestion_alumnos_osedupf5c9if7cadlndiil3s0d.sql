COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'ch[eé]r[ií][eé]' OR  a.rut ~* 'ch[eé]r[ií][eé]' OR  lower(a.email) ~* 'ch[eé]r[ií][eé]' OR  text(a.id) ~* 'ch[eé]r[ií][eé]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
