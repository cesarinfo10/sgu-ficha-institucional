COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[uú][eé]bl[aá]' OR  a.rut ~* 'p[uú][eé]bl[aá]' OR  lower(a.email) ~* 'p[uú][eé]bl[aá]' OR  text(a.id) ~* 'p[uú][eé]bl[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67,150) 
