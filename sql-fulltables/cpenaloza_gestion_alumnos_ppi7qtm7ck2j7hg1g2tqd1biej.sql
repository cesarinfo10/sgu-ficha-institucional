COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[uú]v[aá]n' OR  a.rut ~* 'd[uú]v[aá]n' OR  lower(a.email) ~* 'd[uú]v[aá]n' OR  text(a.id) ~* 'd[uú]v[aá]n' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
