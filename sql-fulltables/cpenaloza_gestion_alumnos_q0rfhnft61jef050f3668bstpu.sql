COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[eé]n[oó]' OR  a.rut ~* 'm[oó]r[eé]n[oó]' OR  lower(a.email) ~* 'm[oó]r[eé]n[oó]' OR  text(a.id) ~* 'm[oó]r[eé]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]nr[ií]q[uú][eé]' OR  a.rut ~* 'h[eé]nr[ií]q[uú][eé]' OR  lower(a.email) ~* 'h[eé]nr[ií]q[uú][eé]' OR  text(a.id) ~* 'h[eé]nr[ií]q[uú][eé]' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
