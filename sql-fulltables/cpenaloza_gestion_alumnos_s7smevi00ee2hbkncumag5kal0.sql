COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '19282502' OR  a.rut ~* '19282502' OR  lower(a.email) ~* '19282502' OR  text(a.id) ~* '19282502' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
