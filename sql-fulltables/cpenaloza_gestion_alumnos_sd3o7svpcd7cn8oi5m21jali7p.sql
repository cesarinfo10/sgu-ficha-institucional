COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '13931588' OR  a.rut ~* '13931588' OR  lower(a.email) ~* '13931588' OR  text(a.id) ~* '13931588' )  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
