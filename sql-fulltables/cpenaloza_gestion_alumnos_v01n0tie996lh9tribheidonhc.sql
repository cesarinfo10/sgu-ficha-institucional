COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]t[eé]l[oó]' OR  a.rut ~* 's[oó]t[eé]l[oó]' OR  lower(a.email) ~* 's[oó]t[eé]l[oó]' OR  text(a.id) ~* 's[oó]t[eé]l[oó]' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67) 
