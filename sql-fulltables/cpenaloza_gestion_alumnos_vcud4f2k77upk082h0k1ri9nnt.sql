COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]l[ií]ss[aá]' OR  a.rut ~* 'm[eé]l[ií]ss[aá]' OR  lower(a.email) ~* 'm[eé]l[ií]ss[aá]' OR  text(a.id) ~* 'm[eé]l[ií]ss[aá]' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67,164) 
