COPY (SELECT poas.id,tipo_act,coalesce(p.nombre,'') AS proyecto,actividad,prioridad,
               ...
               WHERE date_part('year',fecha_prog_termino)=2023  AND date_part('month',fecha_prog_termino)=4  AND poas.id_unidad='27'  AND tipo_act='POA'  AND estado NOT IN ('OK','Terminada','Eliminada') 
