COPY (SELECT *,CASE WHEN tasa_riesgo BETWEEN 75 and 100 THEN 'Alto'
                ...
                LEFT JOIN (SELECT rut_alumno,max(nota) AS nota FROM sinergia.resp_pruebas_diag WHERE ano=2023 AND semestre=2 GROUP BY rut_alumno) AS niv ON niv.rut_alumno=a.rut
                LEFT JOIN matriculas        AS mat ON (mat.id_alumno=a.id AND mat.semestre=2 AND mat.ano=2022)
                WHERE true  AND (a.cohorte = '2023') AND (a.semestre_cohorte = 2) AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2022 AND semestre=2))  AND a.carrera_actual IN (101,62,26,88,18,3,36,67) 
