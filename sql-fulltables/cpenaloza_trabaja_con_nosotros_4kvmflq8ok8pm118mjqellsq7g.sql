COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'n[aá]ncy') OR (lower(pp.apellidos) ~* 'n[aá]ncy') OR (lower(pp.rut) ~* 'n[aá]ncy') AND (lower(pp.nombres) ~* 'm[oó]r[aá]l[eé]s') OR (lower(pp.apellidos) ~* 'm[oó]r[aá]l[eé]s') OR (lower(pp.rut) ~* 'm[oó]r[aá]l[eé]s')
