COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'gl[oó]r[ií][aá]') OR (lower(pp.apellidos) ~* 'gl[oó]r[ií][aá]') OR (lower(pp.rut) ~* 'gl[oó]r[ií][aá]') AND (lower(pp.nombres) ~* 'c[aá]n[aá]l[eé]s[aá]') OR (lower(pp.apellidos) ~* 'c[aá]n[aá]l[eé]s[aá]') OR (lower(pp.rut) ~* 'c[aá]n[aá]l[eé]s[aá]')
