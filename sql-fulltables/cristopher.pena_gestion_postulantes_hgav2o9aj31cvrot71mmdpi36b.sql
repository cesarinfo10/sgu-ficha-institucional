COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]rg[eé]' OR  pap.rut ~* 'j[oó]rg[eé]' OR  text(pap.id) ~* 'j[oó]rg[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'z[aá]p[aá]t[aá]' OR  pap.rut ~* 'z[aá]p[aá]t[aá]' OR  text(pap.id) ~* 'z[aá]p[aá]t[aá]' ) 
