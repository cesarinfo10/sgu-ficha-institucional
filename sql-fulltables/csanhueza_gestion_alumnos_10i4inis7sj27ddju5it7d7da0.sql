COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]g[aá]th[aá]' OR  a.rut ~* '[aá]g[aá]th[aá]' OR  lower(a.email) ~* '[aá]g[aá]th[aá]' OR  text(a.id) ~* '[aá]g[aá]th[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rr[aá]sc[oó]' OR  a.rut ~* 'c[aá]rr[aá]sc[oó]' OR  lower(a.email) ~* 'c[aá]rr[aá]sc[oó]' OR  text(a.id) ~* 'c[aá]rr[aá]sc[oó]' ) 
