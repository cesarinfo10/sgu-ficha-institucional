COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]x' OR  a.rut ~* '[aá]l[eé]x' OR  lower(a.email) ~* '[aá]l[eé]x' OR  text(a.id) ~* '[aá]l[eé]x' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  a.rut ~* 'r[oó]dr[ií]g[oó]' OR  lower(a.email) ~* 'r[oó]dr[ií]g[oó]' OR  text(a.id) ~* 'r[oó]dr[ií]g[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[aá]g[oó]s' OR  a.rut ~* 'l[aá]g[oó]s' OR  lower(a.email) ~* 'l[aá]g[oó]s' OR  text(a.id) ~* 'l[aá]g[oó]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]p[aá]bl[aá]z[aá]' OR  a.rut ~* '[aá]p[aá]bl[aá]z[aá]' OR  lower(a.email) ~* '[aá]p[aá]bl[aá]z[aá]' OR  text(a.id) ~* '[aá]p[aá]bl[aá]z[aá]' ) 
