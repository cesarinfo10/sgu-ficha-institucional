COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá][ií]m[eé]' OR  a.rut ~* 'j[aá][ií]m[eé]' OR  lower(a.email) ~* 'j[aá][ií]m[eé]' OR  text(a.id) ~* 'j[aá][ií]m[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]d[ií]n[aá]' OR  a.rut ~* 'm[eé]d[ií]n[aá]' OR  lower(a.email) ~* 'm[eé]d[ií]n[aá]' OR  text(a.id) ~* 'm[eé]d[ií]n[aá]' ) 
