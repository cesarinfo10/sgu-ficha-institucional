COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]ss[aá]n[aá]' OR  a.rut ~* 'r[oó]ss[aá]n[aá]' OR  lower(a.email) ~* 'r[oó]ss[aá]n[aá]' OR  text(a.id) ~* 'r[oó]ss[aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]ñ[eé]t[eé]' OR  a.rut ~* 'c[aá]ñ[eé]t[eé]' OR  lower(a.email) ~* 'c[aá]ñ[eé]t[eé]' OR  text(a.id) ~* 'c[aá]ñ[eé]t[eé]' ) 
