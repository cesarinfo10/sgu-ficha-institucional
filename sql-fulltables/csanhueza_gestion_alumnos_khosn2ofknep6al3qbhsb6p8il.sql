COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[ií]n[aá]' OR  a.rut ~* 'k[aá]r[ií]n[aá]' OR  lower(a.email) ~* 'k[aá]r[ií]n[aá]' OR  text(a.id) ~* 'k[aá]r[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[uú]r[aá]n' OR  a.rut ~* 'd[uú]r[aá]n' OR  lower(a.email) ~* 'd[uú]r[aá]n' OR  text(a.id) ~* 'd[uú]r[aá]n' ) 
