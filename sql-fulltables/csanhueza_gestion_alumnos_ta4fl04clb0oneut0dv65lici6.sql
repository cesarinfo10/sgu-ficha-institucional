COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[uú]v[aá]n' OR  a.rut ~* 'd[uú]v[aá]n' OR  lower(a.email) ~* 'd[uú]v[aá]n' OR  text(a.id) ~* 'd[uú]v[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]y[aá]' OR  a.rut ~* '[aá]r[aá]y[aá]' OR  lower(a.email) ~* '[aá]r[aá]y[aá]' OR  text(a.id) ~* '[aá]r[aá]y[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]n[aá]' OR  a.rut ~* 'm[eé]n[aá]' OR  lower(a.email) ~* 'm[eé]n[aá]' OR  text(a.id) ~* 'm[eé]n[aá]' ) 
