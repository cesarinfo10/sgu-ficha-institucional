COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]g[uú][eé]l' OR  a.rut ~* 's[eé]g[uú][eé]l' OR  lower(a.email) ~* 's[eé]g[uú][eé]l' OR  text(a.id) ~* 's[eé]g[uú][eé]l' )  AND a.carrera_actual IN (61,68,21) 
