COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]n[aá]' OR  a.rut ~* 'm[eé]n[aá]' OR  lower(a.email) ~* 'm[eé]n[aá]' OR  text(a.id) ~* 'm[eé]n[aá]' )  AND a.carrera_actual IN (61,68,21) 
