COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]lv[ií]s' OR  a.rut ~* '[eé]lv[ií]s' OR  lower(a.email) ~* '[eé]lv[ií]s' OR  text(a.id) ~* '[eé]lv[ií]s' )  AND a.carrera_actual IN (61,68,21) 
