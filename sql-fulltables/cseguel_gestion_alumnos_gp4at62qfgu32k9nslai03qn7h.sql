COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'pr[aá]d[ií]n[eé]s' OR  a.rut ~* 'pr[aá]d[ií]n[eé]s' OR  lower(a.email) ~* 'pr[aá]d[ií]n[eé]s' OR  text(a.id) ~* 'pr[aá]d[ií]n[eé]s' )  AND a.carrera_actual IN (61,68,21) 
