COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé]s[aá]r' OR  a.rut ~* 'c[eé]s[aá]r' OR  lower(a.email) ~* 'c[eé]s[aá]r' OR  text(a.id) ~* 'c[eé]s[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]ñ[aá]' OR  a.rut ~* 'p[eé]ñ[aá]' OR  lower(a.email) ~* 'p[eé]ñ[aá]' OR  text(a.id) ~* 'p[eé]ñ[aá]' )  AND a.carrera_actual IN (61,68,21) 
