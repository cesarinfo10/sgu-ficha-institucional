COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][uú]l[oó]' OR  a.rut ~* 'p[aá][uú]l[oó]' OR  lower(a.email) ~* 'p[aá][uú]l[oó]' OR  text(a.id) ~* 'p[aá][uú]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]y[aá]n' OR  a.rut ~* 'g[aá]y[aá]n' OR  lower(a.email) ~* 'g[aá]y[aá]n' OR  text(a.id) ~* 'g[aá]y[aá]n' )  AND a.carrera_actual IN (61,68,21) 
