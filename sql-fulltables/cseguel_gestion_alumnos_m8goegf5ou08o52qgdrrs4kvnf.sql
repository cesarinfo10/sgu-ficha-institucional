COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]l[eé]sk[aá]' OR  a.rut ~* 'b[aá]l[eé]sk[aá]' OR  lower(a.email) ~* 'b[aá]l[eé]sk[aá]' OR  text(a.id) ~* 'b[aá]l[eé]sk[aá]' )  AND a.carrera_actual IN (61,68,21) 
