COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]rm[aá]n' OR  a.rut ~* 'g[eé]rm[aá]n' OR  lower(a.email) ~* 'g[eé]rm[aá]n' OR  text(a.id) ~* 'g[eé]rm[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé]t[eé]l[ií][eé]r' OR  a.rut ~* 'l[eé]t[eé]l[ií][eé]r' OR  lower(a.email) ~* 'l[eé]t[eé]l[ií][eé]r' OR  text(a.id) ~* 'l[eé]t[eé]l[ií][eé]r' )  AND a.carrera_actual IN (61,68,21) 
