COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]ld[oó]n[aá]d[oó]' OR  a.rut ~* 'm[aá]ld[oó]n[aá]d[oó]' OR  lower(a.email) ~* 'm[aá]ld[oó]n[aá]d[oó]' OR  text(a.id) ~* 'm[aá]ld[oó]n[aá]d[oó]' )  AND a.carrera_actual IN (61,68,21) 
