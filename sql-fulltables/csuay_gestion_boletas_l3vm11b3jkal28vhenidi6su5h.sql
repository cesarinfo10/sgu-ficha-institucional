COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '61384' OR  pap.rut ~* '61384' OR lower(a.nombres||' '||a.apellidos) ~* '61384' OR lower(a3.nombres||' '||a3.apellidos) ~* '61384' OR  a.rut ~* '61384' OR a2.rut ~* '61384' OR a3.rut ~* '61384'  OR text(nro_boleta) = '61384' )  
