COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE (p.nro_boleta IS NOT NULL OR p.nro_factura IS NOT NULL) AND p.fecha BETWEEN now()-'1 days'::interval AND now() AND ('PRE' IN (car.regimen,car2.regimen,car3.regimen) OR p.nulo)  
