COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[aá]nc[ií]sc[oó]' OR  a.rut ~* 'fr[aá]nc[ií]sc[oó]' OR  lower(a.email) ~* 'fr[aá]nc[ií]sc[oó]' OR  text(a.id) ~* 'fr[aá]nc[ií]sc[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]ld[eé]r[oó]n' OR  a.rut ~* 'c[aá]ld[eé]r[oó]n' OR  lower(a.email) ~* 'c[aá]ld[eé]r[oó]n' OR  text(a.id) ~* 'c[aá]ld[eé]r[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[uú]j[aá]d[aá]s' OR  a.rut ~* 'p[uú]j[aá]d[aá]s' OR  lower(a.email) ~* 'p[uú]j[aá]d[aá]s' OR  text(a.id) ~* 'p[uú]j[aá]d[aá]s' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
