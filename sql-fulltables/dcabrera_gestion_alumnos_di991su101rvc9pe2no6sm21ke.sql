COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]t[ií][aá]' OR  a.rut ~* 'k[aá]t[ií][aá]' OR  lower(a.email) ~* 'k[aá]t[ií][aá]' OR  text(a.id) ~* 'k[aá]t[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]p[eé]z' OR  a.rut ~* '[oó]p[eé]z' OR  lower(a.email) ~* '[oó]p[eé]z' OR  text(a.id) ~* '[oó]p[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]str[oó]' OR  a.rut ~* 'c[aá]str[oó]' OR  lower(a.email) ~* 'c[aá]str[oó]' OR  text(a.id) ~* 'c[aá]str[oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148,161,160,163) 
