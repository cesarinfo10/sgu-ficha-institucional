COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]n[aá]r[oó]' OR  a.rut ~* 'g[eé]n[aá]r[oó]' OR  lower(a.email) ~* 'g[eé]n[aá]r[oó]' OR  text(a.id) ~* 'g[eé]n[aá]r[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'br[ií][oó]n[eé]s' OR  a.rut ~* 'br[ií][oó]n[eé]s' OR  lower(a.email) ~* 'br[ií][oó]n[eé]s' OR  text(a.id) ~* 'br[ií][oó]n[eé]s' ) 
