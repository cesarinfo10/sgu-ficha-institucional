COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]ts[ií]' OR  a.rut ~* 'l[ií]ts[ií]' OR  lower(a.email) ~* 'l[ií]ts[ií]' OR  text(a.id) ~* 'l[ií]ts[ií]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]ntr[eé]r[aá]s' OR  a.rut ~* 'c[oó]ntr[eé]r[aá]s' OR  lower(a.email) ~* 'c[oó]ntr[eé]r[aá]s' OR  text(a.id) ~* 'c[oó]ntr[eé]r[aá]s' ) 
