COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií][oó]s[eé]c[oó]' OR  a.rut ~* 'r[ií][oó]s[eé]c[oó]' OR  lower(a.email) ~* 'r[ií][oó]s[eé]c[oó]' OR  text(a.id) ~* 'r[ií][oó]s[eé]c[oó]' ) 
