COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]h[aá]nn[aá]' OR  a.rut ~* 'j[oó]h[aá]nn[aá]' OR  lower(a.email) ~* 'j[oó]h[aá]nn[aá]' OR  text(a.id) ~* 'j[oó]h[aá]nn[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lv[aá]r[eé]z' OR  a.rut ~* '[aá]lv[aá]r[eé]z' OR  lower(a.email) ~* '[aá]lv[aá]r[eé]z' OR  text(a.id) ~* '[aá]lv[aá]r[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'cr[ií]st[ií]' OR  a.rut ~* 'cr[ií]st[ií]' OR  lower(a.email) ~* 'cr[ií]st[ií]' OR  text(a.id) ~* 'cr[ií]st[ií]' ) 
