COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'v[ií]ct[oó]r'  OR u.rut ~* 'v[ií]ct[oó]r')  AND (lower(u.nombre||' '||u.apellido) ~* 's[oó]t[oó]'  OR u.rut ~* 's[oó]t[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[aá]m[ií]r[eé]z'  OR u.rut ~* 'r[aá]m[ií]r[eé]z') 
