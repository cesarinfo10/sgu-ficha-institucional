COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]l[eé]j[aá]ndr[oó]'  OR u.rut ~* '[aá]l[eé]j[aá]ndr[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[eé]t[eé]l[ií][eé]r'  OR u.rut ~* 'l[eé]t[eé]l[ií][eé]r') 
