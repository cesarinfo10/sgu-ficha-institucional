COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]r[oó]l[ií]n[aá]'  OR u.rut ~* 'c[aá]r[oó]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]st[ií]ll[oó]'  OR u.rut ~* 'c[aá]st[ií]ll[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[oó]dr[ií]g[uú][eé]z'  OR u.rut ~* 'r[oó]dr[ií]g[uú][eé]z') 
