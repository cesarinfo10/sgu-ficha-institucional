COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'r[aá]q[uú][eé]l'  OR u.rut ~* 'r[aá]q[uú][eé]l')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[eé]r[eé][ií]r[aá]'  OR u.rut ~* 'p[eé]r[eé][ií]r[aá]') 
