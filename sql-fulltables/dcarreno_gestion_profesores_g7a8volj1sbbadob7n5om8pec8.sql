COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]str[oó]'  OR u.rut ~* 'c[aá]str[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]rd[eé]n[aá]s'  OR u.rut ~* 'c[aá]rd[eé]n[aá]s') 
