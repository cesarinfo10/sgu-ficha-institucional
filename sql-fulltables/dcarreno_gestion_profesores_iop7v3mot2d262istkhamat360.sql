COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'd[aá]nny'  OR u.rut ~* 'd[aá]nny')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[aá]m[ií]r[eé]z'  OR u.rut ~* 'r[aá]m[ií]r[eé]z') 
