COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'gr[eé]g[oó]r[ií][oó]'  OR u.rut ~* 'gr[eé]g[oó]r[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]g[uú][ií]l[eé]r[aá]'  OR u.rut ~* '[aá]g[uú][ií]l[eé]r[aá]') 
