COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]r[oó]l[ií]n[aá]'  OR u.rut ~* 'c[aá]r[oó]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[eé]y[eé]s'  OR u.rut ~* 'r[eé]y[eé]s') 
