COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'l[uú][ií]s'  OR u.rut ~* 'l[uú][ií]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]st[ií]ll[oó]'  OR u.rut ~* 'c[aá]st[ií]ll[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[oó]d[oó]y'  OR u.rut ~* 'g[oó]d[oó]y') 
