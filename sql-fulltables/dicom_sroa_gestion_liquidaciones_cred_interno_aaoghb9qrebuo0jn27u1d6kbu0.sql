COPY (SELECT 825570 AS cod_aportante, replace(lpad(trim(a.rut),10,'0'),'-','') AS rut,
                  ...
                  WHERE (lower(va.nombre) ~* '10051233' OR  a.rut ~* '10051233' OR  text(c.id) ~* '10051233' )  AND (SELECT sum(monto-coalesce(monto_abonado,0)) FROM finanzas.cobros WHERE id_convenio_ci=c.id AND (NOT pagado OR abonado) AND fecha_venc<now()::date AND fecha_dicom IS NULL) > 0
