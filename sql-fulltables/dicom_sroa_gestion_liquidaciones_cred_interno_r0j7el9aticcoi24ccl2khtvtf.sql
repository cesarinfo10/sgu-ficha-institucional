COPY (SELECT 825570 AS cod_aportante, replace(lpad(trim(a.rut),10,'0'),'-','') AS rut,
                  ...
                  WHERE true AND NOT c.nulo AND (car.regimen = 'PRE') AND vc.cant_cuotas_morosas >= 1  AND (SELECT sum(monto-coalesce(monto_abonado,0)) FROM finanzas.cobros WHERE id_convenio_ci=c.id AND (NOT pagado OR abonado) AND fecha_venc<now()::date AND fecha_dicom IS NULL) > 0
