COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]rl[oó]s'  OR u.rut ~* 'c[aá]rl[oó]s')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]g[uú][ií]l[aá]r'  OR u.rut ~* '[aá]g[uú][ií]l[aá]r') 
