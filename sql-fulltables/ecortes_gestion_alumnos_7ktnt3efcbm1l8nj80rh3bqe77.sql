COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]n[aá]' OR  a.rut ~* 'l[uú]n[aá]' OR  lower(a.email) ~* 'l[uú]n[aá]' OR  text(a.id) ~* 'l[uú]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá][aá]r[oó]n' OR  a.rut ~* '[aá][aá]r[oó]n' OR  lower(a.email) ~* '[aá][aá]r[oó]n' OR  text(a.id) ~* '[aá][aá]r[oó]n' ) 
