COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]c[oó]l[eé]tt' OR  a.rut ~* 'n[ií]c[oó]l[eé]tt' OR  lower(a.email) ~* 'n[ií]c[oó]l[eé]tt' OR  text(a.id) ~* 'n[ií]c[oó]l[eé]tt' ) 
