COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]v[aá]' OR  a.rut ~* '[eé]v[aá]' OR  lower(a.email) ~* '[eé]v[aá]' OR  text(a.id) ~* '[eé]v[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó][eé]n[aá]' OR  a.rut ~* 'm[oó][eé]n[aá]' OR  lower(a.email) ~* 'm[oó][eé]n[aá]' OR  text(a.id) ~* 'm[oó][eé]n[aá]' ) 
