COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]st' OR  a.rut ~* 'g[uú]st' OR  lower(a.email) ~* 'g[uú]st' OR  text(a.id) ~* 'g[uú]st' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g' OR  a.rut ~* 'g' OR  lower(a.email) ~* 'g' OR  text(a.id) ~* 'g' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
