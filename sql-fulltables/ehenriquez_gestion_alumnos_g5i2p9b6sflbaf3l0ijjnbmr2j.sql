COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  a.rut ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  lower(a.email) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  text(a.id) ~* 'v[aá]l[eé]nt[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]ch[eé]c[oó]' OR  a.rut ~* 'p[aá]ch[eé]c[oó]' OR  lower(a.email) ~* 'p[aá]ch[eé]c[oó]' OR  text(a.id) ~* 'p[aá]ch[eé]c[oó]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
