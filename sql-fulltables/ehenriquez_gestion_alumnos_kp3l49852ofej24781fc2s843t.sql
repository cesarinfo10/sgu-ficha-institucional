COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'sch[eé]yl[aá]' OR  a.rut ~* 'sch[eé]yl[aá]' OR  lower(a.email) ~* 'sch[eé]yl[aá]' OR  text(a.id) ~* 'sch[eé]yl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]n[aá]' OR  a.rut ~* 'l[uú]n[aá]' OR  lower(a.email) ~* 'l[uú]n[aá]' OR  text(a.id) ~* 'l[uú]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]nd[aá]' OR  a.rut ~* '[aá]r[aá]nd[aá]' OR  lower(a.email) ~* '[aá]r[aá]nd[aá]' OR  text(a.id) ~* '[aá]r[aá]nd[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  a.rut ~* 'r[oó]dr[ií]g[uú][eé]z' OR  lower(a.email) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  text(a.id) ~* 'r[oó]dr[ií]g[uú][eé]z' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
