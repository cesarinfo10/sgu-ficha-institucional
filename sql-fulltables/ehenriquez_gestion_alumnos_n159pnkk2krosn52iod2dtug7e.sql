COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]r[ií]nk[aá]' OR  a.rut ~* 'd[aá]r[ií]nk[aá]' OR  lower(a.email) ~* 'd[aá]r[ií]nk[aá]' OR  text(a.id) ~* 'd[aá]r[ií]nk[aá]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
