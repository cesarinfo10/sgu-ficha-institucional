COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]t' OR  a.rut ~* 'k[aá]t' OR  lower(a.email) ~* 'k[aá]t' OR  text(a.id) ~* 'k[aá]t' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]zm[aá]n' OR  a.rut ~* 'g[uú]zm[aá]n' OR  lower(a.email) ~* 'g[uú]zm[aá]n' OR  text(a.id) ~* 'g[uú]zm[aá]n' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
