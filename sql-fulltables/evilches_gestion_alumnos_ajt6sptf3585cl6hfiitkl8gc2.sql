COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[ií]c[ií][aá]n[aá]' OR  a.rut ~* 't[ií]c[ií][aá]n[aá]' OR  lower(a.email) ~* 't[ií]c[ií][aá]n[aá]' OR  text(a.id) ~* 't[ií]c[ií][aá]n[aá]' ) 
