COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR  a.rut ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR  lower(a.email) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR  text(a.id) ~* 'p[eé]rn[ií]g[oó]tt[ií]' ) 
