COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE (p.nro_boleta IS NOT NULL OR p.nro_factura IS NOT NULL) AND p.fecha BETWEEN '01-06-2023'::date AND '30-07-2023'::date AND p.id_cajero=1180 AND ('POST-TD' IN (car.regimen,car2.regimen,car3.regimen) OR p.nulo)  
