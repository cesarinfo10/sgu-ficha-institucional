COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'y[aá]m[eé]tt[ií]' OR  pap.rut ~* 'y[aá]m[eé]tt[ií]' OR lower(a.nombres||' '||a.apellidos) ~* 'y[aá]m[eé]tt[ií]' OR lower(a3.nombres||' '||a3.apellidos) ~* 'y[aá]m[eé]tt[ií]' OR  a.rut ~* 'y[aá]m[eé]tt[ií]' OR a2.rut ~* 'y[aá]m[eé]tt[ií]' OR a3.rut ~* 'y[aá]m[eé]tt[ií]' )  
