COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'br[ií]ss[oó]' OR  pap.rut ~* 'br[ií]ss[oó]' OR lower(a.nombres||' '||a.apellidos) ~* 'br[ií]ss[oó]' OR lower(a3.nombres||' '||a3.apellidos) ~* 'br[ií]ss[oó]' OR  a.rut ~* 'br[ií]ss[oó]' OR a2.rut ~* 'br[ií]ss[oó]' OR a3.rut ~* 'br[ií]ss[oó]' )  
