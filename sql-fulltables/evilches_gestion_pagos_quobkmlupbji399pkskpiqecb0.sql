COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[aá]p[ií][aá]' OR  pap.rut ~* 't[aá]p[ií][aá]' OR lower(a.nombres||' '||a.apellidos) ~* 't[aá]p[ií][aá]' OR lower(a3.nombres||' '||a3.apellidos) ~* 't[aá]p[ií][aá]' OR  a.rut ~* 't[aá]p[ií][aá]' OR a2.rut ~* 't[aá]p[ií][aá]' OR a3.rut ~* 't[aá]p[ií][aá]' )  
