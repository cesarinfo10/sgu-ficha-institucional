COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '14148317-k' OR  pap.rut ~* '14148317-k' OR lower(a.nombres||' '||a.apellidos) ~* '14148317-k' OR lower(a3.nombres||' '||a3.apellidos) ~* '14148317-k' OR  a.rut ~* '14148317-k' OR a2.rut ~* '14148317-k' OR a3.rut ~* '14148317-k' )  
