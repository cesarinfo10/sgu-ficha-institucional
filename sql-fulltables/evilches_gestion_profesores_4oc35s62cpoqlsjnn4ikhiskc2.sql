COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]l[eé]x[ií]s'  OR u.rut ~* '[aá]l[eé]x[ií]s')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá][aá]v[eé]dr[aá]'  OR u.rut ~* 's[aá][aá]v[eé]dr[aá]') 
