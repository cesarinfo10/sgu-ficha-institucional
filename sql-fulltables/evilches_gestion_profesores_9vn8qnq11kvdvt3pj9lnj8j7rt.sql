COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'r[oó]ss[aá]n[aá]'  OR u.rut ~* 'r[oó]ss[aá]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]ñ[eé]t[eé]'  OR u.rut ~* 'c[aá]ñ[eé]t[eé]') 
