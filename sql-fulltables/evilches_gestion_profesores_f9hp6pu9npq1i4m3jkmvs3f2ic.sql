COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'd[aá]n[ií][eé]l'  OR u.rut ~* 'd[aá]n[ií][eé]l')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá]nt[ií]b[aá]ñ[eé]z'  OR u.rut ~* 's[aá]nt[ií]b[aá]ñ[eé]z') 
