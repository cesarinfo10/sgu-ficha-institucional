COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 't[aá]ll[eé]r'  OR u.rut ~* 't[aá]ll[eé]r')  AND (lower(u.nombre||' '||u.apellido) ~* 'd[eé]'  OR u.rut ~* 'd[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* 'pr[oó]c[eé]d[ií]m[ií][eé]nt[oó]'  OR u.rut ~* 'pr[oó]c[eé]d[ií]m[ií][eé]nt[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'd[eé]'  OR u.rut ~* 'd[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[oó]l[ií]c[ií][aá]'  OR u.rut ~* 'p[oó]l[ií]c[ií][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[oó]c[aá]l'  OR u.rut ~* 'l[oó]c[aá]l') 
