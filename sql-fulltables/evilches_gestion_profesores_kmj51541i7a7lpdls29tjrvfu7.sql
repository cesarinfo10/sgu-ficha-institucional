COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'fl[oó]r[eé]s'  OR u.rut ~* 'fl[oó]r[eé]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'j[oó]s[eé]'  OR u.rut ~* 'j[oó]s[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
