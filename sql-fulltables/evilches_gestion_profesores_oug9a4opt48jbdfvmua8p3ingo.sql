COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 's[eé]rg[ií][oó]'  OR u.rut ~* 's[eé]rg[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[eé][aá]ndr[oó]'  OR u.rut ~* 'l[eé][aá]ndr[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[uú][aá]j[aá]rd[oó]'  OR u.rut ~* 'g[uú][aá]j[aá]rd[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[oó]ntr[eé]r[aá]s'  OR u.rut ~* 'c[oó]ntr[eé]r[aá]s') 
