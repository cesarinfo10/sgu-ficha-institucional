COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[aá]n[ií]s' OR  a.rut ~* '[aá]l[aá]n[ií]s' OR  lower(a.email) ~* '[aá]l[aá]n[ií]s' OR  text(a.id) ~* '[aá]l[aá]n[ií]s' )  AND a.carrera_actual IN (106,120) 
