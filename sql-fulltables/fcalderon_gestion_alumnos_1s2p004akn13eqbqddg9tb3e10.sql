COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]l[ií][aá]n[aá]' OR  a.rut ~* 'l[ií]l[ií][aá]n[aá]' OR  lower(a.email) ~* 'l[ií]l[ií][aá]n[aá]' OR  text(a.id) ~* 'l[ií]l[ií][aá]n[aá]' )  AND a.carrera_actual IN (106,120) 
