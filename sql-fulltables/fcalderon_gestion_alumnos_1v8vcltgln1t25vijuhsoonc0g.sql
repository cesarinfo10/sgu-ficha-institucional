COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '26907355-1' OR  a.rut ~* '26907355-1' OR  lower(a.email) ~* '26907355-1' OR  text(a.id) ~* '26907355-1' )  AND a.carrera_actual IN (106,120) 
