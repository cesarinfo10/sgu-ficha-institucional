COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]n[ií][aá]' OR  a.rut ~* 'd[aá]n[ií][aá]' OR  lower(a.email) ~* 'd[aá]n[ií][aá]' OR  text(a.id) ~* 'd[aá]n[ií][aá]' )  AND a.carrera_actual IN (106,120) 
