COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '16151937-5' OR  a.rut ~* '16151937-5' OR  lower(a.email) ~* '16151937-5' OR  text(a.id) ~* '16151937-5' )  AND a.carrera_actual IN (106,120) 
