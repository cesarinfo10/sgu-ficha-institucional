COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'tr[oó]nc[oó]s[oó]' OR  a.rut ~* 'tr[oó]nc[oó]s[oó]' OR  lower(a.email) ~* 'tr[oó]nc[oó]s[oó]' OR  text(a.id) ~* 'tr[oó]nc[oó]s[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]r[oó]' OR  a.rut ~* 't[oó]r[oó]' OR  lower(a.email) ~* 't[oó]r[oó]' OR  text(a.id) ~* 't[oó]r[oó]' )  AND a.carrera_actual IN (106,120) 
