COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '168051093' OR  a.rut ~* '168051093' OR  lower(a.email) ~* '168051093' OR  text(a.id) ~* '168051093' )  AND a.carrera_actual IN (106,120) 
