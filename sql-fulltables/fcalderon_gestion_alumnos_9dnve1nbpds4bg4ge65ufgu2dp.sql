COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]nt[oó]' OR  a.rut ~* 'p[ií]nt[oó]' OR  lower(a.email) ~* 'p[ií]nt[oó]' OR  text(a.id) ~* 'p[ií]nt[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ndr[aá]d[eé]' OR  a.rut ~* '[aá]ndr[aá]d[eé]' OR  lower(a.email) ~* '[aá]ndr[aá]d[eé]' OR  text(a.id) ~* '[aá]ndr[aá]d[eé]' )  AND a.carrera_actual IN (106,120) 
