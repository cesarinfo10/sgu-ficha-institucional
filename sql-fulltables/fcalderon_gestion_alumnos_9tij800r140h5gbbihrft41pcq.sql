COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]m[ií]n[aá]' OR  a.rut ~* 'r[oó]m[ií]n[aá]' OR  lower(a.email) ~* 'r[oó]m[ií]n[aá]' OR  text(a.id) ~* 'r[oó]m[ií]n[aá]' )  AND a.carrera_actual IN (106,120) 
