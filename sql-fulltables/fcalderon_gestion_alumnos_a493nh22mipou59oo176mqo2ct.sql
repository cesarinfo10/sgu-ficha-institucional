COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][uú]l[ií]n[aá]' OR  a.rut ~* 'p[aá][uú]l[ií]n[aá]' OR  lower(a.email) ~* 'p[aá][uú]l[ií]n[aá]' OR  text(a.id) ~* 'p[aá][uú]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]mp[oó]s' OR  a.rut ~* 'c[aá]mp[oó]s' OR  lower(a.email) ~* 'c[aá]mp[oó]s' OR  text(a.id) ~* 'c[aá]mp[oó]s' )  AND a.carrera_actual IN (106,120) 
