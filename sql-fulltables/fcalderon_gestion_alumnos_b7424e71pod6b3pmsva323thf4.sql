COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]nd[aá]c[aá]' OR  a.rut ~* 'm[oó]nd[aá]c[aá]' OR  lower(a.email) ~* 'm[oó]nd[aá]c[aá]' OR  text(a.id) ~* 'm[oó]nd[aá]c[aá]' )  AND a.carrera_actual IN (106,120) 
