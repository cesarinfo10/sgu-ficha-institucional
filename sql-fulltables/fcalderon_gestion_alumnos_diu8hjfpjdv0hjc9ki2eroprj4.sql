COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[aá]mb[ií][oó]' OR  a.rut ~* 'm[aá]r[aá]mb[ií][oó]' OR  lower(a.email) ~* 'm[aá]r[aá]mb[ií][oó]' OR  text(a.id) ~* 'm[aá]r[aá]mb[ií][oó]' )  AND a.carrera_actual IN (106,120) 
