COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]l[ií][aá]n[aá]' OR  a.rut ~* 'l[ií]l[ií][aá]n[aá]' OR  lower(a.email) ~* 'l[ií]l[ií][aá]n[aá]' OR  text(a.id) ~* 'l[ií]l[ií][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá][eé]d[oó]' OR  a.rut ~* '[aá][eé]d[oó]' OR  lower(a.email) ~* '[aá][eé]d[oó]' OR  text(a.id) ~* '[aá][eé]d[oó]' )  AND a.carrera_actual IN (106,120) 
