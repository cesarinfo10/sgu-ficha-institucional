COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]v[ií][eé]d[eé]s' OR  a.rut ~* 'c[aá]v[ií][eé]d[eé]s' OR  lower(a.email) ~* 'c[aá]v[ií][eé]d[eé]s' OR  text(a.id) ~* 'c[aá]v[ií][eé]d[eé]s' )  AND a.carrera_actual IN (106,120) 
