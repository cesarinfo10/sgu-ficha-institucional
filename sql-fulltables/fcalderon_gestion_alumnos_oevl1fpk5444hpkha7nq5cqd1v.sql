COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[ií]s[eé]ll[aá]' OR  a.rut ~* 'g[ií]s[eé]ll[aá]' OR  lower(a.email) ~* 'g[ií]s[eé]ll[aá]' OR  text(a.id) ~* 'g[ií]s[eé]ll[aá]' )  AND a.carrera_actual IN (106,120) 
