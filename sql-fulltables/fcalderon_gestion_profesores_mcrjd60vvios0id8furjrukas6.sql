COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND u.funcion='Docente'  AND u.id IN (SELECT vc.id_profesor 
						  FROM vista_cursos  AS vc
						  LEFT JOIN carreras AS car ON car.id=vc.id_carrera 
						  WHERE true  AND vc.id_carrera=120  AND vc.ano=2022  AND vc.id_carrera IN (106,120)  AND car.regimen='PRE-D' )
