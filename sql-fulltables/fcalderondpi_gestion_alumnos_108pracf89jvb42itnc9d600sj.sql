COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'gl[aá]dys' OR  a.rut ~* 'gl[aá]dys' OR  lower(a.email) ~* 'gl[aá]dys' OR  text(a.id) ~* 'gl[aá]dys' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií][eé]rr[aá]' OR  a.rut ~* 's[ií][eé]rr[aá]' OR  lower(a.email) ~* 's[ií][eé]rr[aá]' OR  text(a.id) ~* 's[ií][eé]rr[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
