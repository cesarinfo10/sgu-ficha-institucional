COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2023 AND regimen='DIP-D' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2023  
                FROM alumnos AS a
                ...
                WHERE true  AND (c.regimen = 'DIP-D') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2022 AND semestre=2))  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
