COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]rr[eé]s' OR  a.rut ~* 't[oó]rr[eé]s' OR  lower(a.email) ~* 't[oó]rr[eé]s' OR  text(a.id) ~* 't[oó]rr[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]st[oó]rg[aá]' OR  a.rut ~* '[aá]st[oó]rg[aá]' OR  lower(a.email) ~* '[aá]st[oó]rg[aá]' OR  text(a.id) ~* '[aá]st[oó]rg[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]s[aá]r[ií][oó]' OR  a.rut ~* 'r[oó]s[aá]r[ií][oó]' OR  lower(a.email) ~* 'r[oó]s[aá]r[ií][oó]' OR  text(a.id) ~* 'r[oó]s[aá]r[ií][oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
