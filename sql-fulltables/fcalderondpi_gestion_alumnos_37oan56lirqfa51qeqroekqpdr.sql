COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[oó]h[aá]n[aá]' OR  a.rut ~* 'y[oó]h[aá]n[aá]' OR  lower(a.email) ~* 'y[oó]h[aá]n[aá]' OR  text(a.id) ~* 'y[oó]h[aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]t[ií]c[aá]' OR  a.rut ~* 'g[aá]t[ií]c[aá]' OR  lower(a.email) ~* 'g[aá]t[ií]c[aá]' OR  text(a.id) ~* 'g[aá]t[ií]c[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
