COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú]l[ií][eé]t[aá]' OR  a.rut ~* 'j[uú]l[ií][eé]t[aá]' OR  lower(a.email) ~* 'j[uú]l[ií][eé]t[aá]' OR  text(a.id) ~* 'j[uú]l[ií][eé]t[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[aá]br[aá]' OR  a.rut ~* 'l[aá]br[aá]' OR  lower(a.email) ~* 'l[aá]br[aá]' OR  text(a.id) ~* 'l[aá]br[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
