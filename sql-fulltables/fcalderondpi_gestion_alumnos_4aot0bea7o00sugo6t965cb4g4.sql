COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[ií][oó]v[aá]nn[aá]' OR  a.rut ~* 'g[ií][oó]v[aá]nn[aá]' OR  lower(a.email) ~* 'g[ií][oó]v[aá]nn[aá]' OR  text(a.id) ~* 'g[ií][oó]v[aá]nn[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[aá]ld[oó]' OR  a.rut ~* 'c[aá]t[aá]ld[oó]' OR  lower(a.email) ~* 'c[aá]t[aá]ld[oó]' OR  text(a.id) ~* 'c[aá]t[aá]ld[oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
