COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé]nn[ií]f[eé]r' OR  a.rut ~* 'j[eé]nn[ií]f[eé]r' OR  lower(a.email) ~* 'j[eé]nn[ií]f[eé]r' OR  text(a.id) ~* 'j[eé]nn[ií]f[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][ií]ñ[oó]n[eé]z' OR  a.rut ~* 'q[uú][ií]ñ[oó]n[eé]z' OR  lower(a.email) ~* 'q[uú][ií]ñ[oó]n[eé]z' OR  text(a.id) ~* 'q[uú][ií]ñ[oó]n[eé]z' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
