COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nst[aá]nz[aá]' OR  a.rut ~* 'c[oó]nst[aá]nz[aá]' OR  lower(a.email) ~* 'c[oó]nst[aá]nz[aá]' OR  text(a.id) ~* 'c[oó]nst[aá]nz[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]ñ[aá]t[eé]' OR  a.rut ~* '[oó]ñ[aá]t[eé]' OR  lower(a.email) ~* '[oó]ñ[aá]t[eé]' OR  text(a.id) ~* '[oó]ñ[aá]t[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  a.rut ~* 'm[aá]nr[ií]q[uú][eé]z' OR  lower(a.email) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  text(a.id) ~* 'm[aá]nr[ií]q[uú][eé]z' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
