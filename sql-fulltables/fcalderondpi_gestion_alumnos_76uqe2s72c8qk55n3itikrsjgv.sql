COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[ií]c[ií][aá]' OR  a.rut ~* '[aá]l[ií]c[ií][aá]' OR  lower(a.email) ~* '[aá]l[ií]c[ií][aá]' OR  text(a.id) ~* '[aá]l[ií]c[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]nc[ií]b[ií][aá]' OR  a.rut ~* '[aá]r[aá]nc[ií]b[ií][aá]' OR  lower(a.email) ~* '[aá]r[aá]nc[ií]b[ií][aá]' OR  text(a.id) ~* '[aá]r[aá]nc[ií]b[ií][aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
