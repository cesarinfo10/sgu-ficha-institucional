COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[oó][eé]l[ií][aá]' OR  a.rut ~* 'n[oó][eé]l[ií][aá]' OR  lower(a.email) ~* 'n[oó][eé]l[ií][aá]' OR  text(a.id) ~* 'n[oó][eé]l[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]bl[eé]s' OR  a.rut ~* 'r[oó]bl[eé]s' OR  lower(a.email) ~* 'r[oó]bl[eé]s' OR  text(a.id) ~* 'r[oó]bl[eé]s' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
