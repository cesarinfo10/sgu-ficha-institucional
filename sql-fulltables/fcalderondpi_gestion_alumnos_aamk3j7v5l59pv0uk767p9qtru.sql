COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l[ií]c[aá]' OR  a.rut ~* '[aá]ng[eé]l[ií]c[aá]' OR  lower(a.email) ~* '[aá]ng[eé]l[ií]c[aá]' OR  text(a.id) ~* '[aá]ng[eé]l[ií]c[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]l[ií]n[aá]s' OR  a.rut ~* 's[aá]l[ií]n[aá]s' OR  lower(a.email) ~* 's[aá]l[ií]n[aá]s' OR  text(a.id) ~* 's[aá]l[ií]n[aá]s' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
