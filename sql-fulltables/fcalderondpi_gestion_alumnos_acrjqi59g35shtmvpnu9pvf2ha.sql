COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]r[oó]n[ií]c[aá]' OR  a.rut ~* 'v[eé]r[oó]n[ií]c[aá]' OR  lower(a.email) ~* 'v[eé]r[oó]n[ií]c[aá]' OR  text(a.id) ~* 'v[eé]r[oó]n[ií]c[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]ñ[aá]' OR  a.rut ~* 'p[eé]ñ[aá]' OR  lower(a.email) ~* 'p[eé]ñ[aá]' OR  text(a.id) ~* 'p[eé]ñ[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]c[uú]ñ[aá]' OR  a.rut ~* '[aá]c[uú]ñ[aá]' OR  lower(a.email) ~* '[aá]c[uú]ñ[aá]' OR  text(a.id) ~* '[aá]c[uú]ñ[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
