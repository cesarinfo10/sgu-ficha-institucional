COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]r[eé]nch[uú]' OR  a.rut ~* 'm[ií]r[eé]nch[uú]' OR  lower(a.email) ~* 'm[ií]r[eé]nch[uú]' OR  text(a.id) ~* 'm[ií]r[eé]nch[uú]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  a.rut ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  lower(a.email) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  text(a.id) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
