COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]t[ií]h[uú]sk[aá]' OR  a.rut ~* 'k[aá]t[ií]h[uú]sk[aá]' OR  lower(a.email) ~* 'k[aá]t[ií]h[uú]sk[aá]' OR  text(a.id) ~* 'k[aá]t[ií]h[uú]sk[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]t[aá]' OR  a.rut ~* 'm[oó]t[aá]' OR  lower(a.email) ~* 'm[oó]t[aá]' OR  text(a.id) ~* 'm[oó]t[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
