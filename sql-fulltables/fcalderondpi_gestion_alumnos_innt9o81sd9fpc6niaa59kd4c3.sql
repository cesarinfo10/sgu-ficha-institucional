COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]r[ií]s' OR  a.rut ~* 'p[aá]r[ií]s' OR  lower(a.email) ~* 'p[aá]r[ií]s' OR  text(a.id) ~* 'p[aá]r[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]g[uú][eé]l' OR  a.rut ~* 's[eé]g[uú][eé]l' OR  lower(a.email) ~* 's[eé]g[uú][eé]l' OR  text(a.id) ~* 's[eé]g[uú][eé]l' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
