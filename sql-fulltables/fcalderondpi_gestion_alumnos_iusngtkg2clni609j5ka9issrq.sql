COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nst[aá]nz[aá]' OR  a.rut ~* 'c[oó]nst[aá]nz[aá]' OR  lower(a.email) ~* 'c[oó]nst[aá]nz[aá]' OR  text(a.id) ~* 'c[oó]nst[aá]nz[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]rc[ií][aá]' OR  a.rut ~* 'g[aá]rc[ií][aá]' OR  lower(a.email) ~* 'g[aá]rc[ií][aá]' OR  text(a.id) ~* 'g[aá]rc[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]r[eé]d[eé]s' OR  a.rut ~* 'p[aá]r[eé]d[eé]s' OR  lower(a.email) ~* 'p[aá]r[eé]d[eé]s' OR  text(a.id) ~* 'p[aá]r[eé]d[eé]s' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
