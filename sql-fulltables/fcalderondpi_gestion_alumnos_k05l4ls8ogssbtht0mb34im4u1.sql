COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]ngr[ií]d' OR  a.rut ~* '[ií]ngr[ií]d' OR  lower(a.email) ~* '[ií]ngr[ií]d' OR  text(a.id) ~* '[ií]ngr[ií]d' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[uú]t[uú]rr[uú]f[oó]' OR  a.rut ~* 'c[uú]t[uú]rr[uú]f[oó]' OR  lower(a.email) ~* 'c[uú]t[uú]rr[uú]f[oó]' OR  text(a.id) ~* 'c[uú]t[uú]rr[uú]f[oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148) 
