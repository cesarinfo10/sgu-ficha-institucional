COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]n[aá]th[aá]n' OR  a.rut ~* 'j[oó]n[aá]th[aá]n' OR  lower(a.email) ~* 'j[oó]n[aá]th[aá]n' OR  text(a.id) ~* 'j[oó]n[aá]th[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé][aá]' OR  a.rut ~* 'c[eé][aá]' OR  lower(a.email) ~* 'c[eé][aá]' OR  text(a.id) ~* 'c[eé][aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
