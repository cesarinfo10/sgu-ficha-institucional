COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]c[oó]l[aá]' OR  a.rut ~* 'n[ií]c[oó]l[aá]' OR  lower(a.email) ~* 'n[ií]c[oó]l[aá]' OR  text(a.id) ~* 'n[ií]c[oó]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]nn[aá]r[oó]' OR  a.rut ~* 'g[eé]nn[aá]r[oó]' OR  lower(a.email) ~* 'g[eé]nn[aá]r[oó]' OR  text(a.id) ~* 'g[eé]nn[aá]r[oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148) 
