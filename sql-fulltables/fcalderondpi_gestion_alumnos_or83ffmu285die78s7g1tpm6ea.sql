COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l[aá]' OR  a.rut ~* '[aá]ng[eé]l[aá]' OR  lower(a.email) ~* '[aá]ng[eé]l[aá]' OR  text(a.id) ~* '[aá]ng[eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé]lg[aá]d[oó]' OR  a.rut ~* 'd[eé]lg[aá]d[oó]' OR  lower(a.email) ~* 'd[eé]lg[aá]d[oó]' OR  text(a.id) ~* 'd[eé]lg[aá]d[oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
