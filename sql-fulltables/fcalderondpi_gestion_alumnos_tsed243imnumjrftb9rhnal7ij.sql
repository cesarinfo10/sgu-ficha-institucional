COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[uú]s[aá]n' OR  a.rut ~* 's[uú]s[aá]n' OR  lower(a.email) ~* 's[uú]s[aá]n' OR  text(a.id) ~* 's[uú]s[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]h[uú]m[aá]d[aá]' OR  a.rut ~* '[aá]h[uú]m[aá]d[aá]' OR  lower(a.email) ~* '[aá]h[uú]m[aá]d[aá]' OR  text(a.id) ~* '[aá]h[uú]m[aá]d[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
