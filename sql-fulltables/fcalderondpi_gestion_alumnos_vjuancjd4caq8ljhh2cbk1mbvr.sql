COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]' OR  a.rut ~* 'm[aá]r[ií][aá]' OR  lower(a.email) ~* 'm[aá]r[ií][aá]' OR  text(a.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[uú][eé][ií]ch[aá]q[uú][eé][oó]' OR  a.rut ~* 'h[uú][eé][ií]ch[aá]q[uú][eé][oó]' OR  lower(a.email) ~* 'h[uú][eé][ií]ch[aá]q[uú][eé][oó]' OR  text(a.id) ~* 'h[uú][eé][ií]ch[aá]q[uú][eé][oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
