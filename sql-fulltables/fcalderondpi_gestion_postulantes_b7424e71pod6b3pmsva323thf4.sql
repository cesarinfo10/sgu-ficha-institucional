COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[eé]ss[ií]c[aá]' OR  pap.rut ~* 'j[eé]ss[ií]c[aá]' OR  text(pap.id) ~* 'j[eé]ss[ií]c[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]nd[aá]c[aá]' OR  pap.rut ~* 'm[oó]nd[aá]c[aá]' OR  text(pap.id) ~* 'm[oó]nd[aá]c[aá]' ) 
