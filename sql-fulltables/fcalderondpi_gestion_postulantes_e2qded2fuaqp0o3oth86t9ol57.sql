COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá][oó]l[aá]' OR  pap.rut ~* 'p[aá][oó]l[aá]' OR  text(pap.id) ~* 'p[aá][oó]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[aá]p[ií][aá]' OR  pap.rut ~* 't[aá]p[ií][aá]' OR  text(pap.id) ~* 't[aá]p[ií][aá]' ) 
