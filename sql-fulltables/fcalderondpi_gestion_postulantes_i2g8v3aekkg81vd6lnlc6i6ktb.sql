COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]rt[ií]l[aá]' OR  pap.rut ~* 'b[eé]rt[ií]l[aá]' OR  text(pap.id) ~* 'b[eé]rt[ií]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[eé]r[eé]c[eé]d[aá]' OR  pap.rut ~* 'c[eé]r[eé]c[eé]d[aá]' OR  text(pap.id) ~* 'c[eé]r[eé]c[eé]d[aá]' ) 
