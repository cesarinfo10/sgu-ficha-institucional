COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]st[ií]ll[oó]' OR  pap.rut ~* 'c[aá]st[ií]ll[oó]' OR  text(pap.id) ~* 'c[aá]st[ií]ll[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]r[eé]ll[aá]n[aá]' OR  pap.rut ~* '[oó]r[eé]ll[aá]n[aá]' OR  text(pap.id) ~* '[oó]r[eé]ll[aá]n[aá]' ) 
