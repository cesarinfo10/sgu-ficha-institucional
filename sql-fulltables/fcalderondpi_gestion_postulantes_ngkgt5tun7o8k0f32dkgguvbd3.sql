COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú]c[aá]s' OR  pap.rut ~* 'l[uú]c[aá]s' OR  text(pap.id) ~* 'l[uú]c[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]mpb[eé]ll' OR  pap.rut ~* 'c[aá]mpb[eé]ll' OR  text(pap.id) ~* 'c[aá]mpb[eé]ll' ) 
