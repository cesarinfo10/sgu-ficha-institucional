COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'k[aá]th[eé]r[ií]n[eé]' OR  pap.rut ~* 'k[aá]th[eé]r[ií]n[eé]' OR  text(pap.id) ~* 'k[aá]th[eé]r[ií]n[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[ií][oó]v[aá]nn[aá]' OR  pap.rut ~* 'g[ií][oó]v[aá]nn[aá]' OR  text(pap.id) ~* 'g[ií][oó]v[aá]nn[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]ff[oó]l[ií]' OR  pap.rut ~* 't[oó]ff[oó]l[ií]' OR  text(pap.id) ~* 't[oó]ff[oó]l[ií]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]sp[eé]' OR  pap.rut ~* '[aá]sp[eé]' OR  text(pap.id) ~* '[aá]sp[eé]' ) 
