COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]rg[ií][oó]' OR  pap.rut ~* 's[eé]rg[ií][oó]' OR  text(pap.id) ~* 's[eé]rg[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú]c[oó]' OR  pap.rut ~* 'l[uú]c[oó]' OR  text(pap.id) ~* 'l[uú]c[oó]' ) 
