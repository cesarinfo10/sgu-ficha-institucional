COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'h[eé]rr[eé]r[aá]'  OR u.rut ~* 'h[eé]rr[eé]r[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[ií][uú]d[aá]d'  OR u.rut ~* 'c[ií][uú]d[aá]d') 
