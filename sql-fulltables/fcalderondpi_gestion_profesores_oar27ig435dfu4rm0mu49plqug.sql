COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]r[ií][aá]'  OR u.rut ~* 'm[aá]r[ií][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[eé]g[ií]n[aá]'  OR u.rut ~* 'r[eé]g[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
