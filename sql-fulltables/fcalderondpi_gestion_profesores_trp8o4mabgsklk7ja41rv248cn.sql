COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'k[aá]t[ií]h[uú]sk[aá]'  OR u.rut ~* 'k[aá]t[ií]h[uú]sk[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[oó]t[aá]'  OR u.rut ~* 'm[oó]t[aá]') 
