COPY (SELECT act.*,(SELECT char_comma_sum(tipo_publico||': '||coalesce((coalesce(cant_personas,0)+coalesce(cant_personas_virtuales,0))::text,'*')) 
			...
			WHERE true   AND (lower(act.nombre) ~* 'b[oó]l[eé]t[ií]n' OR  text(act.id) ~* 'b[oó]l[eé]t[ií]n' ) AND (lower(act.nombre) ~* 'n[oó]v[ií][eé]mbr[eé]' OR  text(act.id) ~* 'n[oó]v[ií][eé]mbr[eé]' ) AND (lower(act.nombre) ~* '2023' OR  text(act.id) ~* '2023' ) AND (lower(act.nombre) ~* 'd[eé]' OR  text(act.id) ~* 'd[eé]' ) AND (lower(act.nombre) ~* 'ps[ií]c[oó]l[oó]g[ií][aá]' OR  text(act.id) ~* 'ps[ií]c[oó]l[oó]g[ií][aá]' ) 
