COPY (SELECT poas.id,tipo_act,coalesce(p.nombre,'') AS proyecto,actividad,prioridad,
               ...
               WHERE date_part('year',fecha_prog_termino)=2023  AND date_part('month',fecha_prog_termino)=11  AND poas.id_unidad='26'  AND estado NOT IN ('OK','Terminada','Eliminada')  AND poas.comentarios_ult_fecha BETWEEN now()-'7 days'::interval AND now() 
