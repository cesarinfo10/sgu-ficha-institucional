COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE true AND (4 IN (pap.carrera1_post,pap.carrera2_post,pap.carrera3_post))  AND (pap.cohorte='2023')  AND (pap.semestre_cohorte='1')  AND (vpe.estado='Provisorio') AND ('PRE' IN (c1.regimen,c2.regimen,c3.regimen,c4.regimen,c5.regimen,c6.regimen)) 
