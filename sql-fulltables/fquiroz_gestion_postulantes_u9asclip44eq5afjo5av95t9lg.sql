COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  pap.rut ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  text(pap.id) ~* 'h[oó]rm[aá]z[aá]b[aá]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]c[eé]r[eé]s' OR  pap.rut ~* 'c[aá]c[eé]r[eé]s' OR  text(pap.id) ~* 'c[aá]c[eé]r[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]v[ií][eé]r[aá]' OR  pap.rut ~* 'j[aá]v[ií][eé]r[aá]' OR  text(pap.id) ~* 'j[aá]v[ií][eé]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]c[ií][oó]' OR  pap.rut ~* 'r[oó]c[ií][oó]' OR  text(pap.id) ~* 'r[oó]c[ií][oó]' ) 
