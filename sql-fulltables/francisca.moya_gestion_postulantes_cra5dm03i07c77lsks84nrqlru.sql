COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]lv[ií]r[aá]' OR  pap.rut ~* '[eé]lv[ií]r[aá]' OR  text(pap.id) ~* '[eé]lv[ií]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]t[aá]r[ií]z' OR  pap.rut ~* 'b[eé]t[aá]r[ií]z' OR  text(pap.id) ~* 'b[eé]t[aá]r[ií]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]ld[eé]z' OR  pap.rut ~* 'v[aá]ld[eé]z' OR  text(pap.id) ~* 'v[aá]ld[eé]z' ) 
