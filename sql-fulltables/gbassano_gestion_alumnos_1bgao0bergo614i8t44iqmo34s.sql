COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]ss[aá]n[oó]' OR  a.rut ~* 'b[aá]ss[aá]n[oó]' OR  lower(a.email) ~* 'b[aá]ss[aá]n[oó]' OR  text(a.id) ~* 'b[aá]ss[aá]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[ií][aá]g[aá]' OR  a.rut ~* '[aá]l[ií][aá]g[aá]' OR  lower(a.email) ~* '[aá]l[ií][aá]g[aá]' OR  text(a.id) ~* '[aá]l[ií][aá]g[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'gl[oó]r[ií][aá]' OR  a.rut ~* 'gl[oó]r[ií][aá]' OR  lower(a.email) ~* 'gl[oó]r[ií][aá]' OR  text(a.id) ~* 'gl[oó]r[ií][aá]' ) 
