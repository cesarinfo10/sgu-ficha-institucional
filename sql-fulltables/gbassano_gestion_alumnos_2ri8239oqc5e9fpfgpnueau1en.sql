COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[oó]n' OR  a.rut ~* '[aá]r[oó]n' OR  lower(a.email) ~* '[aá]r[oó]n' OR  text(a.id) ~* '[aá]r[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]n[aá]' OR  a.rut ~* 'l[uú]n[aá]' OR  lower(a.email) ~* 'l[uú]n[aá]' OR  text(a.id) ~* 'l[uú]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]m[aá]ch[oó]' OR  a.rut ~* 'c[aá]m[aá]ch[oó]' OR  lower(a.email) ~* 'c[aá]m[aá]ch[oó]' OR  text(a.id) ~* 'c[aá]m[aá]ch[oó]' ) 
