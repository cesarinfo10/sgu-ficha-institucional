COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]mm[aá]' OR  a.rut ~* '[eé]mm[aá]' OR  lower(a.email) ~* '[eé]mm[aá]' OR  text(a.id) ~* '[eé]mm[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[ií]f[uú][eé]nt[eé]s' OR  a.rut ~* 'c[ií]f[uú][eé]nt[eé]s' OR  lower(a.email) ~* 'c[ií]f[uú][eé]nt[eé]s' OR  text(a.id) ~* 'c[ií]f[uú][eé]nt[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[ií]ñ[ií]r' OR  a.rut ~* '[aá]n[ií]ñ[ií]r' OR  lower(a.email) ~* '[aá]n[ií]ñ[ií]r' OR  text(a.id) ~* '[aá]n[ií]ñ[ií]r' ) 
