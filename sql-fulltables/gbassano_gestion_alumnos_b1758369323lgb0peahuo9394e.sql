COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]f[ií][aá]' OR  a.rut ~* 's[oó]f[ií][aá]' OR  lower(a.email) ~* 's[oó]f[ií][aá]' OR  text(a.id) ~* 's[oó]f[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[ií][aá]s' OR  a.rut ~* '[aá]r[ií][aá]s' OR  lower(a.email) ~* '[aá]r[ií][aá]s' OR  text(a.id) ~* '[aá]r[ií][aá]s' ) 
