COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó][ií]s[eé]s' OR  a.rut ~* 'm[oó][ií]s[eé]s' OR  lower(a.email) ~* 'm[oó][ií]s[eé]s' OR  text(a.id) ~* 'm[oó][ií]s[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'ch[aá]v[eé]z' OR  a.rut ~* 'ch[aá]v[eé]z' OR  lower(a.email) ~* 'ch[aá]v[eé]z' OR  text(a.id) ~* 'ch[aá]v[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  a.rut ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  lower(a.email) ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  text(a.id) ~* 'h[oó]rm[aá]z[aá]b[aá]l' ) 
