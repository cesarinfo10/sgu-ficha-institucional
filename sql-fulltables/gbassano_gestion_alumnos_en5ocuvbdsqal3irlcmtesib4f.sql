COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]rg[eé]' OR  a.rut ~* 'j[oó]rg[eé]' OR  lower(a.email) ~* 'j[oó]rg[eé]' OR  text(a.id) ~* 'j[oó]rg[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]ll[eé]g[oó]' OR  a.rut ~* 'g[aá]ll[eé]g[oó]' OR  lower(a.email) ~* 'g[aá]ll[eé]g[oó]' OR  text(a.id) ~* 'g[aá]ll[eé]g[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[aá]d[eé]m[aá]nn' OR  a.rut ~* 'fr[aá]d[eé]m[aá]nn' OR  lower(a.email) ~* 'fr[aá]d[eé]m[aá]nn' OR  text(a.id) ~* 'fr[aá]d[eé]m[aá]nn' ) 
