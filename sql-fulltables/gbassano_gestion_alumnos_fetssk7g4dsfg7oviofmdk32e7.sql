COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]dr[oó]' OR  a.rut ~* 'p[eé]dr[oó]' OR  lower(a.email) ~* 'p[eé]dr[oó]' OR  text(a.id) ~* 'p[eé]dr[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]z[uú][eé]l[aá]' OR  a.rut ~* 'm[aá]z[uú][eé]l[aá]' OR  lower(a.email) ~* 'm[aá]z[uú][eé]l[aá]' OR  text(a.id) ~* 'm[aá]z[uú][eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]q[uú][eé]' OR  a.rut ~* 'r[oó]q[uú][eé]' OR  lower(a.email) ~* 'r[oó]q[uú][eé]' OR  text(a.id) ~* 'r[oó]q[uú][eé]' ) 
