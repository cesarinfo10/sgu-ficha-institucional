COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]c[eé]lyn' OR  a.rut ~* 'j[oó]c[eé]lyn' OR  lower(a.email) ~* 'j[oó]c[eé]lyn' OR  text(a.id) ~* 'j[oó]c[eé]lyn' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]sp[ií]n[oó]z[aá]' OR  a.rut ~* '[eé]sp[ií]n[oó]z[aá]' OR  lower(a.email) ~* '[eé]sp[ií]n[oó]z[aá]' OR  text(a.id) ~* '[eé]sp[ií]n[oó]z[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií][oó]s' OR  a.rut ~* 'r[ií][oó]s' OR  lower(a.email) ~* 'r[ií][oó]s' OR  text(a.id) ~* 'r[ií][oó]s' ) 
