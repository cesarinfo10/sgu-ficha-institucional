COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé][ií]sy' OR  a.rut ~* 'd[eé][ií]sy' OR  lower(a.email) ~* 'd[eé][ií]sy' OR  text(a.id) ~* 'd[eé][ií]sy' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'sch[aá][aá]f' OR  a.rut ~* 'sch[aá][aá]f' OR  lower(a.email) ~* 'sch[aá][aá]f' OR  text(a.id) ~* 'sch[aá][aá]f' ) 
