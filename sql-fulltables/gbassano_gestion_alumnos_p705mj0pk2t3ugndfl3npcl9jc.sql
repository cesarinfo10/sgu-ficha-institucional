COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rg[aá]r[ií]t[aá]' OR  a.rut ~* 'm[aá]rg[aá]r[ií]t[aá]' OR  lower(a.email) ~* 'm[aá]rg[aá]r[ií]t[aá]' OR  text(a.id) ~* 'm[aá]rg[aá]r[ií]t[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]g[uú][ií]l[eé]r[aá]' OR  a.rut ~* '[aá]g[uú][ií]l[eé]r[aá]' OR  lower(a.email) ~* '[aá]g[uú][ií]l[eé]r[aá]' OR  text(a.id) ~* '[aá]g[uú][ií]l[eé]r[aá]' ) 
