COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT count(id) FROM sinergia.respuestas WHERE ano=2022 AND semestre=2 AND rut_alumno=a.rut)=2 THEN 'Si' ELSE 'No' END AS prubeas_psico,
                       CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2022 AND semestre=2 LIMIT 1)=1  THEN 'Si' ELSE 'No' END AS matriculado,(SELECT max(fecha) FROM matriculas WHERE id_alumno=a.id) AS fecha_ult_mat,
                       ...
                       CASE WHEN (SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE semestre=1 AND ano=2022))>0 THEN ((SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_estado=1 AND id_curso IN (SELECT id FROM cursos WHERE semestre=1 AND ano=2022))::real/(SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE semestre=1 AND ano=2022))::real*100)::numeric(4,1) ELSE 0 END AS tasa_aprobacion_ant,
                       CASE WHEN (SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE ano=2022 AND semestre=2))>0 THEN ((SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_estado=1 AND id_curso IN (SELECT id FROM cursos WHERE ano=2022 AND semestre=2))::real/(SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE ano=2022 AND semestre=2))::real*100)::numeric(4,1) ELSE 0 END AS tasa_aprobacion_act,
                       CASE WHEN (SELECT count(id_alumno) AS cant_insc FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2022) AND id_alumno=a.id)>0 THEN ((SELECT count(id_alumno) AS cant_aprob FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2022) AND id_estado=1 AND id_alumno=a.id)::real/(SELECT count(id_alumno) AS cant_insc FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2022) AND id_alumno=a.id)::real*100)::numeric(4,1) ELSE 0 END AS porcentaje_avance,
                       ...
                       (SELECT monto_arancel*CASE trim(tipo) WHEN 'Semestral' THEN 2 ELSE 1 END AS arancel_real FROM finanzas.contratos WHERE ano=2022 AND (id_alumno=a.id OR id_pap=a.id_pap) AND estado IN ('E','S','R','A') LIMIT 1) AS arancel_real,
                       ...
                       WHERE vac.ano=2022 AND vac.id_alumno=a.id),1) AS nivel_estudios, 
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2023 AND regimen='' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2023  
                FROM alumnos AS a
                ...
                LEFT JOIN (SELECT id_alumno,round(avg(nota_final),1) as nota_final,count(id) AS cant_asig FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2022-1) AND id_estado IN (1,2) GROUP BY id_alumno)       AS paa ON paa.id_alumno=a.id
                LEFT JOIN (SELECT id_alumno,round(avg(nota_final),1) as nota_final,count(id) AS cant_asig FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2022-1) AND id_estado=1 GROUP BY id_alumno) AS paaa ON paaa.id_alumno=a.id
                ...
                LEFT JOIN (SELECT DISTINCT ON (rut,be.nombre) rut,be.nombre AS benef_fiscal 
                               ...
                               WHERE ano=2022 AND id_beca_externa IS NOT NULL) AS be ON be.rut=a.rut
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]p[aá]z[oó]' OR  a.rut ~* '[oó]p[aá]z[oó]' OR  lower(a.email) ~* '[oó]p[aá]z[oó]' OR  text(a.id) ~* '[oó]p[aá]z[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]rq[uú][ií]z[aá]' OR  a.rut ~* '[uú]rq[uú][ií]z[aá]' OR  lower(a.email) ~* '[uú]rq[uú][ií]z[aá]' OR  text(a.id) ~* '[uú]rq[uú][ií]z[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]l[ií]l[aá]b' OR  a.rut ~* 'd[aá]l[ií]l[aá]b' OR  lower(a.email) ~* 'd[aá]l[ií]l[aá]b' OR  text(a.id) ~* 'd[aá]l[ií]l[aá]b' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]n[oó]l[eé]p[eé]' OR  a.rut ~* 'p[eé]n[oó]l[eé]p[eé]' OR  lower(a.email) ~* 'p[eé]n[oó]l[eé]p[eé]' OR  text(a.id) ~* 'p[eé]n[oó]l[eé]p[eé]' ) 
