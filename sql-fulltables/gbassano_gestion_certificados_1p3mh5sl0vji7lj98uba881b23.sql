COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]z' OR  a.rut ~* 'l[uú]z' OR  text(ac.folio) ~* 'l[uú]z' OR  vac.cod ~* 'l[uú]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][ií]v[aá]' OR  a.rut ~* 'l[eé][ií]v[aá]' OR  text(ac.folio) ~* 'l[eé][ií]v[aá]' OR  vac.cod ~* 'l[eé][ií]v[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]t[oó]' OR  a.rut ~* 's[oó]t[oó]' OR  text(ac.folio) ~* 's[oó]t[oó]' OR  vac.cod ~* 's[oó]t[oó]' ) 
