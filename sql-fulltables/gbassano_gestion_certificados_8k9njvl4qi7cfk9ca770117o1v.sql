COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,a.semestre_cohorte||'-'||a.cohorte AS cohorte,a.mes_cohorte,
                         ...
                         CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2024 AND semestre=1 LIMIT 1)=1 THEN 'Si' ELSE 'No' END AS matric,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  a.rut ~* 'r[oó]dr[ií]g[oó]' OR  text(ac.folio) ~* 'r[oó]dr[ií]g[oó]' OR  vac.cod ~* 'r[oó]dr[ií]g[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]t[ií][eé]rr[eé]z' OR  a.rut ~* 'g[uú]t[ií][eé]rr[eé]z' OR  text(ac.folio) ~* 'g[uú]t[ií][eé]rr[eé]z' OR  vac.cod ~* 'g[uú]t[ií][eé]rr[eé]z' ) 
