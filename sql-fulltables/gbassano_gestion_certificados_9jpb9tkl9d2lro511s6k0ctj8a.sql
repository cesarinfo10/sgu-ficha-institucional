COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[eé]l[aá]' OR  a.rut ~* 'm[aá]rc[eé]l[aá]' OR  text(ac.folio) ~* 'm[aá]rc[eé]l[aá]' OR  vac.cod ~* 'm[aá]rc[eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[ií]d[aá]lg[oó]' OR  a.rut ~* 'h[ií]d[aá]lg[oó]' OR  text(ac.folio) ~* 'h[ií]d[aá]lg[oó]' OR  vac.cod ~* 'h[ií]d[aá]lg[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]j[aá]s' OR  a.rut ~* 'r[oó]j[aá]s' OR  text(ac.folio) ~* 'r[oó]j[aá]s' OR  vac.cod ~* 'r[oó]j[aá]s' ) 
