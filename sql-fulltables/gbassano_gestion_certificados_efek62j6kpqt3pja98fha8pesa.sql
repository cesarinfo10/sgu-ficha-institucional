COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]l[oó]' OR  a.rut ~* 'v[ií]l[oó]' OR  text(ac.folio) ~* 'v[ií]l[oó]' OR  vac.cod ~* 'v[ií]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nd[oó]v[aá]l' OR  a.rut ~* 's[aá]nd[oó]v[aá]l' OR  text(ac.folio) ~* 's[aá]nd[oó]v[aá]l' OR  vac.cod ~* 's[aá]nd[oó]v[aá]l' ) 
