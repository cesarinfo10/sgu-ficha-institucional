COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* '[aá]bn[eé]r' OR  a.rut ~* '[aá]bn[eé]r' OR  text(ac.folio) ~* '[aá]bn[eé]r' OR  vac.cod ~* '[aá]bn[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]nd[oó]z[aá]' OR  a.rut ~* 'm[eé]nd[oó]z[aá]' OR  text(ac.folio) ~* 'm[eé]nd[oó]z[aá]' OR  vac.cod ~* 'm[eé]nd[oó]z[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nt[oó]s' OR  a.rut ~* 's[aá]nt[oó]s' OR  text(ac.folio) ~* 's[aá]nt[oó]s' OR  vac.cod ~* 's[aá]nt[oó]s' ) 
