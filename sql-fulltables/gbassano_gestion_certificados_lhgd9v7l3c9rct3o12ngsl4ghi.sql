COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]m[aá]n' OR  a.rut ~* 'r[oó]m[aá]n' OR  text(ac.folio) ~* 'r[oó]m[aá]n' OR  vac.cod ~* 'r[oó]m[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]v[eé]r[oó]s' OR  a.rut ~* 'r[ií]v[eé]r[oó]s' OR  text(ac.folio) ~* 'r[ií]v[eé]r[oó]s' OR  vac.cod ~* 'r[ií]v[eé]r[oó]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[aá]l[aá]n' OR  a.rut ~* 'c[aá]t[aá]l[aá]n' OR  text(ac.folio) ~* 'c[aá]t[aá]l[aá]n' OR  vac.cod ~* 'c[aá]t[aá]l[aá]n' ) 
