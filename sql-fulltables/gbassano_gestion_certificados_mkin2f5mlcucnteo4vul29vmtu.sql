COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'sc[aá]rl[eé]tt[eé]' OR  a.rut ~* 'sc[aá]rl[eé]tt[eé]' OR  text(ac.folio) ~* 'sc[aá]rl[eé]tt[eé]' OR  vac.cod ~* 'sc[aá]rl[eé]tt[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]ns[aá]lv[eé]s' OR  a.rut ~* 'm[oó]ns[aá]lv[eé]s' OR  text(ac.folio) ~* 'm[oó]ns[aá]lv[eé]s' OR  vac.cod ~* 'm[oó]ns[aá]lv[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]p[aá]z[oó]' OR  a.rut ~* '[oó]p[aá]z[oó]' OR  text(ac.folio) ~* '[oó]p[aá]z[oó]' OR  vac.cod ~* '[oó]p[aá]z[oó]' ) 
