COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]sq[uú][eé]z' OR  a.rut ~* 'v[aá]sq[uú][eé]z' OR  text(ac.folio) ~* 'v[aá]sq[uú][eé]z' OR  vac.cod ~* 'v[aá]sq[uú][eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lf[aá]r[oó]' OR  a.rut ~* '[aá]lf[aá]r[oó]' OR  text(ac.folio) ~* '[aá]lf[aá]r[oó]' OR  vac.cod ~* '[aá]lf[aá]r[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  a.rut ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  text(ac.folio) ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  vac.cod ~* 'm[aá][uú]r[ií]c[ií][oó]' ) 
