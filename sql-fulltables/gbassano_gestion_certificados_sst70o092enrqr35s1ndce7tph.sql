COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]x[ií]s' OR  a.rut ~* '[aá]l[eé]x[ií]s' OR  text(ac.folio) ~* '[aá]l[eé]x[ií]s' OR  vac.cod ~* '[aá]l[eé]x[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]j[aá]s' OR  a.rut ~* 'r[oó]j[aá]s' OR  text(ac.folio) ~* 'r[oó]j[aá]s' OR  vac.cod ~* 'r[oó]j[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá][eé]z[aá]' OR  a.rut ~* 'b[aá][eé]z[aá]' OR  text(ac.folio) ~* 'b[aá][eé]z[aá]' OR  vac.cod ~* 'b[aá][eé]z[aá]' ) 
