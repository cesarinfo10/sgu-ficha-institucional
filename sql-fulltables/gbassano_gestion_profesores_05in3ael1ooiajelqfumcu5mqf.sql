COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'chr[ií]st[ií][aá]ns[eé]n'  OR u.rut ~* 'chr[ií]st[ií][aá]ns[eé]n')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[oó]d[oó]y'  OR u.rut ~* 'g[oó]d[oó]y') 
