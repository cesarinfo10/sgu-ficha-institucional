COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'cr[ií]st[ií][aá]n'  OR u.rut ~* 'cr[ií]st[ií][aá]n')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá]nd[oó]v[aá]l'  OR u.rut ~* 's[aá]nd[oó]v[aá]l') 
