COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]st[eé]f[aá]ny'  OR u.rut ~* '[eé]st[eé]f[aá]ny')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá]nt[oó]r[oó]'  OR u.rut ~* 's[aá]nt[oó]r[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'h[eé]rr[eé]r[aá]'  OR u.rut ~* 'h[eé]rr[eé]r[aá]') 
