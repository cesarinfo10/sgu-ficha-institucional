COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'n[eé]v[aá]d[oó]'  OR u.rut ~* 'n[eé]v[aá]d[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[oó]l[ií]v[aá]r[eé]s'  OR u.rut ~* '[oó]l[ií]v[aá]r[eé]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'f[aá]t[ií]m[aá]'  OR u.rut ~* 'f[aá]t[ií]m[aá]') 
