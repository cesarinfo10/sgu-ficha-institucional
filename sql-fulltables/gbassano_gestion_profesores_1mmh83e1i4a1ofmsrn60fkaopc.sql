COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'cr[ií]st[ií][aá]n'  OR u.rut ~* 'cr[ií]st[ií][aá]n')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[uú]ñ[oó]z'  OR u.rut ~* 'm[uú]ñ[oó]z')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]'  OR u.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]') 
