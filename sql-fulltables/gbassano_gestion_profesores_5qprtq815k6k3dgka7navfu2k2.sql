COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]nt[oó]n[ií][oó]'  OR u.rut ~* '[aá]nt[oó]n[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[eé]t[eé]l[ií][eé]r'  OR u.rut ~* 'l[eé]t[eé]l[ií][eé]r')  AND (lower(u.nombre||' '||u.apellido) ~* 's[oó]t[oó]'  OR u.rut ~* 's[oó]t[oó]') 
