COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[oó]n[ií]c[aá]'  OR u.rut ~* 'm[oó]n[ií]c[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[aá]l[aá]rc[eé]'  OR u.rut ~* 'g[aá]l[aá]rc[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
