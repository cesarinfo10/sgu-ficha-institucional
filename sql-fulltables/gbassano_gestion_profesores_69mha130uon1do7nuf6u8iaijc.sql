COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'r[aá][uú]l'  OR u.rut ~* 'r[aá][uú]l')  AND (lower(u.nombre||' '||u.apellido) ~* '[oó]rt[ií]z'  OR u.rut ~* '[oó]rt[ií]z')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[oó]ntr[eé]r[aá]s'  OR u.rut ~* 'c[oó]ntr[eé]r[aá]s') 
