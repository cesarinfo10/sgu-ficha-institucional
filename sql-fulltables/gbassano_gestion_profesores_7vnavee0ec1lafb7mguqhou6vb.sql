COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'v[ií]ct[oó]r'  OR u.rut ~* 'v[ií]ct[oó]r')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]lm[oó]n[aá]c[ií]d'  OR u.rut ~* '[aá]lm[oó]n[aá]c[ií]d') 
