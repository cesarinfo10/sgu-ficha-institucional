COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'b[oó]r[ií]s'  OR u.rut ~* 'b[oó]r[ií]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[eé]r[aá]lt[aá]'  OR u.rut ~* 'p[eé]r[aá]lt[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
