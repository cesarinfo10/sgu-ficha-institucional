COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]x'  OR u.rut ~* 'm[aá]x')  AND (lower(u.nombre||' '||u.apellido) ~* '[ií]r[ií][aá]rt[eé]'  OR u.rut ~* '[ií]r[ií][aá]rt[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá]nt[oó]r[oó]'  OR u.rut ~* 's[aá]nt[oó]r[oó]') 
