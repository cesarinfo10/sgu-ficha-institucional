COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]t[aá]l[ií]n[aá]'  OR u.rut ~* 'c[aá]t[aá]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'n[uú]ñ[eé]z'  OR u.rut ~* 'n[uú]ñ[eé]z')  AND (lower(u.nombre||' '||u.apellido) ~* 't[oó]rr[eé][aá]lb[aá]'  OR u.rut ~* 't[oó]rr[eé][aá]lb[aá]') 
