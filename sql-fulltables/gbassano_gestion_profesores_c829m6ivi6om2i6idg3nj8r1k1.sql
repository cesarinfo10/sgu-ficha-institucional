COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[oó]s[eé]'  OR u.rut ~* 'j[oó]s[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* '[ií]gn[aá]c[ií][oó]'  OR u.rut ~* '[ií]gn[aá]c[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá]ff[ií][eé]'  OR u.rut ~* 's[aá]ff[ií][eé]') 
