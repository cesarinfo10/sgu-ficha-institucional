COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 's[uú]s[aá]n[aá]'  OR u.rut ~* 's[uú]s[aá]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'b[aá]s[ií]s'  OR u.rut ~* 'b[aá]s[ií]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[eé]r[ií]n[oó]'  OR u.rut ~* 'm[eé]r[ií]n[oó]') 
