COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'd[aá]nny'  OR u.rut ~* 'd[aá]nny')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]l[eé]j[aá]ndr[oó]'  OR u.rut ~* '[aá]l[eé]j[aá]ndr[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[aá]m[ií]r[eé]z'  OR u.rut ~* 'r[aá]m[ií]r[eé]z')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[oó]ntr[eé]r[aá]s'  OR u.rut ~* 'c[oó]ntr[eé]r[aá]s') 
