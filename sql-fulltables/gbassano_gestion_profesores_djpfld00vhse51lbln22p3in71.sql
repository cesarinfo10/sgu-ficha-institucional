COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'fr[aá]nc[ií]sc[oó]'  OR u.rut ~* 'fr[aá]nc[ií]sc[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[eé]g[aá]'  OR u.rut ~* 'v[eé]g[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[eé]nd[eé]z'  OR u.rut ~* 'm[eé]nd[eé]z')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
