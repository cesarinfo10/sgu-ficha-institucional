COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]r[oó]l[ií]n[aá]'  OR u.rut ~* '[aá]r[oó]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]ss[oó]n'  OR u.rut ~* 'm[aá]ss[oó]n') 
