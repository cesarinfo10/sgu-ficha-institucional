COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'f[aá]lc[uú]n[eé]r[ií]'  OR u.rut ~* 'f[aá]lc[uú]n[eé]r[ií]')  AND (lower(u.nombre||' '||u.apellido) ~* '[oó]c[aá]ñ[aá]'  OR u.rut ~* '[oó]c[aá]ñ[aá]') 
