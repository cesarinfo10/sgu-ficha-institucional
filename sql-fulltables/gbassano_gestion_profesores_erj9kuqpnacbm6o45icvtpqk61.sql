COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 's[eé]rg[ií][oó]'  OR u.rut ~* 's[eé]rg[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[aá]l[eé]nc[ií][aá]'  OR u.rut ~* 'v[aá]l[eé]nc[ií][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[eé]r[ií]t[oó]'  OR u.rut ~* 'p[eé]r[ií]t[oó]') 
