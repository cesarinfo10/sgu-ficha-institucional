COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'th[oó]m[aá]s'  OR u.rut ~* 'th[oó]m[aá]s')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[ií]x'  OR u.rut ~* 'm[ií]x') 
