COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]ndr[eé][aá]'  OR u.rut ~* '[aá]ndr[eé][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 's[eé]rr[aá]n[oó]'  OR u.rut ~* 's[eé]rr[aá]n[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'fl[oó]r[eé]s'  OR u.rut ~* 'fl[oó]r[eé]s') 
