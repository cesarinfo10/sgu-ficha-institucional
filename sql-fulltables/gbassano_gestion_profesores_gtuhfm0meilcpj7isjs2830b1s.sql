COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'p[aá][uú]l[ií]n[aá]'  OR u.rut ~* 'p[aá][uú]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[oó]nz[aá]l[eé]z'  OR u.rut ~* 'g[oó]nz[aá]l[eé]z')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]lv[eé][aá]r'  OR u.rut ~* '[aá]lv[eé][aá]r') 
