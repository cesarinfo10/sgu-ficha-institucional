COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[oó]s[eé]'  OR u.rut ~* 'j[oó]s[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[uú][eé]ng[oó]'  OR u.rut ~* 'l[uú][eé]ng[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
