COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'g[uú][ií]d[oó]'  OR u.rut ~* 'g[uú][ií]d[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]nr[ií]q[uú][eé]z'  OR u.rut ~* 'm[aá]nr[ií]q[uú][eé]z') 
