COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]lv[aá]r[oó]'  OR u.rut ~* '[aá]lv[aá]r[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[oó]rr[eé][aá]'  OR u.rut ~* 'c[oó]rr[eé][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[uú]zm[aá]n'  OR u.rut ~* 'g[uú]zm[aá]n') 
