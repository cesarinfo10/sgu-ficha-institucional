COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]l[eé]j[aá]ndr[oó]'  OR u.rut ~* '[aá]l[eé]j[aá]ndr[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]st[eé]b[aá]n'  OR u.rut ~* '[eé]st[eé]b[aá]n')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]rr[aá]sc[oó]'  OR u.rut ~* 'c[aá]rr[aá]sc[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[uú]n[aá]'  OR u.rut ~* 'l[uú]n[aá]') 
