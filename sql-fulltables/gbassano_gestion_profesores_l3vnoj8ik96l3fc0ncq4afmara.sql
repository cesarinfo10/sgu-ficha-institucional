COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]lb[eé]rt[oó]'  OR u.rut ~* '[aá]lb[eé]rt[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]l[ií]z[oó]nd[oó]'  OR u.rut ~* '[eé]l[ií]z[oó]nd[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]r[eé]n[aá]s'  OR u.rut ~* '[aá]r[eé]n[aá]s')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
