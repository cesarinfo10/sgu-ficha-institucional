COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[ií]t[aá]l[oó]'  OR u.rut ~* '[ií]t[aá]l[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[ií]r[aá][uú]d[oó]'  OR u.rut ~* 'g[ií]r[aá][uú]d[oó]') 
