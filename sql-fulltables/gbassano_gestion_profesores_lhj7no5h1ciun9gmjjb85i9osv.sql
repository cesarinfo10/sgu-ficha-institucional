COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]d[ií]s[oó]n'  OR u.rut ~* '[eé]d[ií]s[oó]n')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[eé]v[ií]ll[aá]'  OR u.rut ~* 'r[eé]v[ií]ll[aá]') 
