COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]d[uú][aá]rd[oó]'  OR u.rut ~* '[eé]d[uú][aá]rd[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[eé]rd[uú]g[oó]'  OR u.rut ~* 'v[eé]rd[uú]g[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'h[ií]d[aá]lg[oó]'  OR u.rut ~* 'h[ií]d[aá]lg[oó]') 
