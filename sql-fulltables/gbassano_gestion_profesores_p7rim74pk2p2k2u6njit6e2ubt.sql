COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]lb[eé]rt[oó]'  OR u.rut ~* '[aá]lb[eé]rt[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[uú][eé]ng[oó]'  OR u.rut ~* 'l[uú][eé]ng[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'f[eé]r[eé][ií]r[aá]'  OR u.rut ~* 'f[eé]r[eé][ií]r[aá]') 
