COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]st[eé]f[aá]ny'  OR u.rut ~* '[eé]st[eé]f[aá]ny')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá]nt[oó]r[oó]'  OR u.rut ~* 's[aá]nt[oó]r[oó]') 
