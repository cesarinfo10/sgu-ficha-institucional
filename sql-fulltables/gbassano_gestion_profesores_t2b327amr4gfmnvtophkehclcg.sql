COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'n[aá]ncy'  OR u.rut ~* 'n[aá]ncy')  AND (lower(u.nombre||' '||u.apellido) ~* 'f[uú][eé]nt[eé]s'  OR u.rut ~* 'f[uú][eé]nt[eé]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'f[uú][eé]nt[eé]s'  OR u.rut ~* 'f[uú][eé]nt[eé]s') 
