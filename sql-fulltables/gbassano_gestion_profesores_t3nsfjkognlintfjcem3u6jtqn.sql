COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]b[eé]l'  OR u.rut ~* 'm[aá]b[eé]l')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[aá]lm[aá]'  OR u.rut ~* 'p[aá]lm[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'f[uú][eé]nt[eé]s'  OR u.rut ~* 'f[uú][eé]nt[eé]s') 
