COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[oó]s[eé]'  OR u.rut ~* 'j[oó]s[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]d[uú][aá]rd[oó]'  OR u.rut ~* '[eé]d[uú][aá]rd[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '')  AND (lower(u.nombre||' '||u.apellido) ~* 'h[eé]rn[aá]nd[eé]z'  OR u.rut ~* 'h[eé]rn[aá]nd[eé]z') 
