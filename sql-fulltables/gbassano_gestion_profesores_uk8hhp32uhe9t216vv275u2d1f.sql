COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[aá][ií]m[eé]'  OR u.rut ~* 'j[aá][ií]m[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[ií]nt[oó]'  OR u.rut ~* 'p[ií]nt[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'k[aá]l[ií]sk[ií]'  OR u.rut ~* 'k[aá]l[ií]sk[ií]') 
