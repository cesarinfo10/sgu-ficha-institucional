COPY (SELECT act.*,(SELECT char_comma_sum(tipo_publico||': '||coalesce((coalesce(cant_personas,0)+coalesce(cant_personas_virtuales,0))::text,'*')) 
			...
			WHERE true  AND (ano = 2023) AND (alcance = 'Nacional') AND (modalidad = 'Presencial') AND (12 IN (id_unidad1,id_unidad2,id_unidad3)) AND ('Apoyo a la Docencia' IN (u1.tipo,u2.tipo,u3.tipo)) AND (id_responsable = 419	) AND ('Público General' = ANY (tipo_publico)) 
