COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]lch[eé]s' OR  a.rut ~* 'v[ií]lch[eé]s' OR  lower(a.email) ~* 'v[ií]lch[eé]s' OR  text(a.id) ~* 'v[ií]lch[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]v[aá]s' OR  a.rut ~* 'r[ií]v[aá]s' OR  lower(a.email) ~* 'r[ií]v[aá]s' OR  text(a.id) ~* 'r[ií]v[aá]s' ) 
