COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá][ií]m[eé]' OR  a.rut ~* 'j[aá][ií]m[eé]' OR  lower(a.email) ~* 'j[aá][ií]m[eé]' OR  text(a.id) ~* 'j[aá][ií]m[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'sp[aá][uú]d[oó]' OR  a.rut ~* 'sp[aá][uú]d[oó]' OR  lower(a.email) ~* 'sp[aá][uú]d[oó]' OR  text(a.id) ~* 'sp[aá][uú]d[oó]' ) 
