COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'w[ií]ll[ií][aá]ms' OR  a.rut ~* 'w[ií]ll[ií][aá]ms' OR  lower(a.email) ~* 'w[ií]ll[ií][aá]ms' OR  text(a.id) ~* 'w[ií]ll[ií][aá]ms' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]fr[eé]' OR  a.rut ~* 'j[oó]fr[eé]' OR  lower(a.email) ~* 'j[oó]fr[eé]' OR  text(a.id) ~* 'j[oó]fr[eé]' ) 
