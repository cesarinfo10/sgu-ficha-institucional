COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú]l[ií][eé]t' OR  a.rut ~* 'j[uú]l[ií][eé]t' OR  lower(a.email) ~* 'j[uú]l[ií][eé]t' OR  text(a.id) ~* 'j[uú]l[ií][eé]t' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]ld[eé]z' OR  a.rut ~* 'v[aá]ld[eé]z' OR  lower(a.email) ~* 'v[aá]ld[eé]z' OR  text(a.id) ~* 'v[aá]ld[eé]z' ) 
