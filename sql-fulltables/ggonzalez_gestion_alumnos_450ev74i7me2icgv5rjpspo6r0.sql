COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'cl[aá][uú]d[ií][aá]' OR  a.rut ~* 'cl[aá][uú]d[ií][aá]' OR  lower(a.email) ~* 'cl[aá][uú]d[ií][aá]' OR  text(a.id) ~* 'cl[aá][uú]d[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]l[ií]n[aá]s' OR  a.rut ~* 's[aá]l[ií]n[aá]s' OR  lower(a.email) ~* 's[aá]l[ií]n[aá]s' OR  text(a.id) ~* 's[aá]l[ií]n[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]lg[uú][eé]t[aá]' OR  a.rut ~* '[eé]lg[uú][eé]t[aá]' OR  lower(a.email) ~* '[eé]lg[uú][eé]t[aá]' OR  text(a.id) ~* '[eé]lg[uú][eé]t[aá]' ) 
