COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[aá]b[ií][oó]l[aá]' OR  a.rut ~* 'f[aá]b[ií][oó]l[aá]' OR  lower(a.email) ~* 'f[aá]b[ií][oó]l[aá]' OR  text(a.id) ~* 'f[aá]b[ií][oó]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]sm[aá]d' OR  a.rut ~* '[aá]sm[aá]d' OR  lower(a.email) ~* '[aá]sm[aá]d' OR  text(a.id) ~* '[aá]sm[aá]d' ) 
