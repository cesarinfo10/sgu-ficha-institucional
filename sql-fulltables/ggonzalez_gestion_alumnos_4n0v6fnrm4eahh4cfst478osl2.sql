COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[uú]g[oó]' OR  a.rut ~* 'h[uú]g[oó]' OR  lower(a.email) ~* 'h[uú]g[oó]' OR  text(a.id) ~* 'h[uú]g[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rr[aá]z[aá]' OR  a.rut ~* 'b[aá]rr[aá]z[aá]' OR  lower(a.email) ~* 'b[aá]rr[aá]z[aá]' OR  text(a.id) ~* 'b[aá]rr[aá]z[aá]' ) 
