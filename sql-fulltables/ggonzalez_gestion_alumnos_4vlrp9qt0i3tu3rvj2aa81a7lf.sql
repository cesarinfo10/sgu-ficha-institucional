COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[eé]r[eé]s[aá]' OR  a.rut ~* 't[eé]r[eé]s[aá]' OR  lower(a.email) ~* 't[eé]r[eé]s[aá]' OR  text(a.id) ~* 't[eé]r[eé]s[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]v[aá]s' OR  a.rut ~* 'r[ií]v[aá]s' OR  lower(a.email) ~* 'r[ií]v[aá]s' OR  text(a.id) ~* 'r[ií]v[aá]s' ) 
