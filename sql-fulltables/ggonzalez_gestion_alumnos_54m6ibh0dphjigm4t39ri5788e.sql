COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ndr[eé]h[aá]' OR  a.rut ~* '[aá]ndr[eé]h[aá]' OR  lower(a.email) ~* '[aá]ndr[eé]h[aá]' OR  text(a.id) ~* '[aá]ndr[eé]h[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá][eé]z[aá]' OR  a.rut ~* 'b[aá][eé]z[aá]' OR  lower(a.email) ~* 'b[aá][eé]z[aá]' OR  text(a.id) ~* 'b[aá][eé]z[aá]' ) 
