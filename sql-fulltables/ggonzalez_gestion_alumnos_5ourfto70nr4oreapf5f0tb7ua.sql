COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]n[eé]ss[aá]' OR  a.rut ~* 'v[aá]n[eé]ss[aá]' OR  lower(a.email) ~* 'v[aá]n[eé]ss[aá]' OR  text(a.id) ~* 'v[aá]n[eé]ss[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[ií]m[eé]n[eé]z' OR  a.rut ~* 'j[ií]m[eé]n[eé]z' OR  lower(a.email) ~* 'j[ií]m[eé]n[eé]z' OR  text(a.id) ~* 'j[ií]m[eé]n[eé]z' ) 
