COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]rl[eé]tt[eé]' OR  a.rut ~* '[aá]rl[eé]tt[eé]' OR  lower(a.email) ~* '[aá]rl[eé]tt[eé]' OR  text(a.id) ~* '[aá]rl[eé]tt[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]g[uú][ií]l[aá]r' OR  a.rut ~* '[aá]g[uú][ií]l[aá]r' OR  lower(a.email) ~* '[aá]g[uú][ií]l[aá]r' OR  text(a.id) ~* '[aá]g[uú][ií]l[aá]r' ) 
