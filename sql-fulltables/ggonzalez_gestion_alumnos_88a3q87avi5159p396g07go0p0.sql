COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]rk[oó]' OR  a.rut ~* 'm[ií]rk[oó]' OR  lower(a.email) ~* 'm[ií]rk[oó]' OR  text(a.id) ~* 'm[ií]rk[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]c[aá]y[aá]' OR  a.rut ~* 'm[aá]c[aá]y[aá]' OR  lower(a.email) ~* 'm[aá]c[aá]y[aá]' OR  text(a.id) ~* 'm[aá]c[aá]y[aá]' ) 
