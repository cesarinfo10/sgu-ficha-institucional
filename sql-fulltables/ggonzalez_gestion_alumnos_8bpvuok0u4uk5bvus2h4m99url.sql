COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[ií]v[ií][eé]t[aá]' OR  a.rut ~* 'c[ií]v[ií][eé]t[aá]' OR  lower(a.email) ~* 'c[ií]v[ií][eé]t[aá]' OR  text(a.id) ~* 'c[ií]v[ií][eé]t[aá]' ) 
