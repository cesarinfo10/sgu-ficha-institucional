COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]d[eé]l[aá]' OR  a.rut ~* 'v[ií]d[eé]l[aá]' OR  lower(a.email) ~* 'v[ií]d[eé]l[aá]' OR  text(a.id) ~* 'v[ií]d[eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n' OR  a.rut ~* '[aá]n' OR  lower(a.email) ~* '[aá]n' OR  text(a.id) ~* '[aá]n' ) 
