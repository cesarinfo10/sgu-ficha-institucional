COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]y' OR  a.rut ~* 'g[uú]y' OR  lower(a.email) ~* 'g[uú]y' OR  text(a.id) ~* 'g[uú]y' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][uú]l' OR  a.rut ~* 'p[aá][uú]l' OR  lower(a.email) ~* 'p[aá][uú]l' OR  text(a.id) ~* 'p[aá][uú]l' ) 
