COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]n[eé]s' OR  a.rut ~* '[ií]n[eé]s' OR  lower(a.email) ~* '[ií]n[eé]s' OR  text(a.id) ~* '[ií]n[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]c[eé]nc[ií][oó]' OR  a.rut ~* 'v[ií]c[eé]nc[ií][oó]' OR  lower(a.email) ~* 'v[ií]c[eé]nc[ií][oó]' OR  text(a.id) ~* 'v[ií]c[eé]nc[ií][oó]' ) 
