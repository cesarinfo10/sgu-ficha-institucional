COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nt[oó]s' OR  a.rut ~* 's[aá]nt[oó]s' OR  lower(a.email) ~* 's[aá]nt[oó]s' OR  text(a.id) ~* 's[aá]nt[oó]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]d[uú]ll[ií]' OR  a.rut ~* 'v[aá]d[uú]ll[ií]' OR  lower(a.email) ~* 'v[aá]d[uú]ll[ií]' OR  text(a.id) ~* 'v[aá]d[uú]ll[ií]' ) 
