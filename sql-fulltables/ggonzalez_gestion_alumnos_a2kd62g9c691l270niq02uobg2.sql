COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]r[eé]t[oó]' OR  a.rut ~* 'l[oó]r[eé]t[oó]' OR  lower(a.email) ~* 'l[oó]r[eé]t[oó]' OR  text(a.id) ~* 'l[oó]r[eé]t[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]rn[aá]l' OR  a.rut ~* 'b[eé]rn[aá]l' OR  lower(a.email) ~* 'b[eé]rn[aá]l' OR  text(a.id) ~* 'b[eé]rn[aá]l' ) 
