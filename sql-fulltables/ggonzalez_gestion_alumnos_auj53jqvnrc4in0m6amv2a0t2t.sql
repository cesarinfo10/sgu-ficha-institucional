COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]n[ií][aá]' OR  a.rut ~* 's[oó]n[ií][aá]' OR  lower(a.email) ~* 's[oó]n[ií][aá]' OR  text(a.id) ~* 's[oó]n[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'br[ií]t[oó]' OR  a.rut ~* 'br[ií]t[oó]' OR  lower(a.email) ~* 'br[ií]t[oó]' OR  text(a.id) ~* 'br[ií]t[oó]' ) 
