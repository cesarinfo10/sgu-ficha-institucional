COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]ncy' OR  a.rut ~* 'n[aá]ncy' OR  lower(a.email) ~* 'n[aá]ncy' OR  text(a.id) ~* 'n[aá]ncy' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]nd[aá]' OR  a.rut ~* '[aá]r[aá]nd[aá]' OR  lower(a.email) ~* '[aá]r[aá]nd[aá]' OR  text(a.id) ~* '[aá]r[aá]nd[aá]' ) 
