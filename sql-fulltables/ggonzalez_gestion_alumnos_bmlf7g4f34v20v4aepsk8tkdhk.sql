COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]dr[oó]' OR  a.rut ~* 'p[eé]dr[oó]' OR  lower(a.email) ~* 'p[eé]dr[oó]' OR  text(a.id) ~* 'p[eé]dr[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[aá]' OR  a.rut ~* 'm[oó]r[aá]' OR  lower(a.email) ~* 'm[oó]r[aá]' OR  text(a.id) ~* 'm[oó]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]nd[aá]n[aá]' OR  a.rut ~* '[aá]nd[aá]n[aá]' OR  lower(a.email) ~* '[aá]nd[aá]n[aá]' OR  text(a.id) ~* '[aá]nd[aá]n[aá]' ) 
