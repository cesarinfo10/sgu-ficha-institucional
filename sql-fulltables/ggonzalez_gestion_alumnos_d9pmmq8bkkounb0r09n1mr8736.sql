COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[ií][aá]' OR  a.rut ~* 'm[aá]rc[ií][aá]' OR  lower(a.email) ~* 'm[aá]rc[ií][aá]' OR  text(a.id) ~* 'm[aá]rc[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé][uú]s[eé]r' OR  a.rut ~* 'h[eé][uú]s[eé]r' OR  lower(a.email) ~* 'h[eé][uú]s[eé]r' OR  text(a.id) ~* 'h[eé][uú]s[eé]r' ) 
