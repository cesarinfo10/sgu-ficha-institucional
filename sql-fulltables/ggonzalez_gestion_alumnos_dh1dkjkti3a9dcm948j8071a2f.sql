COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]sn[aá]' OR  a.rut ~* 'y[aá]sn[aá]' OR  lower(a.email) ~* 'y[aá]sn[aá]' OR  text(a.id) ~* 'y[aá]sn[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé][aá]' OR  a.rut ~* 'c[eé][aá]' OR  lower(a.email) ~* 'c[eé][aá]' OR  text(a.id) ~* 'c[eé][aá]' ) 
