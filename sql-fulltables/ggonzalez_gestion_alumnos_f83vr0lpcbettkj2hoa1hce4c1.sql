COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií]lyn' OR  a.rut ~* 'm[aá]r[ií]lyn' OR  lower(a.email) ~* 'm[aá]r[ií]lyn' OR  text(a.id) ~* 'm[aá]r[ií]lyn' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'gr[aá]c[eé]' OR  a.rut ~* 'gr[aá]c[eé]' OR  lower(a.email) ~* 'gr[aá]c[eé]' OR  text(a.id) ~* 'gr[aá]c[eé]' ) 
