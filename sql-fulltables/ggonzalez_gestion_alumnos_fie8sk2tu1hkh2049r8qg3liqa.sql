COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]r[aá]' OR  a.rut ~* 's[aá]r[aá]' OR  lower(a.email) ~* 's[aá]r[aá]' OR  text(a.id) ~* 's[aá]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]v[aá]nc[oó]' OR  a.rut ~* 'v[ií]v[aá]nc[oó]' OR  lower(a.email) ~* 'v[ií]v[aá]nc[oó]' OR  text(a.id) ~* 'v[ií]v[aá]nc[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó]' OR  a.rut ~* 'g[oó]' OR  lower(a.email) ~* 'g[oó]' OR  text(a.id) ~* 'g[oó]' ) 
