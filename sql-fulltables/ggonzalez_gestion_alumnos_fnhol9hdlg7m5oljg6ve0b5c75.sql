COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[aá]nc[ií]sc[aá]' OR  a.rut ~* 'fr[aá]nc[ií]sc[aá]' OR  lower(a.email) ~* 'fr[aá]nc[ií]sc[aá]' OR  text(a.id) ~* 'fr[aá]nc[ií]sc[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]n' OR  a.rut ~* 's[aá]n' OR  lower(a.email) ~* 's[aá]n' OR  text(a.id) ~* 's[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rt[ií]n' OR  a.rut ~* 'm[aá]rt[ií]n' OR  lower(a.email) ~* 'm[aá]rt[ií]n' OR  text(a.id) ~* 'm[aá]rt[ií]n' ) 
