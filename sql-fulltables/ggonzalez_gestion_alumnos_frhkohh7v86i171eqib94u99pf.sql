COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'fl[oó]r[eé]s' OR  a.rut ~* 'fl[oó]r[eé]s' OR  lower(a.email) ~* 'fl[oó]r[eé]s' OR  text(a.id) ~* 'fl[oó]r[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]z[aá]n[aá]' OR  a.rut ~* 'l[ií]z[aá]n[aá]' OR  lower(a.email) ~* 'l[ií]z[aá]n[aá]' OR  text(a.id) ~* 'l[ií]z[aá]n[aá]' ) 
