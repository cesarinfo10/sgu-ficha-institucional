COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]ll[aá]r[aá]y' OR  a.rut ~* 'm[ií]ll[aá]r[aá]y' OR  lower(a.email) ~* 'm[ií]ll[aá]r[aá]y' OR  text(a.id) ~* 'm[ií]ll[aá]r[aá]y' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]ch[eé]c[oó]' OR  a.rut ~* 'p[aá]ch[eé]c[oó]' OR  lower(a.email) ~* 'p[aá]ch[eé]c[oó]' OR  text(a.id) ~* 'p[aá]ch[eé]c[oó]' ) 
