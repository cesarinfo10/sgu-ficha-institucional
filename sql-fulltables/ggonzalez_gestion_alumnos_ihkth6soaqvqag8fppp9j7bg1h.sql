COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú][aá]n[aá]' OR  a.rut ~* 'j[uú][aá]n[aá]' OR  lower(a.email) ~* 'j[uú][aá]n[aá]' OR  text(a.id) ~* 'j[uú][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]rr[aá]' OR  a.rut ~* 'p[aá]rr[aá]' OR  lower(a.email) ~* 'p[aá]rr[aá]' OR  text(a.id) ~* 'p[aá]rr[aá]' ) 
