COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]fn[eé]' OR  a.rut ~* 'd[aá]fn[eé]' OR  lower(a.email) ~* 'd[aá]fn[eé]' OR  text(a.id) ~* 'd[aá]fn[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[uú]ss[ií]n[ií]' OR  a.rut ~* 'm[uú]ss[ií]n[ií]' OR  lower(a.email) ~* 'm[uú]ss[ií]n[ií]' OR  text(a.id) ~* 'm[uú]ss[ií]n[ií]' ) 
