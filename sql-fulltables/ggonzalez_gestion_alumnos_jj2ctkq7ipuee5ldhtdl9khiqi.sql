COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]x[ií]s' OR  a.rut ~* '[aá]l[eé]x[ií]s' OR  lower(a.email) ~* '[aá]l[eé]x[ií]s' OR  text(a.id) ~* '[aá]l[eé]x[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nd[oó]v[aá]l' OR  a.rut ~* 's[aá]nd[oó]v[aá]l' OR  lower(a.email) ~* 's[aá]nd[oó]v[aá]l' OR  text(a.id) ~* 's[aá]nd[oó]v[aá]l' ) 
