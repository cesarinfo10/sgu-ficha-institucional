COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'gh[eé]sth[eé]r' OR  a.rut ~* 'gh[eé]sth[eé]r' OR  lower(a.email) ~* 'gh[eé]sth[eé]r' OR  text(a.id) ~* 'gh[eé]sth[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[uú]ñ[oó]z' OR  a.rut ~* 'm[uú]ñ[oó]z' OR  lower(a.email) ~* 'm[uú]ñ[oó]z' OR  text(a.id) ~* 'm[uú]ñ[oó]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rch[aá]nt' OR  a.rut ~* 'm[aá]rch[aá]nt' OR  lower(a.email) ~* 'm[aá]rch[aá]nt' OR  text(a.id) ~* 'm[aá]rch[aá]nt' ) 
