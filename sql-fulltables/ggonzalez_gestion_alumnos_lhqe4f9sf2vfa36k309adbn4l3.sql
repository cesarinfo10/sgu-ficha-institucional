COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ll[aá]t[oó]r[oó]' OR  a.rut ~* 'v[ií]ll[aá]t[oó]r[oó]' OR  lower(a.email) ~* 'v[ií]ll[aá]t[oó]r[oó]' OR  text(a.id) ~* 'v[ií]ll[aá]t[oó]r[oó]' ) 
