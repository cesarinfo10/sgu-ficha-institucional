COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]d[ií]th' OR  a.rut ~* '[eé]d[ií]th' OR  lower(a.email) ~* '[eé]d[ií]th' OR  text(a.id) ~* '[eé]d[ií]th' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]b[aá]r' OR  a.rut ~* 't[oó]b[aá]r' OR  lower(a.email) ~* 't[oó]b[aá]r' OR  text(a.id) ~* 't[oó]b[aá]r' ) 
