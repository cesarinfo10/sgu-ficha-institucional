COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[oó]l[ií]n[aá]' OR  a.rut ~* 'c[aá]r[oó]l[ií]n[aá]' OR  lower(a.email) ~* 'c[aá]r[oó]l[ií]n[aá]' OR  text(a.id) ~* 'c[aá]r[oó]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]rn[aá]nd[eé]z' OR  a.rut ~* 'h[eé]rn[aá]nd[eé]z' OR  lower(a.email) ~* 'h[eé]rn[aá]nd[eé]z' OR  text(a.id) ~* 'h[eé]rn[aá]nd[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]y[eé]s' OR  a.rut ~* 'r[eé]y[eé]s' OR  lower(a.email) ~* 'r[eé]y[eé]s' OR  text(a.id) ~* 'r[eé]y[eé]s' ) 
