COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'sylv[ií][aá]' OR  a.rut ~* 'sylv[ií][aá]' OR  lower(a.email) ~* 'sylv[ií][aá]' OR  text(a.id) ~* 'sylv[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]ss[eé]s' OR  a.rut ~* '[oó]ss[eé]s' OR  lower(a.email) ~* '[oó]ss[eé]s' OR  text(a.id) ~* '[oó]ss[eé]s' ) 
