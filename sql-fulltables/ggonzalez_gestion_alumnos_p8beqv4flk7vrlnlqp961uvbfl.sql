COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]mbr[oó]s[eé]tt[ií]' OR  a.rut ~* '[aá]mbr[oó]s[eé]tt[ií]' OR  lower(a.email) ~* '[aá]mbr[oó]s[eé]tt[ií]' OR  text(a.id) ~* '[aá]mbr[oó]s[eé]tt[ií]' ) 
