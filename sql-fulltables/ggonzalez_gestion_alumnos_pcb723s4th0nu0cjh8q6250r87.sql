COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]rw[ií]n' OR  a.rut ~* '[eé]rw[ií]n' OR  lower(a.email) ~* '[eé]rw[ií]n' OR  text(a.id) ~* '[eé]rw[ií]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]ns[ií]ll[aá]' OR  a.rut ~* 'm[aá]ns[ií]ll[aá]' OR  lower(a.email) ~* 'm[aá]ns[ií]ll[aá]' OR  text(a.id) ~* 'm[aá]ns[ií]ll[aá]' ) 
