COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rj[oó]r[ií][eé]' OR  a.rut ~* 'm[aá]rj[oó]r[ií][eé]' OR  lower(a.email) ~* 'm[aá]rj[oó]r[ií][eé]' OR  text(a.id) ~* 'm[aá]rj[oó]r[ií][eé]' ) 
