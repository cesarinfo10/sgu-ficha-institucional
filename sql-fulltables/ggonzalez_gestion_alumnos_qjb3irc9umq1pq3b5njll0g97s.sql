COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií][eé]nq[uú][eé][oó]' OR  a.rut ~* 'l[ií][eé]nq[uú][eé][oó]' OR  lower(a.email) ~* 'l[ií][eé]nq[uú][eé][oó]' OR  text(a.id) ~* 'l[ií][eé]nq[uú][eé][oó]' ) 
