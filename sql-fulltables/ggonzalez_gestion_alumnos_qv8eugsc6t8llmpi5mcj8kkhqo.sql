COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  a.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  lower(a.email) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  text(a.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]z[aá]' OR  a.rut ~* 'm[eé]z[aá]' OR  lower(a.email) ~* 'm[eé]z[aá]' OR  text(a.id) ~* 'm[eé]z[aá]' ) 
