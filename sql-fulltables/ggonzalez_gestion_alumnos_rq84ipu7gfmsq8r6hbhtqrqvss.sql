COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'cl[aá]v[eé]r[ií][aá]' OR  a.rut ~* 'cl[aá]v[eé]r[ií][aá]' OR  lower(a.email) ~* 'cl[aá]v[eé]r[ií][aá]' OR  text(a.id) ~* 'cl[aá]v[eé]r[ií][aá]' ) 
