COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[oó]m[ií]ng[oó]' OR  a.rut ~* 'd[oó]m[ií]ng[oó]' OR  lower(a.email) ~* 'd[oó]m[ií]ng[oó]' OR  text(a.id) ~* 'd[oó]m[ií]ng[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]v[aá]ll[eé]' OR  a.rut ~* '[oó]v[aá]ll[eé]' OR  lower(a.email) ~* '[oó]v[aá]ll[eé]' OR  text(a.id) ~* '[oó]v[aá]ll[eé]' ) 
