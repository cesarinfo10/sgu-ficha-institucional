COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rt[aá]' OR  a.rut ~* 'm[aá]rt[aá]' OR  lower(a.email) ~* 'm[aá]rt[aá]' OR  text(a.id) ~* 'm[aá]rt[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]rq[uú][eé]r[aá]' OR  a.rut ~* 'j[oó]rq[uú][eé]r[aá]' OR  lower(a.email) ~* 'j[oó]rq[uú][eé]r[aá]' OR  text(a.id) ~* 'j[oó]rq[uú][eé]r[aá]' ) 
