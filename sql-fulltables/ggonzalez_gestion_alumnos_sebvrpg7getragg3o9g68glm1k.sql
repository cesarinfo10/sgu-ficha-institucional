COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]rn[aá]rd[oó]' OR  a.rut ~* 'b[eé]rn[aá]rd[oó]' OR  lower(a.email) ~* 'b[eé]rn[aá]rd[oó]' OR  text(a.id) ~* 'b[eé]rn[aá]rd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]mp[oó]s' OR  a.rut ~* 'c[aá]mp[oó]s' OR  lower(a.email) ~* 'c[aá]mp[oó]s' OR  text(a.id) ~* 'c[aá]mp[oó]s' ) 
