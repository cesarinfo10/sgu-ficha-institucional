COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]l[aá]r' OR  a.rut ~* 'p[ií]l[aá]r' OR  lower(a.email) ~* 'p[ií]l[aá]r' OR  text(a.id) ~* 'p[ií]l[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]r[aá]y' OR  a.rut ~* 'g[aá]r[aá]y' OR  lower(a.email) ~* 'g[aá]r[aá]y' OR  text(a.id) ~* 'g[aá]r[aá]y' ) 
