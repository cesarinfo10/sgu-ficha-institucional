COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]z[oó][aá]' OR  a.rut ~* 'p[eé]z[oó][aá]' OR  lower(a.email) ~* 'p[eé]z[oó][aá]' OR  text(a.id) ~* 'p[eé]z[oó][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]t[uú]s' OR  a.rut ~* 'm[aá]t[uú]s' OR  lower(a.email) ~* 'm[aá]t[uú]s' OR  text(a.id) ~* 'm[aá]t[uú]s' ) 
