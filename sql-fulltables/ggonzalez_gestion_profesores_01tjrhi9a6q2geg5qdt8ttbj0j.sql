COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]c[aá]r[eé]n[aá]'  OR u.rut ~* 'm[aá]c[aá]r[eé]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'r[uú]b[ií]l[aá]r'  OR u.rut ~* 'r[uú]b[ií]l[aá]r') 
