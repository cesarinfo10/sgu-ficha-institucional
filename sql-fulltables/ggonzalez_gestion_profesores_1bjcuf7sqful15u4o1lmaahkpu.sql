COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]ndr[eé][aá]'  OR u.rut ~* '[aá]ndr[eé][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]r[aá]n[eé]l[aá]'  OR u.rut ~* '[aá]r[aá]n[eé]l[aá]') 
