COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'cl[aá][uú]d[ií][oó]'  OR u.rut ~* 'cl[aá][uú]d[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'f[eé]rn[aá]nd[eé]z'  OR u.rut ~* 'f[eé]rn[aá]nd[eé]z') 
