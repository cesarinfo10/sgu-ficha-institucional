COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'gl[oó]r[ií][aá]'  OR u.rut ~* 'gl[oó]r[ií][aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]n[aá]l[eé]s'  OR u.rut ~* 'c[aá]n[aá]l[eé]s') 
