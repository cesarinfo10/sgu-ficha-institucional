COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'g[aá]st[oó]n'  OR u.rut ~* 'g[aá]st[oó]n')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[oó]nz[aá]l[eé]z'  OR u.rut ~* 'g[oó]nz[aá]l[eé]z') 
