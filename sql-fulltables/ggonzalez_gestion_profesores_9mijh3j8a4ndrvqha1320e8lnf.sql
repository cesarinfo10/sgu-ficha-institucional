COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]r[oó]l[ií]n[aá]'  OR u.rut ~* 'c[aá]r[oó]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[uú]c[uú]m[ií]d[eé]s'  OR u.rut ~* 'c[uú]c[uú]m[ií]d[eé]s') 
