COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'v[aá]l[eé]nt[ií]n[aá]'  OR u.rut ~* 'v[aá]l[eé]nt[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'l[oó]b[oó]s'  OR u.rut ~* 'l[oó]b[oó]s') 
