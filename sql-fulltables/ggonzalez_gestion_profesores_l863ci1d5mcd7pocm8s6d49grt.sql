COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'p[eé]dr[oó]'  OR u.rut ~* 'p[eé]dr[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'h[eé]nr[ií]q[uú][eé]z'  OR u.rut ~* 'h[eé]nr[ií]q[uú][eé]z') 
