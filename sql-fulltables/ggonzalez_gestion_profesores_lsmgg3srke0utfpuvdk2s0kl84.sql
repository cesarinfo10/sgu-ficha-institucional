COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]r[oó]l[ií]n[aá]'  OR u.rut ~* 'c[aá]r[oó]l[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'fr[aá]nc[ií]sc[aá]'  OR u.rut ~* 'fr[aá]nc[ií]sc[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[oó]rt[eé]s'  OR u.rut ~* 'c[oó]rt[eé]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[ií]n[oó]'  OR u.rut ~* 'p[ií]n[oó]') 
