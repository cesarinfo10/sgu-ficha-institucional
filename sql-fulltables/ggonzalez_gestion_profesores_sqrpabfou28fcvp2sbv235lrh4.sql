COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'g[eé]n[aá]r[oó]'  OR u.rut ~* 'g[eé]n[aá]r[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[oó]y[aá]n[oó]'  OR u.rut ~* 'm[oó]y[aá]n[oó]') 
