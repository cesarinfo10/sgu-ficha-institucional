COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]r[ií][oó]'  OR u.rut ~* 'm[aá]r[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]l[ií]z[oó]nd[oó]'  OR u.rut ~* '[eé]l[ií]z[oó]nd[oó]') 
