COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[ií]g[uú][eé]l'  OR u.rut ~* 'm[ií]g[uú][eé]l')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]ng[eé]l'  OR u.rut ~* '[aá]ng[eé]l')  AND (lower(u.nombre||' '||u.apellido) ~* 's[ií]lv[aá]'  OR u.rut ~* 's[ií]lv[aá]') 
