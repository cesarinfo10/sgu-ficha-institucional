COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2024 AND regimen='DIP-D' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2024   
                FROM alumnos AS a
                ...
                WHERE true  AND (a.cohorte = '2023' ) AND (c.regimen = 'DIP-D') 
