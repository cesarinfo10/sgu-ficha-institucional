COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]ct[oó]r' OR  pap.rut ~* 'v[ií]ct[oó]r' OR  text(pap.id) ~* 'v[ií]ct[oó]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[uú]st[oó]s' OR  pap.rut ~* '[uú]st[oó]s' OR  text(pap.id) ~* '[uú]st[oó]s' ) 
