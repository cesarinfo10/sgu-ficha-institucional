COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'fr[aá]nc[ií]sc[aá]' OR  pap.rut ~* 'fr[aá]nc[ií]sc[aá]' OR  text(pap.id) ~* 'fr[aá]nc[ií]sc[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]nd[ií][aá]' OR  pap.rut ~* 'c[aá]nd[ií][aá]' OR  text(pap.id) ~* 'c[aá]nd[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]l[aá]z[aá]r' OR  pap.rut ~* 's[aá]l[aá]z[aá]r' OR  text(pap.id) ~* 's[aá]l[aá]z[aá]r' ) 
