COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    LEFT JOIN vista_contratos_anos AS vca ON (vca.rut=pap.rut AND vca.ano=2022) 
                    LEFT JOIN admision.referencias AS r ON r.id=pap.referencia
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[oó]l[aá]ng[eé]' OR  pap.rut ~* 's[oó]l[aá]ng[eé]' OR  text(pap.id) ~* 's[oó]l[aá]ng[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]rn[eé]j[oó]' OR  pap.rut ~* 'c[oó]rn[eé]j[oó]' OR  text(pap.id) ~* 'c[oó]rn[eé]j[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]b[aá]r' OR  pap.rut ~* 't[oó]b[aá]r' OR  text(pap.id) ~* 't[oó]b[aá]r' ) 
