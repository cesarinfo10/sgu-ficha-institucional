COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]tr[ií]c[ií][aá]' OR  pap.rut ~* 'p[aá]tr[ií]c[ií][aá]' OR  text(pap.id) ~* 'p[aá]tr[ií]c[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]t[eé]r[oó]' OR  pap.rut ~* '[oó]t[eé]r[oó]' OR  text(pap.id) ~* '[oó]t[eé]r[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]n[oó]' OR  pap.rut ~* 'c[aá]n[oó]' OR  text(pap.id) ~* 'c[aá]n[oó]' ) 
