COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]j[aá]ndr[oó]' OR  pap.rut ~* '[aá]l[eé]j[aá]ndr[oó]' OR  text(pap.id) ~* '[aá]l[eé]j[aá]ndr[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]c[eé]r[eé]s' OR  pap.rut ~* 'c[aá]c[eé]r[eé]s' OR  text(pap.id) ~* 'c[aá]c[eé]r[eé]s' ) 
