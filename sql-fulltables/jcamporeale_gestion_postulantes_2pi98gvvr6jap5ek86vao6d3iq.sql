COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'n[ií]c[oó]l[eé]' OR  pap.rut ~* 'n[ií]c[oó]l[eé]' OR  text(pap.id) ~* 'n[ií]c[oó]l[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]ll[eé]g[uú][ií]ll[oó]s' OR  pap.rut ~* 'g[aá]ll[eé]g[uú][ií]ll[oó]s' OR  text(pap.id) ~* 'g[aá]ll[eé]g[uú][ií]ll[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
