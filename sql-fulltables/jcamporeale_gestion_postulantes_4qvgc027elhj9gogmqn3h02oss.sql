COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  pap.rut ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  text(pap.id) ~* 'm[aá][uú]r[ií]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]y[aá]' OR  pap.rut ~* 'm[oó]y[aá]' OR  text(pap.id) ~* 'm[oó]y[aá]' ) 
