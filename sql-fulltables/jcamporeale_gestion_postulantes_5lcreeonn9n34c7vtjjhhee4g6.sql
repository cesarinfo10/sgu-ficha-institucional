COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'jh[oó]n[aá]t[aá]n' OR  pap.rut ~* 'jh[oó]n[aá]t[aá]n' OR  text(pap.id) ~* 'jh[oó]n[aá]t[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lv[eé][aá]l' OR  pap.rut ~* '[aá]lv[eé][aá]l' OR  text(pap.id) ~* '[aá]lv[eé][aá]l' ) 
