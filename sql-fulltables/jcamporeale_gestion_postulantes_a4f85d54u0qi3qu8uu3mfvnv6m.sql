COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rly' OR  pap.rut ~* 'm[aá]rly' OR  text(pap.id) ~* 'm[aá]rly' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]l[aá]s' OR  pap.rut ~* 's[aá]l[aá]s' OR  text(pap.id) ~* 's[aá]l[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[oó]mm[eé]r' OR  pap.rut ~* 's[oó]mm[eé]r' OR  text(pap.id) ~* 's[oó]mm[eé]r' ) 
