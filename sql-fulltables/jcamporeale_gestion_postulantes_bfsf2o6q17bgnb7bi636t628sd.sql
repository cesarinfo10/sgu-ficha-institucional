COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'br[eé]nd[aá]' OR  pap.rut ~* 'br[eé]nd[aá]' OR  text(pap.id) ~* 'br[eé]nd[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  pap.rut ~* 'm[aá]nr[ií]q[uú][eé]z' OR  text(pap.id) ~* 'm[aá]nr[ií]q[uú][eé]z' ) 
