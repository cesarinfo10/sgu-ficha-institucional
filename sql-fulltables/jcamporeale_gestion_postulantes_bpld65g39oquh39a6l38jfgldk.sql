COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá][uú]r[eé][eé]n' OR  pap.rut ~* 'm[aá][uú]r[eé][eé]n' OR  text(pap.id) ~* 'm[aá][uú]r[eé][eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[uú]ñ[oó]z' OR  pap.rut ~* 'm[uú]ñ[oó]z' OR  text(pap.id) ~* 'm[uú]ñ[oó]z' ) 
