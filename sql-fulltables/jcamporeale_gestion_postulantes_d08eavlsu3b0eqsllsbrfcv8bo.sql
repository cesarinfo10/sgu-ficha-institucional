COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cr[ií]st[oó]b[aá]l' OR  pap.rut ~* 'cr[ií]st[oó]b[aá]l' OR  text(pap.id) ~* 'cr[ií]st[oó]b[aá]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]rr[eé][aá]' OR  pap.rut ~* 'c[oó]rr[eé][aá]' OR  text(pap.id) ~* 'c[oó]rr[eé][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
