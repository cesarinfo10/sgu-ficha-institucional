COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[ií][eé]g[oó]' OR  pap.rut ~* 'd[ií][eé]g[oó]' OR  text(pap.id) ~* 'd[ií][eé]g[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rd[oó]n[eé]s' OR  pap.rut ~* 'm[aá]rd[oó]n[eé]s' OR  text(pap.id) ~* 'm[aá]rd[oó]n[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]r[aá]m[ií]ll[oó]' OR  pap.rut ~* 'j[aá]r[aá]m[ií]ll[oó]' OR  text(pap.id) ~* 'j[aá]r[aá]m[ií]ll[oó]' ) 
