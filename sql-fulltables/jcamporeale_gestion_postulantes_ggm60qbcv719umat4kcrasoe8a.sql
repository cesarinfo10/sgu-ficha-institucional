COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[ií]ch[aá][eé]l' OR  pap.rut ~* 'm[ií]ch[aá][eé]l' OR  text(pap.id) ~* 'm[ií]ch[aá][eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nc[ií][aá]' OR  pap.rut ~* 'v[aá]l[eé]nc[ií][aá]' OR  text(pap.id) ~* 'v[aá]l[eé]nc[ií][aá]' ) 
