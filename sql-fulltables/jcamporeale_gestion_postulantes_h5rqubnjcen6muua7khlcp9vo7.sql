COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lh[ií][eé]s' OR  pap.rut ~* '[aá]lh[ií][eé]s' OR  text(pap.id) ~* '[aá]lh[ií][eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]lv[eé]z' OR  pap.rut ~* 'g[aá]lv[eé]z' OR  text(pap.id) ~* 'g[aá]lv[eé]z' ) 
