COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'k[aá]th[eé]r[ií]n[eé]' OR  pap.rut ~* 'k[aá]th[eé]r[ií]n[eé]' OR  text(pap.id) ~* 'k[aá]th[eé]r[ií]n[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]str[oó]' OR  pap.rut ~* 'c[aá]str[oó]' OR  text(pap.id) ~* 'c[aá]str[oó]' ) 
