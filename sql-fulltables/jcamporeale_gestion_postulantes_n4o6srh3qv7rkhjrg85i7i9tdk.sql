COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'fr[aá]nc[oó][ií]s[eé]' OR  pap.rut ~* 'fr[aá]nc[oó][ií]s[eé]' OR  text(pap.id) ~* 'fr[aá]nc[oó][ií]s[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'sp[eé]nc[eé]r' OR  pap.rut ~* 'sp[eé]nc[eé]r' OR  text(pap.id) ~* 'sp[eé]nc[eé]r' ) 
