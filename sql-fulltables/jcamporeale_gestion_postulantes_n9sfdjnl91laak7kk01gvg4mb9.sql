COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]sq[uú][eé]z' OR  pap.rut ~* 'v[aá]sq[uú][eé]z' OR  text(pap.id) ~* 'v[aá]sq[uú][eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé][ií]v[aá]' OR  pap.rut ~* 'l[eé][ií]v[aá]' OR  text(pap.id) ~* 'l[eé][ií]v[aá]' ) 
