COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú][ií]s' OR  pap.rut ~* 'l[uú][ií]s' OR  text(pap.id) ~* 'l[uú][ií]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'br[ií]c[eé]ñ[oó]' OR  pap.rut ~* 'br[ií]c[eé]ñ[oó]' OR  text(pap.id) ~* 'br[ií]c[eé]ñ[oó]' ) 
