COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'k[aá]th[eé]r[ií]n[eé]' OR  pap.rut ~* 'k[aá]th[eé]r[ií]n[eé]' OR  text(pap.id) ~* 'k[aá]th[eé]r[ií]n[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'h[eé]rn[aá]nd[eé]z' OR  pap.rut ~* 'h[eé]rn[aá]nd[eé]z' OR  text(pap.id) ~* 'h[eé]rn[aá]nd[eé]z' ) 
