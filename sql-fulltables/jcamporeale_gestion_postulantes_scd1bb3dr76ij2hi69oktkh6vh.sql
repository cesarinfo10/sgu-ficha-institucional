COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]s[eé]' OR  pap.rut ~* 'j[oó]s[eé]' OR  text(pap.id) ~* 'j[oó]s[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]gn[aá]c[ií][oó]' OR  pap.rut ~* '[ií]gn[aá]c[ií][oó]' OR  text(pap.id) ~* '[ií]gn[aá]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]g[uú][ií]l[aá]r' OR  pap.rut ~* '[aá]g[uú][ií]l[aá]r' OR  text(pap.id) ~* '[aá]g[uú][ií]l[aá]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sp[ií]n[oó]s[aá]' OR  pap.rut ~* '[eé]sp[ií]n[oó]s[aá]' OR  text(pap.id) ~* '[eé]sp[ií]n[oó]s[aá]' ) 
