COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[eé]s[uú]s' OR  pap.rut ~* 'j[eé]s[uú]s' OR  text(pap.id) ~* 'j[eé]s[uú]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]d[ií]n[aá]' OR  pap.rut ~* 'm[eé]d[ií]n[aá]' OR  text(pap.id) ~* 'm[eé]d[ií]n[aá]' ) 
