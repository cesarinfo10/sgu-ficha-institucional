COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]sc[aá]r' OR  pap.rut ~* '[oó]sc[aá]r' OR  text(pap.id) ~* '[oó]sc[aá]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]ss' OR  pap.rut ~* 'r[oó]ss' OR  text(pap.id) ~* 'r[oó]ss' ) 
