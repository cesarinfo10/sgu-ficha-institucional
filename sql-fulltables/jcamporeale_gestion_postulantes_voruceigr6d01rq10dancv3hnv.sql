COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]bl[oó]' OR  pap.rut ~* 'p[aá]bl[oó]' OR  text(pap.id) ~* 'p[aá]bl[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lt[aá]m[ií]r[aá]n[oó]' OR  pap.rut ~* '[aá]lt[aá]m[ií]r[aá]n[oó]' OR  text(pap.id) ~* '[aá]lt[aá]m[ií]r[aá]n[oó]' ) 
