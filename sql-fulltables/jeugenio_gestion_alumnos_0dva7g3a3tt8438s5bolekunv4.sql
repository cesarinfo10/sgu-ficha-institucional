COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]ld' OR  a.rut ~* 'c[oó]ld' OR  lower(a.email) ~* 'c[oó]ld' OR  text(a.id) ~* 'c[oó]ld' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá][ií]r' OR  a.rut ~* '[aá][ií]r' OR  lower(a.email) ~* '[aá][ií]r' OR  text(a.id) ~* '[aá][ií]r' ) 
