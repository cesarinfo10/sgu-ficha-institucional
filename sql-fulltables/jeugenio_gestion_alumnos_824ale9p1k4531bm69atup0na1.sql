COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]l[ií][aá]s' OR  a.rut ~* '[eé]l[ií][aá]s' OR  lower(a.email) ~* '[eé]l[ií][aá]s' OR  text(a.id) ~* '[eé]l[ií][aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]nc[oó]p[aá]n' OR  a.rut ~* 'l[ií]nc[oó]p[aá]n' OR  lower(a.email) ~* 'l[ií]nc[oó]p[aá]n' OR  text(a.id) ~* 'l[ií]nc[oó]p[aá]n' ) 
