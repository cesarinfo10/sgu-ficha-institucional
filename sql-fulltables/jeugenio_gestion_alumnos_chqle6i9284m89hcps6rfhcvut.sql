COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'bl[aá]nc[aá]' OR  a.rut ~* 'bl[aá]nc[aá]' OR  lower(a.email) ~* 'bl[aá]nc[aá]' OR  text(a.id) ~* 'bl[aá]nc[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]l[ií]n[aá]s' OR  a.rut ~* 's[aá]l[ií]n[aá]s' OR  lower(a.email) ~* 's[aá]l[ií]n[aá]s' OR  text(a.id) ~* 's[aá]l[ií]n[aá]s' ) 
