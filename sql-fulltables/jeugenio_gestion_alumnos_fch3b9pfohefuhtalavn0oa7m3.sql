COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá]v[ií]' OR  a.rut ~* 'j[aá]v[ií]' OR  lower(a.email) ~* 'j[aá]v[ií]' OR  text(a.id) ~* 'j[aá]v[ií]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'tr[eé]sk[oó]w' OR  a.rut ~* 'tr[eé]sk[oó]w' OR  lower(a.email) ~* 'tr[eé]sk[oó]w' OR  text(a.id) ~* 'tr[eé]sk[oó]w' ) 
