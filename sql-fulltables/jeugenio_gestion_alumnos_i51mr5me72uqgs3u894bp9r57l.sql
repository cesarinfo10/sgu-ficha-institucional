COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé]' OR  a.rut ~* 'j[eé]' OR  lower(a.email) ~* 'j[eé]' OR  text(a.id) ~* 'j[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]n[eé]d[aá]' OR  a.rut ~* 'p[ií]n[eé]d[aá]' OR  lower(a.email) ~* 'p[ií]n[eé]d[aá]' OR  text(a.id) ~* 'p[ií]n[eé]d[aá]' ) 
