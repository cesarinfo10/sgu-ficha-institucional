COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]ss[oó]n' OR  a.rut ~* 'm[aá]ss[oó]n' OR  lower(a.email) ~* 'm[aá]ss[oó]n' OR  text(a.id) ~* 'm[aá]ss[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  a.rut ~* 'm[aá]nr[ií]q[uú][eé]z' OR  lower(a.email) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  text(a.id) ~* 'm[aá]nr[ií]q[uú][eé]z' ) 
