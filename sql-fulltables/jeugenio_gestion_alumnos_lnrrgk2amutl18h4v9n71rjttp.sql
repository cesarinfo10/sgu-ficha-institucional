COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                       ...
                       (SELECT estado
                     FROM dae.fuas
                     WHERE id_alumno=a.id and ano=2024
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT count(id) FROM sinergia.respuestas WHERE ano=2024 AND semestre=1 AND rut_alumno=a.rut)=2 THEN 'Si' ELSE 'No' END AS prubeas_psico,
                       CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2024 AND semestre=1 LIMIT 1)=1  THEN 'Si' ELSE 'No' END AS matriculado,(SELECT max(fecha) FROM matriculas WHERE id_alumno=a.id) AS fecha_ult_mat,
                       ...
                       CASE WHEN (SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE semestre=2 AND ano=2023))>0 THEN ((SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_estado=1 AND id_curso IN (SELECT id FROM cursos WHERE semestre=2 AND ano=2023))::real/(SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE semestre=2 AND ano=2023))::real*100)::numeric(4,1) ELSE 0 END AS tasa_aprobacion_ant,
                       CASE WHEN (SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE ano=2024 AND semestre=1))>0 THEN ((SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_estado=1 AND id_curso IN (SELECT id FROM cursos WHERE ano=2024 AND semestre=1))::real/(SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE ano=2024 AND semestre=1))::real*100)::numeric(4,1) ELSE 0 END AS tasa_aprobacion_act,
                       CASE WHEN (SELECT count(id_alumno) AS cant_insc FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2024) AND id_alumno=a.id)>0 THEN ((SELECT count(id_alumno) AS cant_aprob FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2024) AND id_estado=1 AND id_alumno=a.id)::real/(SELECT count(id_alumno) AS cant_insc FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2024) AND id_alumno=a.id)::real*100)::numeric(4,1) ELSE 0 END AS porcentaje_avance,
                       ...
                       (SELECT monto_arancel*CASE trim(tipo) WHEN 'Semestral' THEN 2 ELSE 1 END AS arancel_real FROM finanzas.contratos WHERE ano=2024 AND (id_alumno=a.id OR id_pap=a.id_pap) AND estado IN ('E','S','R','A') LIMIT 1) AS arancel_real,
                       ...
                       WHERE vac.ano=2024 AND vac.id_alumno=a.id),1) AS nivel_estudios, 
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2024 AND regimen='' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2024   
                FROM alumnos AS a
                ...
                LEFT JOIN (SELECT id_alumno,round(avg(nota_final),1) as nota_final,count(id) AS cant_asig FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2024-1) AND id_estado IN (1,2) GROUP BY id_alumno)       AS paa ON paa.id_alumno=a.id
                LEFT JOIN (SELECT id_alumno,round(avg(nota_final),1) as nota_final,count(id) AS cant_asig FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2024-1) AND id_estado=1 GROUP BY id_alumno) AS paaa ON paaa.id_alumno=a.id
                ...
                LEFT JOIN (SELECT DISTINCT ON (rut,be.nombre) rut,be.nombre AS benef_fiscal 
                               ...
                               WHERE ano=2024 AND id_beca_externa IS NOT NULL) AS be ON be.rut=a.rut
                
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé][aá]tr[ií]z' OR  a.rut ~* 'b[eé][aá]tr[ií]z' OR  lower(a.email) ~* 'b[eé][aá]tr[ií]z' OR  text(a.id) ~* 'b[eé][aá]tr[ií]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]' OR  a.rut ~* 'm[aá]r[ií][aá]' OR  lower(a.email) ~* 'm[aá]r[ií][aá]' OR  text(a.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'y[uú]l[ií][aá]n[aá]' OR  a.rut ~* 'y[uú]l[ií][aá]n[aá]' OR  lower(a.email) ~* 'y[uú]l[ií][aá]n[aá]' OR  text(a.id) ~* 'y[uú]l[ií][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]l[ií]n[aá]' OR  a.rut ~* 'm[oó]l[ií]n[aá]' OR  lower(a.email) ~* 'm[oó]l[ií]n[aá]' OR  text(a.id) ~* 'm[oó]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'fl[oó]r[eé]s' OR  a.rut ~* 'fl[oó]r[eé]s' OR  lower(a.email) ~* 'fl[oó]r[eé]s' OR  text(a.id) ~* 'fl[oó]r[eé]s' ) 
