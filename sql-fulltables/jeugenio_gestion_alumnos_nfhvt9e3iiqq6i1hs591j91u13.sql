COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ct[oó]r[ií][aá]' OR  a.rut ~* 'v[ií]ct[oó]r[ií][aá]' OR  lower(a.email) ~* 'v[ií]ct[oó]r[ií][aá]' OR  text(a.id) ~* 'v[ií]ct[oó]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]b[aá]r' OR  a.rut ~* 't[oó]b[aá]r' OR  lower(a.email) ~* 't[oó]b[aá]r' OR  text(a.id) ~* 't[oó]b[aá]r' ) 
