COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]n[aá][eé]' OR  a.rut ~* 'd[aá]n[aá][eé]' OR  lower(a.email) ~* 'd[aá]n[aá][eé]' OR  text(a.id) ~* 'd[aá]n[aá][eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][ií]v[aá]' OR  a.rut ~* 'l[eé][ií]v[aá]' OR  lower(a.email) ~* 'l[eé][ií]v[aá]' OR  text(a.id) ~* 'l[eé][ií]v[aá]' ) 
