COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true  AND (c.regimen = 'PRE')  AND presel_mineduc.rut IS NOT NULL AND presel_mineduc.cant_becas > 0 AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1)) 
