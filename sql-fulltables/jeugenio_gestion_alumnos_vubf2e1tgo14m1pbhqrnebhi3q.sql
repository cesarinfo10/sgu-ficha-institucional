COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[aá]l[ií]n[aá]' OR  a.rut ~* 'c[aá]t[aá]l[ií]n[aá]' OR  lower(a.email) ~* 'c[aá]t[aá]l[ií]n[aá]' OR  text(a.id) ~* 'c[aá]t[aá]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]rr[eé]s' OR  a.rut ~* 't[oó]rr[eé]s' OR  lower(a.email) ~* 't[oó]rr[eé]s' OR  text(a.id) ~* 't[oó]rr[eé]s' ) 
