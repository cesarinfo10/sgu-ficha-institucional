COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '17240284-4' OR  pap.rut ~* '17240284-4' OR lower(a.nombres||' '||a.apellidos) ~* '17240284-4' OR lower(a3.nombres||' '||a3.apellidos) ~* '17240284-4' OR  a.rut ~* '17240284-4' OR a2.rut ~* '17240284-4' OR a3.rut ~* '17240284-4'  OR text(nro_boleta) = '17240284-4' )  
