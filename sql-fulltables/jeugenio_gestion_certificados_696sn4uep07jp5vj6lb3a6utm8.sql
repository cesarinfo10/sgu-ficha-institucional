COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  a.rut ~* 'r[oó]dr[ií]g[uú][eé]z' OR  text(ac.folio) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  vac.cod ~* 'r[oó]dr[ií]g[uú][eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]b[eé]ll[oó]' OR  a.rut ~* 'c[aá]b[eé]ll[oó]' OR  text(ac.folio) ~* 'c[aá]b[eé]ll[oó]' OR  vac.cod ~* 'c[aá]b[eé]ll[oó]' ) 
