COPY (SELECT et.id,to_char(et.fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
				  ...
				  WHERE true   AND (vete.nombres::text ~* 'b[aá]rr[ií][eé]nt[oó]s' OR  vete.ruts::text ~* 'b[aá]rr[ií][eé]nt[oó]s' ) AND (vete.nombres::text ~* 'j[aá]r[aá]' OR  vete.ruts::text ~* 'j[aá]r[aá]' )  			  
