COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]g[uú][eé]l' OR  a.rut ~* 'm[ií]g[uú][eé]l' OR  text(a.id) ~* 'm[ií]g[uú][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]t[eé][ií]z[aá]' OR  a.rut ~* '[oó]t[eé][ií]z[aá]' OR  text(a.id) ~* '[oó]t[eé][ií]z[aá]' ) 
