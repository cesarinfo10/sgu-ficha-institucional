COPY (SELECT c.id,to_char(c.fecha,'DD-tmMon-YYYY') AS fecha,c.estado,
                            ...
                            ((SELECT sum(monto_pagado) 
                                 ...
                                 WHERE true  AND p.fecha::date BETWEEN '2023-01-01'::date AND '2023-12-31'::date  AND id_convenio_ci=vc.id
                     ...
                     WHERE true AND date_part('year',c.fecha) = 2023 AND NOT c.nulo AND (car.regimen = 'PRE') 
