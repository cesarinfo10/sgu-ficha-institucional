COPY (SELECT * FROM (SELECT DISTINCT ON (nc.nro_docto) nc.nro_docto,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto_pago,nc.id_pago,
                              ...
                              END AS rut_alumno,
							  to_char(p.fecha,'DD-MM-YYYY') AS fecha_pago,u2.nombre_usuario AS cajero_pago,
		   ...
		   WHERE (p.nro_boleta IS NOT NULL OR p.nro_boleta_e IS NOT NULL OR p.nro_factura IS NOT NULL) AND nc.fecha BETWEEN now()-'1 month'::interval AND now() AND ('PRE' IN ( car.regimen,car2.regimen,car3.regimen) )  
