COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (p.nro_boleta IS NOT NULL OR p.nro_boleta_e IS NOT NULL OR p.nro_factura IS NOT NULL) AND p.fecha BETWEEN now()-'6 month'::interval AND now() AND nro_boleta_e IS NOT NULL AND bol_e_cod_erp IS NULL AND ('PRE' IN (car.regimen,car2.regimen,car3.regimen) OR p.nulo)  
