COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]t[ií][aá]s' OR  pap.rut ~* 'm[aá]t[ií][aá]s' OR  text(pap.id) ~* 'm[aá]t[ií][aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[uú]zm[aá]n' OR  pap.rut ~* 'g[uú]zm[aá]n' OR  text(pap.id) ~* 'g[uú]zm[aá]n' ) 
