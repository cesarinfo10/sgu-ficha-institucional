COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]rl[aá]nd[oó]' OR  pap.rut ~* '[oó]rl[aá]nd[oó]' OR  text(pap.id) ~* '[oó]rl[aá]nd[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú]c[eé]r[oó]' OR  pap.rut ~* 'l[uú]c[eé]r[oó]' OR  text(pap.id) ~* 'l[uú]c[eé]r[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rt[eé]r' OR  pap.rut ~* 'c[aá]rt[eé]r' OR  text(pap.id) ~* 'c[aá]rt[eé]r' ) 
