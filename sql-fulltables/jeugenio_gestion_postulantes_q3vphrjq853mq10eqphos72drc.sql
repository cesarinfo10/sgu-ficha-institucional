COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]' OR  pap.rut ~* 'd[eé]' OR  text(pap.id) ~* 'd[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]' OR  pap.rut ~* 'l[aá]' OR  text(pap.id) ~* 'l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]s[aá]' OR  pap.rut ~* 'r[oó]s[aá]' OR  text(pap.id) ~* 'r[oó]s[aá]' ) 
