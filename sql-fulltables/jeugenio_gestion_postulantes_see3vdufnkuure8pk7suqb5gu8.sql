COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]v[ií]d' OR  pap.rut ~* 'd[aá]v[ií]d' OR  text(pap.id) ~* 'd[aá]v[ií]d' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]r[eé]ll[aá]n[aá]' OR  pap.rut ~* '[oó]r[eé]ll[aá]n[aá]' OR  text(pap.id) ~* '[oó]r[eé]ll[aá]n[aá]' ) 
