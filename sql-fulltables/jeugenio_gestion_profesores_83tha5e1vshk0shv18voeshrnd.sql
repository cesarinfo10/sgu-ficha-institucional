COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'v[aá]n[eé]ss[aá]'  OR u.rut ~* 'v[aá]n[eé]ss[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'cr[ií]st[ií]n[aá]'  OR u.rut ~* 'cr[ií]st[ií]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* '[ií]n[aá]y'  OR u.rut ~* '[ií]n[aá]y')  AND (lower(u.nombre||' '||u.apellido) ~* 's[aá]r[aá]b[ií][aá]'  OR u.rut ~* 's[aá]r[aá]b[ií][aá]') 
