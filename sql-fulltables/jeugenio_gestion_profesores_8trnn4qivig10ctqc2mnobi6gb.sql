COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]n[uú][eé]l'  OR u.rut ~* 'm[aá]n[uú][eé]l')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]'  OR u.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]') 
