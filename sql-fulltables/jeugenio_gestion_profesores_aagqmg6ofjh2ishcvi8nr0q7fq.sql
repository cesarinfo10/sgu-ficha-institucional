COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[ií]n[eé]s'  OR u.rut ~* '[ií]n[eé]s')  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]l[eé]j[aá]ndr[aá]'  OR u.rut ~* '[aá]l[eé]j[aá]ndr[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[ií]c[eé]nc[ií][oó]'  OR u.rut ~* 'v[ií]c[eé]nc[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[aá]rd[oó]'  OR u.rut ~* 'p[aá]rd[oó]') 
