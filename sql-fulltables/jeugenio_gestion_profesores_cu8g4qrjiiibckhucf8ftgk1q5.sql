COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[uú]l[ií][oó]'  OR u.rut ~* 'j[uú]l[ií][oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'n[aá]v[aá]rr[oó]'  OR u.rut ~* 'n[aá]v[aá]rr[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 't[aá]p[ií][aá]'  OR u.rut ~* 't[aá]p[ií][aá]') 
