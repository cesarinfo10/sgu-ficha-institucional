COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* '[aá]lfr[eé]d[oó]'  OR u.rut ~* '[aá]lfr[eé]d[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'd[aá]n[ií]l[oó]'  OR u.rut ~* 'd[aá]n[ií]l[oó]')  AND (lower(u.nombre||' '||u.apellido) ~* 'g[oó]nz[aá]l[eé]z'  OR u.rut ~* 'g[oó]nz[aá]l[eé]z')  AND (lower(u.nombre||' '||u.apellido) ~* 'm[eé]d[ií]n[aá]'  OR u.rut ~* 'm[eé]d[ií]n[aá]') 
