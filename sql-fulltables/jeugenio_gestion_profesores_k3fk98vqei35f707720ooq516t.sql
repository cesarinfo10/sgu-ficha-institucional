COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'r[oó]x[aá]n[aá]'  OR u.rut ~* 'r[oó]x[aá]n[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* '[eé]sp[ií]n[oó]z[aá]'  OR u.rut ~* '[eé]sp[ií]n[oó]z[aá]') 
