COPY (SELECT *,CASE WHEN tasa_riesgo BETWEEN 75 and 100 THEN 'Alto'
			    ...
			    FROM (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos) AS apellidos,initcap(a.nombres) AS nombres,
                       ...
                       CASE WHEN moroso_financiero AND (SELECT sum(coalesce(cuotas_morosas,0)) AS cuotas_morosas FROM vista_contratos WHERE id_alumno=a.id OR id_pap=a.id_pap) >= 3 AND (SELECT sum(coalesce(monto_repactado,0)) AS monto_repactado FROM vista_contratos WHERE id_alumno=a.id OR id_pap=a.id_pap) = 0               THEN 25
                            ...
                            WHEN moroso_financiero AND (SELECT sum(coalesce(monto_moroso,0)) AS monto_moroso FROM vista_contratos WHERE id_alumno=a.id OR id_pap=a.id_pap) > (270833) AND (SELECT sum(coalesce(monto_repactado,0)) AS monto_repactado FROM vista_contratos WHERE id_alumno=a.id OR id_pap=a.id_pap) = 0 THEN 15
                            WHEN moroso_financiero AND (SELECT sum(coalesce(monto_moroso,0)) AS monto_moroso FROM vista_contratos WHERE id_alumno=a.id OR id_pap=a.id_pap) BETWEEN 1 AND (270833)                    THEN 5
                ...
                WHERE true  AND (a.cohorte = '2023') AND (a.semestre_cohorte = 1) AND (a.mes_cohorte = 3) AND (a.estado = '1') AND (c.regimen = 'POST-GD') 
