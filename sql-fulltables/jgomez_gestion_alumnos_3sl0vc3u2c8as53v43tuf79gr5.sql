COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[aá]c[aá]r[eé]n' OR  a.rut ~* '[aá]n[aá]c[aá]r[eé]n' OR  lower(a.email) ~* '[aá]n[aá]c[aá]r[eé]n' OR  text(a.id) ~* '[aá]n[aá]c[aá]r[eé]n' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
