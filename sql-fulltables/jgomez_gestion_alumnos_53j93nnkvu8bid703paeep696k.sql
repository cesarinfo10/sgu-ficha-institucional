COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]y[aá]rz[uú]n' OR  a.rut ~* '[oó]y[aá]rz[uú]n' OR  lower(a.email) ~* '[oó]y[aá]rz[uú]n' OR  text(a.id) ~* '[oó]y[aá]rz[uú]n' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
