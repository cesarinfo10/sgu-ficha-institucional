COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]t[ií]n' OR  a.rut ~* 't[oó]t[ií]n' OR  lower(a.email) ~* 't[oó]t[ií]n' OR  text(a.id) ~* 't[oó]t[ií]n' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17,153) 
