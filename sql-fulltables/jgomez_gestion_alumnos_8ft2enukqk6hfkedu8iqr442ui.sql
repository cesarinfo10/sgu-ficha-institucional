COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[aá]ns' OR  a.rut ~* 'h[aá]ns' OR  lower(a.email) ~* 'h[aá]ns' OR  text(a.id) ~* 'h[aá]ns' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
