COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]lly' OR  a.rut ~* 'm[oó]lly' OR  lower(a.email) ~* 'm[oó]lly' OR  text(a.id) ~* 'm[oó]lly' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
