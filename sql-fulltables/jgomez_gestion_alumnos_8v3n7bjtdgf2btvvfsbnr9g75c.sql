COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'wl[aá]d[ií]m[ií]r' OR  a.rut ~* 'wl[aá]d[ií]m[ií]r' OR  lower(a.email) ~* 'wl[aá]d[ií]m[ií]r' OR  text(a.id) ~* 'wl[aá]d[ií]m[ií]r' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
