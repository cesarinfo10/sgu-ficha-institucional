COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[uú]b[ií][aá]br[eé]' OR  a.rut ~* 's[uú]b[ií][aá]br[eé]' OR  lower(a.email) ~* 's[uú]b[ií][aá]br[eé]' OR  text(a.id) ~* 's[uú]b[ií][aá]br[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
