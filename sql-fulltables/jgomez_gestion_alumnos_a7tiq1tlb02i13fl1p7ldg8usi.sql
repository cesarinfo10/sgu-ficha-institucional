COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]ñ[eé]z' OR  a.rut ~* 'y[aá]ñ[eé]z' OR  lower(a.email) ~* 'y[aá]ñ[eé]z' OR  text(a.id) ~* 'y[aá]ñ[eé]z' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
