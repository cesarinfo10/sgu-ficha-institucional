COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]sm[aá][eé]l' OR  a.rut ~* '[ií]sm[aá][eé]l' OR  lower(a.email) ~* '[ií]sm[aá][eé]l' OR  text(a.id) ~* '[ií]sm[aá][eé]l' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
