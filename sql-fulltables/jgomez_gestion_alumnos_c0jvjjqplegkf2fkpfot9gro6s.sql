COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]b[eé]l' OR  a.rut ~* 'm[aá]b[eé]l' OR  lower(a.email) ~* 'm[aá]b[eé]l' OR  text(a.id) ~* 'm[aá]b[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]s[aá][uú]r[eé]' OR  a.rut ~* 'b[aá]s[aá][uú]r[eé]' OR  lower(a.email) ~* 'b[aá]s[aá][uú]r[eé]' OR  text(a.id) ~* 'b[aá]s[aá][uú]r[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
