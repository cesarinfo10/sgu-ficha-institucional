COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[oó]rm[aá]s' OR  a.rut ~* 'f[oó]rm[aá]s' OR  lower(a.email) ~* 'f[oó]rm[aá]s' OR  text(a.id) ~* 'f[oó]rm[aá]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
