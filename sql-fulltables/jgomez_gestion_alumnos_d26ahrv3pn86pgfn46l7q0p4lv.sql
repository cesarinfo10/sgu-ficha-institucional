COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '16553796' OR  a.rut ~* '16553796' OR  lower(a.email) ~* '16553796' OR  text(a.id) ~* '16553796' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
