COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]th[eé]rs' OR  a.rut ~* 's[oó]th[eé]rs' OR  lower(a.email) ~* 's[oó]th[eé]rs' OR  text(a.id) ~* 's[oó]th[eé]rs' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
