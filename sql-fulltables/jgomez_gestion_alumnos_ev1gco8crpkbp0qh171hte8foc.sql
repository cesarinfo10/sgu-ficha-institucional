COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]rt[ií]g[aá]s' OR  a.rut ~* '[aá]rt[ií]g[aá]s' OR  lower(a.email) ~* '[aá]rt[ií]g[aá]s' OR  text(a.id) ~* '[aá]rt[ií]g[aá]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
