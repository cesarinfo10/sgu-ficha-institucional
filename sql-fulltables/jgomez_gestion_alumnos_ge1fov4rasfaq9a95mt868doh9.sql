COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]ndr[aá]' OR  a.rut ~* 's[aá]ndr[aá]' OR  lower(a.email) ~* 's[aá]ndr[aá]' OR  text(a.id) ~* 's[aá]ndr[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'fl[oó]r[eé]s' OR  a.rut ~* 'fl[oó]r[eé]s' OR  lower(a.email) ~* 'fl[oó]r[eé]s' OR  text(a.id) ~* 'fl[oó]r[eé]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
