COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]p[uú]lv[eé]d[aá]' OR  a.rut ~* 's[eé]p[uú]lv[eé]d[aá]' OR  lower(a.email) ~* 's[eé]p[uú]lv[eé]d[aá]' OR  text(a.id) ~* 's[eé]p[uú]lv[eé]d[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'tr[ií]b[ií]ñ[oó]' OR  a.rut ~* 'tr[ií]b[ií]ñ[oó]' OR  lower(a.email) ~* 'tr[ií]b[ií]ñ[oó]' OR  text(a.id) ~* 'tr[ií]b[ií]ñ[oó]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17,153) 
