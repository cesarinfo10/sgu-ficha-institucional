COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]m[ií]lp[aá]n' OR  a.rut ~* '[ií]m[ií]lp[aá]n' OR  lower(a.email) ~* '[ií]m[ií]lp[aá]n' OR  text(a.id) ~* '[ií]m[ií]lp[aá]n' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
