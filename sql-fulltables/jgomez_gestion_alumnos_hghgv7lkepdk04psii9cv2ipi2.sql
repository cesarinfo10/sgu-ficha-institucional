COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií]n' OR  a.rut ~* 'm[aá]r[ií]n' OR  lower(a.email) ~* 'm[aá]r[ií]n' OR  text(a.id) ~* 'm[aá]r[ií]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[aá]l[aá]n' OR  a.rut ~* 'c[aá]t[aá]l[aá]n' OR  lower(a.email) ~* 'c[aá]t[aá]l[aá]n' OR  text(a.id) ~* 'c[aá]t[aá]l[aá]n' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
