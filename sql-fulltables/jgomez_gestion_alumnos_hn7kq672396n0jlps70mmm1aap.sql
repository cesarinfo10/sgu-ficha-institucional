COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]mp[eé]r[ií][aá]l[eé]' OR  a.rut ~* '[eé]mp[eé]r[ií][aá]l[eé]' OR  lower(a.email) ~* '[eé]mp[eé]r[ií][aá]l[eé]' OR  text(a.id) ~* '[eé]mp[eé]r[ií][aá]l[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
