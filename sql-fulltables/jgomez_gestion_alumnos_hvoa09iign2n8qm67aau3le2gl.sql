COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]rn[aá]nd[eé]z' OR  a.rut ~* 'h[eé]rn[aá]nd[eé]z' OR  lower(a.email) ~* 'h[eé]rn[aá]nd[eé]z' OR  text(a.id) ~* 'h[eé]rn[aá]nd[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]v[aá]s' OR  a.rut ~* 'r[ií]v[aá]s' OR  lower(a.email) ~* 'r[ií]v[aá]s' OR  text(a.id) ~* 'r[ií]v[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]th[eé]r[ií]n[eé]' OR  a.rut ~* 'k[aá]th[eé]r[ií]n[eé]' OR  lower(a.email) ~* 'k[aá]th[eé]r[ií]n[eé]' OR  text(a.id) ~* 'k[aá]th[eé]r[ií]n[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]sk[aá]' OR  a.rut ~* 'v[aá]l[eé]sk[aá]' OR  lower(a.email) ~* 'v[aá]l[eé]sk[aá]' OR  text(a.id) ~* 'v[aá]l[eé]sk[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
