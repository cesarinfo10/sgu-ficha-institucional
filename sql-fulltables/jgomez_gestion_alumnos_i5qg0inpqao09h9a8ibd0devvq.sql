COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé][oó]' OR  a.rut ~* 'g[eé][oó]' OR  lower(a.email) ~* 'g[eé][oó]' OR  text(a.id) ~* 'g[eé][oó]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
