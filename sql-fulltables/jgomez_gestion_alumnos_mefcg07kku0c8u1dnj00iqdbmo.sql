COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[oó]l[aá]' OR  a.rut ~* 'c[aá]r[oó]l[aá]' OR  lower(a.email) ~* 'c[aá]r[oó]l[aá]' OR  text(a.id) ~* 'c[aá]r[oó]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]rr[eé]s' OR  a.rut ~* 't[oó]rr[eé]s' OR  lower(a.email) ~* 't[oó]rr[eé]s' OR  text(a.id) ~* 't[oó]rr[eé]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
