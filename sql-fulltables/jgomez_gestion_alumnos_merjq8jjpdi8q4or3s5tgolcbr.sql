COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé]m[ií]' OR  a.rut ~* 'd[eé]m[ií]' OR  lower(a.email) ~* 'd[eé]m[ií]' OR  text(a.id) ~* 'd[eé]m[ií]' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
