COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]q[uú][eé]lm[eé]' OR  a.rut ~* 'r[ií]q[uú][eé]lm[eé]' OR  lower(a.email) ~* 'r[ií]q[uú][eé]lm[eé]' OR  text(a.id) ~* 'r[ií]q[uú][eé]lm[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
