COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rv[aá]j[aá]l' OR  a.rut ~* 'c[aá]rv[aá]j[aá]l' OR  lower(a.email) ~* 'c[aá]rv[aá]j[aá]l' OR  text(a.id) ~* 'c[aá]rv[aá]j[aá]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]g[aá]' OR  a.rut ~* 'v[eé]g[aá]' OR  lower(a.email) ~* 'v[eé]g[aá]' OR  text(a.id) ~* 'v[eé]g[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
