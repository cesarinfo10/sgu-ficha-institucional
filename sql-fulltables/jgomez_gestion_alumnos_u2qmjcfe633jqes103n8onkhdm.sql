COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][aá]l' OR  a.rut ~* 'l[eé][aá]l' OR  lower(a.email) ~* 'l[eé][aá]l' OR  text(a.id) ~* 'l[eé][aá]l' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
