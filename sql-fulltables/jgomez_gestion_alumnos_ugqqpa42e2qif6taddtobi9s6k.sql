COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  a.rut ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  lower(a.email) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  text(a.id) ~* 'n[aá]v[aá]rr[eé]t[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
