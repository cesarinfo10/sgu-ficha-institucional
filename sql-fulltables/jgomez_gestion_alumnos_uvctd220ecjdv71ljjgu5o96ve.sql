COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[uú]f[aá]d[eé]l' OR  a.rut ~* 'b[uú]f[aá]d[eé]l' OR  lower(a.email) ~* 'b[uú]f[aá]d[eé]l' OR  text(a.id) ~* 'b[uú]f[aá]d[eé]l' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
