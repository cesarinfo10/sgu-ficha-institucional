COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                           ...
                           CASE WHEN vcpe.id IS NOT NULL THEN 'Si' ELSE 'No' END AS desertor, 
                           ae.nombre AS estado_estudiante,pap.regimen,vcpe.nombre_usuario AS ejecutivo,
                    ...
                    LEFT JOIN vista_contratos_pap_estado AS vcpe ON (pap.id= vcpe.id_pap AND vcpe.ano=2023)
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]x[aá]ndr[aá]' OR  pap.rut ~* '[aá]l[eé]x[aá]ndr[aá]' OR  text(pap.id) ~* '[aá]l[eé]x[aá]ndr[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá][uú]l[eé]tt[eé]' OR  pap.rut ~* 'p[aá][uú]l[eé]tt[eé]' OR  text(pap.id) ~* 'p[aá][uú]l[eé]tt[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[eé]' OR  pap.rut ~* 'd[eé]' OR  text(pap.id) ~* 'd[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]' OR  pap.rut ~* 'l[aá]' OR  text(pap.id) ~* 'l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nt[eé]' OR  pap.rut ~* 'f[uú][eé]nt[eé]' OR  text(pap.id) ~* 'f[uú][eé]nt[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]ld[eé]s' OR  pap.rut ~* 'v[aá]ld[eé]s' OR  text(pap.id) ~* 'v[aá]ld[eé]s' ) 
