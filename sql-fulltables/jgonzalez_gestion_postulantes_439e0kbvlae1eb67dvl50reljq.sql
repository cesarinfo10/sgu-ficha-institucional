COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]pùlv[eé]d[aá]' OR  pap.rut ~* 's[eé]pùlv[eé]d[aá]' OR  text(pap.id) ~* 's[eé]pùlv[eé]d[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'tr[oó]nc[oó]s[oó]' OR  pap.rut ~* 'tr[oó]nc[oó]s[oó]' OR  text(pap.id) ~* 'tr[oó]nc[oó]s[oó]' ) 
