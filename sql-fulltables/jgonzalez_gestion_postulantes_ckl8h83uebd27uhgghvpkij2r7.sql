COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]n[ií][eé]l' OR  pap.rut ~* 'd[aá]n[ií][eé]l' OR  text(pap.id) ~* 'd[aá]n[ií][eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[uú]ñ[oó]z' OR  pap.rut ~* 'm[uú]ñ[oó]z' OR  text(pap.id) ~* 'm[uú]ñ[oó]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
