COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[eé]rn[aá]n' OR  pap.rut ~* 'h[eé]rn[aá]n' OR  text(pap.id) ~* 'h[eé]rn[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]st[oó]rg[aá]' OR  pap.rut ~* '[aá]st[oó]rg[aá]' OR  text(pap.id) ~* '[aá]st[oó]rg[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
