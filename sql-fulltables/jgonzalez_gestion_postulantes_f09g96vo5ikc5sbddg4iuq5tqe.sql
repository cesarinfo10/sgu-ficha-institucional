COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]s[aá][aá]c' OR  pap.rut ~* '[ií]s[aá][aá]c' OR  text(pap.id) ~* '[ií]s[aá][aá]c' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[oó]d[oó]y' OR  pap.rut ~* 'g[oó]d[oó]y' OR  text(pap.id) ~* 'g[oó]d[oó]y' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
