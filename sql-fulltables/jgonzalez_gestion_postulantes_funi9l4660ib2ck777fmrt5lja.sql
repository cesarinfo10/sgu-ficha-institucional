COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]rm[aá]' OR  pap.rut ~* '[ií]rm[aá]' OR  text(pap.id) ~* '[ií]rm[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]ltr[aá]n' OR  pap.rut ~* 'b[eé]ltr[aá]n' OR  text(pap.id) ~* 'b[eé]ltr[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
