COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[aá][uú]l' OR  pap.rut ~* 'r[aá][uú]l' OR  text(pap.id) ~* 'r[aá][uú]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]n[aá]r[eé]s' OR  pap.rut ~* 'm[eé]n[aá]r[eé]s' OR  text(pap.id) ~* 'm[eé]n[aá]r[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
