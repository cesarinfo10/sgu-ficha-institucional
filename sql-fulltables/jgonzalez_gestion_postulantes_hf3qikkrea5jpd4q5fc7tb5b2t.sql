COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]n[aá]rd[eé]s' OR  pap.rut ~* 'm[oó]n[aá]rd[eé]s' OR  text(pap.id) ~* 'm[oó]n[aá]rd[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]rt[eé]g[aá]' OR  pap.rut ~* '[oó]rt[eé]g[aá]' OR  text(pap.id) ~* '[oó]rt[eé]g[aá]' ) 
