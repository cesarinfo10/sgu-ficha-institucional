COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[uú][ií]s[eé]ll[eé]' OR  pap.rut ~* 'g[uú][ií]s[eé]ll[eé]' OR  text(pap.id) ~* 'g[uú][ií]s[eé]ll[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]n[oó]str[oó]z[aá]' OR  pap.rut ~* '[ií]n[oó]str[oó]z[aá]' OR  text(pap.id) ~* '[ií]n[oó]str[oó]z[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
