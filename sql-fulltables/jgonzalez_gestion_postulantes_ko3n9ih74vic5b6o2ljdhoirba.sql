COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'br[eé]nd[aá]' OR  pap.rut ~* 'br[eé]nd[aá]' OR  text(pap.id) ~* 'br[eé]nd[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'sc[aá]rl[eé]t' OR  pap.rut ~* 'sc[aá]rl[eé]t' OR  text(pap.id) ~* 'sc[aá]rl[eé]t' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[oó]nz[aá]l[eé]z' OR  pap.rut ~* 'g[oó]nz[aá]l[eé]z' OR  text(pap.id) ~* 'g[oó]nz[aá]l[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]rg[aá]r[aá]' OR  pap.rut ~* 'v[eé]rg[aá]r[aá]' OR  text(pap.id) ~* 'v[eé]rg[aá]r[aá]' ) 
