COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                           ...
                           (SELECT 1 FROM finanzas.contratos WHERE id_pap=pap.id AND estado IN ('R','S','A') AND ano=2023 ORDER BY fecha DESC LIMIT 1) AS desertor,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]tr[ií]c[ií][aá]' OR  pap.rut ~* 'p[aá]tr[ií]c[ií][aá]' OR  text(pap.id) ~* 'p[aá]tr[ií]c[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ng[eé]l[ií]c[aá]' OR  pap.rut ~* '[aá]ng[eé]l[ií]c[aá]' OR  text(pap.id) ~* '[aá]ng[eé]l[ií]c[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]dr[ií]g[uú][eé]z' OR  pap.rut ~* 'r[oó]dr[ií]g[uú][eé]z' OR  text(pap.id) ~* 'r[oó]dr[ií]g[uú][eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]str[aá]d[aá]' OR  pap.rut ~* '[eé]str[aá]d[aá]' OR  text(pap.id) ~* '[eé]str[aá]d[aá]' ) 
