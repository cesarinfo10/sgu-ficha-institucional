COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]c[aá]r[eé]n[aá]' OR  pap.rut ~* 'm[aá]c[aá]r[eé]n[aá]' OR  text(pap.id) ~* 'm[aá]c[aá]r[eé]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[aá][eé]z[aá]' OR  pap.rut ~* 'b[aá][eé]z[aá]' OR  text(pap.id) ~* 'b[aá][eé]z[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
