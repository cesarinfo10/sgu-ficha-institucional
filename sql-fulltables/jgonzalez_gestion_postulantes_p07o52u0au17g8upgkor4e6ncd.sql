COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]nt[oó]n[ií][aá]' OR  pap.rut ~* '[aá]nt[oó]n[ií][aá]' OR  text(pap.id) ~* '[aá]nt[oó]n[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[uú]b[ií][oó]' OR  pap.rut ~* 'r[uú]b[ií][oó]' OR  text(pap.id) ~* 'r[uú]b[ií][oó]' ) 
