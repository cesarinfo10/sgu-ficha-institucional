COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]c[eé]nt[eé]' OR  pap.rut ~* 'v[ií]c[eé]nt[eé]' OR  text(pap.id) ~* 'v[ií]c[eé]nt[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]m[eé]r[oó]' OR  pap.rut ~* 'r[oó]m[eé]r[oó]' OR  text(pap.id) ~* 'r[oó]m[eé]r[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
