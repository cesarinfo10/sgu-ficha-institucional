COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]nd[eé]r[oó]s' OR  pap.rut ~* 'l[aá]nd[eé]r[oó]s' OR  text(pap.id) ~* 'l[aá]nd[eé]r[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
