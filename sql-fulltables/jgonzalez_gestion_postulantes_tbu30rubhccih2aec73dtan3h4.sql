COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'n[aá]t[aá]l[ií][aá]' OR  pap.rut ~* 'n[aá]t[aá]l[ií][aá]' OR  text(pap.id) ~* 'n[aá]t[aá]l[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lc[aá][ií]n[oó]' OR  pap.rut ~* '[aá]lc[aá][ií]n[oó]' OR  text(pap.id) ~* '[aá]lc[aá][ií]n[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
