COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'fr[aá]nc[ií]sc[oó]' OR  pap.rut ~* 'fr[aá]nc[ií]sc[oó]' OR  text(pap.id) ~* 'fr[aá]nc[ií]sc[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]v[ií][eé]r' OR  pap.rut ~* 'j[aá]v[ií][eé]r' OR  text(pap.id) ~* 'j[aá]v[ií][eé]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
