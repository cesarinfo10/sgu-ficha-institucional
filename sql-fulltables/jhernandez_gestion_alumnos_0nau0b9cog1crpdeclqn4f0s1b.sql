COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé]ss[ií]c[aá]' OR  a.rut ~* 'j[eé]ss[ií]c[aá]' OR  lower(a.email) ~* 'j[eé]ss[ií]c[aá]' OR  text(a.id) ~* 'j[eé]ss[ií]c[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]v[ií]l[aá]' OR  a.rut ~* '[aá]v[ií]l[aá]' OR  lower(a.email) ~* '[aá]v[ií]l[aá]' OR  text(a.id) ~* '[aá]v[ií]l[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
