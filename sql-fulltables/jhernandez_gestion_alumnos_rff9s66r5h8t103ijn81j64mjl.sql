COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé][aá]nn[eé]tt[eé]' OR  a.rut ~* 'j[eé][aá]nn[eé]tt[eé]' OR  lower(a.email) ~* 'j[eé][aá]nn[eé]tt[eé]' OR  text(a.id) ~* 'j[eé][aá]nn[eé]tt[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17,153) 
