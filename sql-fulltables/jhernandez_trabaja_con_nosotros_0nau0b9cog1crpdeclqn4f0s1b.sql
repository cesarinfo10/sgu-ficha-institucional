COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'j[oó]s[eé]') OR (lower(pp.apellidos) ~* 'j[oó]s[eé]') OR (lower(pp.rut) ~* 'j[oó]s[eé]') AND (lower(pp.nombres) ~* 'h[eé]rn[aá]nd[eé]z') OR (lower(pp.apellidos) ~* 'h[eé]rn[aá]nd[eé]z') OR (lower(pp.rut) ~* 'h[eé]rn[aá]nd[eé]z')
