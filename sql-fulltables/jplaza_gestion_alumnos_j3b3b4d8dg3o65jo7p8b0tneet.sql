COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'pr[eé]c[ií][aá]d[oó]' OR  a.rut ~* 'pr[eé]c[ií][aá]d[oó]' OR  lower(a.email) ~* 'pr[eé]c[ií][aá]d[oó]' OR  text(a.id) ~* 'pr[eé]c[ií][aá]d[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rn[eé]j[oó]' OR  a.rut ~* 'c[oó]rn[eé]j[oó]' OR  lower(a.email) ~* 'c[oó]rn[eé]j[oó]' OR  text(a.id) ~* 'c[oó]rn[eé]j[oó]' ) 
