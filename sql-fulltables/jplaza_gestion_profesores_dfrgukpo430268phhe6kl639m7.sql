COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'fr[aá]nc[ií]sc[aá]'  OR u.rut ~* 'fr[aá]nc[ií]sc[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* '[oó]rt[eé]g[aá]'  OR u.rut ~* '[oó]rt[eé]g[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
