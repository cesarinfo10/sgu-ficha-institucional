COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]r[oó]n[ií]c[aá]' OR  pap.rut ~* 'v[eé]r[oó]n[ií]c[aá]' OR  text(pap.id) ~* 'v[eé]r[oó]n[ií]c[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]j[eé]d[aá]' OR  pap.rut ~* '[oó]j[eé]d[aá]' OR  text(pap.id) ~* '[oó]j[eé]d[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]nch[eé]z' OR  pap.rut ~* 's[aá]nch[eé]z' OR  text(pap.id) ~* 's[aá]nch[eé]z' ) 
