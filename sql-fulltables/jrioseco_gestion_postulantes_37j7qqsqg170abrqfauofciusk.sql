COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]rn[aá]nd[oó]' OR  pap.rut ~* 'f[eé]rn[aá]nd[oó]' OR  text(pap.id) ~* 'f[eé]rn[aá]nd[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]x[ií]s' OR  pap.rut ~* '[aá]l[eé]x[ií]s' OR  text(pap.id) ~* '[aá]l[eé]x[ií]s' ) 
