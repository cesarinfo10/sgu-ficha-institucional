COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá][oó]l[aá]' OR  pap.rut ~* 'p[aá][oó]l[aá]' OR  text(pap.id) ~* 'p[aá][oó]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]rc[ií][aá]' OR  pap.rut ~* 'g[aá]rc[ií][aá]' OR  text(pap.id) ~* 'g[aá]rc[ií][aá]' ) 
