COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rl[aá]' OR  pap.rut ~* 'c[aá]rl[aá]' OR  text(pap.id) ~* 'c[aá]rl[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]l[aá]t[eé]' OR  pap.rut ~* '[oó]l[aá]t[eé]' OR  text(pap.id) ~* '[oó]l[aá]t[eé]' ) 
