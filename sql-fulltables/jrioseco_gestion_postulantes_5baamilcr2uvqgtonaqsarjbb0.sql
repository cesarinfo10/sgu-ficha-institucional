COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]ndr[aá]' OR  pap.rut ~* 's[aá]ndr[aá]' OR  text(pap.id) ~* 's[aá]ndr[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[ií]v[eé]r[oó]' OR  pap.rut ~* 'r[ií]v[eé]r[oó]' OR  text(pap.id) ~* 'r[ií]v[eé]r[oó]' ) 
