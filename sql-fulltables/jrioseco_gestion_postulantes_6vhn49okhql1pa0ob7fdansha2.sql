COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá][uú]l[ií]n[aá]' OR  pap.rut ~* 'p[aá][uú]l[ií]n[aá]' OR  text(pap.id) ~* 'p[aá][uú]l[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]ld[ií]v[ií][aá]' OR  pap.rut ~* 'v[aá]ld[ií]v[ií][aá]' OR  text(pap.id) ~* 'v[aá]ld[ií]v[ií][aá]' ) 
