COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]ls[aá]' OR  pap.rut ~* '[eé]ls[aá]' OR  text(pap.id) ~* '[eé]ls[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]b[eé]ll[oó]' OR  pap.rut ~* 'c[aá]b[eé]ll[oó]' OR  text(pap.id) ~* 'c[aá]b[eé]ll[oó]' ) 
