COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]v[eé]lyn' OR  pap.rut ~* '[eé]v[eé]lyn' OR  text(pap.id) ~* '[eé]v[eé]lyn' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rm[eé]n' OR  pap.rut ~* 'c[aá]rm[eé]n' OR  text(pap.id) ~* 'c[aá]rm[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
