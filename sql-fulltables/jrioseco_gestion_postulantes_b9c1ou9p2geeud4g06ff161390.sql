COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]rn[aá]nd[oó]' OR  pap.rut ~* 'f[eé]rn[aá]nd[oó]' OR  text(pap.id) ~* 'f[eé]rn[aá]nd[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'br[aá]v[oó]' OR  pap.rut ~* 'br[aá]v[oó]' OR  text(pap.id) ~* 'br[aá]v[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lm[ií]r[aá][ií]s' OR  pap.rut ~* '[aá]lm[ií]r[aá][ií]s' OR  text(pap.id) ~* '[aá]lm[ií]r[aá][ií]s' ) 
