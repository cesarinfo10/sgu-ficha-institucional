COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]v[ií]d' OR  pap.rut ~* 'd[aá]v[ií]d' OR  text(pap.id) ~* 'd[aá]v[ií]d' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]l[aá]t[eé]' OR  pap.rut ~* '[oó]l[aá]t[eé]' OR  text(pap.id) ~* '[oó]l[aá]t[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
