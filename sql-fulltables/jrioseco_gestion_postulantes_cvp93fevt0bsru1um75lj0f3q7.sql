COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[ií][eé]g[oó]' OR  pap.rut ~* 'd[ií][eé]g[oó]' OR  text(pap.id) ~* 'd[ií][eé]g[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]gn[aá]c[ií][oó]' OR  pap.rut ~* '[ií]gn[aá]c[ií][oó]' OR  text(pap.id) ~* '[ií]gn[aá]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]r[eé]ng[uú][eé]r' OR  pap.rut ~* 'b[eé]r[eé]ng[uú][eé]r' OR  text(pap.id) ~* 'b[eé]r[eé]ng[uú][eé]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[uú][aá]z[oó]' OR  pap.rut ~* 's[uú][aá]z[oó]' OR  text(pap.id) ~* 's[uú][aá]z[oó]' ) 
