COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cl[aá][uú]d[ií][aá]' OR  pap.rut ~* 'cl[aá][uú]d[ií][aá]' OR  text(pap.id) ~* 'cl[aá][uú]d[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[ií]q[uú][eé]lm[eé]' OR  pap.rut ~* 'r[ií]q[uú][eé]lm[eé]' OR  text(pap.id) ~* 'r[ií]q[uú][eé]lm[eé]' ) 
