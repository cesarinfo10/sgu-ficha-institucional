COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'vl[aá]tk[oó]' OR  pap.rut ~* 'vl[aá]tk[oó]' OR  text(pap.id) ~* 'vl[aá]tk[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[uú][eé]ll[oó]' OR  pap.rut ~* 'c[uú][eé]ll[oó]' OR  text(pap.id) ~* 'c[uú][eé]ll[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]v[aá]n[ií][aá]' OR  pap.rut ~* '[ií]v[aá]n[ií][aá]' OR  text(pap.id) ~* '[ií]v[aá]n[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]l[eé]n' OR  pap.rut ~* 'b[eé]l[eé]n' OR  text(pap.id) ~* 'b[eé]l[eé]n' ) 
