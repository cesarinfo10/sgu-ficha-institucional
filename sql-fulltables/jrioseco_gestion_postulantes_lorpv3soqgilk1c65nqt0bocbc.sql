COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]b[uú]rt[oó]s' OR  pap.rut ~* '[aá]b[uú]rt[oó]s' OR  text(pap.id) ~* '[aá]b[uú]rt[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá][eé]nz' OR  pap.rut ~* 's[aá][eé]nz' OR  text(pap.id) ~* 's[aá][eé]nz' ) 
