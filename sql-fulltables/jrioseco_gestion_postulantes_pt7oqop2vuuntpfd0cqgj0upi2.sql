COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]s[eé]f[aá]' OR  pap.rut ~* 'j[oó]s[eé]f[aá]' OR  text(pap.id) ~* 'j[oó]s[eé]f[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'fry' OR  pap.rut ~* 'fry' OR  text(pap.id) ~* 'fry' ) 
