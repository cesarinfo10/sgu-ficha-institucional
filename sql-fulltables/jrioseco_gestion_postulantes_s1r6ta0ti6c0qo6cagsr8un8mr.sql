COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[uú]mb[eé]rt[oó]' OR  pap.rut ~* 'h[uú]mb[eé]rt[oó]' OR  text(pap.id) ~* 'h[uú]mb[eé]rt[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]r[aá]l[eé]s' OR  pap.rut ~* 'm[oó]r[aá]l[eé]s' OR  text(pap.id) ~* 'm[oó]r[aá]l[eé]s' ) 
