COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cl[aá][uú]d[ií][aá]' OR  pap.rut ~* 'cl[aá][uú]d[ií][aá]' OR  text(pap.id) ~* 'cl[aá][uú]d[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
