COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé][ií]n[aá]ld[oó]' OR  a.rut ~* 'r[eé][ií]n[aá]ld[oó]' OR  lower(a.email) ~* 'r[eé][ií]n[aá]ld[oó]' OR  text(a.id) ~* 'r[eé][ií]n[aá]ld[oó]' ) 
