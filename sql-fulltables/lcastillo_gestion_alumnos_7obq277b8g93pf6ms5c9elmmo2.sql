COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]r[eé]t[aá]' OR  a.rut ~* '[uú]r[eé]t[aá]' OR  lower(a.email) ~* '[uú]r[eé]t[aá]' OR  text(a.id) ~* '[uú]r[eé]t[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
