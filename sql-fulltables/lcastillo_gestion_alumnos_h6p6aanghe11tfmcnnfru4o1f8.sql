COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][ií]n[eé]n' OR  a.rut ~* 'p[aá][ií]n[eé]n' OR  lower(a.email) ~* 'p[aá][ií]n[eé]n' OR  text(a.id) ~* 'p[aá][ií]n[eé]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
