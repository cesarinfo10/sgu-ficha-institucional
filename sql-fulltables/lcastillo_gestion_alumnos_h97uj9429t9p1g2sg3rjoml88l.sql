COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá][ií]m[eé]' OR  a.rut ~* 'j[aá][ií]m[eé]' OR  lower(a.email) ~* 'j[aá][ií]m[eé]' OR  text(a.id) ~* 'j[aá][ií]m[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]s[oó]r[ií][oó]' OR  a.rut ~* '[oó]s[oó]r[ií][oó]' OR  lower(a.email) ~* '[oó]s[oó]r[ií][oó]' OR  text(a.id) ~* '[oó]s[oó]r[ií][oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
