COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]z[aá]r[eé]n[aá]' OR  a.rut ~* 'n[aá]z[aá]r[eé]n[aá]' OR  lower(a.email) ~* 'n[aá]z[aá]r[eé]n[aá]' OR  text(a.id) ~* 'n[aá]z[aá]r[eé]n[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
