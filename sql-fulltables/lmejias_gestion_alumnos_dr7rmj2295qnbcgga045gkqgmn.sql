COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé][aá]tr[ií]z' OR  a.rut ~* 'b[eé][aá]tr[ií]z' OR  lower(a.email) ~* 'b[eé][aá]tr[ií]z' OR  text(a.id) ~* 'b[eé][aá]tr[ií]z' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
