COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]c[aá]rt[eé]' OR  a.rut ~* '[ií]c[aá]rt[eé]' OR  lower(a.email) ~* '[ií]c[aá]rt[eé]' OR  text(a.id) ~* '[ií]c[aá]rt[eé]' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
