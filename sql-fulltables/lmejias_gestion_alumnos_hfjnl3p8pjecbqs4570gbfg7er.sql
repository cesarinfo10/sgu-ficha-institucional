COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[uú][eé]v[aá]s' OR  a.rut ~* 'c[uú][eé]v[aá]s' OR  lower(a.email) ~* 'c[uú][eé]v[aá]s' OR  text(a.id) ~* 'c[uú][eé]v[aá]s' )  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
