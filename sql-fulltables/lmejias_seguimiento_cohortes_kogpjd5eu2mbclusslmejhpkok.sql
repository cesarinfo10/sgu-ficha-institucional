COPY (SELECT *,CASE WHEN tasa_riesgo BETWEEN 75 and 100 THEN 'Alto'
                ...
                LEFT JOIN (SELECT rut_alumno,max(nota) AS nota FROM sinergia.resp_pruebas_diag WHERE ano=2023 AND semestre=1 GROUP BY rut_alumno) AS niv ON niv.rut_alumno=a.rut
                LEFT JOIN matriculas        AS mat ON (mat.id_alumno=a.id AND mat.semestre=1 AND mat.ano=2023)
                WHERE true  AND (a.cohorte = '2023') AND (a.semestre_cohorte = 1) AND (a.estado = '1') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=1))  AND a.carrera_actual IN (94,97,104,110,98,105,102,91,15,103,126,4) 
