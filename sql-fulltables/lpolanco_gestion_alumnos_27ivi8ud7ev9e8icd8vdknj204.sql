COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[oó]' OR  a.rut ~* 'm[aá]rc[oó]' OR  lower(a.email) ~* 'm[aá]rc[oó]' OR  text(a.id) ~* 'm[aá]rc[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[uú]ñ[oó]z' OR  a.rut ~* 'm[uú]ñ[oó]z' OR  lower(a.email) ~* 'm[uú]ñ[oó]z' OR  text(a.id) ~* 'm[uú]ñ[oó]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]t[aá]m[aá]l[eé]s' OR  a.rut ~* 'r[eé]t[aá]m[aá]l[eé]s' OR  lower(a.email) ~* 'r[eé]t[aá]m[aá]l[eé]s' OR  text(a.id) ~* 'r[eé]t[aá]m[aá]l[eé]s' ) 
