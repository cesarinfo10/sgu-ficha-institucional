COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]m[eé]ly' OR  a.rut ~* '[aá]m[eé]ly' OR  lower(a.email) ~* '[aá]m[eé]ly' OR  text(a.id) ~* '[aá]m[eé]ly' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]n[aá]r[eé]s' OR  a.rut ~* 'l[ií]n[aá]r[eé]s' OR  lower(a.email) ~* 'l[ií]n[aá]r[eé]s' OR  text(a.id) ~* 'l[ií]n[aá]r[eé]s' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
