COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND u.id IN (SELECT vc.id_profesor 
						  FROM vista_cursos  AS vc
						  LEFT JOIN carreras AS car ON car.id=vc.id_carrera 
						  WHERE true  AND vc.ano=2022  AND vc.semestre=2  AND vc.id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125)  AND car.regimen='POST-GD' )
