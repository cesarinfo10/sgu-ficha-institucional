COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[aá]v[ií][eé]r[aá]'  OR u.rut ~* 'j[aá]v[ií][eé]r[aá]')  AND (lower(u.nombre||' '||u.apellido) ~* 'c[aá]rr[aá]sc[oó]'  OR u.rut ~* 'c[aá]rr[aá]sc[oó]') 
