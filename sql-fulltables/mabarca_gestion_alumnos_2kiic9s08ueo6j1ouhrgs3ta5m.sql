COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[ií]n' OR  a.rut ~* 'k[aá]r[ií]n' OR  lower(a.email) ~* 'k[aá]r[ií]n' OR  text(a.id) ~* 'k[aá]r[ií]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[aá]rf[aá]n' OR  a.rut ~* 'f[aá]rf[aá]n' OR  lower(a.email) ~* 'f[aá]rf[aá]n' OR  text(a.id) ~* 'f[aá]rf[aá]n' ) 
