COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]r[ií][aá]' OR  a.rut ~* 's[ií]r[ií][aá]' OR  lower(a.email) ~* 's[ií]r[ií][aá]' OR  text(a.id) ~* 's[ií]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[eé]j[oó]' OR  a.rut ~* 't[eé]j[oó]' OR  lower(a.email) ~* 't[eé]j[oó]' OR  text(a.id) ~* 't[eé]j[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  a.rut ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  lower(a.email) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR  text(a.id) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' ) 
