COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'w[aá]lt[eé]r' OR  a.rut ~* 'w[aá]lt[eé]r' OR  lower(a.email) ~* 'w[aá]lt[eé]r' OR  text(a.id) ~* 'w[aá]lt[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]rr[ií][oó]l[aá]' OR  a.rut ~* '[uú]rr[ií][oó]l[aá]' OR  lower(a.email) ~* '[uú]rr[ií][oó]l[aá]' OR  text(a.id) ~* '[uú]rr[ií][oó]l[aá]' ) 
