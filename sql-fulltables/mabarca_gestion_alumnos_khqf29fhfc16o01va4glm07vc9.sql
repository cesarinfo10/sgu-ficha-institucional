COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'cl[eé]m[eé]nt[ií]n[aá]' OR  a.rut ~* 'cl[eé]m[eé]nt[ií]n[aá]' OR  lower(a.email) ~* 'cl[eé]m[eé]nt[ií]n[aá]' OR  text(a.id) ~* 'cl[eé]m[eé]nt[ií]n[aá]' ) 
