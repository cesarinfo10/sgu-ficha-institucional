COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]g[uú][eé]l[eé]' OR  a.rut ~* 'm[ií]g[uú][eé]l[eé]' OR  lower(a.email) ~* 'm[ií]g[uú][eé]l[eé]' OR  text(a.id) ~* 'm[ií]g[uú][eé]l[eé]' ) 
