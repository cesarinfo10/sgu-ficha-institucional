COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'gr[aá]c[ií][eé]l[aá]' OR  a.rut ~* 'gr[aá]c[ií][eé]l[aá]' OR  lower(a.email) ~* 'gr[aá]c[ií][eé]l[aá]' OR  text(a.id) ~* 'gr[aá]c[ií][eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lf[eé]ss[ií]' OR  a.rut ~* '[aá]lf[eé]ss[ií]' OR  lower(a.email) ~* '[aá]lf[eé]ss[ií]' OR  text(a.id) ~* '[aá]lf[eé]ss[ií]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148,161,160,163) 
