COPY (SELECT c.id,to_char(c.fecha,'DD-tmMon-YYYY') AS fecha,c.estado,
                     ...
                     WHERE (lower(va.nombre) ~* 'c[aá]rc[aá]m[oó]' OR  a.rut ~* 'c[aá]rc[aá]m[oó]' OR  text(c.id) ~* 'c[aá]rc[aá]m[oó]' ) AND (lower(va.nombre) ~* 's[aá]nch[eé]z' OR  a.rut ~* 's[aá]nch[eé]z' OR  text(c.id) ~* 's[aá]nch[eé]z' ) 
