COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]c[oó]l[eé]' OR  a.rut ~* 'n[ií]c[oó]l[eé]' OR  lower(a.email) ~* 'n[ií]c[oó]l[eé]' OR  text(a.id) ~* 'n[ií]c[oó]l[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]v[ií]l[aá]' OR  a.rut ~* '[aá]v[ií]l[aá]' OR  lower(a.email) ~* '[aá]v[ií]l[aá]' OR  text(a.id) ~* '[aá]v[ií]l[aá]' ) 
