COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]ls[aá]' OR  a.rut ~* '[eé]ls[aá]' OR  lower(a.email) ~* '[eé]ls[aá]' OR  text(a.id) ~* '[eé]ls[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]ll[aá]rd[oó]' OR  a.rut ~* 'g[aá]ll[aá]rd[oó]' OR  lower(a.email) ~* 'g[aá]ll[aá]rd[oó]' OR  text(a.id) ~* 'g[aá]ll[aá]rd[oó]' ) 
