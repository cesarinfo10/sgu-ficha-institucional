COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'j[uú]v[eé]n[aá]l'  OR u.rut ~* 'j[uú]v[eé]n[aá]l')  AND (lower(u.nombre||' '||u.apellido) ~* 'h[eé]rn[aá]nd[eé]z'  OR u.rut ~* 'h[eé]rn[aá]nd[eé]z')  AND (lower(u.nombre||' '||u.apellido) ~* ''  OR u.rut ~* '') 
