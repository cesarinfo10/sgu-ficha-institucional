COPY (SELECT act.*,(SELECT char_comma_sum(tipo_publico||': '||coalesce((coalesce(cant_personas,0)+coalesce(cant_personas_virtuales,0))::text,'*')) 
			...
			WHERE true  AND (ano = 2024) AND (id_tipo IN (SELECT id FROM vcm.tipos_act WHERE dimension='Extensión')) AND (id_tipo = 2) AND (44 IN (id_unidad1,id_unidad2,id_unidad3)) AND ('Centro/Instituto' IN (u1.tipo,u2.tipo,u3.tipo)) AND (id_responsable = 5	) 
