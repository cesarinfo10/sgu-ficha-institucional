COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[eé]n' OR  a.rut ~* 'k[aá]r[eé]n' OR  lower(a.email) ~* 'k[aá]r[eé]n' OR  text(a.id) ~* 'k[aá]r[eé]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]sc[oó]b[aá]r' OR  a.rut ~* '[eé]sc[oó]b[aá]r' OR  lower(a.email) ~* '[eé]sc[oó]b[aá]r' OR  text(a.id) ~* '[eé]sc[oó]b[aá]r' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67,164) 
