COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '26731627-9' OR  a.rut ~* '26731627-9' OR  lower(a.email) ~* '26731627-9' OR  text(a.id) ~* '26731627-9' )  AND a.carrera_actual IN (101,62,26,88,18,3,159,156,36,67,164) 
