COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[eé]s[aá]r' OR  pap.rut ~* 'c[eé]s[aá]r' OR  text(pap.id) ~* 'c[eé]s[aá]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]rn[aá]nd[eé]z' OR  pap.rut ~* 'f[eé]rn[aá]nd[eé]z' OR  text(pap.id) ~* 'f[eé]rn[aá]nd[eé]z' ) 
