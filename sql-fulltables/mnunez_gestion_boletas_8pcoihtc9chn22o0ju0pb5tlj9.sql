COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '13166902-k' OR  pap.rut ~* '13166902-k' OR lower(a.nombres||' '||a.apellidos) ~* '13166902-k' OR lower(a3.nombres||' '||a3.apellidos) ~* '13166902-k' OR  a.rut ~* '13166902-k' OR a2.rut ~* '13166902-k' OR a3.rut ~* '13166902-k'  OR text(nro_boleta) = '13166902-k' )  
                ORDER BY coalesce(p.nro_boleta,p.nro_factura) DESC ) to stdout WITH CSV HEADER