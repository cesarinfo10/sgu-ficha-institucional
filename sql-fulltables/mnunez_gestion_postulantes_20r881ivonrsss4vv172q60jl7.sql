COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]l[ií]z' OR  pap.rut ~* 'v[eé]l[ií]z' OR  text(pap.id) ~* 'v[eé]l[ií]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]r[eé]d[eé]s' OR  pap.rut ~* 'p[aá]r[eé]d[eé]s' OR  text(pap.id) ~* 'p[aá]r[eé]d[eé]s' ) 
