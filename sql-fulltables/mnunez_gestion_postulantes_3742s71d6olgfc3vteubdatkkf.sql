COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[uú]ñ[oó]z' OR  pap.rut ~* 'm[uú]ñ[oó]z' OR  text(pap.id) ~* 'm[uú]ñ[oó]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rr[aá]sc[oó]' OR  pap.rut ~* 'c[aá]rr[aá]sc[oó]' OR  text(pap.id) ~* 'c[aá]rr[aá]sc[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[oó]r[eé]n[aá]' OR  pap.rut ~* 'l[oó]r[eé]n[aá]' OR  text(pap.id) ~* 'l[oó]r[eé]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ndr[eé][aá]' OR  pap.rut ~* '[aá]ndr[eé][aá]' OR  text(pap.id) ~* '[aá]ndr[eé][aá]' ) 
