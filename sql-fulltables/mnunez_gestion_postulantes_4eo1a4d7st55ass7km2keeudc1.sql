COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'z[eé]p[eé]d[aá]' OR  pap.rut ~* 'z[eé]p[eé]d[aá]' OR  text(pap.id) ~* 'z[eé]p[eé]d[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]bl[eé]d[oó]' OR  pap.rut ~* 'r[oó]bl[eé]d[oó]' OR  text(pap.id) ~* 'r[oó]bl[eé]d[oó]' ) 
