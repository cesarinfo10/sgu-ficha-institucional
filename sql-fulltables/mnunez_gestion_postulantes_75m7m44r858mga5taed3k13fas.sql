COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'n[aá]y[aá]d[eé]' OR  pap.rut ~* 'n[aá]y[aá]d[eé]' OR  text(pap.id) ~* 'n[aá]y[aá]d[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'h[ií]d[aá]lg[oó]' OR  pap.rut ~* 'h[ií]d[aá]lg[oó]' OR  text(pap.id) ~* 'h[ií]d[aá]lg[oó]' ) 
