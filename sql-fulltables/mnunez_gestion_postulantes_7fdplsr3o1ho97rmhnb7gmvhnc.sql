COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[eé]s[uú]s' OR  pap.rut ~* 'j[eé]s[uú]s' OR  text(pap.id) ~* 'j[eé]s[uú]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lv[aá]r[aá]d[oó]' OR  pap.rut ~* '[aá]lv[aá]r[aá]d[oó]' OR  text(pap.id) ~* '[aá]lv[aá]r[aá]d[oó]' ) 
