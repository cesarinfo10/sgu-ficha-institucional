COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[ií]r[eé]y[aá]' OR  pap.rut ~* 'm[ií]r[eé]y[aá]' OR  text(pap.id) ~* 'm[ií]r[eé]y[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  pap.rut ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  text(pap.id) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' ) 
