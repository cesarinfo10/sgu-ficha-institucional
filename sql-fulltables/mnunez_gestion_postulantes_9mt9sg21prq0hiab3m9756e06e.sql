COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]l[ií]p[eé]' OR  pap.rut ~* 'f[eé]l[ií]p[eé]' OR  text(pap.id) ~* 'f[eé]l[ií]p[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nt[eé]s' OR  pap.rut ~* 'f[uú][eé]nt[eé]s' OR  text(pap.id) ~* 'f[uú][eé]nt[eé]s' ) 
