COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[uú][aá]n' OR  pap.rut ~* 'j[uú][aá]n' OR  text(pap.id) ~* 'j[uú][aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]n[uú][eé]l' OR  pap.rut ~* 'm[aá]n[uú][eé]l' OR  text(pap.id) ~* 'm[aá]n[uú][eé]l' ) 
