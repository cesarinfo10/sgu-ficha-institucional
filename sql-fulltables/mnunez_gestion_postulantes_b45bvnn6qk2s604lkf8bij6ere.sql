COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]s[eé]' OR  pap.rut ~* 'j[oó]s[eé]' OR  text(pap.id) ~* 'j[oó]s[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]rr[eé]s' OR  pap.rut ~* 't[oó]rr[eé]s' OR  text(pap.id) ~* 't[oó]rr[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[ií][oó]s' OR  pap.rut ~* 'r[ií][oó]s' OR  text(pap.id) ~* 'r[ií][oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
