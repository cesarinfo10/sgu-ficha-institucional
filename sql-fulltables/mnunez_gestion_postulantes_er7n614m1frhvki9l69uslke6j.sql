COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]g[uú][ií]l[aá]r' OR  pap.rut ~* '[aá]g[uú][ií]l[aá]r' OR  text(pap.id) ~* '[aá]g[uú][ií]l[aá]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'tr[uú]j[ií]ll[oó]' OR  pap.rut ~* 'tr[uú]j[ií]ll[oó]' OR  text(pap.id) ~* 'tr[uú]j[ií]ll[oó]' ) 
