COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá][ií]m[eé]' OR  pap.rut ~* 'j[aá][ií]m[eé]' OR  text(pap.id) ~* 'j[aá][ií]m[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]rr[aá][ií]n' OR  pap.rut ~* 'l[aá]rr[aá][ií]n' OR  text(pap.id) ~* 'l[aá]rr[aá][ií]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
