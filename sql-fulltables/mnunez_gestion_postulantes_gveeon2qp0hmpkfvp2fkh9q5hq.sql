COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]b[aá]st[ií][aá]n' OR  pap.rut ~* 's[eé]b[aá]st[ií][aá]n' OR  text(pap.id) ~* 's[eé]b[aá]st[ií][aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[ií]r[aá]' OR  pap.rut ~* 'l[ií]r[aá]' OR  text(pap.id) ~* 'l[ií]r[aá]' ) 
