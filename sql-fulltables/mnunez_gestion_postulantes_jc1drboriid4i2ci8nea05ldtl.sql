COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]bl[eé]s' OR  pap.rut ~* 'r[oó]bl[eé]s' OR  text(pap.id) ~* 'r[oó]bl[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]ll[aá]l[oó]b[oó]s' OR  pap.rut ~* 'v[ií]ll[aá]l[oó]b[oó]s' OR  text(pap.id) ~* 'v[ií]ll[aá]l[oó]b[oó]s' ) 
