COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]r[ií]ck' OR  pap.rut ~* '[eé]r[ií]ck' OR  text(pap.id) ~* '[eé]r[ií]ck' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé][aá]l' OR  pap.rut ~* 'l[eé][aá]l' OR  text(pap.id) ~* 'l[eé][aá]l' ) 
