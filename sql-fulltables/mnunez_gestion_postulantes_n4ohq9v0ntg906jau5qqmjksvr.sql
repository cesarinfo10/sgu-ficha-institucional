COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[ií]rt[aá]' OR  pap.rut ~* 'm[ií]rt[aá]' OR  text(pap.id) ~* 'm[ií]rt[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]y[aá]' OR  pap.rut ~* '[aá]r[aá]y[aá]' OR  text(pap.id) ~* '[aá]r[aá]y[aá]' ) 
