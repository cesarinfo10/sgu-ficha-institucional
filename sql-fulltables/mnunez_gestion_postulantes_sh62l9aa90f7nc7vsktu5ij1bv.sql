COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'n[ií]c[oó]l[eé]' OR  pap.rut ~* 'n[ií]c[oó]l[eé]' OR  text(pap.id) ~* 'n[ií]c[oó]l[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]c[aá]r[eé]zz[aá]' OR  pap.rut ~* 'v[aá]c[aá]r[eé]zz[aá]' OR  text(pap.id) ~* 'v[aá]c[aá]r[eé]zz[aá]' ) 
