COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé][oó]n[aá]rd[oó]' OR  pap.rut ~* 'l[eé][oó]n[aá]rd[oó]' OR  text(pap.id) ~* 'l[eé][oó]n[aá]rd[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]r[oó]' OR  pap.rut ~* 't[oó]r[oó]' OR  text(pap.id) ~* 't[oó]r[oó]' ) 
