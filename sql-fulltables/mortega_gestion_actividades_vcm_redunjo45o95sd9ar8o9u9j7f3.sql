COPY (SELECT act.*,(SELECT char_comma_sum(tipo_publico||': '||coalesce((coalesce(cant_personas,0)+coalesce(cant_personas_virtuales,0))::text,'*')) 
			...
			WHERE true   AND (lower(act.nombre) ~* 'j[uú]st[ií]c[ií][aá]' OR  text(act.id) ~* 'j[uú]st[ií]c[ií][aá]' ) AND (lower(act.nombre) ~* 'tr[ií]b[uú]t[aá]r[ií][aá]' OR  text(act.id) ~* 'tr[ií]b[uú]t[aá]r[ií][aá]' ) AND (lower(act.nombre) ~* '' OR  text(act.id) ~* '' ) 
