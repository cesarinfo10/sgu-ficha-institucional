COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]j[aá]ndr[aá]' OR  a.rut ~* '[aá]l[eé]j[aá]ndr[aá]' OR  lower(a.email) ~* '[aá]l[eé]j[aá]ndr[aá]' OR  text(a.id) ~* '[aá]l[eé]j[aá]ndr[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[eé]gr[eé]t[eé]' OR  a.rut ~* 'n[eé]gr[eé]t[eé]' OR  lower(a.email) ~* 'n[eé]gr[eé]t[eé]' OR  text(a.id) ~* 'n[eé]gr[eé]t[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
