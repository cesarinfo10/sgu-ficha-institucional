COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT count(id_alumno) AS cant_insc FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2022) AND id_alumno=a.id)>0 THEN (()::real/(SELECT count(id_alumno) AS cant_insc FROM cargas_academicas WHERE id_curso IN (SELECT id FROM cursos WHERE ano=2022) AND id_alumno=a.id::real*100)::numeric(4,1) ELSE 0 END AS porcentaje_avance,
