COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]n[uú][ií]r[ií]' OR  a.rut ~* 's[ií]n[uú][ií]r[ií]' OR  lower(a.email) ~* 's[ií]n[uú][ií]r[ií]' OR  text(a.id) ~* 's[ií]n[uú][ií]r[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
