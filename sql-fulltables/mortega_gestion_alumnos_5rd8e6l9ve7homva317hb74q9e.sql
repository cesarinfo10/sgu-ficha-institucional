COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'gr[ií]ll[eé]' OR  a.rut ~* 'gr[ií]ll[eé]' OR  lower(a.email) ~* 'gr[ií]ll[eé]' OR  text(a.id) ~* 'gr[ií]ll[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
