COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]ld[ií][aá]s' OR  a.rut ~* 's[aá]ld[ií][aá]s' OR  lower(a.email) ~* 's[aá]ld[ií][aá]s' OR  text(a.id) ~* 's[aá]ld[ií][aá]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
