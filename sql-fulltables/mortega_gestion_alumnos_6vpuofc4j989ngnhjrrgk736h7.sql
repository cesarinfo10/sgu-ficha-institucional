COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]lb[oó][aá]' OR  a.rut ~* 'b[aá]lb[oó][aá]' OR  lower(a.email) ~* 'b[aá]lb[oó][aá]' OR  text(a.id) ~* 'b[aá]lb[oó][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
