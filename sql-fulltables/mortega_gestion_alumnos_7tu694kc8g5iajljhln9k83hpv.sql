COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'schl[oó][eé]' OR  a.rut ~* 'schl[oó][eé]' OR  lower(a.email) ~* 'schl[oó][eé]' OR  text(a.id) ~* 'schl[oó][eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l[ií]l' OR  a.rut ~* '[aá]ng[eé]l[ií]l' OR  lower(a.email) ~* '[aá]ng[eé]l[ií]l' OR  text(a.id) ~* '[aá]ng[eé]l[ií]l' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
