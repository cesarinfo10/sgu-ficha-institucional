COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]br[aá]h[aá]m' OR  a.rut ~* '[aá]br[aá]h[aá]m' OR  lower(a.email) ~* '[aá]br[aá]h[aá]m' OR  text(a.id) ~* '[aá]br[aá]h[aá]m' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]ld[eé]r[oó]n' OR  a.rut ~* 'c[aá]ld[eé]r[oó]n' OR  lower(a.email) ~* 'c[aá]ld[eé]r[oó]n' OR  text(a.id) ~* 'c[aá]ld[eé]r[oó]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
