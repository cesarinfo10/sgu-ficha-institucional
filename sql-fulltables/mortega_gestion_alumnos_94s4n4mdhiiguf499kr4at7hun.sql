COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]rll[aá]' OR  a.rut ~* 'k[aá]rll[aá]' OR  lower(a.email) ~* 'k[aá]rll[aá]' OR  text(a.id) ~* 'k[aá]rll[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
