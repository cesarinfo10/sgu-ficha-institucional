COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rv[aá]j[aá]l' OR  a.rut ~* 'c[aá]rv[aá]j[aá]l' OR  lower(a.email) ~* 'c[aá]rv[aá]j[aá]l' OR  text(a.id) ~* 'c[aá]rv[aá]j[aá]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
