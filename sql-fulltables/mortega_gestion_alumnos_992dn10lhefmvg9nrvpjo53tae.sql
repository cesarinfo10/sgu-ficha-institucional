COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[aá]r[eé]n[aá]s' OR  a.rut ~* 'l[aá]r[eé]n[aá]s' OR  lower(a.email) ~* 'l[aá]r[eé]n[aá]s' OR  text(a.id) ~* 'l[aá]r[eé]n[aá]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
