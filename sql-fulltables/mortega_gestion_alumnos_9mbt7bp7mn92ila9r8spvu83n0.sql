COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'sch[aá]ff' OR  a.rut ~* 'sch[aá]ff' OR  lower(a.email) ~* 'sch[aá]ff' OR  text(a.id) ~* 'sch[aá]ff' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
