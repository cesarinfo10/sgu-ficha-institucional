COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]c[oó]l[eé]t' OR  a.rut ~* 'n[ií]c[oó]l[eé]t' OR  lower(a.email) ~* 'n[ií]c[oó]l[eé]t' OR  text(a.id) ~* 'n[ií]c[oó]l[eé]t' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]l[ií]n[aá]' OR  a.rut ~* 'm[oó]l[ií]n[aá]' OR  lower(a.email) ~* 'm[oó]l[ií]n[aá]' OR  text(a.id) ~* 'm[oó]l[ií]n[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
