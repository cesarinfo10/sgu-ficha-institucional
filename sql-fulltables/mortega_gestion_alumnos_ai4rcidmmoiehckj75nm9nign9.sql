COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[oó]rr[oó]' OR  a.rut ~* 'p[oó]rr[oó]' OR  lower(a.email) ~* 'p[oó]rr[oó]' OR  text(a.id) ~* 'p[oó]rr[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
