COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]gl[aá]s' OR  a.rut ~* 'r[eé]gl[aá]s' OR  lower(a.email) ~* 'r[eé]gl[aá]s' OR  text(a.id) ~* 'r[eé]gl[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]m[uú]n[eé]s' OR  a.rut ~* 'c[oó]m[uú]n[eé]s' OR  lower(a.email) ~* 'c[oó]m[uú]n[eé]s' OR  text(a.id) ~* 'c[oó]m[uú]n[eé]s' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
