COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[aá]b[aá]l[oó]n' OR  a.rut ~* '[aá]n[aá]b[aá]l[oó]n' OR  lower(a.email) ~* '[aá]n[aá]b[aá]l[oó]n' OR  text(a.id) ~* '[aá]n[aá]b[aá]l[oó]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
