COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]n' OR  a.rut ~* 's[aá]n' OR  lower(a.email) ~* 's[aá]n' OR  text(a.id) ~* 's[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rt[ií]' OR  a.rut ~* 'm[aá]rt[ií]' OR  lower(a.email) ~* 'm[aá]rt[ií]' OR  text(a.id) ~* 'm[aá]rt[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
