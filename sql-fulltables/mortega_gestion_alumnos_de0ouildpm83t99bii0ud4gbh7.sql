COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]nny' OR  a.rut ~* 't[oó]nny' OR  lower(a.email) ~* 't[oó]nny' OR  text(a.id) ~* 't[oó]nny' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'br[ií]t[oó]' OR  a.rut ~* 'br[ií]t[oó]' OR  lower(a.email) ~* 'br[ií]t[oó]' OR  text(a.id) ~* 'br[ií]t[oó]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
