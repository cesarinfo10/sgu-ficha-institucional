COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[aá]m[aá]r[aá]' OR  a.rut ~* 't[aá]m[aá]r[aá]' OR  lower(a.email) ~* 't[aá]m[aá]r[aá]' OR  text(a.id) ~* 't[aá]m[aá]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
