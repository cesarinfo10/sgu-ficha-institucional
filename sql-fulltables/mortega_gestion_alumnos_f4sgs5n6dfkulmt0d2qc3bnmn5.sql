COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'f[eé]rn[aá]nd[oó]' OR  a.rut ~* 'f[eé]rn[aá]nd[oó]' OR  lower(a.email) ~* 'f[eé]rn[aá]nd[oó]' OR  text(a.id) ~* 'f[eé]rn[aá]nd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ll[eé]g[aá]s' OR  a.rut ~* 'v[ií]ll[eé]g[aá]s' OR  lower(a.email) ~* 'v[ií]ll[eé]g[aá]s' OR  text(a.id) ~* 'v[ií]ll[eé]g[aá]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
