COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[aá]ll[eé]r' OR  a.rut ~* 't[aá]ll[eé]r' OR  lower(a.email) ~* 't[aá]ll[eé]r' OR  text(a.id) ~* 't[aá]ll[eé]r' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
