COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií]b[eé]l' OR  a.rut ~* 'm[aá]r[ií]b[eé]l' OR  lower(a.email) ~* 'm[aá]r[ií]b[eé]l' OR  text(a.id) ~* 'm[aá]r[ií]b[eé]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
