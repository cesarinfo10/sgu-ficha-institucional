COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé]n[ií]ss[eé]' OR  a.rut ~* 'd[eé]n[ií]ss[eé]' OR  lower(a.email) ~* 'd[eé]n[ií]ss[eé]' OR  text(a.id) ~* 'd[eé]n[ií]ss[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
