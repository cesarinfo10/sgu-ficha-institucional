COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]sn[aá]' OR  a.rut ~* 'y[aá]sn[aá]' OR  lower(a.email) ~* 'y[aá]sn[aá]' OR  text(a.id) ~* 'y[aá]sn[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rr[aá]' OR  a.rut ~* 'b[aá]rr[aá]' OR  lower(a.email) ~* 'b[aá]rr[aá]' OR  text(a.id) ~* 'b[aá]rr[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
