COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l' OR  a.rut ~* '[aá]ng[eé]l' OR  lower(a.email) ~* '[aá]ng[eé]l' OR  text(a.id) ~* '[aá]ng[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]ss[oó]' OR  a.rut ~* 's[aá]ss[oó]' OR  lower(a.email) ~* 's[aá]ss[oó]' OR  text(a.id) ~* 's[aá]ss[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
