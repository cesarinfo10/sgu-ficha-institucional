COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[oó]m[ií]ng[oó]' OR  a.rut ~* 'd[oó]m[ií]ng[oó]' OR  lower(a.email) ~* 'd[oó]m[ií]ng[oó]' OR  text(a.id) ~* 'd[oó]m[ií]ng[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]rr[eé]s' OR  a.rut ~* 't[oó]rr[eé]s' OR  lower(a.email) ~* 't[oó]rr[eé]s' OR  text(a.id) ~* 't[oó]rr[eé]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
