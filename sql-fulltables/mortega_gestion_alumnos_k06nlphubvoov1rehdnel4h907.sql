COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]v[eé]ly' OR  a.rut ~* '[eé]v[eé]ly' OR  lower(a.email) ~* '[eé]v[eé]ly' OR  text(a.id) ~* '[eé]v[eé]ly' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]br[eé]r[aá]' OR  a.rut ~* 'c[aá]br[eé]r[aá]' OR  lower(a.email) ~* 'c[aá]br[eé]r[aá]' OR  text(a.id) ~* 'c[aá]br[eé]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
