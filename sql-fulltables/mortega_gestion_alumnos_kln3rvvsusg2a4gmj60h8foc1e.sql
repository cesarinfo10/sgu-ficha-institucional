COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]rl[aá]' OR  a.rut ~* 'p[eé]rl[aá]' OR  lower(a.email) ~* 'p[eé]rl[aá]' OR  text(a.id) ~* 'p[eé]rl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[uú]rg[oó]s' OR  a.rut ~* 'b[uú]rg[oó]s' OR  lower(a.email) ~* 'b[uú]rg[oó]s' OR  text(a.id) ~* 'b[uú]rg[oó]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
