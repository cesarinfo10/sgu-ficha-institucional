COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]b[aá]st[ií][aá]n' OR  a.rut ~* 's[eé]b[aá]st[ií][aá]n' OR  lower(a.email) ~* 's[eé]b[aá]st[ií][aá]n' OR  text(a.id) ~* 's[eé]b[aá]st[ií][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]v[eé]n[aá]' OR  a.rut ~* '[aá]r[aá]v[eé]n[aá]' OR  lower(a.email) ~* '[aá]r[aá]v[eé]n[aá]' OR  text(a.id) ~* '[aá]r[aá]v[eé]n[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
