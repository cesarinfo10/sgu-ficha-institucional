COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]v[aá]n' OR  a.rut ~* '[ií]v[aá]n' OR  lower(a.email) ~* '[ií]v[aá]n' OR  text(a.id) ~* '[ií]v[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rr[ií]ll[oó]' OR  a.rut ~* 'c[aá]rr[ií]ll[oó]' OR  lower(a.email) ~* 'c[aá]rr[ií]ll[oó]' OR  text(a.id) ~* 'c[aá]rr[ií]ll[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
