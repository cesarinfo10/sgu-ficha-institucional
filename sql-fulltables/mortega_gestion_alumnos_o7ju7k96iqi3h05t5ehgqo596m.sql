COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá]v[ií][eé]r' OR  a.rut ~* 'j[aá]v[ií][eé]r' OR  lower(a.email) ~* 'j[aá]v[ií][eé]r' OR  text(a.id) ~* 'j[aá]v[ií][eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  a.rut ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  lower(a.email) ~* 'h[oó]rm[aá]z[aá]b[aá]l' OR  text(a.id) ~* 'h[oó]rm[aá]z[aá]b[aá]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
