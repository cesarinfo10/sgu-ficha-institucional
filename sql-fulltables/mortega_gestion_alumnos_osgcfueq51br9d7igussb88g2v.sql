COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[eé][oó]f[ií]l[aá]' OR  a.rut ~* 't[eé][oó]f[ií]l[aá]' OR  lower(a.email) ~* 't[eé][oó]f[ií]l[aá]' OR  text(a.id) ~* 't[eé][oó]f[ií]l[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
