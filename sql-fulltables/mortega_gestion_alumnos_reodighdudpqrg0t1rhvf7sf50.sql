COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá]n[ií]c[eé]' OR  a.rut ~* 'j[aá]n[ií]c[eé]' OR  lower(a.email) ~* 'j[aá]n[ií]c[eé]' OR  text(a.id) ~* 'j[aá]n[ií]c[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
