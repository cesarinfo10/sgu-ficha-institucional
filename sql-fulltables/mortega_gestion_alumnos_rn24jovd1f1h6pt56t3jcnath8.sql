COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'ñ[oó]z' OR  a.rut ~* 'ñ[oó]z' OR  lower(a.email) ~* 'ñ[oó]z' OR  text(a.id) ~* 'ñ[oó]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'cr[ií]st[ií]' OR  a.rut ~* 'cr[ií]st[ií]' OR  lower(a.email) ~* 'cr[ií]st[ií]' OR  text(a.id) ~* 'cr[ií]st[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
