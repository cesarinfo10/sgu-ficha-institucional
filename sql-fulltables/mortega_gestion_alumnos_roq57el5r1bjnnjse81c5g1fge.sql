COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]l[eé]d[aá]d' OR  a.rut ~* 's[oó]l[eé]d[aá]d' OR  lower(a.email) ~* 's[oó]l[eé]d[aá]d' OR  text(a.id) ~* 's[oó]l[eé]d[aá]d' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
