COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]rn[aá]' OR  a.rut ~* 'l[oó]rn[aá]' OR  lower(a.email) ~* 'l[oó]rn[aá]' OR  text(a.id) ~* 'l[oó]rn[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
