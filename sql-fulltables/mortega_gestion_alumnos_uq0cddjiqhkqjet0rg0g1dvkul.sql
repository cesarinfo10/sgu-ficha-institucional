COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[eé]l[ií]t[oó]s' OR  a.rut ~* 'd[eé]l[ií]t[oó]s' OR  lower(a.email) ~* 'd[eé]l[ií]t[oó]s' OR  text(a.id) ~* 'd[eé]l[ií]t[oó]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
