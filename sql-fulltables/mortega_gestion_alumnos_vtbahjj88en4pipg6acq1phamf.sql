COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR  a.rut ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR  lower(a.email) ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR  text(a.id) ~* 's[eé]m[ií]n[aá]r[ií][oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
