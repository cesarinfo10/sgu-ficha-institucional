COPY (SELECT poas.id,tipo_act,coalesce(p.nombre,'') AS proyecto,actividad,prioridad,
               ...
               WHERE date_part('year',fecha_prog_termino)=2024  AND date_part('month',fecha_prog_termino)=2  AND poas.id_unidad='25'  AND tipo_act='PIA'  AND estado NOT IN ('OK','Terminada','Eliminada')  AND poas.comentarios_ult_fecha BETWEEN now()-'1 month'::interval AND now() 
