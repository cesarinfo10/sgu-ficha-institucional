COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'f[eé]l[ií]p[eé]'  OR u.rut ~* 'f[eé]l[ií]p[eé]')  AND (lower(u.nombre||' '||u.apellido) ~* 'v[aá]rg[aá]s'  OR u.rut ~* 'v[aá]rg[aá]s')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[oó]bl[eé]t[eé]'  OR u.rut ~* 'p[oó]bl[eé]t[eé]') 
