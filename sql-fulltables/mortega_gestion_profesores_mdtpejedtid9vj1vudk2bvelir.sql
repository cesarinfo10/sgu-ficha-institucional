COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'm[aá]r[ií]'  OR u.rut ~* 'm[aá]r[ií]')  AND (lower(u.nombre||' '||u.apellido) ~* 'p[aá]z'  OR u.rut ~* 'p[aá]z')  AND (lower(u.nombre||' '||u.apellido) ~* 'y[aá]ñ[eé]z'  OR u.rut ~* 'y[aá]ñ[eé]z') 
