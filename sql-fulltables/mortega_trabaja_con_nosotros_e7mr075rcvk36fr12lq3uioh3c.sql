COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'cl[aá][uú]d[ií][oó]') OR (lower(pp.apellidos) ~* 'cl[aá][uú]d[ií][oó]') OR (lower(pp.rut) ~* 'cl[aá][uú]d[ií][oó]') AND (lower(pp.nombres) ~* 'f[eé]rn[aá]nd[eé]z') OR (lower(pp.apellidos) ~* 'f[eé]rn[aá]nd[eé]z') OR (lower(pp.rut) ~* 'f[eé]rn[aá]nd[eé]z')
