COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* '[oó]d[eé]tt[eé]') OR (lower(pp.apellidos) ~* '[oó]d[eé]tt[eé]') OR (lower(pp.rut) ~* '[oó]d[eé]tt[eé]') AND (lower(pp.nombres) ~* 'b[aá]st[ií][aá]s') OR (lower(pp.apellidos) ~* 'b[aá]st[ií][aá]s') OR (lower(pp.rut) ~* 'b[aá]st[ií][aá]s')
