COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]ñ[oó]l[eé]s' OR  a.rut ~* 'n[ií]ñ[oó]l[eé]s' OR  lower(a.email) ~* 'n[ií]ñ[oó]l[eé]s' OR  text(a.id) ~* 'n[ií]ñ[oó]l[eé]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
