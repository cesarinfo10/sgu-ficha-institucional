COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]c[ií][aá]' OR  a.rut ~* 'd[aá]c[ií][aá]' OR  lower(a.email) ~* 'd[aá]c[ií][aá]' OR  text(a.id) ~* 'd[aá]c[ií][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
