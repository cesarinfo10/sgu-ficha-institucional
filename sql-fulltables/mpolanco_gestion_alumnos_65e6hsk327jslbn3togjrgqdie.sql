COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]nt[oó]n[ií][aá]' OR  a.rut ~* '[aá]nt[oó]n[ií][aá]' OR  lower(a.email) ~* '[aá]nt[oó]n[ií][aá]' OR  text(a.id) ~* '[aá]nt[oó]n[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nch[eé]z' OR  a.rut ~* 's[aá]nch[eé]z' OR  lower(a.email) ~* 's[aá]nch[eé]z' OR  text(a.id) ~* 's[aá]nch[eé]z' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
