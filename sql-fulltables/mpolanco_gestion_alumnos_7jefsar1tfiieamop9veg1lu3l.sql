COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'tr[uú]j[ií]ll[oó]' OR  a.rut ~* 'tr[uú]j[ií]ll[oó]' OR  lower(a.email) ~* 'tr[uú]j[ií]ll[oó]' OR  text(a.id) ~* 'tr[uú]j[ií]ll[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
