COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rt[eé]g[aá]' OR  a.rut ~* '[oó]rt[eé]g[aá]' OR  lower(a.email) ~* '[oó]rt[eé]g[aá]' OR  text(a.id) ~* '[oó]rt[eé]g[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]x' OR  a.rut ~* 'm[aá]x' OR  lower(a.email) ~* 'm[aá]x' OR  text(a.id) ~* 'm[aá]x' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
