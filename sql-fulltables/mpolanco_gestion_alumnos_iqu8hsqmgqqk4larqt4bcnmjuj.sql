COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'pr[eé][uú]ss' OR  a.rut ~* 'pr[eé][uú]ss' OR  lower(a.email) ~* 'pr[eé][uú]ss' OR  text(a.id) ~* 'pr[eé][uú]ss' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
