COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  a.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  lower(a.email) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  text(a.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'sc[aá]r' OR  a.rut ~* 'sc[aá]r' OR  lower(a.email) ~* 'sc[aá]r' OR  text(a.id) ~* 'sc[aá]r' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
