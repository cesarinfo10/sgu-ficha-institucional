COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l[ií]c[aá]' OR  a.rut ~* '[aá]ng[eé]l[ií]c[aá]' OR  lower(a.email) ~* '[aá]ng[eé]l[ií]c[aá]' OR  text(a.id) ~* '[aá]ng[eé]l[ií]c[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]z[aá]rr[oó]' OR  a.rut ~* 'p[ií]z[aá]rr[oó]' OR  lower(a.email) ~* 'p[ií]z[aá]rr[oó]' OR  text(a.id) ~* 'p[ií]z[aá]rr[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
