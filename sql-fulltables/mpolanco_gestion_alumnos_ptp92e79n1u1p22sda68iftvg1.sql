COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]rc[aá]' OR  a.rut ~* 't[oó]rc[aá]' OR  lower(a.email) ~* 't[oó]rc[aá]' OR  text(a.id) ~* 't[oó]rc[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
