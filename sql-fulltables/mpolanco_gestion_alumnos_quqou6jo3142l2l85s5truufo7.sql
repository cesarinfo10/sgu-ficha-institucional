COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[ií]s[ií]' OR  a.rut ~* 'd[ií]s[ií]' OR  lower(a.email) ~* 'd[ií]s[ií]' OR  text(a.id) ~* 'd[ií]s[ií]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
