COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[eé]ls[oó]n' OR  a.rut ~* 'n[eé]ls[oó]n' OR  lower(a.email) ~* 'n[eé]ls[oó]n' OR  text(a.id) ~* 'n[eé]ls[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]l[aá]z[aá]r' OR  a.rut ~* 's[aá]l[aá]z[aá]r' OR  lower(a.email) ~* 's[aá]l[aá]z[aá]r' OR  text(a.id) ~* 's[aá]l[aá]z[aá]r' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
