COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rb[aá]r[aá]' OR  a.rut ~* 'b[aá]rb[aá]r[aá]' OR  lower(a.email) ~* 'b[aá]rb[aá]r[aá]' OR  text(a.id) ~* 'b[aá]rb[aá]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]t[ií]c[aá]' OR  a.rut ~* 'g[aá]t[ií]c[aá]' OR  lower(a.email) ~* 'g[aá]t[ií]c[aá]' OR  text(a.id) ~* 'g[aá]t[ií]c[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
