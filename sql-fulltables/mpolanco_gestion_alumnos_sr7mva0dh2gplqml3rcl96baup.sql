COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]c[oó]st[aá]' OR  a.rut ~* '[aá]c[oó]st[aá]' OR  lower(a.email) ~* '[aá]c[oó]st[aá]' OR  text(a.id) ~* '[aá]c[oó]st[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
