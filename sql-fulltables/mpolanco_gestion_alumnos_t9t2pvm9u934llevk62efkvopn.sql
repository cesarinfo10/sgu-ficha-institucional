COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rm[eé]ñ[oó]' OR  a.rut ~* '[oó]rm[eé]ñ[oó]' OR  lower(a.email) ~* '[oó]rm[eé]ñ[oó]' OR  text(a.id) ~* '[oó]rm[eé]ñ[oó]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
