COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[oó]' OR  a.rut ~* 'm[aá]rc[oó]' OR  lower(a.email) ~* 'm[aá]rc[oó]' OR  text(a.id) ~* 'm[aá]rc[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][ií]r[oó]z' OR  a.rut ~* 'q[uú][ií]r[oó]z' OR  lower(a.email) ~* 'q[uú][ií]r[oó]z' OR  text(a.id) ~* 'q[uú][ií]r[oó]z' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
