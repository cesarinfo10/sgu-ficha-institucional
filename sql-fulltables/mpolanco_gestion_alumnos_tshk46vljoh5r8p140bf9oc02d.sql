COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][uú]l[oó]' OR  a.rut ~* 'p[aá][uú]l[oó]' OR  lower(a.email) ~* 'p[aá][uú]l[oó]' OR  text(a.id) ~* 'p[aá][uú]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]n[oó]' OR  a.rut ~* 'p[ií]n[oó]' OR  lower(a.email) ~* 'p[ií]n[oó]' OR  text(a.id) ~* 'p[ií]n[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
