COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'gl[oó]r[ií][aá]' OR  a.rut ~* 'gl[oó]r[ií][aá]' OR  lower(a.email) ~* 'gl[oó]r[ií][aá]' OR  text(a.id) ~* 'gl[oó]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rt[eé]s' OR  a.rut ~* 'c[oó]rt[eé]s' OR  lower(a.email) ~* 'c[oó]rt[eé]s' OR  text(a.id) ~* 'c[oó]rt[eé]s' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
