COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]v[eé]lyn' OR  a.rut ~* '[eé]v[eé]lyn' OR  lower(a.email) ~* '[eé]v[eé]lyn' OR  text(a.id) ~* '[eé]v[eé]lyn' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[eé][ií]r[aá]' OR  a.rut ~* 'n[eé][ií]r[aá]' OR  lower(a.email) ~* 'n[eé][ií]r[aá]' OR  text(a.id) ~* 'n[eé][ií]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
