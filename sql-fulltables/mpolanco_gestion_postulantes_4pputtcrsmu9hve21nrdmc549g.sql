COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]rr[ií][aá]g[aá]d[aá]' OR  pap.rut ~* '[aá]rr[ií][aá]g[aá]d[aá]' OR  text(pap.id) ~* '[aá]rr[ií][aá]g[aá]d[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'fr[aá]nc[ií]sc[oó]' OR  pap.rut ~* 'fr[aá]nc[ií]sc[oó]' OR  text(pap.id) ~* 'fr[aá]nc[ií]sc[oó]' ) 
