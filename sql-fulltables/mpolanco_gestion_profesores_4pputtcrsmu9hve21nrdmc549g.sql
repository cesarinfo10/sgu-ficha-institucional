COPY (SELECT u.id AS id_profesor,initcap(u.apellido) AS apellidos,initcap(u.nombre) AS nombres,
                   ...
                   WHERE tipo=3  AND (lower(u.nombre||' '||u.apellido) ~* 'd[ií][aá]z'  OR u.rut ~* 'd[ií][aá]z')  AND (lower(u.nombre||' '||u.apellido) ~* 'q[uú][ií]nt[aá]n[ií]ll[aá]'  OR u.rut ~* 'q[uú][ií]nt[aá]n[ií]ll[aá]') 
