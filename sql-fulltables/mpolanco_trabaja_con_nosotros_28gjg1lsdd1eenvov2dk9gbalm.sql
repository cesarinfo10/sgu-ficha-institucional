COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'r[oó][aá]') OR (lower(pp.apellidos) ~* 'r[oó][aá]') OR (lower(pp.rut) ~* 'r[oó][aá]') AND (lower(pp.nombres) ~* 's[uú]l[aá]nt[aá]y') OR (lower(pp.apellidos) ~* 's[uú]l[aá]nt[aá]y') OR (lower(pp.rut) ~* 's[uú]l[aá]nt[aá]y')
