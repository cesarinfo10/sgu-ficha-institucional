COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* '[eé]sc[oó]b[aá]r') OR (lower(pp.apellidos) ~* '[eé]sc[oó]b[aá]r') OR (lower(pp.rut) ~* '[eé]sc[oó]b[aá]r') AND (lower(pp.nombres) ~* 'l[eé]m[uú]s') OR (lower(pp.apellidos) ~* 'l[eé]m[uú]s') OR (lower(pp.rut) ~* 'l[eé]m[uú]s')
