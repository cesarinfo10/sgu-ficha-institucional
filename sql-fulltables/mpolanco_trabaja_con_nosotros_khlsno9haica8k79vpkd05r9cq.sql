COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* '[ií]ngr[ií]d') OR (lower(pp.apellidos) ~* '[ií]ngr[ií]d') OR (lower(pp.rut) ~* '[ií]ngr[ií]d') AND (lower(pp.nombres) ~* 'c[oó]nch[aá]') OR (lower(pp.apellidos) ~* 'c[oó]nch[aá]') OR (lower(pp.rut) ~* 'c[oó]nch[aá]')
