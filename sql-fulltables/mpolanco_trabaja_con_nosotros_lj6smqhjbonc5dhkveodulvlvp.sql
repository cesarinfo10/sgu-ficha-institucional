COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'm[ií]g[uú][eé]l') OR (lower(pp.apellidos) ~* 'm[ií]g[uú][eé]l') OR (lower(pp.rut) ~* 'm[ií]g[uú][eé]l') AND (lower(pp.nombres) ~* 's[ií]lv[aá]') OR (lower(pp.apellidos) ~* 's[ií]lv[aá]') OR (lower(pp.rut) ~* 's[ií]lv[aá]')
