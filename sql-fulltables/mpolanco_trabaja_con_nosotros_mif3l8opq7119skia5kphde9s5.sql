COPY (SELECT pp.id,pp.rut,pp.apellidos,initcap(pp.nombres) AS nombres,(SELECT char_comma_sum(alias) FROM carreras WHERE id::text = ANY(string_to_array(pp.carreras,','))) AS carreras,pp.genero,
                   ...
                   WHERE true  AND (lower(pp.nombres) ~* 'm[aá]r[ií][oó]') OR (lower(pp.apellidos) ~* 'm[aá]r[ií][oó]') OR (lower(pp.rut) ~* 'm[aá]r[ií][oó]') AND (lower(pp.nombres) ~* '[ií]d[ií][aá]q[uú][eé]z') OR (lower(pp.apellidos) ~* '[ií]d[ií][aá]q[uú][eé]z') OR (lower(pp.rut) ~* '[ií]d[ií][aá]q[uú][eé]z')
