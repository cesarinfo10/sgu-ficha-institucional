COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]n[ií][aá]' OR  pap.rut ~* 'v[aá]n[ií][aá]' OR  text(pap.id) ~* 'v[aá]n[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[aá]p[ií]m[aá]n' OR  pap.rut ~* 'r[aá]p[ií]m[aá]n' OR  text(pap.id) ~* 'r[aá]p[ií]m[aá]n' ) 
