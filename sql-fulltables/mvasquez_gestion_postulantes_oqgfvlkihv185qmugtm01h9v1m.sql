COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]nst[aá]nz[aá]' OR  pap.rut ~* 'c[oó]nst[aá]nz[aá]' OR  text(pap.id) ~* 'c[oó]nst[aá]nz[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[uú]r[ií]b[eé]' OR  pap.rut ~* '[uú]r[ií]b[eé]' OR  text(pap.id) ~* '[uú]r[ií]b[eé]' ) 
