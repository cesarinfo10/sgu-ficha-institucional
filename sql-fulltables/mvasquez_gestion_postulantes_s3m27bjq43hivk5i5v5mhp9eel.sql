COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'k[aá]rl[aá]' OR  pap.rut ~* 'k[aá]rl[aá]' OR  text(pap.id) ~* 'k[aá]rl[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]ld[eé]s' OR  pap.rut ~* 'v[aá]ld[eé]s' OR  text(pap.id) ~* 'v[aá]ld[eé]s' ) 
