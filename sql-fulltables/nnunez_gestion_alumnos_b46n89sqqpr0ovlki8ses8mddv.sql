COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]sc[aá]r' OR  a.rut ~* '[oó]sc[aá]r' OR  lower(a.email) ~* '[oó]sc[aá]r' OR  text(a.id) ~* '[oó]sc[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]ss' OR  a.rut ~* 'r[oó]ss' OR  lower(a.email) ~* 'r[oó]ss' OR  text(a.id) ~* 'r[oó]ss' ) 
