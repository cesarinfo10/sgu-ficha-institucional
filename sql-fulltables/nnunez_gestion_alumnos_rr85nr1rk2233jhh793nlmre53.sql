COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[uú]g[oó]' OR  a.rut ~* 'h[uú]g[oó]' OR  lower(a.email) ~* 'h[uú]g[oó]' OR  text(a.id) ~* 'h[uú]g[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií]v[ií]l' OR  a.rut ~* 'm[aá]r[ií]v[ií]l' OR  lower(a.email) ~* 'm[aá]r[ií]v[ií]l' OR  text(a.id) ~* 'm[aá]r[ií]v[ií]l' ) 
