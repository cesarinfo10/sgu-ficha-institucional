COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rl[oó]s' OR  pap.rut ~* 'c[aá]rl[oó]s' OR  text(pap.id) ~* 'c[aá]rl[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'fl[oó]r[eé]s' OR  pap.rut ~* 'fl[oó]r[eé]s' OR  text(pap.id) ~* 'fl[oó]r[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]s[oó]r[ií][oó]' OR  pap.rut ~* '[oó]s[oó]r[ií][oó]' OR  text(pap.id) ~* '[oó]s[oó]r[ií][oó]' ) 
