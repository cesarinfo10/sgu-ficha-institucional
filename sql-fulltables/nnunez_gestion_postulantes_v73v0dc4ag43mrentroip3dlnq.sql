COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]j[aá]ndr[oó]' OR  pap.rut ~* '[aá]l[eé]j[aá]ndr[oó]' OR  text(pap.id) ~* '[aá]l[eé]j[aá]ndr[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'z[eé]p[eé]d[aá]' OR  pap.rut ~* 'z[eé]p[eé]d[aá]' OR  text(pap.id) ~* 'z[eé]p[eé]d[aá]' ) 
