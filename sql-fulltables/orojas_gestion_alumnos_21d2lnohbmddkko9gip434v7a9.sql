COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[eé]ss[ií]c[aá]' OR  a.rut ~* 'y[eé]ss[ií]c[aá]' OR  lower(a.email) ~* 'y[eé]ss[ií]c[aá]' OR  text(a.id) ~* 'y[eé]ss[ií]c[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó]nz[aá]l[eé]z' OR  a.rut ~* 'g[oó]nz[aá]l[eé]z' OR  lower(a.email) ~* 'g[oó]nz[aá]l[eé]z' OR  text(a.id) ~* 'g[oó]nz[aá]l[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]st[uú]d[ií]ll[oó]' OR  a.rut ~* '[aá]st[uú]d[ií]ll[oó]' OR  lower(a.email) ~* '[aá]st[uú]d[ií]ll[oó]' OR  text(a.id) ~* '[aá]st[uú]d[ií]ll[oó]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148) 
