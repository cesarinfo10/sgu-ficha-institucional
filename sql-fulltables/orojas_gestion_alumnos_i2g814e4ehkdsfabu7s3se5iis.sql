COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2023 AND semestre=2 LIMIT 1)=1  THEN 'Si' ELSE 'No' END AS matriculado,(SELECT max(fecha) FROM matriculas WHERE id_alumno=a.id) AS fecha_ult_mat,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]nd[ií][aá]' OR  a.rut ~* '[aá]nd[ií][aá]' OR  lower(a.email) ~* '[aá]nd[ií][aá]' OR  text(a.id) ~* '[aá]nd[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  lower(a.email) ~* '' OR  text(a.id) ~* '' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]rc[ií][aá]' OR  a.rut ~* 'g[aá]rc[ií][aá]' OR  lower(a.email) ~* 'g[aá]rc[ií][aá]' OR  text(a.id) ~* 'g[aá]rc[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rb[aá]r[aá]' OR  a.rut ~* 'b[aá]rb[aá]r[aá]' OR  lower(a.email) ~* 'b[aá]rb[aá]r[aá]' OR  text(a.id) ~* 'b[aá]rb[aá]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]h[aá]nn[aá]' OR  a.rut ~* 'j[oó]h[aá]nn[aá]' OR  lower(a.email) ~* 'j[oó]h[aá]nn[aá]' OR  text(a.id) ~* 'j[oó]h[aá]nn[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148) 
