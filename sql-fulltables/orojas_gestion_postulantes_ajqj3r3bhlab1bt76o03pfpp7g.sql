COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[eé]n[aá]r[oó]' OR  pap.rut ~* 'g[eé]n[aá]r[oó]' OR  text(pap.id) ~* 'g[eé]n[aá]r[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]y[aá]n[oó]' OR  pap.rut ~* 'm[oó]y[aá]n[oó]' OR  text(pap.id) ~* 'm[oó]y[aá]n[oó]' ) 
