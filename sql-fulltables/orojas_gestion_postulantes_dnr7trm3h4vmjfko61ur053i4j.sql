COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]n[ií][eé]l[aá]' OR  pap.rut ~* 'd[aá]n[ií][eé]l[aá]' OR  text(pap.id) ~* 'd[aá]n[ií][eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]l[eé]n' OR  pap.rut ~* 'b[eé]l[eé]n' OR  text(pap.id) ~* 'b[eé]l[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[uú]ñ[oó]z' OR  pap.rut ~* 'm[uú]ñ[oó]z' OR  text(pap.id) ~* 'm[uú]ñ[oó]z' ) 
