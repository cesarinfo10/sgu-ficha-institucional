COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé][ií]s[eé]rm[aá]nn' OR  pap.rut ~* '[eé][ií]s[eé]rm[aá]nn' OR  text(pap.id) ~* '[eé][ií]s[eé]rm[aá]nn' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]rg[ií][oó]' OR  pap.rut ~* 's[eé]rg[ií][oó]' OR  text(pap.id) ~* 's[eé]rg[ií][oó]' ) 
