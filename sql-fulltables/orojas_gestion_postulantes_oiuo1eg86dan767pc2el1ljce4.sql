COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]s[eé]' OR  pap.rut ~* 'j[oó]s[eé]' OR  text(pap.id) ~* 'j[oó]s[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]s[aá]l[eé]s' OR  pap.rut ~* 'r[oó]s[aá]l[eé]s' OR  text(pap.id) ~* 'r[oó]s[aá]l[eé]s' ) 
