COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       cod_asignatura ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       text(c.id) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       lower(profesor) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d')  
