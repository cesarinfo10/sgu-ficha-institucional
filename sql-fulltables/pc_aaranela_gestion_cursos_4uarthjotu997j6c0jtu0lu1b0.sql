COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'b[eé][aá]tr[ií]z' OR       cod_asignatura ~* 'b[eé][aá]tr[ií]z' OR       text(c.id) ~* 'b[eé][aá]tr[ií]z' OR       lower(profesor) ~* 'b[eé][aá]tr[ií]z')  AND (lower(asignatura) ~* 'd[eé]n[ií]s[eé]' OR       cod_asignatura ~* 'd[eé]n[ií]s[eé]' OR       text(c.id) ~* 'd[eé]n[ií]s[eé]' OR       lower(profesor) ~* 'd[eé]n[ií]s[eé]')  AND (lower(asignatura) ~* 'r[ií]q[uú][eé]lm[eé]' OR       cod_asignatura ~* 'r[ií]q[uú][eé]lm[eé]' OR       text(c.id) ~* 'r[ií]q[uú][eé]lm[eé]' OR       lower(profesor) ~* 'r[ií]q[uú][eé]lm[eé]')  AND (lower(asignatura) ~* '[eé]st[aá]y' OR       cod_asignatura ~* '[eé]st[aá]y' OR       text(c.id) ~* '[eé]st[aá]y' OR       lower(profesor) ~* '[eé]st[aá]y')  
