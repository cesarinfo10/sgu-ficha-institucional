COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]v[aá]' OR       cod_asignatura ~* '[eé]v[aá]' OR       text(c.id) ~* '[eé]v[aá]' OR       lower(profesor) ~* '[eé]v[aá]')  AND (lower(asignatura) ~* 'c[oó]rt[eé]s' OR       cod_asignatura ~* 'c[oó]rt[eé]s' OR       text(c.id) ~* 'c[oó]rt[eé]s' OR       lower(profesor) ~* 'c[oó]rt[eé]s')  
