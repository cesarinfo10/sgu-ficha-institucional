COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[eé]dy' OR       cod_asignatura ~* 'h[eé]dy' OR       text(c.id) ~* 'h[eé]dy' OR       lower(profesor) ~* 'h[eé]dy')  AND (lower(asignatura) ~* 'kr[uú]s[eé]' OR       cod_asignatura ~* 'kr[uú]s[eé]' OR       text(c.id) ~* 'kr[uú]s[eé]' OR       lower(profesor) ~* 'kr[uú]s[eé]')  AND (lower(asignatura) ~* 'r[ií]v[eé]r[aá]' OR       cod_asignatura ~* 'r[ií]v[eé]r[aá]' OR       text(c.id) ~* 'r[ií]v[eé]r[aá]' OR       lower(profesor) ~* 'r[ií]v[eé]r[aá]')  
