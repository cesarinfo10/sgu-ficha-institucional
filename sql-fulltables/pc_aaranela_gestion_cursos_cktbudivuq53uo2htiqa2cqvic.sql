COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[oó]b[ií]d[aá]d' OR       cod_asignatura ~* 'pr[oó]b[ií]d[aá]d' OR       text(c.id) ~* 'pr[oó]b[ií]d[aá]d' OR       lower(profesor) ~* 'pr[oó]b[ií]d[aá]d')  
