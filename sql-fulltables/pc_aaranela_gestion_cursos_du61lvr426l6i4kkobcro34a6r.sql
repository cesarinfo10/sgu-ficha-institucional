COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rt[ií]z' OR       cod_asignatura ~* '[oó]rt[ií]z' OR       text(c.id) ~* '[oó]rt[ií]z' OR       lower(profesor) ~* '[oó]rt[ií]z')  AND (lower(asignatura) ~* 'c[oó]ntr[eé]r[aá]s' OR       cod_asignatura ~* 'c[oó]ntr[eé]r[aá]s' OR       text(c.id) ~* 'c[oó]ntr[eé]r[aá]s' OR       lower(profesor) ~* 'c[oó]ntr[eé]r[aá]s')  
