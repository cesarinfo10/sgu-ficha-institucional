COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]dr[ií][aá]n' OR       cod_asignatura ~* '[aá]dr[ií][aá]n' OR       text(c.id) ~* '[aá]dr[ií][aá]n' OR       lower(profesor) ~* '[aá]dr[ií][aá]n')  AND (lower(asignatura) ~* '[eé]st[eé]b[aá]n' OR       cod_asignatura ~* '[eé]st[eé]b[aá]n' OR       text(c.id) ~* '[eé]st[eé]b[aá]n' OR       lower(profesor) ~* '[eé]st[eé]b[aá]n')  AND (lower(asignatura) ~* 'p[eé]r[eé][ií]r[aá]' OR       cod_asignatura ~* 'p[eé]r[eé][ií]r[aá]' OR       text(c.id) ~* 'p[eé]r[eé][ií]r[aá]' OR       lower(profesor) ~* 'p[eé]r[eé][ií]r[aá]')  AND (lower(asignatura) ~* 's[aá]nt[aá]n[aá]' OR       cod_asignatura ~* 's[aá]nt[aá]n[aá]' OR       text(c.id) ~* 's[aá]nt[aá]n[aá]' OR       lower(profesor) ~* 's[aá]nt[aá]n[aá]')  
