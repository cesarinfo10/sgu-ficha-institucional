COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ngl[eé]s' OR       cod_asignatura ~* '[ií]ngl[eé]s' OR       text(c.id) ~* '[ií]ngl[eé]s' OR       lower(profesor) ~* '[ií]ngl[eé]s')  AND (lower(asignatura) ~* '[ií]' OR       cod_asignatura ~* '[ií]' OR       text(c.id) ~* '[ií]' OR       lower(profesor) ~* '[ií]')  
