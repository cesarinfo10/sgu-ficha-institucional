COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]d[eé]c[ií]d[oó]' OR       cod_asignatura ~* 'c[oó]d[eé]c[ií]d[oó]' OR       text(c.id) ~* 'c[oó]d[eé]c[ií]d[oó]' OR       lower(profesor) ~* 'c[oó]d[eé]c[ií]d[oó]')  
