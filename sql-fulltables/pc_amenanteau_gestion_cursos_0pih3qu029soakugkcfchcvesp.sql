COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]rk[eé]t[ií]ng' OR       cod_asignatura ~* 'm[aá]rk[eé]t[ií]ng' OR       text(c.id) ~* 'm[aá]rk[eé]t[ií]ng' OR       lower(profesor) ~* 'm[aá]rk[eé]t[ií]ng')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
