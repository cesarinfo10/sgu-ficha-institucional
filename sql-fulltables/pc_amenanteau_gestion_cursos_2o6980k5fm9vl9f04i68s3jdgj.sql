COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[eé]n[aá]n' OR       cod_asignatura ~* 'm[eé]n[aá]n' OR       text(c.id) ~* 'm[eé]n[aá]n' OR       lower(profesor) ~* 'm[eé]n[aá]n')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
