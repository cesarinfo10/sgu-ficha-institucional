COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]rk[eé]t[ií]ng' OR       cod_asignatura ~* 'm[aá]rk[eé]t[ií]ng' OR       text(c.id) ~* 'm[aá]rk[eé]t[ií]ng' OR       lower(profesor) ~* 'm[aá]rk[eé]t[ií]ng')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
