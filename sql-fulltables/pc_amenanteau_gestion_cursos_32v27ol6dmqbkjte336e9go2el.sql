COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '18616' OR       cod_asignatura ~* '18616' OR       text(c.id) ~* '18616' OR       lower(profesor) ~* '18616')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
