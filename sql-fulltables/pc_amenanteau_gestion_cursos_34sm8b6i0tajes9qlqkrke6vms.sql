COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]l[ií]z' OR       cod_asignatura ~* '[eé]l[ií]z' OR       text(c.id) ~* '[eé]l[ií]z' OR       lower(profesor) ~* '[eé]l[ií]z')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
