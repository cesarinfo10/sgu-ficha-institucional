COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[uú]c[uú]m[ií]d[eé]s' OR       cod_asignatura ~* 'c[uú]c[uú]m[ií]d[eé]s' OR       text(c.id) ~* 'c[uú]c[uú]m[ií]d[eé]s' OR       lower(profesor) ~* 'c[uú]c[uú]m[ií]d[eé]s')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
