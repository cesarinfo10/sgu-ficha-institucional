COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]n[aá]l[ií]s[ií]s' OR       cod_asignatura ~* '[aá]n[aá]l[ií]s[ií]s' OR       text(c.id) ~* '[aá]n[aá]l[ií]s[ií]s' OR       lower(profesor) ~* '[aá]n[aá]l[ií]s[ií]s')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
