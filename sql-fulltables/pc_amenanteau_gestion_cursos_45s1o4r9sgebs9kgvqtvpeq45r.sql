COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]lfr[eé]d[oó]' OR       cod_asignatura ~* '[aá]lfr[eé]d[oó]' OR       text(c.id) ~* '[aá]lfr[eé]d[oó]' OR       lower(profesor) ~* '[aá]lfr[eé]d[oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
