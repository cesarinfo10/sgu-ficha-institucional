COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       cod_asignatura ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       text(c.id) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       lower(profesor) ~* 'h[uú]m[aá]n[ií]sm[oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
