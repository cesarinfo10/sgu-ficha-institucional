COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '15066' OR       cod_asignatura ~* '15066' OR       text(c.id) ~* '15066' OR       lower(profesor) ~* '15066')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
