COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú]nd[aá]m[eé]nt[oó]' OR       cod_asignatura ~* 'f[uú]nd[aá]m[eé]nt[oó]' OR       text(c.id) ~* 'f[uú]nd[aá]m[eé]nt[oó]' OR       lower(profesor) ~* 'f[uú]nd[aá]m[eé]nt[oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
