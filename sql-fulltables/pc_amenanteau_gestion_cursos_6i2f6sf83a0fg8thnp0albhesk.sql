COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[aá]m[oó]n' OR       cod_asignatura ~* 'r[aá]m[oó]n' OR       text(c.id) ~* 'r[aá]m[oó]n' OR       lower(profesor) ~* 'r[aá]m[oó]n')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
