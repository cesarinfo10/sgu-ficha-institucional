COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[aá]b[aá]t' OR       cod_asignatura ~* 's[aá]b[aá]t' OR       text(c.id) ~* 's[aá]b[aá]t' OR       lower(profesor) ~* 's[aá]b[aá]t')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
