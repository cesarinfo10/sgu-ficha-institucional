COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]lc[uú]l[oó]' OR       cod_asignatura ~* 'c[aá]lc[uú]l[oó]' OR       text(c.id) ~* 'c[aá]lc[uú]l[oó]' OR       lower(profesor) ~* 'c[aá]lc[uú]l[oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
