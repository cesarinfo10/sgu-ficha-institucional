COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'cl[eé]rc' OR       cod_asignatura ~* 'cl[eé]rc' OR       text(c.id) ~* 'cl[eé]rc' OR       lower(profesor) ~* 'cl[eé]rc')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
