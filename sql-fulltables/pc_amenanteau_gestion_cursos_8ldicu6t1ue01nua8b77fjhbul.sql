COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]l[ií]z[oó]nd[oó]' OR       cod_asignatura ~* '[eé]l[ií]z[oó]nd[oó]' OR       text(c.id) ~* '[eé]l[ií]z[oó]nd[oó]' OR       lower(profesor) ~* '[eé]l[ií]z[oó]nd[oó]')  AND array[101,62,26,88,18,3,36,67,156,12] && (ids_carreras)  
