COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]str[aá]t[eé]g' OR       cod_asignatura ~* '[eé]str[aá]t[eé]g' OR       text(c.id) ~* '[eé]str[aá]t[eé]g' OR       lower(profesor) ~* '[eé]str[aá]t[eé]g')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
