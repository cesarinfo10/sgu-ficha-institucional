COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]d[eé]l[eé][ií]n[eé]' OR       cod_asignatura ~* 'm[aá]d[eé]l[eé][ií]n[eé]' OR       text(c.id) ~* 'm[aá]d[eé]l[eé][ií]n[eé]' OR       lower(profesor) ~* 'm[aá]d[eé]l[eé][ií]n[eé]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
