COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'b[aá]s[ií]c[aá]' OR       cod_asignatura ~* 'b[aá]s[ií]c[aá]' OR       text(c.id) ~* 'b[aá]s[ií]c[aá]' OR       lower(profesor) ~* 'b[aá]s[ií]c[aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
