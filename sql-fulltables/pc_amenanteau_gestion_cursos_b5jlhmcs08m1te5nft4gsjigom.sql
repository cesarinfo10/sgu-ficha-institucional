COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá][uú]l[ií]n[aá]' OR       cod_asignatura ~* 'p[aá][uú]l[ií]n[aá]' OR       text(c.id) ~* 'p[aá][uú]l[ií]n[aá]' OR       lower(profesor) ~* 'p[aá][uú]l[ií]n[aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
