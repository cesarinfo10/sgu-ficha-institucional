COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]st[aá]b[aá]' OR       cod_asignatura ~* '[eé]st[aá]b[aá]' OR       text(c.id) ~* '[eé]st[aá]b[aá]' OR       lower(profesor) ~* '[eé]st[aá]b[aá]')  AND array[101,62,26,88,18,3,36,67,156,159,12] && (ids_carreras)  
