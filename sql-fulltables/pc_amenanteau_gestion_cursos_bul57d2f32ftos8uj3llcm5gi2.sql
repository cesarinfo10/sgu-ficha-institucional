COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR       cod_asignatura ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR       text(c.id) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR       lower(profesor) ~* 'm[aá]d[aá]r[ií][aá]g[aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
