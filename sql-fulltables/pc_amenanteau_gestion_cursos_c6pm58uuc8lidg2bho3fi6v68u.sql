COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]s[ií]s' OR       cod_asignatura ~* 't[eé]s[ií]s' OR       text(c.id) ~* 't[eé]s[ií]s' OR       lower(profesor) ~* 't[eé]s[ií]s')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
