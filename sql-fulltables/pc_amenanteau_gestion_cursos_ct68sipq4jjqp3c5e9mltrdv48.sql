COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]r[ií][aá]s' OR       cod_asignatura ~* '[aá]r[ií][aá]s' OR       text(c.id) ~* '[aá]r[ií][aá]s' OR       lower(profesor) ~* '[aá]r[ií][aá]s')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
