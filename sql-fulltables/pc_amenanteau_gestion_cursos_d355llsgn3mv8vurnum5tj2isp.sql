COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]lg[eé]br' OR       cod_asignatura ~* '[aá]lg[eé]br' OR       text(c.id) ~* '[aá]lg[eé]br' OR       lower(profesor) ~* '[aá]lg[eé]br')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
