COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND id_carrera=67  AND c.ano=2022  AND c.semestre=2  AND (car.regimen = 'PRE')  AND 2 IN (c.dia1,c.dia2,c.dia3)  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
