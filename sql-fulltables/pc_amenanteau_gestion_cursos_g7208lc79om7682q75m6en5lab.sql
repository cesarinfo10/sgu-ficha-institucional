COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       cod_asignatura ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       text(c.id) ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       lower(profesor) ~* '[aá]p[aá]r[ií]c[ií][oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
