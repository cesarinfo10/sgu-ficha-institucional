COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       cod_asignatura ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       text(c.id) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       lower(profesor) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
