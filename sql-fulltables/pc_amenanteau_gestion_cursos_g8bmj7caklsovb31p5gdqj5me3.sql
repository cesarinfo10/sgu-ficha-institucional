COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND c.ano=2023  AND c.semestre=2  AND c.seccion=9  AND ('PRE' = ANY (regimenes))  AND array[101,62,26,88,18,3,36,67,156,159,12] && (ids_carreras)  
