COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[eé]rd[uú]g[oó]' OR       cod_asignatura ~* 'v[eé]rd[uú]g[oó]' OR       text(c.id) ~* 'v[eé]rd[uú]g[oó]' OR       lower(profesor) ~* 'v[eé]rd[uú]g[oó]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
