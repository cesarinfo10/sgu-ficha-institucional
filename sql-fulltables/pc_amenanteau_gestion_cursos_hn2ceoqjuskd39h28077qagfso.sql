COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[eé]n[aá]nt[eé][aá][uú]' OR       cod_asignatura ~* 'm[eé]n[aá]nt[eé][aá][uú]' OR       text(c.id) ~* 'm[eé]n[aá]nt[eé][aá][uú]' OR       lower(profesor) ~* 'm[eé]n[aá]nt[eé][aá][uú]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
