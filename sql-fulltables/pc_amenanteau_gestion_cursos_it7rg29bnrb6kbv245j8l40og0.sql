COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]t[aá]l[oó]' OR       cod_asignatura ~* '[ií]t[aá]l[oó]' OR       text(c.id) ~* '[ií]t[aá]l[oó]' OR       lower(profesor) ~* '[ií]t[aá]l[oó]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
