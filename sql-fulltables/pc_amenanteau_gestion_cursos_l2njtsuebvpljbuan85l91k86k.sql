COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]t[aá]l[oó]' OR       cod_asignatura ~* '[ií]t[aá]l[oó]' OR       text(c.id) ~* '[ií]t[aá]l[oó]' OR       lower(profesor) ~* '[ií]t[aá]l[oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
