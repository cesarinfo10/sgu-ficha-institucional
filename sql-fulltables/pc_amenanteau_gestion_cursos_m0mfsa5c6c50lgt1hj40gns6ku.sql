COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]nd[ií]v[ií]d[uú][oó]' OR       cod_asignatura ~* '[ií]nd[ií]v[ií]d[uú][oó]' OR       text(c.id) ~* '[ií]nd[ií]v[ií]d[uú][oó]' OR       lower(profesor) ~* '[ií]nd[ií]v[ií]d[uú][oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
