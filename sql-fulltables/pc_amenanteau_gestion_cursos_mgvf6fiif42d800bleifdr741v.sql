COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]n[eé]s' OR       cod_asignatura ~* '[ií]n[eé]s' OR       text(c.id) ~* '[ií]n[eé]s' OR       lower(profesor) ~* '[ií]n[eé]s')  AND (lower(asignatura) ~* '[aá]l[eé]j[aá]ndr[aá]' OR       cod_asignatura ~* '[aá]l[eé]j[aá]ndr[aá]' OR       text(c.id) ~* '[aá]l[eé]j[aá]ndr[aá]' OR       lower(profesor) ~* '[aá]l[eé]j[aá]ndr[aá]')  AND (lower(asignatura) ~* 'v[ií]c[eé]nc[ií][oó]' OR       cod_asignatura ~* 'v[ií]c[eé]nc[ií][oó]' OR       text(c.id) ~* 'v[ií]c[eé]nc[ií][oó]' OR       lower(profesor) ~* 'v[ií]c[eé]nc[ií][oó]')  AND (lower(asignatura) ~* 'p[aá]rd[oó]' OR       cod_asignatura ~* 'p[aá]rd[oó]' OR       text(c.id) ~* 'p[aá]rd[oó]' OR       lower(profesor) ~* 'p[aá]rd[oó]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
