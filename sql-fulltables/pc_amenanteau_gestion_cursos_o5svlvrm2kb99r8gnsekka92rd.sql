COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]t[ií]c[aá]' OR       cod_asignatura ~* '[eé]t[ií]c[aá]' OR       text(c.id) ~* '[eé]t[ií]c[aá]' OR       lower(profesor) ~* '[eé]t[ií]c[aá]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
