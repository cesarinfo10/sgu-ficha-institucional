COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[uú]b[eé]rt[ií]n[oó]' OR       cod_asignatura ~* '[uú]b[eé]rt[ií]n[oó]' OR       text(c.id) ~* '[uú]b[eé]rt[ií]n[oó]' OR       lower(profesor) ~* '[uú]b[eé]rt[ií]n[oó]')  AND array[101,62,26,88,18,3,36,67,156,159,12] && (ids_carreras)  
