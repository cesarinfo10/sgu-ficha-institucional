COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]nt[eé]l[ií]g' OR       cod_asignatura ~* '[ií]nt[eé]l[ií]g' OR       text(c.id) ~* '[ií]nt[eé]l[ií]g' OR       lower(profesor) ~* '[ií]nt[eé]l[ií]g')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
