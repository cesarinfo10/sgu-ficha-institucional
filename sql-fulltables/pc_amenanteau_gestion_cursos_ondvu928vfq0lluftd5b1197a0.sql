COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó]x[aá]n[aá]' OR       cod_asignatura ~* 'r[oó]x[aá]n[aá]' OR       text(c.id) ~* 'r[oó]x[aá]n[aá]' OR       lower(profesor) ~* 'r[oó]x[aá]n[aá]')  AND (lower(asignatura) ~* '[aá]ndr[eé][aá]' OR       cod_asignatura ~* '[aá]ndr[eé][aá]' OR       text(c.id) ~* '[aá]ndr[eé][aá]' OR       lower(profesor) ~* '[aá]ndr[eé][aá]')  AND (lower(asignatura) ~* '[eé]sp[ií]n[oó]z[aá]' OR       cod_asignatura ~* '[eé]sp[ií]n[oó]z[aá]' OR       text(c.id) ~* '[eé]sp[ií]n[oó]z[aá]' OR       lower(profesor) ~* '[eé]sp[ií]n[oó]z[aá]')  AND (lower(asignatura) ~* 's[aá]nch[eé]z' OR       cod_asignatura ~* 's[aá]nch[eé]z' OR       text(c.id) ~* 's[aá]nch[eé]z' OR       lower(profesor) ~* 's[aá]nch[eé]z')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
