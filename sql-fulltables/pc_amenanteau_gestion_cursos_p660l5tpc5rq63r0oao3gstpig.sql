COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]s[aá]rr' OR       cod_asignatura ~* 'd[eé]s[aá]rr' OR       text(c.id) ~* 'd[eé]s[aá]rr' OR       lower(profesor) ~* 'd[eé]s[aá]rr')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
