COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '18063' OR       cod_asignatura ~* '18063' OR       text(c.id) ~* '18063' OR       lower(profesor) ~* '18063')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
