COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[aá]rr[ií]d[oó]' OR       cod_asignatura ~* 'g[aá]rr[ií]d[oó]' OR       text(c.id) ~* 'g[aá]rr[ií]d[oó]' OR       lower(profesor) ~* 'g[aá]rr[ií]d[oó]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
