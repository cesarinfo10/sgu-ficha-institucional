COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]st[aá]d[ií]st[ií]c[aá]' OR       cod_asignatura ~* '[eé]st[aá]d[ií]st[ií]c[aá]' OR       text(c.id) ~* '[eé]st[aá]d[ií]st[ií]c[aá]' OR       lower(profesor) ~* '[eé]st[aá]d[ií]st[ií]c[aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
