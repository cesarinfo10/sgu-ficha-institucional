COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]t[aá]l[ií]' OR       cod_asignatura ~* 'v[ií]t[aá]l[ií]' OR       text(c.id) ~* 'v[ií]t[aá]l[ií]' OR       lower(profesor) ~* 'v[ií]t[aá]l[ií]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
