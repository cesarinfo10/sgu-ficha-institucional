COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[oó]r[aá]l[eé]s' OR       cod_asignatura ~* 'm[oó]r[aá]l[eé]s' OR       text(c.id) ~* 'm[oó]r[aá]l[eé]s' OR       lower(profesor) ~* 'm[oó]r[aá]l[eé]s')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
