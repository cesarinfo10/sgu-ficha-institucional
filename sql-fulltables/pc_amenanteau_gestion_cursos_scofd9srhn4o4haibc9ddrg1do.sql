COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[aá]nny' OR       cod_asignatura ~* 'd[aá]nny' OR       text(c.id) ~* 'd[aá]nny' OR       lower(profesor) ~* 'd[aá]nny')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
