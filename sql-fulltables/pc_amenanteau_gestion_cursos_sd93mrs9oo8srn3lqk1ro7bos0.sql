COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[ií]cr[oó]' OR       cod_asignatura ~* 'm[ií]cr[oó]' OR       text(c.id) ~* 'm[ií]cr[oó]' OR       lower(profesor) ~* 'm[ií]cr[oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
