COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]l[aá]rc[oó]n' OR       cod_asignatura ~* '[aá]l[aá]rc[oó]n' OR       text(c.id) ~* '[aá]l[aá]rc[oó]n' OR       lower(profesor) ~* '[aá]l[aá]rc[oó]n')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
