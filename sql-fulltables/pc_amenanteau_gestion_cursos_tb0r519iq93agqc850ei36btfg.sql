COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú]nd[aá]' OR       cod_asignatura ~* 'f[uú]nd[aá]' OR       text(c.id) ~* 'f[uú]nd[aá]' OR       lower(profesor) ~* 'f[uú]nd[aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
