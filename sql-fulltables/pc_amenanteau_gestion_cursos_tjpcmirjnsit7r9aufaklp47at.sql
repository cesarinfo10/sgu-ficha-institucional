COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[eé]nt[aá]' OR       cod_asignatura ~* 'v[eé]nt[aá]' OR       text(c.id) ~* 'v[eé]nt[aá]' OR       lower(profesor) ~* 'v[eé]nt[aá]')  AND array[101,62,26,88,18,3,36,67,156,159,12] && (ids_carreras)  
