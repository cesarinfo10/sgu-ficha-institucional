COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]s[ií]s' OR       cod_asignatura ~* 't[eé]s[ií]s' OR       text(c.id) ~* 't[eé]s[ií]s' OR       lower(profesor) ~* 't[eé]s[ií]s')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
