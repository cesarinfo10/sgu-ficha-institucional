COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[ií]n[aá]nz[aá]s' OR       cod_asignatura ~* 'f[ií]n[aá]nz[aá]s' OR       text(c.id) ~* 'f[ií]n[aá]nz[aá]s' OR       lower(profesor) ~* 'f[ií]n[aá]nz[aá]s')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
