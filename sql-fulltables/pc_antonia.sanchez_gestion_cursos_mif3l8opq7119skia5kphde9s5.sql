COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]n[aá]l' OR       cod_asignatura ~* 'p[eé]n[aá]l' OR       text(c.id) ~* 'p[eé]n[aá]l' OR       lower(profesor) ~* 'p[eé]n[aá]l')  AND (lower(asignatura) ~* '[ií]nt[eé]rn[aá]c[ií][oó]n[aá]l' OR       cod_asignatura ~* '[ií]nt[eé]rn[aá]c[ií][oó]n[aá]l' OR       text(c.id) ~* '[ií]nt[eé]rn[aá]c[ií][oó]n[aá]l' OR       lower(profesor) ~* '[ií]nt[eé]rn[aá]c[ií][oó]n[aá]l')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
