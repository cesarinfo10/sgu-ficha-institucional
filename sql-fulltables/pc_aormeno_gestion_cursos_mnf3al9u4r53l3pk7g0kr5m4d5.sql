COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]sc[aá]r' OR       cod_asignatura ~* '[oó]sc[aá]r' OR       text(c.id) ~* '[oó]sc[aá]r' OR       lower(profesor) ~* '[oó]sc[aá]r')  AND (lower(asignatura) ~* '[aá]lfr[eé]d[oó]' OR       cod_asignatura ~* '[aá]lfr[eé]d[oó]' OR       text(c.id) ~* '[aá]lfr[eé]d[oó]' OR       lower(profesor) ~* '[aá]lfr[eé]d[oó]')  AND (lower(asignatura) ~* 'r[oó]j[aá]s' OR       cod_asignatura ~* 'r[oó]j[aá]s' OR       text(c.id) ~* 'r[oó]j[aá]s' OR       lower(profesor) ~* 'r[oó]j[aá]s')  AND (lower(asignatura) ~* 'c[aá]rr[aá]sc[oó]' OR       cod_asignatura ~* 'c[aá]rr[aá]sc[oó]' OR       text(c.id) ~* 'c[aá]rr[aá]sc[oó]' OR       lower(profesor) ~* 'c[aá]rr[aá]sc[oó]')  AND array[59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12] && (ids_carreras)  
