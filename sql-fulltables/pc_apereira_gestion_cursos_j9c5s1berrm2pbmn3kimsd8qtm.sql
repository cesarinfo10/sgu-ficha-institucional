COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ntr' OR       cod_asignatura ~* '[ií]ntr' OR       text(c.id) ~* '[ií]ntr' OR       lower(profesor) ~* '[ií]ntr')  AND (lower(asignatura) ~* 'v[ií]d[aá]' OR       cod_asignatura ~* 'v[ií]d[aá]' OR       text(c.id) ~* 'v[ií]d[aá]' OR       lower(profesor) ~* 'v[ií]d[aá]')  
