COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[aá]ct[ií]c[aá]' OR       cod_asignatura ~* 'pr[aá]ct[ií]c[aá]' OR       text(c.id) ~* 'pr[aá]ct[ií]c[aá]' OR       lower(profesor) ~* 'pr[aá]ct[ií]c[aá]')  
