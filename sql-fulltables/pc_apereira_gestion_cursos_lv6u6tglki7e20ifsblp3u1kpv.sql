COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'b[aá]s[eé]s' OR       cod_asignatura ~* 'b[aá]s[eé]s' OR       text(c.id) ~* 'b[aá]s[eé]s' OR       lower(profesor) ~* 'b[aá]s[eé]s')  AND (lower(asignatura) ~* '[eé]p[ií]st[eé]m[oó]l[oó]g[ií]c[aá]s' OR       cod_asignatura ~* '[eé]p[ií]st[eé]m[oó]l[oó]g[ií]c[aá]s' OR       text(c.id) ~* '[eé]p[ií]st[eé]m[oó]l[oó]g[ií]c[aá]s' OR       lower(profesor) ~* '[eé]p[ií]st[eé]m[oó]l[oó]g[ií]c[aá]s')  
