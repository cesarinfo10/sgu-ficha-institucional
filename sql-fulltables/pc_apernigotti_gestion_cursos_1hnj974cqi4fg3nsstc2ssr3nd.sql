COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'y[aá]ñ[eé]z' OR       cod_asignatura ~* 'y[aá]ñ[eé]z' OR       text(c.id) ~* 'y[aá]ñ[eé]z' OR       lower(profesor) ~* 'y[aá]ñ[eé]z')  AND id_carrera IN (95,100,19,2,69,1,12)  
