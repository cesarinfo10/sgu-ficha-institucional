COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'b[ií][eé]n[eé]s' OR       cod_asignatura ~* 'b[ií][eé]n[eé]s' OR       text(c.id) ~* 'b[ií][eé]n[eé]s' OR       lower(profesor) ~* 'b[ií][eé]n[eé]s')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
