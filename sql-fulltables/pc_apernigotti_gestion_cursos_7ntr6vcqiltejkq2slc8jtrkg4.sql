COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[oó]rm[aá]' OR       cod_asignatura ~* 'n[oó]rm[aá]' OR       text(c.id) ~* 'n[oó]rm[aá]' OR       lower(profesor) ~* 'n[oó]rm[aá]')  AND id_carrera IN (95,100,19,2,69,1,12)  
