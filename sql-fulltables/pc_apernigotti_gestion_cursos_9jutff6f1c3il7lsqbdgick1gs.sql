COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       cod_asignatura ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       text(c.id) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       lower(profesor) ~* 'p[eé]rn[ií]g[oó]tt[ií]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
