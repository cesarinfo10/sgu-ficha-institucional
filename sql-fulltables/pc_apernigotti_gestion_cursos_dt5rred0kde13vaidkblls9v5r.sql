COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[oó]s' OR       cod_asignatura ~* 'c[aá]rl[oó]s' OR       text(c.id) ~* 'c[aá]rl[oó]s' OR       lower(profesor) ~* 'c[aá]rl[oó]s')  AND (lower(asignatura) ~* 'v[eé]rg[aá]r[aá]' OR       cod_asignatura ~* 'v[eé]rg[aá]r[aá]' OR       text(c.id) ~* 'v[eé]rg[aá]r[aá]' OR       lower(profesor) ~* 'v[eé]rg[aá]r[aá]')  AND id_carrera IN (95,100,19,2,69,1,12)  
