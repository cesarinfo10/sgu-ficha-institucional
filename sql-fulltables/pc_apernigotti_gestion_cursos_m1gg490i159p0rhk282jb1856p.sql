COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú][eé]nt[eé]s' OR       cod_asignatura ~* 'f[uú][eé]nt[eé]s' OR       text(c.id) ~* 'f[uú][eé]nt[eé]s' OR       lower(profesor) ~* 'f[uú][eé]nt[eé]s')  AND id_carrera IN (95,100,19,2,69,1,12)  
