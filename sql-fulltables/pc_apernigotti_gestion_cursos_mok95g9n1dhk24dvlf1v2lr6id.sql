COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[oó]c[ií][eé]d[aá]d[eé]s' OR       cod_asignatura ~* 's[oó]c[ií][eé]d[aá]d[eé]s' OR       text(c.id) ~* 's[oó]c[ií][eé]d[aá]d[eé]s' OR       lower(profesor) ~* 's[oó]c[ií][eé]d[aá]d[eé]s')  AND id_carrera IN (95,100,19,2,69,1,12)  
