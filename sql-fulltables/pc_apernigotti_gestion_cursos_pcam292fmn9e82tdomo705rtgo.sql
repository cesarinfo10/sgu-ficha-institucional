COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]rq[uú][eé]z' OR       cod_asignatura ~* 'm[aá]rq[uú][eé]z' OR       text(c.id) ~* 'm[aá]rq[uú][eé]z' OR       lower(profesor) ~* 'm[aá]rq[uú][eé]z')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
