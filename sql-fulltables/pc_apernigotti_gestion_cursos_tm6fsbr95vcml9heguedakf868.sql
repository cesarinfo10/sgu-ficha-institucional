COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR       cod_asignatura ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR       text(c.id) ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR       lower(profesor) ~* 'f[uú][eé]nt[eé][aá]lb[aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
