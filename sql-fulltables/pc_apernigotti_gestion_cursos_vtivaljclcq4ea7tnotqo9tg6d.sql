COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó][aá]' OR       cod_asignatura ~* 'r[oó][aá]' OR       text(c.id) ~* 'r[oó][aá]' OR       lower(profesor) ~* 'r[oó][aá]')  AND id_carrera IN (95,100,19,2,69,1,12)  
