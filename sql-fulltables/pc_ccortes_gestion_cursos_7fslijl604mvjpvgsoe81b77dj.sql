COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '18646' OR       cod_asignatura ~* '18646' OR       text(c.id) ~* '18646' OR       lower(profesor) ~* '18646')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
