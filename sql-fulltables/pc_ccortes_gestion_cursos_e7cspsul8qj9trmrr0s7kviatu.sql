COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]s[aá]n[oó]v[aá]' OR       cod_asignatura ~* 'c[aá]s[aá]n[oó]v[aá]' OR       text(c.id) ~* 'c[aá]s[aá]n[oó]v[aá]' OR       lower(profesor) ~* 'c[aá]s[aá]n[oó]v[aá]')  AND (lower(asignatura) ~* 't[ií]c[ií][aá]n[aá]' OR       cod_asignatura ~* 't[ií]c[ií][aá]n[aá]' OR       text(c.id) ~* 't[ií]c[ií][aá]n[aá]' OR       lower(profesor) ~* 't[ií]c[ií][aá]n[aá]')  AND id_carrera IN (95,100,19,2,69,1,12)  
