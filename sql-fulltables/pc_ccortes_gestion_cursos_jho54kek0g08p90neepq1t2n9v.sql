COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[ií]t[uú]l[oó]s' OR       cod_asignatura ~* 't[ií]t[uú]l[oó]s' OR       text(c.id) ~* 't[ií]t[uú]l[oó]s' OR       lower(profesor) ~* 't[ií]t[uú]l[oó]s')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'cr[eé]d[ií]t[oó]' OR       cod_asignatura ~* 'cr[eé]d[ií]t[oó]' OR       text(c.id) ~* 'cr[eé]d[ií]t[oó]' OR       lower(profesor) ~* 'cr[eé]d[ií]t[oó]')  AND id_carrera IN (95,100,2,69,1,19,12)  
