COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[eé]l[ií]s' OR       cod_asignatura ~* 'c[eé]l[ií]s' OR       text(c.id) ~* 'c[eé]l[ií]s' OR       lower(profesor) ~* 'c[eé]l[ií]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
