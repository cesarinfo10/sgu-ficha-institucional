COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[eé]nny' OR       cod_asignatura ~* 'j[eé]nny' OR       text(c.id) ~* 'j[eé]nny' OR       lower(profesor) ~* 'j[eé]nny')  AND id_carrera IN (95,100,2,69,1,19,12)  
