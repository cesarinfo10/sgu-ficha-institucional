COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[aá]rg[aá]s' OR       cod_asignatura ~* 'v[aá]rg[aá]s' OR       text(c.id) ~* 'v[aá]rg[aá]s' OR       lower(profesor) ~* 'v[aá]rg[aá]s')  AND id_carrera IN (95,100,2,69,1,19,12)  
