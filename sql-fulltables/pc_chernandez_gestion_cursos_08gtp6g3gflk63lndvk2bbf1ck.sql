COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       cod_asignatura ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       text(c.id) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       lower(profesor) ~* 'f[uú]nd[aá]m[eé]nt[oó]s')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
