COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[aá]ct[ií]c[aá]' OR       cod_asignatura ~* 'pr[aá]ct[ií]c[aá]' OR       text(c.id) ~* 'pr[aá]ct[ií]c[aá]' OR       lower(profesor) ~* 'pr[aá]ct[ií]c[aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
