COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]rt[eé]s' OR       cod_asignatura ~* 'c[oó]rt[eé]s' OR       text(c.id) ~* 'c[oó]rt[eé]s' OR       lower(profesor) ~* 'c[oó]rt[eé]s')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
