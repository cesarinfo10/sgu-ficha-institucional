COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[aá]y[ií]b[eé]' OR       cod_asignatura ~* 'n[aá]y[ií]b[eé]' OR       text(c.id) ~* 'n[aá]y[ií]b[eé]' OR       lower(profesor) ~* 'n[aá]y[ií]b[eé]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
