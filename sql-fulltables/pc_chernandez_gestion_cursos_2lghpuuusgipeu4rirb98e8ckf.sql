COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]t[ií]c[aá]' OR       cod_asignatura ~* '[eé]t[ií]c[aá]' OR       text(c.id) ~* '[eé]t[ií]c[aá]' OR       lower(profesor) ~* '[eé]t[ií]c[aá]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
