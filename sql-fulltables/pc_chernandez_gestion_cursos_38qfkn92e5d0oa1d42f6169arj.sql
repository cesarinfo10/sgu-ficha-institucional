COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[oó]y[eé]ct[oó]' OR       cod_asignatura ~* 'pr[oó]y[eé]ct[oó]' OR       text(c.id) ~* 'pr[oó]y[eé]ct[oó]' OR       lower(profesor) ~* 'pr[oó]y[eé]ct[oó]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
