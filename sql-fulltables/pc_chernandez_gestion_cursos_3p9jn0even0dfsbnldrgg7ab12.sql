COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[aá]ll[aá]rd[oó]' OR       cod_asignatura ~* 'g[aá]ll[aá]rd[oó]' OR       text(c.id) ~* 'g[aá]ll[aá]rd[oó]' OR       lower(profesor) ~* 'g[aá]ll[aá]rd[oó]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
