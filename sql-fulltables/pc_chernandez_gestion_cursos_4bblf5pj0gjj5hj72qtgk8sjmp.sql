COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'cl[ií]n[ií]c[aá]' OR       cod_asignatura ~* 'cl[ií]n[ií]c[aá]' OR       text(c.id) ~* 'cl[ií]n[ií]c[aá]' OR       lower(profesor) ~* 'cl[ií]n[ií]c[aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
