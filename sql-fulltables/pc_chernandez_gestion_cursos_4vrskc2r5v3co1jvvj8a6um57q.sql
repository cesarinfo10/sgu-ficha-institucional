COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]n[oó]rm[aá]l' OR       cod_asignatura ~* '[aá]n[oó]rm[aá]l' OR       text(c.id) ~* '[aá]n[oó]rm[aá]l' OR       lower(profesor) ~* '[aá]n[oó]rm[aá]l')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
