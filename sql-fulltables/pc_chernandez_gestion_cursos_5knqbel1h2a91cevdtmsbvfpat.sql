COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[ií]n[ií]ll[aá]' OR       cod_asignatura ~* 'p[ií]n[ií]ll[aá]' OR       text(c.id) ~* 'p[ií]n[ií]ll[aá]' OR       lower(profesor) ~* 'p[ií]n[ií]ll[aá]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
