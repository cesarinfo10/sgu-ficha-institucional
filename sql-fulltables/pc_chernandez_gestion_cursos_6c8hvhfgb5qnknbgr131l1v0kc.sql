COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[oó]c[ií][aá]l' OR       cod_asignatura ~* 's[oó]c[ií][aá]l' OR       text(c.id) ~* 's[oó]c[ií][aá]l' OR       lower(profesor) ~* 's[oó]c[ií][aá]l')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
