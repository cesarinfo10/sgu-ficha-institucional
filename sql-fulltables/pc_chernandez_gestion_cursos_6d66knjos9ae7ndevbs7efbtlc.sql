COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '18572' OR       cod_asignatura ~* '18572' OR       text(c.id) ~* '18572' OR       lower(profesor) ~* '18572')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
