COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[ií]cl[oó]' OR       cod_asignatura ~* 'c[ií]cl[oó]' OR       text(c.id) ~* 'c[ií]cl[oó]' OR       lower(profesor) ~* 'c[ií]cl[oó]')  AND (lower(asignatura) ~* 'v[ií]t[aá]l' OR       cod_asignatura ~* 'v[ií]t[aá]l' OR       text(c.id) ~* 'v[ií]t[aá]l' OR       lower(profesor) ~* 'v[ií]t[aá]l')  AND (lower(asignatura) ~* '[ií]' OR       cod_asignatura ~* '[ií]' OR       text(c.id) ~* '[ií]' OR       lower(profesor) ~* '[ií]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
