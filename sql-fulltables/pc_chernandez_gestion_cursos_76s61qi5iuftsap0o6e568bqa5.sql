COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rt[ií]z' OR       cod_asignatura ~* '[oó]rt[ií]z' OR       text(c.id) ~* '[oó]rt[ií]z' OR       lower(profesor) ~* '[oó]rt[ií]z')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
