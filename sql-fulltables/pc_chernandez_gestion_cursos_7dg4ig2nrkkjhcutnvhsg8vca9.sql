COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       cod_asignatura ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       text(c.id) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       lower(profesor) ~* 'h[uú]m[aá]n[ií]sm[oó]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
