COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[eé]j[ií][aá]s' OR       cod_asignatura ~* 'm[eé]j[ií][aá]s' OR       text(c.id) ~* 'm[eé]j[ií][aá]s' OR       lower(profesor) ~* 'm[eé]j[ií][aá]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
