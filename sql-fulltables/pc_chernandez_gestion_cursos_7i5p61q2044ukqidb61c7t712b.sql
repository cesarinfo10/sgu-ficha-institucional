COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[aá]r[aá]y' OR       cod_asignatura ~* 'g[aá]r[aá]y' OR       text(c.id) ~* 'g[aá]r[aá]y' OR       lower(profesor) ~* 'g[aá]r[aá]y')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
