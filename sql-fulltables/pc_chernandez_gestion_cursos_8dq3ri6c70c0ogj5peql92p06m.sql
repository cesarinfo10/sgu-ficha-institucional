COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n' OR       cod_asignatura ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n' OR       text(c.id) ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n' OR       lower(profesor) ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
