COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[oó]c[eé]s[oó]s' OR       cod_asignatura ~* 'pr[oó]c[eé]s[oó]s' OR       text(c.id) ~* 'pr[oó]c[eé]s[oó]s' OR       lower(profesor) ~* 'pr[oó]c[eé]s[oó]s')  AND (lower(asignatura) ~* '[ií]' OR       cod_asignatura ~* '[ií]' OR       text(c.id) ~* '[ií]' OR       lower(profesor) ~* '[ií]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
