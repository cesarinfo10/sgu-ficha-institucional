COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]d[uú]c[aá]c[ií][oó]n[aá]l' OR       cod_asignatura ~* '[eé]d[uú]c[aá]c[ií][oó]n[aá]l' OR       text(c.id) ~* '[eé]d[uú]c[aá]c[ií][oó]n[aá]l' OR       lower(profesor) ~* '[eé]d[uú]c[aá]c[ií][oó]n[aá]l')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
