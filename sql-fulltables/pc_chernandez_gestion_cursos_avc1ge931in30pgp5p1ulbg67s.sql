COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá]tr[ií]c[ií][aá]' OR       cod_asignatura ~* 'p[aá]tr[ií]c[ií][aá]' OR       text(c.id) ~* 'p[aá]tr[ií]c[ií][aá]' OR       lower(profesor) ~* 'p[aá]tr[ií]c[ií][aá]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
