COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       cod_asignatura ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       text(c.id) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       lower(profesor) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
