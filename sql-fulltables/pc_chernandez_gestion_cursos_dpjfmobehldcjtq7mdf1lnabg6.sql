COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[eé]t[ií]c[ií][aá]' OR       cod_asignatura ~* 'l[eé]t[ií]c[ií][aá]' OR       text(c.id) ~* 'l[eé]t[ií]c[ií][aá]' OR       lower(profesor) ~* 'l[eé]t[ií]c[ií][aá]')  AND (lower(asignatura) ~* 'v[ií]lch[eé]s' OR       cod_asignatura ~* 'v[ií]lch[eé]s' OR       text(c.id) ~* 'v[ií]lch[eé]s' OR       lower(profesor) ~* 'v[ií]lch[eé]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
