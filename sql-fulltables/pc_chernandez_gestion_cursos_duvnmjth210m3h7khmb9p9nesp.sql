COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[uú][ií]s[aá]' OR       cod_asignatura ~* 'l[uú][ií]s[aá]' OR       text(c.id) ~* 'l[uú][ií]s[aá]' OR       lower(profesor) ~* 'l[uú][ií]s[aá]')  AND (lower(asignatura) ~* '[oó]l[ií]v[ií][aá]' OR       cod_asignatura ~* '[oó]l[ií]v[ií][aá]' OR       text(c.id) ~* '[oó]l[ií]v[ií][aá]' OR       lower(profesor) ~* '[oó]l[ií]v[ií][aá]')  AND (lower(asignatura) ~* 'm[eé]j[ií][aá]s' OR       cod_asignatura ~* 'm[eé]j[ií][aá]s' OR       text(c.id) ~* 'm[eé]j[ií][aá]s' OR       lower(profesor) ~* 'm[eé]j[ií][aá]s')  AND (lower(asignatura) ~* 'b[aá]nd[aá]' OR       cod_asignatura ~* 'b[aá]nd[aá]' OR       text(c.id) ~* 'b[aá]nd[aá]' OR       lower(profesor) ~* 'b[aá]nd[aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
