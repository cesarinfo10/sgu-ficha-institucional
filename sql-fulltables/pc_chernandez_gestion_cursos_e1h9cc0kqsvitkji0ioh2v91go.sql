COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]x[uú][aá]l[ií]d[aá]d' OR       cod_asignatura ~* 's[eé]x[uú][aá]l[ií]d[aá]d' OR       text(c.id) ~* 's[eé]x[uú][aá]l[ií]d[aá]d' OR       lower(profesor) ~* 's[eé]x[uú][aá]l[ií]d[aá]d')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
