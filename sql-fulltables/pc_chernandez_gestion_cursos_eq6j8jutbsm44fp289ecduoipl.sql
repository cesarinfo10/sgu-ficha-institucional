COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[uú]nd[oó]' OR       cod_asignatura ~* 'm[uú]nd[oó]' OR       text(c.id) ~* 'm[uú]nd[oó]' OR       lower(profesor) ~* 'm[uú]nd[oó]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,157,12] && (ids_carreras)  
