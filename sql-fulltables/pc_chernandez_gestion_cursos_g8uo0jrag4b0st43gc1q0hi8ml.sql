COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[aá]y[ií]b[eé]' OR       cod_asignatura ~* 'n[aá]y[ií]b[eé]' OR       text(c.id) ~* 'n[aá]y[ií]b[eé]' OR       lower(profesor) ~* 'n[aá]y[ií]b[eé]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
