COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé][ií]n[oó]' OR       cod_asignatura ~* 'd[eé][ií]n[oó]' OR       text(c.id) ~* 'd[eé][ií]n[oó]' OR       lower(profesor) ~* 'd[eé][ií]n[oó]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
