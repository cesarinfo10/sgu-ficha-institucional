COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]s[ií]n[aá]' OR       cod_asignatura ~* 't[eé]s[ií]n[aá]' OR       text(c.id) ~* 't[eé]s[ií]n[aá]' OR       lower(profesor) ~* 't[eé]s[ií]n[aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
