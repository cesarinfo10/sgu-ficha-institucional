COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]ll[aá]r' OR       cod_asignatura ~* 'v[ií]ll[aá]r' OR       text(c.id) ~* 'v[ií]ll[aá]r' OR       lower(profesor) ~* 'v[ií]ll[aá]r')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
