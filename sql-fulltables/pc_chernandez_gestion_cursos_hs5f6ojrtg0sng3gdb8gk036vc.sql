COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       cod_asignatura ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       text(c.id) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       lower(profesor) ~* 'f[uú]nd[aá]m[eé]nt[oó]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
