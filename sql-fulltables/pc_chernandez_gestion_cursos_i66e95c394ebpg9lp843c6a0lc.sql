COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]ss[oó]n' OR       cod_asignatura ~* 'm[aá]ss[oó]n' OR       text(c.id) ~* 'm[aá]ss[oó]n' OR       lower(profesor) ~* 'm[aá]ss[oó]n')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
