COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'cl[ií]n[ií]c[aá]' OR       cod_asignatura ~* 'cl[ií]n[ií]c[aá]' OR       text(c.id) ~* 'cl[ií]n[ií]c[aá]' OR       lower(profesor) ~* 'cl[ií]n[ií]c[aá]')  AND (lower(asignatura) ~* 'd[eé]l' OR       cod_asignatura ~* 'd[eé]l' OR       text(c.id) ~* 'd[eé]l' OR       lower(profesor) ~* 'd[eé]l')  AND (lower(asignatura) ~* '[aá]d[uú]lt[oó]' OR       cod_asignatura ~* '[aá]d[uú]lt[oó]' OR       text(c.id) ~* '[aá]d[uú]lt[oó]' OR       lower(profesor) ~* '[aá]d[uú]lt[oó]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
