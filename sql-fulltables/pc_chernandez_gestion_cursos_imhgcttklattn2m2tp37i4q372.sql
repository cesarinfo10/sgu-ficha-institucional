COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]d[eé]c[ií]d[oó]' OR       cod_asignatura ~* 'c[oó]d[eé]c[ií]d[oó]' OR       text(c.id) ~* 'c[oó]d[eé]c[ií]d[oó]' OR       lower(profesor) ~* 'c[oó]d[eé]c[ií]d[oó]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,157,12] && (ids_carreras)  
