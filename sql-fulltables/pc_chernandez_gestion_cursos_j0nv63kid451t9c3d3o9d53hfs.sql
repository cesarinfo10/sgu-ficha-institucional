COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]br[eé]r[aá]' OR       cod_asignatura ~* 'c[aá]br[eé]r[aá]' OR       text(c.id) ~* 'c[aá]br[eé]r[aá]' OR       lower(profesor) ~* 'c[aá]br[eé]r[aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
