COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]ct[oó]r' OR       cod_asignatura ~* 'v[ií]ct[oó]r' OR       text(c.id) ~* 'v[ií]ct[oó]r' OR       lower(profesor) ~* 'v[ií]ct[oó]r')  AND (lower(asignatura) ~* 'r[aá][uú]l' OR       cod_asignatura ~* 'r[aá][uú]l' OR       text(c.id) ~* 'r[aá][uú]l' OR       lower(profesor) ~* 'r[aá][uú]l')  AND (lower(asignatura) ~* '[oó]rt[ií]z' OR       cod_asignatura ~* '[oó]rt[ií]z' OR       text(c.id) ~* '[oó]rt[ií]z' OR       lower(profesor) ~* '[oó]rt[ií]z')  AND (lower(asignatura) ~* 'c[oó]ntr[eé]r[aá]s' OR       cod_asignatura ~* 'c[oó]ntr[eé]r[aá]s' OR       text(c.id) ~* 'c[oó]ntr[eé]r[aá]s' OR       lower(profesor) ~* 'c[oó]ntr[eé]r[aá]s')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
