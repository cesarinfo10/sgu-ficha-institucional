COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[uú]v[eé]n[aá]l' OR       cod_asignatura ~* 'j[uú]v[eé]n[aá]l' OR       text(c.id) ~* 'j[uú]v[eé]n[aá]l' OR       lower(profesor) ~* 'j[uú]v[eé]n[aá]l')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
