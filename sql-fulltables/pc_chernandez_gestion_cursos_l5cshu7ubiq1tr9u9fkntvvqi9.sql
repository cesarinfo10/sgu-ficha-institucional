COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[oó]bl[eé]t[eé]' OR       cod_asignatura ~* 'p[oó]bl[eé]t[eé]' OR       text(c.id) ~* 'p[oó]bl[eé]t[eé]' OR       lower(profesor) ~* 'p[oó]bl[eé]t[eé]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
