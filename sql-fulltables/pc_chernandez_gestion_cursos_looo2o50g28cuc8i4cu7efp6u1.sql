COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]p[ií]st[eé]' OR       cod_asignatura ~* '[eé]p[ií]st[eé]' OR       text(c.id) ~* '[eé]p[ií]st[eé]' OR       lower(profesor) ~* '[eé]p[ií]st[eé]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
