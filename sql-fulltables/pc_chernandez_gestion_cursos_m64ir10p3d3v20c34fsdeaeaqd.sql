COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]lm[oó]n[aá]c[ií]d' OR       cod_asignatura ~* '[aá]lm[oó]n[aá]c[ií]d' OR       text(c.id) ~* '[aá]lm[oó]n[aá]c[ií]d' OR       lower(profesor) ~* '[aá]lm[oó]n[aá]c[ií]d')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
