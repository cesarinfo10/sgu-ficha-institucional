COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[ií]sc[ií]ll[aá]' OR       cod_asignatura ~* 'pr[ií]sc[ií]ll[aá]' OR       text(c.id) ~* 'pr[ií]sc[ií]ll[aá]' OR       lower(profesor) ~* 'pr[ií]sc[ií]ll[aá]')  AND (lower(asignatura) ~* 'r[uú]th' OR       cod_asignatura ~* 'r[uú]th' OR       text(c.id) ~* 'r[uú]th' OR       lower(profesor) ~* 'r[uú]th')  AND (lower(asignatura) ~* 'p[uú][eé]nt[eé]s' OR       cod_asignatura ~* 'p[uú][eé]nt[eé]s' OR       text(c.id) ~* 'p[uú][eé]nt[eé]s' OR       lower(profesor) ~* 'p[uú][eé]nt[eé]s')  AND (lower(asignatura) ~* 'r[eé]y[eé]s' OR       cod_asignatura ~* 'r[eé]y[eé]s' OR       text(c.id) ~* 'r[eé]y[eé]s' OR       lower(profesor) ~* 'r[eé]y[eé]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
