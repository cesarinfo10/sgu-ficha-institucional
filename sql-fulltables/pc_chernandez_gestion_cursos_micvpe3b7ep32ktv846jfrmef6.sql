COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[eé]v[aá]d[oó]' OR       cod_asignatura ~* 'n[eé]v[aá]d[oó]' OR       text(c.id) ~* 'n[eé]v[aá]d[oó]' OR       lower(profesor) ~* 'n[eé]v[aá]d[oó]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
