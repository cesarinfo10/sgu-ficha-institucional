COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]n[oó]rm[aá]l' OR       cod_asignatura ~* '[aá]n[oó]rm[aá]l' OR       text(c.id) ~* '[aá]n[oó]rm[aá]l' OR       lower(profesor) ~* '[aá]n[oó]rm[aá]l')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
