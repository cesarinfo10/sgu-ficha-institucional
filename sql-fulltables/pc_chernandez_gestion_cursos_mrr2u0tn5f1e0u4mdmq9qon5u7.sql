COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '5117' OR       cod_asignatura ~* '5117' OR       text(c.id) ~* '5117' OR       lower(profesor) ~* '5117')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
