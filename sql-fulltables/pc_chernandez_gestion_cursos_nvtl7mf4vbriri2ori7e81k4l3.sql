COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[aá]lc[uú]n[eé]r[ií]' OR       cod_asignatura ~* 'f[aá]lc[uú]n[eé]r[ií]' OR       text(c.id) ~* 'f[aá]lc[uú]n[eé]r[ií]' OR       lower(profesor) ~* 'f[aá]lc[uú]n[eé]r[ií]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
