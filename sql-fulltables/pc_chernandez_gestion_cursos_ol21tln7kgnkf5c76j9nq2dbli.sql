COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]lv[aá]r[oó]' OR       cod_asignatura ~* '[aá]lv[aá]r[oó]' OR       text(c.id) ~* '[aá]lv[aá]r[oó]' OR       lower(profesor) ~* '[aá]lv[aá]r[oó]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
