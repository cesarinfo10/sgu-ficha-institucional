COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[aá]y[ií]b[eé]' OR       cod_asignatura ~* 'n[aá]y[ií]b[eé]' OR       text(c.id) ~* 'n[aá]y[ií]b[eé]' OR       lower(profesor) ~* 'n[aá]y[ií]b[eé]')  AND (lower(asignatura) ~* '[aá]br[aá]h[aá]m' OR       cod_asignatura ~* '[aá]br[aá]h[aá]m' OR       text(c.id) ~* '[aá]br[aá]h[aá]m' OR       lower(profesor) ~* '[aá]br[aá]h[aá]m')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'c[oó]h[eé]n' OR       cod_asignatura ~* 'c[oó]h[eé]n' OR       text(c.id) ~* 'c[oó]h[eé]n' OR       lower(profesor) ~* 'c[oó]h[eé]n')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
