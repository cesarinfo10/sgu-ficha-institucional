COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]d[ií]cc[ií][oó]n[eé]s' OR       cod_asignatura ~* '[aá]d[ií]cc[ií][oó]n[eé]s' OR       text(c.id) ~* '[aá]d[ií]cc[ií][oó]n[eé]s' OR       lower(profesor) ~* '[aá]d[ií]cc[ií][oó]n[eé]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
