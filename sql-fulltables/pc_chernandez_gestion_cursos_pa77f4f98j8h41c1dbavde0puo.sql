COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]rr[eé][aá]' OR       cod_asignatura ~* 'c[oó]rr[eé][aá]' OR       text(c.id) ~* 'c[oó]rr[eé][aá]' OR       lower(profesor) ~* 'c[oó]rr[eé][aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
