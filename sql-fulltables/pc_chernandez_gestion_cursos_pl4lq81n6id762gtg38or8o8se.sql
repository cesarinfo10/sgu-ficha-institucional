COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá]n[oó]r[aá]m[aá]' OR       cod_asignatura ~* 'p[aá]n[oó]r[aá]m[aá]' OR       text(c.id) ~* 'p[aá]n[oó]r[aá]m[aá]' OR       lower(profesor) ~* 'p[aá]n[oó]r[aá]m[aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
