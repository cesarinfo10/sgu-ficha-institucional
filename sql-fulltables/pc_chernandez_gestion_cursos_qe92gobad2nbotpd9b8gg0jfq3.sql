COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[oó]r[eé]ns[eé]' OR       cod_asignatura ~* 'f[oó]r[eé]ns[eé]' OR       text(c.id) ~* 'f[oó]r[eé]ns[eé]' OR       lower(profesor) ~* 'f[oó]r[eé]ns[eé]')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
