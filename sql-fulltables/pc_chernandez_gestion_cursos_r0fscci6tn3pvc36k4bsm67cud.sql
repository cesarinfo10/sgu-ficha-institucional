COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       cod_asignatura ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       text(c.id) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       lower(profesor) ~* 's[aá]nt[ií]b[aá]ñ[eé]z')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
