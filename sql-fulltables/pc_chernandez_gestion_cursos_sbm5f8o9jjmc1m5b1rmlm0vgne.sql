COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[eé]nr[ií]q[uú][eé]z' OR       cod_asignatura ~* 'h[eé]nr[ií]q[uú][eé]z' OR       text(c.id) ~* 'h[eé]nr[ií]q[uú][eé]z' OR       lower(profesor) ~* 'h[eé]nr[ií]q[uú][eé]z')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
