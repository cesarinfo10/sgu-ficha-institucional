COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]s[aá]rr[oó]ll[oó]' OR       cod_asignatura ~* 'd[eé]s[aá]rr[oó]ll[oó]' OR       text(c.id) ~* 'd[eé]s[aá]rr[oó]ll[oó]' OR       lower(profesor) ~* 'd[eé]s[aá]rr[oó]ll[oó]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
