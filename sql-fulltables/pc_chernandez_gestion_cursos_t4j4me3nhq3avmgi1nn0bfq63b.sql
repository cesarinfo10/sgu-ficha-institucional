COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'ps[ií]c[oó]f[ií]s' OR       cod_asignatura ~* 'ps[ií]c[oó]f[ií]s' OR       text(c.id) ~* 'ps[ií]c[oó]f[ií]s' OR       lower(profesor) ~* 'ps[ií]c[oó]f[ií]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
