COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé][oó]r[ií][aá]s' OR       cod_asignatura ~* 't[eé][oó]r[ií][aá]s' OR       text(c.id) ~* 't[eé][oó]r[ií][aá]s' OR       lower(profesor) ~* 't[eé][oó]r[ií][aá]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
