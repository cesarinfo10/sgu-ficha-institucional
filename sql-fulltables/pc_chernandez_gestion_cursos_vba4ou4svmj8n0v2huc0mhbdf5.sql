COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d[eé]s' OR       cod_asignatura ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d[eé]s' OR       text(c.id) ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d[eé]s' OR       lower(profesor) ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d[eé]s')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
