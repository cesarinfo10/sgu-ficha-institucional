COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[ií]cl[oó]' OR       cod_asignatura ~* 'c[ií]cl[oó]' OR       text(c.id) ~* 'c[ií]cl[oó]' OR       lower(profesor) ~* 'c[ií]cl[oó]')  AND array[94,97,104,110,98,105,102,91,15,103,126,157,4,12] && (ids_carreras)  
