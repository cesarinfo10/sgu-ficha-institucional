COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[aá]l[uú]d' OR       cod_asignatura ~* 's[aá]l[uú]d' OR       text(c.id) ~* 's[aá]l[uú]d' OR       lower(profesor) ~* 's[aá]l[uú]d')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
