COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]ss[oó]n' OR       cod_asignatura ~* 'm[aá]ss[oó]n' OR       text(c.id) ~* 'm[aá]ss[oó]n' OR       lower(profesor) ~* 'm[aá]ss[oó]n')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
