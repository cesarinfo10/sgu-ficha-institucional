COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n' OR       cod_asignatura ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n' OR       text(c.id) ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n' OR       lower(profesor) ~* 's[eé]ns[oó]p[eé]rc[eé]pc[ií][oó]n')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  
