COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]v[aá]l[uú][aá]c[ií][oó]n' OR       cod_asignatura ~* '[eé]v[aá]l[uú][aá]c[ií][oó]n' OR       text(c.id) ~* '[eé]v[aá]l[uú][aá]c[ií][oó]n' OR       lower(profesor) ~* '[eé]v[aá]l[uú][aá]c[ií][oó]n')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'l[aá]' OR       cod_asignatura ~* 'l[aá]' OR       text(c.id) ~* 'l[aá]' OR       lower(profesor) ~* 'l[aá]')  AND (lower(asignatura) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       cod_asignatura ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       text(c.id) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d' OR       lower(profesor) ~* 'p[eé]rs[oó]n[aá]l[ií]d[aá]d')  
