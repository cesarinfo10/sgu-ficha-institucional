COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]nf[oó]rm[aá]t[ií]c[aá]' OR       cod_asignatura ~* '[ií]nf[oó]rm[aá]t[ií]c[aá]' OR       text(c.id) ~* '[ií]nf[oó]rm[aá]t[ií]c[aá]' OR       lower(profesor) ~* '[ií]nf[oó]rm[aá]t[ií]c[aá]')  AND (lower(asignatura) ~* '[aá]pl[ií]c[aá]d[aá]' OR       cod_asignatura ~* '[aá]pl[ií]c[aá]d[aá]' OR       text(c.id) ~* '[aá]pl[ií]c[aá]d[aá]' OR       lower(profesor) ~* '[aá]pl[ií]c[aá]d[aá]')  
