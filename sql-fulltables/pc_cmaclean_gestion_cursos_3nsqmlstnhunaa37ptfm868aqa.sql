COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n' OR       cod_asignatura ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n' OR       text(c.id) ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n' OR       lower(profesor) ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n')  
