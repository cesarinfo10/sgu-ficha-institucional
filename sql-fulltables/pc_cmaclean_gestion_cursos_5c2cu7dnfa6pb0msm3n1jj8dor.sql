COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[ií]st' OR       cod_asignatura ~* 's[ií]st' OR       text(c.id) ~* 's[ií]st' OR       lower(profesor) ~* 's[ií]st')  AND (lower(asignatura) ~* 'c[oó]nt[aá]' OR       cod_asignatura ~* 'c[oó]nt[aá]' OR       text(c.id) ~* 'c[oó]nt[aá]' OR       lower(profesor) ~* 'c[oó]nt[aá]')  AND c.tipo_clase = 'Presencial'  
