COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]x[uú][aá]l[ií]d[aá]d' OR       cod_asignatura ~* 's[eé]x[uú][aá]l[ií]d[aá]d' OR       text(c.id) ~* 's[eé]x[uú][aá]l[ií]d[aá]d' OR       lower(profesor) ~* 's[eé]x[uú][aá]l[ií]d[aá]d')  
