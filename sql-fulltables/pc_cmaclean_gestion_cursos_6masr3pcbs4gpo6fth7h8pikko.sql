COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[aá]' OR       cod_asignatura ~* 'c[aá]rl[aá]' OR       text(c.id) ~* 'c[aá]rl[aá]' OR       lower(profesor) ~* 'c[aá]rl[aá]')  AND (lower(asignatura) ~* 'm[aá]cl[eé][aá]n' OR       cod_asignatura ~* 'm[aá]cl[eé][aá]n' OR       text(c.id) ~* 'm[aá]cl[eé][aá]n' OR       lower(profesor) ~* 'm[aá]cl[eé][aá]n')  
