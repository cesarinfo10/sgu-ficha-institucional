COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[aá]t[ií][aá]n[aá]' OR       cod_asignatura ~* 't[aá]t[ií][aá]n[aá]' OR       text(c.id) ~* 't[aá]t[ií][aá]n[aá]' OR       lower(profesor) ~* 't[aá]t[ií][aá]n[aá]')  AND (lower(asignatura) ~* 'p[eé]r[eé]z' OR       cod_asignatura ~* 'p[eé]r[eé]z' OR       text(c.id) ~* 'p[eé]r[eé]z' OR       lower(profesor) ~* 'p[eé]r[eé]z')  
