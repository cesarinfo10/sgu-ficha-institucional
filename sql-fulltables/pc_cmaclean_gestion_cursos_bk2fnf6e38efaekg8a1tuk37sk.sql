COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n' OR       cod_asignatura ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n' OR       text(c.id) ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n' OR       lower(profesor) ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n')  AND (lower(asignatura) ~* 'y' OR       cod_asignatura ~* 'y' OR       text(c.id) ~* 'y' OR       lower(profesor) ~* 'y')  AND (lower(asignatura) ~* '[eé]scr[ií]t[uú]r[aá]' OR       cod_asignatura ~* '[eé]scr[ií]t[uú]r[aá]' OR       text(c.id) ~* '[eé]scr[ií]t[uú]r[aá]' OR       lower(profesor) ~* '[eé]scr[ií]t[uú]r[aá]')  AND (lower(asignatura) ~* '[aá]c[aá]d[eé]m[ií]c[aá]' OR       cod_asignatura ~* '[aá]c[aá]d[eé]m[ií]c[aá]' OR       text(c.id) ~* '[aá]c[aá]d[eé]m[ií]c[aá]' OR       lower(profesor) ~* '[aá]c[aá]d[eé]m[ií]c[aá]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  
