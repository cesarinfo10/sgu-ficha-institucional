COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       cod_asignatura ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       text(c.id) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       lower(profesor) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]')  AND (lower(asignatura) ~* 't[eé]c' OR       cod_asignatura ~* 't[eé]c' OR       text(c.id) ~* 't[eé]c' OR       lower(profesor) ~* 't[eé]c')  
