COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[aá]t[ií]m[aá]' OR       cod_asignatura ~* 'f[aá]t[ií]m[aá]' OR       text(c.id) ~* 'f[aá]t[ií]m[aá]' OR       lower(profesor) ~* 'f[aá]t[ií]m[aá]')  AND (lower(asignatura) ~* 'n[eé]v[aá]d[oó]' OR       cod_asignatura ~* 'n[eé]v[aá]d[oó]' OR       text(c.id) ~* 'n[eé]v[aá]d[oó]' OR       lower(profesor) ~* 'n[eé]v[aá]d[oó]')  
