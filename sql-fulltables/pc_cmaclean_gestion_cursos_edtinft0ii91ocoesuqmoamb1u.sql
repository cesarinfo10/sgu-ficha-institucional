COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]s[ií]s' OR       cod_asignatura ~* 't[eé]s[ií]s' OR       text(c.id) ~* 't[eé]s[ií]s' OR       lower(profesor) ~* 't[eé]s[ií]s')  AND (lower(asignatura) ~* '[ií][ií]' OR       cod_asignatura ~* '[ií][ií]' OR       text(c.id) ~* '[ií][ií]' OR       lower(profesor) ~* '[ií][ií]')  
