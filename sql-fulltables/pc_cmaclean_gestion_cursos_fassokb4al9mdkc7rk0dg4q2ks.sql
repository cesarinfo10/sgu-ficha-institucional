COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'ps[ií]c[oó][eé]st[aá]d[ií]st[ií]c[aá]' OR       cod_asignatura ~* 'ps[ií]c[oó][eé]st[aá]d[ií]st[ií]c[aá]' OR       text(c.id) ~* 'ps[ií]c[oó][eé]st[aá]d[ií]st[ií]c[aá]' OR       lower(profesor) ~* 'ps[ií]c[oó][eé]st[aá]d[ií]st[ií]c[aá]')  
