COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'z[aá]ñ[aá]rt[uú]' OR       cod_asignatura ~* 'z[aá]ñ[aá]rt[uú]' OR       text(c.id) ~* 'z[aá]ñ[aá]rt[uú]' OR       lower(profesor) ~* 'z[aá]ñ[aá]rt[uú]')  
