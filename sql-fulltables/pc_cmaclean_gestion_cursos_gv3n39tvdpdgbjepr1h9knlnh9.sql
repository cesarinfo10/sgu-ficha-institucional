COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[eé]l[ií]p[eé]' OR       cod_asignatura ~* 'f[eé]l[ií]p[eé]' OR       text(c.id) ~* 'f[eé]l[ií]p[eé]' OR       lower(profesor) ~* 'f[eé]l[ií]p[eé]')  AND (lower(asignatura) ~* 'v[aá]rg[aá]s' OR       cod_asignatura ~* 'v[aá]rg[aá]s' OR       text(c.id) ~* 'v[aá]rg[aá]s' OR       lower(profesor) ~* 'v[aá]rg[aá]s')  
