COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]n[aá]l[ií]s[ií]s' OR       cod_asignatura ~* '[aá]n[aá]l[ií]s[ií]s' OR       text(c.id) ~* '[aá]n[aá]l[ií]s[ií]s' OR       lower(profesor) ~* '[aá]n[aá]l[ií]s[ií]s')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'd[aá]t[oó]s' OR       cod_asignatura ~* 'd[aá]t[oó]s' OR       text(c.id) ~* 'd[aá]t[oó]s' OR       lower(profesor) ~* 'd[aá]t[oó]s')  AND (lower(asignatura) ~* '[ií]' OR       cod_asignatura ~* '[ií]' OR       text(c.id) ~* '[ií]' OR       lower(profesor) ~* '[ií]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  
