COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]d[ií]cc[ií][oó]n[eé]s' OR       cod_asignatura ~* '[aá]d[ií]cc[ií][oó]n[eé]s' OR       text(c.id) ~* '[aá]d[ií]cc[ií][oó]n[eé]s' OR       lower(profesor) ~* '[aá]d[ií]cc[ií][oó]n[eé]s')  
