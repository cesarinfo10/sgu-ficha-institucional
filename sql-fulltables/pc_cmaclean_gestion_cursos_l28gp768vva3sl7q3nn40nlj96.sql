COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rd[eé]n' OR       cod_asignatura ~* '[oó]rd[eé]n' OR       text(c.id) ~* '[oó]rd[eé]n' OR       lower(profesor) ~* '[oó]rd[eé]n')  AND (lower(asignatura) ~* 'p[uú]b' OR       cod_asignatura ~* 'p[uú]b' OR       text(c.id) ~* 'p[uú]b' OR       lower(profesor) ~* 'p[uú]b')  
