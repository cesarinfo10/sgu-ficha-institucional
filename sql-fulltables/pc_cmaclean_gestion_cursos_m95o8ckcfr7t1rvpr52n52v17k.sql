COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]lg[oó]r[ií]tm[oó]' OR       cod_asignatura ~* '[aá]lg[oó]r[ií]tm[oó]' OR       text(c.id) ~* '[aá]lg[oó]r[ií]tm[oó]' OR       lower(profesor) ~* '[aá]lg[oó]r[ií]tm[oó]')  
