COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ntr[oó]' OR       cod_asignatura ~* '[ií]ntr[oó]' OR       text(c.id) ~* '[ií]ntr[oó]' OR       lower(profesor) ~* '[ií]ntr[oó]')  AND (lower(asignatura) ~* '[aá]l' OR       cod_asignatura ~* '[aá]l' OR       text(c.id) ~* '[aá]l' OR       lower(profesor) ~* '[aá]l')  AND (lower(asignatura) ~* 'd[eé]r[eé]ch[oó]' OR       cod_asignatura ~* 'd[eé]r[eé]ch[oó]' OR       text(c.id) ~* 'd[eé]r[eé]ch[oó]' OR       lower(profesor) ~* 'd[eé]r[eé]ch[oó]')  
