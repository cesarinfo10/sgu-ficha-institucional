COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[ií]d[eé]r[aá]zg[oó]' OR       cod_asignatura ~* 'l[ií]d[eé]r[aá]zg[oó]' OR       text(c.id) ~* 'l[ií]d[eé]r[aá]zg[oó]' OR       lower(profesor) ~* 'l[ií]d[eé]r[aá]zg[oó]')  
