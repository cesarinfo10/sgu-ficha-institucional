COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n' OR       cod_asignatura ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n' OR       text(c.id) ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n' OR       lower(profesor) ~* 'c[oó]m[uú]n[ií]c[aá]c[ií][oó]n')  
