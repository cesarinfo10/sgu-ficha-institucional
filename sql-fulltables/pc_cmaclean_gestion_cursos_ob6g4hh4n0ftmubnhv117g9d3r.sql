COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]g[ií]str[aá]l' OR       cod_asignatura ~* 'r[eé]g[ií]str[aá]l' OR       text(c.id) ~* 'r[eé]g[ií]str[aá]l' OR       lower(profesor) ~* 'r[eé]g[ií]str[aá]l')  
