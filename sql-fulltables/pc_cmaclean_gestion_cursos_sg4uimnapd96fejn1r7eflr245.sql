COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n' OR       cod_asignatura ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n' OR       text(c.id) ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n' OR       lower(profesor) ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n')  AND (lower(asignatura) ~* 'g[eé]n[eé]r[aá]l' OR       cod_asignatura ~* 'g[eé]n[eé]r[aá]l' OR       text(c.id) ~* 'g[eé]n[eé]r[aá]l' OR       lower(profesor) ~* 'g[eé]n[eé]r[aá]l')  
