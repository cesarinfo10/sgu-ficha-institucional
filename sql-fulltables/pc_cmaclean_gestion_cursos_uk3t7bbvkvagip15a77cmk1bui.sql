COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[uú][ií]s' OR       cod_asignatura ~* 'l[uú][ií]s' OR       text(c.id) ~* 'l[uú][ií]s' OR       lower(profesor) ~* 'l[uú][ií]s')  AND (lower(asignatura) ~* 'c[aá]st[ií]ll[oó]' OR       cod_asignatura ~* 'c[aá]st[ií]ll[oó]' OR       text(c.id) ~* 'c[aá]st[ií]ll[oó]' OR       lower(profesor) ~* 'c[aá]st[ií]ll[oó]')  
