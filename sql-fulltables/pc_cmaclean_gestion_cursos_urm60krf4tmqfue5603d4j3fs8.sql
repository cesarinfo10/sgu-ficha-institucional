COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[eé][uú]r[oó]' OR       cod_asignatura ~* 'n[eé][uú]r[oó]' OR       text(c.id) ~* 'n[eé][uú]r[oó]' OR       lower(profesor) ~* 'n[eé][uú]r[oó]')  AND (lower(asignatura) ~* '[aá]n[aá]t[oó]' OR       cod_asignatura ~* '[aá]n[aá]t[oó]' OR       text(c.id) ~* '[aá]n[aá]t[oó]' OR       lower(profesor) ~* '[aá]n[aá]t[oó]')  
