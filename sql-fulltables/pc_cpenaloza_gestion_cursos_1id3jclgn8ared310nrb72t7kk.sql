COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'chr[ií]st[ií][aá]n' OR       cod_asignatura ~* 'chr[ií]st[ií][aá]n' OR       text(c.id) ~* 'chr[ií]st[ií][aá]n' OR       lower(profesor) ~* 'chr[ií]st[ií][aá]n')  AND (lower(asignatura) ~* '[aá]ndr[eé]s' OR       cod_asignatura ~* '[aá]ndr[eé]s' OR       text(c.id) ~* '[aá]ndr[eé]s' OR       lower(profesor) ~* '[aá]ndr[eé]s')  AND (lower(asignatura) ~* 'c[oó]d[eé]c[ií]d[oó]' OR       cod_asignatura ~* 'c[oó]d[eé]c[ií]d[oó]' OR       text(c.id) ~* 'c[oó]d[eé]c[ií]d[oó]' OR       lower(profesor) ~* 'c[oó]d[eé]c[ií]d[oó]')  AND (lower(asignatura) ~* 'g[eé][eé]ll' OR       cod_asignatura ~* 'g[eé][eé]ll' OR       text(c.id) ~* 'g[eé][eé]ll' OR       lower(profesor) ~* 'g[eé][eé]ll')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
