COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[oó]l[ií]t[ií]c' OR       cod_asignatura ~* 'p[oó]l[ií]t[ií]c' OR       text(c.id) ~* 'p[oó]l[ií]t[ií]c' OR       lower(profesor) ~* 'p[oó]l[ií]t[ií]c')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
