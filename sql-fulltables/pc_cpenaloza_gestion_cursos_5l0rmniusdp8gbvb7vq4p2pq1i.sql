COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[ií]d[eé]r[aá]zg[oó]' OR       cod_asignatura ~* 'l[ií]d[eé]r[aá]zg[oó]' OR       text(c.id) ~* 'l[ií]d[eé]r[aá]zg[oó]' OR       lower(profesor) ~* 'l[ií]d[eé]r[aá]zg[oó]')  AND array[101,62,26,88,18,3,36,67,156,159,12] && (ids_carreras)  
