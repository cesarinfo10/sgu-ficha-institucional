COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'gl[oó]r[ií][aá]' OR       cod_asignatura ~* 'gl[oó]r[ií][aá]' OR       text(c.id) ~* 'gl[oó]r[ií][aá]' OR       lower(profesor) ~* 'gl[oó]r[ií][aá]')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
