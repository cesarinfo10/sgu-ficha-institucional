COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[eé]' OR       cod_asignatura ~* 'pr[eé]' OR       text(c.id) ~* 'pr[eé]' OR       lower(profesor) ~* 'pr[eé]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
