COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]t[aá]l[ií]' OR       cod_asignatura ~* 'v[ií]t[aá]l[ií]' OR       text(c.id) ~* 'v[ií]t[aá]l[ií]' OR       lower(profesor) ~* 'v[ií]t[aá]l[ií]')  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
