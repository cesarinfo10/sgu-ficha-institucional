COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[eé]t[oó]d[oó]s' OR       cod_asignatura ~* 'm[eé]t[oó]d[oó]s' OR       text(c.id) ~* 'm[eé]t[oó]d[oó]s' OR       lower(profesor) ~* 'm[eé]t[oó]d[oó]s')  AND (lower(asignatura) ~* 'm[aá]t[eé]m[aá]t[ií]c[oó]s' OR       cod_asignatura ~* 'm[aá]t[eé]m[aá]t[ií]c[oó]s' OR       text(c.id) ~* 'm[aá]t[eé]m[aá]t[ií]c[oó]s' OR       lower(profesor) ~* 'm[aá]t[eé]m[aá]t[ií]c[oó]s')  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
