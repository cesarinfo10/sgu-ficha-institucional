COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ndr[eé][aá]' OR       cod_asignatura ~* '[aá]ndr[eé][aá]' OR       text(c.id) ~* '[aá]ndr[eé][aá]' OR       lower(profesor) ~* '[aá]ndr[eé][aá]')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
