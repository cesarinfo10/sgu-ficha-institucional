COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       cod_asignatura ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       text(c.id) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       lower(profesor) ~* 'f[uú]nd[aá]m[eé]nt[oó]s')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
