COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]nd[uú]ct[aá]' OR       cod_asignatura ~* 'c[oó]nd[uú]ct[aá]' OR       text(c.id) ~* 'c[oó]nd[uú]ct[aá]' OR       lower(profesor) ~* 'c[oó]nd[uú]ct[aá]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
