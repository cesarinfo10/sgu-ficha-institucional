COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá]lm[aá]' OR       cod_asignatura ~* 'p[aá]lm[aá]' OR       text(c.id) ~* 'p[aá]lm[aá]' OR       lower(profesor) ~* 'p[aá]lm[aá]')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
