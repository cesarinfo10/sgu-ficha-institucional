COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]ns[ií][oó]n[eé]s' OR       cod_asignatura ~* 'p[eé]ns[ií][oó]n[eé]s' OR       text(c.id) ~* 'p[eé]ns[ií][oó]n[eé]s' OR       lower(profesor) ~* 'p[eé]ns[ií][oó]n[eé]s')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
