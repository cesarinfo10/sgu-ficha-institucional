COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]l[aá]c[ií][oó]n[eé]s' OR       cod_asignatura ~* 'r[eé]l[aá]c[ií][oó]n[eé]s' OR       text(c.id) ~* 'r[eé]l[aá]c[ií][oó]n[eé]s' OR       lower(profesor) ~* 'r[eé]l[aá]c[ií][oó]n[eé]s')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
