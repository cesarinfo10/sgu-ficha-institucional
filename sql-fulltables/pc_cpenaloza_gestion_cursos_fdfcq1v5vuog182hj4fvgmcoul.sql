COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]cl[uú]t[aá]m[ií][eé]nt[oó]' OR       cod_asignatura ~* 'r[eé]cl[uú]t[aá]m[ií][eé]nt[oó]' OR       text(c.id) ~* 'r[eé]cl[uú]t[aá]m[ií][eé]nt[oó]' OR       lower(profesor) ~* 'r[eé]cl[uú]t[aá]m[ií][eé]nt[oó]')  AND array[101,62,26,88,18,3,36,67,156,159,12] && (ids_carreras)  
