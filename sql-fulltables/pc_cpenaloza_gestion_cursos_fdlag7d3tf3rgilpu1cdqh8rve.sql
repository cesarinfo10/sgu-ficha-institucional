COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '18230' OR       cod_asignatura ~* '18230' OR       text(c.id) ~* '18230' OR       lower(profesor) ~* '18230')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
