COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]st[aá]d[ií]st[ií]c' OR       cod_asignatura ~* '[eé]st[aá]d[ií]st[ií]c' OR       text(c.id) ~* '[eé]st[aá]d[ií]st[ií]c' OR       lower(profesor) ~* '[eé]st[aá]d[ií]st[ií]c')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
