COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[ií]n[aá]nz[aá]' OR       cod_asignatura ~* 'f[ií]n[aá]nz[aá]' OR       text(c.id) ~* 'f[ií]n[aá]nz[aá]' OR       lower(profesor) ~* 'f[ií]n[aá]nz[aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
