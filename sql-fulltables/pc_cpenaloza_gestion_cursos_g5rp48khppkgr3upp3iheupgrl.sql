COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       cod_asignatura ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       text(c.id) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       lower(profesor) ~* 'h[uú]m[aá]n[ií]sm[oó]')  AND (lower(asignatura) ~* 's[oó]c[ií][aá]l' OR       cod_asignatura ~* 's[oó]c[ií][aá]l' OR       text(c.id) ~* 's[oó]c[ií][aá]l' OR       lower(profesor) ~* 's[oó]c[ií][aá]l')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
