COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND c.ano=2024  AND c.semestre=1  AND ('PRE' = ANY (regimenes))  AND 1 IN (c.dia1,c.dia2,c.dia3)  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
