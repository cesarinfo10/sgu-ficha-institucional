COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'ps[ií]c[oó]' OR       cod_asignatura ~* 'ps[ií]c[oó]' OR       text(c.id) ~* 'ps[ií]c[oó]' OR       lower(profesor) ~* 'ps[ií]c[oó]')  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
