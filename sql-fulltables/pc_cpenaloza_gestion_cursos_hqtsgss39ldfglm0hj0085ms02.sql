COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[ií]x' OR       cod_asignatura ~* 'm[ií]x' OR       text(c.id) ~* 'm[ií]x' OR       lower(profesor) ~* 'm[ií]x')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
