COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]s[uú]p[uú][eé]st[oó]s' OR       cod_asignatura ~* 'r[eé]s[uú]p[uú][eé]st[oó]s' OR       text(c.id) ~* 'r[eé]s[uú]p[uú][eé]st[oó]s' OR       lower(profesor) ~* 'r[eé]s[uú]p[uú][eé]st[oó]s')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
