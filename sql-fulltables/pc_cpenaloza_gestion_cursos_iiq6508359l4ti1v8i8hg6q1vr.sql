COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]r[eé]ch[oó]' OR       cod_asignatura ~* 'd[eé]r[eé]ch[oó]' OR       text(c.id) ~* 'd[eé]r[eé]ch[oó]' OR       lower(profesor) ~* 'd[eé]r[eé]ch[oó]')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
