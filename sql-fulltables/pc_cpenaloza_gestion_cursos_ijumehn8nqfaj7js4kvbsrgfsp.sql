COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'ps[ií]c[oó]l[oó]' OR       cod_asignatura ~* 'ps[ií]c[oó]l[oó]' OR       text(c.id) ~* 'ps[ií]c[oó]l[oó]' OR       lower(profesor) ~* 'ps[ií]c[oó]l[oó]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
