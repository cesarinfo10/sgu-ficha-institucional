COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '18231' OR       cod_asignatura ~* '18231' OR       text(c.id) ~* '18231' OR       lower(profesor) ~* '18231')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
