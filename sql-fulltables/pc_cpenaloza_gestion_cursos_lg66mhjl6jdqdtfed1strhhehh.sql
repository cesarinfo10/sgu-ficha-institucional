COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[aá]' OR       cod_asignatura ~* 'c[aá]rl[aá]' OR       text(c.id) ~* 'c[aá]rl[aá]' OR       lower(profesor) ~* 'c[aá]rl[aá]')  AND (lower(asignatura) ~* 'r[eé]b[eé]c[aá]' OR       cod_asignatura ~* 'r[eé]b[eé]c[aá]' OR       text(c.id) ~* 'r[eé]b[eé]c[aá]' OR       lower(profesor) ~* 'r[eé]b[eé]c[aá]')  AND (lower(asignatura) ~* 'm[aá]cl[eé][aá]n' OR       cod_asignatura ~* 'm[aá]cl[eé][aá]n' OR       text(c.id) ~* 'm[aá]cl[eé][aá]n' OR       lower(profesor) ~* 'm[aá]cl[eé][aá]n')  AND (lower(asignatura) ~* 'g[aá]ll[aá]rd[oó]' OR       cod_asignatura ~* 'g[aá]ll[aá]rd[oó]' OR       text(c.id) ~* 'g[aá]ll[aá]rd[oó]' OR       lower(profesor) ~* 'g[aá]ll[aá]rd[oó]')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
