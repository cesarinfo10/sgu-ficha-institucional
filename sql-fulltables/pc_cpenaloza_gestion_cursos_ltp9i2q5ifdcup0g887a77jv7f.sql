COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[oó]nt[eé]c[ií]n[oó]' OR       cod_asignatura ~* 'm[oó]nt[eé]c[ií]n[oó]' OR       text(c.id) ~* 'm[oó]nt[eé]c[ií]n[oó]' OR       lower(profesor) ~* 'm[oó]nt[eé]c[ií]n[oó]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
