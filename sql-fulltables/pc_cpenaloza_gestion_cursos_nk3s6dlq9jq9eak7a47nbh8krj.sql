COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[eé]t[oó]d[oó]s' OR       cod_asignatura ~* 'm[eé]t[oó]d[oó]s' OR       text(c.id) ~* 'm[eé]t[oó]d[oó]s' OR       lower(profesor) ~* 'm[eé]t[oó]d[oó]s')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
