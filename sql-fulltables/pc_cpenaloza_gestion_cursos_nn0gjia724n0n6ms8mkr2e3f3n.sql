COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[eé]rn[aá]nd[oó]' OR       cod_asignatura ~* 'f[eé]rn[aá]nd[oó]' OR       text(c.id) ~* 'f[eé]rn[aá]nd[oó]' OR       lower(profesor) ~* 'f[eé]rn[aá]nd[oó]')  AND (lower(asignatura) ~* 'm[oó]r[aá]l[eé]s' OR       cod_asignatura ~* 'm[oó]r[aá]l[eé]s' OR       text(c.id) ~* 'm[oó]r[aá]l[eé]s' OR       lower(profesor) ~* 'm[oó]r[aá]l[eé]s')  AND (lower(asignatura) ~* 'p[eé]r[eé]z' OR       cod_asignatura ~* 'p[eé]r[eé]z' OR       text(c.id) ~* 'p[eé]r[eé]z' OR       lower(profesor) ~* 'p[eé]r[eé]z')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
