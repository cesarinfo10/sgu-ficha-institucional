COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]st[oó]s' OR       cod_asignatura ~* 'c[oó]st[oó]s' OR       text(c.id) ~* 'c[oó]st[oó]s' OR       lower(profesor) ~* 'c[oó]st[oó]s')  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
