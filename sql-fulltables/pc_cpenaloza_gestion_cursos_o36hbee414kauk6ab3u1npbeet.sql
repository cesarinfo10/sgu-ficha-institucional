COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d' OR       cod_asignatura ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d' OR       text(c.id) ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d' OR       lower(profesor) ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d')  AND (lower(asignatura) ~* '[aá]v[aá]nz[aá]d[aá]' OR       cod_asignatura ~* '[aá]v[aá]nz[aá]d[aá]' OR       text(c.id) ~* '[aá]v[aá]nz[aá]d[aá]' OR       lower(profesor) ~* '[aá]v[aá]nz[aá]d[aá]')  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
