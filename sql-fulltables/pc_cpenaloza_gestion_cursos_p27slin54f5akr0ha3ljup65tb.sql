COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       cod_asignatura ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       text(c.id) ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       lower(profesor) ~* '[eé]c[oó]n[oó]m[ií][aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
