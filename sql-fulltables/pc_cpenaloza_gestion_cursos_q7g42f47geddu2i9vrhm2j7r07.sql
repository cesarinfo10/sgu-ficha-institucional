COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]t[aá]l[ií]n[aá]' OR       cod_asignatura ~* 'c[aá]t[aá]l[ií]n[aá]' OR       text(c.id) ~* 'c[aá]t[aá]l[ií]n[aá]' OR       lower(profesor) ~* 'c[aá]t[aá]l[ií]n[aá]')  AND (lower(asignatura) ~* '[aá]l[eé]j[aá]ndr[aá]' OR       cod_asignatura ~* '[aá]l[eé]j[aá]ndr[aá]' OR       text(c.id) ~* '[aá]l[eé]j[aá]ndr[aá]' OR       lower(profesor) ~* '[aá]l[eé]j[aá]ndr[aá]')  AND (lower(asignatura) ~* 'n[uú]ñ[eé]z' OR       cod_asignatura ~* 'n[uú]ñ[eé]z' OR       text(c.id) ~* 'n[uú]ñ[eé]z' OR       lower(profesor) ~* 'n[uú]ñ[eé]z')  AND (lower(asignatura) ~* 't[oó]rr[eé][aá]lb[aá]' OR       cod_asignatura ~* 't[oó]rr[eé][aá]lb[aá]' OR       text(c.id) ~* 't[oó]rr[eé][aá]lb[aá]' OR       lower(profesor) ~* 't[oó]rr[eé][aá]lb[aá]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
