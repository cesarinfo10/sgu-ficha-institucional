COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]dm[ií]n[ií]str[aá]c[ií]' OR       cod_asignatura ~* '[aá]dm[ií]n[ií]str[aá]c[ií]' OR       text(c.id) ~* '[aá]dm[ií]n[ií]str[aá]c[ií]' OR       lower(profesor) ~* '[aá]dm[ií]n[ií]str[aá]c[ií]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
