COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'fr[aá]nc[oó]' OR       cod_asignatura ~* 'fr[aá]nc[oó]' OR       text(c.id) ~* 'fr[aá]nc[oó]' OR       lower(profesor) ~* 'fr[aá]nc[oó]')  AND (lower(asignatura) ~* 'n[oó]r[ií][eé]g[aá]' OR       cod_asignatura ~* 'n[oó]r[ií][eé]g[aá]' OR       text(c.id) ~* 'n[oó]r[ií][eé]g[aá]' OR       lower(profesor) ~* 'n[oó]r[ií][eé]g[aá]')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
