COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[eé]s[uú]p[uú][eé]st[oó]' OR       cod_asignatura ~* 'pr[eé]s[uú]p[uú][eé]st[oó]' OR       text(c.id) ~* 'pr[eé]s[uú]p[uú][eé]st[oó]' OR       lower(profesor) ~* 'pr[eé]s[uú]p[uú][eé]st[oó]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
