COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[oó]s' OR       cod_asignatura ~* 'c[aá]rl[oó]s' OR       text(c.id) ~* 'c[aá]rl[oó]s' OR       lower(profesor) ~* 'c[aá]rl[oó]s')  AND (lower(asignatura) ~* 'v[ií]t[aá]l[ií]' OR       cod_asignatura ~* 'v[ií]t[aá]l[ií]' OR       text(c.id) ~* 'v[ií]t[aá]l[ií]' OR       lower(profesor) ~* 'v[ií]t[aá]l[ií]')  AND (lower(asignatura) ~* 's[aá]n' OR       cod_asignatura ~* 's[aá]n' OR       text(c.id) ~* 's[aá]n' OR       lower(profesor) ~* 's[aá]n')  AND (lower(asignatura) ~* 'm[aá]rt[ií]n' OR       cod_asignatura ~* 'm[aá]rt[ií]n' OR       text(c.id) ~* 'm[aá]rt[ií]n' OR       lower(profesor) ~* 'm[aá]rt[ií]n')  AND array[101,62,26,88,18,3,159,156,36,67,12] && (ids_carreras)  
