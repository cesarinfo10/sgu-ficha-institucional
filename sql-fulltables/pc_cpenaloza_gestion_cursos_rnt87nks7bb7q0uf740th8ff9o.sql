COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d' OR       cod_asignatura ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d' OR       text(c.id) ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d' OR       lower(profesor) ~* 'c[oó]nt[aá]b[ií]l[ií]d[aá]d')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
