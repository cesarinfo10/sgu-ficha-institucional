COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[aá]ll[eé]r' OR       cod_asignatura ~* 't[aá]ll[eé]r' OR       text(c.id) ~* 't[aá]ll[eé]r' OR       lower(profesor) ~* 't[aá]ll[eé]r')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
