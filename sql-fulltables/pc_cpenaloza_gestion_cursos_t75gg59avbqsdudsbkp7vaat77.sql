COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       cod_asignatura ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       text(c.id) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]' OR       lower(profesor) ~* '[aá][uú]d[ií]t[oó]r[ií][aá]')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
