COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ld[oó]' OR       cod_asignatura ~* '[aá]ld[oó]' OR       text(c.id) ~* '[aá]ld[oó]' OR       lower(profesor) ~* '[aá]ld[oó]')  AND (lower(asignatura) ~* '[eé]nz[oó]' OR       cod_asignatura ~* '[eé]nz[oó]' OR       text(c.id) ~* '[eé]nz[oó]' OR       lower(profesor) ~* '[eé]nz[oó]')  AND (lower(asignatura) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       cod_asignatura ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       text(c.id) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       lower(profesor) ~* 'p[eé]rn[ií]g[oó]tt[ií]')  AND (lower(asignatura) ~* 'r[ií][oó]s' OR       cod_asignatura ~* 'r[ií][oó]s' OR       text(c.id) ~* 'r[ií][oó]s' OR       lower(profesor) ~* 'r[ií][oó]s')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
