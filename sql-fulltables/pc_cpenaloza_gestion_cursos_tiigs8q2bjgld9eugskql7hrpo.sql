COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[uú]m[aá]n[ií]sm' OR       cod_asignatura ~* 'h[uú]m[aá]n[ií]sm' OR       text(c.id) ~* 'h[uú]m[aá]n[ií]sm' OR       lower(profesor) ~* 'h[uú]m[aá]n[ií]sm')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
