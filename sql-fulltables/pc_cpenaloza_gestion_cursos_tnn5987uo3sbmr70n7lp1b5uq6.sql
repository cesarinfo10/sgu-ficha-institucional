COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ntr[oó]d[uú]cc' OR       cod_asignatura ~* '[ií]ntr[oó]d[uú]cc' OR       text(c.id) ~* '[ií]ntr[oó]d[uú]cc' OR       lower(profesor) ~* '[ií]ntr[oó]d[uú]cc')  AND id_carrera IN (101,62,26,88,18,3,36,67,12)  
