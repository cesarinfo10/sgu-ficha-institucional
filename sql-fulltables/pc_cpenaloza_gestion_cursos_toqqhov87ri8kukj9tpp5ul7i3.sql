COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ngl[eé]s' OR       cod_asignatura ~* '[ií]ngl[eé]s' OR       text(c.id) ~* '[ií]ngl[eé]s' OR       lower(profesor) ~* '[ií]ngl[eé]s')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
