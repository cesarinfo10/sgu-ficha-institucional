COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'b[oó]r[ií]s' OR       cod_asignatura ~* 'b[oó]r[ií]s' OR       text(c.id) ~* 'b[oó]r[ií]s' OR       lower(profesor) ~* 'b[oó]r[ií]s')  AND (lower(asignatura) ~* '[aá]lfr[eé]d[oó]' OR       cod_asignatura ~* '[aá]lfr[eé]d[oó]' OR       text(c.id) ~* '[aá]lfr[eé]d[oó]' OR       lower(profesor) ~* '[aá]lfr[eé]d[oó]')  AND (lower(asignatura) ~* 'p[eé]r[aá]lt[aá]' OR       cod_asignatura ~* 'p[eé]r[aá]lt[aá]' OR       text(c.id) ~* 'p[eé]r[aá]lt[aá]' OR       lower(profesor) ~* 'p[eé]r[aá]lt[aá]')  AND (lower(asignatura) ~* 'j[aá]r[aá]' OR       cod_asignatura ~* 'j[aá]r[aá]' OR       text(c.id) ~* 'j[aá]r[aá]' OR       lower(profesor) ~* 'j[aá]r[aá]')  AND array[101,62,26,88,18,3,36,67,150,12] && (ids_carreras)  
