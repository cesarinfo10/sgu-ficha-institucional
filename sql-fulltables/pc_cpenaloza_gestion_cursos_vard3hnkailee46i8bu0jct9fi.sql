COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]nd[uú]ct[aá]' OR       cod_asignatura ~* 'c[oó]nd[uú]ct[aá]' OR       text(c.id) ~* 'c[oó]nd[uú]ct[aá]' OR       lower(profesor) ~* 'c[oó]nd[uú]ct[aá]')  AND (lower(asignatura) ~* 'd[eé]l' OR       cod_asignatura ~* 'd[eé]l' OR       text(c.id) ~* 'd[eé]l' OR       lower(profesor) ~* 'd[eé]l')  AND (lower(asignatura) ~* 'c[oó]ns[uú]m[ií]d[oó]r' OR       cod_asignatura ~* 'c[oó]ns[uú]m[ií]d[oó]r' OR       text(c.id) ~* 'c[oó]ns[uú]m[ií]d[oó]r' OR       lower(profesor) ~* 'c[oó]ns[uú]m[ií]d[oó]r')  AND array[101,62,26,88,18,3,36,67,12] && (ids_carreras)  
