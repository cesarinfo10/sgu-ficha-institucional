COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND c.ano=2024  AND c.semestre=1  AND c.seccion BETWEEN 1 AND 4  AND c.tipo_clase = 'Presencial'  AND ('PRE' = ANY (regimenes))  AND 1 IN (c.dia1,c.dia2,c.dia3)  
