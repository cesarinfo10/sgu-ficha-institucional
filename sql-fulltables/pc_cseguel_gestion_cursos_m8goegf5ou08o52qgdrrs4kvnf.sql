COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[uú][aá]j[aá]rd[oó]' OR       cod_asignatura ~* 'g[uú][aá]j[aá]rd[oó]' OR       text(c.id) ~* 'g[uú][aá]j[aá]rd[oó]' OR       lower(profesor) ~* 'g[uú][aá]j[aá]rd[oó]')  AND id_carrera IN (61,68,21,12)  
