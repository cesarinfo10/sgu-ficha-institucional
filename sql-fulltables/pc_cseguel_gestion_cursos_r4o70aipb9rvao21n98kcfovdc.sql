COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[uú][aá]n' OR       cod_asignatura ~* 'j[uú][aá]n' OR       text(c.id) ~* 'j[uú][aá]n' OR       lower(profesor) ~* 'j[uú][aá]n')  AND (lower(asignatura) ~* 'p[aá]bl[oó]' OR       cod_asignatura ~* 'p[aá]bl[oó]' OR       text(c.id) ~* 'p[aá]bl[oó]' OR       lower(profesor) ~* 'p[aá]bl[oó]')  AND (lower(asignatura) ~* 'v[aá]sq[uú][eé]z' OR       cod_asignatura ~* 'v[aá]sq[uú][eé]z' OR       text(c.id) ~* 'v[aá]sq[uú][eé]z' OR       lower(profesor) ~* 'v[aá]sq[uú][eé]z')  AND (lower(asignatura) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR       cod_asignatura ~* 'b[uú]st[aá]m[aá]nt[eé]' OR       text(c.id) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR       lower(profesor) ~* 'b[uú]st[aá]m[aá]nt[eé]')  AND id_carrera IN (61,68,21,12)  
