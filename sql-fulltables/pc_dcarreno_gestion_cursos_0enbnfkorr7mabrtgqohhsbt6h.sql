COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[uú][aá]n[aá]' OR       cod_asignatura ~* 'l[uú][aá]n[aá]' OR       text(c.id) ~* 'l[uú][aá]n[aá]' OR       lower(profesor) ~* 'l[uú][aá]n[aá]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* 's[oó][uú]s[aá]' OR       cod_asignatura ~* 's[oó][uú]s[aá]' OR       text(c.id) ~* 's[oó][uú]s[aá]' OR       lower(profesor) ~* 's[oó][uú]s[aá]')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'l[ií]m[aá]' OR       cod_asignatura ~* 'l[ií]m[aá]' OR       text(c.id) ~* 'l[ií]m[aá]' OR       lower(profesor) ~* 'l[ií]m[aá]')  
