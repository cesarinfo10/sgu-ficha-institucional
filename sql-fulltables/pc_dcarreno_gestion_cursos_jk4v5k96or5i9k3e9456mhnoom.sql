COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]b[aá]st[ií][aá]n' OR       cod_asignatura ~* 's[eé]b[aá]st[ií][aá]n' OR       text(c.id) ~* 's[eé]b[aá]st[ií][aá]n' OR       lower(profesor) ~* 's[eé]b[aá]st[ií][aá]n')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* '[aá]sc[uú][ií]' OR       cod_asignatura ~* '[aá]sc[uú][ií]' OR       text(c.id) ~* '[aá]sc[uú][ií]' OR       lower(profesor) ~* '[aá]sc[uú][ií]')  AND (lower(asignatura) ~* 'g[aá]c' OR       cod_asignatura ~* 'g[aá]c' OR       text(c.id) ~* 'g[aá]c' OR       lower(profesor) ~* 'g[aá]c')  
