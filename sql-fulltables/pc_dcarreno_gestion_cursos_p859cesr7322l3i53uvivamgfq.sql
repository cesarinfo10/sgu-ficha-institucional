COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]d[eé]l[eé][ií]n[eé]' OR       cod_asignatura ~* 'm[aá]d[eé]l[eé][ií]n[eé]' OR       text(c.id) ~* 'm[aá]d[eé]l[eé][ií]n[eé]' OR       lower(profesor) ~* 'm[aá]d[eé]l[eé][ií]n[eé]')  AND (lower(asignatura) ~* 'm[aá]r[ií][aá]' OR       cod_asignatura ~* 'm[aá]r[ií][aá]' OR       text(c.id) ~* 'm[aá]r[ií][aá]' OR       lower(profesor) ~* 'm[aá]r[ií][aá]')  AND (lower(asignatura) ~* 'cl[eé]rc' OR       cod_asignatura ~* 'cl[eé]rc' OR       text(c.id) ~* 'cl[eé]rc' OR       lower(profesor) ~* 'cl[eé]rc')  AND (lower(asignatura) ~* 't[aá]p[ií][aá]' OR       cod_asignatura ~* 't[aá]p[ií][aá]' OR       text(c.id) ~* 't[aá]p[ií][aá]' OR       lower(profesor) ~* 't[aá]p[ií][aá]')  
