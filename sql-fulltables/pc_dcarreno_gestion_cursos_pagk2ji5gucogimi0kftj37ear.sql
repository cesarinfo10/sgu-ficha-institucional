COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[ií]ts[ií]' OR       cod_asignatura ~* 'l[ií]ts[ií]' OR       text(c.id) ~* 'l[ií]ts[ií]' OR       lower(profesor) ~* 'l[ií]ts[ií]')  AND (lower(asignatura) ~* 'm[ií]r[eé][eé]n' OR       cod_asignatura ~* 'm[ií]r[eé][eé]n' OR       text(c.id) ~* 'm[ií]r[eé][eé]n' OR       lower(profesor) ~* 'm[ií]r[eé][eé]n')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* 'c[oó]ntr[eé]r[aá]s' OR       cod_asignatura ~* 'c[oó]ntr[eé]r[aá]s' OR       text(c.id) ~* 'c[oó]ntr[eé]r[aá]s' OR       lower(profesor) ~* 'c[oó]ntr[eé]r[aá]s')  AND (lower(asignatura) ~* 's[aá]lv[oó]' OR       cod_asignatura ~* 's[aá]lv[oó]' OR       text(c.id) ~* 's[aá]lv[oó]' OR       lower(profesor) ~* 's[aá]lv[oó]')  
