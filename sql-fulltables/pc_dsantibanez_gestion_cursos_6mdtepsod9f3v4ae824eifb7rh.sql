COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]st[eé]f[aá]ny' OR       cod_asignatura ~* '[eé]st[eé]f[aá]ny' OR       text(c.id) ~* '[eé]st[eé]f[aá]ny' OR       lower(profesor) ~* '[eé]st[eé]f[aá]ny')  AND (lower(asignatura) ~* '[ií]gn[aá]c[ií][aá]' OR       cod_asignatura ~* '[ií]gn[aá]c[ií][aá]' OR       text(c.id) ~* '[ií]gn[aá]c[ií][aá]' OR       lower(profesor) ~* '[ií]gn[aá]c[ií][aá]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* 's[aá]nt[oó]r[oó]' OR       cod_asignatura ~* 's[aá]nt[oó]r[oó]' OR       text(c.id) ~* 's[aá]nt[oó]r[oó]' OR       lower(profesor) ~* 's[aá]nt[oó]r[oó]')  AND (lower(asignatura) ~* 'h[eé]rr[eé]r[aá]' OR       cod_asignatura ~* 'h[eé]rr[eé]r[aá]' OR       text(c.id) ~* 'h[eé]rr[eé]r[aá]' OR       lower(profesor) ~* 'h[eé]rr[eé]r[aá]')  AND array[61,68,21,12] && (ids_carreras)  
