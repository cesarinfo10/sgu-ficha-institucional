COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[uú]s[aá]n[aá]' OR       cod_asignatura ~* 's[uú]s[aá]n[aá]' OR       text(c.id) ~* 's[uú]s[aá]n[aá]' OR       lower(profesor) ~* 's[uú]s[aá]n[aá]')  AND (lower(asignatura) ~* 'p[aá]z' OR       cod_asignatura ~* 'p[aá]z' OR       text(c.id) ~* 'p[aá]z' OR       lower(profesor) ~* 'p[aá]z')  AND (lower(asignatura) ~* 'b[aá]s[ií]s' OR       cod_asignatura ~* 'b[aá]s[ií]s' OR       text(c.id) ~* 'b[aá]s[ií]s' OR       lower(profesor) ~* 'b[aá]s[ií]s')  AND (lower(asignatura) ~* 'm[eé]r[ií]n[oó]' OR       cod_asignatura ~* 'm[eé]r[ií]n[oó]' OR       text(c.id) ~* 'm[eé]r[ií]n[oó]' OR       lower(profesor) ~* 'm[eé]r[ií]n[oó]')  AND id_carrera IN (61,68,21,12)  
