COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]cn[ií]c[aá]s' OR       cod_asignatura ~* 't[eé]cn[ií]c[aá]s' OR       text(c.id) ~* 't[eé]cn[ií]c[aá]s' OR       lower(profesor) ~* 't[eé]cn[ií]c[aá]s')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'r[eé]s[oó]l[uú]c[ií][oó]n' OR       cod_asignatura ~* 'r[eé]s[oó]l[uú]c[ií][oó]n' OR       text(c.id) ~* 'r[eé]s[oó]l[uú]c[ií][oó]n' OR       lower(profesor) ~* 'r[eé]s[oó]l[uú]c[ií][oó]n')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
