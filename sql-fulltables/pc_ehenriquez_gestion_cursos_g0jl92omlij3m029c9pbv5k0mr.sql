COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]cn[ií]' OR       cod_asignatura ~* 't[eé]cn[ií]' OR       text(c.id) ~* 't[eé]cn[ií]' OR       lower(profesor) ~* 't[eé]cn[ií]')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'r[eé]s' OR       cod_asignatura ~* 'r[eé]s' OR       text(c.id) ~* 'r[eé]s' OR       lower(profesor) ~* 'r[eé]s')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
