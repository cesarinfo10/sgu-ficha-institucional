COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]c' OR       cod_asignatura ~* 't[eé]c' OR       text(c.id) ~* 't[eé]c' OR       lower(profesor) ~* 't[eé]c')  AND (lower(asignatura) ~* 'r[eé]s' OR       cod_asignatura ~* 'r[eé]s' OR       text(c.id) ~* 'r[eé]s' OR       lower(profesor) ~* 'r[eé]s')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
