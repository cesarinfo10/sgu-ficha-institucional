COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[ií]c[aá]rd[oó]' OR       cod_asignatura ~* 'r[ií]c[aá]rd[oó]' OR       text(c.id) ~* 'r[ií]c[aá]rd[oó]' OR       lower(profesor) ~* 'r[ií]c[aá]rd[oó]')  AND (lower(asignatura) ~* '[aá]ndr[eé]s' OR       cod_asignatura ~* '[aá]ndr[eé]s' OR       text(c.id) ~* '[aá]ndr[eé]s' OR       lower(profesor) ~* '[aá]ndr[eé]s')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* 'm[aá]rq[uú][eé]z' OR       cod_asignatura ~* 'm[aá]rq[uú][eé]z' OR       text(c.id) ~* 'm[aá]rq[uú][eé]z' OR       lower(profesor) ~* 'm[aá]rq[uú][eé]z')  AND (lower(asignatura) ~* '[aá]c[eé]v[eé]d[oó]' OR       cod_asignatura ~* '[aá]c[eé]v[eé]d[oó]' OR       text(c.id) ~* '[aá]c[eé]v[eé]d[oó]' OR       lower(profesor) ~* '[aá]c[eé]v[eé]d[oó]')  
