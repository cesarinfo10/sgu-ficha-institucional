COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'gr[eé]g[oó]r[ií][oó]' OR       cod_asignatura ~* 'gr[eé]g[oó]r[ií][oó]' OR       text(c.id) ~* 'gr[eé]g[oó]r[ií][oó]' OR       lower(profesor) ~* 'gr[eé]g[oó]r[ií][oó]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* '[aá]g[uú][ií]l[eé]r[aá]' OR       cod_asignatura ~* '[aá]g[uú][ií]l[eé]r[aá]' OR       text(c.id) ~* '[aá]g[uú][ií]l[eé]r[aá]' OR       lower(profesor) ~* '[aá]g[uú][ií]l[eé]r[aá]')  AND (lower(asignatura) ~* '[aá]nt[uú]n[eé]z' OR       cod_asignatura ~* '[aá]nt[uú]n[eé]z' OR       text(c.id) ~* '[aá]nt[uú]n[eé]z' OR       lower(profesor) ~* '[aá]nt[uú]n[eé]z')  
