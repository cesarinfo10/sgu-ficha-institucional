COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[eé]rn[aá]nd[oó]' OR       cod_asignatura ~* 'f[eé]rn[aá]nd[oó]' OR       text(c.id) ~* 'f[eé]rn[aá]nd[oó]' OR       lower(profesor) ~* 'f[eé]rn[aá]nd[oó]')  AND (lower(asignatura) ~* 't[aá]ll[aá]r' OR       cod_asignatura ~* 't[aá]ll[aá]r' OR       text(c.id) ~* 't[aá]ll[aá]r' OR       lower(profesor) ~* 't[aá]ll[aá]r')  AND (lower(asignatura) ~* 'd[eé]l[uú]cch[ií]' OR       cod_asignatura ~* 'd[eé]l[uú]cch[ií]' OR       text(c.id) ~* 'd[eé]l[uú]cch[ií]' OR       lower(profesor) ~* 'd[eé]l[uú]cch[ií]')  
