COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND 120 = ANY (ids_carreras)  AND c.ano=2023  AND c.semestre=2  AND c.seccion=3  AND ('PRE-D' = ANY (regimenes))  AND array[106,120,12] && (ids_carreras)  
