COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '17880' OR       cod_asignatura ~* '17880' OR       text(c.id) ~* '17880' OR       lower(profesor) ~* '17880')  AND array[106,120,12] && (ids_carreras)  
