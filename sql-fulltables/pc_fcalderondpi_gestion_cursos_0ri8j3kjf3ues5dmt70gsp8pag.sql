COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]ll[oó]' OR       cod_asignatura ~* 's[eé]ll[oó]' OR       text(c.id) ~* 's[eé]ll[oó]' OR       lower(profesor) ~* 's[eé]ll[oó]')  AND array[59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12] && (ids_carreras)  
