COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[oó]r[aá]g[aá]' OR       cod_asignatura ~* 'm[oó]r[aá]g[aá]' OR       text(c.id) ~* 'm[oó]r[aá]g[aá]' OR       lower(profesor) ~* 'm[oó]r[aá]g[aá]')  AND (lower(asignatura) ~* 'c[aá]t[aá]l[aá]n' OR       cod_asignatura ~* 'c[aá]t[aá]l[aá]n' OR       text(c.id) ~* 'c[aá]t[aá]l[aá]n' OR       lower(profesor) ~* 'c[aá]t[aá]l[aá]n')  AND array[59,57,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,27,28,38,115,77,125,114,147,148,161,160,163,12] && (ids_carreras)  
