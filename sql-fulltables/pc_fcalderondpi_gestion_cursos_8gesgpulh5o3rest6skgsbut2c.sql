COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[oó]s' OR       cod_asignatura ~* 'c[aá]rl[oó]s' OR       text(c.id) ~* 'c[aá]rl[oó]s' OR       lower(profesor) ~* 'c[aá]rl[oó]s')  AND (lower(asignatura) ~* '[aá]d[oó]lf[oó]' OR       cod_asignatura ~* '[aá]d[oó]lf[oó]' OR       text(c.id) ~* '[aá]d[oó]lf[oó]' OR       lower(profesor) ~* '[aá]d[oó]lf[oó]')  AND (lower(asignatura) ~* 'c[oó]nch[aá]' OR       cod_asignatura ~* 'c[oó]nch[aá]' OR       text(c.id) ~* 'c[oó]nch[aá]' OR       lower(profesor) ~* 'c[oó]nch[aá]')  AND (lower(asignatura) ~* 'r[oó]j[aá]s' OR       cod_asignatura ~* 'r[oó]j[aá]s' OR       text(c.id) ~* 'r[oó]j[aá]s' OR       lower(profesor) ~* 'r[oó]j[aá]s')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
