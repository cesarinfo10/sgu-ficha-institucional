COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'tr[aá]b[aá]j[oó]' OR       cod_asignatura ~* 'tr[aá]b[aá]j[oó]' OR       text(c.id) ~* 'tr[aá]b[aá]j[oó]' OR       lower(profesor) ~* 'tr[aá]b[aá]j[oó]')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
