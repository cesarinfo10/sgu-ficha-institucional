COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó]s[eé]m[aá]r[ií][eé]' OR       cod_asignatura ~* 'r[oó]s[eé]m[aá]r[ií][eé]' OR       text(c.id) ~* 'r[oó]s[eé]m[aá]r[ií][eé]' OR       lower(profesor) ~* 'r[oó]s[eé]m[aá]r[ií][eé]')  AND (lower(asignatura) ~* '[aá]ndr[eé][aá]' OR       cod_asignatura ~* '[aá]ndr[eé][aá]' OR       text(c.id) ~* '[aá]ndr[eé][aá]' OR       lower(profesor) ~* '[aá]ndr[eé][aá]')  AND (lower(asignatura) ~* 'v[eé]g[aá]' OR       cod_asignatura ~* 'v[eé]g[aá]' OR       text(c.id) ~* 'v[eé]g[aá]' OR       lower(profesor) ~* 'v[eé]g[aá]')  AND (lower(asignatura) ~* '[aá]rc[oó]s' OR       cod_asignatura ~* '[aá]rc[oó]s' OR       text(c.id) ~* '[aá]rc[oó]s' OR       lower(profesor) ~* '[aá]rc[oó]s')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
