COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]rl[eé]n[ií]s' OR       cod_asignatura ~* 'm[aá]rl[eé]n[ií]s' OR       text(c.id) ~* 'm[aá]rl[eé]n[ií]s' OR       lower(profesor) ~* 'm[aá]rl[eé]n[ií]s')  AND (lower(asignatura) ~* 'm[aá]r[ií]s[oó]l' OR       cod_asignatura ~* 'm[aá]r[ií]s[oó]l' OR       text(c.id) ~* 'm[aá]r[ií]s[oó]l' OR       lower(profesor) ~* 'm[aá]r[ií]s[oó]l')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* 'm[aá]rt[ií]n[eé]z' OR       cod_asignatura ~* 'm[aá]rt[ií]n[eé]z' OR       text(c.id) ~* 'm[aá]rt[ií]n[eé]z' OR       lower(profesor) ~* 'm[aá]rt[ií]n[eé]z')  AND (lower(asignatura) ~* 'f[uú][eé]nt[eé]s' OR       cod_asignatura ~* 'f[uú][eé]nt[eé]s' OR       text(c.id) ~* 'f[uú][eé]nt[eé]s' OR       lower(profesor) ~* 'f[uú][eé]nt[eé]s')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
