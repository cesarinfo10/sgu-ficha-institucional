COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[uú][ií]s' OR       cod_asignatura ~* 'l[uú][ií]s' OR       text(c.id) ~* 'l[uú][ií]s' OR       lower(profesor) ~* 'l[uú][ií]s')  AND (lower(asignatura) ~* '[aá]lb[eé]rt[oó]' OR       cod_asignatura ~* '[aá]lb[eé]rt[oó]' OR       text(c.id) ~* '[aá]lb[eé]rt[oó]' OR       lower(profesor) ~* '[aá]lb[eé]rt[oó]')  AND (lower(asignatura) ~* 'v[eé]n[eé]g[aá]s' OR       cod_asignatura ~* 'v[eé]n[eé]g[aá]s' OR       text(c.id) ~* 'v[eé]n[eé]g[aá]s' OR       lower(profesor) ~* 'v[eé]n[eé]g[aá]s')  AND (lower(asignatura) ~* 'r[aá]m[oó]s' OR       cod_asignatura ~* 'r[aá]m[oó]s' OR       text(c.id) ~* 'r[aá]m[oó]s' OR       lower(profesor) ~* 'r[aá]m[oó]s')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
