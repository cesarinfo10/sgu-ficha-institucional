COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]t[ií]c[aá]' OR       cod_asignatura ~* '[eé]t[ií]c[aá]' OR       text(c.id) ~* '[eé]t[ií]c[aá]' OR       lower(profesor) ~* '[eé]t[ií]c[aá]')  AND (lower(asignatura) ~* 'y' OR       cod_asignatura ~* 'y' OR       text(c.id) ~* 'y' OR       lower(profesor) ~* 'y')  AND (lower(asignatura) ~* '[eé]d[uú]c[aá]c[ií][oó]n' OR       cod_asignatura ~* '[eé]d[uú]c[aá]c[ií][oó]n' OR       text(c.id) ~* '[eé]d[uú]c[aá]c[ií][oó]n' OR       lower(profesor) ~* '[eé]d[uú]c[aá]c[ií][oó]n')  AND (lower(asignatura) ~* 'c[ií]v[ií]c[aá]' OR       cod_asignatura ~* 'c[ií]v[ií]c[aá]' OR       text(c.id) ~* 'c[ií]v[ií]c[aá]' OR       lower(profesor) ~* 'c[ií]v[ií]c[aá]')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
