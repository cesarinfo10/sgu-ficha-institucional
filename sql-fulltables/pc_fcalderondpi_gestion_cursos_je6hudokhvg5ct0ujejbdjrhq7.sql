COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'cl[aá][uú]d[ií][aá]' OR       cod_asignatura ~* 'cl[aá][uú]d[ií][aá]' OR       text(c.id) ~* 'cl[aá][uú]d[ií][aá]' OR       lower(profesor) ~* 'cl[aá][uú]d[ií][aá]')  AND (lower(asignatura) ~* 'h[eé]rr[eé]r[aá]' OR       cod_asignatura ~* 'h[eé]rr[eé]r[aá]' OR       text(c.id) ~* 'h[eé]rr[eé]r[aá]' OR       lower(profesor) ~* 'h[eé]rr[eé]r[aá]')  AND (lower(asignatura) ~* 'c[ií][uú]d[aá]d' OR       cod_asignatura ~* 'c[ií][uú]d[aá]d' OR       text(c.id) ~* 'c[ií][uú]d[aá]d' OR       lower(profesor) ~* 'c[ií][uú]d[aá]d')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
