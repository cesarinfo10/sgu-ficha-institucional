COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '17643667' OR       cod_asignatura ~* '17643667' OR       text(c.id) ~* '17643667' OR       lower(profesor) ~* '17643667')  AND array[59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12] && (ids_carreras)  
