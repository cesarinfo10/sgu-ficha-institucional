COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó]c[ií][oó]' OR       cod_asignatura ~* 'r[oó]c[ií][oó]' OR       text(c.id) ~* 'r[oó]c[ií][oó]' OR       lower(profesor) ~* 'r[oó]c[ií][oó]')  AND (lower(asignatura) ~* 's[oó]l[eé]d[aá]d' OR       cod_asignatura ~* 's[oó]l[eé]d[aá]d' OR       text(c.id) ~* 's[oó]l[eé]d[aá]d' OR       lower(profesor) ~* 's[oó]l[eé]d[aá]d')  AND (lower(asignatura) ~* 'r[ií]ff[oó]' OR       cod_asignatura ~* 'r[ií]ff[oó]' OR       text(c.id) ~* 'r[ií]ff[oó]' OR       lower(profesor) ~* 'r[ií]ff[oó]')  AND (lower(asignatura) ~* 's[aá]n' OR       cod_asignatura ~* 's[aá]n' OR       text(c.id) ~* 's[aá]n' OR       lower(profesor) ~* 's[aá]n')  AND (lower(asignatura) ~* 'm[aá]rt[ií]n' OR       cod_asignatura ~* 'm[aá]rt[ií]n' OR       text(c.id) ~* 'm[aá]rt[ií]n' OR       lower(profesor) ~* 'm[aá]rt[ií]n')  AND id_carrera IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12)  
