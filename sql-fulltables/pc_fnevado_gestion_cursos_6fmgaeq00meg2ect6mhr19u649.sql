COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR       cod_asignatura ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR       text(c.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR       lower(profesor) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,157,12] && (ids_carreras)  
