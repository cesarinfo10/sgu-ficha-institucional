COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá]s[aá]nt[ií][aá]s' OR       cod_asignatura ~* 'p[aá]s[aá]nt[ií][aá]s' OR       text(c.id) ~* 'p[aá]s[aá]nt[ií][aá]s' OR       lower(profesor) ~* 'p[aá]s[aá]nt[ií][aá]s')  
