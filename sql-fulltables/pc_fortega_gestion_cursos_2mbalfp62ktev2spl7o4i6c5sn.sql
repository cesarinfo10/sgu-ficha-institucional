COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó]b[eé]rt[oó]' OR       cod_asignatura ~* 'r[oó]b[eé]rt[oó]' OR       text(c.id) ~* 'r[oó]b[eé]rt[oó]' OR       lower(profesor) ~* 'r[oó]b[eé]rt[oó]')  AND (lower(asignatura) ~* 'd[ií][aá]z' OR       cod_asignatura ~* 'd[ií][aá]z' OR       text(c.id) ~* 'd[ií][aá]z' OR       lower(profesor) ~* 'd[ií][aá]z')  
