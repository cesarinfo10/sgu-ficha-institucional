COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]r[eé]ch[oó]s' OR       cod_asignatura ~* 'd[eé]r[eé]ch[oó]s' OR       text(c.id) ~* 'd[eé]r[eé]ch[oó]s' OR       lower(profesor) ~* 'd[eé]r[eé]ch[oó]s')  AND (lower(asignatura) ~* 'h[uú]m[aá]n[oó]s' OR       cod_asignatura ~* 'h[uú]m[aá]n[oó]s' OR       text(c.id) ~* 'h[uú]m[aá]n[oó]s' OR       lower(profesor) ~* 'h[uú]m[aá]n[oó]s')  
