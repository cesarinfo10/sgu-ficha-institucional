COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]sv[aá]ld[oó]' OR       cod_asignatura ~* '[oó]sv[aá]ld[oó]' OR       text(c.id) ~* '[oó]sv[aá]ld[oó]' OR       lower(profesor) ~* '[oó]sv[aá]ld[oó]')  AND (lower(asignatura) ~* '[aá]r[aá]v[eé]n[aá]' OR       cod_asignatura ~* '[aá]r[aá]v[eé]n[aá]' OR       text(c.id) ~* '[aá]r[aá]v[eé]n[aá]' OR       lower(profesor) ~* '[aá]r[aá]v[eé]n[aá]')  
