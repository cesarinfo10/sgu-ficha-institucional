COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]m[eé]rg[eé]nc[ií][aá]' OR       cod_asignatura ~* '[eé]m[eé]rg[eé]nc[ií][aá]' OR       text(c.id) ~* '[eé]m[eé]rg[eé]nc[ií][aá]' OR       lower(profesor) ~* '[eé]m[eé]rg[eé]nc[ií][aá]')  
