COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d' OR       cod_asignatura ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d' OR       text(c.id) ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d' OR       lower(profesor) ~* 'd[ií]sc[aá]p[aá]c[ií]d[aá]d')  
