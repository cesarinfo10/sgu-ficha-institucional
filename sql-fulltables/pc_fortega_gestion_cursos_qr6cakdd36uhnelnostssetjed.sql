COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rt[eé]g[aá]' OR       cod_asignatura ~* '[oó]rt[eé]g[aá]' OR       text(c.id) ~* '[oó]rt[eé]g[aá]' OR       lower(profesor) ~* '[oó]rt[eé]g[aá]')  AND (lower(asignatura) ~* 'fr[eé][ií]' OR       cod_asignatura ~* 'fr[eé][ií]' OR       text(c.id) ~* 'fr[eé][ií]' OR       lower(profesor) ~* 'fr[eé][ií]')  
