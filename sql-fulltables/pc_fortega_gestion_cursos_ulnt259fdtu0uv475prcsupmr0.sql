COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       cod_asignatura ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       text(c.id) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       lower(profesor) ~* 's[aá]nt[ií]b[aá]ñ[eé]z')  
