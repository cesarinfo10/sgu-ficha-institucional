COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'chr[ií]st[ií][aá]ns[eé]n' OR       cod_asignatura ~* 'chr[ií]st[ií][aá]ns[eé]n' OR       text(c.id) ~* 'chr[ií]st[ií][aá]ns[eé]n' OR       lower(profesor) ~* 'chr[ií]st[ií][aá]ns[eé]n')  AND (lower(asignatura) ~* 'g[oó]d[oó]y' OR       cod_asignatura ~* 'g[oó]d[oó]y' OR       text(c.id) ~* 'g[oó]d[oó]y' OR       lower(profesor) ~* 'g[oó]d[oó]y')  AND (lower(asignatura) ~* '[aá]ld[aá]y' OR       cod_asignatura ~* '[aá]ld[aá]y' OR       text(c.id) ~* '[aá]ld[aá]y' OR       lower(profesor) ~* '[aá]ld[aá]y')  
