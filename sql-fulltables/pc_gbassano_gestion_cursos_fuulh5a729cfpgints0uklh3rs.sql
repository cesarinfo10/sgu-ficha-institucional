COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'th[oó]m[aá]s' OR       cod_asignatura ~* 'th[oó]m[aá]s' OR       text(c.id) ~* 'th[oó]m[aá]s' OR       lower(profesor) ~* 'th[oó]m[aá]s')  AND (lower(asignatura) ~* 'h[eé]ct[oó]r' OR       cod_asignatura ~* 'h[eé]ct[oó]r' OR       text(c.id) ~* 'h[eé]ct[oó]r' OR       lower(profesor) ~* 'h[eé]ct[oó]r')  AND (lower(asignatura) ~* 'm[ií]x' OR       cod_asignatura ~* 'm[ií]x' OR       text(c.id) ~* 'm[ií]x' OR       lower(profesor) ~* 'm[ií]x')  AND (lower(asignatura) ~* 'j[ií]m[eé]n[eé]z' OR       cod_asignatura ~* 'j[ií]m[eé]n[eé]z' OR       text(c.id) ~* 'j[ií]m[eé]n[eé]z' OR       lower(profesor) ~* 'j[ií]m[eé]n[eé]z')  
