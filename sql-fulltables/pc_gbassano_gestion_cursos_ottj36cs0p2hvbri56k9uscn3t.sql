COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[eé]rn[aá]nd[oó]' OR       cod_asignatura ~* 'f[eé]rn[aá]nd[oó]' OR       text(c.id) ~* 'f[eé]rn[aá]nd[oó]' OR       lower(profesor) ~* 'f[eé]rn[aá]nd[oó]')  AND (lower(asignatura) ~* 'm[oó]r[aá]l[eé]s' OR       cod_asignatura ~* 'm[oó]r[aá]l[eé]s' OR       text(c.id) ~* 'm[oó]r[aá]l[eé]s' OR       lower(profesor) ~* 'm[oó]r[aá]l[eé]s')  
