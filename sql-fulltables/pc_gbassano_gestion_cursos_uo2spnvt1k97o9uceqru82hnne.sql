COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[oó]rg[eé]' OR       cod_asignatura ~* 'j[oó]rg[eé]' OR       text(c.id) ~* 'j[oó]rg[eé]' OR       lower(profesor) ~* 'j[oó]rg[eé]')  AND (lower(asignatura) ~* '[oó]s[oó]r[ií][oó]' OR       cod_asignatura ~* '[oó]s[oó]r[ií][oó]' OR       text(c.id) ~* '[oó]s[oó]r[ií][oó]' OR       lower(profesor) ~* '[oó]s[oó]r[ií][oó]')  AND (lower(asignatura) ~* 'm[uú]ñ[oó]z' OR       cod_asignatura ~* 'm[uú]ñ[oó]z' OR       text(c.id) ~* 'm[uú]ñ[oó]z' OR       lower(profesor) ~* 'm[uú]ñ[oó]z')  
