COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'fl[oó]r[aá]' OR       cod_asignatura ~* 'fl[oó]r[aá]' OR       text(c.id) ~* 'fl[oó]r[aá]' OR       lower(profesor) ~* 'fl[oó]r[aá]')  AND (lower(asignatura) ~* 'm[aá]r[ií][aá]' OR       cod_asignatura ~* 'm[aá]r[ií][aá]' OR       text(c.id) ~* 'm[aá]r[ií][aá]' OR       lower(profesor) ~* 'm[aá]r[ií][aá]')  AND (lower(asignatura) ~* 'v[ií]d[aá]l' OR       cod_asignatura ~* 'v[ií]d[aá]l' OR       text(c.id) ~* 'v[ií]d[aá]l' OR       lower(profesor) ~* 'v[ií]d[aá]l')  AND (lower(asignatura) ~* 's[aá]l[aá]s' OR       cod_asignatura ~* 's[aá]l[aá]s' OR       text(c.id) ~* 's[aá]l[aá]s' OR       lower(profesor) ~* 's[aá]l[aá]s')  
