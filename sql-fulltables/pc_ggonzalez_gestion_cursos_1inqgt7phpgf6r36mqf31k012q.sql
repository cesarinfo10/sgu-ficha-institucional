COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[ií]l[oó]s[oó]f[ií][aá]' OR       cod_asignatura ~* 'f[ií]l[oó]s[oó]f[ií][aá]' OR       text(c.id) ~* 'f[ií]l[oó]s[oó]f[ií][aá]' OR       lower(profesor) ~* 'f[ií]l[oó]s[oó]f[ií][aá]')  
