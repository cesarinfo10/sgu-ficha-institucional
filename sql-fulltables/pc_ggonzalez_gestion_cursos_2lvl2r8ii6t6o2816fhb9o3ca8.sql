COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]n[ií][eé]ll[oó]' OR       cod_asignatura ~* '[aá]n[ií][eé]ll[oó]' OR       text(c.id) ~* '[aá]n[ií][eé]ll[oó]' OR       lower(profesor) ~* '[aá]n[ií][eé]ll[oó]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* 'g[aá]rg[ií][uú]l[oó]' OR       cod_asignatura ~* 'g[aá]rg[ií][uú]l[oó]' OR       text(c.id) ~* 'g[aá]rg[ií][uú]l[oó]' OR       lower(profesor) ~* 'g[aá]rg[ií][uú]l[oó]')  
