COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       cod_asignatura ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       text(c.id) ~* 'f[uú]nd[aá]m[eé]nt[oó]s' OR       lower(profesor) ~* 'f[uú]nd[aá]m[eé]nt[oó]s')  
