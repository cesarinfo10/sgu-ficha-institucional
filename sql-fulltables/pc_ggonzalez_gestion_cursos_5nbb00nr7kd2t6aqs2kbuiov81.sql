COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]c[aá]r[eé]n[aá]' OR       cod_asignatura ~* 'm[aá]c[aá]r[eé]n[aá]' OR       text(c.id) ~* 'm[aá]c[aá]r[eé]n[aá]' OR       lower(profesor) ~* 'm[aá]c[aá]r[eé]n[aá]')  AND (lower(asignatura) ~* 'r[uú]b[ií]l[aá]r' OR       cod_asignatura ~* 'r[uú]b[ií]l[aá]r' OR       text(c.id) ~* 'r[uú]b[ií]l[aá]r' OR       lower(profesor) ~* 'r[uú]b[ií]l[aá]r')  AND (lower(asignatura) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR       cod_asignatura ~* 'n[aá]v[aá]rr[eé]t[eé]' OR       text(c.id) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR       lower(profesor) ~* 'n[aá]v[aá]rr[eé]t[eé]')  
