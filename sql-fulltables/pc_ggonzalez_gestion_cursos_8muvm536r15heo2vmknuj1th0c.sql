COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[oó]s' OR       cod_asignatura ~* 'c[aá]rl[oó]s' OR       text(c.id) ~* 'c[aá]rl[oó]s' OR       lower(profesor) ~* 'c[aá]rl[oó]s')  AND (lower(asignatura) ~* 'h[eé]rn[aá]n' OR       cod_asignatura ~* 'h[eé]rn[aá]n' OR       text(c.id) ~* 'h[eé]rn[aá]n' OR       lower(profesor) ~* 'h[eé]rn[aá]n')  AND (lower(asignatura) ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       cod_asignatura ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       text(c.id) ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       lower(profesor) ~* '[aá]p[aá]r[ií]c[ií][oó]')  AND (lower(asignatura) ~* 'p[uú][eé]nt[eé]s' OR       cod_asignatura ~* 'p[uú][eé]nt[eé]s' OR       text(c.id) ~* 'p[uú][eé]nt[eé]s' OR       lower(profesor) ~* 'p[uú][eé]nt[eé]s')  
