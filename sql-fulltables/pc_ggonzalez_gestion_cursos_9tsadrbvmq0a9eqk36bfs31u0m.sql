COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]nr[ií]q[uú][eé]z' OR       cod_asignatura ~* 'm[aá]nr[ií]q[uú][eé]z' OR       text(c.id) ~* 'm[aá]nr[ií]q[uú][eé]z' OR       lower(profesor) ~* 'm[aá]nr[ií]q[uú][eé]z')  
