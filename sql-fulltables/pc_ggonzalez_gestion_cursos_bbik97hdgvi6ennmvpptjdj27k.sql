COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]nn[oó]v[aá]c[ií][oó]n' OR       cod_asignatura ~* '[ií]nn[oó]v[aá]c[ií][oó]n' OR       text(c.id) ~* '[ií]nn[oó]v[aá]c[ií][oó]n' OR       lower(profesor) ~* '[ií]nn[oó]v[aá]c[ií][oó]n')  
