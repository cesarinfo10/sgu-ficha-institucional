COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]m[eé]rg[eé]nc' OR       cod_asignatura ~* '[eé]m[eé]rg[eé]nc' OR       text(c.id) ~* '[eé]m[eé]rg[eé]nc' OR       lower(profesor) ~* '[eé]m[eé]rg[eé]nc')  
