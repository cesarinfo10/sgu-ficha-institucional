COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       cod_asignatura ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       text(c.id) ~* 'h[uú]m[aá]n[ií]sm[oó]' OR       lower(profesor) ~* 'h[uú]m[aá]n[ií]sm[oó]')  
