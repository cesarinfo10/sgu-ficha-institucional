COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]lv[aá]r[oó]' OR       cod_asignatura ~* '[aá]lv[aá]r[oó]' OR       text(c.id) ~* '[aá]lv[aá]r[oó]' OR       lower(profesor) ~* '[aá]lv[aá]r[oó]')  AND (lower(asignatura) ~* 'c[oó]rr[eé][aá]' OR       cod_asignatura ~* 'c[oó]rr[eé][aá]' OR       text(c.id) ~* 'c[oó]rr[eé][aá]' OR       lower(profesor) ~* 'c[oó]rr[eé][aá]')  
