COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ntr[oó]d' OR       cod_asignatura ~* '[ií]ntr[oó]d' OR       text(c.id) ~* '[ií]ntr[oó]d' OR       lower(profesor) ~* '[ií]ntr[oó]d')  AND (lower(asignatura) ~* '[aá]' OR       cod_asignatura ~* '[aá]' OR       text(c.id) ~* '[aá]' OR       lower(profesor) ~* '[aá]')  AND (lower(asignatura) ~* 'v[ií]d[aá]' OR       cod_asignatura ~* 'v[ií]d[aá]' OR       text(c.id) ~* 'v[ií]d[aá]' OR       lower(profesor) ~* 'v[ií]d[aá]')  
