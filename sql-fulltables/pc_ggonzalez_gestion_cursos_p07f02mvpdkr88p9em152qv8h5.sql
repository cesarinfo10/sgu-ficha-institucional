COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]ns[aá]m[ií][eé]nt[oó]' OR       cod_asignatura ~* 'p[eé]ns[aá]m[ií][eé]nt[oó]' OR       text(c.id) ~* 'p[eé]ns[aá]m[ií][eé]nt[oó]' OR       lower(profesor) ~* 'p[eé]ns[aá]m[ií][eé]nt[oó]')  
