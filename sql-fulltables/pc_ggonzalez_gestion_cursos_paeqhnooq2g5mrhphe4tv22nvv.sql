COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]d[aá]' OR       cod_asignatura ~* 'v[ií]d[aá]' OR       text(c.id) ~* 'v[ií]d[aá]' OR       lower(profesor) ~* 'v[ií]d[aá]')  AND (lower(asignatura) ~* '[aá]c[aá]' OR       cod_asignatura ~* '[aá]c[aá]' OR       text(c.id) ~* '[aá]c[aá]' OR       lower(profesor) ~* '[aá]c[aá]')  
