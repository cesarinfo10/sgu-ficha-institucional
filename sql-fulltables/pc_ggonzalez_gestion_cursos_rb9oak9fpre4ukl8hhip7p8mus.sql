COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá][uú]l[aá]' OR       cod_asignatura ~* 'p[aá][uú]l[aá]' OR       text(c.id) ~* 'p[aá][uú]l[aá]' OR       lower(profesor) ~* 'p[aá][uú]l[aá]')  AND (lower(asignatura) ~* 'p[oó]bl[eé]t[eé]' OR       cod_asignatura ~* 'p[oó]bl[eé]t[eé]' OR       text(c.id) ~* 'p[oó]bl[eé]t[eé]' OR       lower(profesor) ~* 'p[oó]bl[eé]t[eé]')  
