COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[aá]m[oó]n' OR       cod_asignatura ~* 'r[aá]m[oó]n' OR       text(c.id) ~* 'r[aá]m[oó]n' OR       lower(profesor) ~* 'r[aá]m[oó]n')  AND (lower(asignatura) ~* 'j[oó][aá]q[uú][ií]n' OR       cod_asignatura ~* 'j[oó][aá]q[uú][ií]n' OR       text(c.id) ~* 'j[oó][aá]q[uú][ií]n' OR       lower(profesor) ~* 'j[oó][aá]q[uú][ií]n')  AND (lower(asignatura) ~* '[eé]st[aá]b[aá]' OR       cod_asignatura ~* '[eé]st[aá]b[aá]' OR       text(c.id) ~* '[eé]st[aá]b[aá]' OR       lower(profesor) ~* '[eé]st[aá]b[aá]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  
