COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       cod_asignatura ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       text(c.id) ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       lower(profesor) ~* '[eé]c[oó]n[oó]m[ií][aá]')  AND (lower(asignatura) ~* 'y' OR       cod_asignatura ~* 'y' OR       text(c.id) ~* 'y' OR       lower(profesor) ~* 'y')  AND (lower(asignatura) ~* 's[oó]st[eé]n[ií]b[ií]l[ií]d[aá]d' OR       cod_asignatura ~* 's[oó]st[eé]n[ií]b[ií]l[ií]d[aá]d' OR       text(c.id) ~* 's[oó]st[eé]n[ií]b[ií]l[ií]d[aá]d' OR       lower(profesor) ~* 's[oó]st[eé]n[ií]b[ií]l[ií]d[aá]d')  
