COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[uú]s[aá]n' OR       cod_asignatura ~* 's[uú]s[aá]n' OR       text(c.id) ~* 's[uú]s[aá]n' OR       lower(profesor) ~* 's[uú]s[aá]n')  AND (lower(asignatura) ~* 'b[aá]s[ií]s' OR       cod_asignatura ~* 'b[aá]s[ií]s' OR       text(c.id) ~* 'b[aá]s[ií]s' OR       lower(profesor) ~* 'b[aá]s[ií]s')  
