COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'cr[ií]st[ií][aá]n' OR       cod_asignatura ~* 'cr[ií]st[ií][aá]n' OR       text(c.id) ~* 'cr[ií]st[ií][aá]n' OR       lower(profesor) ~* 'cr[ií]st[ií][aá]n')  AND (lower(asignatura) ~* 'm[uú]ñ[oó]z' OR       cod_asignatura ~* 'm[uú]ñ[oó]z' OR       text(c.id) ~* 'm[uú]ñ[oó]z' OR       lower(profesor) ~* 'm[uú]ñ[oó]z')  
