COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[ií]t' OR       cod_asignatura ~* 's[ií]t' OR       text(c.id) ~* 's[ií]t' OR       lower(profesor) ~* 's[ií]t')  AND (lower(asignatura) ~* '[eé]m[eé]rg' OR       cod_asignatura ~* '[eé]m[eé]rg' OR       text(c.id) ~* '[eé]m[eé]rg' OR       lower(profesor) ~* '[eé]m[eé]rg')  
