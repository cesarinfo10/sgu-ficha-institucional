COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]g[ií]m[eé]n' OR       cod_asignatura ~* 'r[eé]g[ií]m[eé]n' OR       text(c.id) ~* 'r[eé]g[ií]m[eé]n' OR       lower(profesor) ~* 'r[eé]g[ií]m[eé]n')  AND (lower(asignatura) ~* 'tr[ií]b[uú]t[aá]r[ií][oó]' OR       cod_asignatura ~* 'tr[ií]b[uú]t[aá]r[ií][oó]' OR       text(c.id) ~* 'tr[ií]b[uú]t[aá]r[ií][oó]' OR       lower(profesor) ~* 'tr[ií]b[uú]t[aá]r[ií][oó]')  AND (lower(asignatura) ~* '[eé]sp[eé]c[ií][aá]l' OR       cod_asignatura ~* '[eé]sp[eé]c[ií][aá]l' OR       text(c.id) ~* '[eé]sp[eé]c[ií][aá]l' OR       lower(profesor) ~* '[eé]sp[eé]c[ií][aá]l')  
