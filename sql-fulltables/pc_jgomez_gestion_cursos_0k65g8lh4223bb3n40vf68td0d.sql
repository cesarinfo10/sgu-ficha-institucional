COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[oó]m[eé]z' OR       cod_asignatura ~* 'g[oó]m[eé]z' OR       text(c.id) ~* 'g[oó]m[eé]z' OR       lower(profesor) ~* 'g[oó]m[eé]z')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
