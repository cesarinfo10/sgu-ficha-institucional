COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[aá]ct[ií]c[aá]s' OR       cod_asignatura ~* 'pr[aá]ct[ií]c[aá]s' OR       text(c.id) ~* 'pr[aá]ct[ií]c[aá]s' OR       lower(profesor) ~* 'pr[aá]ct[ií]c[aá]s')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
