COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[oó]s[eé]' OR       cod_asignatura ~* 'j[oó]s[eé]' OR       text(c.id) ~* 'j[oó]s[eé]' OR       lower(profesor) ~* 'j[oó]s[eé]')  AND (lower(asignatura) ~* 'l[uú][ií]s' OR       cod_asignatura ~* 'l[uú][ií]s' OR       text(c.id) ~* 'l[uú][ií]s' OR       lower(profesor) ~* 'l[uú][ií]s')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
