COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       cod_asignatura ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       text(c.id) ~* '[eé]c[oó]n[oó]m[ií][aá]' OR       lower(profesor) ~* '[eé]c[oó]n[oó]m[ií][aá]')  AND array[108,96,25,109,37,70,135,153,112,17,12] && (ids_carreras)  
