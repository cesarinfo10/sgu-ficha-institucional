COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[eé]rn[aá]nd[eé]z' OR       cod_asignatura ~* 'h[eé]rn[aá]nd[eé]z' OR       text(c.id) ~* 'h[eé]rn[aá]nd[eé]z' OR       lower(profesor) ~* 'h[eé]rn[aá]nd[eé]z')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
