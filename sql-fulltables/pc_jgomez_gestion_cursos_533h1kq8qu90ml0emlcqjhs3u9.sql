COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]cr[oó]' OR       cod_asignatura ~* 'm[aá]cr[oó]' OR       text(c.id) ~* 'm[aá]cr[oó]' OR       lower(profesor) ~* 'm[aá]cr[oó]')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
