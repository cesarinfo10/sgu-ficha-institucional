COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[ií]lv[aá]' OR       cod_asignatura ~* 's[ií]lv[aá]' OR       text(c.id) ~* 's[ií]lv[aá]' OR       lower(profesor) ~* 's[ií]lv[aá]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
