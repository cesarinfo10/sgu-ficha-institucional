COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[oó]c[ií][aá]l' OR       cod_asignatura ~* 's[oó]c[ií][aá]l' OR       text(c.id) ~* 's[oó]c[ií][aá]l' OR       lower(profesor) ~* 's[oó]c[ií][aá]l')  AND (lower(asignatura) ~* '[ií][ií]' OR       cod_asignatura ~* '[ií][ií]' OR       text(c.id) ~* '[ií][ií]' OR       lower(profesor) ~* '[ií][ií]')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
