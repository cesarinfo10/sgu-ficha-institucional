COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[ií]st[eé]m[aá]s' OR       cod_asignatura ~* 's[ií]st[eé]m[aá]s' OR       text(c.id) ~* 's[ií]st[eé]m[aá]s' OR       lower(profesor) ~* 's[ií]st[eé]m[aá]s')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
