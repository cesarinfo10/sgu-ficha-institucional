COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n' OR       cod_asignatura ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n' OR       text(c.id) ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n' OR       lower(profesor) ~* '[aá]dm[ií]n[ií]str[aá]c[ií][oó]n')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
