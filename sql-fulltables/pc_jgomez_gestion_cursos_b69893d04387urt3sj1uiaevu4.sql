COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'myr[ií][aá]m' OR       cod_asignatura ~* 'myr[ií][aá]m' OR       text(c.id) ~* 'myr[ií][aá]m' OR       lower(profesor) ~* 'myr[ií][aá]m')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
