COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[uú]g[oó]' OR       cod_asignatura ~* 'h[uú]g[oó]' OR       text(c.id) ~* 'h[uú]g[oó]' OR       lower(profesor) ~* 'h[uú]g[oó]')  AND array[108,96,25,109,37,70,135,153,112,17,12] && (ids_carreras)  
