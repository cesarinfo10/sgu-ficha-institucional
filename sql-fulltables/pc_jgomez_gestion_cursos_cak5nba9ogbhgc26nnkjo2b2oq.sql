COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND 17 = ANY (ids_carreras)  AND c.ano=2023  AND c.semestre=2  AND ('PRE' = ANY (regimenes))  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
