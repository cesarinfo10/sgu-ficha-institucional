COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[aá]nd[oó]v[aá]l' OR       cod_asignatura ~* 's[aá]nd[oó]v[aá]l' OR       text(c.id) ~* 's[aá]nd[oó]v[aá]l' OR       lower(profesor) ~* 's[aá]nd[oó]v[aá]l')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
