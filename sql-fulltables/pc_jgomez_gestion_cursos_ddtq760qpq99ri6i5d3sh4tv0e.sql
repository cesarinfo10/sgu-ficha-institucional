COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rr[eé]ñ[oó]' OR       cod_asignatura ~* 'c[aá]rr[eé]ñ[oó]' OR       text(c.id) ~* 'c[aá]rr[eé]ñ[oó]' OR       lower(profesor) ~* 'c[aá]rr[eé]ñ[oó]')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
