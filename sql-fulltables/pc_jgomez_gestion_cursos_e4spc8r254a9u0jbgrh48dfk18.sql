COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rt[eé]g[aá]' OR       cod_asignatura ~* '[oó]rt[eé]g[aá]' OR       text(c.id) ~* '[oó]rt[eé]g[aá]' OR       lower(profesor) ~* '[oó]rt[eé]g[aá]')  AND array[108,96,25,109,37,70,135,153,112,17,12] && (ids_carreras)  
