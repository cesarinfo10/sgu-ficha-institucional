COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[ií]cr[oó]' OR       cod_asignatura ~* 'm[ií]cr[oó]' OR       text(c.id) ~* 'm[ií]cr[oó]' OR       lower(profesor) ~* 'm[ií]cr[oó]')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
