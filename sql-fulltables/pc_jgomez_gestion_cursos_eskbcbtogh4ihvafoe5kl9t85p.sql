COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá]tr[ií]c[ií][aá]' OR       cod_asignatura ~* 'p[aá]tr[ií]c[ií][aá]' OR       text(c.id) ~* 'p[aá]tr[ií]c[ií][aá]' OR       lower(profesor) ~* 'p[aá]tr[ií]c[ií][aá]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
