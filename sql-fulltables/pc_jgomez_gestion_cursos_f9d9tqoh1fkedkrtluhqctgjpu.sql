COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]r[ií][aá]' OR       cod_asignatura ~* 'm[aá]r[ií][aá]' OR       text(c.id) ~* 'm[aá]r[ií][aá]' OR       lower(profesor) ~* 'm[aá]r[ií][aá]')  AND (lower(asignatura) ~* 'p[aá]z' OR       cod_asignatura ~* 'p[aá]z' OR       text(c.id) ~* 'p[aá]z' OR       lower(profesor) ~* 'p[aá]z')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
