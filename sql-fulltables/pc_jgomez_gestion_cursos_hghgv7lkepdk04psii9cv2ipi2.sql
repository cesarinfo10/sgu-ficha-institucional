COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]d[ií]s[oó]n' OR       cod_asignatura ~* '[eé]d[ií]s[oó]n' OR       text(c.id) ~* '[eé]d[ií]s[oó]n' OR       lower(profesor) ~* '[eé]d[ií]s[oó]n')  AND array[108,96,25,109,37,70,135,153,112,17,12] && (ids_carreras)  
