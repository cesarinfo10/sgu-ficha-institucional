COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[oó]l[aá]nc[oó]' OR       cod_asignatura ~* 'p[oó]l[aá]nc[oó]' OR       text(c.id) ~* 'p[oó]l[aá]nc[oó]' OR       lower(profesor) ~* 'p[oó]l[aá]nc[oó]')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
