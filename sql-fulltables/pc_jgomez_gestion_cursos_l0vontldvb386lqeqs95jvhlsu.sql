COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]rg[ií][oó]' OR       cod_asignatura ~* 's[eé]rg[ií][oó]' OR       text(c.id) ~* 's[eé]rg[ií][oó]' OR       lower(profesor) ~* 's[eé]rg[ií][oó]')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
