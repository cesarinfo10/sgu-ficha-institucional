COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n' OR       cod_asignatura ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n' OR       text(c.id) ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n' OR       lower(profesor) ~* '[ií]ntr[oó]d[uú]cc[ií][oó]n')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
