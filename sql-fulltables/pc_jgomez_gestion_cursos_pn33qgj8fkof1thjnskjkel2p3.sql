COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR       cod_asignatura ~* 'v[aá]l[eé]nt[ií]n[aá]' OR       text(c.id) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR       lower(profesor) ~* 'v[aá]l[eé]nt[ií]n[aá]')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
