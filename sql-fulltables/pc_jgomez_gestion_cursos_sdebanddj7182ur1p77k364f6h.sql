COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'fl[oó]r[eé]s' OR       cod_asignatura ~* 'fl[oó]r[eé]s' OR       text(c.id) ~* 'fl[oó]r[eé]s' OR       lower(profesor) ~* 'fl[oó]r[eé]s')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
