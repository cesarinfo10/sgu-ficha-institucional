COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]c[eé]nt[eé]' OR       cod_asignatura ~* 'v[ií]c[eé]nt[eé]' OR       text(c.id) ~* 'v[ií]c[eé]nt[eé]' OR       lower(profesor) ~* 'v[ií]c[eé]nt[eé]')  AND array[108,96,25,109,37,70,135,153,112,17,12] && (ids_carreras)  
