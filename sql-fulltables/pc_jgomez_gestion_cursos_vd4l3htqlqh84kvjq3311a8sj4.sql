COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'll[aá]n[oó]s' OR       cod_asignatura ~* 'll[aá]n[oó]s' OR       text(c.id) ~* 'll[aá]n[oó]s' OR       lower(profesor) ~* 'll[aá]n[oó]s')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
