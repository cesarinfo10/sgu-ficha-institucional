COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]mpr[eé]ns[ií][oó]n' OR       cod_asignatura ~* 'c[oó]mpr[eé]ns[ií][oó]n' OR       text(c.id) ~* 'c[oó]mpr[eé]ns[ií][oó]n' OR       lower(profesor) ~* 'c[oó]mpr[eé]ns[ií][oó]n')  
