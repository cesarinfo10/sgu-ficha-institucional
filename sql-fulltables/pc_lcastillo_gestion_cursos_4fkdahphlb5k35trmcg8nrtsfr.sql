COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]ct[oó]r' OR       cod_asignatura ~* 'v[ií]ct[oó]r' OR       text(c.id) ~* 'v[ií]ct[oó]r' OR       lower(profesor) ~* 'v[ií]ct[oó]r')  AND (lower(asignatura) ~* 's[eé]rg[ií][oó]' OR       cod_asignatura ~* 's[eé]rg[ií][oó]' OR       text(c.id) ~* 's[eé]rg[ií][oó]' OR       lower(profesor) ~* 's[eé]rg[ií][oó]')  AND (lower(asignatura) ~* 'm[eé]n[aá]' OR       cod_asignatura ~* 'm[eé]n[aá]' OR       text(c.id) ~* 'm[eé]n[aá]' OR       lower(profesor) ~* 'm[eé]n[aá]')  AND (lower(asignatura) ~* 'v[eé]rg[aá]r[aá]' OR       cod_asignatura ~* 'v[eé]rg[aá]r[aá]' OR       text(c.id) ~* 'v[eé]rg[aá]r[aá]' OR       lower(profesor) ~* 'v[eé]rg[aá]r[aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
