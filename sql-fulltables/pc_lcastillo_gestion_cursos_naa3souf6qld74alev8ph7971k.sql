COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[eé]st[ií][oó]n' OR       cod_asignatura ~* 'g[eé]st[ií][oó]n' OR       text(c.id) ~* 'g[eé]st[ií][oó]n' OR       lower(profesor) ~* 'g[eé]st[ií][oó]n')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
