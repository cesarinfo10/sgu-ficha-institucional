COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[eé]j[oó]r' OR       cod_asignatura ~* 'm[eé]j[oó]r' OR       text(c.id) ~* 'm[eé]j[oó]r' OR       lower(profesor) ~* 'm[eé]j[oó]r')  AND (lower(asignatura) ~* 'n[ií]ñ[eé]z' OR       cod_asignatura ~* 'n[ií]ñ[eé]z' OR       text(c.id) ~* 'n[ií]ñ[eé]z' OR       lower(profesor) ~* 'n[ií]ñ[eé]z')  AND array[94,97,104,110,98,105,102,91,15,103,126,4,12] && (ids_carreras)  
