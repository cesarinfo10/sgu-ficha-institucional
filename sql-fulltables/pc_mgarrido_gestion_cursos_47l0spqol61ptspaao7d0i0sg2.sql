COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]rt[ií]n' OR       cod_asignatura ~* 'm[aá]rt[ií]n' OR       text(c.id) ~* 'm[aá]rt[ií]n' OR       lower(profesor) ~* 'm[aá]rt[ií]n')  AND (lower(asignatura) ~* 'g[aá]rr[ií]d[oó]' OR       cod_asignatura ~* 'g[aá]rr[ií]d[oó]' OR       text(c.id) ~* 'g[aá]rr[ií]d[oó]' OR       lower(profesor) ~* 'g[aá]rr[ií]d[oó]')  
