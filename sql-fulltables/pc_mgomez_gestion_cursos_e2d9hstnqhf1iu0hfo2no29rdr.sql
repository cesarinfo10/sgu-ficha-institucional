COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rg[aá]n[ií]z[aá]c[ií][oó]n' OR       cod_asignatura ~* '[oó]rg[aá]n[ií]z[aá]c[ií][oó]n' OR       text(c.id) ~* '[oó]rg[aá]n[ií]z[aá]c[ií][oó]n' OR       lower(profesor) ~* '[oó]rg[aá]n[ií]z[aá]c[ií][oó]n')  AND (lower(asignatura) ~* 'd[eé]l' OR       cod_asignatura ~* 'd[eé]l' OR       text(c.id) ~* 'd[eé]l' OR       lower(profesor) ~* 'd[eé]l')  AND (lower(asignatura) ~* '[eé]st[aá]d[oó]' OR       cod_asignatura ~* '[eé]st[aá]d[oó]' OR       text(c.id) ~* '[eé]st[aá]d[oó]' OR       lower(profesor) ~* '[eé]st[aá]d[oó]')  
