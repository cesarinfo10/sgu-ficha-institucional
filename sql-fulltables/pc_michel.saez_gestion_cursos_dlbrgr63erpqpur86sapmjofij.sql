COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé][oó]r[ií][aá]' OR       cod_asignatura ~* 't[eé][oó]r[ií][aá]' OR       text(c.id) ~* 't[eé][oó]r[ií][aá]' OR       lower(profesor) ~* 't[eé][oó]r[ií][aá]')  AND (lower(asignatura) ~* '[eé]c[oó]' OR       cod_asignatura ~* '[eé]c[oó]' OR       text(c.id) ~* '[eé]c[oó]' OR       lower(profesor) ~* '[eé]c[oó]')  
