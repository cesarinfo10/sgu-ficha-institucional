COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]t[eé]m[aá]t[ií]c[aá]' OR       cod_asignatura ~* 'm[aá]t[eé]m[aá]t[ií]c[aá]' OR       text(c.id) ~* 'm[aá]t[eé]m[aá]t[ií]c[aá]' OR       lower(profesor) ~* 'm[aá]t[eé]m[aá]t[ií]c[aá]')  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
