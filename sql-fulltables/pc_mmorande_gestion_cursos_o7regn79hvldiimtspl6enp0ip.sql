COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND 36 = ANY (ids_carreras)  AND c.ano=2024  AND c.semestre=1  AND c.seccion BETWEEN 5 AND 9  AND ('PRE' = ANY (regimenes))  AND array[101,62,26,88,18,3,159,156,36,67,164,12] && (ids_carreras)  
