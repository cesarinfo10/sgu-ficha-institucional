COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR       cod_asignatura ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR       text(c.id) ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR       lower(profesor) ~* 's[eé]m[ií]n[aá]r[ií][oó]')  AND (lower(asignatura) ~* 'pr[oó]c[eé]s[aá]l' OR       cod_asignatura ~* 'pr[oó]c[eé]s[aá]l' OR       text(c.id) ~* 'pr[oó]c[eé]s[aá]l' OR       lower(profesor) ~* 'pr[oó]c[eé]s[aá]l')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
