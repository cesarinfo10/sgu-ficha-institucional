COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[aá]ll[eé]r' OR       cod_asignatura ~* 't[aá]ll[eé]r' OR       text(c.id) ~* 't[aá]ll[eé]r' OR       lower(profesor) ~* 't[aá]ll[eé]r')  AND id_carrera IN (95,100,2,69,1,19,12)  
