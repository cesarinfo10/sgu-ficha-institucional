COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rg[aá]n[ií]c[oó]' OR       cod_asignatura ~* '[oó]rg[aá]n[ií]c[oó]' OR       text(c.id) ~* '[oó]rg[aá]n[ií]c[oó]' OR       lower(profesor) ~* '[oó]rg[aá]n[ií]c[oó]')  AND id_carrera IN (95,100,19,2,69,1,12)  
