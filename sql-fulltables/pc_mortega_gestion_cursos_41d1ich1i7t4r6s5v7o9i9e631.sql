COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]c[uú]rs[oó]s' OR       cod_asignatura ~* '[eé]c[uú]rs[oó]s' OR       text(c.id) ~* '[eé]c[uú]rs[oó]s' OR       lower(profesor) ~* '[eé]c[uú]rs[oó]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
