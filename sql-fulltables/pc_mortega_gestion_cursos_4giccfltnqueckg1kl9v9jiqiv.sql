COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[eé]l[ií][aá]' OR       cod_asignatura ~* 'c[eé]l[ií][aá]' OR       text(c.id) ~* 'c[eé]l[ií][aá]' OR       lower(profesor) ~* 'c[eé]l[ií][aá]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND id_carrera IN (95,100,19,2,69,1,12)  
