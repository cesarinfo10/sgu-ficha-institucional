COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]l[ií]s[aá]b[eé]th' OR       cod_asignatura ~* '[eé]l[ií]s[aá]b[eé]th' OR       text(c.id) ~* '[eé]l[ií]s[aá]b[eé]th' OR       lower(profesor) ~* '[eé]l[ií]s[aá]b[eé]th')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
