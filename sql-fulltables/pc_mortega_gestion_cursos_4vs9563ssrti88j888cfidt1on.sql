COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[oó]l[ií]t[ií]c[aá]' OR       cod_asignatura ~* 'p[oó]l[ií]t[ií]c[aá]' OR       text(c.id) ~* 'p[oó]l[ií]t[ií]c[aá]' OR       lower(profesor) ~* 'p[oó]l[ií]t[ií]c[aá]')  AND id_carrera IN (95,100,19,2,69,1,12)  
