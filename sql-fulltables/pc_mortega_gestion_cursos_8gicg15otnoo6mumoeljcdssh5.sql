COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]c[uú]rs[oó]s' OR       cod_asignatura ~* 'r[eé]c[uú]rs[oó]s' OR       text(c.id) ~* 'r[eé]c[uú]rs[oó]s' OR       lower(profesor) ~* 'r[eé]c[uú]rs[oó]s')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND id_carrera IN (95,100,19,2,69,1,12)  
