COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]mb[ií][eé]nt[aá]l' OR       cod_asignatura ~* '[aá]mb[ií][eé]nt[aá]l' OR       text(c.id) ~* '[aá]mb[ií][eé]nt[aá]l' OR       lower(profesor) ~* '[aá]mb[ií][eé]nt[aá]l')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
