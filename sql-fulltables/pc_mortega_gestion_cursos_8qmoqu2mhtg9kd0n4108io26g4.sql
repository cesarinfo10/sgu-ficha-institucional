COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]c[uú]rs[oó]' OR       cod_asignatura ~* 'r[eé]c[uú]rs[oó]' OR       text(c.id) ~* 'r[eé]c[uú]rs[oó]' OR       lower(profesor) ~* 'r[eé]c[uú]rs[oó]')  AND (lower(asignatura) ~* 'pr[oó]c[eé]s[aá]l[eé]s' OR       cod_asignatura ~* 'pr[oó]c[eé]s[aá]l[eé]s' OR       text(c.id) ~* 'pr[oó]c[eé]s[aá]l[eé]s' OR       lower(profesor) ~* 'pr[oó]c[eé]s[aá]l[eé]s')  AND id_carrera IN (95,100,19,2,69,1,12)  
