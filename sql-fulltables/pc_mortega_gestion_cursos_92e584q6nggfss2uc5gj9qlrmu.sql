COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]l[ií]t[oó]s' OR       cod_asignatura ~* 'd[eé]l[ií]t[oó]s' OR       text(c.id) ~* 'd[eé]l[ií]t[oó]s' OR       lower(profesor) ~* 'd[eé]l[ií]t[oó]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
