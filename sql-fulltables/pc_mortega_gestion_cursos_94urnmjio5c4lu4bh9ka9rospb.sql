COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]gl[aá]s' OR       cod_asignatura ~* 'r[eé]gl[aá]s' OR       text(c.id) ~* 'r[eé]gl[aá]s' OR       lower(profesor) ~* 'r[eé]gl[aá]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
