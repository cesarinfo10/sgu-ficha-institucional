COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[oó]s[eé]' OR       cod_asignatura ~* 'j[oó]s[eé]' OR       text(c.id) ~* 'j[oó]s[eé]' OR       lower(profesor) ~* 'j[oó]s[eé]')  AND (lower(asignatura) ~* '[ií]sm[aá][eé]l' OR       cod_asignatura ~* '[ií]sm[aá][eé]l' OR       text(c.id) ~* '[ií]sm[aá][eé]l' OR       lower(profesor) ~* '[ií]sm[aá][eé]l')  AND (lower(asignatura) ~* 'r[oó][aá]' OR       cod_asignatura ~* 'r[oó][aá]' OR       text(c.id) ~* 'r[oó][aá]' OR       lower(profesor) ~* 'r[oó][aá]')  AND (lower(asignatura) ~* '[oó]j[eé]d[aá]' OR       cod_asignatura ~* '[oó]j[eé]d[aá]' OR       text(c.id) ~* '[oó]j[eé]d[aá]' OR       lower(profesor) ~* '[oó]j[eé]d[aá]')  AND id_carrera IN (95,100,19,2,69,1,12)  
