COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[eé]n[eé]r[aá]l[eé]s' OR       cod_asignatura ~* 'g[eé]n[eé]r[aá]l[eé]s' OR       text(c.id) ~* 'g[eé]n[eé]r[aá]l[eé]s' OR       lower(profesor) ~* 'g[eé]n[eé]r[aá]l[eé]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
