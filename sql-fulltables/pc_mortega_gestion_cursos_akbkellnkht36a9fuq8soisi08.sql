COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ct[oó]s' OR       cod_asignatura ~* '[aá]ct[oó]s' OR       text(c.id) ~* '[aá]ct[oó]s' OR       lower(profesor) ~* '[aá]ct[oó]s')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'c[oó]m[eé]rc[ií][oó]' OR       cod_asignatura ~* 'c[oó]m[eé]rc[ií][oó]' OR       text(c.id) ~* 'c[oó]m[eé]rc[ií][oó]' OR       lower(profesor) ~* 'c[oó]m[eé]rc[ií][oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
