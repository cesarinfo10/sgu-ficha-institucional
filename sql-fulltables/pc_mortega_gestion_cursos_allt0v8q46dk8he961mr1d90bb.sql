COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]gl[aá]s' OR       cod_asignatura ~* 'r[eé]gl[aá]s' OR       text(c.id) ~* 'r[eé]gl[aá]s' OR       lower(profesor) ~* 'r[eé]gl[aá]s')  AND (lower(asignatura) ~* 'c[oó]m[uú]n[eé]s' OR       cod_asignatura ~* 'c[oó]m[uú]n[eé]s' OR       text(c.id) ~* 'c[oó]m[uú]n[eé]s' OR       lower(profesor) ~* 'c[oó]m[uú]n[eé]s')  AND id_carrera IN (95,100,19,2,69,1,12)  
