COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[ií]t[ií]g[aá]c[ií][oó]n' OR       cod_asignatura ~* 'l[ií]t[ií]g[aá]c[ií][oó]n' OR       text(c.id) ~* 'l[ií]t[ií]g[aá]c[ií][oó]n' OR       lower(profesor) ~* 'l[ií]t[ií]g[aá]c[ií][oó]n')  AND id_carrera IN (95,100,19,2,69,1,12)  
