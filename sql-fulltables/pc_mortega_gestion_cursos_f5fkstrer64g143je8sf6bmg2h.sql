COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[aá]t[aá]l[ií][aá]' OR       cod_asignatura ~* 'n[aá]t[aá]l[ií][aá]' OR       text(c.id) ~* 'n[aá]t[aá]l[ií][aá]' OR       lower(profesor) ~* 'n[aá]t[aá]l[ií][aá]')  AND (lower(asignatura) ~* 't[eé]r[eé]s[aá]' OR       cod_asignatura ~* 't[eé]r[eé]s[aá]' OR       text(c.id) ~* 't[eé]r[eé]s[aá]' OR       lower(profesor) ~* 't[eé]r[eé]s[aá]')  AND (lower(asignatura) ~* 'r[aá]gg[ií][oó]' OR       cod_asignatura ~* 'r[aá]gg[ií][oó]' OR       text(c.id) ~* 'r[aá]gg[ií][oó]' OR       lower(profesor) ~* 'r[aá]gg[ií][oó]')  AND (lower(asignatura) ~* 'r[eé]y[eé]s' OR       cod_asignatura ~* 'r[eé]y[eé]s' OR       text(c.id) ~* 'r[eé]y[eé]s' OR       lower(profesor) ~* 'r[eé]y[eé]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
