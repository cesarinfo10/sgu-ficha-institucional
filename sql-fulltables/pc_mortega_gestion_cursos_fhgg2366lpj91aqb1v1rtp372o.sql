COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ct[oó]' OR       cod_asignatura ~* '[aá]ct[oó]' OR       text(c.id) ~* '[aá]ct[oó]' OR       lower(profesor) ~* '[aá]ct[oó]')  AND (lower(asignatura) ~* 'j[uú]r[ií]d[ií]c[oó]' OR       cod_asignatura ~* 'j[uú]r[ií]d[ií]c[oó]' OR       text(c.id) ~* 'j[uú]r[ií]d[ií]c[oó]' OR       lower(profesor) ~* 'j[uú]r[ií]d[ií]c[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
