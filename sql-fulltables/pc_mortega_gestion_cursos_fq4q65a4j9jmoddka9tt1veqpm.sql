COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]ct[oó]r' OR       cod_asignatura ~* 'v[ií]ct[oó]r' OR       text(c.id) ~* 'v[ií]ct[oó]r' OR       lower(profesor) ~* 'v[ií]ct[oó]r')  AND (lower(asignatura) ~* 's[aá]m[uú][eé]l' OR       cod_asignatura ~* 's[aá]m[uú][eé]l' OR       text(c.id) ~* 's[aá]m[uú][eé]l' OR       lower(profesor) ~* 's[aá]m[uú][eé]l')  AND (lower(asignatura) ~* 'c[aá]br[eé]r[aá]' OR       cod_asignatura ~* 'c[aá]br[eé]r[aá]' OR       text(c.id) ~* 'c[aá]br[eé]r[aá]' OR       lower(profesor) ~* 'c[aá]br[eé]r[aá]')  AND (lower(asignatura) ~* 'v[ií]st[oó]s[oó]' OR       cod_asignatura ~* 'v[ií]st[oó]s[oó]' OR       text(c.id) ~* 'v[ií]st[oó]s[oó]' OR       lower(profesor) ~* 'v[ií]st[oó]s[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
