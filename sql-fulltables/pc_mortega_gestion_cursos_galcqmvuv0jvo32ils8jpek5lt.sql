COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]n[aá]' OR       cod_asignatura ~* 'p[eé]n[aá]' OR       text(c.id) ~* 'p[eé]n[aá]' OR       lower(profesor) ~* 'p[eé]n[aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
