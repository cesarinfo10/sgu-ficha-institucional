COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]bl[ií]g[aá]c[ií][oó]n[eé]s' OR       cod_asignatura ~* '[oó]bl[ií]g[aá]c[ií][oó]n[eé]s' OR       text(c.id) ~* '[oó]bl[ií]g[aá]c[ií][oó]n[eé]s' OR       lower(profesor) ~* '[oó]bl[ií]g[aá]c[ií][oó]n[eé]s')  AND id_carrera IN (95,100,19,2,69,1,12)  
