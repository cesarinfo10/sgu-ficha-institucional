COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[oó]c[eé]d[ií]m[ií][eé]nt[oó]s' OR       cod_asignatura ~* 'pr[oó]c[eé]d[ií]m[ií][eé]nt[oó]s' OR       text(c.id) ~* 'pr[oó]c[eé]d[ií]m[ií][eé]nt[oó]s' OR       lower(profesor) ~* 'pr[oó]c[eé]d[ií]m[ií][eé]nt[oó]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
