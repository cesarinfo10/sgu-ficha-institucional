COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]l[aá]rc[oó]n' OR       cod_asignatura ~* '[aá]l[aá]rc[oó]n' OR       text(c.id) ~* '[aá]l[aá]rc[oó]n' OR       lower(profesor) ~* '[aá]l[aá]rc[oó]n')  AND id_carrera IN (95,100,2,69,1,19,12)  
