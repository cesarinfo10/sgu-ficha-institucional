COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]ntr[oó]dcc[ií][oó]n' OR       cod_asignatura ~* '[ií]ntr[oó]dcc[ií][oó]n' OR       text(c.id) ~* '[ií]ntr[oó]dcc[ií][oó]n' OR       lower(profesor) ~* '[ií]ntr[oó]dcc[ií][oó]n')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
