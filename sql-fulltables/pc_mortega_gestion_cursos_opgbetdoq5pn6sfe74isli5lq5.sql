COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[oó]c[eé]s[aá]l' OR       cod_asignatura ~* 'pr[oó]c[eé]s[aá]l' OR       text(c.id) ~* 'pr[oó]c[eé]s[aá]l' OR       lower(profesor) ~* 'pr[oó]c[eé]s[aá]l')  AND id_carrera IN (95,100,19,2,69,1,12)  
