COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'b[ií][eé]n[eé]s' OR       cod_asignatura ~* 'b[ií][eé]n[eé]s' OR       text(c.id) ~* 'b[ií][eé]n[eé]s' OR       lower(profesor) ~* 'b[ií][eé]n[eé]s')  AND id_carrera IN (95,100,2,69,1,19,12)  
