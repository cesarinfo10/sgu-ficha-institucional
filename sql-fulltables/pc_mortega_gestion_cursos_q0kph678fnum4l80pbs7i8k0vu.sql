COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR       cod_asignatura ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR       text(c.id) ~* 's[eé]m[ií]n[aá]r[ií][oó]' OR       lower(profesor) ~* 's[eé]m[ií]n[aá]r[ií][oó]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND id_carrera IN (95,100,19,2,69,1,12)  
