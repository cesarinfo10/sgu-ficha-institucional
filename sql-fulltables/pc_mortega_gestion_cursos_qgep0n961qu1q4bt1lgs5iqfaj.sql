COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n' OR       cod_asignatura ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n' OR       text(c.id) ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n' OR       lower(profesor) ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
