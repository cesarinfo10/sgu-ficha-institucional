COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ld[oó]' OR       cod_asignatura ~* '[aá]ld[oó]' OR       text(c.id) ~* '[aá]ld[oó]' OR       lower(profesor) ~* '[aá]ld[oó]')  AND (lower(asignatura) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       cod_asignatura ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       text(c.id) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR       lower(profesor) ~* 'p[eé]rn[ií]g[oó]tt[ií]')  AND id_carrera IN (95,100,19,2,69,1,12)  
