COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rg[aá]n[ií]c[oó]' OR       cod_asignatura ~* '[oó]rg[aá]n[ií]c[oó]' OR       text(c.id) ~* '[oó]rg[aá]n[ií]c[oó]' OR       lower(profesor) ~* '[oó]rg[aá]n[ií]c[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
