COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[ií][eé]nc[ií][aá]' OR       cod_asignatura ~* 'c[ií][eé]nc[ií][aá]' OR       text(c.id) ~* 'c[ií][eé]nc[ií][aá]' OR       lower(profesor) ~* 'c[ií][eé]nc[ií][aá]')  AND (lower(asignatura) ~* 'p[oó]l[ií]t[ií]c[aá]' OR       cod_asignatura ~* 'p[oó]l[ií]t[ií]c[aá]' OR       text(c.id) ~* 'p[oó]l[ií]t[ií]c[aá]' OR       lower(profesor) ~* 'p[oó]l[ií]t[ií]c[aá]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND id_carrera IN (95,100,2,69,1,19,12)  
