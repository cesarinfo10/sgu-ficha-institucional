COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[aá]l[eé]nt[ií]n' OR       cod_asignatura ~* 'v[aá]l[eé]nt[ií]n' OR       text(c.id) ~* 'v[aá]l[eé]nt[ií]n' OR       lower(profesor) ~* 'v[aá]l[eé]nt[ií]n')  AND (lower(asignatura) ~* 'v[eé]r[aá]' OR       cod_asignatura ~* 'v[eé]r[aá]' OR       text(c.id) ~* 'v[eé]r[aá]' OR       lower(profesor) ~* 'v[eé]r[aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
