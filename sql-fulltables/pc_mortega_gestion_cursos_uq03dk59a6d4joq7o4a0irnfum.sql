COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]s[aá]rr[oó]ll' OR       cod_asignatura ~* 'd[eé]s[aá]rr[oó]ll' OR       text(c.id) ~* 'd[eé]s[aá]rr[oó]ll' OR       lower(profesor) ~* 'd[eé]s[aá]rr[oó]ll')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
