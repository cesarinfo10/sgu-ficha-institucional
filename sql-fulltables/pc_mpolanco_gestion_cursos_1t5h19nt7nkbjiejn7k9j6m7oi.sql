COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[aá]m[ií]l[ií][aá]' OR       cod_asignatura ~* 'f[aá]m[ií]l[ií][aá]' OR       text(c.id) ~* 'f[aá]m[ií]l[ií][aá]' OR       lower(profesor) ~* 'f[aá]m[ií]l[ií][aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
