COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND c.ano=2023  AND c.semestre=2  AND ('PRE' = ANY (regimenes))  AND 2 IN (c.dia1,c.dia2,c.dia3)  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
