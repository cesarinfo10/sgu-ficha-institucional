COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[ií]st[oó]r[ií][aá]' OR       cod_asignatura ~* 'h[ií]st[oó]r[ií][aá]' OR       text(c.id) ~* 'h[ií]st[oó]r[ií][aá]' OR       lower(profesor) ~* 'h[ií]st[oó]r[ií][aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
