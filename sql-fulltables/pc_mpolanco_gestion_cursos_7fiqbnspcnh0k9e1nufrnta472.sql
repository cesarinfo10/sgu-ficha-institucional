COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó]m[aá]n[oó]' OR       cod_asignatura ~* 'r[oó]m[aá]n[oó]' OR       text(c.id) ~* 'r[oó]m[aá]n[oó]' OR       lower(profesor) ~* 'r[oó]m[aá]n[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
