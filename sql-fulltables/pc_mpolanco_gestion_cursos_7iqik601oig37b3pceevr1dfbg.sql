COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]sc[oó]v[ií]ch' OR       cod_asignatura ~* 's[eé]sc[oó]v[ií]ch' OR       text(c.id) ~* 's[eé]sc[oó]v[ií]ch' OR       lower(profesor) ~* 's[eé]sc[oó]v[ií]ch')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
