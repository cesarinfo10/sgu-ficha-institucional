COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n' OR       cod_asignatura ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n' OR       text(c.id) ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n' OR       lower(profesor) ~* '[aá]rg[uú]m[eé]nt[aá]c[ií][oó]n')  AND id_carrera IN (95,100,2,69,1,19,12)  
