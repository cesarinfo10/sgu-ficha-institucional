COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[oó]t[oó]' OR       cod_asignatura ~* 's[oó]t[oó]' OR       text(c.id) ~* 's[oó]t[oó]' OR       lower(profesor) ~* 's[oó]t[oó]')  AND id_carrera IN (95,100,2,69,1,19,12)  
