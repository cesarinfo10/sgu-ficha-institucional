COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]c[aá]r[eé]n[aá]' OR       cod_asignatura ~* 'm[aá]c[aá]r[eé]n[aá]' OR       text(c.id) ~* 'm[aá]c[aá]r[eé]n[aá]' OR       lower(profesor) ~* 'm[aá]c[aá]r[eé]n[aá]')  AND id_carrera IN (95,100,2,69,1,19,12)  
