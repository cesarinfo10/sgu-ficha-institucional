COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[aá]n[ií][eé]l' OR       cod_asignatura ~* 'd[aá]n[ií][eé]l' OR       text(c.id) ~* 'd[aá]n[ií][eé]l' OR       lower(profesor) ~* 'd[aá]n[ií][eé]l')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
