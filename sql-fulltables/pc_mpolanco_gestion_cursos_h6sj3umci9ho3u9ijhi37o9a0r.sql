COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó]dr[ií]g[uú][eé]z' OR       cod_asignatura ~* 'r[oó]dr[ií]g[uú][eé]z' OR       text(c.id) ~* 'r[oó]dr[ií]g[uú][eé]z' OR       lower(profesor) ~* 'r[oó]dr[ií]g[uú][eé]z')  AND id_carrera IN (95,100,19,2,69,1,12)  
