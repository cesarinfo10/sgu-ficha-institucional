COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]dr[oó]' OR       cod_asignatura ~* 'p[eé]dr[oó]' OR       text(c.id) ~* 'p[eé]dr[oó]' OR       lower(profesor) ~* 'p[eé]dr[oó]')  AND (lower(asignatura) ~* 'j[oó]s[eé]' OR       cod_asignatura ~* 'j[oó]s[eé]' OR       text(c.id) ~* 'j[oó]s[eé]' OR       lower(profesor) ~* 'j[oó]s[eé]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND (lower(asignatura) ~* 'tr[oó]nc[oó]s[oó]' OR       cod_asignatura ~* 'tr[oó]nc[oó]s[oó]' OR       text(c.id) ~* 'tr[oó]nc[oó]s[oó]' OR       lower(profesor) ~* 'tr[oó]nc[oó]s[oó]')  AND (lower(asignatura) ~* 'b[uú]st[oó]s' OR       cod_asignatura ~* 'b[uú]st[oó]s' OR       text(c.id) ~* 'b[uú]st[oó]s' OR       lower(profesor) ~* 'b[uú]st[oó]s')  AND id_carrera IN (95,100,19,2,69,1,12)  
