COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[uú][aá]n' OR       cod_asignatura ~* 'j[uú][aá]n' OR       text(c.id) ~* 'j[uú][aá]n' OR       lower(profesor) ~* 'j[uú][aá]n')  AND (lower(asignatura) ~* 'cl[aá][uú]d[ií][oó]' OR       cod_asignatura ~* 'cl[aá][uú]d[ií][oó]' OR       text(c.id) ~* 'cl[aá][uú]d[ií][oó]' OR       lower(profesor) ~* 'cl[aá][uú]d[ií][oó]')  AND (lower(asignatura) ~* '[eé]sc[oó]b[aá]r' OR       cod_asignatura ~* '[eé]sc[oó]b[aá]r' OR       text(c.id) ~* '[eé]sc[oó]b[aá]r' OR       lower(profesor) ~* '[eé]sc[oó]b[aá]r')  AND (lower(asignatura) ~* 'l[eé]m[uú]s' OR       cod_asignatura ~* 'l[eé]m[uú]s' OR       text(c.id) ~* 'l[eé]m[uú]s' OR       lower(profesor) ~* 'l[eé]m[uú]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
