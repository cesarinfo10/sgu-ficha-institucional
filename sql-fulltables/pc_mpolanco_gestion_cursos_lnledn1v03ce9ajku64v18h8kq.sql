COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ct[oó]s' OR       cod_asignatura ~* '[aá]ct[oó]s' OR       text(c.id) ~* '[aá]ct[oó]s' OR       lower(profesor) ~* '[aá]ct[oó]s')  AND id_carrera IN (95,100,2,69,1,19,12)  
