COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[aá][ií]m[eé]' OR       cod_asignatura ~* 'j[aá][ií]m[eé]' OR       text(c.id) ~* 'j[aá][ií]m[eé]' OR       lower(profesor) ~* 'j[aá][ií]m[eé]')  AND id_carrera IN (95,100,19,2,69,1,12)  
