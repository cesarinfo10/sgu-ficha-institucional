COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[oó][aá]' OR       cod_asignatura ~* 'r[oó][aá]' OR       text(c.id) ~* 'r[oó][aá]' OR       lower(profesor) ~* 'r[oó][aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
