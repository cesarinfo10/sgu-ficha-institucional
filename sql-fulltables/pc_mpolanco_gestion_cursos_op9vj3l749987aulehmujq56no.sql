COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[eé]g[aá]' OR       cod_asignatura ~* 'v[eé]g[aá]' OR       text(c.id) ~* 'v[eé]g[aá]' OR       lower(profesor) ~* 'v[eé]g[aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
