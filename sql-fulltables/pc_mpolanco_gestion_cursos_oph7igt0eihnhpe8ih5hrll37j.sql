COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[ií]ld[aá]' OR       cod_asignatura ~* 'g[ií]ld[aá]' OR       text(c.id) ~* 'g[ií]ld[aá]' OR       lower(profesor) ~* 'g[ií]ld[aá]')  AND (lower(asignatura) ~* 'c[ií]cc[ií]' OR       cod_asignatura ~* 'c[ií]cc[ií]' OR       text(c.id) ~* 'c[ií]cc[ií]' OR       lower(profesor) ~* 'c[ií]cc[ií]')  AND (lower(asignatura) ~* 's[aá]l[aá]z[aá]r' OR       cod_asignatura ~* 's[aá]l[aá]z[aá]r' OR       text(c.id) ~* 's[aá]l[aá]z[aá]r' OR       lower(profesor) ~* 's[aá]l[aá]z[aá]r')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
