COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'l[oó]g[ií]c[aá]' OR       cod_asignatura ~* 'l[oó]g[ií]c[aá]' OR       text(c.id) ~* 'l[oó]g[ií]c[aá]' OR       lower(profesor) ~* 'l[oó]g[ií]c[aá]')  AND id_carrera IN (95,100,19,2,69,1,12)  
