COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[uú][ií]ll[eé]rm[oó]' OR       cod_asignatura ~* 'g[uú][ií]ll[eé]rm[oó]' OR       text(c.id) ~* 'g[uú][ií]ll[eé]rm[oó]' OR       lower(profesor) ~* 'g[uú][ií]ll[eé]rm[oó]')  AND (lower(asignatura) ~* 'h[uú]g[oó]' OR       cod_asignatura ~* 'h[uú]g[oó]' OR       text(c.id) ~* 'h[uú]g[oó]' OR       lower(profesor) ~* 'h[uú]g[oó]')  AND (lower(asignatura) ~* 'm[oó]r[aá]' OR       cod_asignatura ~* 'm[oó]r[aá]' OR       text(c.id) ~* 'm[oó]r[aá]' OR       lower(profesor) ~* 'm[oó]r[aá]')  AND (lower(asignatura) ~* '[oó]rt[ií]z' OR       cod_asignatura ~* '[oó]rt[ií]z' OR       text(c.id) ~* '[oó]rt[ií]z' OR       lower(profesor) ~* '[oó]rt[ií]z')  AND id_carrera IN (95,100,19,2,69,1,12)  
