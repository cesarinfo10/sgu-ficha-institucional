COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[eé]n[aá]l' OR       cod_asignatura ~* 'p[eé]n[aá]l' OR       text(c.id) ~* 'p[eé]n[aá]l' OR       lower(profesor) ~* 'p[eé]n[aá]l')  AND (lower(asignatura) ~* '[eé]c[oó]n[oó]m[ií]c[oó]' OR       cod_asignatura ~* '[eé]c[oó]n[oó]m[ií]c[oó]' OR       text(c.id) ~* '[eé]c[oó]n[oó]m[ií]c[oó]' OR       lower(profesor) ~* '[eé]c[oó]n[oó]m[ií]c[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
