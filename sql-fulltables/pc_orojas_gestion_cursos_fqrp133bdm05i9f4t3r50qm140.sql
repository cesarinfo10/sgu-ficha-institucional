COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '16565215' OR       cod_asignatura ~* '16565215' OR       text(c.id) ~* '16565215' OR       lower(profesor) ~* '16565215')  AND array[59,57,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,27,28,38,115,77,125,114,147,148,161,160,163,12] && (ids_carreras)  
