COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'j[oó]s[eé]' OR       cod_asignatura ~* 'j[oó]s[eé]' OR       text(c.id) ~* 'j[oó]s[eé]' OR       lower(profesor) ~* 'j[oó]s[eé]')  AND (lower(asignatura) ~* '[aá]ndr[eé]s' OR       cod_asignatura ~* '[aá]ndr[eé]s' OR       text(c.id) ~* '[aá]ndr[eé]s' OR       lower(profesor) ~* '[aá]ndr[eé]s')  AND (lower(asignatura) ~* 'f[aá]r[ií][aá]s' OR       cod_asignatura ~* 'f[aá]r[ií][aá]s' OR       text(c.id) ~* 'f[aá]r[ií][aá]s' OR       lower(profesor) ~* 'f[aá]r[ií][aá]s')  AND (lower(asignatura) ~* 'v[eé]rd[uú]g[oó]' OR       cod_asignatura ~* 'v[eé]rd[uú]g[oó]' OR       text(c.id) ~* 'v[eé]rd[uú]g[oó]' OR       lower(profesor) ~* 'v[eé]rd[uú]g[oó]')  AND array[59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,12] && (ids_carreras)  
