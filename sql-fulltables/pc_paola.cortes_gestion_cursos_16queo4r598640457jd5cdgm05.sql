COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[ií]st[oó]r[ií][aá]' OR       cod_asignatura ~* 'h[ií]st[oó]r[ií][aá]' OR       text(c.id) ~* 'h[ií]st[oó]r[ií][aá]' OR       lower(profesor) ~* 'h[ií]st[oó]r[ií][aá]')  AND id_carrera IN (94,97,104,110,98,105,102,91,15,103,126,4,12)  
