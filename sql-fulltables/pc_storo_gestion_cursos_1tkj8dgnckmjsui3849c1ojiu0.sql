COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]ct[oó]r' OR       cod_asignatura ~* 'v[ií]ct[oó]r' OR       text(c.id) ~* 'v[ií]ct[oó]r' OR       lower(profesor) ~* 'v[ií]ct[oó]r')  AND (lower(asignatura) ~* 's[oó]t[oó]' OR       cod_asignatura ~* 's[oó]t[oó]' OR       text(c.id) ~* 's[oó]t[oó]' OR       lower(profesor) ~* 's[oó]t[oó]')  AND id_carrera IN (95,100,19,2,69,1,12)  
