COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'r[eé]c[uú]rs[oó]s' OR       cod_asignatura ~* 'r[eé]c[uú]rs[oó]s' OR       text(c.id) ~* 'r[eé]c[uú]rs[oó]s' OR       lower(profesor) ~* 'r[eé]c[uú]rs[oó]s')  AND (lower(asignatura) ~* 'pr[oó]c[eé]s[aá]l[eé]s' OR       cod_asignatura ~* 'pr[oó]c[eé]s[aá]l[eé]s' OR       text(c.id) ~* 'pr[oó]c[eé]s[aá]l[eé]s' OR       lower(profesor) ~* 'pr[oó]c[eé]s[aá]l[eé]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
