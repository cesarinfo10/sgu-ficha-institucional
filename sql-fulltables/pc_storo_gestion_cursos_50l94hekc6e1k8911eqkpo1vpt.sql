COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[aá]ll[eé]r' OR       cod_asignatura ~* 't[aá]ll[eé]r' OR       text(c.id) ~* 't[aá]ll[eé]r' OR       lower(profesor) ~* 't[aá]ll[eé]r')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'p[oó]l[ií]c[ií][aá]' OR       cod_asignatura ~* 'p[oó]l[ií]c[ií][aá]' OR       text(c.id) ~* 'p[oó]l[ií]c[ií][aá]' OR       lower(profesor) ~* 'p[oó]l[ií]c[ií][aá]')  AND (lower(asignatura) ~* 'l[oó]c[aá]l' OR       cod_asignatura ~* 'l[oó]c[aá]l' OR       text(c.id) ~* 'l[oó]c[aá]l' OR       lower(profesor) ~* 'l[oó]c[aá]l')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
