COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]r[eé]ch[oó]' OR       cod_asignatura ~* 'd[eé]r[eé]ch[oó]' OR       text(c.id) ~* 'd[eé]r[eé]ch[oó]' OR       lower(profesor) ~* 'd[eé]r[eé]ch[oó]')  AND (lower(asignatura) ~* 's[uú]c[eé]s[oó]r[ií][oó]' OR       cod_asignatura ~* 's[uú]c[eé]s[oó]r[ií][oó]' OR       text(c.id) ~* 's[uú]c[eé]s[oó]r[ií][oó]' OR       lower(profesor) ~* 's[uú]c[eé]s[oó]r[ií][oó]')  AND id_carrera IN (95,100,2,69,1,19,12)  
