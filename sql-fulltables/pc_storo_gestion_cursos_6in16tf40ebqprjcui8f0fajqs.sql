COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[ií]nt.' OR       cod_asignatura ~* '[ií]nt.' OR       text(c.id) ~* '[ií]nt.' OR       lower(profesor) ~* '[ií]nt.')  AND (lower(asignatura) ~* '[aá]' OR       cod_asignatura ~* '[aá]' OR       text(c.id) ~* '[aá]' OR       lower(profesor) ~* '[aá]')  AND (lower(asignatura) ~* 'l[aá]' OR       cod_asignatura ~* 'l[aá]' OR       text(c.id) ~* 'l[aá]' OR       lower(profesor) ~* 'l[aá]')  AND (lower(asignatura) ~* 'f[ií]l[oó]s[oó]f[ií][aá]' OR       cod_asignatura ~* 'f[ií]l[oó]s[oó]f[ií][aá]' OR       text(c.id) ~* 'f[ií]l[oó]s[oó]f[ií][aá]' OR       lower(profesor) ~* 'f[ií]l[oó]s[oó]f[ií][aá]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
