COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]dm[uú]nd[oó]' OR       cod_asignatura ~* '[eé]dm[uú]nd[oó]' OR       text(c.id) ~* '[eé]dm[uú]nd[oó]' OR       lower(profesor) ~* '[eé]dm[uú]nd[oó]')  AND (lower(asignatura) ~* 'v[ií]lch[eé]s' OR       cod_asignatura ~* 'v[ií]lch[eé]s' OR       text(c.id) ~* 'v[ií]lch[eé]s' OR       lower(profesor) ~* 'v[ií]lch[eé]s')  AND id_carrera IN (95,100,19,2,69,1,12)  
