COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'h[aá]b[ií]l[ií]d[aá]d[eé]s' OR       cod_asignatura ~* 'h[aá]b[ií]l[ií]d[aá]d[eé]s' OR       text(c.id) ~* 'h[aá]b[ií]l[ií]d[aá]d[eé]s' OR       lower(profesor) ~* 'h[aá]b[ií]l[ií]d[aá]d[eé]s')  AND (lower(asignatura) ~* 'l[eé]ct[oó]' OR       cod_asignatura ~* 'l[eé]ct[oó]' OR       text(c.id) ~* 'l[eé]ct[oó]' OR       lower(profesor) ~* 'l[eé]ct[oó]')  AND (lower(asignatura) ~* '[eé]scr[ií]t[aá]s' OR       cod_asignatura ~* '[eé]scr[ií]t[aá]s' OR       text(c.id) ~* '[eé]scr[ií]t[aá]s' OR       lower(profesor) ~* '[eé]scr[ií]t[aá]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
