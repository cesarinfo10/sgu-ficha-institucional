COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]l[ií]t[oó]s' OR       cod_asignatura ~* 'd[eé]l[ií]t[oó]s' OR       text(c.id) ~* 'd[eé]l[ií]t[oó]s' OR       lower(profesor) ~* 'd[eé]l[ií]t[oó]s')  AND (lower(asignatura) ~* '[eé]sp[eé]c[ií][aá]l[eé]s' OR       cod_asignatura ~* '[eé]sp[eé]c[ií][aá]l[eé]s' OR       text(c.id) ~* '[eé]sp[eé]c[ií][aá]l[eé]s' OR       lower(profesor) ~* '[eé]sp[eé]c[ií][aá]l[eé]s')  AND (lower(asignatura) ~* '[ií]' OR       cod_asignatura ~* '[ií]' OR       text(c.id) ~* '[ií]' OR       lower(profesor) ~* '[ií]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
