COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[aá]n[ií][eé]l' OR       cod_asignatura ~* 'd[aá]n[ií][eé]l' OR       text(c.id) ~* 'd[aá]n[ií][eé]l' OR       lower(profesor) ~* 'd[aá]n[ií][eé]l')  AND (lower(asignatura) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       cod_asignatura ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       text(c.id) ~* 's[aá]nt[ií]b[aá]ñ[eé]z' OR       lower(profesor) ~* 's[aá]nt[ií]b[aá]ñ[eé]z')  AND id_carrera IN (95,100,2,69,1,19,12)  
