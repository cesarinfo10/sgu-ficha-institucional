COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[ií]c[oó]l[aá]s' OR       cod_asignatura ~* 'n[ií]c[oó]l[aá]s' OR       text(c.id) ~* 'n[ií]c[oó]l[aá]s' OR       lower(profesor) ~* 'n[ií]c[oó]l[aá]s')  AND (lower(asignatura) ~* '[oó]m[aá]r' OR       cod_asignatura ~* '[oó]m[aá]r' OR       text(c.id) ~* '[oó]m[aá]r' OR       lower(profesor) ~* '[oó]m[aá]r')  AND (lower(asignatura) ~* '[aá]st[oó]rg[aá]' OR       cod_asignatura ~* '[aá]st[oó]rg[aá]' OR       text(c.id) ~* '[aá]st[oó]rg[aá]' OR       lower(profesor) ~* '[aá]st[oó]rg[aá]')  AND (lower(asignatura) ~* 'd[eé]l' OR       cod_asignatura ~* 'd[eé]l' OR       text(c.id) ~* 'd[eé]l' OR       lower(profesor) ~* 'd[eé]l')  AND (lower(asignatura) ~* 'c[aá]nt[oó]' OR       cod_asignatura ~* 'c[aá]nt[oó]' OR       text(c.id) ~* 'c[aá]nt[oó]' OR       lower(profesor) ~* 'c[aá]nt[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
