COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'd[eé]r[eé]ch[oó]' OR       cod_asignatura ~* 'd[eé]r[eé]ch[oó]' OR       text(c.id) ~* 'd[eé]r[eé]ch[oó]' OR       lower(profesor) ~* 'd[eé]r[eé]ch[oó]')  AND (lower(asignatura) ~* '[aá]mb[ií][eé]nt[aá]l' OR       cod_asignatura ~* '[aá]mb[ií][eé]nt[aá]l' OR       text(c.id) ~* '[aá]mb[ií][eé]nt[aá]l' OR       lower(profesor) ~* '[aá]mb[ií][eé]nt[aá]l')  AND id_carrera IN (95,100,2,69,1,19,12)  
