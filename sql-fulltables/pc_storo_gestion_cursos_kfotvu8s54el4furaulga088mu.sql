COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]rt[aá]' OR       cod_asignatura ~* 'm[aá]rt[aá]' OR       text(c.id) ~* 'm[aá]rt[aá]' OR       lower(profesor) ~* 'm[aá]rt[aá]')  AND (lower(asignatura) ~* 'p[oó]l[aá]nc[oó]' OR       cod_asignatura ~* 'p[oó]l[aá]nc[oó]' OR       text(c.id) ~* 'p[oó]l[aá]nc[oó]' OR       lower(profesor) ~* 'p[oó]l[aá]nc[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
