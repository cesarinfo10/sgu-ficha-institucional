COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[aá]st[oó]n' OR       cod_asignatura ~* 'g[aá]st[oó]n' OR       text(c.id) ~* 'g[aá]st[oó]n' OR       lower(profesor) ~* 'g[aá]st[oó]n')  AND (lower(asignatura) ~* 'g[oó]nz[aá]l[eé]z' OR       cod_asignatura ~* 'g[oó]nz[aá]l[eé]z' OR       text(c.id) ~* 'g[oó]nz[aá]l[eé]z' OR       lower(profesor) ~* 'g[oó]nz[aá]l[eé]z')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
