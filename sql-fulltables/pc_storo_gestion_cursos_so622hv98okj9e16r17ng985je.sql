COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]l[ií]s[aá]b[eé]th' OR       cod_asignatura ~* '[eé]l[ií]s[aá]b[eé]th' OR       text(c.id) ~* '[eé]l[ií]s[aá]b[eé]th' OR       lower(profesor) ~* '[eé]l[ií]s[aá]b[eé]th')  AND (lower(asignatura) ~* 's[eé]sc[oó]v[ií]ch' OR       cod_asignatura ~* 's[eé]sc[oó]v[ií]ch' OR       text(c.id) ~* 's[eé]sc[oó]v[ií]ch' OR       lower(profesor) ~* 's[eé]sc[oó]v[ií]ch')  AND (lower(asignatura) ~* 'p[eé]q[uú][eé]ñ[oó]' OR       cod_asignatura ~* 'p[eé]q[uú][eé]ñ[oó]' OR       text(c.id) ~* 'p[eé]q[uú][eé]ñ[oó]' OR       lower(profesor) ~* 'p[eé]q[uú][eé]ñ[oó]')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
