COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[oó]ntr[aá]t[oó]s' OR       cod_asignatura ~* 'c[oó]ntr[aá]t[oó]s' OR       text(c.id) ~* 'c[oó]ntr[aá]t[oó]s' OR       lower(profesor) ~* 'c[oó]ntr[aá]t[oó]s')  AND array[95,100,2,69,1,19,12] && (ids_carreras)  
