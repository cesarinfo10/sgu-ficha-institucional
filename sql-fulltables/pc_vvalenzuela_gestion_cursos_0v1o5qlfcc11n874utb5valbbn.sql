COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[oó]n[ií]c[aá]' OR       cod_asignatura ~* 'm[oó]n[ií]c[aá]' OR       text(c.id) ~* 'm[oó]n[ií]c[aá]' OR       lower(profesor) ~* 'm[oó]n[ií]c[aá]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
