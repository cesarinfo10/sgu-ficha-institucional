COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[oó]n[ií][aá]' OR       cod_asignatura ~* 's[oó]n[ií][aá]' OR       text(c.id) ~* 's[oó]n[ií][aá]' OR       lower(profesor) ~* 's[oó]n[ií][aá]')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
