COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'pr[aá]ct[ií]c[aá]s' OR       cod_asignatura ~* 'pr[aá]ct[ií]c[aá]s' OR       text(c.id) ~* 'pr[aá]ct[ií]c[aá]s' OR       lower(profesor) ~* 'pr[aá]ct[ií]c[aá]s')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
