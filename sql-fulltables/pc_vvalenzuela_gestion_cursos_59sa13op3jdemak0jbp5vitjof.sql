COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[eé]r[oó]n[ií]c[aá]' OR       cod_asignatura ~* 'v[eé]r[oó]n[ií]c[aá]' OR       text(c.id) ~* 'v[eé]r[oó]n[ií]c[aá]' OR       lower(profesor) ~* 'v[eé]r[oó]n[ií]c[aá]')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
