COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[oó]rt[eé]g[aá]' OR       cod_asignatura ~* '[oó]rt[eé]g[aá]' OR       text(c.id) ~* '[oó]rt[eé]g[aá]' OR       lower(profesor) ~* '[oó]rt[eé]g[aá]')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
