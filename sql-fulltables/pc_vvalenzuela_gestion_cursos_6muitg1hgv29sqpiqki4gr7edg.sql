COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[eé][oó]vv[aá]n[ií][eé]' OR       cod_asignatura ~* 'g[eé][oó]vv[aá]n[ií][eé]' OR       text(c.id) ~* 'g[eé][oó]vv[aá]n[ií][eé]' OR       lower(profesor) ~* 'g[eé][oó]vv[aá]n[ií][eé]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
