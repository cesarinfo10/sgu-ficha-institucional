COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá][oó]l[aá]' OR       cod_asignatura ~* 'p[aá][oó]l[aá]' OR       text(c.id) ~* 'p[aá][oó]l[aá]' OR       lower(profesor) ~* 'p[aá][oó]l[aá]')  AND (lower(asignatura) ~* '[eé]l[ií][aá]n[aá]' OR       cod_asignatura ~* '[eé]l[ií][aá]n[aá]' OR       text(c.id) ~* '[eé]l[ií][aá]n[aá]' OR       lower(profesor) ~* '[eé]l[ií][aá]n[aá]')  AND (lower(asignatura) ~* 'r[oó]j[aá]s' OR       cod_asignatura ~* 'r[oó]j[aá]s' OR       text(c.id) ~* 'r[oó]j[aá]s' OR       lower(profesor) ~* 'r[oó]j[aá]s')  AND (lower(asignatura) ~* 'm[aá]r[ií]n' OR       cod_asignatura ~* 'm[aá]r[ií]n' OR       text(c.id) ~* 'm[aá]r[ií]n' OR       lower(profesor) ~* 'm[aá]r[ií]n')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
