COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[eé]r[oó]n[ií]c[aá]' OR       cod_asignatura ~* 'v[eé]r[oó]n[ií]c[aá]' OR       text(c.id) ~* 'v[eé]r[oó]n[ií]c[aá]' OR       lower(profesor) ~* 'v[eé]r[oó]n[ií]c[aá]')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
