COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[aá]' OR       cod_asignatura ~* 'c[aá]rl[aá]' OR       text(c.id) ~* 'c[aá]rl[aá]' OR       lower(profesor) ~* 'c[aá]rl[aá]')  AND (lower(asignatura) ~* 'v[ií]v[ií][aá]n[aá]' OR       cod_asignatura ~* 'v[ií]v[ií][aá]n[aá]' OR       text(c.id) ~* 'v[ií]v[ií][aá]n[aá]' OR       lower(profesor) ~* 'v[ií]v[ií][aá]n[aá]')  AND (lower(asignatura) ~* 'p[oó]l[aá]nc[oó]' OR       cod_asignatura ~* 'p[oó]l[aá]nc[oó]' OR       text(c.id) ~* 'p[oó]l[aá]nc[oó]' OR       lower(profesor) ~* 'p[oó]l[aá]nc[oó]')  AND (lower(asignatura) ~* 'm[eé]rc[aá]d[oó]' OR       cod_asignatura ~* 'm[eé]rc[aá]d[oó]' OR       text(c.id) ~* 'm[eé]rc[aá]d[oó]' OR       lower(profesor) ~* 'm[eé]rc[aá]d[oó]')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
