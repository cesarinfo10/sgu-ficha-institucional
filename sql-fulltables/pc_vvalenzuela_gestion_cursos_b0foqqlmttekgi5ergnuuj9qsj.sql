COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'f[aá]t[ií]m[aá]' OR       cod_asignatura ~* 'f[aá]t[ií]m[aá]' OR       text(c.id) ~* 'f[aá]t[ií]m[aá]' OR       lower(profesor) ~* 'f[aá]t[ií]m[aá]')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
