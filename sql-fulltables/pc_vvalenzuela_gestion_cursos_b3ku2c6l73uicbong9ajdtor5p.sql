COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 's[eé]rg[ií][oó]' OR       cod_asignatura ~* 's[eé]rg[ií][oó]' OR       text(c.id) ~* 's[eé]rg[ií][oó]' OR       lower(profesor) ~* 's[eé]rg[ií][oó]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
