COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[ií]g[uú][eé]l' OR       cod_asignatura ~* 'm[ií]g[uú][eé]l' OR       text(c.id) ~* 'm[ií]g[uú][eé]l' OR       lower(profesor) ~* 'm[ií]g[uú][eé]l')  AND (lower(asignatura) ~* '[aá]ng[eé]l' OR       cod_asignatura ~* '[aá]ng[eé]l' OR       text(c.id) ~* '[aá]ng[eé]l' OR       lower(profesor) ~* '[aá]ng[eé]l')  AND (lower(asignatura) ~* 's[ií]lv[aá]' OR       cod_asignatura ~* 's[ií]lv[aá]' OR       text(c.id) ~* 's[ií]lv[aá]' OR       lower(profesor) ~* 's[ií]lv[aá]')  AND (lower(asignatura) ~* 'r[aá]m[ií]r[eé]z' OR       cod_asignatura ~* 'r[aá]m[ií]r[eé]z' OR       text(c.id) ~* 'r[aá]m[ií]r[eé]z' OR       lower(profesor) ~* 'r[aá]m[ií]r[eé]z')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
