COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'v[ií]c[eé]nt[eé]' OR       cod_asignatura ~* 'v[ií]c[eé]nt[eé]' OR       text(c.id) ~* 'v[ií]c[eé]nt[eé]' OR       lower(profesor) ~* 'v[ií]c[eé]nt[eé]')  AND (lower(asignatura) ~* 'd[eé]' OR       cod_asignatura ~* 'd[eé]' OR       text(c.id) ~* 'd[eé]' OR       lower(profesor) ~* 'd[eé]')  AND (lower(asignatura) ~* 'p[aá][uú]l' OR       cod_asignatura ~* 'p[aá][uú]l' OR       text(c.id) ~* 'p[aá][uú]l' OR       lower(profesor) ~* 'p[aá][uú]l')  AND (lower(asignatura) ~* 'm[uú]ñ[oó]z' OR       cod_asignatura ~* 'm[uú]ñ[oó]z' OR       text(c.id) ~* 'm[uú]ñ[oó]z' OR       lower(profesor) ~* 'm[uú]ñ[oó]z')  AND (lower(asignatura) ~* 'gr[ií]ff[ií]th' OR       cod_asignatura ~* 'gr[ií]ff[ií]th' OR       text(c.id) ~* 'gr[ií]ff[ií]th' OR       lower(profesor) ~* 'gr[ií]ff[ií]th')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
