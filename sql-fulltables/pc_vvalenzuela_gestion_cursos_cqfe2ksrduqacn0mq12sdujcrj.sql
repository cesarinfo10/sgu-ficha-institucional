COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'cr[ií]st[ií][aá]n' OR       cod_asignatura ~* 'cr[ií]st[ií][aá]n' OR       text(c.id) ~* 'cr[ií]st[ií][aá]n' OR       lower(profesor) ~* 'cr[ií]st[ií][aá]n')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
