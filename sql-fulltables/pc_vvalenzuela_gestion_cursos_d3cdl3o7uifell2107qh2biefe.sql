COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       cod_asignatura ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       text(c.id) ~* '[aá]p[aá]r[ií]c[ií][oó]' OR       lower(profesor) ~* '[aá]p[aá]r[ií]c[ií][oó]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
