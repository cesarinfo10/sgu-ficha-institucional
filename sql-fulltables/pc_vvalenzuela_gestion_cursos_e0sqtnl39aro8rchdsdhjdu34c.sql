COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[uú][aá]j[aá]rd[oó]' OR       cod_asignatura ~* 'g[uú][aá]j[aá]rd[oó]' OR       text(c.id) ~* 'g[uú][aá]j[aá]rd[oó]' OR       lower(profesor) ~* 'g[uú][aá]j[aá]rd[oó]')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
