COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'p[aá][oó]l[aá]' OR       cod_asignatura ~* 'p[aá][oó]l[aá]' OR       text(c.id) ~* 'p[aá][oó]l[aá]' OR       lower(profesor) ~* 'p[aá][oó]l[aá]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
