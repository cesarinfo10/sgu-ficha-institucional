COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ndr[eé]s' OR       cod_asignatura ~* '[aá]ndr[eé]s' OR       text(c.id) ~* '[aá]ndr[eé]s' OR       lower(profesor) ~* '[aá]ndr[eé]s')  AND (lower(asignatura) ~* '[oó]sv[aá]ld[oó]' OR       cod_asignatura ~* '[oó]sv[aá]ld[oó]' OR       text(c.id) ~* '[oó]sv[aá]ld[oó]' OR       lower(profesor) ~* '[oó]sv[aá]ld[oó]')  AND (lower(asignatura) ~* 'll[aá]n[oó]s' OR       cod_asignatura ~* 'll[aá]n[oó]s' OR       text(c.id) ~* 'll[aá]n[oó]s' OR       lower(profesor) ~* 'll[aá]n[oó]s')  AND (lower(asignatura) ~* 's[ií]lv[aá]' OR       cod_asignatura ~* 's[ií]lv[aá]' OR       text(c.id) ~* 's[ií]lv[aá]' OR       lower(profesor) ~* 's[ií]lv[aá]')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
