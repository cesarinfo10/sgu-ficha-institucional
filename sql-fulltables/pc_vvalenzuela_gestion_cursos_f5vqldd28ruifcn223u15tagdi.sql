COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[uú][ií]d[oó]' OR       cod_asignatura ~* 'g[uú][ií]d[oó]' OR       text(c.id) ~* 'g[uú][ií]d[oó]' OR       lower(profesor) ~* 'g[uú][ií]d[oó]')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
