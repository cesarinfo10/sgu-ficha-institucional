COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]s[ií]n[aá]' OR       cod_asignatura ~* 't[eé]s[ií]n[aá]' OR       text(c.id) ~* 't[eé]s[ií]n[aá]' OR       lower(profesor) ~* 't[eé]s[ií]n[aá]')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
