COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[aá]b[eé]l' OR       cod_asignatura ~* 'm[aá]b[eé]l' OR       text(c.id) ~* 'm[aá]b[eé]l' OR       lower(profesor) ~* 'm[aá]b[eé]l')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
