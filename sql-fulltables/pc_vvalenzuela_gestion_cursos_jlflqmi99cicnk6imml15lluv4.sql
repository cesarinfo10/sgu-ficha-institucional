COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'c[aá]rl[aá]' OR       cod_asignatura ~* 'c[aá]rl[aá]' OR       text(c.id) ~* 'c[aá]rl[aá]' OR       lower(profesor) ~* 'c[aá]rl[aá]')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
