COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '17939' OR       cod_asignatura ~* '17939' OR       text(c.id) ~* '17939' OR       lower(profesor) ~* '17939')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
