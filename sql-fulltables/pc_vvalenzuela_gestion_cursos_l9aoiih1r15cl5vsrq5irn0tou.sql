COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]pl[ií]c[aá]d[oó]' OR       cod_asignatura ~* '[aá]pl[ií]c[aá]d[oó]' OR       text(c.id) ~* '[aá]pl[ií]c[aá]d[oó]' OR       lower(profesor) ~* '[aá]pl[ií]c[aá]d[oó]')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
