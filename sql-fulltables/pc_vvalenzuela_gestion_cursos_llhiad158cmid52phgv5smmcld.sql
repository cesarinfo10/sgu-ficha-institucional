COPY (SELECT c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'g[uú][ií]d[oó]' OR       cod_asignatura ~* 'g[uú][ií]d[oó]' OR       text(c.id) ~* 'g[uú][ií]d[oó]' OR       lower(profesor) ~* 'g[uú][ií]d[oó]')  AND id_carrera IN (108,96,25,109,37,70,135,112,17,12)  
