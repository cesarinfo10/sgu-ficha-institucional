COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]s[ií]s' OR       cod_asignatura ~* 't[eé]s[ií]s' OR       text(c.id) ~* 't[eé]s[ií]s' OR       lower(profesor) ~* 't[eé]s[ií]s')  AND (lower(asignatura) ~* '[ií][ií]' OR       cod_asignatura ~* '[ií][ií]' OR       text(c.id) ~* '[ií][ií]' OR       lower(profesor) ~* '[ií][ií]')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
