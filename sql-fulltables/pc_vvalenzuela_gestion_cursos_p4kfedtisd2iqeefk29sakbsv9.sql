COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'n[aá]ncy' OR       cod_asignatura ~* 'n[aá]ncy' OR       text(c.id) ~* 'n[aá]ncy' OR       lower(profesor) ~* 'n[aá]ncy')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
