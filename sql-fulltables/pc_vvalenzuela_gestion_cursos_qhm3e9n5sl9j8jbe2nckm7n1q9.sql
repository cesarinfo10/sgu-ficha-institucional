COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[aá]ndr[eé]s' OR       cod_asignatura ~* '[aá]ndr[eé]s' OR       text(c.id) ~* '[aá]ndr[eé]s' OR       lower(profesor) ~* '[aá]ndr[eé]s')  AND (lower(asignatura) ~* 'll[aá]n[oó]s' OR       cod_asignatura ~* 'll[aá]n[oó]s' OR       text(c.id) ~* 'll[aá]n[oó]s' OR       lower(profesor) ~* 'll[aá]n[oó]s')  AND id_carrera IN (108,96,25,109,37,135,70,112,17,12)  
