COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 'm[oó]n[ií]c[aá]' OR       cod_asignatura ~* 'm[oó]n[ií]c[aá]' OR       text(c.id) ~* 'm[oó]n[ií]c[aá]' OR       lower(profesor) ~* 'm[oó]n[ií]c[aá]')  AND (lower(asignatura) ~* '[ií]v[oó]nn[eé]' OR       cod_asignatura ~* '[ií]v[oó]nn[eé]' OR       text(c.id) ~* '[ií]v[oó]nn[eé]' OR       lower(profesor) ~* '[ií]v[oó]nn[eé]')  AND (lower(asignatura) ~* 'g[aá]l[aá]rc[eé]' OR       cod_asignatura ~* 'g[aá]l[aá]rc[eé]' OR       text(c.id) ~* 'g[aá]l[aá]rc[eé]' OR       lower(profesor) ~* 'g[aá]l[aá]rc[eé]')  AND (lower(asignatura) ~* 'c[oó]ntr[eé]r[aá]s' OR       cod_asignatura ~* 'c[oó]ntr[eé]r[aá]s' OR       text(c.id) ~* 'c[oó]ntr[eé]r[aá]s' OR       lower(profesor) ~* 'c[oó]ntr[eé]r[aá]s')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
