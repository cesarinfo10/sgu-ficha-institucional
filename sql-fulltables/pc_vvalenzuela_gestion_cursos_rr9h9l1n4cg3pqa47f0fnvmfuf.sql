COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]d[ií]s[oó]n' OR       cod_asignatura ~* '[eé]d[ií]s[oó]n' OR       text(c.id) ~* '[eé]d[ií]s[oó]n' OR       lower(profesor) ~* '[eé]d[ií]s[oó]n')  AND (lower(asignatura) ~* 'r[aá]f[aá][eé]l' OR       cod_asignatura ~* 'r[aá]f[aá][eé]l' OR       text(c.id) ~* 'r[aá]f[aá][eé]l' OR       lower(profesor) ~* 'r[aá]f[aá][eé]l')  AND (lower(asignatura) ~* 'r[eé]v[ií]ll[aá]' OR       cod_asignatura ~* 'r[eé]v[ií]ll[aá]' OR       text(c.id) ~* 'r[eé]v[ií]ll[aá]' OR       lower(profesor) ~* 'r[eé]v[ií]ll[aá]')  AND (lower(asignatura) ~* 'h[eé]rm[aá]n' OR       cod_asignatura ~* 'h[eé]rm[aá]n' OR       text(c.id) ~* 'h[eé]rm[aá]n' OR       lower(profesor) ~* 'h[eé]rm[aá]n')  AND array[108,96,25,109,37,70,135,112,17,153,12] && (ids_carreras)  
