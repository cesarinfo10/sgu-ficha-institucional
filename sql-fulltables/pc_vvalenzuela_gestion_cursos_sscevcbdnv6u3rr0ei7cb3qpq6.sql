COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND 70 = ANY (ids_carreras)  AND c.ano=2024  AND c.semestre=1  AND ('PRE-D' = ANY (regimenes))  AND array[108,96,25,109,37,70,135,153,112,17,12] && (ids_carreras)  
