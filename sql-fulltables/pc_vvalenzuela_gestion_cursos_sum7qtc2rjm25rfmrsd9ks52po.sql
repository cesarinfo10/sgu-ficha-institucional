COPY (SELECT c.id,cod_asignatura||'-'||c.seccion||' '||asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* 't[eé]s[ií]n[aá]' OR       cod_asignatura ~* 't[eé]s[ií]n[aá]' OR       text(c.id) ~* 't[eé]s[ií]n[aá]' OR       lower(profesor) ~* 't[eé]s[ií]n[aá]')  AND (lower(asignatura) ~* '' OR       cod_asignatura ~* '' OR       text(c.id) ~* '' OR       lower(profesor) ~* '')  AND id_carrera IN (108,96,25,109,37,70,112,17,12)  
