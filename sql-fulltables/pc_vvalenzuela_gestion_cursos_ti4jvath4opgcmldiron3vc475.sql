COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '[eé]t[ií]c[aá]' OR       cod_asignatura ~* '[eé]t[ií]c[aá]' OR       text(c.id) ~* '[eé]t[ií]c[aá]' OR       lower(profesor) ~* '[eé]t[ií]c[aá]')  AND (lower(asignatura) ~* 'y' OR       cod_asignatura ~* 'y' OR       text(c.id) ~* 'y' OR       lower(profesor) ~* 'y')  AND (lower(asignatura) ~* 'tr[aá]b[aá]j[oó]' OR       cod_asignatura ~* 'tr[aá]b[aá]j[oó]' OR       text(c.id) ~* 'tr[aá]b[aá]j[oó]' OR       lower(profesor) ~* 'tr[aá]b[aá]j[oó]')  AND (lower(asignatura) ~* 's[oó]c[ií][aá]l' OR       cod_asignatura ~* 's[oó]c[ií][aá]l' OR       text(c.id) ~* 's[oó]c[ií][aá]l' OR       lower(profesor) ~* 's[oó]c[ií][aá]l')  AND array[108,96,25,109,37,70,135,112,17,12] && (ids_carreras)  
