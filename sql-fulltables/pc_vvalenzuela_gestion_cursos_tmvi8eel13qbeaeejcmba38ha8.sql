COPY (SELECT vc.carreras,c.id,vc.cod_asignatura||'-'||c.seccion||' '||vc.asignatura AS asignatura,profesor,rut,
                    ...
                    WHERE id_fusion IS NULL  AND (lower(asignatura) ~* '18852' OR       cod_asignatura ~* '18852' OR       text(c.id) ~* '18852' OR       lower(profesor) ~* '18852')  AND array[108,96,25,109,37,70,135,153,112,17,12] && (ids_carreras)  
