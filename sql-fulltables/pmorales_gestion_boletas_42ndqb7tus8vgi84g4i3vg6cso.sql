COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '20446269' OR  pap.rut ~* '20446269' OR lower(a.nombres||' '||a.apellidos) ~* '20446269' OR lower(a3.nombres||' '||a3.apellidos) ~* '20446269' OR  a.rut ~* '20446269' OR a2.rut ~* '20446269' OR a3.rut ~* '20446269'  OR text(nro_boleta) = '20446269' )  
