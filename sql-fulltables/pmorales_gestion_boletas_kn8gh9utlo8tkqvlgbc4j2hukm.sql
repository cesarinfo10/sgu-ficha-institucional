COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '32693' OR  pap.rut ~* '32693' OR lower(a.nombres||' '||a.apellidos) ~* '32693' OR lower(a3.nombres||' '||a3.apellidos) ~* '32693' OR  a.rut ~* '32693' OR a2.rut ~* '32693' OR a3.rut ~* '32693'  OR text(nro_boleta) = '32693' )  
