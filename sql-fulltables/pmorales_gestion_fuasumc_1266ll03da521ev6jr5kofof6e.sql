COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]nr[ií]q[uú][eé]' OR  a.rut ~* '[eé]nr[ií]q[uú][eé]' OR  text(a.id) ~* '[eé]nr[ií]q[uú][eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó]nz[aá]l[oó]' OR  a.rut ~* 'g[oó]nz[aá]l[oó]' OR  text(a.id) ~* 'g[oó]nz[aá]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]p[eé]z' OR  a.rut ~* 'l[oó]p[eé]z' OR  text(a.id) ~* 'l[oó]p[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]t[aá]m[aá]l[aá]' OR  a.rut ~* 'm[aá]t[aá]m[aá]l[aá]' OR  text(a.id) ~* 'm[aá]t[aá]m[aá]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR  text(a.id) ~* '' ) 
