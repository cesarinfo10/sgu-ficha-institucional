COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '20787753' OR  pap.rut ~* '20787753' OR lower(a.nombres||' '||a.apellidos) ~* '20787753' OR lower(a3.nombres||' '||a3.apellidos) ~* '20787753' OR  a.rut ~* '20787753' OR a2.rut ~* '20787753' OR a3.rut ~* '20787753' )  
