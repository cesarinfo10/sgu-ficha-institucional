COPY (SELECT s.id,a.rut,a.nombres,a.apellidos,va.carrera||'-'||a.jornada AS carrera,
			  ...
			  WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]st[oó]n' OR  a.rut ~* 'g[aá]st[oó]n' OR  text(a.id) ~* 'g[aá]st[oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lfr[eé]d[oó]' OR  a.rut ~* '[aá]lfr[eé]d[oó]' OR  text(a.id) ~* '[aá]lfr[eé]d[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rt[eé]s' OR  a.rut ~* 'c[oó]rt[eé]s' OR  text(a.id) ~* 'c[oó]rt[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rt[eé]g[aá]' OR  a.rut ~* '[oó]rt[eé]g[aá]' OR  text(a.id) ~* '[oó]rt[eé]g[aá]' ) 
