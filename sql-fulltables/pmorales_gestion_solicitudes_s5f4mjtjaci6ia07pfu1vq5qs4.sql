COPY (SELECT s.id,a.rut,a.nombres,a.apellidos,va.carrera||'-'||a.jornada AS carrera,
			  ...
			  WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[oó]l[aá]' OR  a.rut ~* 'c[aá]r[oó]l[aá]' OR  text(a.id) ~* 'c[aá]r[oó]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]rr[eé]s' OR  a.rut ~* 't[oó]rr[eé]s' OR  text(a.id) ~* 't[oó]rr[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]m[ií]g[oó]' OR  a.rut ~* '[aá]m[ií]g[oó]' OR  text(a.id) ~* '[aá]m[ií]g[oó]' ) 
