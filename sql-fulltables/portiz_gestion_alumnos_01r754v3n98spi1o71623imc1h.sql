COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]l[eé]n[aá]' OR  a.rut ~* '[eé]l[eé]n[aá]' OR  lower(a.email) ~* '[eé]l[eé]n[aá]' OR  text(a.id) ~* '[eé]l[eé]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[oó]t[eé]' OR  a.rut ~* 'm[oó]r[oó]t[eé]' OR  lower(a.email) ~* 'm[oó]r[oó]t[eé]' OR  text(a.id) ~* 'm[oó]r[oó]t[eé]' ) 
