COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]c[aá]r[eé]n[aá]' OR  a.rut ~* 'm[aá]c[aá]r[eé]n[aá]' OR  lower(a.email) ~* 'm[aá]c[aá]r[eé]n[aá]' OR  text(a.id) ~* 'm[aá]c[aá]r[eé]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]t[aá]l[aá]n' OR  a.rut ~* 'c[aá]t[aá]l[aá]n' OR  lower(a.email) ~* 'c[aá]t[aá]l[aá]n' OR  text(a.id) ~* 'c[aá]t[aá]l[aá]n' ) 
