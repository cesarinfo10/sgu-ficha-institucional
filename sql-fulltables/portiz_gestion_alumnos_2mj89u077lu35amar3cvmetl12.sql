COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]n[aá]t[aá]' OR  a.rut ~* 'r[eé]n[aá]t[aá]' OR  lower(a.email) ~* 'r[eé]n[aá]t[aá]' OR  text(a.id) ~* 'r[eé]n[aá]t[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]mp[oó]s' OR  a.rut ~* 'c[aá]mp[oó]s' OR  lower(a.email) ~* 'c[aá]mp[oó]s' OR  text(a.id) ~* 'c[aá]mp[oó]s' ) 
