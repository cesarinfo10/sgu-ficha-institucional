COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'br[uú]n[oó]' OR  a.rut ~* 'br[uú]n[oó]' OR  lower(a.email) ~* 'br[uú]n[oó]' OR  text(a.id) ~* 'br[uú]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]sl[aá]' OR  a.rut ~* '[ií]sl[aá]' OR  lower(a.email) ~* '[ií]sl[aá]' OR  text(a.id) ~* '[ií]sl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]v[eé]r[aá]' OR  a.rut ~* 'r[ií]v[eé]r[aá]' OR  lower(a.email) ~* 'r[ií]v[eé]r[aá]' OR  text(a.id) ~* 'r[ií]v[eé]r[aá]' ) 
