COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]shl[eé]y' OR  a.rut ~* '[aá]shl[eé]y' OR  lower(a.email) ~* '[aá]shl[eé]y' OR  text(a.id) ~* '[aá]shl[eé]y' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nst[aá]nz[aá]' OR  a.rut ~* 'c[oó]nst[aá]nz[aá]' OR  lower(a.email) ~* 'c[oó]nst[aá]nz[aá]' OR  text(a.id) ~* 'c[oó]nst[aá]nz[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[uú][ií]z' OR  a.rut ~* 'r[uú][ií]z' OR  lower(a.email) ~* 'r[uú][ií]z' OR  text(a.id) ~* 'r[uú][ií]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rd[eé]n[aá]s' OR  a.rut ~* 'c[aá]rd[eé]n[aá]s' OR  lower(a.email) ~* 'c[aá]rd[eé]n[aá]s' OR  text(a.id) ~* 'c[aá]rd[eé]n[aá]s' ) 
