COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]rg[eé]' OR  a.rut ~* 'j[oó]rg[eé]' OR  lower(a.email) ~* 'j[oó]rg[eé]' OR  text(a.id) ~* 'j[oó]rg[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]v[eé]n[aá]' OR  a.rut ~* '[aá]r[aá]v[eé]n[aá]' OR  lower(a.email) ~* '[aá]r[aá]v[eé]n[aá]' OR  text(a.id) ~* '[aá]r[aá]v[eé]n[aá]' ) 
