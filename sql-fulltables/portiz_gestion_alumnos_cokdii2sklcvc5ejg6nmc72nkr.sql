COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]lyl[aá]' OR  a.rut ~* 'd[aá]lyl[aá]' OR  lower(a.email) ~* 'd[aá]lyl[aá]' OR  text(a.id) ~* 'd[aá]lyl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]p[aá]z[oó]' OR  a.rut ~* '[oó]p[aá]z[oó]' OR  lower(a.email) ~* '[oó]p[aá]z[oó]' OR  text(a.id) ~* '[oó]p[aá]z[oó]' ) 
