COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN dae.fuas_mineduc AS fuasm ON (fuasm.ano=2022 AND fuasm.rut=split_part(trim(a.rut),'-',1)::int8)
                ...
                WHERE true  AND (c.regimen = 'PRE')  AND fuasm.rut IS NOT NULL AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023)) 
