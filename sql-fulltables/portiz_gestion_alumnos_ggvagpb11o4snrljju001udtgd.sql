COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN dae.preseleccionados_beneficios_mineduc AS presel_mineduc ON (presel_mineduc.ano=2024 AND presel_mineduc.rut=split_part(trim(a.rut),'-',1)::int8)
                ...
                WHERE true  AND (c.regimen = 'PRE')  AND presel_mineduc.rut IS NOT NULL AND presel_mineduc.nbro_asignacion_becas IS NOT NULL AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1)) 
