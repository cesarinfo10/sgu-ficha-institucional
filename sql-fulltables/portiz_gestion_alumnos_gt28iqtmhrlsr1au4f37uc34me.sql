COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]m[ií]l[eé]t' OR  a.rut ~* 'y[aá]m[ií]l[eé]t' OR  lower(a.email) ~* 'y[aá]m[ií]l[eé]t' OR  text(a.id) ~* 'y[aá]m[ií]l[eé]t' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]ll[oó][aá]' OR  a.rut ~* '[uú]ll[oó][aá]' OR  lower(a.email) ~* '[uú]ll[oó][aá]' OR  text(a.id) ~* '[uú]ll[oó][aá]' ) 
