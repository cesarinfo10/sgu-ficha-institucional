COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
				...
				LEFT JOIN (SELECT rut,count(rut) AS dias FROM (SELECT rut FROM vista_torniquete_rut WHERE fecha_reg::date BETWEEN '2023-08-07'::date AND '2023-12-17'::date GROUP BY rut,fecha_reg::date) AS foo GROUP BY rut) AS vtr ON vtr.rut=split_part(a.rut,'-',1)
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2023' ) AND (a.estado = '1') AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2)) 
