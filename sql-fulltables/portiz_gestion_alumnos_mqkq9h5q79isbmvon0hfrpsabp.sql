COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú][aá]n' OR  a.rut ~* 'j[uú][aá]n' OR  lower(a.email) ~* 'j[uú][aá]n' OR  text(a.id) ~* 'j[uú][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]fr[eé]' OR  a.rut ~* 'j[oó]fr[eé]' OR  lower(a.email) ~* 'j[oó]fr[eé]' OR  text(a.id) ~* 'j[oó]fr[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[eé]r[eé]d[ií][aá]' OR  a.rut ~* 'h[eé]r[eé]d[ií][aá]' OR  lower(a.email) ~* 'h[eé]r[eé]d[ií][aá]' OR  text(a.id) ~* 'h[eé]r[eé]d[ií][aá]' ) 
