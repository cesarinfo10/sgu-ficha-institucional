COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '16616' OR  a.rut ~* '16616' OR  lower(a.email) ~* '16616' OR  text(a.id) ~* '16616' ) AND (lower(a.nombres||' '||a.apellidos) ~* '094-4' OR  a.rut ~* '094-4' OR  lower(a.email) ~* '094-4' OR  text(a.id) ~* '094-4' ) 
