COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[oó]ns[oó]' OR  a.rut ~* '[aá]l[oó]ns[oó]' OR  text(ac.folio) ~* '[aá]l[oó]ns[oó]' OR  vac.cod ~* '[aá]l[oó]ns[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rr[eé][aá]' OR  a.rut ~* 'c[oó]rr[eé][aá]' OR  text(ac.folio) ~* 'c[oó]rr[eé][aá]' OR  vac.cod ~* 'c[oó]rr[eé][aá]' ) 
