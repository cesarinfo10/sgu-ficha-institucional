COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rf[eé]l[ií]n[aá]' OR  a.rut ~* '[oó]rf[eé]l[ií]n[aá]' OR  text(a.id) ~* '[oó]rf[eé]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[aá]r[eé]n[aá]s' OR  a.rut ~* 'l[aá]r[eé]n[aá]s' OR  text(a.id) ~* 'l[aá]r[eé]n[aá]s' ) 
