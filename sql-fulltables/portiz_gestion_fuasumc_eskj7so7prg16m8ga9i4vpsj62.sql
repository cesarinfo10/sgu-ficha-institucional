COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]rth[aá]' OR  a.rut ~* '[eé]rth[aá]' OR  text(a.id) ~* '[eé]rth[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'bl[aá]nch[eé]t' OR  a.rut ~* 'bl[aá]nch[eé]t' OR  text(a.id) ~* 'bl[aá]nch[eé]t' ) 
