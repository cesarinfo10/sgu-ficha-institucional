COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[ií][eé]l' OR  a.rut ~* '[aá]r[ií][eé]l' OR  text(a.id) ~* '[aá]r[ií][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[uú]n[ií]z[aá]g[aá]' OR  a.rut ~* 'm[uú]n[ií]z[aá]g[aá]' OR  text(a.id) ~* 'm[uú]n[ií]z[aá]g[aá]' ) 
