COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[eé]n' OR  a.rut ~* 'k[aá]r[eé]n' OR  text(a.id) ~* 'k[aá]r[eé]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[ií][aá]z' OR  a.rut ~* 'd[ií][aá]z' OR  text(a.id) ~* 'd[ií][aá]z' ) 
