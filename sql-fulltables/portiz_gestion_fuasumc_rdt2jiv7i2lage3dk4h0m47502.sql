COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]l[aá]y[aá]' OR  a.rut ~* '[oó]l[aá]y[aá]' OR  text(a.id) ~* '[oó]l[aá]y[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]sm[eé]n[ií][aá]' OR  a.rut ~* '[ií]sm[eé]n[ií][aá]' OR  text(a.id) ~* '[ií]sm[eé]n[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]br[eé]r[aá]' OR  a.rut ~* 'c[aá]br[eé]r[aá]' OR  text(a.id) ~* 'c[aá]br[eé]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[aá]br[aá]ñ[aá]' OR  a.rut ~* 'l[aá]br[aá]ñ[aá]' OR  text(a.id) ~* 'l[aá]br[aá]ñ[aá]' ) 
