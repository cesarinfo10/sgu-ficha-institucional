COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true  AND ((a.cohorte = 2021 AND a.semestre_cohorte = 2 AND a.mes_cohorte = 8)      OR (a.cohorte_reinc = 2021 AND a.semestre_cohorte_reinc = 2 AND a.mes_cohorte_reinc = 8)) AND (a.cohorte = '2021' OR a.cohorte_reinc = '2021') AND (a.estado = '1') AND (a.carrera_actual = 28) AND (c.regimen = 'POST-GD') 
