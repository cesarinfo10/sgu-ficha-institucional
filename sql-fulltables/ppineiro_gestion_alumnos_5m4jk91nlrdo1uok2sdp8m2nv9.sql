COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'bl[aá]nc[oó]' OR  a.rut ~* 'bl[aá]nc[oó]' OR  lower(a.email) ~* 'bl[aá]nc[oó]' OR  text(a.id) ~* 'bl[aá]nc[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]c' OR  a.rut ~* 'v[ií]c' OR  lower(a.email) ~* 'v[ií]c' OR  text(a.id) ~* 'v[ií]c' ) 
