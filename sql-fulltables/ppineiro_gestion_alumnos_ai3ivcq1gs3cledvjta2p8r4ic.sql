COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2024 AND regimen='POST-TD' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2024   
                FROM alumnos AS a
                ...
                WHERE true  AND ((a.cohorte = 2023 AND a.semestre_cohorte = 2 AND a.mes_cohorte = 11)      OR (a.cohorte_reinc = 2023 AND a.semestre_cohorte_reinc = 2 AND a.mes_cohorte_reinc = 11)) AND (a.cohorte = '2023' OR a.cohorte_reinc = '2023') AND (a.carrera_actual = 54) AND (c.regimen = 'POST-TD') 
