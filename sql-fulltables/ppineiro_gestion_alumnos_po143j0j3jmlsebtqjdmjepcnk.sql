COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true  AND ((a.cohorte = 2022 AND a.semestre_cohorte = 1 AND a.mes_cohorte = 6)      OR (a.cohorte_reinc = 2022 AND a.semestre_cohorte_reinc = 1 AND a.mes_cohorte_reinc = 6)) AND (a.cohorte = '2022' OR a.cohorte_reinc = '2022') AND (a.estado = '1') AND (c.regimen = 'POST-GD') 
