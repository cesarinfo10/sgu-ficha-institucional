COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[oó]nz[aá]l[eé]z' OR  pap.rut ~* 'g[oó]nz[aá]l[eé]z' OR  text(pap.id) ~* 'g[oó]nz[aá]l[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'h[eé]rn[aá]nd[eé]z' OR  pap.rut ~* 'h[eé]rn[aá]nd[eé]z' OR  text(pap.id) ~* 'h[eé]rn[aá]nd[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]l[ií][aá]n[aá]' OR  pap.rut ~* '[eé]l[ií][aá]n[aá]' OR  text(pap.id) ~* '[eé]l[ií][aá]n[aá]' ) 
