COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'ch[aá]m[oó]rr[oó]' OR  pap.rut ~* 'ch[aá]m[oó]rr[oó]' OR  text(pap.id) ~* 'ch[aá]m[oó]rr[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR  pap.rut ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR  text(pap.id) ~* 'f[uú][eé]nt[eé][aá]lb[aá]' ) 
