COPY (SELECT *,(SELECT char_comma_sum(tipo_publico||': '||coalesce((coalesce(cant_personas,0)+coalesce(cant_personas_virtuales,0))::text,'*')) 
			...
			WHERE true  AND (ano = 2023)  AND (act.estado IN ('Realizada','Pendiente','Finalizada'))AND (id_tipo IN (SELECT id FROM vcm.tipos_act WHERE dimension='Educación Continua')) 
