COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '7607794-0' OR  a.rut ~* '7607794-0' OR  lower(a.email) ~* '7607794-0' OR  text(a.id) ~* '7607794-0' )  AND a.carrera_actual IN (52,99,92,49,111,89,93,107,86,87,72,84,85,44,40,46,64,39,45,42,47,53,43,51,71,73,74,41,65,66,90,75,63,113,124,121,122,123,129,130,134,133,132,127,131) 
