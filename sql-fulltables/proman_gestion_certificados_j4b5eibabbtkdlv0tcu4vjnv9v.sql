COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                         ...
                         CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2023 AND semestre=2 LIMIT 1)=1 THEN 'Si' ELSE 'No' END AS matric,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]' OR  a.rut ~* 'm[aá]r[ií][aá]' OR  text(ac.folio) ~* 'm[aá]r[ií][aá]' OR  vac.cod ~* 'm[aá]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l[ií]c[aá]' OR  a.rut ~* '[aá]ng[eé]l[ií]c[aá]' OR  text(ac.folio) ~* '[aá]ng[eé]l[ií]c[aá]' OR  vac.cod ~* '[aá]ng[eé]l[ií]c[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]rb[oó]n[aá]' OR  a.rut ~* 'n[aá]rb[oó]n[aá]' OR  text(ac.folio) ~* 'n[aá]rb[oó]n[aá]' OR  vac.cod ~* 'n[aá]rb[oó]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]nc[ií][aá]' OR  a.rut ~* 'v[aá]l[eé]nc[ií][aá]' OR  text(ac.folio) ~* 'v[aá]l[eé]nc[ií][aá]' OR  vac.cod ~* 'v[aá]l[eé]nc[ií][aá]' ) 
