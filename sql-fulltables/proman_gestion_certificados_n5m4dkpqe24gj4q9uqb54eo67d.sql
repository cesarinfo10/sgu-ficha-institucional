COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]tr[ií]c[ií][aá]' OR  a.rut ~* 'p[aá]tr[ií]c[ií][aá]' OR  text(ac.folio) ~* 'p[aá]tr[ií]c[ií][aá]' OR  vac.cod ~* 'p[aá]tr[ií]c[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rr[ií]g[aá]' OR  a.rut ~* 'b[aá]rr[ií]g[aá]' OR  text(ac.folio) ~* 'b[aá]rr[ií]g[aá]' OR  vac.cod ~* 'b[aá]rr[ií]g[aá]' ) 
