COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó]m[eé]z' OR  a.rut ~* 'g[oó]m[eé]z' OR  lower(a.email) ~* 'g[oó]m[eé]z' OR  text(a.id) ~* 'g[oó]m[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rv[aá]j[aá]l' OR  a.rut ~* 'c[aá]rv[aá]j[aá]l' OR  lower(a.email) ~* 'c[aá]rv[aá]j[aá]l' OR  text(a.id) ~* 'c[aá]rv[aá]j[aá]l' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148) 
