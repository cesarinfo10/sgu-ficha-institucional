COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]m[ií]l[oó]' OR  a.rut ~* 'c[aá]m[ií]l[oó]' OR  lower(a.email) ~* 'c[aá]m[ií]l[oó]' OR  text(a.id) ~* 'c[aá]m[ií]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  a.rut ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  lower(a.email) ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  text(a.id) ~* 'v[ií]ll[aá]rr[oó][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm' OR  a.rut ~* 'm' OR  lower(a.email) ~* 'm' OR  text(a.id) ~* 'm' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125) 
