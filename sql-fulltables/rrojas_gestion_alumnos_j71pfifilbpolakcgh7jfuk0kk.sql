COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó][eé]l' OR  a.rut ~* 'j[oó][eé]l' OR  lower(a.email) ~* 'j[oó][eé]l' OR  text(a.id) ~* 'j[oó][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[aá]c[oó]n[aá]' OR  a.rut ~* '[aá]n[aá]c[oó]n[aá]' OR  lower(a.email) ~* '[aá]n[aá]c[oó]n[aá]' OR  text(a.id) ~* '[aá]n[aá]c[oó]n[aá]' )  AND a.carrera_actual IN (59,57,27,28,33,54,56,31,58,60,55,30,32,29,76,79,82,81,83,78,80,38,115,114,77,125,147,148) 
