COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]rd[oó]' OR  a.rut ~* 'p[aá]rd[oó]' OR  lower(a.email) ~* 'p[aá]rd[oó]' OR  text(a.id) ~* 'p[aá]rd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[aá]r[ií]ñ[aá]' OR  a.rut ~* 'f[aá]r[ií]ñ[aá]' OR  lower(a.email) ~* 'f[aá]r[ií]ñ[aá]' OR  text(a.id) ~* 'f[aá]r[ií]ñ[aá]' ) 
