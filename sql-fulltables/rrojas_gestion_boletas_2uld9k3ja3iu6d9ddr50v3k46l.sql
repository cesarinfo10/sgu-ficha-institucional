COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '43743' OR  pap.rut ~* '43743' OR lower(a.nombres||' '||a.apellidos) ~* '43743' OR lower(a3.nombres||' '||a3.apellidos) ~* '43743' OR  a.rut ~* '43743' OR a2.rut ~* '43743' OR a3.rut ~* '43743'  OR text(nro_boleta) = '43743' )  
