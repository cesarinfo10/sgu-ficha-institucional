COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '58934' OR  pap.rut ~* '58934' OR lower(a.nombres||' '||a.apellidos) ~* '58934' OR lower(a3.nombres||' '||a3.apellidos) ~* '58934' OR  a.rut ~* '58934' OR a2.rut ~* '58934' OR a3.rut ~* '58934'  OR text(nro_boleta) = '58934' )  
