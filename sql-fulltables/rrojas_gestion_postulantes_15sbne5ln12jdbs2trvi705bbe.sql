COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sq[uú][ií]v[eé]l' OR  pap.rut ~* '[eé]sq[uú][ií]v[eé]l' OR  text(pap.id) ~* '[eé]sq[uú][ií]v[eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'tr[oó]nc[oó]s[oó]' OR  pap.rut ~* 'tr[oó]nc[oó]s[oó]' OR  text(pap.id) ~* 'tr[oó]nc[oó]s[oó]' ) 
