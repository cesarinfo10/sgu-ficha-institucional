COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[oó]m[eé]z' OR  pap.rut ~* 'g[oó]m[eé]z' OR  text(pap.id) ~* 'g[oó]m[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rv[aá]j[aá]l' OR  pap.rut ~* 'c[aá]rv[aá]j[aá]l' OR  text(pap.id) ~* 'c[aá]rv[aá]j[aá]l' ) 
