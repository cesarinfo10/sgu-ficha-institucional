COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]m[ií]n[aá]' OR  pap.rut ~* 'r[oó]m[ií]n[aá]' OR  text(pap.id) ~* 'r[oó]m[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]v[ií][eé]r[eé]s' OR  pap.rut ~* 'c[aá]v[ií][eé]r[eé]s' OR  text(pap.id) ~* 'c[aá]v[ií][eé]r[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá][eé]z' OR  pap.rut ~* 'p[aá][eé]z' OR  text(pap.id) ~* 'p[aá][eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
