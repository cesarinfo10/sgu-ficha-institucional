COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[eé]ss[ií]c[aá]' OR  pap.rut ~* 'j[eé]ss[ií]c[aá]' OR  text(pap.id) ~* 'j[eé]ss[ií]c[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]s[oó]r[ií][oó]' OR  pap.rut ~* '[oó]s[oó]r[ií][oó]' OR  text(pap.id) ~* '[oó]s[oó]r[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]l[ií]n[aá]' OR  pap.rut ~* 'm[oó]l[ií]n[aá]' OR  text(pap.id) ~* 'm[oó]l[ií]n[aá]' ) 
