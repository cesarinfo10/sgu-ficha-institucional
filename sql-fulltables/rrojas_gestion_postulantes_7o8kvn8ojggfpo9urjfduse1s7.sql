COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]tr[ií]c[ií][aá]' OR  pap.rut ~* 'p[aá]tr[ií]c[ií][aá]' OR  text(pap.id) ~* 'p[aá]tr[ií]c[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]r[oó]' OR  pap.rut ~* 't[oó]r[oó]' OR  text(pap.id) ~* 't[oó]r[oó]' ) 
