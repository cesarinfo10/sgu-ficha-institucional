COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]l[ií][aá]n[aá]' OR  pap.rut ~* '[eé]l[ií][aá]n[aá]' OR  text(pap.id) ~* '[eé]l[ií][aá]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rq[uú][eé]z' OR  pap.rut ~* 'm[aá]rq[uú][eé]z' OR  text(pap.id) ~* 'm[aá]rq[uú][eé]z' ) 
