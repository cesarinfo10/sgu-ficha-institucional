COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]ndr[aá]' OR  pap.rut ~* 's[aá]ndr[aá]' OR  text(pap.id) ~* 's[aá]ndr[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  pap.rut ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  text(pap.id) ~* 'f[ií]g[uú][eé]r[oó][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[eé]v[aá]l[oó]' OR  pap.rut ~* '[aá]r[eé]v[aá]l[oó]' OR  text(pap.id) ~* '[aá]r[eé]v[aá]l[oó]' ) 
