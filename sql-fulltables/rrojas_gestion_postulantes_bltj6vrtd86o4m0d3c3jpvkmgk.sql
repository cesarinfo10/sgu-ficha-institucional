COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[ií][aá]n[aá]' OR  pap.rut ~* 'm[aá]r[ií][aá]n[aá]' OR  text(pap.id) ~* 'm[aá]r[ií][aá]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  pap.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  text(pap.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'tr[eé]j[oó]s' OR  pap.rut ~* 'tr[eé]j[oó]s' OR  text(pap.id) ~* 'tr[eé]j[oó]s' ) 
