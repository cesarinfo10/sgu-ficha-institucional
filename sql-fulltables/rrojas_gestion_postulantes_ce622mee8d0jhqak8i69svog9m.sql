COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[ií][aá]' OR  pap.rut ~* 'm[aá]r[ií][aá]' OR  text(pap.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]z' OR  pap.rut ~* 'p[aá]z' OR  text(pap.id) ~* 'p[aá]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[eé]r[aá]lt[aá]' OR  pap.rut ~* 'p[eé]r[aá]lt[aá]' OR  text(pap.id) ~* 'p[eé]r[aá]lt[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  pap.rut ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  text(pap.id) ~* 'v[ií]ll[aá]rr[oó][eé]l' ) 
