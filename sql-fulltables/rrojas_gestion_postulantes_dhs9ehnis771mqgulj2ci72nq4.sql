COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 't[eé]r[eé]s[aá]' OR  pap.rut ~* 't[eé]r[eé]s[aá]' OR  text(pap.id) ~* 't[eé]r[eé]s[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[aá]mb[ií][oó]' OR  pap.rut ~* 'm[aá]r[aá]mb[ií][oó]' OR  text(pap.id) ~* 'm[aá]r[aá]mb[ií][oó]' ) 
