COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'gl[oó]r[ií][aá]' OR  pap.rut ~* 'gl[oó]r[ií][aá]' OR  text(pap.id) ~* 'gl[oó]r[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú]n[aá]' OR  pap.rut ~* 'l[uú]n[aá]' OR  text(pap.id) ~* 'l[uú]n[aá]' ) 
