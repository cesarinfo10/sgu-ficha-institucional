COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]r[ií][aá]' OR  pap.rut ~* 'm[aá]r[ií][aá]' OR  text(pap.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]z' OR  pap.rut ~* 'p[aá]z' OR  text(pap.id) ~* 'p[aá]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'n[uú]ñ[eé]z' OR  pap.rut ~* 'n[uú]ñ[eé]z' OR  text(pap.id) ~* 'n[uú]ñ[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  pap.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  text(pap.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' ) 
