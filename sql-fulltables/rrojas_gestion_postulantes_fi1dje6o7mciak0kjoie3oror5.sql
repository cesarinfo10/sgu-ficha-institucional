COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]lc[aá]' OR  pap.rut ~* '[ií]lc[aá]' OR  text(pap.id) ~* '[ií]lc[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]p[eé]' OR  pap.rut ~* 'c[aá]p[eé]' OR  text(pap.id) ~* 'c[aá]p[eé]' ) 
