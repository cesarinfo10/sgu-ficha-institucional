COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]n[ií][eé]l[aá]' OR  pap.rut ~* 'd[aá]n[ií][eé]l[aá]' OR  text(pap.id) ~* 'd[aá]n[ií][eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]ld[ií]v[ií][aá]' OR  pap.rut ~* 's[aá]ld[ií]v[ií][aá]' OR  text(pap.id) ~* 's[aá]ld[ií]v[ií][aá]' ) 
