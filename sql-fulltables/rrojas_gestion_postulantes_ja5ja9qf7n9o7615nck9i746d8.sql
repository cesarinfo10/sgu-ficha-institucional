COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[ií]lt[oó]n' OR  pap.rut ~* 'm[ií]lt[oó]n' OR  text(pap.id) ~* 'm[ií]lt[oó]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[oó]ns[aá]lv[eé]' OR  pap.rut ~* 'm[oó]ns[aá]lv[eé]' OR  text(pap.id) ~* 'm[oó]ns[aá]lv[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]v[ií]l[aá]' OR  pap.rut ~* '[aá]v[ií]l[aá]' OR  text(pap.id) ~* '[aá]v[ií]l[aá]' ) 
