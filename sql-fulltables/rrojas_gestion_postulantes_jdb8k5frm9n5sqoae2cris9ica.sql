COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]h[aá]n[aá]' OR  pap.rut ~* 'j[oó]h[aá]n[aá]' OR  text(pap.id) ~* 'j[oó]h[aá]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'pr[aá]d[eé]n[aá]' OR  pap.rut ~* 'pr[aá]d[eé]n[aá]' OR  text(pap.id) ~* 'pr[aá]d[eé]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]z[aá]' OR  pap.rut ~* 'm[eé]z[aá]' OR  text(pap.id) ~* 'm[eé]z[aá]' ) 
