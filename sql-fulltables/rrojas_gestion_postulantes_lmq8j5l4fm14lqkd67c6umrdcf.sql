COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]nst[aá]nz[aá]' OR  pap.rut ~* 'c[oó]nst[aá]nz[aá]' OR  text(pap.id) ~* 'c[oó]nst[aá]nz[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[ií]d' OR  pap.rut ~* 'c[ií]d' OR  text(pap.id) ~* 'c[ií]d' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nt[eé]s' OR  pap.rut ~* 'f[uú][eé]nt[eé]s' OR  text(pap.id) ~* 'f[uú][eé]nt[eé]s' ) 
