COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá][oó]l[aá]' OR  pap.rut ~* 'p[aá][oó]l[aá]' OR  text(pap.id) ~* 'p[aá][oó]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ndr[eé][aá]' OR  pap.rut ~* '[aá]ndr[eé][aá]' OR  text(pap.id) ~* '[aá]ndr[eé][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]ntr[eé]r[aá]s' OR  pap.rut ~* 'c[oó]ntr[eé]r[aá]s' OR  text(pap.id) ~* 'c[oó]ntr[eé]r[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]r[aá]' OR  pap.rut ~* 'v[eé]r[aá]' OR  text(pap.id) ~* 'v[eé]r[aá]' ) 
