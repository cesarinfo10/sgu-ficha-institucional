COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]d[uú][aá]rd[oó]' OR  pap.rut ~* '[eé]d[uú][aá]rd[oó]' OR  text(pap.id) ~* '[eé]d[uú][aá]rd[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ndr[eé]s' OR  pap.rut ~* '[aá]ndr[eé]s' OR  text(pap.id) ~* '[aá]ndr[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[uú]zm[aá]n' OR  pap.rut ~* 'g[uú]zm[aá]n' OR  text(pap.id) ~* 'g[uú]zm[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[uú]zm[aá]n' OR  pap.rut ~* 'g[uú]zm[aá]n' OR  text(pap.id) ~* 'g[uú]zm[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
