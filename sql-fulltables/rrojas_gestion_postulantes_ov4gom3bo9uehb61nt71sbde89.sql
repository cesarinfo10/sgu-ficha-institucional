COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '15719307' OR  pap.rut ~* '15719307' OR  text(pap.id) ~* '15719307' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '-4' OR  pap.rut ~* '-4' OR  text(pap.id) ~* '-4' ) 
