COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]c[ií][oó]' OR  pap.rut ~* 'r[oó]c[ií][oó]' OR  text(pap.id) ~* 'r[oó]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]fr[eé]' OR  pap.rut ~* 'c[oó]fr[eé]' OR  text(pap.id) ~* 'c[oó]fr[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]nd[oó]v[aá]l' OR  pap.rut ~* 's[aá]nd[oó]v[aá]l' OR  text(pap.id) ~* 's[aá]nd[oó]v[aá]l' ) 
