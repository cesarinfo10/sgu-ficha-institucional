COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]v[eé]nd[aá]ñ[oó]' OR  pap.rut ~* '[aá]v[eé]nd[aá]ñ[oó]' OR  text(pap.id) ~* '[aá]v[eé]nd[aá]ñ[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]' OR  pap.rut ~* '[aá]' OR  text(pap.id) ~* '[aá]' ) 
