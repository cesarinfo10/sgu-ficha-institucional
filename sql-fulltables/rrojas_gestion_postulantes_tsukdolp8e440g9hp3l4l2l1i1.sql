COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]x' OR  pap.rut ~* '[aá]l[eé]x' OR  text(pap.id) ~* '[aá]l[eé]x' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]n[aá]l' OR  pap.rut ~* 'p[aá]n[aá]l' OR  text(pap.id) ~* 'p[aá]n[aá]l' ) 
