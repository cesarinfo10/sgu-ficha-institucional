COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[ií]n[aá]' OR  pap.rut ~* 'g[ií]n[aá]' OR  text(pap.id) ~* 'g[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]br[ií][eé]l[aá]' OR  pap.rut ~* 'g[aá]br[ií][eé]l[aá]' OR  text(pap.id) ~* 'g[aá]br[ií][eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]g[uú][aá]y[oó]' OR  pap.rut ~* '[aá]g[uú][aá]y[oó]' OR  text(pap.id) ~* '[aá]g[uú][aá]y[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[eé]l[eé]ll[ií]' OR  pap.rut ~* 'm[eé]l[eé]ll[ií]' OR  text(pap.id) ~* 'm[eé]l[eé]ll[ií]' ) 
