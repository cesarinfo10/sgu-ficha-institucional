COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]ld[eé]s' OR  pap.rut ~* 'v[aá]ld[eé]s' OR  text(pap.id) ~* 'v[aá]ld[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sp[ií]n[aá]' OR  pap.rut ~* '[eé]sp[ií]n[aá]' OR  text(pap.id) ~* '[eé]sp[ií]n[aá]' ) 
