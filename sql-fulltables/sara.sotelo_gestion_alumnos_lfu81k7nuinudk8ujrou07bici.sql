COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá][ií]t[eé]' OR  a.rut ~* 'm[aá][ií]t[eé]' OR  lower(a.email) ~* 'm[aá][ií]t[eé]' OR  text(a.id) ~* 'm[aá][ií]t[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[oó]r[ií][eé]g[aá]' OR  a.rut ~* 'n[oó]r[ií][eé]g[aá]' OR  lower(a.email) ~* 'n[oó]r[ií][eé]g[aá]' OR  text(a.id) ~* 'n[oó]r[ií][eé]g[aá]' ) 
