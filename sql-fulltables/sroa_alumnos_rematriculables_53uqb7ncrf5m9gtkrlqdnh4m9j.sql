COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2024 AND semestre=1 LIMIT 1)=1  THEN 'Si' ELSE 'No' END AS matriculado,(SELECT to_char(max(fecha),'DD/MM/YYYY') FROM matriculas WHERE id_alumno=a.id) AS fecha_ult_mat,
                       ...
                       WHERE vac.id_alumno=a.id),1) AS nivel_estudios, 
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2024 AND regimen='PRE' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2024   
                FROM alumnos AS a
                ...
                WHERE true  AND (a.semestre_cohorte = 1 ) AND (a.cohorte = '2022' ) AND (a.estado = '1') AND (a.moroso_financiero = 'f') AND (c.regimen = 'PRE') AND (a.id NOT IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1))  AND ((CASE WHEN (SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE semestre=2 AND ano=2023)) > 0 THEN ((SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_estado=1 AND id_curso IN (SELECT id FROM cursos WHERE semestre=2 AND ano=2023))::real/(SELECT count(id_curso) FROM cargas_academicas WHERE id_alumno=a.id AND id_curso IN (SELECT id FROM cursos WHERE semestre=2 AND ano=2023))::real*100)::numeric(4,1) ELSE 0 END) BETWEEN 1 AND 100) 
