COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN dae.renovantes_cae AS renov_cae ON (renov_cae.ano=2022 AND renov_cae.rut=split_part(trim(a.rut),'-',1)::int8)
                ...
                WHERE true  AND (a.estado = '1') AND (c.regimen = 'PRE')  AND renov_cae.rut IS NOT NULL AND (a.id NOT IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2)) 
