COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                LEFT JOIN dae.cae_preseleccionados_publicacion AS presel_pub ON (presel_pub.ano=2023 AND presel_pub.rut=split_part(trim(a.rut),'-',1)::int8)
                LEFT JOIN dae.vista_presel_mineduc AS presel_mineduc ON (presel_mineduc.ano=2024 AND presel_mineduc.rut=split_part(trim(a.rut),'-',1)::int8)
                ...
                WHERE true  AND (a.estado = '1') AND (c.regimen = 'PRE')  AND presel_pub.rut IS NOT NULL  AND presel_mineduc.rut IS NOT NULL AND presel_mineduc.cant_becas > 0 AND (a.id NOT IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1)) 
