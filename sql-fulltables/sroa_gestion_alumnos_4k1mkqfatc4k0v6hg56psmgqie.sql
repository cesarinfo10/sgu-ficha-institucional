COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]v[eé]lyn' OR  a.rut ~* '[eé]v[eé]lyn' OR  lower(a.email) ~* '[eé]v[eé]lyn' OR  text(a.id) ~* '[eé]v[eé]lyn' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ll[eé]nd[eé]s' OR  a.rut ~* '[aá]ll[eé]nd[eé]s' OR  lower(a.email) ~* '[aá]ll[eé]nd[eé]s' OR  text(a.id) ~* '[aá]ll[eé]nd[eé]s' ) 
