COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]p[uú]lv[eé]' OR  a.rut ~* 's[eé]p[uú]lv[eé]' OR  lower(a.email) ~* 's[eé]p[uú]lv[eé]' OR  text(a.id) ~* 's[eé]p[uú]lv[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ll[aá]gr[aá]n' OR  a.rut ~* 'v[ií]ll[aá]gr[aá]n' OR  lower(a.email) ~* 'v[ií]ll[aá]gr[aá]n' OR  text(a.id) ~* 'v[ií]ll[aá]gr[aá]n' ) 
