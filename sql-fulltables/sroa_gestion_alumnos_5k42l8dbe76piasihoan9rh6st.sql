COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]gl[eé]s[ií][aá]s' OR  a.rut ~* '[ií]gl[eé]s[ií][aá]s' OR  lower(a.email) ~* '[ií]gl[eé]s[ií][aá]s' OR  text(a.id) ~* '[ií]gl[eé]s[ií][aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]h[uú]m[aá]d[aá]' OR  a.rut ~* '[aá]h[uú]m[aá]d[aá]' OR  lower(a.email) ~* '[aá]h[uú]m[aá]d[aá]' OR  text(a.id) ~* '[aá]h[uú]m[aá]d[aá]' ) 
