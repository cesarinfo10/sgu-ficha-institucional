COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]br[ií][eé]l' OR  a.rut ~* 'g[aá]br[ií][eé]l' OR  lower(a.email) ~* 'g[aá]br[ií][eé]l' OR  text(a.id) ~* 'g[aá]br[ií][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[ií]ll[aá]m[aá]n' OR  a.rut ~* 'b[ií]ll[aá]m[aá]n' OR  lower(a.email) ~* 'b[ií]ll[aá]m[aá]n' OR  text(a.id) ~* 'b[ií]ll[aá]m[aá]n' ) 
