COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'jh[oó]n[aá]t[aá]n' OR  a.rut ~* 'jh[oó]n[aá]t[aá]n' OR  lower(a.email) ~* 'jh[oó]n[aá]t[aá]n' OR  text(a.id) ~* 'jh[oó]n[aá]t[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó][ií]c[oó]ch[eé][aá]' OR  a.rut ~* 'g[oó][ií]c[oó]ch[eé][aá]' OR  lower(a.email) ~* 'g[oó][ií]c[oó]ch[eé][aá]' OR  text(a.id) ~* 'g[oó][ií]c[oó]ch[eé][aá]' ) 
