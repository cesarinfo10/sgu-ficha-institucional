COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó][aá]nn[ií]' OR  a.rut ~* 'j[oó][aá]nn[ií]' OR  lower(a.email) ~* 'j[oó][aá]nn[ií]' OR  text(a.id) ~* 'j[oó][aá]nn[ií]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[eé]n[ií][aá]' OR  a.rut ~* 't[eé]n[ií][aá]' OR  lower(a.email) ~* 't[eé]n[ií][aá]' OR  text(a.id) ~* 't[eé]n[ií][aá]' ) 
