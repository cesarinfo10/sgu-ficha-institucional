COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]v[oó]nn[eé]' OR  a.rut ~* '[ií]v[oó]nn[eé]' OR  lower(a.email) ~* '[ií]v[oó]nn[eé]' OR  text(a.id) ~* '[ií]v[oó]nn[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[ií]zm[eé]nd[ií]' OR  a.rut ~* '[aá]r[ií]zm[eé]nd[ií]' OR  lower(a.email) ~* '[aá]r[ií]zm[eé]nd[ií]' OR  text(a.id) ~* '[aá]r[ií]zm[eé]nd[ií]' ) 
