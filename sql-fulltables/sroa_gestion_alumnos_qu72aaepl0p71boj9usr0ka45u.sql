COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]tr[ií]c[ií][oó]' OR  a.rut ~* 'p[aá]tr[ií]c[ií][oó]' OR  lower(a.email) ~* 'p[aá]tr[ií]c[ií][oó]' OR  text(a.id) ~* 'p[aá]tr[ií]c[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]n[aá]r[eé]s' OR  a.rut ~* 'm[eé]n[aá]r[eé]s' OR  lower(a.email) ~* 'm[eé]n[aá]r[eé]s' OR  text(a.id) ~* 'm[eé]n[aá]r[eé]s' ) 
