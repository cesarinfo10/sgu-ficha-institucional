COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ld[oó]' OR  a.rut ~* '[aá]ld[oó]' OR  lower(a.email) ~* '[aá]ld[oó]' OR  text(a.id) ~* '[aá]ld[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR  a.rut ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR  lower(a.email) ~* 'p[eé]rn[ií]g[oó]tt[ií]' OR  text(a.id) ~* 'p[eé]rn[ií]g[oó]tt[ií]' ) 
