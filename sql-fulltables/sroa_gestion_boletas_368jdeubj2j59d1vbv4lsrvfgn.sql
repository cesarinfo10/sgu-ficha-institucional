COPY (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_factura) AS nro_docto,
                ...
                WHERE nro_boleta IS NOT NULL AND (lower(pap.nombres||' '||pap.apellidos) ~* '16769152' OR  pap.rut ~* '16769152' OR lower(a.nombres||' '||a.apellidos) ~* '16769152' OR lower(a3.nombres||' '||a3.apellidos) ~* '16769152' OR  a.rut ~* '16769152' OR a2.rut ~* '16769152' OR a3.rut ~* '16769152'  OR text(nro_boleta) = '16769152' )  
