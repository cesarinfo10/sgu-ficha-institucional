COPY (SELECT c.id,to_char(c.fecha,'DD-MM-YYYY') AS fecha,to_char(c.fecha,'HH24:MI') AS hora,c.tipo,c.estado,to_char(c.estado_fecha,'DD-MM-YYYY') AS estado_fecha,
                         ...
                         CASE WHEN c.monto_arancel > 0 THEN round(vc.monto_beca_arancel_calc*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2021-01-01'::date AND '2023-02-15'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS arancel_beca_contable,
                         ...
                         CASE WHEN c.monto_arancel > 0 THEN round(c.arancel_cred_interno*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2021-01-01'::date AND '2023-02-15'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS arancel_cred_int_contable,
                         ...
                         CASE WHEN c.fecha_condonacion BETWEEN '2021-01-01'::date AND '2023-02-15'::date THEN c.monto_condonacion ELSE null END AS monto_condonacion,
