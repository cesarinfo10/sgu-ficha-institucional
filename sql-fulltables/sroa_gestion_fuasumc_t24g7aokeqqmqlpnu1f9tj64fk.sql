COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]rm[aá]nd[oó]' OR  a.rut ~* '[aá]rm[aá]nd[oó]' OR  text(a.id) ~* '[aá]rm[aá]nd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'br[ií]c[eé]ñ[oó]' OR  a.rut ~* 'br[ií]c[eé]ñ[oó]' OR  text(a.id) ~* 'br[ií]c[eé]ñ[oó]' ) 
