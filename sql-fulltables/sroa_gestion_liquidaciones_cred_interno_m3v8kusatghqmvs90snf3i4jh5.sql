COPY (SELECT c.id,to_char(c.fecha,'DD-tmMon-YYYY') AS fecha,c.estado,
                     ...
                     WHERE (lower(va.nombre) ~* 'cr[ií]st[ií][aá]n' OR  a.rut ~* 'cr[ií]st[ií][aá]n' OR  text(c.id) ~* 'cr[ií]st[ií][aá]n' ) AND (lower(va.nombre) ~* 's[aá]b[aá]ll[aá]' OR  a.rut ~* 's[aá]b[aá]ll[aá]' OR  text(c.id) ~* 's[aá]b[aá]ll[aá]' ) 
