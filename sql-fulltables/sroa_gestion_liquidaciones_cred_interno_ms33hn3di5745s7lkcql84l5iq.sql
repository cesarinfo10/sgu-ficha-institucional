COPY (SELECT c.id,to_char(c.fecha,'DD-tmMon-YYYY') AS fecha,c.estado,
                     ...
                     WHERE (lower(va.nombre) ~* '' OR  a.rut ~* '' OR  text(c.id) ~* '' ) AND (lower(va.nombre) ~* 's[eé]rg[ií][oó]' OR  a.rut ~* 's[eé]rg[ií][oó]' OR  text(c.id) ~* 's[eé]rg[ií][oó]' ) AND (lower(va.nombre) ~* '[oó]sv[aá]ld[oó]' OR  a.rut ~* '[oó]sv[aá]ld[oó]' OR  text(c.id) ~* '[oó]sv[aá]ld[oó]' ) AND (lower(va.nombre) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  a.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  text(c.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' ) AND (lower(va.nombre) ~* 'm[oó]nd[aá]c[aá]' OR  a.rut ~* 'm[oó]nd[aá]c[aá]' OR  text(c.id) ~* 'm[oó]nd[aá]c[aá]' ) 
