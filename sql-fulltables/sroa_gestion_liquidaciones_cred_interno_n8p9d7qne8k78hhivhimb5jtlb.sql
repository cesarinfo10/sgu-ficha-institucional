COPY (SELECT c.id,to_char(c.fecha,'DD-tmMon-YYYY') AS fecha,c.estado,
                     ...
                     WHERE (lower(va.nombre) ~* 'v[aá]n[eé]ss[aá]' OR  a.rut ~* 'v[aá]n[eé]ss[aá]' OR  text(c.id) ~* 'v[aá]n[eé]ss[aá]' ) AND (lower(va.nombre) ~* '[ií]g[oó]r' OR  a.rut ~* '[ií]g[oó]r' OR  text(c.id) ~* '[ií]g[oó]r' ) 
