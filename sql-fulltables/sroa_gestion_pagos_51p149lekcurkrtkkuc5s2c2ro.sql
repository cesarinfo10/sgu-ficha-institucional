COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '15485584' OR  pap.rut ~* '15485584' OR lower(a.nombres||' '||a.apellidos) ~* '15485584' OR lower(a3.nombres||' '||a3.apellidos) ~* '15485584' OR  a.rut ~* '15485584' OR a2.rut ~* '15485584' OR a3.rut ~* '15485584' )  
