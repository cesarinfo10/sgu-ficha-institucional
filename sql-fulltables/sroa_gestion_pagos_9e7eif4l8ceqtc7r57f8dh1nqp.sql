COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '13892252-9' OR  pap.rut ~* '13892252-9' OR lower(a.nombres||' '||a.apellidos) ~* '13892252-9' OR lower(a3.nombres||' '||a3.apellidos) ~* '13892252-9' OR  a.rut ~* '13892252-9' OR a2.rut ~* '13892252-9' OR a3.rut ~* '13892252-9' )  
