COPY (SELECT coalesce(nro_boleta,nro_boleta_e,nro_factura) AS nro_docto,
			...
			WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '13493729' OR  pap.rut ~* '13493729' OR lower(a.nombres||' '||a.apellidos) ~* '13493729' OR lower(a3.nombres||' '||a3.apellidos) ~* '13493729' OR  a.rut ~* '13493729' OR a2.rut ~* '13493729' OR a3.rut ~* '13493729' ) 
