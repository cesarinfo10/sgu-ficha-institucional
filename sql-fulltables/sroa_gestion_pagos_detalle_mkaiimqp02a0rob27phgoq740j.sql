COPY (SELECT coalesce(nro_boleta,nro_boleta_e,nro_factura) AS nro_docto,
			...
			WHERE (p.nro_boleta IS NOT NULL OR p.nro_boleta_e IS NOT NULL OR p.nro_factura IS NOT NULL) AND p.fecha BETWEEN '2022-11-01'::date AND '2022-11-30'::date AND ('PRE' IN (car.regimen,car2.regimen,car3.regimen) OR p.nulo) 
