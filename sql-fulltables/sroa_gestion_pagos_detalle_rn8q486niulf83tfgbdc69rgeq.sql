COPY (SELECT coalesce(nro_boleta,nro_boleta_e,nro_factura) AS nro_docto,
			...
			WHERE (p.nro_boleta IS NOT NULL OR p.nro_boleta_e IS NOT NULL OR p.nro_factura IS NOT NULL) AND p.fecha BETWEEN '2023-04-26'::date AND '2023-04-26'::date AND ('PRE' IN (car.regimen,car2.regimen,car3.regimen) OR p.nulo) 
