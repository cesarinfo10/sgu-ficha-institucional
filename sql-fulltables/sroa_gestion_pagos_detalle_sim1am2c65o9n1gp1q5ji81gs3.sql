COPY (SELECT coalesce(nro_boleta,nro_boleta_e,nro_factura) AS nro_docto,
			...
			WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '15350760' OR  pap.rut ~* '15350760' OR lower(a.nombres||' '||a.apellidos) ~* '15350760' OR lower(a3.nombres||' '||a3.apellidos) ~* '15350760' OR  a.rut ~* '15350760' OR a2.rut ~* '15350760' OR a3.rut ~* '15350760' ) 
