COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]v[eé]lyn' OR  pap.rut ~* '[eé]v[eé]lyn' OR lower(a.nombres||' '||a.apellidos) ~* '[eé]v[eé]lyn' OR lower(a3.nombres||' '||a3.apellidos) ~* '[eé]v[eé]lyn' OR  a.rut ~* '[eé]v[eé]lyn' OR a2.rut ~* '[eé]v[eé]lyn' OR a3.rut ~* '[eé]v[eé]lyn' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé][ií]v[aá]' OR  pap.rut ~* 'l[eé][ií]v[aá]' OR lower(a.nombres||' '||a.apellidos) ~* 'l[eé][ií]v[aá]' OR lower(a3.nombres||' '||a3.apellidos) ~* 'l[eé][ií]v[aá]' OR  a.rut ~* 'l[eé][ií]v[aá]' OR a2.rut ~* 'l[eé][ií]v[aá]' OR a3.rut ~* 'l[eé][ií]v[aá]' )  
