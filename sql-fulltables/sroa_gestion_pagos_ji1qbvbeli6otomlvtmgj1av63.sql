COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]rth[aá]' OR  pap.rut ~* '[eé]rth[aá]' OR lower(a.nombres||' '||a.apellidos) ~* '[eé]rth[aá]' OR lower(a3.nombres||' '||a3.apellidos) ~* '[eé]rth[aá]' OR  a.rut ~* '[eé]rth[aá]' OR a2.rut ~* '[eé]rth[aá]' OR a3.rut ~* '[eé]rth[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'bl[aá]nch[eé]t' OR  pap.rut ~* 'bl[aá]nch[eé]t' OR lower(a.nombres||' '||a.apellidos) ~* 'bl[aá]nch[eé]t' OR lower(a3.nombres||' '||a3.apellidos) ~* 'bl[aá]nch[eé]t' OR  a.rut ~* 'bl[aá]nch[eé]t' OR a2.rut ~* 'bl[aá]nch[eé]t' OR a3.rut ~* 'bl[aá]nch[eé]t' )  
