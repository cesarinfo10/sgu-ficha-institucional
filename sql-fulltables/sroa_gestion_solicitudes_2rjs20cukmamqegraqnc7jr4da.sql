COPY (SELECT s.id,a.rut,a.nombres,a.apellidos,va.carrera||'-'||a.jornada AS carrera,
			  ...
			  WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]t[eé]n[eé][aá]' OR  a.rut ~* '[aá]t[eé]n[eé][aá]' OR  text(a.id) ~* '[aá]t[eé]n[eé][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]t[oó]' OR  a.rut ~* 's[oó]t[oó]' OR  text(a.id) ~* 's[oó]t[oó]' ) 
