COPY (SELECT s.id,a.rut,a.nombres,a.apellidos,va.carrera||'-'||a.jornada AS carrera,
			  ...
			  WHERE true  AND (s.estado IN ('Aceptada')) AND (date_part('year',s.fecha) = 2023) AND (s.id IN (SELECT id_solicitud FROM gestion.solic_respuestas WHERE id_usuario=1180 AND fecha_reasignacion IS NULL AND id_usuario_reasig IS NULL)) 
