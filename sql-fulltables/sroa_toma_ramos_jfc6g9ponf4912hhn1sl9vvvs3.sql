COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE (SELECT count(id_alumno) FROM inscripciones_cursos WHERE id_alumno=a.id)=0   AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[eé]s[ií][aá]' OR  a.rut ~* 'fr[eé]s[ií][aá]' OR  lower(a.email) ~* 'fr[eé]s[ií][aá]' OR  text(a.id) ~* 'fr[eé]s[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]p[eé]z' OR  a.rut ~* 'l[oó]p[eé]z' OR  lower(a.email) ~* 'l[oó]p[eé]z' OR  text(a.id) ~* 'l[oó]p[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]mp[uú]z[aá]n[oó]' OR  a.rut ~* 'c[aá]mp[uú]z[aá]n[oó]' OR  lower(a.email) ~* 'c[aá]mp[uú]z[aá]n[oó]' OR  text(a.id) ~* 'c[aá]mp[uú]z[aá]n[oó]' ) 
