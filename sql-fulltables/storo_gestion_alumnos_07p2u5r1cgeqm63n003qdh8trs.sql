COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]m[eé]tt[ií]' OR  a.rut ~* 'y[aá]m[eé]tt[ií]' OR  lower(a.email) ~* 'y[aá]m[eé]tt[ií]' OR  text(a.id) ~* 'y[aá]m[eé]tt[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
