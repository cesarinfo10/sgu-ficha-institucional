COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][oó]' OR  a.rut ~* 'm[aá]r[ií][oó]' OR  lower(a.email) ~* 'm[aá]r[ií][oó]' OR  text(a.id) ~* 'm[aá]r[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  a.rut ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  lower(a.email) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' OR  text(a.id) ~* 'f[uú][eé]nz[aá]l[ií]d[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
