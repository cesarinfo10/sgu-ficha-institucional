COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]l' OR  a.rut ~* 's[oó]l' OR  lower(a.email) ~* 's[oó]l' OR  text(a.id) ~* 's[oó]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  a.rut ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  lower(a.email) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  text(a.id) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
