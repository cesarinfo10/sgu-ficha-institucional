COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[aá]nc[ií]sc[aá]' OR  a.rut ~* 'fr[aá]nc[ií]sc[aá]' OR  lower(a.email) ~* 'fr[aá]nc[ií]sc[aá]' OR  text(a.id) ~* 'fr[aá]nc[ií]sc[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]l[aá]sq[uú][eé]z' OR  a.rut ~* 'v[eé]l[aá]sq[uú][eé]z' OR  lower(a.email) ~* 'v[eé]l[aá]sq[uú][eé]z' OR  text(a.id) ~* 'v[eé]l[aá]sq[uú][eé]z' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
