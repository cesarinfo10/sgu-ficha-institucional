COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[eé]l[aá]' OR  a.rut ~* 'm[aá]rc[eé]l[aá]' OR  lower(a.email) ~* 'm[aá]rc[eé]l[aá]' OR  text(a.id) ~* 'm[aá]rc[eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[eé]ll[aá]n[oó]' OR  a.rut ~* '[aá]r[eé]ll[aá]n[oó]' OR  lower(a.email) ~* '[aá]r[eé]ll[aá]n[oó]' OR  text(a.id) ~* '[aá]r[eé]ll[aá]n[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
