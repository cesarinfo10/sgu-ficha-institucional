COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[aá]n[oó]' OR  a.rut ~* 'fr[aá]n[oó]' OR  lower(a.email) ~* 'fr[aá]n[oó]' OR  text(a.id) ~* 'fr[aá]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[oó]d[uú]j[eé]' OR  a.rut ~* 'p[oó]d[uú]j[eé]' OR  lower(a.email) ~* 'p[oó]d[uú]j[eé]' OR  text(a.id) ~* 'p[oó]d[uú]j[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
