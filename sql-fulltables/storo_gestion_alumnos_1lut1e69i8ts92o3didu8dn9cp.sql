COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[aá]nn[ií][aá]' OR  a.rut ~* 't[aá]nn[ií][aá]' OR  lower(a.email) ~* 't[aá]nn[ií][aá]' OR  text(a.id) ~* 't[aá]nn[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[oó]z[oó]' OR  a.rut ~* 'p[oó]z[oó]' OR  lower(a.email) ~* 'p[oó]z[oó]' OR  text(a.id) ~* 'p[oó]z[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
