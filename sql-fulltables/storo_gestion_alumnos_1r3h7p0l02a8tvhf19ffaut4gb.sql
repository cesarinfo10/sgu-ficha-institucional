COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '10422288' OR  a.rut ~* '10422288' OR  lower(a.email) ~* '10422288' OR  text(a.id) ~* '10422288' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
