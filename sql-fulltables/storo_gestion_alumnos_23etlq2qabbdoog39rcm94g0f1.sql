COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]d[ií]n[aá]' OR  a.rut ~* 'm[eé]d[ií]n[aá]' OR  lower(a.email) ~* 'm[eé]d[ií]n[aá]' OR  text(a.id) ~* 'm[eé]d[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[ií]' OR  a.rut ~* 'c[aá]r[ií]' OR  lower(a.email) ~* 'c[aá]r[ií]' OR  text(a.id) ~* 'c[aá]r[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
