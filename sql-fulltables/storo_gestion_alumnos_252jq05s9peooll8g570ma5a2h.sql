COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]' OR  a.rut ~* 'm[aá]r[ií][aá]' OR  lower(a.email) ~* 'm[aá]r[ií][aá]' OR  text(a.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú][ií]s[aá]' OR  a.rut ~* 'l[uú][ií]s[aá]' OR  lower(a.email) ~* 'l[uú][ií]s[aá]' OR  text(a.id) ~* 'l[uú][ií]s[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]n[oó]t' OR  a.rut ~* 'p[ií]n[oó]t' OR  lower(a.email) ~* 'p[ií]n[oó]t' OR  text(a.id) ~* 'p[ií]n[oó]t' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
