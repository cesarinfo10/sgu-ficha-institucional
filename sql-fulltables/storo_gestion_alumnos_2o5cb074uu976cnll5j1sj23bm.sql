COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[aá][uú]l' OR  a.rut ~* 'r[aá][uú]l' OR  lower(a.email) ~* 'r[aá][uú]l' OR  text(a.id) ~* 'r[aá][uú]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[ií][eé]rr[oó]' OR  a.rut ~* 'f[ií][eé]rr[oó]' OR  lower(a.email) ~* 'f[ií][eé]rr[oó]' OR  text(a.id) ~* 'f[ií][eé]rr[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
