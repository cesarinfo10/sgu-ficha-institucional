COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lt[aá]m[ií]r[aá]n[oó]' OR  a.rut ~* '[aá]lt[aá]m[ií]r[aá]n[oó]' OR  lower(a.email) ~* '[aá]lt[aá]m[ií]r[aá]n[oó]' OR  text(a.id) ~* '[aá]lt[aá]m[ií]r[aá]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]g[aá]' OR  a.rut ~* 'v[eé]g[aá]' OR  lower(a.email) ~* 'v[eé]g[aá]' OR  text(a.id) ~* 'v[eé]g[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]bl[oó]' OR  a.rut ~* 'p[aá]bl[oó]' OR  lower(a.email) ~* 'p[aá]bl[oó]' OR  text(a.id) ~* 'p[aá]bl[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
