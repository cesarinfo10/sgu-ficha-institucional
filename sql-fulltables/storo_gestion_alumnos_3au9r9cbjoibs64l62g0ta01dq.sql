COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]n[ií][eé]l' OR  a.rut ~* 'd[aá]n[ií][eé]l' OR  lower(a.email) ~* 'd[aá]n[ií][eé]l' OR  text(a.id) ~* 'd[aá]n[ií][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[eé]n[aá]s' OR  a.rut ~* '[aá]r[eé]n[aá]s' OR  lower(a.email) ~* '[aá]r[eé]n[aá]s' OR  text(a.id) ~* '[aá]r[eé]n[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]y[aá]' OR  a.rut ~* 'm[oó]y[aá]' OR  lower(a.email) ~* 'm[oó]y[aá]' OR  text(a.id) ~* 'm[oó]y[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
