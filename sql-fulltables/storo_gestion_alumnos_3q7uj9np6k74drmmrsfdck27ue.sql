COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '27.760.979' OR  a.rut ~* '27.760.979' OR  lower(a.email) ~* '27.760.979' OR  text(a.id) ~* '27.760.979' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
