COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[eé]nn' OR  a.rut ~* 'k[aá]r[eé]nn' OR  lower(a.email) ~* 'k[aá]r[eé]nn' OR  text(a.id) ~* 'k[aá]r[eé]nn' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
