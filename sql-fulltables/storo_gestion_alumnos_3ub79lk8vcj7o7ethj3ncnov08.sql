COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'cl[aá][uú]d[ií][oó]' OR  a.rut ~* 'cl[aá][uú]d[ií][oó]' OR  lower(a.email) ~* 'cl[aá][uú]d[ií][oó]' OR  text(a.id) ~* 'cl[aá][uú]d[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]t[aá]r[oó]l[aá]' OR  a.rut ~* '[oó]t[aá]r[oó]l[aá]' OR  lower(a.email) ~* '[oó]t[aá]r[oó]l[aá]' OR  text(a.id) ~* '[oó]t[aá]r[oó]l[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
