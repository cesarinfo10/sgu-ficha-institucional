COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]t[ií][aá]s' OR  a.rut ~* 'm[aá]t[ií][aá]s' OR  lower(a.email) ~* 'm[aá]t[ií][aá]s' OR  text(a.id) ~* 'm[aá]t[ií][aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó]m[ií]l[aá]' OR  a.rut ~* 'g[oó]m[ií]l[aá]' OR  lower(a.email) ~* 'g[oó]m[ií]l[aá]' OR  text(a.id) ~* 'g[oó]m[ií]l[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
