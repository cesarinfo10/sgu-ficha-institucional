COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'br[aá]y[aá]n' OR  a.rut ~* 'br[aá]y[aá]n' OR  lower(a.email) ~* 'br[aá]y[aá]n' OR  text(a.id) ~* 'br[aá]y[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[oó]rr[eé][aá]lb[aá]' OR  a.rut ~* 't[oó]rr[eé][aá]lb[aá]' OR  lower(a.email) ~* 't[oó]rr[eé][aá]lb[aá]' OR  text(a.id) ~* 't[oó]rr[eé][aá]lb[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
