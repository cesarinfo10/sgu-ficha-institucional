COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]s[eé]' OR  a.rut ~* 'j[oó]s[eé]' OR  lower(a.email) ~* 'j[oó]s[eé]' OR  text(a.id) ~* 'j[oó]s[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]s[aá]n[oó]v[aá]' OR  a.rut ~* 'c[aá]s[aá]n[oó]v[aá]' OR  lower(a.email) ~* 'c[aá]s[aá]n[oó]v[aá]' OR  text(a.id) ~* 'c[aá]s[aá]n[oó]v[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
