COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]dr[ií][aá]n' OR  a.rut ~* '[aá]dr[ií][aá]n' OR  lower(a.email) ~* '[aá]dr[ií][aá]n' OR  text(a.id) ~* '[aá]dr[ií][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]ll[oó][aá]' OR  a.rut ~* '[uú]ll[oó][aá]' OR  lower(a.email) ~* '[uú]ll[oó][aá]' OR  text(a.id) ~* '[uú]ll[oó][aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
