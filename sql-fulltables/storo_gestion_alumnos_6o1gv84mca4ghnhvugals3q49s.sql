COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]lv[aá]' OR  a.rut ~* 's[ií]lv[aá]' OR  lower(a.email) ~* 's[ií]lv[aá]' OR  text(a.id) ~* 's[ií]lv[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]l[aá]b[aá]c[eé]r[oó]s' OR  a.rut ~* 'c[aá]l[aá]b[aá]c[eé]r[oó]s' OR  lower(a.email) ~* 'c[aá]l[aá]b[aá]c[eé]r[oó]s' OR  text(a.id) ~* 'c[aá]l[aá]b[aá]c[eé]r[oó]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
