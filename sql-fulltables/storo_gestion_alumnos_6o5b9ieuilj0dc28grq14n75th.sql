COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]l[eé]nk[oó]' OR  a.rut ~* 'm[ií]l[eé]nk[oó]' OR  lower(a.email) ~* 'm[ií]l[eé]nk[oó]' OR  text(a.id) ~* 'm[ií]l[eé]nk[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
