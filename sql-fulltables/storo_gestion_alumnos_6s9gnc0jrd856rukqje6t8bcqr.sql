COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 't[aá]b[ií]t[aá]' OR  a.rut ~* 't[aá]b[ií]t[aá]' OR  lower(a.email) ~* 't[aá]b[ií]t[aá]' OR  text(a.id) ~* 't[aá]b[ií]t[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]r[ií]b[eé]' OR  a.rut ~* '[uú]r[ií]b[eé]' OR  lower(a.email) ~* '[uú]r[ií]b[eé]' OR  text(a.id) ~* '[uú]r[ií]b[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
