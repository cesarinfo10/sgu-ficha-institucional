COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú]l[ií]ss[aá]' OR  a.rut ~* 'j[uú]l[ií]ss[aá]' OR  lower(a.email) ~* 'j[uú]l[ií]ss[aá]' OR  text(a.id) ~* 'j[uú]l[ií]ss[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[ií]ch[eé]r' OR  a.rut ~* 'f[ií]ch[eé]r' OR  lower(a.email) ~* 'f[ií]ch[eé]r' OR  text(a.id) ~* 'f[ií]ch[eé]r' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
