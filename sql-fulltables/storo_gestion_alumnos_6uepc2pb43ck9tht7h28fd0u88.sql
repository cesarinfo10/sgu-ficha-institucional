COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]rr[uú]t[ií][aá]' OR  a.rut ~* '[uú]rr[uú]t[ií][aá]' OR  lower(a.email) ~* '[uú]rr[uú]t[ií][aá]' OR  text(a.id) ~* '[uú]rr[uú]t[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]sq[uú][eé]z' OR  a.rut ~* 'v[aá]sq[uú][eé]z' OR  lower(a.email) ~* 'v[aá]sq[uú][eé]z' OR  text(a.id) ~* 'v[aá]sq[uú][eé]z' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
