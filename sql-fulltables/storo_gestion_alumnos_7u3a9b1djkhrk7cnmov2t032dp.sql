COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nst[aá]nz[aá]' OR  a.rut ~* 'c[oó]nst[aá]nz[aá]' OR  lower(a.email) ~* 'c[oó]nst[aá]nz[aá]' OR  text(a.id) ~* 'c[oó]nst[aá]nz[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'h[oó]ffm[aá]nn' OR  a.rut ~* 'h[oó]ffm[aá]nn' OR  lower(a.email) ~* 'h[oó]ffm[aá]nn' OR  text(a.id) ~* 'h[oó]ffm[aá]nn' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
