COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]m[aá]r[ií]s' OR  a.rut ~* 'd[aá]m[aá]r[ií]s' OR  lower(a.email) ~* 'd[aá]m[aá]r[ií]s' OR  text(a.id) ~* 'd[aá]m[aá]r[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rt[ií]z' OR  a.rut ~* '[oó]rt[ií]z' OR  lower(a.email) ~* '[oó]rt[ií]z' OR  text(a.id) ~* '[oó]rt[ií]z' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
