COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]ry[aá]n' OR  a.rut ~* 'm[aá]ry[aá]n' OR  lower(a.email) ~* 'm[aá]ry[aá]n' OR  text(a.id) ~* 'm[aá]ry[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[ií]d' OR  a.rut ~* 'c[ií]d' OR  lower(a.email) ~* 'c[ií]d' OR  text(a.id) ~* 'c[ií]d' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
