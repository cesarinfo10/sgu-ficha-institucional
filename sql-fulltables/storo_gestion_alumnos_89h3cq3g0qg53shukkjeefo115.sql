COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[eé]n[eé]s[ií]s' OR  a.rut ~* 'g[eé]n[eé]s[ií]s' OR  lower(a.email) ~* 'g[eé]n[eé]s[ií]s' OR  text(a.id) ~* 'g[eé]n[eé]s[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]rc[ií][aá]' OR  a.rut ~* 'g[aá]rc[ií][aá]' OR  lower(a.email) ~* 'g[aá]rc[ií][aá]' OR  text(a.id) ~* 'g[aá]rc[ií][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
