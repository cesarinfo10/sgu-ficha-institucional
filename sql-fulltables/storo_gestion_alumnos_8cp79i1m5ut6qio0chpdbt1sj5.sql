COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]ss[eé]tt' OR  a.rut ~* 'l[ií]ss[eé]tt' OR  lower(a.email) ~* 'l[ií]ss[eé]tt' OR  text(a.id) ~* 'l[ií]ss[eé]tt' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nch[aá]' OR  a.rut ~* 'c[oó]nch[aá]' OR  lower(a.email) ~* 'c[oó]nch[aá]' OR  text(a.id) ~* 'c[oó]nch[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
