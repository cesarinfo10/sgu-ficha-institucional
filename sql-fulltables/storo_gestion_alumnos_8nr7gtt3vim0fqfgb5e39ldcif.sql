COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[ií]n[aá]' OR  a.rut ~* 'k[aá]r[ií]n[aá]' OR  lower(a.email) ~* 'k[aá]r[ií]n[aá]' OR  text(a.id) ~* 'k[aá]r[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[ií]g[uú][ií]r' OR  a.rut ~* '[aá]n[ií]g[uú][ií]r' OR  lower(a.email) ~* '[aá]n[ií]g[uú][ií]r' OR  text(a.id) ~* '[aá]n[ií]g[uú][ií]r' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
