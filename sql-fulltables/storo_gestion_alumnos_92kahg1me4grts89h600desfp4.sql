COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé][uú]g[eé]n[ií][aá]' OR  a.rut ~* '[eé][uú]g[eé]n[ií][aá]' OR  lower(a.email) ~* '[eé][uú]g[eé]n[ií][aá]' OR  text(a.id) ~* '[eé][uú]g[eé]n[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]ñ[eé]z' OR  a.rut ~* 'y[aá]ñ[eé]z' OR  lower(a.email) ~* 'y[aá]ñ[eé]z' OR  text(a.id) ~* 'y[aá]ñ[eé]z' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
