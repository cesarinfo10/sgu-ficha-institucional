COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé]s[aá]r' OR  a.rut ~* 'c[eé]s[aá]r' OR  lower(a.email) ~* 'c[eé]s[aá]r' OR  text(a.id) ~* 'c[eé]s[aá]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]ll[aá]h[uú][eé]ñ[eé]' OR  a.rut ~* '[eé]ll[aá]h[uú][eé]ñ[eé]' OR  lower(a.email) ~* '[eé]ll[aá]h[uú][eé]ñ[eé]' OR  text(a.id) ~* '[eé]ll[aá]h[uú][eé]ñ[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
