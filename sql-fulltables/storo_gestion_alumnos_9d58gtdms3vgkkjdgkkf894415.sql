COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]bl[oó]' OR  a.rut ~* 'p[aá]bl[oó]' OR  lower(a.email) ~* 'p[aá]bl[oó]' OR  text(a.id) ~* 'p[aá]bl[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]ll[oó]' OR  a.rut ~* 'l[ií]ll[oó]' OR  lower(a.email) ~* 'l[ií]ll[oó]' OR  text(a.id) ~* 'l[ií]ll[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
