COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]s[aá]b[eé]l' OR  a.rut ~* '[ií]s[aá]b[eé]l' OR  lower(a.email) ~* '[ií]s[aá]b[eé]l' OR  text(a.id) ~* '[ií]s[aá]b[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]ll[aá]rd[oó]' OR  a.rut ~* 'g[aá]ll[aá]rd[oó]' OR  lower(a.email) ~* 'g[aá]ll[aá]rd[oó]' OR  text(a.id) ~* 'g[aá]ll[aá]rd[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
