COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[aá][uú]l' OR  a.rut ~* 'r[aá][uú]l' OR  lower(a.email) ~* 'r[aá][uú]l' OR  text(a.id) ~* 'r[aá][uú]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]rc[aá]' OR  a.rut ~* 'l[oó]rc[aá]' OR  lower(a.email) ~* 'l[oó]rc[aá]' OR  text(a.id) ~* 'l[oó]rc[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
