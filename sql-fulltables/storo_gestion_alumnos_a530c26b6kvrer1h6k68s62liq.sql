COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[eé]nn[ií]f[eé]r' OR  a.rut ~* 'y[eé]nn[ií]f[eé]r' OR  lower(a.email) ~* 'y[eé]nn[ií]f[eé]r' OR  text(a.id) ~* 'y[eé]nn[ií]f[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[ií]n[eé]d[aá]' OR  a.rut ~* 'p[ií]n[eé]d[aá]' OR  lower(a.email) ~* 'p[ií]n[eé]d[aá]' OR  text(a.id) ~* 'p[ií]n[eé]d[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
