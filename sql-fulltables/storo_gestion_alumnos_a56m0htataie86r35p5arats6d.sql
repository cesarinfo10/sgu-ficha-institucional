COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá]v[ií][eé]r[aá]' OR  a.rut ~* 'j[aá]v[ií][eé]r[aá]' OR  lower(a.email) ~* 'j[aá]v[ií][eé]r[aá]' OR  text(a.id) ~* 'j[aá]v[ií][eé]r[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]y[oó]p[aá]y' OR  a.rut ~* 'c[oó]y[oó]p[aá]y' OR  lower(a.email) ~* 'c[oó]y[oó]p[aá]y' OR  text(a.id) ~* 'c[oó]y[oó]p[aá]y' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
