COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]n[ií][eé]l' OR  a.rut ~* 'd[aá]n[ií][eé]l' OR  lower(a.email) ~* 'd[aá]n[ií][eé]l' OR  text(a.id) ~* 'd[aá]n[ií][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'pr[aá]n[aá][oó]' OR  a.rut ~* 'pr[aá]n[aá][oó]' OR  lower(a.email) ~* 'pr[aá]n[aá][oó]' OR  text(a.id) ~* 'pr[aá]n[aá][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'pr[aá]n[aá][oó]' OR  a.rut ~* 'pr[aá]n[aá][oó]' OR  lower(a.email) ~* 'pr[aá]n[aá][oó]' OR  text(a.id) ~* 'pr[aá]n[aá][oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
