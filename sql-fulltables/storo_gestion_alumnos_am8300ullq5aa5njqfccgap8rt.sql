COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]r[eé]t[oó]' OR  a.rut ~* 'l[oó]r[eé]t[oó]' OR  lower(a.email) ~* 'l[oó]r[eé]t[oó]' OR  text(a.id) ~* 'l[oó]r[eé]t[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]l[eé]br[aá]n' OR  a.rut ~* 'm[aá]l[eé]br[aá]n' OR  lower(a.email) ~* 'm[aá]l[eé]br[aá]n' OR  text(a.id) ~* 'm[aá]l[eé]br[aá]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
