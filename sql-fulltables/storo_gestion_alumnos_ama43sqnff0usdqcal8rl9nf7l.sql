COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]nt[ií]ll[aá]n[oó]' OR  a.rut ~* 'c[aá]nt[ií]ll[aá]n[oó]' OR  lower(a.email) ~* 'c[aá]nt[ií]ll[aá]n[oó]' OR  text(a.id) ~* 'c[aá]nt[ií]ll[aá]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]j[aá]ndr[oó]' OR  a.rut ~* '[aá]l[eé]j[aá]ndr[oó]' OR  lower(a.email) ~* '[aá]l[eé]j[aá]ndr[oó]' OR  text(a.id) ~* '[aá]l[eé]j[aá]ndr[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
