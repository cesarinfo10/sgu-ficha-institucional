COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]l[oó]m[aá]' OR  a.rut ~* 'p[aá]l[oó]m[aá]' OR  lower(a.email) ~* 'p[aá]l[oó]m[aá]' OR  text(a.id) ~* 'p[aá]l[oó]m[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]rn[aá]l[eé]s' OR  a.rut ~* 'b[eé]rn[aá]l[eé]s' OR  lower(a.email) ~* 'b[eé]rn[aá]l[eé]s' OR  text(a.id) ~* 'b[eé]rn[aá]l[eé]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
