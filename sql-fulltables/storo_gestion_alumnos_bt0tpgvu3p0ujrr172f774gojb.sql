COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]v[ií][aá]n[aá]' OR  a.rut ~* 'v[ií]v[ií][aá]n[aá]' OR  lower(a.email) ~* 'v[ií]v[ií][aá]n[aá]' OR  text(a.id) ~* 'v[ií]v[ií][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[uú]r[ií]h[uú][aá]n' OR  a.rut ~* 'c[uú]r[ií]h[uú][aá]n' OR  lower(a.email) ~* 'c[uú]r[ií]h[uú][aá]n' OR  text(a.id) ~* 'c[uú]r[ií]h[uú][aá]n' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
