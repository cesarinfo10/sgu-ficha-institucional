COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]l[ií][aá]n[aá]' OR  a.rut ~* '[eé]l[ií][aá]n[aá]' OR  lower(a.email) ~* '[eé]l[ií][aá]n[aá]' OR  text(a.id) ~* '[eé]l[ií][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]rg[aá]s' OR  a.rut ~* 'v[aá]rg[aá]s' OR  lower(a.email) ~* 'v[aá]rg[aá]s' OR  text(a.id) ~* 'v[aá]rg[aá]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
