COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]br[ií][eé]l[aá]' OR  a.rut ~* 'g[aá]br[ií][eé]l[aá]' OR  lower(a.email) ~* 'g[aá]br[ií][eé]l[aá]' OR  text(a.id) ~* 'g[aá]br[ií][eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]rg[aá]r[aá]' OR  a.rut ~* 'v[eé]rg[aá]r[aá]' OR  lower(a.email) ~* 'v[eé]rg[aá]r[aá]' OR  text(a.id) ~* 'v[eé]rg[aá]r[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
