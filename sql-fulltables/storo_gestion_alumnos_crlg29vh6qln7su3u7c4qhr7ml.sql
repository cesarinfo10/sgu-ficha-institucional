COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]c[ií][oó]' OR  a.rut ~* 'r[oó]c[ií][oó]' OR  lower(a.email) ~* 'r[oó]c[ií][oó]' OR  text(a.id) ~* 'r[oó]c[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rr[ií]l' OR  a.rut ~* 'c[aá]rr[ií]l' OR  lower(a.email) ~* 'c[aá]rr[ií]l' OR  text(a.id) ~* 'c[aá]rr[ií]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
