COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé][aá]nn[eé]tt[eé]' OR  a.rut ~* 'j[eé][aá]nn[eé]tt[eé]' OR  lower(a.email) ~* 'j[eé][aá]nn[eé]tt[eé]' OR  text(a.id) ~* 'j[eé][aá]nn[eé]tt[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]ñ[oó]l[eé]s' OR  a.rut ~* 'n[ií]ñ[oó]l[eé]s' OR  lower(a.email) ~* 'n[ií]ñ[oó]l[eé]s' OR  text(a.id) ~* 'n[ií]ñ[oó]l[eé]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
