COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]m[aá]n' OR  a.rut ~* 'r[oó]m[aá]n' OR  lower(a.email) ~* 'r[oó]m[aá]n' OR  text(a.id) ~* 'r[oó]m[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]v[eé]r[oó]s' OR  a.rut ~* 'r[ií]v[eé]r[oó]s' OR  lower(a.email) ~* 'r[ií]v[eé]r[oó]s' OR  text(a.id) ~* 'r[ií]v[eé]r[oó]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
