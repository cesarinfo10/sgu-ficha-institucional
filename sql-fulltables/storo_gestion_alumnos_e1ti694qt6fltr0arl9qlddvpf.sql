COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]dr[oó]' OR  a.rut ~* 'p[eé]dr[oó]' OR  lower(a.email) ~* 'p[eé]dr[oó]' OR  text(a.id) ~* 'p[eé]dr[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rq[uú][eé]z' OR  a.rut ~* 'm[aá]rq[uú][eé]z' OR  lower(a.email) ~* 'm[aá]rq[uú][eé]z' OR  text(a.id) ~* 'm[aá]rq[uú][eé]z' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
