COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]rg[ií][oó]' OR  a.rut ~* 's[eé]rg[ií][oó]' OR  lower(a.email) ~* 's[eé]rg[ií][oó]' OR  text(a.id) ~* 's[eé]rg[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]sp[ií]nd[oó]l[aá]' OR  a.rut ~* '[eé]sp[ií]nd[oó]l[aá]' OR  lower(a.email) ~* '[eé]sp[ií]nd[oó]l[aá]' OR  text(a.id) ~* '[eé]sp[ií]nd[oó]l[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
