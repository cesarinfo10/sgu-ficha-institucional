COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2024 AND regimen='t' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2024  ,CASE WHEN (SELECT count(id_alumno) FROM vista_alumnos_asistencia WHERE id_alumno=a.id  AND fecha BETWEEN '2023-08-07'::date AND '2023-12-17'::date)>0 THEN round(100*(SELECT count(id_alumno) FROM vista_alumnos_asistencia WHERE id_alumno=a.id  AND fecha BETWEEN '2023-08-07'::date AND '2023-12-17'::date AND presente)::real/(SELECT count(id_alumno) FROM vista_alumnos_asistencia WHERE id_alumno=a.id  AND fecha BETWEEN '2023-08-07'::date AND '2023-12-17'::date)) ELSE 0 END AS tasa_presente 
                FROM alumnos AS a
                ...
                WHERE true  AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2))  AND a.carrera_actual IN (95,100,2,69,1,19) 
