COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'cr[ií]st[ií][aá]n' OR  a.rut ~* 'cr[ií]st[ií][aá]n' OR  lower(a.email) ~* 'cr[ií]st[ií][aá]n' OR  text(a.id) ~* 'cr[ií]st[ií][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]d[ií]ll[aá]' OR  a.rut ~* 'p[aá]d[ií]ll[aá]' OR  lower(a.email) ~* 'p[aá]d[ií]ll[aá]' OR  text(a.id) ~* 'p[aá]d[ií]ll[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
