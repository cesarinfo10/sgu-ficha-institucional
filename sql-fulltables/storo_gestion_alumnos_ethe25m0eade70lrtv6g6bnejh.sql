COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]l[ií]z[aá]b[eé]th' OR  a.rut ~* '[eé]l[ií]z[aá]b[eé]th' OR  lower(a.email) ~* '[eé]l[ií]z[aá]b[eé]th' OR  text(a.id) ~* '[eé]l[ií]z[aá]b[eé]th' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá][uú]r[eé][ií]r[aá]' OR  a.rut ~* 'm[aá][uú]r[eé][ií]r[aá]' OR  lower(a.email) ~* 'm[aá][uú]r[eé][ií]r[aá]' OR  text(a.id) ~* 'm[aá][uú]r[eé][ií]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
