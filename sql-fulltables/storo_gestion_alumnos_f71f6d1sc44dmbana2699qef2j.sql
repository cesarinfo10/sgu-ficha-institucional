COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]rn[eé]st[oó]' OR  a.rut ~* '[eé]rn[eé]st[oó]' OR  lower(a.email) ~* '[eé]rn[eé]st[oó]' OR  text(a.id) ~* '[eé]rn[eé]st[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[oó]r[eé]n[oó]' OR  a.rut ~* 'm[oó]r[eé]n[oó]' OR  lower(a.email) ~* 'm[oó]r[eé]n[oó]' OR  text(a.id) ~* 'm[oó]r[eé]n[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
