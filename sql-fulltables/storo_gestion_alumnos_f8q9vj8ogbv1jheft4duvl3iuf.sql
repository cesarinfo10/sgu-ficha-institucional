COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][uú]l[ií]n[aá]' OR  a.rut ~* 'p[aá][uú]l[ií]n[aá]' OR  lower(a.email) ~* 'p[aá][uú]l[ií]n[aá]' OR  text(a.id) ~* 'p[aá][uú]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rr[eé]ñ[oó]' OR  a.rut ~* 'c[aá]rr[eé]ñ[oó]' OR  lower(a.email) ~* 'c[aá]rr[eé]ñ[oó]' OR  text(a.id) ~* 'c[aá]rr[eé]ñ[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
