COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]' OR  a.rut ~* 'm[aá]r[ií][aá]' OR  lower(a.email) ~* 'm[aá]r[ií][aá]' OR  text(a.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[oó]b[aá]d[ií]ll[aá]' OR  a.rut ~* 'b[oó]b[aá]d[ií]ll[aá]' OR  lower(a.email) ~* 'b[oó]b[aá]d[ií]ll[aá]' OR  text(a.id) ~* 'b[oó]b[aá]d[ií]ll[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
