COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]n[uú][eé]l' OR  a.rut ~* 'm[aá]n[uú][eé]l' OR  lower(a.email) ~* 'm[aá]n[uú][eé]l' OR  text(a.id) ~* 'm[aá]n[uú][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]m[oó]y[aá][oó]' OR  a.rut ~* '[aá]m[oó]y[aá][oó]' OR  lower(a.email) ~* '[aá]m[oó]y[aá][oó]' OR  text(a.id) ~* '[aá]m[oó]y[aá][oó]' )  AND a.carrera_actual IN (95,100,2,19,69,1) 
