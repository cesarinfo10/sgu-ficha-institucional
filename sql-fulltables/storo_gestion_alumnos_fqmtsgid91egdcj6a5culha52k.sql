COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú][aá]n' OR  a.rut ~* 'j[uú][aá]n' OR  lower(a.email) ~* 'j[uú][aá]n' OR  text(a.id) ~* 'j[uú][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]nt[oó]n[ií][oó]' OR  a.rut ~* '[aá]nt[oó]n[ií][oó]' OR  lower(a.email) ~* '[aá]nt[oó]n[ií][oó]' OR  text(a.id) ~* '[aá]nt[oó]n[ií][oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]g[uú]r[aá]' OR  a.rut ~* 's[eé]g[uú]r[aá]' OR  lower(a.email) ~* 's[eé]g[uú]r[aá]' OR  text(a.id) ~* 's[eé]g[uú]r[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
