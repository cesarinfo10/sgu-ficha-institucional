COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[uú][eé]nch[uú]m[aá]n' OR  a.rut ~* 'h[uú][eé]nch[uú]m[aá]n' OR  lower(a.email) ~* 'h[uú][eé]nch[uú]m[aá]n' OR  text(a.id) ~* 'h[uú][eé]nch[uú]m[aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]nc[oó]p[aá]n' OR  a.rut ~* 'l[oó]nc[oó]p[aá]n' OR  lower(a.email) ~* 'l[oó]nc[oó]p[aá]n' OR  text(a.id) ~* 'l[oó]nc[oó]p[aá]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
