COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé]t[ií]c[ií][aá]' OR  a.rut ~* 'l[eé]t[ií]c[ií][aá]' OR  lower(a.email) ~* 'l[eé]t[ií]c[ií][aá]' OR  text(a.id) ~* 'l[eé]t[ií]c[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]y[aá]' OR  a.rut ~* '[aá]r[aá]y[aá]' OR  lower(a.email) ~* '[aá]r[aá]y[aá]' OR  text(a.id) ~* '[aá]r[aá]y[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
