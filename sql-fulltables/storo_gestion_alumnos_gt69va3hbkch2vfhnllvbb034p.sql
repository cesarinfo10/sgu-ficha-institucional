COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]sm[aá]d' OR  a.rut ~* '[aá]sm[aá]d' OR  lower(a.email) ~* '[aá]sm[aá]d' OR  text(a.id) ~* '[aá]sm[aá]d' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
