COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  a.rut ~* 'r[oó]dr[ií]g[oó]' OR  lower(a.email) ~* 'r[oó]dr[ií]g[oó]' OR  text(a.id) ~* 'r[oó]dr[ií]g[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  a.rut ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  lower(a.email) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' OR  text(a.id) ~* 'm[aá]d[aá]r[ií][aá]g[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
