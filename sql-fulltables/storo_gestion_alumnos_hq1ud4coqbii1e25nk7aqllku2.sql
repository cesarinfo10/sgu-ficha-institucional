COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]m[ií]l[aá]' OR  a.rut ~* 'c[aá]m[ií]l[aá]' OR  lower(a.email) ~* 'c[aá]m[ií]l[aá]' OR  text(a.id) ~* 'c[aá]m[ií]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rr[eé]g[oó]' OR  a.rut ~* '[oó]rr[eé]g[oó]' OR  lower(a.email) ~* '[oó]rr[eé]g[oó]' OR  text(a.id) ~* '[oó]rr[eé]g[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
