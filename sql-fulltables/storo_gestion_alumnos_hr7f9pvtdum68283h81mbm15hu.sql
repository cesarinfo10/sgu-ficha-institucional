COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]r[ií]m[aá]' OR  a.rut ~* 'k[aá]r[ií]m[aá]' OR  lower(a.email) ~* 'k[aá]r[ií]m[aá]' OR  text(a.id) ~* 'k[aá]r[ií]m[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]f[ií]' OR  a.rut ~* 's[aá]f[ií]' OR  lower(a.email) ~* 's[aá]f[ií]' OR  text(a.id) ~* 's[aá]f[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
