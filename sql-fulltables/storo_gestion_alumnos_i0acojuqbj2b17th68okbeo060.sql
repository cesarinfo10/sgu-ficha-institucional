COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]g[uú][eé]l' OR  a.rut ~* 'm[ií]g[uú][eé]l' OR  lower(a.email) ~* 'm[ií]g[uú][eé]l' OR  text(a.id) ~* 'm[ií]g[uú][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]t[ií][eé]rr[eé]z' OR  a.rut ~* 'g[uú]t[ií][eé]rr[eé]z' OR  lower(a.email) ~* 'g[uú]t[ií][eé]rr[eé]z' OR  text(a.id) ~* 'g[uú]t[ií][eé]rr[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[uú][ií]z' OR  a.rut ~* 'r[uú][ií]z' OR  lower(a.email) ~* 'r[uú][ií]z' OR  text(a.id) ~* 'r[uú][ií]z' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
