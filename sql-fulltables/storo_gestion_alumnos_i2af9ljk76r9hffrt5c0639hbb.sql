COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá]gd[aá]l[ií][eé]' OR  a.rut ~* 'j[aá]gd[aá]l[ií][eé]' OR  lower(a.email) ~* 'j[aá]gd[aá]l[ií][eé]' OR  text(a.id) ~* 'j[aá]gd[aá]l[ií][eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR  a.rut ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR  lower(a.email) ~* 'f[uú][eé]nt[eé][aá]lb[aá]' OR  text(a.id) ~* 'f[uú][eé]nt[eé][aá]lb[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
