COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]lv[aá]d[oó]r' OR  a.rut ~* 's[aá]lv[aá]d[oó]r' OR  lower(a.email) ~* 's[aá]lv[aá]d[oó]r' OR  text(a.id) ~* 's[aá]lv[aá]d[oó]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]p[eé]z' OR  a.rut ~* 'l[oó]p[eé]z' OR  lower(a.email) ~* 'l[oó]p[eé]z' OR  text(a.id) ~* 'l[oó]p[eé]z' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
