COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]l[ií][aá]s' OR  a.rut ~* '[eé]l[ií][aá]s' OR  lower(a.email) ~* '[eé]l[ií][aá]s' OR  text(a.id) ~* '[eé]l[ií][aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]d[aá]l' OR  a.rut ~* 'v[ií]d[aá]l' OR  lower(a.email) ~* 'v[ií]d[aá]l' OR  text(a.id) ~* 'v[ií]d[aá]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
