COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]g[uú]nd[oó]' OR  a.rut ~* 's[eé]g[uú]nd[oó]' OR  lower(a.email) ~* 's[eé]g[uú]nd[oó]' OR  text(a.id) ~* 's[eé]g[uú]nd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]t[oó]' OR  a.rut ~* 's[oó]t[oó]' OR  lower(a.email) ~* 's[oó]t[oó]' OR  text(a.id) ~* 's[oó]t[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
