COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]ld[aá]' OR  a.rut ~* 'n[ií]ld[aá]' OR  lower(a.email) ~* 'n[ií]ld[aá]' OR  text(a.id) ~* 'n[ií]ld[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rt[aá]g[eé]n[aá]' OR  a.rut ~* 'c[aá]rt[aá]g[eé]n[aá]' OR  lower(a.email) ~* 'c[aá]rt[aá]g[eé]n[aá]' OR  text(a.id) ~* 'c[aá]rt[aá]g[eé]n[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
