COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]n[aá]t[oó]' OR  a.rut ~* 'r[eé]n[aá]t[oó]' OR  lower(a.email) ~* 'r[eé]n[aá]t[oó]' OR  text(a.id) ~* 'r[eé]n[aá]t[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]d[aá]l[ií]n[ií]' OR  a.rut ~* 'n[aá]d[aá]l[ií]n[ií]' OR  lower(a.email) ~* 'n[aá]d[aá]l[ií]n[ií]' OR  text(a.id) ~* 'n[aá]d[aá]l[ií]n[ií]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
