COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]x[ií]s' OR  a.rut ~* '[aá]l[eé]x[ií]s' OR  lower(a.email) ~* '[aá]l[eé]x[ií]s' OR  text(a.id) ~* '[aá]l[eé]x[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[uú]ñ[oó]z' OR  a.rut ~* 'm[uú]ñ[oó]z' OR  lower(a.email) ~* 'm[uú]ñ[oó]z' OR  text(a.id) ~* 'm[uú]ñ[oó]z' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
