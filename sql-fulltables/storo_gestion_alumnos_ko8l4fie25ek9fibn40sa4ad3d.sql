COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]t[aá]l[oó]' OR  a.rut ~* '[ií]t[aá]l[oó]' OR  lower(a.email) ~* '[ií]t[aá]l[oó]' OR  text(a.id) ~* '[ií]t[aá]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[oó]' OR  a.rut ~* 'c[aá]r[oó]' OR  lower(a.email) ~* 'c[aá]r[oó]' OR  text(a.id) ~* 'c[aá]r[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
