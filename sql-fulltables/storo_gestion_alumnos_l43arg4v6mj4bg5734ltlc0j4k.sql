COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]j[aá]s' OR  a.rut ~* 'r[oó]j[aá]s' OR  lower(a.email) ~* 'r[oó]j[aá]s' OR  text(a.id) ~* 'r[oó]j[aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú][eé]rr[aá]' OR  a.rut ~* 'g[uú][eé]rr[aá]' OR  lower(a.email) ~* 'g[uú][eé]rr[aá]' OR  text(a.id) ~* 'g[uú][eé]rr[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
