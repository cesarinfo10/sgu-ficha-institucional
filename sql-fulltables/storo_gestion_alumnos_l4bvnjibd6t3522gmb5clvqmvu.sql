COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]n[ií]x[aá]' OR  a.rut ~* 'd[aá]n[ií]x[aá]' OR  lower(a.email) ~* 'd[aá]n[ií]x[aá]' OR  text(a.id) ~* 'd[aá]n[ií]x[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[uú][aá]rt[eé]' OR  a.rut ~* 'd[uú][aá]rt[eé]' OR  lower(a.email) ~* 'd[uú][aá]rt[eé]' OR  text(a.id) ~* 'd[uú][aá]rt[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
