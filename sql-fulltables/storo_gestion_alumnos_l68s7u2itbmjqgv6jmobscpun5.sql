COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]d[uú][aá]rd[oó]' OR  a.rut ~* '[eé]d[uú][aá]rd[oó]' OR  lower(a.email) ~* '[eé]d[uú][aá]rd[oó]' OR  text(a.id) ~* '[eé]d[uú][aá]rd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]z[uú]' OR  a.rut ~* '[aá]z[uú]' OR  lower(a.email) ~* '[aá]z[uú]' OR  text(a.id) ~* '[aá]z[uú]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
