COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[oó]l[aá]ng[eé]' OR  a.rut ~* 's[oó]l[aá]ng[eé]' OR  lower(a.email) ~* 's[oó]l[aá]ng[eé]' OR  text(a.id) ~* 's[oó]l[aá]ng[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]v[eé]r[aá]' OR  a.rut ~* 'r[ií]v[eé]r[aá]' OR  lower(a.email) ~* 'r[ií]v[eé]r[aá]' OR  text(a.id) ~* 'r[ií]v[eé]r[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
