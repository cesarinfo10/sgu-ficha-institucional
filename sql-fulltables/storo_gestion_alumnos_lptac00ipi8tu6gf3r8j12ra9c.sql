COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ct[oó]r[ií][aá]' OR  a.rut ~* 'v[ií]ct[oó]r[ií][aá]' OR  lower(a.email) ~* 'v[ií]ct[oó]r[ií][aá]' OR  text(a.id) ~* 'v[ií]ct[oó]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]br[eé]r[aá]' OR  a.rut ~* 'c[aá]br[eé]r[aá]' OR  lower(a.email) ~* 'c[aá]br[eé]r[aá]' OR  text(a.id) ~* 'c[aá]br[eé]r[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
