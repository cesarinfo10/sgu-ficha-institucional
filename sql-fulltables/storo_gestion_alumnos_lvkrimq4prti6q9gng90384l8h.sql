COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'k[aá]rl[aá]' OR  a.rut ~* 'k[aá]rl[aá]' OR  lower(a.email) ~* 'k[aá]rl[aá]' OR  text(a.id) ~* 'k[aá]rl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]t[ií][eé]rr[eé]z' OR  a.rut ~* 'g[uú]t[ií][eé]rr[eé]z' OR  lower(a.email) ~* 'g[uú]t[ií][eé]rr[eé]z' OR  text(a.id) ~* 'g[uú]t[ií][eé]rr[eé]z' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
