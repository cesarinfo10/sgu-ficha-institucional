COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú][ií]ll[eé]rm[oó]' OR  a.rut ~* 'g[uú][ií]ll[eé]rm[oó]' OR  lower(a.email) ~* 'g[uú][ií]ll[eé]rm[oó]' OR  text(a.id) ~* 'g[uú][ií]ll[eé]rm[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá]nd[oó]v[aá]l' OR  a.rut ~* 's[aá]nd[oó]v[aá]l' OR  lower(a.email) ~* 's[aá]nd[oó]v[aá]l' OR  text(a.id) ~* 's[aá]nd[oó]v[aá]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
