COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'w[aá]lt[eé]r' OR  a.rut ~* 'w[aá]lt[eé]r' OR  lower(a.email) ~* 'w[aá]lt[eé]r' OR  text(a.id) ~* 'w[aá]lt[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[uú][aá]z[oó]' OR  a.rut ~* 's[uú][aá]z[oó]' OR  lower(a.email) ~* 's[uú][aá]z[oó]' OR  text(a.id) ~* 's[uú][aá]z[oó]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
