COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]l[oó]ns[oó]' OR  a.rut ~* '[aá]l[oó]ns[oó]' OR  lower(a.email) ~* '[aá]l[oó]ns[oó]' OR  text(a.id) ~* '[aá]l[oó]ns[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rr[eé][aá]' OR  a.rut ~* 'c[oó]rr[eé][aá]' OR  lower(a.email) ~* 'c[oó]rr[eé][aá]' OR  text(a.id) ~* 'c[oó]rr[eé][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
