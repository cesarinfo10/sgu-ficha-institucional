COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]n[eé]' OR  a.rut ~* 'r[eé]n[eé]' OR  lower(a.email) ~* 'r[eé]n[eé]' OR  text(a.id) ~* 'r[eé]n[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[uú]rr[uú]t[ií][aá]' OR  a.rut ~* '[uú]rr[uú]t[ií][aá]' OR  lower(a.email) ~* '[uú]rr[uú]t[ií][aá]' OR  text(a.id) ~* '[uú]rr[uú]t[ií][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
