COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[uú]dy' OR  a.rut ~* 'r[uú]dy' OR  lower(a.email) ~* 'r[uú]dy' OR  text(a.id) ~* 'r[uú]dy' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]p[ií][aá]' OR  a.rut ~* 'j[oó]p[ií][aá]' OR  lower(a.email) ~* 'j[oó]p[ií][aá]' OR  text(a.id) ~* 'j[oó]p[ií][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
