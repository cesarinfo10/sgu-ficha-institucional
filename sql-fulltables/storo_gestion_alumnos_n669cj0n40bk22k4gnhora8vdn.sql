COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]r[aá]ng[uú][ií]z' OR  a.rut ~* '[aá]r[aá]ng[uú][ií]z' OR  lower(a.email) ~* '[aá]r[aá]ng[uú][ií]z' OR  text(a.id) ~* '[aá]r[aá]ng[uú][ií]z' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
