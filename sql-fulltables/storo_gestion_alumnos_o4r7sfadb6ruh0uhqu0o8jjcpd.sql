COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[oó]s[eé]' OR  a.rut ~* 'j[oó]s[eé]' OR  lower(a.email) ~* 'j[oó]s[eé]' OR  text(a.id) ~* 'j[oó]s[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[eé]ltr[aá]n' OR  a.rut ~* 'b[eé]ltr[aá]n' OR  lower(a.email) ~* 'b[eé]ltr[aá]n' OR  text(a.id) ~* 'b[eé]ltr[aá]n' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
