COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[oó][eé]m[ií]' OR  a.rut ~* 'n[oó][eé]m[ií]' OR  lower(a.email) ~* 'n[oó][eé]m[ií]' OR  text(a.id) ~* 'n[oó][eé]m[ií]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 't[aá]p[ií][aá]' OR  a.rut ~* 't[aá]p[ií][aá]' OR  lower(a.email) ~* 't[aá]p[ií][aá]' OR  text(a.id) ~* 't[aá]p[ií][aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
