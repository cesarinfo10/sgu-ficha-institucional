COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[oó]nz[aá]l[oó]' OR  a.rut ~* 'g[oó]nz[aá]l[oó]' OR  lower(a.email) ~* 'g[oó]nz[aá]l[oó]' OR  text(a.id) ~* 'g[oó]nz[aá]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]st[oó]n' OR  a.rut ~* 'g[aá]st[oó]n' OR  lower(a.email) ~* 'g[aá]st[oó]n' OR  text(a.id) ~* 'g[aá]st[oó]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
