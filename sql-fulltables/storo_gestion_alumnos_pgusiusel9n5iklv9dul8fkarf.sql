COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]nt[oó]n[ií][eé]t[aá]' OR  a.rut ~* '[aá]nt[oó]n[ií][eé]t[aá]' OR  lower(a.email) ~* '[aá]nt[oó]n[ií][eé]t[aá]' OR  text(a.id) ~* '[aá]nt[oó]n[ií][eé]t[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]lv[aá]' OR  a.rut ~* 's[ií]lv[aá]' OR  lower(a.email) ~* 's[ií]lv[aá]' OR  text(a.id) ~* 's[ií]lv[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
