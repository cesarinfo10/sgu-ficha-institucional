COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]lv[ií][aá]' OR  a.rut ~* 's[ií]lv[ií][aá]' OR  lower(a.email) ~* 's[ií]lv[ií][aá]' OR  text(a.id) ~* 's[ií]lv[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[aá]r[aá]' OR  a.rut ~* 'l[aá]r[aá]' OR  lower(a.email) ~* 'l[aá]r[aá]' OR  text(a.id) ~* 'l[aá]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
