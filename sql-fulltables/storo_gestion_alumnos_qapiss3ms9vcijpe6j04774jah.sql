COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'chr[ií]st[ií][aá]n' OR  a.rut ~* 'chr[ií]st[ií][aá]n' OR  lower(a.email) ~* 'chr[ií]st[ií][aá]n' OR  text(a.id) ~* 'chr[ií]st[ií][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[oó]b[oó]s' OR  a.rut ~* 'l[oó]b[oó]s' OR  lower(a.email) ~* 'l[oó]b[oó]s' OR  text(a.id) ~* 'l[oó]b[oó]s' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
