COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'x[ií]m[eé]n[aá]' OR  a.rut ~* 'x[ií]m[eé]n[aá]' OR  lower(a.email) ~* 'x[ií]m[eé]n[aá]' OR  text(a.id) ~* 'x[ií]m[eé]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]j[eé]d[aá]' OR  a.rut ~* '[oó]j[eé]d[aá]' OR  lower(a.email) ~* '[oó]j[eé]d[aá]' OR  text(a.id) ~* '[oó]j[eé]d[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
