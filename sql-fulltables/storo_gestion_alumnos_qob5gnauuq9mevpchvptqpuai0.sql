COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]b[eé]l' OR  a.rut ~* 'm[aá]b[eé]l' OR  lower(a.email) ~* 'm[aá]b[eé]l' OR  text(a.id) ~* 'm[aá]b[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]m[eé]st[ií]c[aá]' OR  a.rut ~* '[aá]m[eé]st[ií]c[aá]' OR  lower(a.email) ~* '[aá]m[eé]st[ií]c[aá]' OR  text(a.id) ~* '[aá]m[eé]st[ií]c[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
