COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]l[ií]z[aá]b[eé]th' OR  a.rut ~* '[eé]l[ií]z[aá]b[eé]th' OR  lower(a.email) ~* '[eé]l[ií]z[aá]b[eé]th' OR  text(a.id) ~* '[eé]l[ií]z[aá]b[eé]th' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé]yt[oó]n' OR  a.rut ~* 'l[eé]yt[oó]n' OR  lower(a.email) ~* 'l[eé]yt[oó]n' OR  text(a.id) ~* 'l[eé]yt[oó]n' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
