COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]lv[aá]n[aá]' OR  a.rut ~* 's[ií]lv[aá]n[aá]' OR  lower(a.email) ~* 's[ií]lv[aá]n[aá]' OR  text(a.id) ~* 's[ií]lv[aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]br[eé]r[aá]' OR  a.rut ~* 'c[aá]br[eé]r[aá]' OR  lower(a.email) ~* 'c[aá]br[eé]r[aá]' OR  text(a.id) ~* 'c[aá]br[eé]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
