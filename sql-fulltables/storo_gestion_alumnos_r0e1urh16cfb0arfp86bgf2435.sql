COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]g[uú]st[ií]n' OR  a.rut ~* '[aá]g[uú]st[ií]n' OR  lower(a.email) ~* '[aá]g[uú]st[ií]n' OR  text(a.id) ~* '[aá]g[uú]st[ií]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[oó]t[ií]' OR  a.rut ~* 'c[aá]r[oó]t[ií]' OR  lower(a.email) ~* 'c[aá]r[oó]t[ií]' OR  text(a.id) ~* 'c[aá]r[oó]t[ií]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
