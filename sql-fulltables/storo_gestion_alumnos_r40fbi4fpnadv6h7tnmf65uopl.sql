COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]st[eé]f[aá]n[ií][aá]' OR  a.rut ~* '[eé]st[eé]f[aá]n[ií][aá]' OR  lower(a.email) ~* '[eé]st[eé]f[aá]n[ií][aá]' OR  text(a.id) ~* '[eé]st[eé]f[aá]n[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]r[aá]' OR  a.rut ~* 'v[eé]r[aá]' OR  lower(a.email) ~* 'v[eé]r[aá]' OR  text(a.id) ~* 'v[eé]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
