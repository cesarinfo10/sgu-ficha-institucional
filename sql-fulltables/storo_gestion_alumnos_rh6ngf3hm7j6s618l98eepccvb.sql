COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[ií][aá]z' OR  a.rut ~* 'd[ií][aá]z' OR  lower(a.email) ~* 'd[ií][aá]z' OR  text(a.id) ~* 'd[ií][aá]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]yz[aá]g[uú][ií]rr[eé]' OR  a.rut ~* '[eé]yz[aá]g[uú][ií]rr[eé]' OR  lower(a.email) ~* '[eé]yz[aá]g[uú][ií]rr[eé]' OR  text(a.id) ~* '[eé]yz[aá]g[uú][ií]rr[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
