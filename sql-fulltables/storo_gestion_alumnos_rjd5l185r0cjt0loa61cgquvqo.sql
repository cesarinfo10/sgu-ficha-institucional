COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[aá]nc[oó]' OR  a.rut ~* 'fr[aá]nc[oó]' OR  lower(a.email) ~* 'fr[aá]nc[oó]' OR  text(a.id) ~* 'fr[aá]nc[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[eé]rm[aá]' OR  a.rut ~* 'f[eé]rm[aá]' OR  lower(a.email) ~* 'f[eé]rm[aá]' OR  text(a.id) ~* 'f[eé]rm[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
