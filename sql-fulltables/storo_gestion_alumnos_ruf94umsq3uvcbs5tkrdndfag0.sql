COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  a.rut ~* 'r[oó]dr[ií]g[oó]' OR  lower(a.email) ~* 'r[oó]dr[ií]g[oó]' OR  text(a.id) ~* 'r[oó]dr[ií]g[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rd[eé]n[aá]s' OR  a.rut ~* 'c[aá]rd[eé]n[aá]s' OR  lower(a.email) ~* 'c[aá]rd[eé]n[aá]s' OR  text(a.id) ~* 'c[aá]rd[eé]n[aá]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
