COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[eé]s[uú]s' OR  a.rut ~* 'j[eé]s[uú]s' OR  lower(a.email) ~* 'j[eé]s[uú]s' OR  text(a.id) ~* 'j[eé]s[uú]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]t[aá]m[aá]l' OR  a.rut ~* 'r[eé]t[aá]m[aá]l' OR  lower(a.email) ~* 'r[eé]t[aá]m[aá]l' OR  text(a.id) ~* 'r[eé]t[aá]m[aá]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
