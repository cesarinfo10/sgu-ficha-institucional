COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]c[ií][aá]' OR  a.rut ~* 'l[uú]c[ií][aá]' OR  lower(a.email) ~* 'l[uú]c[ií][aá]' OR  text(a.id) ~* 'l[uú]c[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'tr[aá]n[aá]m[ií]l' OR  a.rut ~* 'tr[aá]n[aá]m[ií]l' OR  lower(a.email) ~* 'tr[aá]n[aá]m[ií]l' OR  text(a.id) ~* 'tr[aá]n[aá]m[ií]l' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
