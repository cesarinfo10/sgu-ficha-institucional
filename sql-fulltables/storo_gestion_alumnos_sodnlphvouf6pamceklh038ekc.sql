COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]x[ií]m[oó]' OR  a.rut ~* 'm[aá]x[ií]m[oó]' OR  lower(a.email) ~* 'm[aá]x[ií]m[oó]' OR  text(a.id) ~* 'm[aá]x[ií]m[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]y[eé]s' OR  a.rut ~* 'r[eé]y[eé]s' OR  lower(a.email) ~* 'r[eé]y[eé]s' OR  text(a.id) ~* 'r[eé]y[eé]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
