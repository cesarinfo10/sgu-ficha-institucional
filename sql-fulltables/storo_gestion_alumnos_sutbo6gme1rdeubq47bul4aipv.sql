COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]l[ií]n[aá]' OR  a.rut ~* 'm[eé]l[ií]n[aá]' OR  lower(a.email) ~* 'm[eé]l[ií]n[aá]' OR  text(a.id) ~* 'm[eé]l[ií]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]nt[ií]ll[aá]n[aá]' OR  a.rut ~* 'c[aá]nt[ií]ll[aá]n[aá]' OR  lower(a.email) ~* 'c[aá]nt[ií]ll[aá]n[aá]' OR  text(a.id) ~* 'c[aá]nt[ií]ll[aá]n[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
