COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]r[eé]n[eé]' OR  a.rut ~* '[ií]r[eé]n[eé]' OR  lower(a.email) ~* '[ií]r[eé]n[eé]' OR  text(a.id) ~* '[ií]r[eé]n[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]l[aá]z' OR  a.rut ~* 'g[aá]l[aá]z' OR  lower(a.email) ~* 'g[aá]l[aá]z' OR  text(a.id) ~* 'g[aá]l[aá]z' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
