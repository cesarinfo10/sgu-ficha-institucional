COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]g[eé]r' OR  a.rut ~* 'r[oó]g[eé]r' OR  lower(a.email) ~* 'r[oó]g[eé]r' OR  text(a.id) ~* 'r[oó]g[eé]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú][eé]b[aá]r[aá]' OR  a.rut ~* 'g[uú][eé]b[aá]r[aá]' OR  lower(a.email) ~* 'g[uú][eé]b[aá]r[aá]' OR  text(a.id) ~* 'g[uú][eé]b[aá]r[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
