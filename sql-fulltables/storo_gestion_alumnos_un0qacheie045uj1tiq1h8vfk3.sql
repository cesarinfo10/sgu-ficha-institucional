COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[ií]ch[eé]l' OR  a.rut ~* 'm[ií]ch[eé]l' OR  lower(a.email) ~* 'm[ií]ch[eé]l' OR  text(a.id) ~* 'm[ií]ch[eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][ií]nt[eé]r[oó]s' OR  a.rut ~* 'q[uú][ií]nt[eé]r[oó]s' OR  lower(a.email) ~* 'q[uú][ií]nt[eé]r[oó]s' OR  text(a.id) ~* 'q[uú][ií]nt[eé]r[oó]s' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
