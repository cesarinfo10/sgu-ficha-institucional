COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[aá]f[aá][eé]l' OR  a.rut ~* 'r[aá]f[aá][eé]l' OR  lower(a.email) ~* 'r[aá]f[aá][eé]l' OR  text(a.id) ~* 'r[aá]f[aá][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá][eé]rt[eé]n' OR  a.rut ~* 'm[aá][eé]rt[eé]n' OR  lower(a.email) ~* 'm[aá][eé]rt[eé]n' OR  text(a.id) ~* 'm[aá][eé]rt[eé]n' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
