COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rc[eé]l[oó]' OR  a.rut ~* 'm[aá]rc[eé]l[oó]' OR  lower(a.email) ~* 'm[aá]rc[eé]l[oó]' OR  text(a.id) ~* 'm[aá]rc[eé]l[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rr[aá]sc[oó]' OR  a.rut ~* 'c[aá]rr[aá]sc[oó]' OR  lower(a.email) ~* 'c[aá]rr[aá]sc[oó]' OR  text(a.id) ~* 'c[aá]rr[aá]sc[oó]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
