COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá][oó]l[aá]' OR  a.rut ~* 'p[aá][oó]l[aá]' OR  lower(a.email) ~* 'p[aá][oó]l[aá]' OR  text(a.id) ~* 'p[aá][oó]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[aá]y[aá]' OR  a.rut ~* '[aá]n[aá]y[aá]' OR  lower(a.email) ~* '[aá]n[aá]y[aá]' OR  text(a.id) ~* '[aá]n[aá]y[aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
