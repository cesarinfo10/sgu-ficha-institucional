COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]n[ií]s' OR  a.rut ~* 'y[aá]n[ií]s' OR  lower(a.email) ~* 'y[aá]n[ií]s' OR  text(a.id) ~* 'y[aá]n[ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[eé]b[oó]ll[eé]d[oó]' OR  a.rut ~* 'r[eé]b[oó]ll[eé]d[oó]' OR  lower(a.email) ~* 'r[eé]b[oó]ll[eé]d[oó]' OR  text(a.id) ~* 'r[eé]b[oó]ll[eé]d[oó]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
