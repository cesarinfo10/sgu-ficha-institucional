COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[ií][aá]n' OR  a.rut ~* 'g[ií][aá]n' OR  lower(a.email) ~* 'g[ií][aá]n' OR  text(a.id) ~* 'g[ií][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[oó][oó]l' OR  a.rut ~* 'p[oó][oó]l' OR  lower(a.email) ~* 'p[oó][oó]l' OR  text(a.id) ~* 'p[oó][oó]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[aá]rc[ií][aá]' OR  a.rut ~* 'g[aá]rc[ií][aá]' OR  lower(a.email) ~* 'g[aá]rc[ií][aá]' OR  text(a.id) ~* 'g[aá]rc[ií][aá]' )  AND a.carrera_actual IN (95,100,19,2,69,1) 
