COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]' OR  a.rut ~* 'm[aá]r[ií][aá]' OR  lower(a.email) ~* 'm[aá]r[ií][aá]' OR  text(a.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]x[aá]n[aá]' OR  a.rut ~* 'r[oó]x[aá]n[aá]' OR  lower(a.email) ~* 'r[oó]x[aá]n[aá]' OR  text(a.id) ~* 'r[oó]x[aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  a.rut ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  lower(a.email) ~* 'n[aá]v[aá]rr[eé]t[eé]' OR  text(a.id) ~* 'n[aá]v[aá]rr[eé]t[eé]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
