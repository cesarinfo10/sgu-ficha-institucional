COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'myr[ií][aá]m' OR  a.rut ~* 'myr[ií][aá]m' OR  text(ac.folio) ~* 'myr[ií][aá]m' OR  vac.cod ~* 'myr[ií][aá]m' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rt[eé]g[aá]' OR  a.rut ~* '[oó]rt[eé]g[aá]' OR  text(ac.folio) ~* '[oó]rt[eé]g[aá]' OR  vac.cod ~* '[oó]rt[eé]g[aá]' ) 
