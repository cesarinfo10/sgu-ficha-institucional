COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]n[uú][eé]l' OR  a.rut ~* 'm[aá]n[uú][eé]l' OR  text(ac.folio) ~* 'm[aá]n[uú][eé]l' OR  vac.cod ~* 'm[aá]n[uú][eé]l' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'z[uú]ñ[ií]g[aá]' OR  a.rut ~* 'z[uú]ñ[ií]g[aá]' OR  text(ac.folio) ~* 'z[uú]ñ[ií]g[aá]' OR  vac.cod ~* 'z[uú]ñ[ií]g[aá]' ) 
