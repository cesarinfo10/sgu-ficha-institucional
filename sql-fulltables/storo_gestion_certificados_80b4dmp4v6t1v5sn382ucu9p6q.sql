COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'l[uú][ií]s' OR  a.rut ~* 'l[uú][ií]s' OR  text(ac.folio) ~* 'l[uú][ií]s' OR  vac.cod ~* 'l[uú][ií]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  a.rut ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  text(ac.folio) ~* 'f[ií]g[uú][eé]r[oó][aá]' OR  vac.cod ~* 'f[ií]g[uú][eé]r[oó][aá]' ) 
