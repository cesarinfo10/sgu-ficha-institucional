COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]rl[aá]' OR  a.rut ~* 'p[eé]rl[aá]' OR  text(ac.folio) ~* 'p[eé]rl[aá]' OR  vac.cod ~* 'p[eé]rl[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'b[uú]rg[oó]s' OR  a.rut ~* 'b[uú]rg[oó]s' OR  text(ac.folio) ~* 'b[uú]rg[oó]s' OR  vac.cod ~* 'b[uú]rg[oó]s' ) 
