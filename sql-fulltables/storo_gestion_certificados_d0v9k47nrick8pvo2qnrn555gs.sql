COPY (SELECT ac.folio,vac.cod,trim(a.rut) AS rut,va.nombre AS alumno,trim(c.alias) AS carrera,a.jornada,r.nombre AS regimen,va.estado AS estado_alumno,cert.nombre AS docto,
                  ...
                  WHERE (lower(a.nombres||' '||a.apellidos) ~* '[oó]sv[aá]ld[oó]' OR  a.rut ~* '[oó]sv[aá]ld[oó]' OR  text(ac.folio) ~* '[oó]sv[aá]ld[oó]' OR  vac.cod ~* '[oó]sv[aá]ld[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'd[ií][aá]z' OR  a.rut ~* 'd[ií][aá]z' OR  text(ac.folio) ~* 'd[ií][aá]z' OR  vac.cod ~* 'd[ií][aá]z' ) 
