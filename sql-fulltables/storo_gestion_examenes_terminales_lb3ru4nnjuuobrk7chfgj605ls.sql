COPY (SELECT et.id,to_char(fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
				  ...
				  WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]c[aá]rd[oó]' OR  a.rut ~* 'r[ií]c[aá]rd[oó]' OR  text(a.id) ~* 'r[ií]c[aá]rd[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rq[uú][eé]z' OR  a.rut ~* 'm[aá]rq[uú][eé]z' OR  text(a.id) ~* 'm[aá]rq[uú][eé]z' )  			  
