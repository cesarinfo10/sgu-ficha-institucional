COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE (SELECT count(id_alumno) FROM inscripciones_cursos WHERE id_alumno=a.id)=0   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]l[eé]n[aá]' OR  a.rut ~* '[eé]l[eé]n[aá]' OR  lower(a.email) ~* '[eé]l[eé]n[aá]' OR  text(a.id) ~* '[eé]l[eé]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[ií]b[eé]r[oó]n[aá]' OR  a.rut ~* 'l[ií]b[eé]r[oó]n[aá]' OR  lower(a.email) ~* 'l[ií]b[eé]r[oó]n[aá]' OR  text(a.id) ~* 'l[ií]b[eé]r[oó]n[aá]' )  AND a.carrera_actual IN (95,100,2,69,1,19) 
