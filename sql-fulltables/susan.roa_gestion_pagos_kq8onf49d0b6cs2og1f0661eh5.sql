COPY (SELECT * FROM (SELECT DISTINCT ON (coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura)) p.id,coalesce(p.nro_boleta,p.nro_boleta_e,p.nro_factura) AS nro_docto,
                ...
                WHERE (nro_boleta IS NOT NULL OR nro_boleta_e IS NOT NULL) AND (lower(pap.nombres||' '||pap.apellidos) ~* '19487454' OR  pap.rut ~* '19487454' OR lower(a.nombres||' '||a.apellidos) ~* '19487454' OR lower(a3.nombres||' '||a3.apellidos) ~* '19487454' OR  a.rut ~* '19487454' OR a2.rut ~* '19487454' OR a3.rut ~* '19487454' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR lower(a.nombres||' '||a.apellidos) ~* '' OR lower(a3.nombres||' '||a3.apellidos) ~* '' OR  a.rut ~* '' OR a2.rut ~* '' OR a3.rut ~* '' )  
