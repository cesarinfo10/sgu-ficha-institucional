COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'r[oó]ll[aá]n[oó]' OR  a.rut ~* 'r[oó]ll[aá]n[oó]' OR  lower(a.email) ~* 'r[oó]ll[aá]n[oó]' OR  text(a.id) ~* 'r[oó]ll[aá]n[oó]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]nd[eé]' OR  a.rut ~* 'c[oó]nd[eé]' OR  lower(a.email) ~* 'c[oó]nd[eé]' OR  text(a.id) ~* 'c[oó]nd[eé]' ) 
