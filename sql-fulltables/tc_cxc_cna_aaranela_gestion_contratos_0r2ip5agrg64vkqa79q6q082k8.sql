COPY (SELECT nro_contrato,ano,regimen,fecha_emision,estado,rut,razon_social,fecha_venc,cta_ctble,monto_inicial,nro_pagare,tipo,
              ...
              FROM ((SELECT c.id as nro_contrato,c.ano,reg.nombre AS regimen,to_char(c.fecha,'DD-MM-YYYY') as fecha_emision,vc.estado,
                     ...
                     (SELECT sum(coalesce(monto-monto_abonado,monto)) AS monto_cxc FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa IN (2,20) AND (NOT pagado OR abonado) AND fecha_venc BETWEEN '2023-11-13'::date+'1 days'::interval AND '2023-11-13'::date+'365 days'::interval) AS cxc_novenc_cp,(SELECT sum(coalesce(monto-monto_abonado,monto)) AS monto_cxc FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa IN (2,20) AND (NOT pagado OR abonado) AND fecha_venc BETWEEN '2023-11-13'::date-'90 days'::interval AND '2023-11-13'::date) AS cxc_0a90dias,
                        (SELECT sum(coalesce(monto-monto_abonado,monto)) AS monto_cxc FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa IN (2,20) AND (NOT pagado OR abonado) AND fecha_venc BETWEEN '2023-11-13'::date-'365 days'::interval AND '2023-11-13'::date-'181 days'::interval) AS cxc_91a180dias,
              ...
              WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '16389912-4' OR  pap.rut ~* '16389912-4' OR lower(a.nombres||' '||a.apellidos) ~* '16389912-4' OR  a.rut ~* '16389912-4' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* '16389912-4' OR  av.rf_rut ~* '16389912-4' OR  text(c.id) ~* '16389912-4' ) 
              ORDER BY c.fecha DESC ) UNION (SELECT c.id as nro_contrato,c.ano,reg.nombre AS regimen,to_char(c.fecha,'DD-MM-YYYY') as fecha_emision,vc.estado,
                         ...
                         (SELECT sum(coalesce(castigo_monto*-1,0)) AS monto_castigo FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa IN (2,20) AND  fecha_venc BETWEEN '2023-11-13'::date+'1 days'::interval AND '2023-11-13'::date+'365 days'::interval) AS cxc_novenc_cp,(SELECT sum(coalesce(castigo_monto*-1,0)) AS monto_castigo FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa IN (2,20) AND  fecha_venc BETWEEN '2023-11-13'::date-'90 days'::interval AND '2023-11-13'::date) AS cxc_0a90dias,
                            (SELECT sum(coalesce(castigo_monto*-1,0)) AS monto_castigo FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa IN (2,20) AND  fecha_venc BETWEEN '2023-11-13'::date-'365 days'::interval AND '2023-11-13'::date-'181 days'::interval) AS cxc_91a180dias,
