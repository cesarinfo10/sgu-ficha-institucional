COPY (SELECT nro_contrato,ano,fecha_emision,estado,razon_social,fecha_venc,cta_ctble,monto_inicial,nro_pagare,tipo,
              ...
              FROM ((SELECT c.id as nro_contrato,c.ano,to_char(c.fecha,'DD-MM-YYYY') as fecha_emision,vc.estado,
                     ...
                     (SELECT sum(coalesce(monto-monto_abonado,monto)) AS monto_cxc FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa>1 AND (NOT pagado OR abonado) AND fecha_venc BETWEEN '2022-12-15'::date+'1 days'::interval AND '2022-12-15'::date+'365 days'::interval) AS cxc_novenc_cp,(SELECT sum(coalesce(monto-monto_abonado,monto)) AS monto_cxc FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa>1 AND (NOT pagado OR abonado) AND fecha_venc BETWEEN '2022-12-15'::date-'90 days'::interval AND '2022-12-15'::date) AS cxc_0a90dias,
                        (SELECT sum(coalesce(monto-monto_abonado,monto)) AS monto_cxc FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa>1 AND (NOT pagado OR abonado) AND fecha_venc BETWEEN '2022-12-15'::date-'365 days'::interval AND '2022-12-15'::date-'181 days'::interval) AS cxc_91a180dias,
              ...
              ORDER BY c.fecha DESC ) UNION (SELECT c.id as nro_contrato,c.ano,to_char(c.fecha,'DD-MM-YYYY') as fecha_emision,vc.estado,
                         ...
                         (SELECT sum(coalesce(castigo_monto*-1,0)) AS monto_castigo FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa>1 AND  fecha_venc BETWEEN '2022-12-15'::date+'1 days'::interval AND '2022-12-15'::date+'365 days'::interval) AS cxc_novenc_cp,(SELECT sum(coalesce(castigo_monto*-1,0)) AS monto_castigo FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa>1 AND  fecha_venc BETWEEN '2022-12-15'::date-'90 days'::interval AND '2022-12-15'::date) AS cxc_0a90dias,
                            (SELECT sum(coalesce(castigo_monto*-1,0)) AS monto_castigo FROM finanzas.cobros WHERE id_contrato=c.id AND id_glosa>1 AND  fecha_venc BETWEEN '2022-12-15'::date-'365 days'::interval AND '2022-12-15'::date-'181 days'::interval) AS cxc_91a180dias,
