COPY (SELECT nro_contrato,fecha_emision,rut,nombre,centro_costo,nro_pagare,
               ...
               FROM (SELECT c.id as nro_contrato,
                      ...
                      CASE WHEN c.monto_arancel > 0 THEN round(vc.monto_beca_arancel_calc*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS beca,
                      CASE WHEN c.monto_arancel > 0 THEN round(c.arancel_cred_interno*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS cred_interno,
                      CASE WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date THEN c.monto_condonacion ELSE null END AS condonacion,
                      (CASE WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND c.arancel_efectivo IS NULL AND c.arancel_tarjeta_credito IS NULL 
                                          THEN c.monto_condonacion
                                     WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND (c.arancel_efectivo IS NOt NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                      ...
                      (CASE WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg IS NULL AND (c.arancel_efectivo IS NOT NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                                          THEN c.monto_condonacion
                                     WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND (c.arancel_efectivo IS NOt NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                      ...
                      (CASE WHEN c.fecha_condonacion BETWEEN '2023-01-01'::date AND '2024-03-14'::date AND c.monto_condonacion > 0 AND c.arancel_cheque IS NOT NULL THEN c.monto_condonacion ELSE 0 END) AS condonacion_arancel_cheque,
                      () AS pagos_total,
               ...
               WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cl[aá][uú]d[ií][aá]' OR  pap.rut ~* 'cl[aá][uú]d[ií][aá]' OR lower(a.nombres||' '||a.apellidos) ~* 'cl[aá][uú]d[ií][aá]' OR  a.rut ~* 'cl[aá][uú]d[ií][aá]' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'cl[aá][uú]d[ií][aá]' OR  av.rf_rut ~* 'cl[aá][uú]d[ií][aá]' OR  text(c.id) ~* 'cl[aá][uú]d[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  pap.rut ~* 'm[aá]nr[ií]q[uú][eé]z' OR lower(a.nombres||' '||a.apellidos) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  a.rut ~* 'm[aá]nr[ií]q[uú][eé]z' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'm[aá]nr[ií]q[uú][eé]z' OR  av.rf_rut ~* 'm[aá]nr[ií]q[uú][eé]z' OR  text(c.id) ~* 'm[aá]nr[ií]q[uú][eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]r[aá]d[aá]' OR  pap.rut ~* 'p[aá]r[aá]d[aá]' OR lower(a.nombres||' '||a.apellidos) ~* 'p[aá]r[aá]d[aá]' OR  a.rut ~* 'p[aá]r[aá]d[aá]' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'p[aá]r[aá]d[aá]' OR  av.rf_rut ~* 'p[aá]r[aá]d[aá]' OR  text(c.id) ~* 'p[aá]r[aá]d[aá]' ) 
