COPY (SELECT nro_contrato,fecha_emision,rut,nombre,centro_costo,nro_pagare,
               ...
               FROM (SELECT c.id as nro_contrato,
                      ...
                      CASE WHEN c.monto_arancel > 0 THEN round(vc.monto_beca_arancel_calc*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS beca,
                      CASE WHEN c.monto_arancel > 0 THEN round(c.arancel_cred_interno*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS cred_interno,
                      CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date THEN c.monto_condonacion ELSE null END AS condonacion,
                      (CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND c.arancel_efectivo IS NULL AND c.arancel_tarjeta_credito IS NULL 
                                          THEN c.monto_condonacion
                                     WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND (c.arancel_efectivo IS NOt NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                      ...
                      (CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg IS NULL AND (c.arancel_efectivo IS NOT NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                                          THEN c.monto_condonacion
                                     WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND (c.arancel_efectivo IS NOt NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                      ...
                      (CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-09-26'::date AND c.monto_condonacion > 0 AND c.arancel_cheque IS NOT NULL THEN c.monto_condonacion ELSE 0 END) AS condonacion_arancel_cheque,
                      () AS pagos_total,
               ...
               WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rl[aá]' OR  pap.rut ~* 'c[aá]rl[aá]' OR lower(a.nombres||' '||a.apellidos) ~* 'c[aá]rl[aá]' OR  a.rut ~* 'c[aá]rl[aá]' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'c[aá]rl[aá]' OR  av.rf_rut ~* 'c[aá]rl[aá]' OR  text(c.id) ~* 'c[aá]rl[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[oó]rr[oó]' OR  pap.rut ~* 'p[oó]rr[oó]' OR lower(a.nombres||' '||a.apellidos) ~* 'p[oó]rr[oó]' OR  a.rut ~* 'p[oó]rr[oó]' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'p[oó]rr[oó]' OR  av.rf_rut ~* 'p[oó]rr[oó]' OR  text(c.id) ~* 'p[oó]rr[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'n[oó]r[aá]mb[uú][eé]n[aá]' OR  pap.rut ~* 'n[oó]r[aá]mb[uú][eé]n[aá]' OR lower(a.nombres||' '||a.apellidos) ~* 'n[oó]r[aá]mb[uú][eé]n[aá]' OR  a.rut ~* 'n[oó]r[aá]mb[uú][eé]n[aá]' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'n[oó]r[aá]mb[uú][eé]n[aá]' OR  av.rf_rut ~* 'n[oó]r[aá]mb[uú][eé]n[aá]' OR  text(c.id) ~* 'n[oó]r[aá]mb[uú][eé]n[aá]' ) 
