COPY (SELECT nro_contrato,fecha_emision,rut,nombre,centro_costo,nro_pagare,
               ...
               FROM (SELECT c.id as nro_contrato,
                      ...
                      CASE WHEN c.monto_arancel > 0 THEN round(vc.monto_beca_arancel_calc*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS beca,
                      CASE WHEN c.monto_arancel > 0 THEN round(c.arancel_cred_interno*(1-(coalesce(CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date THEN c.monto_condonacion ELSE null END,0)::float/c.monto_arancel::float))) ELSE 0 END AS cred_interno,
                      CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date THEN c.monto_condonacion ELSE null END AS condonacion,
                      (CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND c.arancel_efectivo IS NULL AND c.arancel_tarjeta_credito IS NULL 
                                          THEN c.monto_condonacion
                                     WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND (c.arancel_efectivo IS NOt NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                      ...
                      (CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg IS NULL AND (c.arancel_efectivo IS NOT NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                                          THEN c.monto_condonacion
                                     WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date AND c.monto_condonacion > 0 AND c.arancel_pagare_coleg > 0 AND (c.arancel_efectivo IS NOt NULL OR c.arancel_tarjeta_credito IS NOT NULL) 
                      ...
                      (CASE WHEN c.fecha_condonacion BETWEEN '2022-01-01'::date AND '2023-03-29'::date AND c.monto_condonacion > 0 AND c.arancel_cheque IS NOT NULL THEN c.monto_condonacion ELSE 0 END) AS condonacion_arancel_cheque,
                      () AS pagos_total,
               ...
               WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR lower(a.nombres||' '||a.apellidos) ~* '' OR  a.rut ~* '' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* '' OR  av.rf_rut ~* '' OR  text(c.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]x' OR  pap.rut ~* '[aá]l[eé]x' OR lower(a.nombres||' '||a.apellidos) ~* '[aá]l[eé]x' OR  a.rut ~* '[aá]l[eé]x' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* '[aá]l[eé]x' OR  av.rf_rut ~* '[aá]l[eé]x' OR  text(c.id) ~* '[aá]l[eé]x' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  pap.rut ~* 'r[oó]dr[ií]g[oó]' OR lower(a.nombres||' '||a.apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  a.rut ~* 'r[oó]dr[ií]g[oó]' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  av.rf_rut ~* 'r[oó]dr[ií]g[oó]' OR  text(c.id) ~* 'r[oó]dr[ií]g[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]g[oó]s' OR  pap.rut ~* 'l[aá]g[oó]s' OR lower(a.nombres||' '||a.apellidos) ~* 'l[aá]g[oó]s' OR  a.rut ~* 'l[aá]g[oó]s' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* 'l[aá]g[oó]s' OR  av.rf_rut ~* 'l[aá]g[oó]s' OR  text(c.id) ~* 'l[aá]g[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]p[aá]bl[aá]z[aá]' OR  pap.rut ~* '[aá]p[aá]bl[aá]z[aá]' OR lower(a.nombres||' '||a.apellidos) ~* '[aá]p[aá]bl[aá]z[aá]' OR  a.rut ~* '[aá]p[aá]bl[aá]z[aá]' OR lower(av.rf_nombres||' '||av.rf_apellidos) ~* '[aá]p[aá]bl[aá]z[aá]' OR  av.rf_rut ~* '[aá]p[aá]bl[aá]z[aá]' OR  text(c.id) ~* '[aá]p[aá]bl[aá]z[aá]' ) 
