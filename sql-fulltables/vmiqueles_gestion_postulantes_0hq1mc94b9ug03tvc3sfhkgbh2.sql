COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]v[ií][eé]r[aá]' OR  pap.rut ~* 'j[aá]v[ií][eé]r[aá]' OR  text(pap.id) ~* 'j[aá]v[ií][eé]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'p[eé]ñ[aá]l[oó]z[aá]' OR  pap.rut ~* 'p[eé]ñ[aá]l[oó]z[aá]' OR  text(pap.id) ~* 'p[eé]ñ[aá]l[oó]z[aá]' ) 
