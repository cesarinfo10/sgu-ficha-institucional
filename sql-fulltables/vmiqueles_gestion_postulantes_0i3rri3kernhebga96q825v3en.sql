COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá][eé]z' OR  pap.rut ~* 's[aá][eé]z' OR  text(pap.id) ~* 's[aá][eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]nh[uú][eé]z[aá]' OR  pap.rut ~* 's[aá]nh[uú][eé]z[aá]' OR  text(pap.id) ~* 's[aá]nh[uú][eé]z[aá]' ) 
