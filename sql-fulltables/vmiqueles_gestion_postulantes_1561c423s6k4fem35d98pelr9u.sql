COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'myr[ií][aá]m' OR  pap.rut ~* 'myr[ií][aá]m' OR  text(pap.id) ~* 'myr[ií][aá]m' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]c[eé]lly' OR  pap.rut ~* '[aá]r[aá]c[eé]lly' OR  text(pap.id) ~* '[aá]r[aá]c[eé]lly' ) 
