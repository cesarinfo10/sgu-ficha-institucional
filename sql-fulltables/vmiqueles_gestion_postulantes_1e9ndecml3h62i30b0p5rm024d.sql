COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]gr[ií][aá]' OR  pap.rut ~* '[aá]l[eé]gr[ií][aá]' OR  text(pap.id) ~* '[aá]l[eé]gr[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá][uú]j[eé]r' OR  pap.rut ~* 'm[aá][uú]j[eé]r' OR  text(pap.id) ~* 'm[aá][uú]j[eé]r' ) 
