COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú]z' OR  pap.rut ~* 'l[uú]z' OR  text(pap.id) ~* 'l[uú]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]n[aá][eé]s' OR  pap.rut ~* 'd[aá]n[aá][eé]s' OR  text(pap.id) ~* 'd[aá]n[aá][eé]s' ) 
