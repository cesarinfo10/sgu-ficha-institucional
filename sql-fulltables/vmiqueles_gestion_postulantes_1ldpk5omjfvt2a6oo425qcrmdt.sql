COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]t[ií][aá]s' OR  pap.rut ~* 'm[aá]t[ií][aá]s' OR  text(pap.id) ~* 'm[aá]t[ií][aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]gn[aá]c[ií][oó]' OR  pap.rut ~* '[ií]gn[aá]c[ií][oó]' OR  text(pap.id) ~* '[ií]gn[aá]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]j[aá]s' OR  pap.rut ~* 'r[oó]j[aá]s' OR  text(pap.id) ~* 'r[oó]j[aá]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'n[aá]v[aá]rr[oó]' OR  pap.rut ~* 'n[aá]v[aá]rr[oó]' OR  text(pap.id) ~* 'n[aá]v[aá]rr[oó]' ) 
