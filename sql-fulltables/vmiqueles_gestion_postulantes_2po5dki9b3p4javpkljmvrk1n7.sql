COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[uú][aá]n' OR  pap.rut ~* 'j[uú][aá]n' OR  text(pap.id) ~* 'j[uú][aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rl[oó]s' OR  pap.rut ~* 'c[aá]rl[oó]s' OR  text(pap.id) ~* 'c[aá]rl[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nt[eé]' OR  pap.rut ~* 'f[uú][eé]nt[eé]' OR  text(pap.id) ~* 'f[uú][eé]nt[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]lb[aá]' OR  pap.rut ~* '[aá]lb[aá]' OR  text(pap.id) ~* '[aá]lb[aá]' ) 
