COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé]t[ií]c[ií][aá]' OR  pap.rut ~* 'l[eé]t[ií]c[ií][aá]' OR  text(pap.id) ~* 'l[eé]t[ií]c[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]lch[eé]s' OR  pap.rut ~* 'v[ií]lch[eé]s' OR  text(pap.id) ~* 'v[ií]lch[eé]s' ) 
