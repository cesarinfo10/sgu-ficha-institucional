COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE true  AND (pap.cohorte='2023')  AND (pap.semestre_cohorte='1')  AND (pap.estado_carpeta_doctos='Incompleta')  AND vcp.estado = 'E' AND ('PRE' IN (c1.regimen,c2.regimen,c3.regimen,c4.regimen,c5.regimen,c6.regimen))  AND (a.id IS NULL)AND (vca.rut IS NOT NULL) 
