COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[ií]g[uú][eé]l' OR  pap.rut ~* 'm[ií]g[uú][eé]l' OR  text(pap.id) ~* 'm[ií]g[uú][eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ng[eé]l' OR  pap.rut ~* '[aá]ng[eé]l' OR  text(pap.id) ~* '[aá]ng[eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'n[uú]ñ[eé]z' OR  pap.rut ~* 'n[uú]ñ[eé]z' OR  text(pap.id) ~* 'n[uú]ñ[eé]z' ) 
