COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]t[aá]l[ií]n[aá]' OR  pap.rut ~* 'c[aá]t[aá]l[ií]n[aá]' OR  text(pap.id) ~* 'c[aá]t[aá]l[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]g[aá]l[ií]' OR  pap.rut ~* 's[eé]g[aá]l[ií]' OR  text(pap.id) ~* 's[eé]g[aá]l[ií]' ) 
