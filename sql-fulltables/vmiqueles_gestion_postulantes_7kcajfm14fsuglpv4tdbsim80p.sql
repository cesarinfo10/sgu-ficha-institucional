COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[ií]gn[aá]c[ií][oó]' OR  pap.rut ~* '[ií]gn[aá]c[ií][oó]' OR  text(pap.id) ~* '[ií]gn[aá]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'd[oó]n[oó]s[oó]' OR  pap.rut ~* 'd[oó]n[oó]s[oó]' OR  text(pap.id) ~* 'd[oó]n[oó]s[oó]' ) 
