COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá]tr[ií]c[ií][oó]' OR  pap.rut ~* 'p[aá]tr[ií]c[ií][oó]' OR  text(pap.id) ~* 'p[aá]tr[ií]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]ll[aá]rd[oó]' OR  pap.rut ~* 'g[aá]ll[aá]rd[oó]' OR  text(pap.id) ~* 'g[aá]ll[aá]rd[oó]' ) 
