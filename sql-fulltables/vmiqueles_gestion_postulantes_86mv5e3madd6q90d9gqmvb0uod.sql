COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cl[aá][uú]d[ií][oó]' OR  pap.rut ~* 'cl[aá][uú]d[ií][oó]' OR  text(pap.id) ~* 'cl[aá][uú]d[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  pap.rut ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  text(pap.id) ~* 'v[ií]ll[aá]rr[oó][eé]l' ) 
