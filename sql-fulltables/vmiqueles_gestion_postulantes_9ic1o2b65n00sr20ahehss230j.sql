COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]nch[eé]z' OR  pap.rut ~* 's[aá]nch[eé]z' OR  text(pap.id) ~* 's[aá]nch[eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]nch[eé]z' OR  pap.rut ~* 's[aá]nch[eé]z' OR  text(pap.id) ~* 's[aá]nch[eé]z' ) 
