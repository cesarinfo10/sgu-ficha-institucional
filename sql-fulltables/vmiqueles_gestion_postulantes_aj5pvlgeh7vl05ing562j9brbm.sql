COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[eé]n[ií]f[eé]r' OR  pap.rut ~* 'j[eé]n[ií]f[eé]r' OR  text(pap.id) ~* 'j[eé]n[ií]f[eé]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]rt[ií]z' OR  pap.rut ~* '[oó]rt[ií]z' OR  text(pap.id) ~* '[oó]rt[ií]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  pap.rut ~* 'b[uú]st[aá]m[aá]nt[eé]' OR  text(pap.id) ~* 'b[uú]st[aá]m[aá]nt[eé]' ) 
