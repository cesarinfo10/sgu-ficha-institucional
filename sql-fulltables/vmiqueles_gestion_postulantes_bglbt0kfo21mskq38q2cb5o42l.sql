COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[aá]rb[aá]r[aá]' OR  pap.rut ~* 'b[aá]rb[aá]r[aá]' OR  text(pap.id) ~* 'b[aá]rb[aá]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]r[aá]' OR  pap.rut ~* 'l[aá]r[aá]' OR  text(pap.id) ~* 'l[aá]r[aá]' ) 
