COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]sm[ií]n' OR  pap.rut ~* 'j[aá]sm[ií]n' OR  text(pap.id) ~* 'j[aá]sm[ií]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[oó]rr[eé]s' OR  pap.rut ~* 't[oó]rr[eé]s' OR  text(pap.id) ~* 't[oó]rr[eé]s' ) 
