COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[eé]rm[aá]n' OR  pap.rut ~* 'g[eé]rm[aá]n' OR  text(pap.id) ~* 'g[eé]rm[aá]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]v[eé]n[aá]' OR  pap.rut ~* '[aá]r[aá]v[eé]n[aá]' OR  text(pap.id) ~* '[aá]r[aá]v[eé]n[aá]' ) 
