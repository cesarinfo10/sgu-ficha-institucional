COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]rl[oó]s' OR  pap.rut ~* 'c[aá]rl[oó]s' OR  text(pap.id) ~* 'c[aá]rl[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]l[eé]j[aá]ndr[oó]' OR  pap.rut ~* '[aá]l[eé]j[aá]ndr[oó]' OR  text(pap.id) ~* '[aá]l[eé]j[aá]ndr[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) 
