COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'f[eé]rn[aá]nd[aá]' OR  pap.rut ~* 'f[eé]rn[aá]nd[aá]' OR  text(pap.id) ~* 'f[eé]rn[aá]nd[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[ií]l[eé]nk[aá]' OR  pap.rut ~* 'm[ií]l[eé]nk[aá]' OR  text(pap.id) ~* 'm[ií]l[eé]nk[aá]' ) 
