COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[eé]rn[aá]rd[ií]t[aá]' OR  pap.rut ~* 'b[eé]rn[aá]rd[ií]t[aá]' OR  text(pap.id) ~* 'b[eé]rn[aá]rd[ií]t[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[eé]p[uú]lv[eé]d[aá]' OR  pap.rut ~* 's[eé]p[uú]lv[eé]d[aá]' OR  text(pap.id) ~* 's[eé]p[uú]lv[eé]d[aá]' ) 
