COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]rc[eé]l[aá]' OR  pap.rut ~* 'm[aá]rc[eé]l[aá]' OR  text(pap.id) ~* 'm[aá]rc[eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá][eé]t[eé]' OR  pap.rut ~* 'g[aá][eé]t[eé]' OR  text(pap.id) ~* 'g[aá][eé]t[eé]' ) 
