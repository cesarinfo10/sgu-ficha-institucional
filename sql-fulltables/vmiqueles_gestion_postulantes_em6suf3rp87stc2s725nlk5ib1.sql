COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]c[ií][oó]' OR  pap.rut ~* 'r[oó]c[ií][oó]' OR  text(pap.id) ~* 'r[oó]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[uú]rg[oó]s' OR  pap.rut ~* 'b[uú]rg[oó]s' OR  text(pap.id) ~* 'b[uú]rg[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]c[uú]ñ[aá]' OR  pap.rut ~* '[aá]c[uú]ñ[aá]' OR  text(pap.id) ~* '[aá]c[uú]ñ[aá]' ) 
