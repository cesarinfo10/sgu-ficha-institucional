COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'n[ií]c[oó]l' OR  pap.rut ~* 'n[ií]c[oó]l' OR  text(pap.id) ~* 'n[ií]c[oó]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]sp[ií]n[oó]z[aá]' OR  pap.rut ~* '[eé]sp[ií]n[oó]z[aá]' OR  text(pap.id) ~* '[eé]sp[ií]n[oó]z[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]r[aá]y[aá]' OR  pap.rut ~* '[aá]r[aá]y[aá]' OR  text(pap.id) ~* '[aá]r[aá]y[aá]' ) 
