COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]r[ií]n[aá]' OR  pap.rut ~* 'c[aá]r[ií]n[aá]' OR  text(pap.id) ~* 'c[aá]r[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]pl[ií]n' OR  pap.rut ~* 'j[oó]pl[ií]n' OR  text(pap.id) ~* 'j[oó]pl[ií]n' ) 
