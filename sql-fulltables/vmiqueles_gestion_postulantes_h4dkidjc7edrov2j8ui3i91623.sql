COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[aá]rr[aá]z[aá]' OR  pap.rut ~* 'b[aá]rr[aá]z[aá]' OR  text(pap.id) ~* 'b[aá]rr[aá]z[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'q[uú][eé]z[aá]d[aá]' OR  pap.rut ~* 'q[uú][eé]z[aá]d[aá]' OR  text(pap.id) ~* 'q[uú][eé]z[aá]d[aá]' ) 
