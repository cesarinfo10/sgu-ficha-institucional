COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  pap.rut ~* 'v[aá]l[eé]nt[ií]n[aá]' OR  text(pap.id) ~* 'v[aá]l[eé]nt[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]c[uú][aá]nt' OR  pap.rut ~* 'r[oó]c[uú][aá]nt' OR  text(pap.id) ~* 'r[oó]c[uú][aá]nt' ) 
