COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[eé]l[ií][aá]s[eé]r' OR  pap.rut ~* '[eé]l[ií][aá]s[eé]r' OR  text(pap.id) ~* '[eé]l[ií][aá]s[eé]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 't[eé]v[eé]z' OR  pap.rut ~* 't[eé]v[eé]z' OR  text(pap.id) ~* 't[eé]v[eé]z' ) 
