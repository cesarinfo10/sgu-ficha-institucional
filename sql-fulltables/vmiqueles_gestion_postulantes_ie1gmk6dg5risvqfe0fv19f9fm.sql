COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'k[aá]r[ií]n[aá]' OR  pap.rut ~* 'k[aá]r[ií]n[aá]' OR  text(pap.id) ~* 'k[aá]r[ií]n[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[oó]p[eé]z' OR  pap.rut ~* 'l[oó]p[eé]z' OR  text(pap.id) ~* 'l[oó]p[eé]z' ) 
