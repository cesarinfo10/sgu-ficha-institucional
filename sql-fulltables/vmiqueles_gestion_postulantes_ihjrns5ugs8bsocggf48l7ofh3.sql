COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]b[eé]l' OR  pap.rut ~* 'm[aá]b[eé]l' OR  text(pap.id) ~* 'm[aá]b[eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'z[uú]ñ[ií]g[aá]' OR  pap.rut ~* 'z[uú]ñ[ií]g[aá]' OR  text(pap.id) ~* 'z[uú]ñ[ií]g[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[eé]t[aá]m[aá]l' OR  pap.rut ~* 'r[eé]t[aá]m[aá]l' OR  text(pap.id) ~* 'r[eé]t[aá]m[aá]l' ) 
