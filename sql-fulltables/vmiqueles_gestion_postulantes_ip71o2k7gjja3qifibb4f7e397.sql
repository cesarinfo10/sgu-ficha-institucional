COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]rg[eé]' OR  pap.rut ~* 'j[oó]rg[eé]' OR  text(pap.id) ~* 'j[oó]rg[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]rq[uú][eé]r[aá]' OR  pap.rut ~* 'j[oó]rq[uú][eé]r[aá]' OR  text(pap.id) ~* 'j[oó]rq[uú][eé]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[aá]zc[aá]n[oó]' OR  pap.rut ~* 'l[aá]zc[aá]n[oó]' OR  text(pap.id) ~* 'l[aá]zc[aá]n[oó]' ) 
