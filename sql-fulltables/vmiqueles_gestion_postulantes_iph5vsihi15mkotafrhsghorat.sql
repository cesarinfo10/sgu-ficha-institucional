COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'j[oó]hn' OR  pap.rut ~* 'j[oó]hn' OR  text(pap.id) ~* 'j[oó]hn' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[oó]d[aá]l[eé][oó]' OR  pap.rut ~* 'b[oó]d[aá]l[eé][oó]' OR  text(pap.id) ~* 'b[oó]d[aá]l[eé][oó]' ) 
