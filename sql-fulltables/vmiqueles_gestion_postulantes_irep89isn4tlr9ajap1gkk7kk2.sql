COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  pap.rut ~* 'm[aá][uú]r[ií]c[ií][oó]' OR  text(pap.id) ~* 'm[aá][uú]r[ií]c[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'z[aá]m[oó]r[aá]' OR  pap.rut ~* 'z[aá]m[oó]r[aá]' OR  text(pap.id) ~* 'z[aá]m[oó]r[aá]' ) 
