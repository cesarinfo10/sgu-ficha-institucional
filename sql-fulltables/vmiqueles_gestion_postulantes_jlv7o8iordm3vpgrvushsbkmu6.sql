COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]str[oó]' OR  pap.rut ~* 'c[aá]str[oó]' OR  text(pap.id) ~* 'c[aá]str[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  pap.rut ~* 'v[ií]ll[aá]rr[oó][eé]l' OR  text(pap.id) ~* 'v[ií]ll[aá]rr[oó][eé]l' ) 
