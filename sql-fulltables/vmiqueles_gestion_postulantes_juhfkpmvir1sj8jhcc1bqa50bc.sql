COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'f[uú][eé]nt[eé]s' OR  pap.rut ~* 'f[uú][eé]nt[eé]s' OR  text(pap.id) ~* 'f[uú][eé]nt[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]fr[eé]' OR  pap.rut ~* 'c[oó]fr[eé]' OR  text(pap.id) ~* 'c[oó]fr[eé]' ) 
