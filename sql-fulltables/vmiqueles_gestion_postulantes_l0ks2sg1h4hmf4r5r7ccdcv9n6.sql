COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'd[aá]v[ií]d' OR  pap.rut ~* 'd[aá]v[ií]d' OR  text(pap.id) ~* 'd[aá]v[ií]d' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  pap.rut ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' OR  text(pap.id) ~* 'v[aá]l[eé]nz[uú][eé]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '' OR  pap.rut ~* '' OR  text(pap.id) ~* '' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]rt[eé]g[aá]' OR  pap.rut ~* '[oó]rt[eé]g[aá]' OR  text(pap.id) ~* '[oó]rt[eé]g[aá]' ) 
