COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]dr[ií]g[oó]' OR  pap.rut ~* 'r[oó]dr[ií]g[oó]' OR  text(pap.id) ~* 'r[oó]dr[ií]g[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]ll[eé]j[oó]s' OR  pap.rut ~* 'v[aá]ll[eé]j[oó]s' OR  text(pap.id) ~* 'v[aá]ll[eé]j[oó]s' ) 
