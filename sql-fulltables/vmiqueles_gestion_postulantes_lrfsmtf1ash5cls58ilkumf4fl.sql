COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ng[eé]l' OR  pap.rut ~* '[aá]ng[eé]l' OR  text(pap.id) ~* '[aá]ng[eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]br[ií][eé]l' OR  pap.rut ~* 'g[aá]br[ií][eé]l' OR  text(pap.id) ~* 'g[aá]br[ií][eé]l' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]rr[eé][aá]' OR  pap.rut ~* 'c[oó]rr[eé][aá]' OR  text(pap.id) ~* 'c[oó]rr[eé][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]lg[aá]d[oó]' OR  pap.rut ~* 's[aá]lg[aá]d[oó]' OR  text(pap.id) ~* 's[aá]lg[aá]d[oó]' ) 
