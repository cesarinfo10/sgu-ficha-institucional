COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'chr[ií]st[ií][aá]' OR  pap.rut ~* 'chr[ií]st[ií][aá]' OR  text(pap.id) ~* 'chr[ií]st[ií][aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]r[aá]p[ií]nt[oó]' OR  pap.rut ~* 'v[eé]r[aá]p[ií]nt[oó]' OR  text(pap.id) ~* 'v[eé]r[aá]p[ií]nt[oó]' ) 
