COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[aá]n[eé]ss[aá]' OR  pap.rut ~* 'v[aá]n[eé]ss[aá]' OR  text(pap.id) ~* 'v[aá]n[eé]ss[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[oó]rr[eé][aá]' OR  pap.rut ~* 'c[oó]rr[eé][aá]' OR  text(pap.id) ~* 'c[oó]rr[eé][aá]' ) 
