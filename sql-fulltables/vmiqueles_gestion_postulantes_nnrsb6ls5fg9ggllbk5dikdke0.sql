COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[eé]nr[ií]q[uú][eé]z' OR  pap.rut ~* 'h[eé]nr[ií]q[uú][eé]z' OR  text(pap.id) ~* 'h[eé]nr[ií]q[uú][eé]z' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[aá]ch[uú]c[aá]' OR  pap.rut ~* 'm[aá]ch[uú]c[aá]' OR  text(pap.id) ~* 'm[aá]ch[uú]c[aá]' ) 
