COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'g[ií]ss[eé]ll[eé]' OR  pap.rut ~* 'g[ií]ss[eé]ll[eé]' OR  text(pap.id) ~* 'g[ií]ss[eé]ll[eé]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'm[ií]r[aá]nd[aá]' OR  pap.rut ~* 'm[ií]r[aá]nd[aá]' OR  text(pap.id) ~* 'm[ií]r[aá]nd[aá]' ) 
