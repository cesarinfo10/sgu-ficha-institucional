COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* '[aá]ndr[eé]s' OR  pap.rut ~* '[aá]ndr[eé]s' OR  text(pap.id) ~* '[aá]ndr[eé]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'g[aá]l[ií]nd[oó]' OR  pap.rut ~* 'g[aá]l[ií]nd[oó]' OR  text(pap.id) ~* 'g[aá]l[ií]nd[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* '[oó]s[oó]r[ií][oó]' OR  pap.rut ~* '[oó]s[oó]r[ií][oó]' OR  text(pap.id) ~* '[oó]s[oó]r[ií][oó]' ) 
