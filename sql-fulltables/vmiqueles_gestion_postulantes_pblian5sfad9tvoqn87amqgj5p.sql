COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'l[uú]d[oó]v[ií]c[aá]' OR  pap.rut ~* 'l[uú]d[oó]v[ií]c[aá]' OR  text(pap.id) ~* 'l[uú]d[oó]v[ií]c[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]ll[aá]gr[aá]' OR  pap.rut ~* 'v[ií]ll[aá]gr[aá]' OR  text(pap.id) ~* 'v[ií]ll[aá]gr[aá]' ) 
