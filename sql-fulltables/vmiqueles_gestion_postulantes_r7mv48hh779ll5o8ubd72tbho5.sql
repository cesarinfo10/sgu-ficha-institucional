COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'v[eé]g[aá]' OR  pap.rut ~* 'v[eé]g[aá]' OR  text(pap.id) ~* 'v[eé]g[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[ií]lv[aá]' OR  pap.rut ~* 's[ií]lv[aá]' OR  text(pap.id) ~* 's[ií]lv[aá]' ) 
