COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'y[oó][ií]s[eé]n' OR  pap.rut ~* 'y[oó][ií]s[eé]n' OR  text(pap.id) ~* 'y[oó][ií]s[eé]n' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'b[aá]sc[uú]r' OR  pap.rut ~* 'b[aá]sc[uú]r' OR  text(pap.id) ~* 'b[aá]sc[uú]r' ) 
