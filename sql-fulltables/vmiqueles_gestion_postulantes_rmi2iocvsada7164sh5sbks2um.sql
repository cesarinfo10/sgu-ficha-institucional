COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]d[oó]lf[oó]' OR  pap.rut ~* 'r[oó]d[oó]lf[oó]' OR  text(pap.id) ~* 'r[oó]d[oó]lf[oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'c[aá]r[oó]c[aá]' OR  pap.rut ~* 'c[aá]r[oó]c[aá]' OR  text(pap.id) ~* 'c[aá]r[oó]c[aá]' ) 
