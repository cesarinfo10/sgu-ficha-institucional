COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'b[aá]sc[uú]r' OR  pap.rut ~* 'b[aá]sc[uú]r' OR  text(pap.id) ~* 'b[aá]sc[uú]r' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'r[uú]b[ií]l[aá]r' OR  pap.rut ~* 'r[uú]b[ií]l[aá]r' OR  text(pap.id) ~* 'r[uú]b[ií]l[aá]r' ) 
