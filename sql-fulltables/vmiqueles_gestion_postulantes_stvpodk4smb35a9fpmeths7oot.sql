COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'p[aá][uú]l[aá]' OR  pap.rut ~* 'p[aá][uú]l[aá]' OR  text(pap.id) ~* 'p[aá][uú]l[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'v[ií]v[aá]nc[oó]' OR  pap.rut ~* 'v[ií]v[aá]nc[oó]' OR  text(pap.id) ~* 'v[ií]v[aá]nc[oó]' ) 
