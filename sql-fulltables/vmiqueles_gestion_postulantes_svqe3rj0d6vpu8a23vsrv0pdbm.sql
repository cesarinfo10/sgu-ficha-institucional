COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'c[uú]b[ií]ll[oó]s' OR  pap.rut ~* 'c[uú]b[ií]ll[oó]s' OR  text(pap.id) ~* 'c[uú]b[ií]ll[oó]s' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'l[eé][ií]v[aá]' OR  pap.rut ~* 'l[eé][ií]v[aá]' OR  text(pap.id) ~* 'l[eé][ií]v[aá]' ) 
