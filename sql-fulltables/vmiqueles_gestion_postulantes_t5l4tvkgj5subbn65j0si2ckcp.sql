COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'h[eé]rr[eé]r[aá]' OR  pap.rut ~* 'h[eé]rr[eé]r[aá]' OR  text(pap.id) ~* 'h[eé]rr[eé]r[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'j[aá]r[aá]' OR  pap.rut ~* 'j[aá]r[aá]' OR  text(pap.id) ~* 'j[aá]r[aá]' ) 
