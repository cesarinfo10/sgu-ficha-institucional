COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'cl[aá][uú]d[ií][oó]' OR  pap.rut ~* 'cl[aá][uú]d[ií][oó]' OR  text(pap.id) ~* 'cl[aá][uú]d[ií][oó]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 'h[aá]v[ií][eé]rsh' OR  pap.rut ~* 'h[aá]v[ií][eé]rsh' OR  text(pap.id) ~* 'h[aá]v[ií][eé]rsh' ) 
