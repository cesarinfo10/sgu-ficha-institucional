COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '16603497-3' OR  a.rut ~* '16603497-3' OR  lower(a.email) ~* '16603497-3' OR  text(a.id) ~* '16603497-3' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
