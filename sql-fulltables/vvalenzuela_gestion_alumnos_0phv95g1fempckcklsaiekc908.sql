COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]ncy' OR  a.rut ~* 'n[aá]ncy' OR  lower(a.email) ~* 'n[aá]ncy' OR  text(a.id) ~* 'n[aá]ncy' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
