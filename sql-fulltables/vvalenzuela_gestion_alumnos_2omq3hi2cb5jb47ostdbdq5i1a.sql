COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]lm[aá]rc[eé]g[uú][ií]' OR  a.rut ~* '[aá]lm[aá]rc[eé]g[uú][ií]' OR  lower(a.email) ~* '[aá]lm[aá]rc[eé]g[uú][ií]' OR  text(a.id) ~* '[aá]lm[aá]rc[eé]g[uú][ií]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
