COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]r[oó]n[ií]c[aá]' OR  a.rut ~* 'v[eé]r[oó]n[ií]c[aá]' OR  lower(a.email) ~* 'v[eé]r[oó]n[ií]c[aá]' OR  text(a.id) ~* 'v[eé]r[oó]n[ií]c[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
