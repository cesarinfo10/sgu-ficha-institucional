COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]t[ií][aá]s' OR  a.rut ~* 'm[aá]t[ií][aá]s' OR  lower(a.email) ~* 'm[aá]t[ií][aá]s' OR  text(a.id) ~* 'm[aá]t[ií][aá]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú]zm[aá]n' OR  a.rut ~* 'g[uú]zm[aá]n' OR  lower(a.email) ~* 'g[uú]zm[aá]n' OR  text(a.id) ~* 'g[uú]zm[aá]n' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
