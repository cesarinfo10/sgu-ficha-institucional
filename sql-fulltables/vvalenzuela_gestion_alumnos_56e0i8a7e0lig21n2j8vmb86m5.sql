COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'b[aá]rr[uú][eé]t[oó]' OR  a.rut ~* 'b[aá]rr[uú][eé]t[oó]' OR  lower(a.email) ~* 'b[aá]rr[uú][eé]t[oó]' OR  text(a.id) ~* 'b[aá]rr[uú][eé]t[oó]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
