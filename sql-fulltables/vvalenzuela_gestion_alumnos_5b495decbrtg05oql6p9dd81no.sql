COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ndr[eé][aá]' OR  a.rut ~* '[aá]ndr[eé][aá]' OR  lower(a.email) ~* '[aá]ndr[eé][aá]' OR  text(a.id) ~* '[aá]ndr[eé][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[uú]b[ií]ll[oó]s' OR  a.rut ~* 'c[uú]b[ií]ll[oó]s' OR  lower(a.email) ~* 'c[uú]b[ií]ll[oó]s' OR  text(a.id) ~* 'c[uú]b[ií]ll[oó]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
