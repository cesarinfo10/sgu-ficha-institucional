COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[aá]l[eé]nt[ií]nn[eé]' OR  a.rut ~* 'v[aá]l[eé]nt[ií]nn[eé]' OR  lower(a.email) ~* 'v[aá]l[eé]nt[ií]nn[eé]' OR  text(a.id) ~* 'v[aá]l[eé]nt[ií]nn[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
