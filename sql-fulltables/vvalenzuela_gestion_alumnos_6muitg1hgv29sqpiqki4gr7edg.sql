COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[oó]l[aá]nd[aá]' OR  a.rut ~* 'y[oó]l[aá]nd[aá]' OR  lower(a.email) ~* 'y[oó]l[aá]nd[aá]' OR  text(a.id) ~* 'y[oó]l[aá]nd[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[aá]str[aá]' OR  a.rut ~* 'l[aá]str[aá]' OR  lower(a.email) ~* 'l[aá]str[aá]' OR  text(a.id) ~* 'l[aá]str[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
