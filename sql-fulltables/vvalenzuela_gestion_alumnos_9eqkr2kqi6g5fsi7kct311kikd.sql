COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'pr[ií]sc[ií]ll[aá]' OR  a.rut ~* 'pr[ií]sc[ií]ll[aá]' OR  lower(a.email) ~* 'pr[ií]sc[ií]ll[aá]' OR  text(a.id) ~* 'pr[ií]sc[ií]ll[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
