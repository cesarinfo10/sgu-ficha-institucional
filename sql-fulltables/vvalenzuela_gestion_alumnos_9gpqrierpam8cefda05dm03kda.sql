COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[aá]r[oó]' OR  a.rut ~* 'c[aá]r[oó]' OR  lower(a.email) ~* 'c[aá]r[oó]' OR  text(a.id) ~* 'c[aá]r[oó]' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
