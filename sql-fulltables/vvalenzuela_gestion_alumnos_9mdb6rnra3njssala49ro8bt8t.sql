COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[eé]ñ[aá]' OR  a.rut ~* 'p[eé]ñ[aá]' OR  lower(a.email) ~* 'p[eé]ñ[aá]' OR  text(a.id) ~* 'p[eé]ñ[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 's[ií]lv[aá]' OR  a.rut ~* 's[ií]lv[aá]' OR  lower(a.email) ~* 's[ií]lv[aá]' OR  text(a.id) ~* 's[ií]lv[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
