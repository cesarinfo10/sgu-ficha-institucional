COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'gh[ií]sl[eé][ií]n[eé]' OR  a.rut ~* 'gh[ií]sl[eé][ií]n[eé]' OR  lower(a.email) ~* 'gh[ií]sl[eé][ií]n[eé]' OR  text(a.id) ~* 'gh[ií]sl[eé][ií]n[eé]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]s[uú]nc[ií][oó]n' OR  a.rut ~* '[aá]s[uú]nc[ií][oó]n' OR  lower(a.email) ~* '[aá]s[uú]nc[ií][oó]n' OR  text(a.id) ~* '[aá]s[uú]nc[ií][oó]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g' OR  a.rut ~* 'g' OR  lower(a.email) ~* 'g' OR  text(a.id) ~* 'g' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
