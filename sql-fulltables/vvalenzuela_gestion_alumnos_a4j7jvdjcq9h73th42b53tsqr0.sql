COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM matriculas WHERE id_alumno=a.id AND ano=2022 LIMIT 1)=1  THEN 'Si' ELSE 'No' END AS matriculado,(SELECT max(fecha) FROM matriculas WHERE id_alumno=a.id) AS fecha_ult_mat,
                ...
                WHERE true  AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2022))  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
