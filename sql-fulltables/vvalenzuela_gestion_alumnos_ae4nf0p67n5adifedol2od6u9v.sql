COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]ll[aá]l[eé][oó]' OR  a.rut ~* 'p[aá]ll[aá]l[eé][oó]' OR  lower(a.email) ~* 'p[aá]ll[aá]l[eé][oó]' OR  text(a.id) ~* 'p[aá]ll[aá]l[eé][oó]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
