COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '18027269-0' OR  a.rut ~* '18027269-0' OR  lower(a.email) ~* '18027269-0' OR  text(a.id) ~* '18027269-0' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
