COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]ng[eé]l[aá]' OR  a.rut ~* '[aá]ng[eé]l[aá]' OR  lower(a.email) ~* '[aá]ng[eé]l[aá]' OR  text(a.id) ~* '[aá]ng[eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[uú][eé]nt[eé]s' OR  a.rut ~* 'f[uú][eé]nt[eé]s' OR  lower(a.email) ~* 'f[uú][eé]nt[eé]s' OR  text(a.id) ~* 'f[uú][eé]nt[eé]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
