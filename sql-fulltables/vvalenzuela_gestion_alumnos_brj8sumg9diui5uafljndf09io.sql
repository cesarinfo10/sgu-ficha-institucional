COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[eé]p[uú]lv[eé]d[aá]' OR  a.rut ~* 's[eé]p[uú]lv[eé]d[aá]' OR  lower(a.email) ~* 's[eé]p[uú]lv[eé]d[aá]' OR  text(a.id) ~* 's[eé]p[uú]lv[eé]d[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
