COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]n[eé]s[eé]s' OR  a.rut ~* 'm[eé]n[eé]s[eé]s' OR  lower(a.email) ~* 'm[eé]n[eé]s[eé]s' OR  text(a.id) ~* 'm[eé]n[eé]s[eé]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
