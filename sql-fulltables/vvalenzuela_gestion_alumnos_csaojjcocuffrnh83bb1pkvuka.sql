COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]z' OR  a.rut ~* 'l[uú]z' OR  lower(a.email) ~* 'l[uú]z' OR  text(a.id) ~* 'l[uú]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]r[eé]d[eé]s' OR  a.rut ~* 'p[aá]r[eé]d[eé]s' OR  lower(a.email) ~* 'p[aá]r[eé]d[eé]s' OR  text(a.id) ~* 'p[aá]r[eé]d[eé]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'f[aá]r[ií][aá]s' OR  a.rut ~* 'f[aá]r[ií][aá]s' OR  lower(a.email) ~* 'f[aá]r[ií][aá]s' OR  text(a.id) ~* 'f[aá]r[ií][aá]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
