COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'h[aá]nn[aá]' OR  a.rut ~* 'h[aá]nn[aá]' OR  lower(a.email) ~* 'h[aá]nn[aá]' OR  text(a.id) ~* 'h[aá]nn[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
