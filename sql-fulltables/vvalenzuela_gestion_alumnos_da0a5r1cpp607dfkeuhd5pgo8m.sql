COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]r[ií][aá]' OR  a.rut ~* 'm[aá]r[ií][aá]' OR  lower(a.email) ~* 'm[aá]r[ií][aá]' OR  text(a.id) ~* 'm[aá]r[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]gn[aá]c[ií][aá]' OR  a.rut ~* '[ií]gn[aá]c[ií][aá]' OR  lower(a.email) ~* '[ií]gn[aá]c[ií][aá]' OR  text(a.id) ~* '[ií]gn[aá]c[ií][aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
