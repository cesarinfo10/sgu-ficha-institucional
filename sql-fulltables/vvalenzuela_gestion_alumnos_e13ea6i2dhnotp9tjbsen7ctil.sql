COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'p[aá]m[eé]l[aá]' OR  a.rut ~* 'p[aá]m[eé]l[aá]' OR  lower(a.email) ~* 'p[aá]m[eé]l[aá]' OR  text(a.id) ~* 'p[aá]m[eé]l[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[eé]r[aá]' OR  a.rut ~* 'v[eé]r[aá]' OR  lower(a.email) ~* 'v[eé]r[aá]' OR  text(a.id) ~* 'v[eé]r[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
