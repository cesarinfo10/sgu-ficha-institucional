COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[oó]s[eé]' OR  a.rut ~* 'y[oó]s[eé]' OR  lower(a.email) ~* 'y[oó]s[eé]' OR  text(a.id) ~* 'y[oó]s[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
