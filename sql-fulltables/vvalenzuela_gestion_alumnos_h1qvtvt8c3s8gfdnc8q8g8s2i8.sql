COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ct[oó]r' OR  a.rut ~* 'v[ií]ct[oó]r' OR  lower(a.email) ~* 'v[ií]ct[oó]r' OR  text(a.id) ~* 'v[ií]ct[oó]r' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  a.rut ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  lower(a.email) ~* 'q[uú][ií]ñ[oó]n[eé]s' OR  text(a.id) ~* 'q[uú][ií]ñ[oó]n[eé]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
