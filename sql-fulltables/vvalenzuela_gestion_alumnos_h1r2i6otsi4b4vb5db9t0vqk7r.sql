COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]gd[aá]l[eé]n[aá]' OR  a.rut ~* 'm[aá]gd[aá]l[eé]n[aá]' OR  lower(a.email) ~* 'm[aá]gd[aá]l[eé]n[aá]' OR  text(a.id) ~* 'm[aá]gd[aá]l[eé]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'r[ií]q[uú][eé]lm[eé]' OR  a.rut ~* 'r[ií]q[uú][eé]lm[eé]' OR  lower(a.email) ~* 'r[ií]q[uú][eé]lm[eé]' OR  text(a.id) ~* 'r[ií]q[uú][eé]lm[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
