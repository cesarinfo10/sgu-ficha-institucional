COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[aá]r[aá]m[ií]ll[oó]' OR  a.rut ~* 'j[aá]r[aá]m[ií]ll[oó]' OR  lower(a.email) ~* 'j[aá]r[aá]m[ií]ll[oó]' OR  text(a.id) ~* 'j[aá]r[aá]m[ií]ll[oó]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
