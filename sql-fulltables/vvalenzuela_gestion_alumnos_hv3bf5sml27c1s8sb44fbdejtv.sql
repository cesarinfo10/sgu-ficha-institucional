COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '13790837-9' OR  a.rut ~* '13790837-9' OR  lower(a.email) ~* '13790837-9' OR  text(a.id) ~* '13790837-9' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
