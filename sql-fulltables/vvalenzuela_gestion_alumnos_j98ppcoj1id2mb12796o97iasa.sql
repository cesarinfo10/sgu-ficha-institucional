COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]nt[oó][ií]n[eé]tt[eé]' OR  a.rut ~* '[aá]nt[oó][ií]n[eé]tt[eé]' OR  lower(a.email) ~* '[aá]nt[oó][ií]n[eé]tt[eé]' OR  text(a.id) ~* '[aá]nt[oó][ií]n[eé]tt[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
