COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]d[eé]l[eé]yn' OR  a.rut ~* 'm[aá]d[eé]l[eé]yn' OR  lower(a.email) ~* 'm[aá]d[eé]l[eé]yn' OR  text(a.id) ~* 'm[aá]d[eé]l[eé]yn' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'q[uú][eé][uú]p[uú]m[ií]l' OR  a.rut ~* 'q[uú][eé][uú]p[uú]m[ií]l' OR  lower(a.email) ~* 'q[uú][eé][uú]p[uú]m[ií]l' OR  text(a.id) ~* 'q[uú][eé][uú]p[uú]m[ií]l' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
