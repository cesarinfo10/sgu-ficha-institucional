COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '26766360-2' OR  a.rut ~* '26766360-2' OR  lower(a.email) ~* '26766360-2' OR  text(a.id) ~* '26766360-2' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
