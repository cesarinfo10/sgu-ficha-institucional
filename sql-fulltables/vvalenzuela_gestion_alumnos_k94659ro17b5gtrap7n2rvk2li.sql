COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                       ...
                       (SELECT estado
                     FROM dae.fuas
                     WHERE id_alumno=a.id and ano=2022
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2022 AND regimen='' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2022  
                FROM alumnos AS a
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[ií]l[aá]b[aá]c[aá]' OR  a.rut ~* '[ií]l[aá]b[aá]c[aá]' OR  lower(a.email) ~* '[ií]l[aá]b[aá]c[aá]' OR  text(a.id) ~* '[ií]l[aá]b[aá]c[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'c[oó]rd[oó]v[aá]' OR  a.rut ~* 'c[oó]rd[oó]v[aá]' OR  lower(a.email) ~* 'c[oó]rd[oó]v[aá]' OR  text(a.id) ~* 'c[oó]rd[oó]v[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]nh[ií]sl[aá]v' OR  a.rut ~* 'y[aá]nh[ií]sl[aá]v' OR  lower(a.email) ~* 'y[aá]nh[ií]sl[aá]v' OR  text(a.id) ~* 'y[aá]nh[ií]sl[aá]v' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]nth[oó]ny' OR  a.rut ~* '[aá]nth[oó]ny' OR  lower(a.email) ~* '[aá]nth[oó]ny' OR  text(a.id) ~* '[aá]nth[oó]ny' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
