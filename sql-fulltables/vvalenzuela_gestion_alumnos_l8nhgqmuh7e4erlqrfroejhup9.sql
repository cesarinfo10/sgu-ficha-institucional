COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'c[eé]c[ií]l[ií][aá]' OR  a.rut ~* 'c[eé]c[ií]l[ií][aá]' OR  lower(a.email) ~* 'c[eé]c[ií]l[ií][aá]' OR  text(a.id) ~* 'c[eé]c[ií]l[ií][aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[oó]rt[eé]g[aá]' OR  a.rut ~* '[oó]rt[eé]g[aá]' OR  lower(a.email) ~* '[oó]rt[eé]g[aá]' OR  text(a.id) ~* '[oó]rt[eé]g[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
