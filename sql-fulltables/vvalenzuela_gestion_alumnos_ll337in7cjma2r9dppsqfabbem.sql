COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]ls[aá]' OR  a.rut ~* '[eé]ls[aá]' OR  lower(a.email) ~* '[eé]ls[aá]' OR  text(a.id) ~* '[eé]ls[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
