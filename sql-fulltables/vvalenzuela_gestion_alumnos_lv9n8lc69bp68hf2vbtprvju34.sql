COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][aá]ndr[aá]' OR  a.rut ~* 'l[eé][aá]ndr[aá]' OR  lower(a.email) ~* 'l[eé][aá]ndr[aá]' OR  text(a.id) ~* 'l[eé][aá]ndr[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17,153) 
