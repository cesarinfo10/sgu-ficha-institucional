COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú][ií]d[oó]' OR  a.rut ~* 'g[uú][ií]d[oó]' OR  lower(a.email) ~* 'g[uú][ií]d[oó]' OR  text(a.id) ~* 'g[uú][ií]d[oó]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
