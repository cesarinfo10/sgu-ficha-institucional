COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '17579986-9' OR  a.rut ~* '17579986-9' OR  lower(a.email) ~* '17579986-9' OR  text(a.id) ~* '17579986-9' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
