COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'y[aá]n[eé]t' OR  a.rut ~* 'y[aá]n[eé]t' OR  lower(a.email) ~* 'y[aá]n[eé]t' OR  text(a.id) ~* 'y[aá]n[eé]t' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
