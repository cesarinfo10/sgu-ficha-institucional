COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[uú]ñ[eé]z' OR  a.rut ~* 'n[uú]ñ[eé]z' OR  lower(a.email) ~* 'n[uú]ñ[eé]z' OR  text(a.id) ~* 'n[uú]ñ[eé]z' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]ll[aá]l[oó]b[oó]s' OR  a.rut ~* 'v[ií]ll[aá]l[oó]b[oó]s' OR  lower(a.email) ~* 'v[ií]ll[aá]l[oó]b[oó]s' OR  text(a.id) ~* 'v[ií]ll[aá]l[oó]b[oó]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* '[aá]n[aá]c[aá]r[eé]n' OR  a.rut ~* '[aá]n[aá]c[aá]r[eé]n' OR  lower(a.email) ~* '[aá]n[aá]c[aá]r[eé]n' OR  text(a.id) ~* '[aá]n[aá]c[aá]r[eé]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'm[aá]rg[aá]r[ií]t[aá]' OR  a.rut ~* 'm[aá]rg[aá]r[ií]t[aá]' OR  lower(a.email) ~* 'm[aá]rg[aá]r[ií]t[aá]' OR  text(a.id) ~* 'm[aá]rg[aá]r[ií]t[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
