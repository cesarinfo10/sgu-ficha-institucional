COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'j[uú][aá]n' OR  a.rut ~* 'j[uú][aá]n' OR  lower(a.email) ~* 'j[uú][aá]n' OR  text(a.id) ~* 'j[uú][aá]n' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'fr[eé][ií]r[eé]' OR  a.rut ~* 'fr[eé][ií]r[eé]' OR  lower(a.email) ~* 'fr[eé][ií]r[eé]' OR  text(a.id) ~* 'fr[eé][ií]r[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
