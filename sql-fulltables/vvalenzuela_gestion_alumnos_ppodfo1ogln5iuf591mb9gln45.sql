COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[aá]v[ií]d' OR  a.rut ~* 'd[aá]v[ií]d' OR  lower(a.email) ~* 'd[aá]v[ií]d' OR  text(a.id) ~* 'd[aá]v[ií]d' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'l[eé][ií]v[aá]' OR  a.rut ~* 'l[eé][ií]v[aá]' OR  lower(a.email) ~* 'l[eé][ií]v[aá]' OR  text(a.id) ~* 'l[eé][ií]v[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
