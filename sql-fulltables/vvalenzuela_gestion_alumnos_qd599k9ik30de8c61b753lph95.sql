COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2024 AND regimen='PRE-D' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2024   
                FROM alumnos AS a
                ...
                WHERE true  AND (a.cohorte = '2024' ) AND (c.regimen = 'PRE-D') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1))  AND a.carrera_actual IN (108,96,25,109,37,70,135,153,112,17) 
