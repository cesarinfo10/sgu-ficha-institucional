COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'l[uú]v[eé]cc[eé]' OR  a.rut ~* 'l[uú]v[eé]cc[eé]' OR  lower(a.email) ~* 'l[uú]v[eé]cc[eé]' OR  text(a.id) ~* 'l[uú]v[eé]cc[eé]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
