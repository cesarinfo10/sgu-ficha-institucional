COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'w[ií]ll[ií][aá]m' OR  a.rut ~* 'w[ií]ll[ií][aá]m' OR  lower(a.email) ~* 'w[ií]ll[ií][aá]m' OR  text(a.id) ~* 'w[ií]ll[ií][aá]m' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
