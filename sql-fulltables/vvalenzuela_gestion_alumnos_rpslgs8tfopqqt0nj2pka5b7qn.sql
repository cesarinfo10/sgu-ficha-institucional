COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '[eé]mm[aá]n[uú][eé]l' OR  a.rut ~* '[eé]mm[aá]n[uú][eé]l' OR  lower(a.email) ~* '[eé]mm[aá]n[uú][eé]l' OR  text(a.id) ~* '[eé]mm[aá]n[uú][eé]l' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
