COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[ií]c[oó]l[aá]s' OR  a.rut ~* 'n[ií]c[oó]l[aá]s' OR  lower(a.email) ~* 'n[ií]c[oó]l[aá]s' OR  text(a.id) ~* 'n[ií]c[oó]l[aá]s' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
