COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'v[ií]v[ií][aá]n[aá]' OR  a.rut ~* 'v[ií]v[ií][aá]n[aá]' OR  lower(a.email) ~* 'v[ií]v[ií][aá]n[aá]' OR  text(a.id) ~* 'v[ií]v[ií][aá]n[aá]' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú][eé]rr[eé]r[oó]' OR  a.rut ~* 'g[uú][eé]rr[eé]r[oó]' OR  lower(a.email) ~* 'g[uú][eé]rr[eé]r[oó]' OR  text(a.id) ~* 'g[uú][eé]rr[eé]r[oó]' )  AND a.carrera_actual IN (108,96,25,109,37,135,70,112,17) 
