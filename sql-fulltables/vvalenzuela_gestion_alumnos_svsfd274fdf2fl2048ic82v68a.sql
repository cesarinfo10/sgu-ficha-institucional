COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'n[aá]t[aá]l[ií][aá]' OR  a.rut ~* 'n[aá]t[aá]l[ií][aá]' OR  lower(a.email) ~* 'n[aá]t[aá]l[ií][aá]' OR  text(a.id) ~* 'n[aá]t[aá]l[ií][aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
