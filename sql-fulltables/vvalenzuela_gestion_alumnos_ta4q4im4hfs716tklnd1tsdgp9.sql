COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true  AND (a.estado = '20') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2023 AND semestre=2))  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
