COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 's[aá][ií]nt[eé]l[uú]s' OR  a.rut ~* 's[aá][ií]nt[eé]l[uú]s' OR  lower(a.email) ~* 's[aá][ií]nt[eé]l[uú]s' OR  text(a.id) ~* 's[aá][ií]nt[eé]l[uú]s' ) AND (lower(a.nombres||' '||a.apellidos) ~* 'ph[aá]r[aá]' OR  a.rut ~* 'ph[aá]r[aá]' OR  lower(a.email) ~* 'ph[aá]r[aá]' OR  text(a.id) ~* 'ph[aá]r[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,112,17) 
