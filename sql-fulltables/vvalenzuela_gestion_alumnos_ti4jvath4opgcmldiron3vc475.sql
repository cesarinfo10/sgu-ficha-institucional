COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'g[uú][ií]ll[eé]rm[ií]n[aá]' OR  a.rut ~* 'g[uú][ií]ll[eé]rm[ií]n[aá]' OR  lower(a.email) ~* 'g[uú][ií]ll[eé]rm[ií]n[aá]' OR  text(a.id) ~* 'g[uú][ií]ll[eé]rm[ií]n[aá]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
