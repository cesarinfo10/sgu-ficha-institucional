COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'd[uú]rr[uú]ty' OR  a.rut ~* 'd[uú]rr[uú]ty' OR  lower(a.email) ~* 'd[uú]rr[uú]ty' OR  text(a.id) ~* 'd[uú]rr[uú]ty' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
