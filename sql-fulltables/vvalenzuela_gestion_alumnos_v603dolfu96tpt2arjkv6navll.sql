COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                ...
                WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* 'm[eé]n[aá]nt[aá][uú]' OR  a.rut ~* 'm[eé]n[aá]nt[aá][uú]' OR  lower(a.email) ~* 'm[eé]n[aá]nt[aá][uú]' OR  text(a.id) ~* 'm[eé]n[aá]nt[aá][uú]' )  AND a.carrera_actual IN (108,96,25,109,37,70,135,112,17) 
