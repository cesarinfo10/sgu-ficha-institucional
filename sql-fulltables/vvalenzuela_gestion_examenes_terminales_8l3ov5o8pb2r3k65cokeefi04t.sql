COPY (SELECT et.id,to_char(fecha_reg,'dd-tmMon-YYYY HH24:MI') AS fecha_reg,
						 ...
						 (SELECT char_comma_sum(upper(a.apellidos)||' '||initcap(a.nombres)||' '||CASE WHEN (SELECT count(id) FROM finanzas.cobros WHERE id_alumno=a.id AND id_glosa IN (47,5,9) AND pagado) >= 1 THEN '✅' ELSE '⛔' END) 
				  ...
				  WHERE true  AND (date_part('year',et.fecha_examen) = 2023) AND (et.id_escuela = 11)  AND carrera_actual IN (108,96,25,109,37,70,112,17)  			  
				  ORDER BY et.fecha_examen DESC ) to stdout WITH CSV HEADER