COPY (SELECT trim(pap.rut) AS rut,vp.id,vp.nombre,to_char(vp.fecha_post,'DD-MM-YYYY') AS fecha_post,
                    ...
                    WHERE (lower(pap.nombres||' '||pap.apellidos) ~* 'r[oó]s[ií]t[aá]' OR  pap.rut ~* 'r[oó]s[ií]t[aá]' OR  text(pap.id) ~* 'r[oó]s[ií]t[aá]' ) AND (lower(pap.nombres||' '||pap.apellidos) ~* 's[aá]gr[eé]d[oó]' OR  pap.rut ~* 's[aá]gr[eé]d[oó]' OR  text(pap.id) ~* 's[aá]gr[eé]d[oó]' ) 
