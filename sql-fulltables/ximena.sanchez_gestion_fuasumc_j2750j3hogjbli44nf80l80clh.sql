COPY (SELECT fuas.id,fuas.ano,a.id AS id_alumno,a.rut,a.nombres,apellidos,c.alias||'-'||a.jornada AS carrera,
             ...
             WHERE true   AND (lower(a.nombres||' '||a.apellidos) ~* '16805872-1' OR  a.rut ~* '16805872-1' OR  text(a.id) ~* '16805872-1' ) AND (lower(a.nombres||' '||a.apellidos) ~* '	' OR  a.rut ~* '	' OR  text(a.id) ~* '	' ) 
