COPY (SELECT a.id,trim(a.rut) AS rut,upper(a.apellidos)||' '||initcap(a.nombres) AS nombre,a.genero,a.fec_nac,c.alias||'-'||a.jornada AS carrera,c.regimen,
                     ...
                     LIMIT 1) AS post_becaumc,
                       ...
                       CASE WHEN (SELECT 1 FROM alumnos_sies WHERE ano=2024 AND regimen='PRE' AND rut=a.rut)=1 THEN 'Si' ELSE 'No' END AS al_sies_2024   ,coalesce(vtr.dias,0) AS cant_dias_accede_edif
                FROM alumnos AS a
                ...
                LEFT JOIN (SELECT rut,count(rut) AS dias FROM (SELECT rut FROM vista_torniquete_rut WHERE fecha_reg::date BETWEEN '2024-03-18'::date AND '2024-07-20'::date GROUP BY rut,fecha_reg::date) AS foo GROUP BY rut) AS vtr ON vtr.rut=split_part(a.rut,'-',1)
                WHERE true  AND (c.regimen = 'PRE') AND (a.id IN (SELECT id_alumno FROM matriculas WHERE ano=2024 AND semestre=1)) 
