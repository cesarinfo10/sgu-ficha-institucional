...
if ($puedeEliminar <> "" ) {
	...
	$SQL_borrar = "
	delete from asist_cursos where id_curso = $id_curso;
	delete from asist_profesores where id_curso = $id_curso;
	delete from ca_temp where id_c = $id_curso;
	delete from ca_temporal where id_curso = $id_curso;
	delete from calendarizaciones where id_curso = $id_curso;
	delete from cargas_academicas where id_curso = $id_curso;
	delete from cursos_botados where id_curso = $id_curso;
	delete from cursos_eliminar where id_curso = $id_curso;
	delete from cursos_horarios where id_curso = $id_curso;
	delete from cursos_pruebas where id_curso = $id_curso;
	delete from cursos_sesiones where id_curso = $id_curso;
	delete from dctos_cursos where id_curso = $id_curso;
	delete from encuestas.estudiantil where id_curso = $id_curso;
	delete from exam_cse_1_2007 where id_curso = $id_curso;
	delete from horarios_cursos where id_curso = $id_curso;
	delete from inscripciones_cursos where id_curso = $id_curso;
...
$SQL_curso = "SELECT vc.id,vc.cod_asignatura||'-'||vc.seccion||' '||vc.asignatura AS asignatura,vc.semestre as sem_num,
                     ...
                     coalesce(to_char(c.fec_sol_recup,'tmDy FMDD-tmMon-YY'),'#N/D') AS fec_sol_recup,tipo_clase,
...
<table cellpadding="4" cellspacing="0" border="0" class="tabla" style='margin-top: 5px'>
  <tr>
    <td class="celdaFiltro" style="vertical-align: middle;">
      ...
      <?php
			if ($_SESSION['tipo'] <= 2) {
				echo("<a href='$enlbase_sm=editar_curso&id_curso=$id_curso' id='sgu_fancybox_medium' class='boton'>Editar</a> ");
			} else { 
			...
			if ($cant_alumnos==0) {
				...
				$SQL = "
				select count(*) as cuenta from permisos_apps
				where id_usuario = {$_SESSION['id_usuario']}
				and id_aplicacion = (select id from aplicaciones where ejecutable = 'eliminar_curso')";				
				//echo("<br>$SQL");
				$fPending = consulta_sql($SQL);	
				$puedeEliminar = $fPending[0]['cuenta'];
				if ($puedeEliminar > 0) {
					echo("<a href='javascript:fEliminarCurso($id_curso)' id='eliminarCurso' class='boton'>Eliminar</a> ");
				}
			}			
...
<table cellpadding="2" cellspacing="0" border="0" class="tabla" style='margin-top: 5px'>
  ...
  -->
    <td class="celdaFiltro">
      <b>Imprimir:</b><br>
      <?php echo("<a href='pruebas_solemnes_tapa.php?id_curso=$id_curso&prueba=s1&token=$cod' class='boton'>Solemne I </a> "); ?>
      <?php echo("<a href='pruebas_solemnes_tapa.php?id_curso=$id_curso&prueba=s2&token=$cod' class='boton'>Solemne II</a> "); ?>
...
//	if (r == true) {
		  //var pSaltar = "/sgu/principal.php?modulo=ver_curso&id_curso="+id_curso+"&puedeEliminar=SI";
		  var pSaltar = "/sgu/principal.php?modulo=eliminar_curso&id_curso="+id_curso;
